...
class StatsTracker {
 public:
  ...
  void PrintResults(const char* testName) {
    ...
    //            std::setiosflags(std::ios::left); std::cout << std::setw(12)
    //            << " ";
    //            //std::cout << " ";
    //            std::cout << std::resetiosflags(std::ios::left) <<
    //            std::setiosflags(std::ios::right); std::cout << std::setw(10)
    //            << "Avg";
    // #ifndef BRIEF_MODE
    ...
    //            std::cout << std::endl;
    // #endif
    // PrintResultsHelper(std::cout, "OS Real", 12, GetAverage(realtimes),
    // GetLow(realtimes), GetHigh(realtimes));
    PrintResultsHelper(std::cout, testName, 40, GetAverage(kerneltimes),
                       GetLow(kerneltimes), GetHigh(kerneltimes), true);
    std::cout << "-------------------------------------------------------------"
                 "--------------------\n";
    // PrintResultsHelper(std::cout, "Soar Total", 12, GetAverage(totaltimes),
    // GetLow(totaltimes), GetHigh(totaltimes));

    time_t t = time(0);
    struct tm* now = localtime(&t);
    std::ostringstream now_string;
    now_string << (now->tm_year + 1900) << '-' << (now->tm_mon + 1) << '-'
               << now->tm_mday << " " << now->tm_hour << ":" << now->tm_min
               << ":" << now->tm_sec;
    summary_string << std::resetiosflags(std::ios::right)
                   << std::setiosflags(std::ios::left);
    summary_string << std::setw(20) << now_string.str();
    summary_string << std::resetiosflags(std::ios::left);
    PrintResultsHelper(summary_string, testName, 40, GetAverage(kerneltimes),
                       GetLow(kerneltimes), GetHigh(kerneltimes));

    std::ofstream resultFile("PerformanceTestResults.txt",
  ...
  void PrintResultsHelper(std::ostream& outStream, std::string label,
                          int label_width, double avg, double low, double high,
                          bool useColors = false) {
    outStream.precision(4);
    if (useColors) outStream << "\033[1;34m";
    outStream << std::resetiosflags(std::ios::right)
              << std::setiosflags(std::ios::left);
    ...
    outStream << std::setw(10) << std::setiosflags(std::ios::fixed)
              << std::setprecision(4) << avg;
#ifndef BRIEF_MODE
    outStream << std::setw(10) << std::setiosflags(std::ios::fixed)
              << std::setprecision(4) << low;
    outStream << std::setw(10) << std::setiosflags(std::ios::fixed)
              << std::setprecision(4) << high;
#endif
    outStream << std::setw(9) << std::setiosflags(std::ios::fixed)
              << std::setprecision(1)
              << (high ? ((high - low) / high) * 100 : 0) << "%";
    if (useColors) outStream << "\033[0;37m";
    outStream << std::endl;
  }
};
