...
void MyPrintEventHandler(smlPrintEventId id, void* pUserData, Agent* pAgent,
                         char const* pMessage) {
  std::cout << pMessage << std::endl;
}

void Run_PerformanceTest(int numTrials, int numDecisions, StatsTracker* pSt,
                         const std::vector<std::string>& commands, int numInits,
                         const std::vector<std::string>& init_commands) {
  for (int i = 0; i < numTrials; i++) {
    ...
    for (int pass = 0; pass <= numInits; ++pass) {
      ...
      }
      {
        ...
        pSt->realtimes.push_back(
            response.GetArgFloat(sml_Names::kParamRealSeconds, 0.0));

        agent->ExecuteCommandLineXML("stats", &response);
        pSt->kerneltimes.push_back(
            response.GetArgFloat(sml_Names::kParamStatsKernelCPUTime, 0.0));
        pSt->totaltimes.push_back(
            response.GetArgFloat(sml_Names::kParamStatsTotalCPUTime, 0.0));
      }
    }
    kernel->Shutdown();
    delete kernel;
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 1) {
    ...
    std::cout << "Usage: " << argv[0]
              << " [default | <agent name>] [<numtrials>] [<num_decisions>] "
                 "[<num_init_and_rerun>]"
              << std::endl;
    return 1;
  }
  ...
  std::cout << "\033[1;31m" << agentname << "\033[0;37m"
            << ": ";
  if (numTrials > 1)
    std::cout << numTrials << " trials";
  else
    std::cout << "single run";
  if (numDCs > 0)
    std::cout << ", " << numDCs << " DCs";
  else
    std::cout << ", run forever";
  if (numInits > 0)
    std::cout << ", " << numInits << " extra init-soar/runs\n";
  else
    std::cout << std::endl;
  std::cout.flush();

  {
    ...
    Run_PerformanceTest(numTrials, numDCs, &l_testStats, commands, numInits,
                        init_commands);
