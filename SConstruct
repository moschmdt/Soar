...
print(
    f"Building Soar{SOAR_VERSION}              * will be built if no target specified"
)
print("Targets available:")
print("   Core:              kernel* cli* scripts*")
print("   Testing:           performance_tests tests")
print(
    f"   SWIG:              {SML_PYTHON_ALIAS} {SML_TCL_ALIAS} {SML_JAVA_ALIAS}* {SML_CSHARP_ALIAS}"
)
print(
    f"   Extras:            debugger* headers* {COMPILE_DB_ALIAS}* tclsoarlib {MSVS_ALIAS} list"
)
print(
...
def vc_version():
    try:
        ...
        print(
            "make sure Microsoft Visual C++ is installed and you are using the Visual Studio Command Prompt"
        )
        Exit(1)
...
AddOption(
    ...
    dest="defflags",
    default=True,
    help="Don't pass any default flags to the compiler or linker",
)
AddOption(
    "--no-scu",
    action="store_false",
...
AddOption(
    ...
    dest="no_scu_kernel",
    default=False,
    help="Never build kernel in a single compilation unit.",
)
AddOption(
    "--no-scu-cli",
    action="store_true",
    dest="no_scu_cli",
    default=False,
    help="Never build CLI in a single compilation unit.",
)
AddOption(
    "--scu",
    action="store_true",
...
AddOption(
    ...
    nargs=1,
    help="Python executable; defaults to same executable used to run SCons",
)
AddOption(
    "--tcl",
    action="store",
    ...
    dest="tcl",
    nargs=1,
    help="Path to Tcl installation (ActiveTcl or otherwise)",
)
AddOption(
    "--tcl-suffix",
    action="store",
    ...
    default="t",
    nargs=1,
    help='Tcl binary suffix (defaults to "t", which is used to indicate full threading support in the standard Tcl build)',
)
AddOption(
    "--static",
    action="store_true",
...
AddOption(
    ...
    dest="dbg",
    default=False,
    help="Enable debug build.  Disables compiler optimizations, includes debugging symbols, debug trace statements and assertions",
)
AddOption(
    "--opt",
    action="store_false",
    dest="dbg",
    default=False,
    help="Enable optimized build.  Enables compiler optimizations, removes debugging symbols, debug trace statements and assertions",
)
AddOption(
    "--verbose",
    action="store_true",
    dest="verbose",
    default=False,
    help="Output full compiler commands",
)
AddOption(
    "--no-svs",
    action="store_true",
    dest="nosvs",
    default=False,
    help="Build Soar without SVS functionality",
)
...
env = Environment(
    ...
    # build without it.
    TCL_REQUIRED="tclsoarlib" in COMMAND_LINE_TARGETS
    or "sml_tcl" in COMMAND_LINE_TARGETS,
...
if compiler == "g++":
    ...
    cflags.extend(
        ["/EHsc", "/D", "_CRT_SECURE_NO_DEPRECATE", "/D", "_WIN32", "/bigobj"]
    )
    if GetOption("nosvs"):
...
env.Replace(
    ...
    CPPPATH=[
        ...
        "#Core/SoarKernel/src/operator_explanation",
...
if sys.platform == "win32":
    ...
    sys_lib_path = list(
        filter(None, os.environ.get("DYLD_LIBRARY_PATH", "").split(":"))
    )
...
if sys.platform == "darwin" or os.name == "nt":
    ...
    py_sources += [env.Install(py_lib_namespace, soarlib)]

if sys.platform == "darwin":
    # For MacOS, also add to the out/ directory,
    # so the linker can pick up on it properly.
    py_extra += [env.Install(env["OUT_DIR"], soarlib)]
