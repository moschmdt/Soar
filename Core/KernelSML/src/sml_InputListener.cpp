...
void InputListener::OnKernelEvent(int eventID, AgentSML* pAgentSML,
                                  void* pCallData) {
  (void)eventID;
...
void InputListener::ProcessPendingInput(AgentSML* pAgentSML, int) {
  ...
  for (PendingInputListIter iter = pPending->begin(); iter != pPending->end();
       iter = pPending->erase(iter)) {
    ...
    if (kDebugInput) {
      PrintDebugFormat("--------- %s starting input ----------",
                       pAgentSML->GetName());
    }
    ...
    for (int i = 0; i < nChildren; i++) {
      ...
      if (add) {
        char const* pID = pWmeXML->GetAttribute(
            sml_Names::kWME_Id);  // May be a client side id value (e.g. "o3"
                                  // not "O3")
        char const* pAttribute =
            pWmeXML->GetAttribute(sml_Names::kWME_Attribute);
        ...
        char const* pTimeTag = pWmeXML->GetAttribute(
            sml_Names::kWME_TimeTag);  // May be a client side time tag (e.g. -3
                                       // not +3)
        ...
        if (kDebugInput) {
          PrintDebugFormat("%s Add %s ^%s %s (type %s tag %s)",
                           pAgentSML->GetName(), pID, pAttribute, pValue, pType,
                           pTimeTag);
        }

        // Add the wme
        ok = pAgentSML->AddInputWME(pID, pAttribute, pValue, pType, pTimeTag) &&
             ok;
      } else if (remove) {
        char const* pTimeTag = pWmeXML->GetAttribute(
            sml_Names::kWME_TimeTag);  // May be (will be?) a client side time
                                       // tag (e.g. -3 not +3)
  ...
  std::list<DirectInputDelta>* pBufferedDirect =
      pAgentSML->GetBufferedDirectList();
  for (std::list<DirectInputDelta>::iterator iter = pBufferedDirect->begin();
       iter != pBufferedDirect->end(); iter = pBufferedDirect->erase(iter)) {
    ...
    switch (delta.type) {
      ...
      case DirectInputDelta::kAddString:
        pAgentSML->AddStringInputWME(delta.id.c_str(), delta.attribute.c_str(),
                                     delta.svalue.c_str(), delta.clientTimeTag);
        break;
      case DirectInputDelta::kAddInt:
        pAgentSML->AddIntInputWME(delta.id.c_str(), delta.attribute.c_str(),
                                  delta.ivalue, delta.clientTimeTag);
        break;
      case DirectInputDelta::kAddDouble:
        pAgentSML->AddDoubleInputWME(delta.id.c_str(), delta.attribute.c_str(),
                                     delta.dvalue, delta.clientTimeTag);
        break;
      case DirectInputDelta::kAddId:
        pAgentSML->AddIdInputWME(delta.id.c_str(), delta.attribute.c_str(),
                                 delta.svalue.c_str(), delta.clientTimeTag);
        break;
      default:
        assert(false);
...
void InputListener::RegisterForKernelSMLEvents() {
  // Listen for input callback events so we can submit pending input to the
  // kernel
