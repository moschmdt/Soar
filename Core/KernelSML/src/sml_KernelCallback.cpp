...
void KernelCallback::KernelCallbackStatic(agent* pAgent, int eventID,
...
// Not returning this as SOAR_CALLBACK_TYPE as that would expose the kernel
// headers through our headers. Want to keep kernel types internal to the
// implementation files so just using an int.
int KernelCallback::InternalGetCallbackFromEventID(int eventID) {
  switch (eventID) {
    ...
    case smlEVENT_BEFORE_PRODUCTION_REMOVED:
      return PRODUCTION_JUST_ABOUT_TO_BE_EXCISED_CALLBACK;
    ...
    case smlEVENT_BEFORE_SMALLEST_STEP:
      return BEFORE_ELABORATION_CALLBACK;  // Elaboration cycle is the smallest
                                           // step
    case smlEVENT_AFTER_SMALLEST_STEP:
      return AFTER_ELABORATION_CALLBACK;  // Elaboration cycle is the smallest
                                          // step
    case smlEVENT_BEFORE_ELABORATION_CYCLE:
      return BEFORE_ELABORATION_CALLBACK;
    case smlEVENT_AFTER_ELABORATION_CYCLE:
      return AFTER_ELABORATION_CALLBACK;
    case smlEVENT_BEFORE_INPUT_PHASE:
      return BEFORE_INPUT_PHASE_CALLBACK;
    case smlEVENT_AFTER_INPUT_PHASE:
      return AFTER_INPUT_PHASE_CALLBACK;
    case smlEVENT_BEFORE_PROPOSE_PHASE:
    ...
    case smlEVENT_AFTER_APPLY_PHASE:
      return AFTER_APPLY_PHASE_CALLBACK;
    case smlEVENT_BEFORE_OUTPUT_PHASE:
      return BEFORE_OUTPUT_PHASE_CALLBACK;
    case smlEVENT_AFTER_OUTPUT_PHASE:
      return AFTER_OUTPUT_PHASE_CALLBACK;
    case smlEVENT_BEFORE_PREFERENCE_PHASE:
      return BEFORE_PREFERENCE_PHASE_CALLBACK;
    case smlEVENT_AFTER_PREFERENCE_PHASE:
      return AFTER_PREFERENCE_PHASE_CALLBACK;
    case smlEVENT_BEFORE_WM_PHASE:
    ...
    case smlEVENT_AFTER_INTERRUPT:
      return AFTER_INTERRUPT_CALLBACK;  // Implemented in SML
    case smlEVENT_AFTER_HALTED:
      return AFTER_HALTED_CALLBACK;  // Implemented in SML
    case smlEVENT_BEFORE_RUN_STARTS:
      return BEFORE_RUN_STARTS_CALLBACK;  // Implemented in SML
    case smlEVENT_AFTER_RUN_ENDS:
      return AFTER_RUN_ENDS_CALLBACK;  // Implemented in SML
...
bool KernelCallback::IsCallbackImplementedInKernel(int eventID) {
  ...
  if (eventID == smlEVENT_BEFORE_PHASE_EXECUTED ||
      eventID == smlEVENT_AFTER_PHASE_EXECUTED) {
    return true;
  }
...
KernelCallback::~KernelCallback() { ClearKernelCallback(); }

void KernelCallback::ClearKernelCallback() {
  for (std::map<int, bool>::iterator mapIter = m_Registered.begin();
       mapIter != m_Registered.end(); mapIter++) {
...
void KernelCallback::RegisterWithKernel(int eventID) {
  ...
  if (eventID == smlEVENT_OUTPUT_PHASE_CALLBACK) {
    add_output_function(pAgent, KernelCallbackStatic, this, NULL,
                        smlEVENT_OUTPUT_PHASE_CALLBACK, "output-link");
  } else if (eventID != smlEVENT_BEFORE_PHASE_EXECUTED &&
             eventID != smlEVENT_AFTER_PHASE_EXECUTED) {
    // This is the standard case
    SOAR_CALLBACK_TYPE callbackType =
        SOAR_CALLBACK_TYPE(GetCallbackFromEventID(eventID));
    soar_add_callback(pAgent, callbackType, KernelCallbackStatic, eventID, this,
                      NULL, callbackID.c_str());
  } else {
    // BEFORE_PHASE and AFTER_PHASE are implemented by listening for all lower
    // level phase events
    int beforePhaseEvents[] = {
        smlEVENT_BEFORE_INPUT_PHASE,  smlEVENT_BEFORE_PROPOSE_PHASE,
        smlEVENT_BEFORE_DECIDE_PHASE, smlEVENT_BEFORE_APPLY_PHASE,
        smlEVENT_BEFORE_OUTPUT_PHASE, smlEVENT_BEFORE_PREFERENCE_PHASE,
        smlEVENT_BEFORE_WM_PHASE};
    int afterPhaseEvents[] = {
        smlEVENT_AFTER_INPUT_PHASE,  smlEVENT_AFTER_PROPOSE_PHASE,
        smlEVENT_AFTER_DECIDE_PHASE, smlEVENT_AFTER_APPLY_PHASE,
        smlEVENT_AFTER_OUTPUT_PHASE, smlEVENT_AFTER_PREFERENCE_PHASE,
        smlEVENT_AFTER_WM_PHASE};

    int* events = (eventID == smlEVENT_BEFORE_PHASE_EXECUTED)
                      ? beforePhaseEvents
                      : afterPhaseEvents;

    for (int i = 0; i < 7; i++) {
      // Note that we register with the kernel for a specific phase event, while
      // passing in the more general BEFORE_PHASE_EXECUTED or
      // AFTER_PHASE_EXECUTED as the eventID so the event comes back to us with
      // that label allowing us to pass it directly up to the caller who is
      // expecting an event of that type, not a subevent.
      int phaseEvent = events[i];
      SOAR_CALLBACK_TYPE callbackType =
          SOAR_CALLBACK_TYPE(GetCallbackFromEventID(phaseEvent));
      soar_add_callback(pAgent, callbackType, KernelCallbackStatic, eventID,
                        this, NULL, callbackID.c_str());
    }
  }
}
