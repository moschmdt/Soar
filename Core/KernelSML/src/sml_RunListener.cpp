...
bool RunListener::RemoveListener(smlRunEventId eventID,
...
void RunListener::OnKernelEvent(int eventID, AgentSML* pAgentSML,
                                void* pCallData) {
  // Get the first listener for this event (or return if there are none)
  ConnectionListIter connectionIter;
  if (!EventManager<smlRunEventId>::GetBegin(smlRunEventId(eventID),
                                             &connectionIter)) {
    return;
  }
  ...
  pConnection->AddParameterToSMLCommand(pMsg, sml_Names::kParamPhase,
                                        to_c_string(phase, buf));

  // Send the message out
  AnalyzeXML response;
  SendEvent(pAgentSML, pConnection, pMsg, &response, connectionIter,
            GetEnd(smlRunEventId(eventID)));
