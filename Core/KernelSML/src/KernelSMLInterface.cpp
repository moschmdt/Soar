...
// file).
//
// KernelSML receives commands in SML (a dialect of XML), sends them to the Soar
// kernel and then returns the results in SML.
//
...
soarxml::ElementXML* ReceivedCall(Connection* pConnection,
...
static EmbeddedConnection* GetConnectionFromHandle(
    Connection_Receiver_Handle hConnection) {
  return reinterpret_cast<EmbeddedConnection*>(hConnection);
}

Connection_Receiver_Handle sml_CreateEmbeddedConnection(
    Connection_Sender_Handle hSenderConnection,
    ProcessMessageFunction pProcessMessage, int connectionType,
    int portToListenOn) {
  ...
  EmbeddedConnection* pConnection =
      synch ? EmbeddedConnectionSynch::CreateEmbeddedConnectionSynch()
  ...
  // The client will also have a Connection object which will not have this flag
  // set.
  ...
  // If this is a synchronous connection then commands will execute on the
  // embedded client's thread and we don't use the receiver thread.  (Why not?
  // If we allowed it to run then we'd have to (a) sychronize execution between
  // the two threads and (b) sometimes Soar would be running in the client's
  // thread and sometimes in the receiver's thread (depending on where "run"
  // came from) and that could easily introduce a lot of complicated bugs or
  // where performance would be different depending on whether you pressed "run"
  // in the environment or "run" in a remote debugger).
  if (!pConnection->IsAsynchronous()) {
    pKernelSML->StopReceiverThread();
  }

  // Register for "calls" from the client.
  pConnection->RegisterCallback(ReceivedCall, NULL, sml_Names::kDocType_Call,
                                true);

  // The original sender is a receiver to us so we need to reverse the type.
  pConnection->AttachConnectionInternal(
      reinterpret_cast<Connection_Receiver_Handle>(hSenderConnection),
      pProcessMessage);
...
ElementXML_Handle sml_ProcessMessage(
  ...
  if (action == SML_MESSAGE_ACTION_CLOSE) {
    if (pConnection) {
      ...
      // When the embedded connection disconnects we're about to exit the
      // application so shutdown any remote connections cleanly and do any other
      // cleanup.
      KernelSML* pKernelSML =
          reinterpret_cast<KernelSML*>(pConnection->GetUserData());
      assert(pKernelSML);
      pKernelSML->Shutdown();

      // The shutdown call above will also delete our connection object as part
      // of its cleanup so set it to NULL here to make sure we don't try to use
      // it again.
      pConnection = NULL;
  ...
  if (action == SML_MESSAGE_ACTION_ASYNCH) {
    // Store the incoming message on a queue and execute it on the receiver's
    // thread (our thread) at a later point.
    EmbeddedConnectionAsynch* eca =
        static_cast<EmbeddedConnectionAsynch*>(pConnection);
    eca->AddToIncomingMessageQueue(hIncomingMsg);

    // There is no immediate response to an asynch message.
    // The response will be sent back to the caller as another asynch message
    // later, once the command has been executed.
    return NULL;
  }

  if (action == SML_MESSAGE_ACTION_TRACE_ON ||
      action == SML_MESSAGE_ACTION_TRACE_OFF) {
    // Report more details on the messages being sent and received.
    // Currently this only affects remote connections but we may extend it to
    // all connections.
    KernelSML* pKernelSML =
        reinterpret_cast<KernelSML*>(pConnection->GetUserData());
    assert(pKernelSML);
