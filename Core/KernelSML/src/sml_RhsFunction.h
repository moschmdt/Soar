...
class RhsFunction {
 ...
 public:
  static Symbol* RhsFunctionCallback(agent* thisAgent, cons* args,
                                     void* user_data);
   ...
   * Because this is a callback, the calling method will release them.  However,
   * if you clone or otherwise copy any values, you are responsible for
   * releasing the copies.
   */
...
class InterruptRhsFunction : public RhsFunction {
 public:
  ...
  bool CanBeStandAlone() const { return true; }
  bool LiteralizeArguments() const { return false; }
...
class ConcatRhsFunction : public RhsFunction {
 public:
  ...
  bool CanBeStandAlone() const { return false; }
  bool LiteralizeArguments() const { return false; }
  virtual Symbol* Execute(std::vector<Symbol*>* pArguments);
};
