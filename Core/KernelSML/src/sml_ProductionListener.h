...
class ProductionListener : public EventManager<smlProductionEventId> {
 ...
 public:
  ProductionListener() { m_pKernelSML = 0; }

  virtual ~ProductionListener() { Clear(); }
  ...
  virtual bool AddListener(smlProductionEventId eventID,
                           Connection* pConnection);

  // Returns true if at least one connection remains listening for this event
  virtual bool RemoveListener(smlProductionEventId eventID,
                              Connection* pConnection);
};
