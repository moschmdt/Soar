...
bool XMLListener::RemoveListener(smlXMLEventId eventID,
...
void XMLListener::OnKernelEvent(int eventIDIn, AgentSML* pAgentSML,
                                void* pCallDataIn) {
  ...
  // NOTE: For this trace message we require that the agent name be the first
  // param here (so we can look it up quickly) and any other changes to the
  // structure of this message should be carefully checked to make sure they
  // don't break the client side code as this path is more brittle and faster
  // than the rest of the message passing.
  ...
  // around, nothing is getting copied. The structure of the message is
