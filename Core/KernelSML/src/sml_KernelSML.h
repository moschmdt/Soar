...
typedef bool (KernelSML::*CommandFunction)(AgentSML*, char const*, Connection*,
                                           AnalyzeXML*, soarxml::ElementXML*);
...
// Map from kernel agent pointers to information we keep for SML about those
// agents.
...
class EXPORT KernelSML {
 ...
 protected:
  ...
  // Map from agent names to AgentSML objects, where we keep additional
  // information required for SML about each agent.
  AgentMap m_AgentMap;
  ...
  // Used to listen for kernel events that are kernel based (not for a specific
  // agent)
  ...
  // so this overrides any suppression setting.  (Use a different flag so it
  // can't be overridden by another call to suppress system stop).
  bool m_RequireSystemStop;

  RunScheduler* m_pRunScheduler;

  // If true, whenever a user issues a command that changes the state of the
  // kernel in some manner the command and its results are echoed to anyone
  // listening.  This is useful when two users are debugging the same kernel
  // (and should be off at other times).
  bool m_EchoCommands;
 ...
 public:
  void SetStopPoint(bool forever, smlRunStepSize runStepSize,
 ...
 public:
  ...
  bool FireRhsEvent(AgentSML* pAgentSML, smlRhsEventId eventID,
                    std::string const& functionName,
                    std::string const& arguments, std::string* pResult) {
    return m_RhsListener.ExecuteRhsCommand(pAgentSML, eventID, functionName,
                                           arguments, pResult);
  }
   ...
   *kernel (and should be off at other times).
   */
  ...
  static void Symbol2String(Symbol* pSymbol, bool refCounts,
                            std::ostringstream& buffer);
  static std::string Wme2String(wme* pWME, bool refCounts);
  static void PrintDebugWme(char const* pMsg, wme* pWME,
                            bool refCounts = false);
  static void PrintDebugSymbol(Symbol* pSymbol, bool refCounts = false);
  void DirectRun(char const* pAgentName, bool forever, int stepSize,
                 int interleaveSizeIn, uint64_t count);
 ...
 protected:
  ...
  bool InvalidArg(Connection* pConnection, soarxml::ElementXML* pResponse,
                  char const* pCommandName, char const* pErrorDescription);

  void BuildCommandMap();

  bool ProcessCommand(char const* pCommandName, Connection* pConnection,
                      AnalyzeXML* pIncoming, soarxml::ElementXML* pResponse);

  // There should always be exactly one local connection to the kernel (the
  // process that loaded us).
   ...
   * @param pError         A gSKI error object, which gSKI will fill in if there
   *are errors.
   * @returns False if we had an error and wish to generate a generic error
   *message (based on the incoming call + pError) True if the call succeeded or
   *we generated another more specific error already.
   */
  bool HandleCreateAgent(AgentSML* pAgentSML, char const* pCommandName,
                         Connection* pConnection, AnalyzeXML* pIncoming,
                         soarxml::ElementXML* pResponse);
  bool HandleGetInputLink(AgentSML* pAgentSML, char const* pCommandName,
                          Connection* pConnection, AnalyzeXML* pIncoming,
                          soarxml::ElementXML* pResponse);
  bool HandleInput(AgentSML* pAgentSML, char const* pCommandName,
                   Connection* pConnection, AnalyzeXML* pIncoming,
                   soarxml::ElementXML* pResponse);
  bool HandleCommandLine(AgentSML* pAgentSML, char const* pCommandName,
                         Connection* pConnection, AnalyzeXML* pIncoming,
                         soarxml::ElementXML* pResponse);
  bool HandleCheckForIncomingCommands(AgentSML* pAgentSML,
                                      char const* pCommandName,
                                      Connection* pConnection,
                                      AnalyzeXML* pIncoming,
                                      soarxml::ElementXML* pResponse);
  bool HandleDestroyAgent(AgentSML* pAgentSML, char const* pCommandName,
                          Connection* pConnection, AnalyzeXML* pIncoming,
                          soarxml::ElementXML* pResponse);
  bool HandleGetAgentList(AgentSML* pAgentSML, char const* pCommandName,
                          Connection* pConnection, AnalyzeXML* pIncoming,
                          soarxml::ElementXML* pResponse);
  bool HandleSetInterruptCheckRate(AgentSML* pAgentSML,
                                   char const* pCommandName,
                                   Connection* pConnection,
                                   AnalyzeXML* pIncoming,
                                   soarxml::ElementXML* pResponse);
  bool HandleFireEvent(AgentSML* pAgentSML, char const* pCommandName,
                       Connection* pConnection, AnalyzeXML* pIncoming,
                       soarxml::ElementXML* pResponse);
  bool HandleSuppressEvent(AgentSML* pAgentSML, char const* pCommandName,
                           Connection* pConnection, AnalyzeXML* pIncoming,
                           soarxml::ElementXML* pResponse);
  bool HandleGetVersion(AgentSML* pAgentSML, char const* pCommandName,
                        Connection* pConnection, AnalyzeXML* pIncoming,
                        soarxml::ElementXML* pResponse);
  bool HandleShutdown(AgentSML* pAgentSML, char const* pCommandName,
                      Connection* pConnection, AnalyzeXML* pIncoming,
                      soarxml::ElementXML* pResponse);
  bool HandleIsSoarRunning(AgentSML* pAgentSML, char const* pCommandName,
                           Connection* pConnection, AnalyzeXML* pIncoming,
                           soarxml::ElementXML* pResponse);
  bool HandleSetConnectionInfo(AgentSML* pAgentSML, char const* pCommandName,
                               Connection* pConnection, AnalyzeXML* pIncoming,
                               soarxml::ElementXML* pResponse);
  bool HandleGetConnections(AgentSML* pAgentSML, char const* pCommandName,
                            Connection* pConnection, AnalyzeXML* pIncoming,
                            soarxml::ElementXML* pResponse);
  bool HandleGetAllInput(AgentSML* pAgentSML, char const* pCommandName,
                         Connection* pConnection, AnalyzeXML* pIncoming,
                         soarxml::ElementXML* pResponse);
  bool HandleGetAllOutput(AgentSML* pAgentSML, char const* pCommandName,
                          Connection* pConnection, AnalyzeXML* pIncoming,
                          soarxml::ElementXML* pResponse);
  bool HandleGetRunState(AgentSML* pAgentSML, char const* pCommandName,
                         Connection* pConnection, AnalyzeXML* pIncoming,
                         soarxml::ElementXML* pResponse);
  bool HandleIsProductionLoaded(AgentSML* pAgentSML, char const* pCommandName,
                                Connection* pConnection, AnalyzeXML* pIncoming,
                                soarxml::ElementXML* pResponse);
  bool HandleSendClientMessage(AgentSML* pAgentSML, char const* pCommandName,
                               Connection* pConnection, AnalyzeXML* pIncoming,
                               soarxml::ElementXML* pResponse);
  bool HandleWasAgentOnRunList(AgentSML* pAgentSML, char const* pCommandName,
                               Connection* pConnection, AnalyzeXML* pIncoming,
                               soarxml::ElementXML* pResponse);
  bool HandleGetResultOfLastRun(AgentSML* pAgentSML, char const* pCommandName,
                                Connection* pConnection, AnalyzeXML* pIncoming,
                                soarxml::ElementXML* pResponse);
  bool HandleGetInitialTimeTag(AgentSML* pAgentSML, char const* pCommandName,
                               Connection* pConnection, AnalyzeXML* pIncoming,
                               soarxml::ElementXML* pResponse);
  bool HandleConvertIdentifier(AgentSML* pAgentSML, char const* pCommandName,
                               Connection* pConnection, AnalyzeXML* pIncoming,
                               soarxml::ElementXML* pResponse);
  bool HandleGetListenerPort(AgentSML* pAgentSML, char const* pCommandName,
                             Connection* pConnection, AnalyzeXML* pIncoming,
                             soarxml::ElementXML* pResponse);

  // Note: Register and unregister are both sent to this one handler
  bool HandleRegisterForEvent(AgentSML* pAgentSML, char const* pCommandName,
                              Connection* pConnection, AnalyzeXML* pIncoming,
                              soarxml::ElementXML* pResponse);

  bool HandleSVSInput(AgentSML* pAgentSML, char const* pCommandName,
                      Connection* pConnection, AnalyzeXML* pIncoming,
                      soarxml::ElementXML* pResponse);
  bool HandleSVSOutput(AgentSML* pAgentSML, char const* pCommandName,
                       Connection* pConnection, AnalyzeXML* pIncoming,
                       soarxml::ElementXML* pResponse);
  bool HandleSVSQuery(AgentSML* pAgentSML, char const* pCommandName,
                      Connection* pConnection, AnalyzeXML* pIncoming,
                      soarxml::ElementXML* pResponse);
};
