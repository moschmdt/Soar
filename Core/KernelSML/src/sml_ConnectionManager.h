...
class ConnectionManager {
 protected:
  ...
  // The list of connections.  One may be an embedded connection (part of same
  // process). Rest will be remote connections over a socket, wrapped in a
  // thread.
 ...
 public:
  ...
  // Get the i-th connection (it's ok to call with i >= number of connections as
  // this returns NULL in that case) So provides easy and thread safe way to get
  ...
  // The messages are sent to the callback registered with the connection when
  // it was created (ReceivedCall currently). Returning false indicates we
  // should stop checking for more messages (and presumably shutdown
  // completely).
