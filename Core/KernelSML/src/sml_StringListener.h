...
struct StringListenerCallbackData {
  ...
  char* pReturnStringBuffer;  // Output, client owned buffer, zero length string
                              // when there is no output (former function
                              // returned false)
...
class StringListener : public EventManager<smlStringEventId> {
 ...
 public:
  StringListener() { m_pKernelSML = NULL; }

  virtual ~StringListener() { Clear(); }

  // Initialize this listener
  void Init(KernelSML* pKernelSML) { m_pKernelSML = pKernelSML; }

  // Called when an event occurs in the kernel, see comments for
  // StringListenerCallbackData
  ...
  virtual bool RemoveListener(smlStringEventId eventID,
