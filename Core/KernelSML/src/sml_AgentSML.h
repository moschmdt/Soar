...
// List of input messages waiting for the next input phase callback from the
// kernel
...
struct DirectInputDelta {
  ...
  DirectInputDelta(DirectInputType type, char const* pID,
                   char const* pAttribute, char const* pValue,
                   int64_t clientTimeTag)
      : id(pID),
        attribute(pAttribute),
        clientTimeTag(clientTimeTag),
        type(type),
        svalue(pValue) {}

  DirectInputDelta(char const* pID, char const* pAttribute, int64_t value,
                   int64_t clientTimeTag)
      : id(pID),
        attribute(pAttribute),
        clientTimeTag(clientTimeTag),
  ...
  DirectInputDelta(char const* pID, char const* pAttribute, double value,
                   int64_t clientTimeTag)
      : id(pID),
        attribute(pAttribute),
        clientTimeTag(clientTimeTag),
...
class EXPORT AgentSML {
 ...
 protected:
  // This function actually creates a wme in Soar (and the associated mappings
  // between client and kernel side timetags) Currently, only the functions
  // below call this, so it's protected
  bool AddInputWME(char const* pID, char const* pAttribute,
                   Symbol* pValueSymbol, int64_t clientTimeTag);

 public:
  // These functions convert values into Symbols so the above function can be
  // called
  bool AddStringInputWME(char const* pID, char const* pAttribute,
                         char const* pValue, int64_t clientTimeTag);
  bool AddIntInputWME(char const* pID, char const* pAttribute, int64_t value,
                      int64_t clientTimeTag);
  bool AddDoubleInputWME(char const* pID, char const* pAttribute, double value,
                         int64_t clientTimeTag);

  // This function converts string values into typed values so the above
  // functions can be called This function is called by InputListener and passed
  // info taken from XML objects
  bool AddIdInputWME(char const* pID, char const* pAttribute,
                     char const* pValue, int64_t clientTimeTag);

  bool AddInputWME(char const* pID, char const* pAttribute, char const* pValue,
                   char const* pType, char const* pTimeTag);
 ...
 public:
  ...
  void BufferedAddStringInputWME(char const* pID, char const* pAttribute,
                                 char const* pValue, int64_t clientTimeTag);
  void BufferedAddIntInputWME(char const* pID, char const* pAttribute,
                              int64_t value, int64_t clientTimeTag);
  void BufferedAddDoubleInputWME(char const* pID, char const* pAttribute,
                                 double value, int64_t clientTimeTag);
  void BufferedAddIdInputWME(char const* pID, char const* pAttribute,
                             char const* pValue, int64_t clientTimeTag);
 ...
 protected:
  ...
  // For cleanup we also need a map from kernel side identifiers to client side
  // ones (for cleanup)
  ...
  // Map from client side time tags to kernel side timetags, and back (for
  // cleanup
  ...
  static void InputWmeGarbageCollectedHandler(agent* pAgent, int eventID,
                                              void* pData, void* pCallData);
 ...
 private:
  ...
  struct CapturedAction {
    ...
    CapturedAction(const CapturedAction& other)
        : dc(other.dc), clientTimeTag(other.clientTimeTag), add(0) {
      if (other.add) {
  ...
  std::string::size_type findDelimReplaceEscape(std::string& line,
 ...
 public:
  ...
  // It's a little less severe than clear() which releases everything we own,
  // not just wmes.
  ...
  void AddProductionListener(smlProductionEventId eventID,
                             Connection* pConnection) {
    m_ProductionListener.AddListener(eventID, pConnection);
  }
  void RemoveProductionListener(smlProductionEventId eventID,
                                Connection* pConnection) {
   ...
   *the trace. (One day we should no longer need to do this). Enabling/disabling
   ...
   *the kernel will assign them a different value when the wme is actually added
   *in the kernel.
   *
   *           RecordIDMapping should be called whenever an identifier is
   *created or linked to on the client side because it increments reference
   *counts to that id so that shared ids work correctly
   */
  ...
  bool CompletedRunType(uint64_t count) {
    // std::cout << std::endl << GetName() << ": (count" << count << " >
    // (m_InitialRunCount" << m_InitialRunCount << " + m_localRunCount" <<
    // m_localRunCount << "))";
...
class AgentRunCallback : public KernelCallback {
  ...
  virtual void OnKernelEvent(int eventID, AgentSML* pAgentSML, void*) {
    if (smlEVENT_AFTER_OUTPUT_PHASE == eventID) {
      ...
      // If the number of outputs generated by this agent has changed record it
      // as having generated output and possibly fire a notification event.
