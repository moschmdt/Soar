...
class RunScheduler : public gSKI::IRunListener {
 protected:
  ...
  // When running multiple agents, we synchronize them to this agent (same
  // phase) before starting the real run.
 ...
 public:
   ...
   * @param synchronize -- if true, synchronize all agents scheduled to run to
   *the same phase before running all agents in step
   * @param pError      -- any error
   *
   * @return Not clear on how to set this when have multiple agents.
   *         Can query each for "GetLastRunResult()".
   */
  egSKIRunResult RunScheduledAgents(egSKIRunType runStepSize,
                                    unsigned long count, smlRunFlags runFlags,
 ...
 protected:
  ...
  unsigned long GetStepCounter(
      gSKI::IAgent* pAgent, egSKIRunType runStepSize);  // for old scheduler...
  unsigned long GetStepCounter(gSKI::IAgent* pAgent,
                               egSKIInterleaveType stepSize);
  unsigned long GetRunCounter(gSKI::IAgent* pAgent, egSKIRunType runStepSize);
  void HandleEvent(egSKIRunEventId eventID, gSKI::IAgent* pAgent,
  ...
  bool IsAgentFinished(gSKI::IAgent* pAgent, AgentSML* pAgentSML,
                       egSKIRunType runStepSize, unsigned long count);
