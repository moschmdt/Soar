...
void AgentSML::InitListeners() {
  ...
  // For KernelSML (us) to work correctly we need to listen for certain events,
  // independently of what any client is interested in Currently: Listen for
  // output callback events so we can send this output over to the clients
  // Listen for "before" init-soar events (we need to know when these happen so
  // we can release all WMEs on the input link, otherwise gSKI will fail to
  // re-init the kernel correctly.) Listen for "after" init-soar events (we need
  // to know when these happen so we can resend the output link over to the
  // client)
...
void AgentSML::Init() {
  ...
  callbackId << "id_0x" << this << "_evt_"
             << INPUT_WME_GARBAGE_COLLECTED_CALLBACK;
  soar_add_callback(GetSoarAgent(), INPUT_WME_GARBAGE_COLLECTED_CALLBACK,
                    InputWmeGarbageCollectedHandler,
                    INPUT_WME_GARBAGE_COLLECTED_CALLBACK, this, 0,
                    const_cast<char*>(callbackId.str().c_str()));
}
...
AgentSML::~AgentSML() {
  ...
  callbackId << "id_0x" << this << "_evt_"
             << INPUT_WME_GARBAGE_COLLECTED_CALLBACK;
  soar_remove_callback(GetSoarAgent(), INPUT_WME_GARBAGE_COLLECTED_CALLBACK,
                       callbackId.str().c_str());

  delete m_pAgentRunCallback;

  /* RPM 9/06 added code from reinitialize_soar to clean up stuff hanging from
     last run need to put it here instead of in destroy_soar_agent because gSKI
     is cleaning up too much stuff and thus it will crash if called later */
...
// prior to deleting AgentSML, but before the underlying gSKI agent has been
// deleted 'deletingThisAgent' should only be true when we're actually in the
// destructor.
void AgentSML::Clear(bool deletingThisAgent) {
#ifdef DEBUG_UPDATE
  sml::PrintDebugFormat(
      "AgentSML::Clear start %s",
      deletingThisAgent ? "deleting this agent." : "not deleting this agent.");
#endif

  // Release any WME objects we still own.
  // (Don't flush removes in this case as we're shutting down rather than just
  // doing an init-soar).
...
#ifdef DEBUG_UPDATE
  sml::PrintDebugFormat(
      "AgentSML::Clear end %s",
      deletingThisAgent ? "deleting this agent." : "not deleting this agent.");
#endif
}

// Release all of the WMEs that we currently have references to
// It's a little less severe than clear() which releases everything we own, not
// just wmes. If flushPendingRemoves is true, make sure gSKI removes all wmes
// from Soar's working memory that have been marked for removal but are still
// waiting for the next input phase to actually be removed (this should
// generally be correct so we'll default to true for it).
void AgentSML::ReleaseAllWmes(bool flushPendingRemoves) {
#ifdef DEBUG_UPDATE
  sml::PrintDebugFormat("****************************************************");
  sml::PrintDebugFormat("%s AgentSML::ReleaseAllWmes start %s",
                        this->GetgSKIAgent()->GetName(),
                        flushPendingRemoves ? "flush pending removes."
                                            : "do not flush pending removes.");
  sml::PrintDebugFormat("****************************************************");
#endif

  if (flushPendingRemoves) {
    // TODO:
    // bool forceAdds = false ;  // It doesn't matter if we do these or not as
    // we're about to release everything.  Seems best to not start things up.
    // bool forceRemoves = true ;    // SML may have deleted a wme but gSKI has
    // yet to act on this.  As SML has removed its object we have no way to free
    // the gSKI object w/o doing this update.
  }

  // PrintDebugFormat("About to release kernel wmes") ;

  // BADBAD: we don't create or maintain this map anymore -- does this loop need
  // to be replaced with something?
  // for (KernelTimeTagMapIter mapIter = m_KernelTimeTagMap.begin() ; mapIter !=
  // m_KernelTimeTagMap.end() ; mapIter++) {
  ...
  for (PendingInputListIter iter = m_PendingInput.begin();
       iter != m_PendingInput.end(); iter++) {
...
#ifdef DEBUG_UPDATE
  sml::PrintDebugFormat("****************************************************");
  sml::PrintDebugFormat("%s AgentSML::ReleaseAllWmes end %s",
                        this->GetgSKIAgent()->GetName(),
                        flushPendingRemoves ? "flush pending removes."
                                            : "do not flush pending removes.");
  sml::PrintDebugFormat("****************************************************");
...
char const* AgentSML::GetName() { return m_agent->name; }
...
bool AgentSML::Reinitialize() {
  ...
  /* This must happen now because old output link identifiers get shipped over
     during do_output_phase above and then the new identifiers get shipped out
     during do_output_phase inside init_agent_memory below. With smem, those
     identifier details can change (output-link not being I3) and this causes
     problems. Can't use smlEVENT_AFTER_AGENT_REINITIALIZED because that happens
     too late.

     Mazin: This may no longer be true with the new model of smem in Soar 9.6.
     Not sure what would need to be changed back or whether it needs to be. */
...
void AgentSML::AddToPendingInputList(ElementXML_Handle hInputMsgHandle) {
  ...
  // an increased ref count so the caller can do whatever they want with the
  // original message
...
//=============================
uint64_t AgentSML::GetNumPhasesExecuted() { return m_agent->run_phase_count; }

//=============================
// Number of elaborations (or completed phases w/o rules firing) since last
// init-soar This definitions of elaborations matches with "run 1 -e". If you
// want to know how many sets of rules have fired you need a different counter.
//=============================
...
//=============================
// Number of outputs generated by this agent (or times reaching
// "max-nil-outputs" limit)
//=============================
...
//=============================
void AgentSML::ResetLastOutputCount() { m_agent->run_last_output_count = 0; }

//=============================
// Returns the current phase (which generally means the phase that is next going
// to execute if you inspect this in between runs)
//=============================
...
void AgentSML::Interrupt(smlStopLocationFlags stopLoc) {
  ...
  // will be caught in the RunScheduler::CompletedRunType() and/or
  // IsAgentFinished(). We don't want to interrupt agents until the appropriate
  // time if the request is sml_STOP_AFTER_DECISION_CYCLE.

  // These are immediate requests for interrupt, such as from RHS or application
  if ((sml_STOP_AFTER_SMALLEST_STEP == stopLoc) ||
      (sml_STOP_AFTER_PHASE == stopLoc)) {
    m_agent->stop_soar = true;
    // If the agent is not running, we should set the runState flag now so agent
    // won't run
    if (m_runState == sml_RUNSTATE_STOPPED) {
      m_runState = sml_RUNSTATE_INTERRUPTED;
    }
    // Running agents must test stopLoc & stop_soar in Step method to see if
    // interrupted. Because we set m_agent->stop_soar == true above, any running
    // agents should return to gSKI at the end of the current phase, even if
    // interleaving by larger steps.  KJC
  }
}
...
void AgentSML::FireRunEvent(smlRunEventId eventId) {
  if (eventId == smlEVENT_AFTER_RUN_ENDS) {
    ...
    xml_invoke_callback(
        GetSoarAgent());  // invokes XML_GENERATION_CALLBACK, clears XML state
  }

  // Trigger a callback from the kernel to propagate the event out to listeners.
  // This allows us to use a single uniform model for all run events (even when
  // some are really originating here in SML).
  int callbackEvent = KernelCallback::GetCallbackFromEventID(eventId);
  soar_invoke_callbacks(
      m_agent, SOAR_CALLBACK_TYPE(callbackEvent),
      reinterpret_cast<soar_call_data>(m_agent->current_phase));
}
...
void AgentSML::FireSimpleXML(char const* pMsg) {
  ...
  // This allows us to use a single uniform model for all run events (even when
  // some are really originating here in SML).
  soar_invoke_first_callback(
      m_agent, PRINT_CALLBACK,
      /*(ClientData)*/ static_cast<void*>(const_cast<char*>(pMsg)));
...
smlRunResult AgentSML::Step(smlRunStepSize stepSize) {
  ...
  uint64_t count = 1;
  uint64_t startCount =
      GetRunCounter(stepSize);  // getReleventCounter(stepSize);
  ...
  if (!interrupted) {
    ...
    // Notify that agent is about to execute. (NOT the start of a run, just a
    // step)
  ...
  if (m_agent->stop_soar || (m_interruptFlags & sml_STOP_AFTER_SMALLEST_STEP) ||
  ...
  // If not at the right phase, but interrupt was requested, then the SML
  // scheduler method IsAgentFinished will return true and
  // MoveTo_StopBeforePhase will step the agent by phases until this test is
  // satisfied.
  if (m_interruptFlags & sml_STOP_AFTER_DECISION_CYCLE) {
    // JRV: Bug 782: I changed the second half of the interrupt test to be true
    // if:
    //  * The agent is in the correct phase for stopping (this was only what was
    //  here before)
    //  * OR stepSize == run-til-output because this current_phase will always
    //  be input here if running until output
    //       and we do want to stop in this case, even though the stop-before
    //       phase isn't technically honored.
    // I've noted that the stop-before phase isn't honored in this case in the
    // bug report.
    bool inCorrectStopBeforePhase =
        m_agent->current_phase ==
        m_pKernelSML->ConvertSMLToSoarPhase(m_pKernelSML->GetStopBefore());
  ...
  if (interrupted) {
    ...
    /* This is probably redundant with the event above, which clients can listen
     * for... */
  ...
  if (m_agent->system_halted) {
    // if the agent halted because it is in an infinite loop of no-change
    // impasses interrupt the agents and allow the user to try to recover.
    if (m_agent->bottom_goal->id->level >=
        m_agent->Decider->settings[DECIDER_MAX_GOAL_DEPTH]) {
      // the agent halted because it seems to be in an infinite loop, so throw
      // interrupt
      ...
      /* This is probably redundant with the event above, which clients can
       * listen for... */
      FireSimpleXML("Interrupt received.");
    } else {
...
void AgentSML::DeleteSelf() {
  ...
  // This is important.  Otherwise if we create a new agent using the same
  // kernel object the listener will still exist inside gSKI and will crash when
  // an agent event is next generated.
  this->GetOutputListener()->UnRegisterForKernelSMLEvents();

  this->GetInputListener()->UnRegisterForKernelSMLEvents();

  // Unregister ourselves (this is important for the same reasons as listed
  // above)
  // m_pKernelSML->GetKernel()->GetAgentManager()->RemoveAgentListener(gSKIEVENT_BEFORE_AGENT_DESTROYED,
  // this) ;
...
//   // We shouldn't do it earlier or we can't be sure it'll be last on the list
//   of listeners which is where we
//   // need it to be (so that we clear our information about the gSKI agent
//   *after* we've notified any of our listeners
//   // about this event).
//   m_pBeforeDestroyedListener = new AgentBeforeDestroyedListener() ;
//   m_pKernelSML->GetKernel()->GetAgentManager()->AddAgentListener(gSKIEVENT_BEFORE_AGENT_DESTROYED,
//   m_pBeforeDestroyedListener) ;
// }
//
 ...
 *collection of wmes at once, so it makes up the ids for those objects. But the
 *kernel will assign them a different value when the wme is actually added in
 *the kernel.
 *************************************************************/
...
void AgentSML::RecordIDMapping(char const* pClientID, char const* pKernelID) {
  ...
  if (iter == m_IdentifierMap.end()) {
    ...
    // Record in both directions, so we can clean up (at which time we only know
    // the kernel side ID).
    ...
    // The mapping already exists, so we need to check and see if we have a
    // reference count for it yet
    IdentifierRefMapIter iter = m_IdentifierRefMap.find(pClientID);
    if (iter == m_IdentifierRefMap.end()) {
      // there is no reference count and this is the second reference, so set it
      // to two
...
void AgentSML::RemoveID(char const* pKernelID) {
  ...
  // Note: sometimes this is called by gSKI when it is removing wmes. gSKI
  // doesn't know if we're a direct connection and therefore aren't using this
  // map, so in that case we need to not update this. Therefore, we can't assert
  // here.
  ...
  // decrement the reference count and remove the identifier from the maps if it
  // is there
  IdentifierRefMapIter refIter = m_IdentifierRefMap.find(clientID);
  if (refIter == m_IdentifierRefMap.end()) {
    // when we have an entry in the m_IdentifierMap but not m_IdentifierRefMap,
    // this means our ref count is one, so we're decrementing to zero, so we
    // remove it
    ...
    // if the count falls to 1, remove it from this map since presence in the
    // map requires at least a ref count of two
    if (refIter->second < 2) {
...
void AgentSML::RegisterRHSFunction(RhsFunction* rhsFunction) {
  ...
  add_rhs_function(
      ...
      rhsFunction->IsValueReturned(), rhsFunction->CanBeStandAlone(),
      static_cast<void*>(rhsFunction), rhsFunction->LiteralizeArguments());
}
...
std::string AgentSML::ExecuteCommandLine(std::string const& commandLine) {
  ...
  bool rawOutput = true;
  soarxml::ElementXML* pMsg =
      pConnection->CreateSMLCommand(sml_Names::kCommand_CommandLine, rawOutput);
  pConnection->AddParameterToSMLCommand(pMsg, sml_Names::kParamAgent,
                                        this->GetName());
  pConnection->AddParameterToSMLCommand(pMsg, sml_Names::kParamLine,
                                        commandLine.c_str());
  ...
  bool ok = pKernel->ProcessCommand(sml_Names::kCommand_CommandLine,
                                    pConnection, &incoming, pResponse);

  std::string result;

  if (ok) {
    // Take the result from executing the command line and fill it in to our
    // "pReturnValue" array.
    AnalyzeXML response;
...
bool AgentSML::AddInputWME(char const* pID, char const* pAttribute,
                           Symbol* pValueSymbol, int64_t clientTimeTag) {
  std::string idKernel;
  ...
  wme* pNewInputWme =
      add_input_wme(m_agent, pIDSymbol, pAttrSymbol, pValueSymbol);
  ...
  // we just created these so lets release our ownership of them (they belong to
  // the wme now)
...
bool AgentSML::AddStringInputWME(char const* pID, char const* pAttribute,
                                 char const* pValue, int64_t clientTimeTag) {
...
bool AgentSML::AddIntInputWME(char const* pID, char const* pAttribute,
                              int64_t value, int64_t clientTimeTag) {
...
bool AgentSML::AddDoubleInputWME(char const* pID, char const* pAttribute,
                                 double value, int64_t clientTimeTag) {
  ...
  if (CaptureQuery()) {
    ...
    valueString << std::setprecision(std::numeric_limits<double>::max_digits10)
                << std::fixed << value;
    ca.Add()->value = valueString.str();
...
bool AgentSML::AddIdInputWME(char const* pID, char const* pAttribute,
                             char const* pValue, int64_t clientTimeTag) {
  Symbol* pValueSymbol = 0;

  // We will always receive a client-side identifier
  // If that identifier is found when we try to convert it, it already exists in
  // the kernel, we make a shared id. If that identifier is not found when we
  // try to convert it, we make a new identifier.
  ...
  if (ConvertID(pValue, &idValue)) {
    ...
    if (isalpha(pAttribute[0])) {
      ...
      idValueLetter =
          static_cast<char>(toupper(idValueLetter));  // make it upper case
    } else {
  ...
  // If pValueSymbol is a new id, then RecordIDMapping will create a map between
  // the client and kernel id names. Otherwise, RecordIDMapping will add a ref
  // count to client id name.
  ...
  // if (kDebugInput)
  //{
  //   PrintDebugFormat("Recording id mapping of %s to %s", pValue,
  //   newid.c_str()) ;
  // }
...
bool AgentSML::AddInputWME(char const* pID, char const* pAttribute,
                           char const* pValue, char const* pType,
                           char const* pClientTimeTag) {
  // TODO:
  // If input performance continues to be an issue, maybe some of these checks
...
bool AgentSML::RemoveInputWME(int64_t clientTimeTag) {
  ...
  // if (kDebugInput)
  //{
  //     std::string printInput1 = this->ExecuteCommandLine("print --internal
  //     --depth 2 I2") ;
  //   PrintDebugFormat("%s\nLooking for %" SCNu64, printInput1.c_str(),
  //   timetag) ;
  // }
  ...
  CHECK_RET_FALSE(
      pWME);  // BADBAD: above check means this will never be triggered; one of
              // the checks should go, but not sure which (can this function be
              // legitimately called with a timetag for a wme that's already
              // been removed?)
...
void AgentSML::InputWmeGarbageCollectedHandler(agent* /*pSoarAgent*/,
...
bool AgentSML::StartCaptureInput(const std::string& pathname, bool autoflush,
  ...
  m_pCaptureFile = new std::fstream(pathname.c_str(),
...
std::string::size_type AgentSML::findDelimReplaceEscape(
    std::string& line, std::string::size_type lpos) {
  std::string::size_type epos;
  std::string::size_type rpos;
  while ((epos = line.find(CAPTURE_ESCAPE, lpos)) <
...
bool AgentSML::CaptureInputWME(const CapturedAction& ca) {
  ...
  *m_pCaptureFile << ca.dc << CAPTURE_SEPARATOR << ca.clientTimeTag
                  << CAPTURE_SEPARATOR;
  if (ca.Add()) {
    *m_pCaptureFile << "add-wme" << CAPTURE_SEPARATOR << ca.Add()->id
                    << CAPTURE_SEPARATOR << escapeDelims(ca.Add()->attr)
                    << CAPTURE_SEPARATOR << escapeDelims(ca.Add()->value)
                    << CAPTURE_SEPARATOR << ca.Add()->type << std::endl;
  } else {
...
void AgentSML::ReplayInputWMEs() {
  /* These prints seem to be the only ones in the sml files.  Should they be
   * using another mechanism? */
  if (m_CapturedActions.empty()) {
    m_agent->outputManager->printa(
  ...
  while (!m_CapturedActions.empty()) {
    ...
    if (ca.Add()) {
      ...
      if (!AddInputWME(ca.Add()->id.c_str(), ca.Add()->attr.c_str(),
                       ca.Add()->value.c_str(), ca.Add()->type,
                       timetagString)) {
...
void AgentSML::BufferedAddStringInputWME(char const* pID,
                                         char const* pAttribute,
                                         char const* pValue,
                                         int64_t clientTimeTag) {
  m_DirectInputDeltaList.push_back(DirectInputDelta(
      DirectInputDelta::kAddString, pID, pAttribute, pValue, clientTimeTag));
}

void AgentSML::BufferedAddIntInputWME(char const* pID, char const* pAttribute,
                                      int64_t value, int64_t clientTimeTag) {
  m_DirectInputDeltaList.push_back(
      DirectInputDelta(pID, pAttribute, value, clientTimeTag));
}

void AgentSML::BufferedAddDoubleInputWME(char const* pID,
                                         char const* pAttribute, double value,
                                         int64_t clientTimeTag) {
  m_DirectInputDeltaList.push_back(
      DirectInputDelta(pID, pAttribute, value, clientTimeTag));
}

void AgentSML::BufferedAddIdInputWME(char const* pID, char const* pAttribute,
                                     char const* pValue,
                                     int64_t clientTimeTag) {
  m_DirectInputDeltaList.push_back(DirectInputDelta(
      DirectInputDelta::kAddId, pID, pAttribute, pValue, clientTimeTag));
}
