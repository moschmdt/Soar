...
ConnectionManager::ConnectionManager(int port, KernelSML* pKernel) {
  ...
  // Start the thread that checks for incoming messages on the remote
  // connections (once the listener has been used to create them).
...
ConnectionManager::~ConnectionManager() {
  ...
  // main process is executing, other threads may already have been forced to
  // exit so this shutdown won't be as clean.  Nothing should crash, but we
  // could cause timeouts etc.
  if (m_ListenerThread) {
    Shutdown();
  }
}
...
void ConnectionManager::SetTraceCommunications(bool state) {
  ...
  // (this list will usually be empty when we set this variable, but this is for
  // completeness).
  for (ConnectionsIter iter = m_Connections.begin();
...
void ConnectionManager::Shutdown() {
  ...
  for (ConnectionsIter iter = m_Connections.begin();
    ...
    // Not clear that we can just delete connections as we might be inside a
    // connection callback when the shutdown comes.  So for safety, move
    // connections to a closed list instead of deleting. For now this will leak
    // the connection object.  Later if we are confident we can delete this
    // list.
    // delete pConnection ;
  ...
  for (ConnectionsIter iter = m_ClosedConnections.begin();
       iter != m_ClosedConnections.end(); iter++) {
    Connection* pConnection = *iter;
    delete pConnection;
  }
...
Connection* ConnectionManager::GetConnectionByIndex(int i) {
  // Just to be consistent with allowing out of range values to be passed in
  // here.
  if (i < 0) {
    return NULL;
  }
  ...
  for (iter = m_Connections.begin(); iter != m_Connections.end() && i > 0;
       iter++) {
    i--;
  }
...
// The messages are sent to the callback registered with the connection when it
// was created (ReceivedCall currently). Those calls could take a long time to
// execute (e.g. a call to Run Soar). Returns true if we received at least one
// message.
bool ConnectionManager::ReceiveAllMessages() {
  ...
  // But we also want to allow the listener thread to add new connections while
  // we're doing this. (E.g. we might want to attach a debugger to a process
  // that's already executing a "run" command inside "receiveMessages"). So we
  // use this slightly cumbersome approach of looking up the connection based on
  // an integer index. The lookup is thread safe and if the connection list
  // changes between lookups that's fine as this function will return NULL once
  // we go out of bounds.  (You could argue that we might miss calling a
  // connection on a particular pass using this method but that should be ok).
  ...
  while (pConnection) {
    ...
    if (!pConnection->IsClosed()) {
      receivedOneMessage =
          pConnection->ReceiveMessages(true) || receivedOneMessage;
    } else {
      // If the connection has closed, delete it from the list of active
      // connections
      RemoveConnection(pConnection);

      // Remove any events that this connection is listening to
      KernelSML* pKernelSML =
          static_cast<KernelSML*>(pConnection->GetUserData());
      pKernelSML->RemoveAllListeners(pConnection);

      // Not clear that we can just delete connections as we might be inside a
      // connection callback when the shutdown comes.  So for safety, move
      // connections to a closed list instead of deleting. For now this will
      // leak the connection object.  Later if we are confident we can delete
      // this list.
      // delete pConnection ;
      m_ClosedConnections.push_back(pConnection);

      // Since we just removed this connection we should look up the same index
      // value again so we don't skip any.  This isn't really that important.
      index--;
    }

    // Get the next connection (will return NULL once we reach the end of the
    // list)
