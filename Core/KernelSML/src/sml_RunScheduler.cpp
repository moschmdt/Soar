 ...
 *
 */
smlRunStepSize RunScheduler::DefaultInterleaveStepSize(
    bool forever, smlRunStepSize runStepSize) {
  if (forever) {
    return sml_PHASE;
  }
 ...
 *
 */
bool RunScheduler::VerifyStepSizeForRunType(bool forever,
                                            smlRunStepSize runStepSize,
                                            smlRunStepSize interleave) {
  if (forever) {
    // can interleave by any smlRunStepSize
    return (sml_PHASE == interleave || sml_ELABORATION == interleave ||
            sml_DECIDE == interleave || sml_UNTIL_OUTPUT == interleave);
  }
...
AgentSML* RunScheduler::GetAgentToSynchronizeWith() {
  ...
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    if (pAgentSML->IsAgentScheduledToRun()) {
      ...
      if (!pSynchAgent ||
          pAgent->GetNumDecisionCyclesExecuted() >
              pSynchAgent->GetNumDecisionCyclesExecuted() ||
          (pAgent->GetNumDecisionCyclesExecuted() ==
               pSynchAgent->GetNumDecisionCyclesExecuted() &&
           IsPhaseLater(pAgentSML->GetCurrentPhase(),
                        pSynchAgent->GetCurrentPhase()))) {
 ...
 */
bool RunScheduler::IsAgentFinished(AgentSML* pAgentSML, bool forever,
                                   smlRunStepSize runStepSize, uint64_t count) {
  ...
  // pAgent->GetName(), static_cast<int64_t>(current),
  // static_cast<int64_t>(initial), static_cast<int64_t>(difference)) ;
  // fflush(stdout) ;
  ...
  if (((sml_DECIDE == runStepSize) || forever) &&
      (pAgentSML->GetInterruptFlags() & sml_STOP_AFTER_DECISION_CYCLE)) {
    finished = true;
  }
...
{
    for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin() ; iter !=
m_pKernelSML->m_AgentMap.end() ; iter++)
    {
        AgentSML* pAgentSML = iter->second ;
 ...
 *
 *           Why do we use events to do this?  You could just call "run x" and
...
void RunScheduler::InitializeUpdateWorldEvents(bool addListeners) {
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    // We register a listener so that the agent counters/flags get updated at
    // the end of Output.
    if (addListeners) {
...
bool RunScheduler::AreAllOutputPhasesComplete() {
  ...
  // or breakpoint etc.) and still generate the event.  However, it also means
  // if we do a "run --self" to only run some agents this event will still fire,
  // so we'll need to know not to update the world based on the runFlags.
  ...
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    // Agents that are paused waiting for other agents finish a RunType, are
    // still m_ScheduledToRun
    if (pAgentSML->IsAgentScheduledToRun()) {
      agents_running = true;
  ...
  // If all running agents completed output, we'll get here.  BUT we could also
  // reach this point if ALL agents are interrupted/halted, and none are
  // m_ScheduledToRun

  if (agents_running) {
    return true;  // we got here only if there are running agents and they all
                  // completed output
  } else {
    // ALL the agents from this run are halted or interrupted.  Check
    // interrupted agents to see if any of them stopped after output, then
    // return true.  We don't force ALL agents to be at end of output, because
    // some may have been RHS interrupted or --self earlier and there's no way
  ...
  // IF we're interrupted at the end of Decision_cycle, then we SHOULD return
  // true above, although if somehow we don't, it's possible that agents will
  // SNC one cycle waiting for I/O to catch up.  But if we returned true here,
  // we could get many false positive events.
 ...
 */
void RunScheduler::MoveTo_StopBeforePhase(bool forever,
                                          smlRunStepSize runStepSize) {
  // If we ran by decision_cycle and we've run the appropriate number of
  // decisions, then  step agent until it reached the correct phase where it
  // should stop. Just before this point, (in scheduler while loop) we've
  // checked and fired the OutputComplete and OutputGenerated events, so it
  // should be fine to step an agent by phases until it gets to StopBefore, and
  // then step the other agents in turn.
  //
  // If an interrupt was requested, it will be detected inside
  // StepInClientThread and the appropriate events will be generated.  The
  // runResult will come back as INTERRUPTED, and the while loops will exit so
  // that the run stops.
  //
  // Support for "Run 0" is somewhat complicated, since we might very well
  // cross the I/O boundary.  So we'll try stepping til OUTPUT done, then
  // generate Update events, then step again til StopBeforePt is reached.  Note:
  // when we support StopBeforeUpdateWorld, we'll need to explicitly check for
  // that before generating events.
  ...
  if ((runStepSize == sml_DECIDE) || forever) {
    for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
      ...
      if (pAgentSML->WasAgentOnRunList()) {
        ...
        // as in Bug 648, it's possible that a client has requested
        // STOP_AFTER_DECISION while the agent was stopped at the
        // m_StopBeforePhase, yet the agent run logic has dropped thru to this
        // point without generating the interrupt yet.
        if ((m_StopBeforePhase == phase) &&
            (pAgent->GetRunState() == sml_RUNSTATE_STOPPED) &&
        ...
        while ((phase != m_StopBeforePhase) &&
               (sml_RUN_COMPLETED == runResult)) {
          runResult = pAgent->StepInClientThread(sml_PHASE);
          phase = pAgentSML->GetCurrentPhase();
          // don't cross Output-Input boundary here just to get to StopBefore
          // phase for "Run 0"
          if (sml_INPUT_PHASE == phase) {
            break;
          }
        }
        ...
        /* pAgent->FireRunEndsEvent() ; */  // not if supporting run0 over O/I
                                            // boundary
      }
    }
    // If agents haven't reached the StopBeforePhase, then agents finished
    // output and we need to possibly generate the UpdateWorld events before
    // stepping any further. If not all agents finished output, then nothing
    // will fire.
    TestForFiringUpdateWorldEvents();

    // This second While loop is only needed if we allow agents to cross the
    // output-update-input boundary on a "run 0" command.  If we don't allow
    // that then all that is left to do is generate the RunEndsEvents. (and we
    // shouldn't even do the update events above.  The FireRunEnds event can go
    ...
    for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
      ...
      if (pAgentSML->WasAgentOnRunList()) {
        ...
        while ((phase != m_StopBeforePhase) &&
          ...
          runResult = pAgent->StepInClientThread(sml_PHASE);
          phase = pAgentSML->GetCurrentPhase();
          // We should never get to this point again, so we need to generate
          // ERROR
...
void RunScheduler::TestForFiringUpdateWorldEvents() {
  if (AreAllOutputPhasesComplete()) {
    ...
    m_pKernelSML->FireUpdateListenerEvent(smlEVENT_AFTER_ALL_OUTPUT_PHASES,
                                          m_RunFlags);
    ...
    if (HaveAllGeneratedOutput()) {
      ...
      m_pKernelSML->FireUpdateListenerEvent(smlEVENT_AFTER_ALL_GENERATED_OUTPUT,
                                            m_RunFlags);
...
void RunScheduler::TerminateUpdateWorldEvents(bool removeListeners) {
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    if (removeListeners) {
      pAgentSML->GetAgentRunCallback()->UnregisterWithKernel(
          smlEVENT_AFTER_OUTPUT_PHASE);
    }
  }
}
...
smlRunResult RunScheduler::GetOverallRunResult() {
  ...
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    if (result == sml_RUN_INTERRUPTED) {
      overallResult = sml_RUN_INTERRUPTED;
    }
  }
...
bool RunScheduler::AnAgentHaltedDuringRun() {
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    if (pAgentSML->WasAgentOnRunList()) {
      AgentSML* pAgent = pAgentSML;
      if (pAgent->GetRunState() == sml_RUNSTATE_HALTED) {
        return true;
      }
    }
  }
 ...
 *           Does not remove anyone from the run list.
 */
bool RunScheduler::TestIfAllFinished(bool forever, smlRunStepSize runStepSize,
                                     uint64_t count) {
  bool allDone = true;
  ...
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    bool agentFinishedRun =
        IsAgentFinished(pAgentSML, forever, runStepSize, count);
 ...
 * @param interleaveStepSize -- how large of a step each agent is run before
 *other agents are run
 * @param synchronize -- if true, synchronize all agents scheduled to run to the
 *same phase before running all agents in step
 * @param pError      -- any error
 *
 * @return Not clear on how to set this when have multiple agents.
 *         Can query each for "GetLastRunResult()".
 */
smlRunResult RunScheduler::RunScheduledAgents(
    bool forever, smlRunStepSize runStepSize, uint64_t count,
    smlRunFlags runFlags, smlRunStepSize interleaveStepSize, bool synchronize) {
  // check to see if we're already running, return an error if so because soar
  // may not be running with the same parameters that the command asked for
  if (m_IsRunning) {
    return sml_RUN_ERROR_ALREADY_RUNNING;
  }

  // Agents were already appropriately added (or not) to the RunList before
  // calling this method. For every Run Command issued, we can find out if agent
  // is still scheduled to run, and/or whether it was scheduled to run at all.
  // When agents stop or Halt, the AgentScheduledToRun bool is set to false, but
  // WasScheduledToRun is unchanged.
  ...
  // Record initial counts and zero the local "run" counter (that we're about to
  // be incrementing)
  InitializeRunCounters(runStepSize);

  // Depending on RunType, set the stop location for
  // gSKI_STOP_AFTER_DECISION_CYCLE interrupts
  ...
  // IF we did a StopBeforeUpdate, this is where we need to test and generate
  // update events...
  TestForFiringUpdateWorldEvents();
  ...
  //  Before running, check to see if an agent is at a point other than the
  //  StopBeforePhase. If so, we'll decrement  the RunCount before entering the
  //  Run loop so as not to run more Decision phases than specified in the
  //  runCount.  See bug #710.
  if ((sml_DECIDE == runStepSize) && !forever)
    if (!AllAgentsAtStopBeforePhase() && (count > 0)) {
      count--;
    }

  // If we issue a "run 0" and all agents are synched and in the correct state
  // we're done.
  ...
  while (!runFinished) {
    ...
    while (AgentsStillStepping()) {
      ...
      // Notify listeners that Soar is going to run.  This event is a kernel
      // level (agent manager) event which allows a single listener to check for
      // client driven interrupts for all agents. Sometimes that's easier to
      // work with than the agent specific events (where you get <n> events from
      // <n> agents)
      //    note that there is not a corresponding AFTER_AGENTS_RUN_STEP
      //    event...
      ...
      for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
        ...
        if (pAgentSML->IsAgentOnStepList()) {
          ...
          smlRunResult runResult =
              pAgentSML->StepInClientThread(interleaveStepSize);
          // ?? pAgentSML->IncrementLocalStepCounter();

          // halted and running agents will return an error from
          // StepInClientThread
          //

          // if agent finished one runType, incr counter and remove from
          // stepList
          if (pAgentSML->CompletedRunType(pAgentSML->GetRunCounter(
                  runStepSize)) /* || pAgent->MaxNilOutputCyclesReached */) {
            pAgentSML->IncrementLocalRunCounter();
          ...
          // if agent finished count runTypes, remove from RunList, else
          // runFinished = false; can also return true if a
          // gSKI_STOP_AFTER_DECISION_CYCLE interrupt occurred or is pending on
          // agents with RunType DECIDE or FOREVER.
          bool agentFinishedRun =
              IsAgentFinished(pAgentSML, forever, runStepSize, count);

          // Have to test the run state to find out if we are still ok to keep
          // running (not sure if runResult provides this as well, but they're
          // from different enums).
          smlRunState runState = pAgentSML->GetRunState();

          // An agent should return "stopped" if it's just pausing in the middle
          // of a run before we run it for the next phase.  Anything else means
          // this agent is done running.
  ...
  MoveTo_StopBeforePhase(
      forever,
      runStepSize);  // agents will have done FireRunEndsEvent() here or above.
  ...
  // Not sure how to quantify the results of running <n> agents in a single
  // value You can query each agent for its GetResultOfRun() to know more.
