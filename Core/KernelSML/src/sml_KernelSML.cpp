...
static soarxml::ElementXML* AddErrorMsg(Connection* pConnection,
...
/** Deletes all agents and optionally waits until this has actually happened (if
 * the agent is running there may be a delay) */
...
std::string KernelSML::FireLoadLibraryEvent(char const* pLibraryCommand) {
  ...
  // This next bit of code is completely redundant, the strcpy below is
  // basically a no-op:
...
std::string KernelSML::FireCliExtensionMessageEvent(
    char const* pCliExtCommand) {
  const int kBufferLength = 10000;
  char response[kBufferLength];
  response[0] = 0;
  ...
  m_StringListener.OnKernelEvent(smlEVENT_TCL_LIBRARY_MESSAGE, 0,
                                 &callbackData);
...
std::string KernelSML::SendClientMessage(AgentSML* pAgentSML,
  ...
  bool ok = m_RhsListener.HandleEvent(smlEVENT_CLIENT_MESSAGE, pAgentSML, false,
                                      pMessageType, pMessage, sizeof(response),
                                      response);
  if (!ok) {
...
bool KernelSML::SendFilterMessage(AgentSML* pAgent, char const* pCommandLine,
  ...
  bool ok = m_RhsListener.HandleFilterEvent(smlEVENT_FILTER, pAgent,
                                            pCommandLine, s_response);
...
Connection* KernelSML::GetEmbeddedConnection() {
  ...
  for (Connection* pConnection =
           m_pConnectionManager->GetConnectionByIndex(index);
       pConnection != NULL; index++) {
...
bool KernelSML::DeleteAgentSML(const char* agentName) {
  ...
  KernelAgentMapIter kiter =
      m_KernelAgentMap.find(iter->second->GetSoarAgent());
...
bool KernelSML::InvalidArg(Connection* pConnection,
  ...
  msg << "Invalid arguments for command : " << pCommandName
      << pErrorDescription;
...
smlPhase KernelSML::GetStopBefore() { return m_pRunScheduler->GetStopBefore(); }
...
top_level_phase KernelSML::ConvertSMLToSoarPhase(smlPhase phase) {
  ...
  assert(PREFERENCE_PHASE ==
         static_cast<top_level_phase>(sml_PREFERENCE_PHASE));
...
bool KernelSML::ProcessCommand(char const* pCommandName,
  ...
  bool result = (this->*pFunction)(pAgentSML, pCommandName, pConnection,
                                   pIncoming, pResponse);
...
// For debugging, it's helpful to be able to look at the incoming message as an
// XML string.  Enable in kernel.h
  ...
  if (pCommandName) {
    ...
    if (!msg.GetCommandTag()) {
      AddErrorMsg(pConnection, pResponse,
                  "Incoming message did not contain a <command> tag");
    } else {
      AddErrorMsg(pConnection, pResponse,
                  "Incoming message did not contain a name attribute in the "
                  "<command> tag");
    }
  }
...
void KernelSML::Symbol2String(Symbol* pSymbol, bool refCounts,
...
// A fully direct run would be a call straight to gSKI but supporting that is
// too dangerous due to the extra events and control logic surrounding the SML
// RunScheduler. So we compromise with a call directly to that scheduler,
// boosting performance over the standard "run" path which goes through the
// command line processor.
void KernelSML::DirectRun(char const* pAgentName, bool forever, int stepSize,
  ...
  smlRunStepSize runType =
      (forever) ? sml_DECIDE : static_cast<smlRunStepSize>(stepSize);

  // Decide how large of a step to run each agent before switching to the next
  // agent
  ...
  // If we're running by decision cycle synchronize up the agents to the same
  // phase before we start
  bool synchronizeAtStart = (runType == sml_DECIDE);

  // Do the run
  pScheduler->RunScheduledAgents(forever, runType, count, runFlags,
                                 smlRunStepSize(interleaveSize),
                                 synchronizeAtStart);
  return;
}
