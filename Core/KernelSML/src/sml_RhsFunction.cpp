...
// This method is called by the kernel, which in turn calls the Execute method
// of the RhsFunction.
Symbol* RhsFunction::RhsFunctionCallback(agent* thisAgent, cons* args,
                                         void* user_data) {
  ...
  if ((rhsFunction->GetNumExpectedParameters() == kPARAM_NUM_VARIABLE) ||
      (static_cast<int>(symVector.size()) ==
       rhsFunction->GetNumExpectedParameters())) {
    // Actually make the call.  We can do the dynamic cast because we passed in
    // the
    ...
    if (rhsFunction->IsValueReturned() == true) {
      ...
      if (pReturn != 0) {
        ...
        // We have to return something to prevent a crash, so we return an error
        // code
        pSoarReturn = thisAgent->symbolManager->make_str_constant(
            "error_expected_rhs_function_to_return_value_but_it_did_NOT");
      }
    } else {
      // Expected that the rhs function would not return a value, but it did.
    ...
    // We can return anything we want to soar; we return an error message so at
    // least the problem is obvious.
    if (rhsFunction->IsValueReturned() == true) {
      pSoarReturn = thisAgent->symbolManager->make_str_constant(
          "error_wrong_number_of_args_passed_to_rhs_function");
    }
  }
...
Symbol* sml::InterruptRhsFunction::Execute(
    std::vector<Symbol*>* /*pArguments*/) {
  // BADBAD?  Should this be just an interrupt on this agent?  Existing
  // semantics are for all agents so doing that here too.
...
Symbol* sml::ConcatRhsFunction::Execute(std::vector<Symbol*>* pArguments) {
  ...
  for (std::vector<Symbol*>::iterator iter = pArguments->begin();
       iter != pArguments->end(); iter++) {
    Symbol* pSymbol = *iter;
    ...
    if (!pSymbol)
      ...
      ostr.append(pSymbol->to_string());
  }

  return m_pAgentSML->GetSoarAgent()->symbolManager->make_str_constant(
      ostr.c_str());
}
...
Symbol* sml::CmdRhsFunction::Execute(std::vector<Symbol*>* pArguments) {
  ...
  Symbol* pResult =
      m_pAgentSML->GetSoarAgent()->symbolManager->make_str_constant(
          result.c_str());
  return pResult;
}
...
Symbol* sml::ExecRhsFunction::Execute(std::vector<Symbol*>* pArguments) {
  ...
  // Get the command line string.
  // We've decided for "exec" to just concatenate all arguments together without
  // inserting spaces.  This allows for powerful operations (such as
  // constructing an argument out of pieces).
  for (std::vector<Symbol*>::iterator iter = (++pArguments->begin());
       iter != pArguments->end(); iter++) {
    Symbol* pSymbol = *iter;
  ...
  std::string result;
  bool ok = m_pAgentSML->GetKernelSML()->FireRhsEvent(
      m_pAgentSML, smlEVENT_RHS_USER_FUNCTION, function, argument, &result);

  if (!ok) {
    result =
        std::string("Error: Nobody was registered to implement rhs function ") +
        function;
  }
