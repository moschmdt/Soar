...
void KernelSML::BuildCommandMap() {
  m_CommandMap[sml_Names::kCommand_CreateAgent] =
      &sml::KernelSML::HandleCreateAgent;
  m_CommandMap[sml_Names::kCommand_DestroyAgent] =
      &sml::KernelSML::HandleDestroyAgent;
  m_CommandMap[sml_Names::kCommand_GetInputLink] =
      &sml::KernelSML::HandleGetInputLink;
  m_CommandMap[sml_Names::kCommand_Input] = &sml::KernelSML::HandleInput;
  m_CommandMap[sml_Names::kCommand_CommandLine] =
      &sml::KernelSML::HandleCommandLine;
  m_CommandMap[sml_Names::kCommand_CheckForIncomingCommands] =
      &sml::KernelSML::HandleCheckForIncomingCommands;
  m_CommandMap[sml_Names::kCommand_GetAgentList] =
      &sml::KernelSML::HandleGetAgentList;
  m_CommandMap[sml_Names::kCommand_RegisterForEvent] =
      &sml::KernelSML::HandleRegisterForEvent;
  m_CommandMap[sml_Names::kCommand_UnregisterForEvent] =
      &sml::KernelSML::HandleRegisterForEvent;  // Note -- both register and
                                                // unregister go to same handler
  m_CommandMap[sml_Names::kCommand_FireEvent] =
      &sml::KernelSML::HandleFireEvent;
  m_CommandMap[sml_Names::kCommand_SuppressEvent] =
      &sml::KernelSML::HandleSuppressEvent;
  m_CommandMap[sml_Names::kCommand_SetInterruptCheckRate] =
      &sml::KernelSML::HandleSetInterruptCheckRate;
  m_CommandMap[sml_Names::kCommand_GetVersion] =
      &sml::KernelSML::HandleGetVersion;
  m_CommandMap[sml_Names::kCommand_Shutdown] = &sml::KernelSML::HandleShutdown;
  m_CommandMap[sml_Names::kCommand_IsSoarRunning] =
      &sml::KernelSML::HandleIsSoarRunning;
  m_CommandMap[sml_Names::kCommand_GetConnections] =
      &sml::KernelSML::HandleGetConnections;
  m_CommandMap[sml_Names::kCommand_SetConnectionInfo] =
      &sml::KernelSML::HandleSetConnectionInfo;
  m_CommandMap[sml_Names::kCommand_GetAllInput] =
      &sml::KernelSML::HandleGetAllInput;
  m_CommandMap[sml_Names::kCommand_GetAllOutput] =
      &sml::KernelSML::HandleGetAllOutput;
  m_CommandMap[sml_Names::kCommand_GetRunState] =
      &sml::KernelSML::HandleGetRunState;
  m_CommandMap[sml_Names::kCommand_IsProductionLoaded] =
      &sml::KernelSML::HandleIsProductionLoaded;
  m_CommandMap[sml_Names::kCommand_SendClientMessage] =
      &sml::KernelSML::HandleSendClientMessage;
  m_CommandMap[sml_Names::kCommand_WasAgentOnRunList] =
      &sml::KernelSML::HandleWasAgentOnRunList;
  m_CommandMap[sml_Names::kCommand_GetResultOfLastRun] =
      &sml::KernelSML::HandleGetResultOfLastRun;
  m_CommandMap[sml_Names::kCommand_GetInitialTimeTag] =
      &sml::KernelSML::HandleGetInitialTimeTag;
  m_CommandMap[sml_Names::kCommand_ConvertIdentifier] =
      &sml::KernelSML::HandleConvertIdentifier;
  m_CommandMap[sml_Names::kCommand_GetListenerPort] =
      &sml::KernelSML::HandleGetListenerPort;
  m_CommandMap[sml_Names::kCommand_SVSInput] = &sml::KernelSML::HandleSVSInput;
  m_CommandMap[sml_Names::kCommand_SVSOutput] =
      &sml::KernelSML::HandleSVSOutput;
 ...
 *command is not agent specific)
 * @param pCommandName   The SML command name (so one handler can handle many
 *incoming calls if we wish)
 * @param pConnection    The connection this command came in on
 * @param pIncoming      The incoming, analyzed message.
 * @param pResponse      The partially formed response.  This handler needs to
 *fill in more of this.
...
bool KernelSML::HandleCreateAgent(AgentSML* pAgentSML, char const* pCommandName,
  ...
  pAgentSML->InitListeners();  // This must happen before the soar agent is
                               // initialized
  ...
  // Note--agent status for connections just refers to the last agent created,
  // i.e. this one.
  m_pConnectionManager->SetAgentStatus(sml_Names::kStatusCreated);

  // We also need to listen to input events so we can pump waiting sockets and
  // get interrupt messages etc. moved to sml_InputListener.cpp

  // pAgentSML->m_inputlink->GetInputLinkMemory()->m_RemoveWmeCallback =
  // RemoveInputWMERecordsCallback;
  ...
  if (this->m_pRunScheduler->IsRunning()) {
    ...
    uint64_t count = pAgentSML->GetRunCounter(
        this->m_pRunScheduler->GetCurrentRunStepSize());
    ...
    pAgentSML->GetAgentRunCallback()->RegisterWithKernel(
  ...
  if (!Soar_Instance::Get_Soar_Instance().was_run_from_unit_test()) {
    ...
    if (!directory.empty()) {
      ...
      // pConnection->AddErrorToSMLResponse(pResponse, "Could not find
      // settings.soar file.", 1);

      /* Returning true.  Otherwise, Soar will exit if the file could not be
       * found. */
      return true;
    }
  }
...
bool KernelSML::HandleRegisterForEvent(AgentSML* pAgentSML,
                                       char const* pCommandName,
                                       Connection* pConnection,
                                       AnalyzeXML* pIncoming,
                                       soarxml::ElementXML* pResponse) {
  // Decide if registering or unregistering
  bool registerForEvent =
      (strcmp(pCommandName, sml_Names::kCommand_RegisterForEvent) == 0);
  ...
  if (IsSystemEventID(id)) {
    ...
    if (registerForEvent) {
      ...
      this->RemoveSystemListener(static_cast<smlSystemEventId>(id),
                                 pConnection);
    }
    ...
    char const* pRhsFunctionName =
        pIncoming->GetArgString(sml_Names::kParamName);

    if (!pRhsFunctionName) {
      return InvalidArg(pConnection, pResponse, pCommandName,
                        "Registering for rhs user function, but no function "
                        "name was provided");
    }
    ...
    if (!pAgentSML) {
      return InvalidArg(
          pConnection, pResponse, pCommandName,
          "No agent name for an event that is handled by an agent");
    }
    ...
    // Register or unregister for this event
    if (registerForEvent) {
      pAgentSML->AddProductionListener(static_cast<smlProductionEventId>(id),
                                       pConnection);
    } else {
      pAgentSML->RemoveProductionListener(static_cast<smlProductionEventId>(id),
                                          pConnection);
    }
    ...
    // Register or unregister for this event
    if (registerForEvent) {
      pAgentSML->AddPrintListener(static_cast<smlPrintEventId>(id),
                                  pConnection);
    } else {
      pAgentSML->RemovePrintListener(static_cast<smlPrintEventId>(id),
                                     pConnection);
    }
    ...
    if (registerForEvent) {
      ...
      pOutputListener->RemoveListener(smlEVENT_OUTPUT_PHASE_CALLBACK,
                                      pConnection);
    }
  } else {
    // The event didn't match any of our handlers
    return InvalidArg(pConnection, pResponse, pCommandName,
                      "KernelSML doesn't know how to handle that event id");
  }

  return true;
}

bool KernelSML::HandleSetConnectionInfo(AgentSML* /*pAgentSML*/,
                                        char const* pCommandName,
                                        Connection* pConnection,
                                        AnalyzeXML* pIncoming,
                                        soarxml::ElementXML* pResponse) {
...
bool KernelSML::HandleGetConnections(AgentSML* /*pAgentSML*/,
                                     char const* /*pCommandName*/,
                                     Connection* /*pCallingConnection*/,
                                     AnalyzeXML* /*pIncoming*/,
                                     soarxml::ElementXML* pResponse) {
  // Create the result tag
  TagResult* pTagResult = new TagResult();
  pTagResult->AddAttribute(sml_Names::kCommandOutput,
                           sml_Names::kStructuredOutput);
  ...
  while (pConnection) {
    ...
    // Fill in the info
    pTagConnection->AddAttribute(sml_Names::kConnectionId,
                                 pConnection->GetID());
    pTagConnection->AddAttribute(sml_Names::kConnectionName,
                                 pConnection->GetName());
    pTagConnection->AddAttribute(sml_Names::kConnectionStatus,
                                 pConnection->GetStatus());
    pTagConnection->AddAttribute(sml_Names::kAgentStatus,
                                 pConnection->GetAgentStatus());
    ...
    // (This provides thread safe access to the list, in case it changes during
    // this enumeration)
    index++;
...
bool KernelSML::HandleDestroyAgent(AgentSML* pAgentSML,
                                   char const* /*pCommandName*/,
                                   Connection* /*pConnection*/,
                                   AnalyzeXML* /*pIncoming*/,
                                   soarxml::ElementXML* /*pResponse*/) {
  ...
  if (m_CommandLineInterface.IsLogOpen()) {
    m_CommandLineInterface.DoCommand(0, pAgentSML, "output log --close", false,
                                     true, 0);
  }
...
// Shutdown is an irrevocable request to delete all agents and prepare for
// kernel deletion.
bool KernelSML::HandleShutdown(AgentSML* /*pAgentSML*/,
                               char const* /*pCommandName*/,
                               Connection* /*pConnection*/,
                               AnalyzeXML* /*pIncoming*/,
                               soarxml::ElementXML* /*pResponse*/) {
  ...
  // Delete all agents explicitly now (so listeners can hear that the agents
  // have been destroyed).
...
// Return information about the current runtime state of the agent (e.g. phase,
// decision cycle count etc.)
bool KernelSML::HandleGetRunState(AgentSML* pAgentSML, char const* pCommandName,
                                  Connection* pConnection,
                                  AnalyzeXML* pIncoming,
                                  soarxml::ElementXML* pResponse) {
  ...
  if (!pValue) {
    return InvalidArg(pConnection, pResponse, pCommandName,
                      "Need to specify the type of information wanted.");
  }
  ...
  if (strcmp(pValue, sml_Names::kParamPhase) == 0) {
    ...
    return InvalidArg(
        pConnection, pResponse, pCommandName,
        "Didn't recognize the type of information requested in GetRunState().");
  }
...
// Return information about the current runtime state of the agent (e.g. phase,
// decision cycle count etc.)
bool KernelSML::HandleWasAgentOnRunList(AgentSML* pAgentSML,
                                        char const* /*pCommandName*/,
                                        Connection* pConnection,
                                        AnalyzeXML* /*pIncoming*/,
                                        soarxml::ElementXML* pResponse) {
...
bool KernelSML::HandleGetResultOfLastRun(AgentSML* pAgentSML,
                                         char const* /*pCommandName*/,
                                         Connection* pConnection,
                                         AnalyzeXML* /*pIncoming*/,
                                         soarxml::ElementXML* pResponse) {
...
bool KernelSML::HandleGetInitialTimeTag(AgentSML* /*pAgentSML*/,
                                        char const* /*pCommandName*/,
                                        Connection* pConnection,
                                        AnalyzeXML* /*pIncoming*/,
                                        soarxml::ElementXML* pResponse) {
  // We use negative values for client time tags (so we can tell they're client
  // side not kernel side)
  ...
  while (maxTries > 0 && !done) {
    // Walk the list of connections and see if we can find an time tag start
    // value that's not in use (We do this walking so if connections are made,
    // broken and remade and we'll reuse the id space).
    int index = 0;
    ...
    while (connect && ok) {
      if (connect->GetInitialTimeTagCounter() == timeTagStart) {
        ok = false;
        timeTagStart -=
            (1 << 27);  // 8 * (1<<27) is (1<<30) so won't overflow.  Allows
                        // (1<<27) values per client w/o collision or more than
                        // 100 million wmes each.
      }
  ...
  // If we fail this it means we couldn't find a valid start value for the time
  // tag counter. Either we have 8 existing connections or there's a bug in this
  // code.
  assert(maxTries >= 0);
...
bool KernelSML::HandleConvertIdentifier(AgentSML* pAgentSML,
                                        char const* pCommandName,
                                        Connection* pConnection,
                                        AnalyzeXML* pIncoming,
                                        soarxml::ElementXML* pResponse) {
  ...
  if (!pClientId) {
    return InvalidArg(pConnection, pResponse, pCommandName,
                      "Need to specify the client-side identifier to convert.");
  }
...
bool KernelSML::HandleIsProductionLoaded(AgentSML* pAgentSML,
                                         char const* pCommandName,
                                         Connection* pConnection,
                                         AnalyzeXML* pIncoming,
                                         soarxml::ElementXML* pResponse) {
  ...
  if (!pName) {
    return InvalidArg(pConnection, pResponse, pCommandName,
                      "Need to specify the production name to check.");
  }

  Symbol* sym =
      pAgentSML->GetSoarAgent()->symbolManager->find_str_constant(pName);
...
bool KernelSML::HandleGetVersion(AgentSML* /*pAgentSML*/,
                                 char const* /*pCommandName*/,
                                 Connection* pConnection,
                                 AnalyzeXML* /*pIncoming*/,
                                 soarxml::ElementXML* pResponse) {
  return this->ReturnResult(pConnection, pResponse,
                            sml_Names::kSoarVersionValue);
}

bool KernelSML::HandleIsSoarRunning(AgentSML* /*pAgentSML*/,
                                    char const* /*pCommandName*/,
                                    Connection* pConnection,
                                    AnalyzeXML* /*pIncoming*/,
                                    soarxml::ElementXML* pResponse) {
...
bool KernelSML::HandleGetAgentList(AgentSML* /*pAgentSML*/,
                                   char const* /*pCommandName*/,
                                   Connection* /*pConnection*/,
                                   AnalyzeXML* /*pIncoming*/,
                                   soarxml::ElementXML* pResponse) {
...
bool KernelSML::HandleSetInterruptCheckRate(
    AgentSML* /*pAgentSML*/, char const* /*pCommandName*/,
...
bool KernelSML::HandleFireEvent(AgentSML* /*pAgentSML*/,
                                char const* pCommandName,
                                Connection* pConnection, AnalyzeXML* pIncoming,
                                soarxml::ElementXML* pResponse) {
  // Get the parameters
...
bool KernelSML::HandleSendClientMessage(AgentSML* pAgentSML,
                                        char const* pCommandName,
                                        Connection* pConnection,
                                        AnalyzeXML* pIncoming,
                                        soarxml::ElementXML* pResponse) {
  ...
  if (!pMessageType || !pMessage) {
    return InvalidArg(
        pConnection, pResponse, pCommandName,
        "Require a message type and a message and one is missing");
  }

  std::string result =
      this->SendClientMessage(pAgentSML, pMessageType, pMessage);
...
bool KernelSML::HandleSuppressEvent(AgentSML* /*pAgentSML*/,
                                    char const* pCommandName,
                                    Connection* pConnection,
                                    AnalyzeXML* pIncoming,
                                    soarxml::ElementXML* pResponse) {
...
// Check if anyone has sent us a command (e.g. over a socket from a remote
// debugger)
bool KernelSML::HandleCheckForIncomingCommands(AgentSML* /*pAgentSML*/,
                                               char const* /*pCommandName*/,
                                               Connection* pConnection,
                                               AnalyzeXML* /*pIncoming*/,
                                               soarxml::ElementXML* pResponse) {
...
bool KernelSML::HandleGetInputLink(AgentSML* pAgentSML,
                                   char const* /*pCommandName*/,
                                   Connection* pConnection,
                                   AnalyzeXML* /*pIncoming*/,
                                   soarxml::ElementXML* pResponse) {
...
static bool AddWmeChildrenToXML(AgentSML* pAgentSML, wme* pRoot,
                                soarxml::ElementXML* pTagResult,
  ...
  for (wme* w = pRoot->value->id->input_wmes; w != NIL; w = w->next) {
    ...
    if (w->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) {
      if (std::find(traversedList.begin(), traversedList.end(), w) ==
          traversedList.end()) {
...
// Send the current state of the input link back to the caller.  (This is not a
// commonly used method).
bool KernelSML::HandleGetAllInput(AgentSML* pAgentSML,
                                  char const* /*pCommandName*/,
                                  Connection* /*pConnection*/,
                                  AnalyzeXML* /*pIncoming*/,
                                  soarxml::ElementXML* pResponse) {
...
// Send the current state of the output link back to the caller.  (This is not a
// commonly used method).
bool KernelSML::HandleGetAllOutput(AgentSML* pAgentSML,
                                   char const* /*pCommandName*/,
                                   Connection* /*pConnection*/,
                                   AnalyzeXML* /*pIncoming*/,
                                   soarxml::ElementXML* pResponse) {
  // Build the SML message we're doing to send which in this case is an output
  // command (just like one you'd get if the agent was generating output rather
  // than being queried for its output link)
  ...
  output_link* ol =
      pSoarAgent->existing_output_links;  // This is technically a list but we
                                          // only support one output link
  ...
  TagWme* pOutputLinkWme =
      OutputListener::CreateTagWme(pAgentSML, ol->link_wme);
...
bool KernelSML::HandleInput(AgentSML* pAgentSML, char const* /*pCommandName*/,
                            Connection* /*pConnection*/, AnalyzeXML* pIncoming,
                            soarxml::ElementXML* /*pResponse*/) {
  ...
  // Echo back the list of wmes received, so other clients can see what's been
  // added (rarely used).
  pAgentSML->FireInputReceivedEvent(pCommand);

  if (kDebugInput) {
    sml::PrintDebugFormat("--------- %s ending input ----------",
                          pAgentSML->GetName());
  }
...
bool KernelSML::HandleCommandLine(AgentSML* pAgentSML, char const* pCommandName,
                                  Connection* pConnection,
                                  AnalyzeXML* pIncoming,
                                  soarxml::ElementXML* pResponse) {
#ifdef _DEBUG
  ...
  // If the user chooses to enable this feature, certain commands are always
  // echoed back. This is primarily to support two users connected to and
  // debugging the same kernel at once.
  ...
  // The caller can ask for simple string output (raw output) or more complex,
  // structured XML output which can then be parsed.
  soarxml::ElementXML const* pCommand = pIncoming->GetCommandTag();
  const char* pCommandOutput =
      pCommand->GetAttribute(sml_Names::kCommandOutput);
  ...
  // If there are no filters (or this command requested not to be filtered),
  // this copies the original line into the filtered line unchanged.
  ...
  if (!noFiltering && HasFilterRegistered()) {
    ...
    // If aliases need to be expanded before going to the filter, we can change
    // this then. Removed code that called removed function
    // m_CommandLineInterface.ExpandCommandToString

    /* The following is a new function to expand a command.  Did not use for
     * something else but it could solve a Tcl problem where soar aliases
     * prevent Tcl substitution) */
    //        std::string lCmd;
    //        lCmd = m_CommandLineInterface.ExpandCommand(pCommandName);
    //        pFunction = m_CommandMap[lCmd.c_str()];

    // We'll send the command over as an XML packet, so there's some structure
    // to work with. The current structure is: <filter command="command"
    // output="generated output" error="true | false"></filter> Each filter is
    // passed this string and can modify it as they go. All attributes are
    // optional although either command or output & error should exist. It's
    // possible, although unlikely that all 3 could exist at once (i.e. another
    // command to execute, yet still have output already)
    TagFilter filterXML;
    ...
    bool filtered =
        this->SendFilterMessage(pAgentSML, pXMLString, &filteredXML);

    // Clean up the XML message
    filterXML.DeleteString(pXMLString);

    // If a filter consumed the entire command, there's no more work for us to
    // do.
    ...
    if (filtered) {
      pFilteredXML =
          soarxml::ElementXML::ParseXMLFromString(filteredXML.c_str());
      if (!pFilteredXML) {
      ...
      char const* pFilteredOutput =
          pFilteredXML->GetAttribute(sml_Names::kFilterOutput);
      char const* pErr = pFilteredXML->GetAttribute(sml_Names::kFilterError);
      filteredError = (pErr && strcasecmp(pErr, "true") == 0);

      // See if the filter consumed the command.  If so, we just need to return
      // the output.
      if (!pFilteredLine || strlen(pFilteredLine) == 0) {
        // We may have no output defined and that's not an error so cover that
        // case
        ...
        // Can only clean this up after we're finished using it or pFilteredLine
        // will become invalid
        delete pFilteredXML;
  ...
  bool result = m_CommandLineInterface.DoCommand(
      pConnection, pAgentSML, pFilteredLine, echoResults, rawOutput, pResponse);

  if (kDebugCommandLine) {
    sml::PrintDebugFormat("Completed %s", pLine);
  }

  // Can only clean this up after we're finished using it or pFilteredLine will
  // become invalid
  delete pFilteredXML;
...
bool KernelSML::HandleGetListenerPort(AgentSML* /*pAgentSML*/,
                                      char const* /*pCommandName*/,
                                      Connection* pConnection,
                                      AnalyzeXML* /*pIncoming*/,
                                      soarxml::ElementXML* pResponse) {
...
bool KernelSML::HandleSVSInput(AgentSML* pAgentSML, char const* pCommandName,
                               Connection* pConnection, AnalyzeXML* pIncoming,
                               soarxml::ElementXML* pResponse) {
#ifndef NO_SVS
...
bool KernelSML::HandleSVSOutput(AgentSML* pAgentSML, char const* pCommandName,
                                Connection* pConnection, AnalyzeXML* pIncoming,
                                soarxml::ElementXML* pResponse) {
  std::string s;
#ifndef NO_SVS
...
bool KernelSML::HandleSVSQuery(AgentSML* pAgentSML, char const* pCommandName,
                               Connection* pConnection, AnalyzeXML* pIncoming,
                               soarxml::ElementXML* pResponse) {
  // Get the parameters
  char const* pLine = pIncoming->GetArgString(sml_Names::kParamLine);
