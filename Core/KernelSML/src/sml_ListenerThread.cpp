...
extern soarxml::ElementXML* ReceivedCall(Connection* pConnection,
...
void ListenerThread::Run() {
  ...
  if (ok) {
    // sml::PrintDebugFormat("Internet listener socket: %s",
    // m_ListenerSocket.GetName().c_str()) ;
  } else {
    sml::PrintDebug(
  ...
  if (ok) {
    // sml::PrintDebugFormat("Local listener socket: %s",
    // m_LocalListenerSocket.GetName().c_str()) ;
  } else {
    sml::PrintDebug(
        "Failed to create the local listener socket.  Shutting down thread.");
  ...
  if (ok) {
    // sml::PrintDebugFormat("Local listener pipe: %s",
    // m_ListenerNamedPipe.GetName().c_str()) ;
  } else {
    sml::PrintDebug(
        "Failed to create the listener pipe.  Shutting down thread.");
    m_ListenerSocket.Close();
    return;
  }
#endif
  ...
  /**
     ...
     %s", f_name ); int del_status = unlink( f_name ); if ( del_status == 0 )
          sml::PrintDebug( "Delete succeeded!" );
      else
          sml::PrintDebugFormat( "Error occurred during delete attempt, error
     code %d", static_cast<int64_t>(errno));
  */
#endif
#ifdef ENABLE_NAMED_PIPES
...
void ListenerThread::CreateConnection(DataSender* pSender) {
  // sml::PrintDebugFormat("Got new connection on %s",
  ...
  // Record our kernel object with this connection.  I think we only want one
  // kernel object even if there are many connections (because there's only one
  // kernel) so for now that's how things are set up.
