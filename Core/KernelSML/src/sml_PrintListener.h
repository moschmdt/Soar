...
class PrintListener : public EventManager<smlPrintEventId> {
 protected:
  const static int kNumberPrintEvents =
      smlEVENT_LAST_PRINT_EVENT - smlEVENT_FIRST_PRINT_EVENT + 1;
  KernelSML* m_pKernelSML;
  std::stringstream m_BufferedPrintOutput[kNumberPrintEvents];
  AgentOutputFlusher* m_pAgentOutputFlusher[kNumberPrintEvents];

  // When false we don't forward print callback events to the listeners. (Useful
  // when we're backdooring into the kernel)
  bool m_EnablePrintCallback;

 public:
  PrintListener() { m_pKernelSML = 0; }

  virtual ~PrintListener() { Clear(); }
  ...
  // Allows us to temporarily stop forwarding print callback output from the
  // kernel to the SML listeners
  void EnablePrintCallback(bool enable) { m_EnablePrintCallback = enable; }

  // Activate the print callback (flush output).  For echo events we want to
  // specify which connection triggered the event.
