    ...
    if (pAgentSML->IsAgentScheduledToRun()) {
      ...
      if (!pSynchAgent ||
          pAgent->GetNumDecisionCyclesExecuted() >
              pSynchAgent->GetIAgent()->GetNumDecisionCyclesExecuted() ||
          (pAgent->GetNumDecisionCyclesExecuted() ==
               pSynchAgent->GetIAgent()->GetNumDecisionCyclesExecuted() &&
           IsPhaseLater(pAgent->GetCurrentPhase(),
                        pSynchAgent->GetIAgent()->GetCurrentPhase()))) {
...
bool RunScheduler::IsAgentFinished(gSKI::IAgent* pAgent, AgentSML* pAgentSML,
  ...
  if (finished && runStepSize == gSKI_RUN_DECISION_CYCLE &&
...
void RunScheduler::InitializeRunCounters(
    egSKIRunType runStepSize, egSKIInterleaveType interleaveStepSize) {
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
       iter != m_pKernelSML->m_AgentMap.end(); iter++) {
    AgentSML* pAgentSML = iter->second;
...
void RunScheduler::InitializeUpdateWorldEvents(bool addListeners) {
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    pAgentSML->SetInitialOutputCount(
        pAgentSML->GetIAgent()->GetNumOutputsExecuted());
    ...
    if (addListeners) {
      ...
      // because the gSKI event handler has to increment counters on these
      // events. Added here for now because I couldn't reconcile the code for
      // making a gSKI agent a RunListener too.  (might be circular...)
      ...
      pAgent->AddRunListener(gSKIEVENT_AFTER_DECIDE_PHASE, this);
      pAgent->AddRunListener(gSKIEVENT_AFTER_APPLY_PHASE, this);
      pAgent->AddRunListener(gSKIEVENT_AFTER_OUTPUT_PHASE, this);
      pAgent->AddRunListener(gSKIEVENT_AFTER_PREFERENCE_PHASE,
                             this);  // Soar-7 mode only
      pAgent->AddRunListener(gSKIEVENT_AFTER_WM_PHASE,
                             this);  // Soar-7 mode only
    }
  }
}
...
bool RunScheduler::HaveAllGeneratedOutput() {
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    // By testing agents that are still scheduled to run this lets us more
    // closely mimic the regular "RunTilOutput" semantics--where agents either
    // drop out because they've generated output OR because they have reached a
    // maximum number of decisions (by default 15) without generating output.
    if (pAgentSML->IsAgentScheduledToRun() &&
...
void RunScheduler::CheckStopBeforePhase(egSKIRunType runStepSize) {
  ...
  // Just before this point, (end of while loop above) we've checked and fired
  // the OutputComplete and OutputGenerated events, so it should be fine to step
  ...
  // Output-Input Boundary together and generate events...  For now, if
  // interrupted, or error or halted, we won't do anything here.
...
void RunScheduler::HandleEvent(egSKIRunEventId eventID, gSKI::IAgent* pAgent,
  ...
  if (eventID == gSKIEVENT_AFTER_PHASE_EXECUTED && phase == gSKI_OUTPUT_PHASE) {
    ...
    if (AreAllOutputPhasesComplete()) {
      ...
      m_pKernelSML->FireUpdateListenerEvent(gSKIEVENT_AFTER_ALL_OUTPUT_PHASES,
                                            m_RunFlags);
...
void RunScheduler::TestForFiringGeneratedOutputEvent() {
  ...
  if (HaveAllGeneratedOutput()) {
    ...
    m_pKernelSML->FireUpdateListenerEvent(gSKIEVENT_AFTER_ALL_GENERATED_OUTPUT,
                                          m_RunFlags);
...
void RunScheduler::TerminateUpdateWorldEvents(bool removeListeners) {
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    if (removeListeners) {
      ...
      pAgent->RemoveRunListener(gSKIEVENT_AFTER_DECIDE_PHASE, this);
      pAgent->RemoveRunListener(gSKIEVENT_AFTER_APPLY_PHASE, this);
      pAgent->RemoveRunListener(gSKIEVENT_AFTER_OUTPUT_PHASE, this);
      pAgent->RemoveRunListener(gSKIEVENT_AFTER_PREFERENCE_PHASE,
                                this);  // Soar-7 mode only
      pAgent->RemoveRunListener(gSKIEVENT_AFTER_WM_PHASE,
                                this);  // Soar-7 mode only
    }
  }
}
...
bool RunScheduler::TestIfAllFinished(egSKIRunType runStepSize,
  ...
  for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
    ...
    bool agentFinishedRun =
        IsAgentFinished(pAgentSML->GetIAgent(), pAgentSML, runStepSize, count);
...
egSKIRunResult RunScheduler::RunScheduledAgents(
  ...
  // In general we really want to assert that runStepSize >= interleaveStepSize
  // but I'm not sure there's a strict relationship and certainly the enums
  // aren't labelled as being in a guaranteed processing size order. So for now,
  // we'll just check some specific cases. If we're running by elaboration
  // cycles, can't be running each agent by something bigger (like a phase).
  if (runStepSize == gSKI_RUN_ELABORATION_CYCLE) {
    assert(interleaveStepSize == gSKI_RUN_ELABORATION_CYCLE);
  }

  // If we want to synchronize the agents to the same phase, we need to run them
  // by phase (or they'll never synch)
  if (synchronize) {
  ...
  while (!runFinished) {
    ...
    // Use a flag to make sure we only fire the all generated event once per
    // step. Otherwise, if the last agent generates output and then stops
    // running we might try to fire it twice.
    m_AllGeneratedOutputEventFired = false;

    // Notify listeners that Soar is running.  This event is a kernel level
    // (agent manager) event which allows a single listener to check for client
    // driven interrupts for all agents. Sometimes that's easier to work with
    // than the agent specific events (where you get <n> events from <n> agents)
    ...
    for (AgentMapIter iter = m_pKernelSML->m_AgentMap.begin();
      ...
      // This is only true if we are in the process of synching up to one agent
      // and this agent is at the correct phase In that case we don't run this
      // agent for a step--which eventually means all agents will synch to the
      // same point.
      bool synched = (m_pSynchAgentSML != NULL &&
                      pAgentSML->GetIAgent()->GetCurrentPhase() ==
                          m_pSynchAgentSML->GetIAgent()->GetCurrentPhase());

      if (pAgentSML->IsAgentScheduledToRun() && !synched) {
        // Run all agents one "interleaveStepSize".
        gSKI::IAgent* pAgent = pAgentSML->GetIAgent();
        egSKIRunResult runResult =
            pAgent->RunInClientThread(interleaveStepSize, 1, pError);
        ...
        bool agentFinishedRun =
            IsAgentFinished(pAgent, pAgentSML, runStepSize, count);
        ...
        if (runState != gSKI_RUNSTATE_STOPPED || agentFinishedRun) {
          ...
          // If an agent stops running and the others have either stopped or
          // have all generated output we should fire the "ALL_GENERATED_OUTPUT"
          // event.
          TestForFiringGeneratedOutputEvent();
    ...
    // If we're synching to an agent (i.e. matching the phases of all agents)
    // see if we're done now.
    if (m_pSynchAgentSML) {
      // This isn't as optimally efficient as possible but only happens while
      // synching agents (just a few phases) and synching should be rare anyway.
