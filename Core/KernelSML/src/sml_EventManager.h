...
// This avoids having to use AgentSML in this file, which is a necessary hack
// because including sml_AgentSML.h currently breaks things.
void flushPrintOnAgent(AgentSML* pFlushPrintOnThisAgent);

template <typename EventType>
class EventManager : public KernelCallback {
 protected:
 ...
 public:
  virtual ~EventManager() {
    // This is interesting.  We can't call clear in the destructor for the event
    // manager because it calls virtual methods, which is apparently illegal (I
    // get a "pure virtual method" error from Visual Studio). So we'll need to
    // either move this to the derived classes destructors or call it explicitly
    // before we destroy the manager. Clear() ;
  }

  // Clear the map and release all listeners
  virtual void Clear() {
    for (EventMapIter mapIter = m_EventMap.begin(); mapIter != m_EventMap.end();
         mapIter++) {
      ...
      // the listener into the kernel will be removed (if RemoveListener for the
      // specific event class wishes to do so).
      while (pList->size() > 0) {
  ...
  // Returns true if this is the first connection listening for this event
  virtual bool BaseAddKernelListener(EventType eventID,
                                     KernelCallback* pKernelCallback) {
    pKernelCallback->RegisterWithKernel(eventID);
    return true;
  }

  // Returns true if this is the first connection listening for this event
  // (Generally calls BaseAddListener and then registers with the kernel for
  // this event)
  ...
  // Returns true if just removed the last listener
  // (Generally calls BaseRemoveListener and then unregisters with the kernel
  // for this event)
  ...
  // pCallbackAgent == 0 means that the event calling this does not want the
  // print event to be flushed
  virtual void SendEvent(AgentSML* pFlushPrintOnThisAgent,
                         Connection* pConnection, soarxml::ElementXML* pMsg,
                         AnalyzeXML* pResponse, ConnectionListIter begin,
                         ConnectionListIter end) {
#ifdef _DEBUG
