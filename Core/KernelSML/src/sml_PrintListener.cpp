...
bool PrintListener::AddListener(smlPrintEventId eventID,
  ...
  if (first) {
    ...
    // Register for specific events at which point we'll flush the buffer for
    // this event
    m_pAgentOutputFlusher[eventID - smlEVENT_FIRST_PRINT_EVENT] =
        new AgentOutputFlusher(this, GetAgentSML(), eventID);
  }
...
bool PrintListener::RemoveListener(smlPrintEventId eventID,
                                   Connection* pConnection) {
...
void PrintListener::OnKernelEvent(int eventID, AgentSML* pAgentSML,
                                  void* pCallData) {
  OnEvent(static_cast<smlPrintEventId>(eventID), pAgentSML,
          static_cast<const char*>(pCallData));
}

void PrintListener::OnEvent(smlPrintEventId eventID, AgentSML* pAgentSML,
                            const char* msg) {
  // We're assuming this is correct in the flush output function, so we should
  // check it here
  ...
  // on to the clients.  This allows us to use the print callback within the
  // kernel to retrieve information without it appearing in the trace.  (One day
  // we won't need to do this enable/disable game).
...
void PrintListener::FlushOutput(Connection* pSourceConnection,
  ...
  if (eventID != smlEVENT_ECHO) {
    ...
    // This allows us to tell a sender if they were the cause of a given message
    // so a client can filter out echoes from commands they originated.
    while (connectionIter != GetEnd(eventID)) {
      ...
      pConnection->AddParameterToSMLCommand(
          pMsg, sml_Names::kParamMessage,
          m_BufferedPrintOutput[buffer].str().c_str());
      pConnection->AddParameterToSMLCommand(pMsg, sml_Names::kParamSelf,
                                            (pSourceConnection == pConnection)
                                                ? sml_Names::kTrue
                                                : sml_Names::kFalse);
