...
#ifdef _MSC_VER
#pragma warning( \
    disable : 4702)  // warning C4702: unreachable code, need to disable for
                     // VS.NET 2003 due to STL "bug" in certain cases
#endif
#include <list>
#include <map>
#ifdef _MSC_VER
...
class KernelCallback {
 ...
 public:
  static void KernelCallbackStatic(agent* pAgent, int eventID, void* pData,
                                   void* pCallData);
  ...
  virtual void OnKernelEvent(int eventID, AgentSML* pAgentSML,
                             void* pCallData) = 0;
};
