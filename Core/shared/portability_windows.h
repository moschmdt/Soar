...
// disable min/max macros -- fixes an issue where some RHS functions use a
// struct that has fields by these names, so the initializer gets interpreted as
// a macro call
#define NOMINMAX
...
// Use named pipes instead of sockets for same-machine interprocess
// communication
...
#if defined(_WIN64)
// on 64 bit, removes warning C4985: 'ceil': attributes not present on previous
// declaration. see
// http://connect.microsoft.com/VisualStudio/feedback/ViewFeedback.aspx?FeedbackID=294649
#include <math.h>
#endif  // _WIN64
...
static inline int set_working_directory_to_executable_path() {
  ...
  if (!length) fprintf(stderr, "Failed to set working directory.\n");
...
inline double get_raw_time_per_usec() {
  ...
  if (QueryPerformanceFrequency(&freq)) {
    return freq.QuadPart /
           1e6;  // freq.QuadPart is in counts per sec, convert to per usec
  } else {
    return 10.0;
  }
}
...
inline bool stdout_supports_ansi_colors() {
  ...
  if (!GetConsoleMode(h, &mode)) {
    fprintf(stderr,
            "Error determining console mode: %d. Assuming ansi colors not "
            "supported.\n",
            GetLastError());
    return false;
  }
