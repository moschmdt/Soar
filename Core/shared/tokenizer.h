...
class tokenizer_current {
 ...
 public:
   ...
   */
  tokenizer_current(const char* initial) { set_current(initial); }
...
class tokenizer {
 private:
  tokenizer_current
      current;  ///< A smart index to the current position in the stream.
 ...
 public:
   ...
   * @return The current offset.
   */
  int get_offset() { return current.get_offset(); }
  ...
  void parse_command() {
    ...
    if (callback) {
      if (((last_arg == "sp") && (argv[0] != "sp")) ||
          ((last_arg == "gp") && (argv[0] != "gp"))) {
        const char* echo_args[] = {"echo", " "};
        std::vector<std::string> echo_command(
            echo_args, echo_args + sizeof(echo_args) / sizeof(echo_args[0]));
  ...
  char parse_escape_sequence() {
    ...
    switch (current.get()) {
      ...
      case '\n':
        ...
        ret = ' ';
        increment =
            false;  // skip_whitespace leaves us past where we want to be
        break;
      default:
        ret = current.get();
        break;
