...
#if (__GNUC__ > 4 ||                                                      \
     (__GNUC__ == 4 && (__GNUC_MINOR__ > 2 || (__GNUC_MINOR__ == 2)))) && \
...
static inline int set_working_directory_to_executable_path() {
  ...
  length = _NSGetExecutablePath(application_path, &size)
               ? -1
...
inline uint64_t get_raw_time() { return mach_absolute_time(); }
...
inline bool stdout_supports_ansi_colors() { return isatty(STDOUT_FILENO); }
