...
template <class T>
bool from_string(T& val, const char* const pStr) {
  return from_string(val, std::string(pStr));
}

template <class T>
inline T cast_and_possibly_truncate(void* ptr) {
...
// TO
const size_t TO_C_STRING_BUFSIZE =
    24;  // uint64: 18446744073709551615 plus a few extra
 ...
 * in many compilers, complaining that it isn't ISO C++.
 *
 *  This function doesn't really do anything to guarantee that the provided
 * types are pointers, so use it appropriately.
 */
...
class soar_timer {
 public:
  soar_timer() : t1(0), elapsed(0), enabled_ptr(NULL) {
    raw_per_usec = get_raw_time_per_usec();
  }

  void set_enabled(bool* new_enabled) { enabled_ptr = new_enabled; }
