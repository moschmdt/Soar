...
class EXPORT SoarAssertionException : public std::exception {
 public:
   ...
   *                 with the caller.
   */
  explicit SoarAssertionException(const char* message, const char* file,
                                  const int line);

  /** Constructor (C++ STL strings).
   *  @param message The error message.
   */
  explicit SoarAssertionException(const std::string& message, const char* file,
                                  const int line);
...
#define assert(X)                 \
  if (!(X))                       \
    throw SoarAssertionException( \
        "Failed assertion: '" + std::string(#X) + "'", __FILE__, __LINE__);
