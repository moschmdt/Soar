...
bool CommandLineInterface::DoLoad(std::vector<std::string> &argv,
  ...
  soar_module::param *my_param =
      thisAgent->command_params->load_params->get(pCmd.c_str());
  if (!my_param) {
    return SetError(
        "Invalid load command.  Use 'load ?' to see a list of valid settings.");
  }
  ...
  } else if (my_param == thisAgent->command_params->load_params->check_cmd) {
  ...
  } else if ((my_param == thisAgent->command_params->load_params->help_cmd) ||
             (my_param == thisAgent->command_params->load_params->qhelp_cmd)) {
    thisAgent->command_params->load_params->print_settings(thisAgent);
  }
  return false;
}

bool CommandLineInterface::AddSaveSetting(bool pShouldAdd,
                                          const char *pAddString) {
  if (!pShouldAdd)
    return true;
  std::string *err = new std::string(pAddString);
  if (!DoCLog(LOG_ADD, 0, err, true))
    return false;
  return true;
}

bool CommandLineInterface::AddSaveSettingOnOff(bool pIsOn,
                                               const char *pAddString) {
  std::string *err = new std::string(pAddString);
...
bool CommandLineInterface::AddSaveSettingInt(const char *pAddString,
                                             const uint64_t pInt) {
  agent *thisAgent = m_pAgentSML->GetSoarAgent();
  std::string *err = new std::string(pAddString);
...
bool CommandLineInterface::DoSave(std::vector<std::string> &argv,
                                  const std::string &pCmd) {
  agent *thisAgent = m_pAgentSML->GetSoarAgent();
  ...
  soar_module::param *my_param =
      thisAgent->command_params->save_params->get(pCmd.c_str());
  if (!my_param) {
    return SetError(
        "Invalid save command.  Use 'save ?' to see a list of valid settings.");
  }
  if (my_param == thisAgent->command_params->save_params->input_cmd) {
    ...
    if ((argv.size() < 3) || (argv.size() > 3)) {
      ...
      if (DoCommandToFile(LOG_NEW, lFile, lCmdVector)) {
        thisAgent->outputManager->printa_sf(
            thisAgent, "Chunks written to file %s.\n", lFile.c_str());
        return true;
      } else {
    ...
    if ((argv.size() < 3) || (argv.size() > 3)) {
      ...
      if (!DoCLog(LOG_NEW, &lFile, 0, true))
        return false;

      /* Save various settings that may be required to run properly.  These
       * aren't exhaustive */
      {
        AddSaveText("# Settings\n");
        if (!AddSaveSetting(thisAgent->SMem->enabled(), "smem -e"))
          return false;
        if (!AddSaveSetting(epmem_enabled(thisAgent), "epmem -e"))
          return false;
        if (!AddSaveSetting(thisAgent->explanationBasedChunker
                                ->ebc_settings[SETTING_EBC_ALWAYS],
                            "chunk always"))
          return false;
        if (!AddSaveSetting(thisAgent->explanationBasedChunker
                                ->ebc_settings[SETTING_EBC_NEVER],
                            "chunk never"))
          return false;
        if (!AddSaveSetting(thisAgent->explanationBasedChunker
                                ->ebc_settings[SETTING_EBC_ONLY],
                            "chunk only"))
          return false;
        if (!AddSaveSetting(thisAgent->explanationBasedChunker
                                ->ebc_settings[SETTING_EBC_EXCEPT],
                            "chunk except"))
          return false;
        if (!AddSaveSettingOnOff(thisAgent->explanationBasedChunker
                                     ->ebc_settings[SETTING_EBC_BOTTOM_ONLY],
                                 "chunk bottom-only"))
          return false;
        if (!AddSaveSettingOnOff(thisAgent->explanationBasedChunker
                                     ->ebc_settings[SETTING_EBC_ADD_OSK],
                                 "chunk add-osk"))
          return false;
        if (!AddSaveSettingOnOff(
                thisAgent->explanationBasedChunker
                    ->ebc_settings[SETTING_EBC_ALLOW_LOCAL_NEGATIONS],
                "chunk allow-local-negations"))
          return false;
        if (!AddSaveSettingOnOff(thisAgent->explanationBasedChunker
                                     ->ebc_settings[SETTING_EBC_ADD_LTM_LINKS],
                                 "chunk add-ltm-links"))
          return false;
        if (!AddSaveSettingInt("chunk max-chunks",
                               thisAgent->explanationBasedChunker->max_chunks))
          return false;
        if (!AddSaveSettingInt("chunk max-dupes",
                               thisAgent->explanationBasedChunker->max_dupes))
          return false;
        if (!AddSaveSettingInt(
                "soar max-elaborations",
                thisAgent->Decider->settings[DECIDER_MAX_ELABORATIONS]))
          return false;
        if (!AddSaveSettingInt(
                "soar max-goal-depth",
                thisAgent->Decider->settings[DECIDER_MAX_GOAL_DEPTH]))
          return false;
        if (!AddSaveSettingOnOff(thisAgent->Decider->settings[DECIDER_WAIT_SNC],
                                 "soar wait-snc"))
          return false;
      }
      ...
      if (!DoCommandToFile(LOG_NEWAPPEND, lFile, lCmdVector))
        return false;

      /* Save semantic memory */
      if (!DoCLog(LOG_NEWAPPEND, &lFile, 0, true))
        return false;
      if (thisAgent->SMem->enabled())
        thisAgent->SMem->attach();
      if (thisAgent->SMem->connected() &&
          (thisAgent->SMem->statistics->nodes->get_value() > 0)) {
      ...
      //            } else {
      //                AddSaveText("# Episodic memory is not enabled.  Did not
      //                save.");
      //            }
      ...
      if (!DoCLog(LOG_CLOSE, 0, 0, true))
        return false;
      PrintCLIMessage(
          "Procedural memory, semantic memory and settings written to file.");
      delete err;
      return result;
    }
  } else if ((my_param == thisAgent->command_params->save_params->help_cmd) ||
             (my_param == thisAgent->command_params->save_params->qhelp_cmd)) {
...
bool CommandLineInterface::ParseReplayInput(std::vector<std::string> &argv) {
  cli::Options opt;
  OptionsData optionsData[] = {{'c', "close", OPTARG_NONE},
...
bool CommandLineInterface::ParseSource(std::vector<std::string> &argv) {
  ...
  if (opt.GetNonOptionArguments() < 2) {
    return SetError(
        "Syntax: load file [--all | --disable | --verbose] <filename>");
  } else if (opt.GetNonOptionArguments() > 3) {
    return SetError("Please supply one file to source. If there are spaces in "
                    "the path, enclose it in quotes.");
  }

  return DoSource(argv[opt.GetArgument() - opt.GetNonOptionArguments() + 1],
                  &options);
}
bool CommandLineInterface::ParseCheck(std::vector<std::string> &argv) {
  cli::Options opt;
  OptionsData optionsData[] = {{0, 0, OPTARG_NONE}};
  ...
  if (opt.GetNonOptionArguments() < 2) {
    return SetError("Syntax: load check <filename>");
  } else if (opt.GetNonOptionArguments() > 2) {
    return SetError("Please supply one file to check. If there are spaces in "
                    "the path, enclose it in quotes.");
  }

  return DoCheck(argv[opt.GetArgument() - opt.GetNonOptionArguments() + 1]);
}
bool CommandLineInterface::ParseReteLoad(std::vector<std::string> &argv) {
  cli::Options opt;
  OptionsData optionsData[] = {{'l', "load", OPTARG_REQUIRED},
...
bool CommandLineInterface::ParseCaptureInput(std::vector<std::string> &argv) {
  ...
  return DoCaptureInput(mode, autoflush,
                        mode == cli::CAPTURE_INPUT_OPEN ? &pathname : 0);
}
...
bool CommandLineInterface::DoReplayInput(eReplayInputMode mode,
                                         std::string *pathname) {
  switch (mode) {
  ...
  case REPLAY_INPUT_OPEN:
    ...
    m_Result << "Loaded " << m_pAgentSML->NumberOfCapturedActions()
             << " actions.";
    break;
...
bool CommandLineInterface::DoCaptureInput(eCaptureInputMode mode,
                                          bool autoflush,
                                          std::string *pathname) {
  switch (mode) {
...
void CommandLineInterface::PrintSourceSummary(
    int sourced, const std::list<std::string> &excised, int ignored) {
  if (!m_SourceFileStack.empty()) {
    AppendArgTagFast(sml_Names::kParamFilename, sml_Names::kTypeString,
                     m_SourceFileStack.top());
  }
  std::string temp;
  AppendArgTag(sml_Names::kParamSourcedProductionCount, sml_Names::kTypeInt,
               to_string(sourced, temp));
  AppendArgTag(sml_Names::kParamExcisedProductionCount, sml_Names::kTypeInt,
               to_string(excised.size(), temp));
  AppendArgTag(sml_Names::kParamIgnoredProductionCount, sml_Names::kTypeInt,
               to_string(ignored, temp));
  ...
  if (m_RawOutput) {
    ...
    m_Result << ": " << sourced << " production"
             << ((sourced == 1) ? " " : "s ") << "sourced.";

    if (!excised.empty()) {
      m_Result << " " << excised.size() << " production"
               << ((excised.size() == 1) ? " " : "s ") << "excised.";
...
bool CommandLineInterface::DoSource(std::string path, SourceBitset *pOptions) {
  ...
  if (lastSeparator == std::string::npos) {
    ...
    if (lastSeparator < path.length()) {
      ...
      filename.assign(
          path.substr(lastSeparator, path.length() - lastSeparator));
    }
  }
  ...
  if (m_pSourceOptions && m_pSourceOptions->test(SOURCE_VERBOSE)) {
    if (m_RawOutput) {
      ...
      outString.append(".\n");
      AppendArgTagFast(sml_Names::kParamChunkNamePrefix, sml_Names::kTypeString,
                       outString);
    }
  }
  ...
  if (m_pSourceOptions && m_pSourceOptions->test(SOURCE_ALL)) {
    PrintSourceSummary(m_NumProductionsSourced, m_ExcisedDuringSource,
  ...
  m_TotalExcisedDuringSource.insert(m_TotalExcisedDuringSource.end(),
                                    m_ExcisedDuringSource.begin(),
                                    m_ExcisedDuringSource.end());
  ...
  if (m_SourceFileStack.empty()) {
    ...
    if (m_pSourceOptions && !m_pSourceOptions->test(SOURCE_DISABLE)) {
      PrintSourceSummary(m_NumTotalProductionsSourced,
                         m_TotalExcisedDuringSource,
                         m_NumTotalProductionsIgnored);
    }
...
bool CommandLineInterface::DoCheck(std::string path) {
  // Parse the file without permanently loading productions.
  // Uses the same DoSource machinery, but DoSP will excise each
  // successfully parsed production immediately when m_CheckMode is set.
  m_CheckMode = true;
  bool ret = DoSource(path, nullptr);
  m_CheckMode = false;

  if (ret) {
    if (m_RawOutput) {
      m_Result << "load check: '" << path << "' parsed successfully ("
               << m_NumTotalProductionsSourced << " production(s)).\n";
    }
  }
  // On failure DoSource already set the error via SetError.
  return ret;
}
...
bool CommandLineInterface::Source(const char *buffer, bool printFileStack) {
  ...
  if (sourceError.empty()) {
    // It's important that we don't return an empty error string, as that will
