...
bool CommandLineInterface::DoGP(const std::string& productionString) {
  ...
  for (pos = productionString.find_first_of(targets, searchpos);
       pos != std::string::npos;
       pos = productionString.find_first_of(targets, searchpos)) {
    switch (productionString[pos]) {
      case '\\':  // skip backslashes

        // if the escaped char is a space, consume the space and drop the
        // backslash
        if (productionString[pos + 1] == ' ') {
          currentValueToken +=
              productionString.substr(searchpos, (pos - searchpos));
          currentValueToken += productionString.substr(
              pos + 1, 1);  // we could just append a space here, but this form
                            // lets us easily add other characters we might want
                            // to append like this
        } else              // otherwise consume it and the escaped char
        {
          currentValueToken +=
              productionString.substr(searchpos, (pos - searchpos) + 2);
        }
        searchpos = pos + 2;
        break;
      ...
      case '|':  // note pipe
        ...
        // consume it
        currentValueToken +=
            productionString.substr(searchpos, (pos - searchpos) + 1);
        searchpos = pos + 1;

        if (!pipe && inValues) {
          if (currentValueToken
                  .size())  // I don't think this is necessary since it should
                            // always contain at least the pipes
          {
            // tokenize
      ...
      case '[':  // start of value list
        if (!pipe) {
          // make sure there is not a square bracket immediately following this
          // one
  ...
  if (m_GPMax != 0) {
    if (total > m_GPMax) {
      std::ostringstream message;
      message << "gp-max exceeded, current production produces " << total
              << " productions.";
      return SetError(message.str());
    }
  }
  ...
  // for ( std::list< valueCollection >::iterator topIter = topLevel.begin(),
  // endIter = topLevel.end(); topIter != endIter; ++topIter )
  //{
  //    std::cout << "~~~" << std::endl;
  //    for ( valueCollection::iterator valueIter = topIter->begin(); valueIter
  //    != topIter->end(); ++valueIter )
  //    {
  //        std::cout << "%" << *valueIter << "% ";
  //    }
  //    std::cout << std::endl;
  ...
  // one that points to the current value, and one that represents the end of
  // the value collection
  std::list<iterTriple> valueIters;
  for (std::list<valueCollection>::iterator topIter = topLevel.begin(),
                                            endIter = topLevel.end();
       topIter != endIter; ++topIter) {
    iterTriple it;
  ...
  while (!finished) {
    ...
    for (std::list<iterTriple>::iterator valueItersIter = valueIters.begin(),
                                         endIter = valueIters.end();
         valueItersIter != endIter; ++valueItersIter) {
      generatedProduction += *(valueItersIter->current);
      // if this is the first part of the production,
      // find the end of the production name and add a number to differentiate
      // it
    ...
    // std::cout << std::endl << "++++++" << std::endl << generatedProduction <<
    // std::endl << "++++++" << std::endl;
    ...
    for (std::list<iterTriple>::iterator valueItersIter = valueIters.begin(),
      ...
      // Otherwise we're at a new value, so break out and generate a new
      // production.
