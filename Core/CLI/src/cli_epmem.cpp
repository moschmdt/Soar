...
bool CommandLineInterface::DoEpMem(const char pOp, const std::string* pAttr,
  ...
  if (!pOp || (pOp == '?')) {
    ...
    PrintCLIMessage_Item("learning:", thisAgent->EpMem->epmem_params->learning,
                         40);
    PrintCLIMessage_Section("Encoding", 40);
    PrintCLIMessage_Item("phase:", thisAgent->EpMem->epmem_params->phase, 40);
    PrintCLIMessage_Item("trigger:", thisAgent->EpMem->epmem_params->trigger,
                         40);
    PrintCLIMessage_Item("force:", thisAgent->EpMem->epmem_params->force, 40);
    PrintCLIMessage_Item(
        "exclusions:", thisAgent->EpMem->epmem_params->exclusions, 40);
    PrintCLIMessage_Section("Storage", 40);
    PrintCLIMessage_Item("database:", thisAgent->EpMem->epmem_params->database,
                         40);
    PrintCLIMessage_Item("append:", thisAgent->EpMem->epmem_params->append_db,
                         40);
    PrintCLIMessage_Item("path:", thisAgent->EpMem->epmem_params->path, 40);
    PrintCLIMessage_Item(
        "lazy-commit:", thisAgent->EpMem->epmem_params->lazy_commit, 40);
    PrintCLIMessage_Section("Retrieval", 40);
    PrintCLIMessage_Item("balance:", thisAgent->EpMem->epmem_params->balance,
                         40);
    PrintCLIMessage_Item(
        "graph-match:", thisAgent->EpMem->epmem_params->graph_match, 40);
    PrintCLIMessage_Item("graph-match-ordering:",
                         thisAgent->EpMem->epmem_params->gm_ordering, 40);
    PrintCLIMessage_Section("Performance", 40);
    PrintCLIMessage_Item(
        "page-size:", thisAgent->EpMem->epmem_params->page_size, 40);
    PrintCLIMessage_Item(
        "cache-size:", thisAgent->EpMem->epmem_params->cache_size, 40);
    PrintCLIMessage_Item("optimization:", thisAgent->EpMem->epmem_params->opt,
                         40);
    ...
    if (!result) {
      SetError(
          "This parameter is protected while the episodic memory database is "
          "open.");
    } else {
    ...
    soar_module::param* my_param =
        thisAgent->EpMem->epmem_params->get(pAttr->c_str());
    if (!my_param) {
    ...
    if ((thisAgent->EpMem->epmem_params->database->get_value() !=
      ...
      PrintCLIMessage(
          "Note: There was no effective change to episodic memory contents \n"
          "      because Soar is storing episodic memory to a database file "
          "and append \n"
          "      mode is on.");
    }
    return true;
    ...
    epmem_param_container::db_choices last_db_mode =
        thisAgent->EpMem->epmem_params->database->get_value();
    ...
    if (!result) {
      ...
      if (thisAgent->EpMem->epmem_db->get_status() == soar_module::connected) {
        if (((!strcmp(pAttr->c_str(), "database")) &&
             (thisAgent->EpMem->epmem_params->database->get_value() !=
              last_db_mode)) ||
            (!strcmp(pAttr->c_str(), "path"))) {
          PrintCLIMessage(
              "To finalize episodic memory database switch, issue an epmem "
              "--init command.\n");
        }
      }
      if (!strcmp(pAttr->c_str(), "append")) {
        if (thisAgent->EpMem->epmem_params->append_db->get_value() == off) {
          PrintCLIMessage(
              "Warning: Since append mode is off, starting/reinitializing,\n"
    ...
    if (!pAttr) {
      ...
      PrintCLIMessage_Item(
          "SQLite Version:", thisAgent->EpMem->epmem_stats->db_lib_version, 40);
      PrintCLIMessage_Item(
          "Memory Usage:", thisAgent->EpMem->epmem_stats->mem_usage, 40);
      PrintCLIMessage_Item(
          "Memory Highwater:", thisAgent->EpMem->epmem_stats->mem_high, 40);
      PrintCLIMessage_Item("Retrievals:", thisAgent->EpMem->epmem_stats->ncbr,
                           40);
      ...
      PrintCLIMessage_Item(
          "Last Retrieval WMEs:", thisAgent->EpMem->epmem_stats->ncb_wmes, 40);
      PrintCLIMessage_Item(
          "Last Query Positive:", thisAgent->EpMem->epmem_stats->qry_pos, 40);
      PrintCLIMessage_Item(
          "Last Query Negative:", thisAgent->EpMem->epmem_stats->qry_neg, 40);
      PrintCLIMessage_Item(
          "Last Query Retrieved:", thisAgent->EpMem->epmem_stats->qry_ret, 40);
      PrintCLIMessage_Item("Last Query Cardinality:",
                           thisAgent->EpMem->epmem_stats->qry_card, 40);
      PrintCLIMessage_Item(
          "Last Query Literals:", thisAgent->EpMem->epmem_stats->qry_lits, 40);
    } else {
      // check attribute name
      soar_module::statistic* my_stat =
          thisAgent->EpMem->epmem_stats->get(pAttr->c_str());
      if (!my_stat) {
    ...
    if (!pAttr) {
      ...
      soar_module::timer* my_timer =
          thisAgent->EpMem->epmem_timers->get(pAttr->c_str());
      if (!my_timer) {
