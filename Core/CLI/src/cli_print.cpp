...
void print_stack_trace(agent* thisAgent, bool print_states,
                       bool print_operators) {
  // We don't want to keep printing forever (in case we're in a state no change
  // cascade).
  ...
  for (Symbol* g = thisAgent->top_goal; g != NIL; g = g->id->lower_goal) {
    ...
    if (print_operators && g->id->operator_slot->wmes) {
      print_stack_trace(thisAgent, g->id->operator_slot->wmes->value, g,
                        FOR_OPERATORS_TF, false);
      thisAgent->outputManager->printa_sf(thisAgent, "\n");
    }
  }
  ...
  if (stateCount > maxStates) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "...Stack goes on for another %d states\n",
        static_cast<int64_t>(stateCount - maxStates));
  }
}

void do_print_for_production(agent* thisAgent, production* prod, bool intern,
  ...
  if (full_prod) {
    ...
    if (prod->rl_rule) {
      ...
      if (thisAgent->RL->rl_params->decay_mode->get_value() ==
          rl_param_container::delta_bar_delta_decay) {
        thisAgent->outputManager->printa_sf(
            thisAgent, " %y",
      ...
      thisAgent->outputManager->printa_sf(
          thisAgent, " %y",
          thisAgent->symbolManager->make_float_constant(prod->rl_update_count));
      thisAgent->outputManager->printa_sf(
          thisAgent, " %y", rhs_value_to_symbol(prod->action_list->referent));
    }
  }
  thisAgent->outputManager->printa_sf(thisAgent, "\n");
}

void print_productions_of_type(agent* thisAgent, bool intern,
                               bool print_filename, bool full_prod,
...
void print_rl_rules(agent* thisAgent, bool intern, bool print_filename,
                    bool full_prod) {
  assert(thisAgent);
  ...
  for (production* prod =
       ...
       prod != NIL; prod = prod->next) {
    if (prod->rl_rule) {
      do_print_for_production(thisAgent, prod, intern, print_filename,
                              full_prod);
    }
  }
...
void neatly_print_wme_augmentation_of_id(agent* thisAgent, wme* w,
                                         int indentation) {
...
// print_augs_of_id
//           Thus, this should probably only be called from
//           do_print_for_identifier
void print_augs_of_id(agent* thisAgent, Symbol* id, int depth, int maxdepth,
                      bool intern, bool tree, tc_number tc) {
  slot* s;
  wme* w;
  ...
  if (id->id->depth > depth) {
    return;  // this can be reached via an equal or shorter path, so return
             // without printing RPM 4/07 bug 988
  }
  ...
  int indent = (maxdepth - depth) *
               2;  // set the indent based on how deep we are, RPM 4/07 bug 988
  ...
  if (tree) {
    ...
    for (attr = 0; attr < num_attr; attr++) {
      ...
      if (intern) {
        ...
        // XML format of an <id> followed by a series of <wmes> each of which
        // shares the original ID. <id id="s1"><wme tag="123" attr="foo"
        // attrtype="string" val="123" valtype="string"></wme><wme attr="bar"
        // ...></wme></id>
...
shallowest depth That is, if an id can be reached by multiple paths, this will
find the shortest one and save the depth of that path on the id.  Thus, when we
print, the wmes will be indented properly, making it much easier to read, and
avoiding bugs (see bug 988).
*/
void mark_depths_augs_of_id(agent* thisAgent, Symbol* id, int depth,
                            tc_number tc) {
  slot* s;
  wme* w;
  ...
  if (id->tc_num == tc && id->id->depth >= depth) {
    return;  // this has already been printed at an equal-or-lower depth, RPM
             // 4/07 bug 988
  }
...
void do_print_for_identifier(agent* thisAgent, Symbol* id, int depth,
                             bool intern, bool tree) {
  tc_number tc;
...
void do_print_for_production_name(agent* thisAgent, soar::Lexeme* lexeme,
                                  const char* prod_name, bool intern,
                                  bool print_filename, bool full_prod) {
  Symbol* sym;

  sym = thisAgent->symbolManager->find_str_constant(lexeme->string());
  if (sym && sym->sc->production) {
    do_print_for_production(thisAgent, sym->sc->production, intern,
...
void do_print_for_wme(agent* thisAgent, wme* w, int depth, bool intern,
                      bool tree) {
  if (intern && (depth == 0)) {
    print_wme(thisAgent, w);
    thisAgent->outputManager->printa_sf(thisAgent, "\n");
...
int read_pattern_component(agent* thisAgent, soar::Lexeme* lexeme,
  ...
  switch (lexeme->type) {
    ...
    case FLOAT_CONSTANT_LEXEME:
      *dest_sym =
          thisAgent->symbolManager->find_float_constant(lexeme->float_val);
      return 2;
    case IDENTIFIER_LEXEME:
      *dest_sym = thisAgent->symbolManager->find_identifier(lexeme->id_letter,
                                                            lexeme->id_number);
      return 2;
    ...
    default:
      thisAgent->outputManager->printa_sf(
          thisAgent, "Expected identifier or constant in wme pattern\n");
      return 0;
  }
}

cons* read_pattern_and_get_matching_wmes(agent* thisAgent,
                                         const char* pattern) {
  ...
  if (lexer.current_lexeme.type != L_PAREN_LEXEME) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "Expected '(' to begin wme pattern not string '%s' or char '%c'\n",
  ...
  if (lexer.current_lexeme.type != UP_ARROW_LEXEME) {
    thisAgent->outputManager->printa_sf(thisAgent,
                                        "Expected ^ in wme pattern\n");
    lexer.skip_ahead_to_balanced_parentheses(parentheses_level - 1);
    return NIL;
  }
  lexer.get_lexeme();
  attr_result =
      read_pattern_component(thisAgent, &(lexer.current_lexeme), &attr);
  if (!attr_result) {
    lexer.skip_ahead_to_balanced_parentheses(parentheses_level - 1);
    return NIL;
  }
  lexer.get_lexeme();
  value_result =
      read_pattern_component(thisAgent, &(lexer.current_lexeme), &value);
  if (!value_result) {
    lexer.skip_ahead_to_balanced_parentheses(parentheses_level - 1);
    return NIL;
  }
...
void print_symbol(agent* thisAgent, const char* arg, bool print_filename,
                  bool intern, bool tree, bool full_prod, int depth,
                  bool exact) {
  cons* c;
  Symbol* id;
  wme* w;
  ...
  switch (lexeme.type) {
      ...
      if (lexeme.string()[0] == '@') {
        ...
        if (lexeme.string()[1] != '\0') {
          ...
          if (lLti_id) {
            ...
            if (lLti_id == NIL) {
              thisAgent->outputManager->printa_sf(
                  thisAgent, "LTI %s not found in semantic memory.",
                  lexeme.string());
              break;
            }
          }
        ...
        if (lLti_id == NIL) {
          ...
          thisAgent->SMem->print_smem_object(lLti_id, depth,
                                             &(smem_print_output));
        }
        thisAgent->outputManager->printa(thisAgent, smem_print_output.c_str());
      } else {
        do_print_for_production_name(thisAgent, &lexeme, arg, intern,
                                     print_filename, full_prod);
      }
      break;
    ...
    case INT_CONSTANT_LEXEME:
      ...
      if (!w) {
        thisAgent->outputManager->printa_sf(
            thisAgent, "No wme %d in working memory.", lexeme.int_val);
      }
      break;
    ...
    case QUOTED_STRING_LEXEME: {
      ...
      if (exact) {
        ...
        while (iter != objects.end()) {
          ...
          while (wmeiter != wmelist.end()) {
            ...
            if (intern) {
              ...
              // taken from print_wme_without_timetag
              thisAgent->outputManager->printa_sf(thisAgent, " ^%y %y", w->attr,
                                                  w->value);
              if (w->acceptable) {
                thisAgent->outputManager->printa(thisAgent, " +");
              }
        ...
        for (c = wmes; c != NIL; c = c->rest) {
          do_print_for_wme(thisAgent, static_cast<wme*>(c->first), depth,
                           intern, tree);
        }
      }
...
bool CommandLineInterface::DoPrint(PrintBitset options, int depth,
  ...
  if (options.test(PRINT_STACK)) {
    ...
    print_stack_trace(thisAgent, options.test(PRINT_STATES),
                      options.test(PRINT_OPERATORS));
    return true;
  }
  ...
  if (pArg) {
    ...
    // Watch out, this case handles all args
    // identifier/pattern/timetag/production_name
    m_VarPrint = options.test(PRINT_VARPRINT);  // This is a member because it
                                                // affects the print callback.
    print_symbol(thisAgent, pArg->c_str(), filename, intern, tree, full, depth,
                 exact);
    m_VarPrint = false;
    return true;
  }
  ...
  if (options.test(PRINT_ALL) ||
      (!options.test(PRINT_CHUNKS) && !options.test(PRINT_DEFAULTS) &&
       ...
       !options.test(PRINT_RL) && !options.test(PRINT_TEMPLATE))) {
    options.set(PRINT_CHUNKS);
    options.set(PRINT_DEFAULTS);
    options.set(PRINT_JUSTIFICATIONS);
    options.set(PRINT_USER);
    // options.set(PRINT_RL);     // these aren't really a different "type",
    // it's just extra data
    options.set(PRINT_TEMPLATE);
  }

  if (options.test(PRINT_DEFAULTS)) {
    print_productions_of_type(thisAgent, intern, filename, full,
