...
bool CommandLineInterface::DoPWD() {
  ...
  if (directory.size()) {
    if (m_RawOutput) {
      ...
      AppendArgTagFast(sml_Names::kParamDirectory, sml_Names::kTypeString,
                       directory);
    }
  }
...
bool CommandLineInterface::DoLS() {
  ...
  do {
    ...
    PrintFilename(FindFileData.cFileName,
...
void CommandLineInterface::PrintFilename(const std::string& name,
                                         bool isDirectory) {
  if (m_RawOutput) {
    if (isDirectory) {
      m_Result << '[';
