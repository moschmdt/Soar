...
class Options {
 public:
      ...
      if (argv[m_Argument][0] == '-') {
        if (argv[m_Argument][1] == '-') {
          ...
          if (argv[m_Argument].size() > 2) {
                ...
                while (liter != possibilities.end()) {
                  if ((*liter).longOpt[index] != longOption[index]) {
                    ...
                    if (longOption == (*liter).longOpt) {
                      ...
                      if (!HandleOptionArgument(argv, liter->longOpt,
                                                liter->argument)) {
                        return false;
                      }
                      return true;
            ...
            if (possibilities.size() != 1) {
              ...
              for (liter = possibilities.begin(); liter != possibilities.end();
                   ++liter) {
                error.append("'--");
            ...
            MoveBack(argv, m_Argument, m_NonOptionArguments);
            if (!HandleOptionArgument(argv, (*(possibilities.begin())).longOpt,
                                      (*(possibilities.begin())).argument)) {
              return false;
            }
            return true;
          }
        ...
        for (int i = 0; options[i].shortOpt != 0; ++i) {
          if (argv[m_Argument][1] == options[i].shortOpt) {
            ...
            MoveBack(argv, m_Argument, m_NonOptionArguments);
            if (!HandleOptionArgument(argv, options[i].longOpt,
                                      options[i].argument)) {
              return false;
            }
            return true;
  ...
  const std::string& GetOptionArgument() const { return m_OptionArgument; }

  int GetNonOptionArguments() const { return m_NonOptionArguments; }
 ...
 private:
  ...
  bool HandleOptionArgument(std::vector<std::string>& argv, const char* option,
                            eOptionArgument arg) {
    switch (arg) {
      case OPTARG_NONE:
        break;
