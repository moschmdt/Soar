...
bool CommandLineInterface::DoVisualize(const std::string* pArg1,
  ...
  if (!pArg1) {
    PrintCLIMessage(
        "Visualize creates graphical representations of Soar's memory systems "
        "and past learning events.\n\n"
        "Use 'visualize ?' and 'help visualize' to learn more about the "
        "visualize command.");
    return true;
  } else {
    /* Single command argument */
    soar_module::param* my_param =
        thisAgent->visualizationManager->settings->get(pArg1->c_str());
    if (!my_param) {
      return SetError(
          "Invalid visualize sub-command.  Use 'soar ?' to see a list of valid "
          "sub-commands and settings.");
    } else if (my_param == thisAgent->visualizationManager->settings->viz_wm) {
      if (pArg2) {
        soar::Lexeme lexeme =
            soar::Lexer::get_lexeme_from_string(thisAgent, pArg2->c_str());
        if (lexeme.type == IDENTIFIER_LEXEME) {
          Symbol* pSym = thisAgent->symbolManager->find_identifier(
      ...
      lValidVisualizationGenerated = true;
               ...
               thisAgent->visualizationManager->settings->viz_smem) {
      uint64_t lti_id = NIL;
      ...
      if (pArg2) {
        ...
        if (lti_id == NIL) {
          return SetError(
              "Invalid long-term identifier for visualize command.");
        }
      }
      int lDepth = 1;
      ...
      lValidVisualizationGenerated = true;
               ...
               thisAgent->visualizationManager->settings->viz_epmem) {
      uint64_t lEpID;
      if (!pArg2 ||
          (pArg2 && (!from_string(lEpID, pArg2->c_str()) || (lEpID < 1)))) {
        return SetError("Please specify a valid episode id.");
      }
      PrintCLIMessage_Header("Visualization of Episodic Memory", 40);
      epmem_visualize_episode(
          thisAgent, lEpID, &thisAgent->visualizationManager->graphviz_output);
      lValidVisualizationGenerated = true;
    } else if (my_param ==
               thisAgent->visualizationManager->settings->viz_last) {
      if (pArg2) {
        return SetError(
            "Explanation visualization cannot take an additional argument.\n");
      }
      if (!thisAgent->explanationMemory->current_discussed_chunk_exists()) {
        return SetError(
            "Please first specify the chunk you want to visualize with the "
            "command 'explain [chunk-name]' or 'explain chunk [chunk ID]'.\n");
      }

      thisAgent->explanationMemory->visualize_last_output();
      lValidVisualizationGenerated = true;
    } else if (my_param ==
               thisAgent->visualizationManager->settings->viz_instantiations) {
      if (pArg2) {
        return SetError(
            "Explanation visualization cannot take an additional argument.\n");
      }
      ...
      lValidVisualizationGenerated = true;
               ...
               thisAgent->visualizationManager->settings->viz_contributors) {
      if (pArg2) {
        return SetError(
            "Explanation visualization cannot take an additional argument.\n");
      }
      ...
      lValidVisualizationGenerated = true;
               ...
               thisAgent->visualizationManager->settings->viz_identity_graph) {
      if (pArg2) {
        return SetError(
            "Explanation visualization cannot take an additional argument.\n");
      }
      ...
      lValidVisualizationGenerated = true;
                ...
                thisAgent->visualizationManager->settings->help_cmd) ||
               (my_param ==
                thisAgent->visualizationManager->settings->qhelp_cmd)) {
      thisAgent->visualizationManager->settings->print_visualization_settings(
          thisAgent);
      return true;
    } else {
      ...
      if (!pArg2) {
        ...
        if (!my_param->validate_string(pArg2->c_str())) {
          return SetError(
              "Invalid argument for visualize command. Use 'visualize ?' to "
              "see a list of valid sub-commands.");
        }
    ...
    if (lValidVisualizationGenerated) {
      ...
      std::string lFileName(
          thisAgent->visualizationManager->get_next_filename());
      ...
      if (!DoCLog(LOG_ADD, 0, &thisAgent->visualizationManager->graphviz_output,
                  true)) {
        thisAgent->visualizationManager->clear_visualization();
      ...
      if (thisAgent->visualizationManager->settings->gen_image->get_value() ||
          thisAgent->visualizationManager->settings->launch_viewer
              ->get_value()) {
        lSystemCommand = "dot -T";
        lSystemCommand +=
            thisAgent->visualizationManager->settings->image_type->get_value();
        lSystemCommand += ' ';
        ...
        if (system(lSystemCommand.c_str()) != 0) {
          thisAgent->visualizationManager->clear_visualization();
          return SetError(
              "Error: Could not generate visualization image because `dot` "
              "command failed. Do you have GraphViz installed?!\n");
        }
      }
      if (thisAgent->visualizationManager->settings->launch_viewer
              ->get_value()) {
        lSystemCommand = "open ";
        lSystemCommand += lFileName;
        lSystemCommand += '.';
        ...
        if (system(lSystemCommand.c_str()) != 0) {
          thisAgent->visualizationManager->clear_visualization();
          return SetError(
              "Error: Could not generate visualization image because failed to "
              "open file generated by `dot` command.\n");
        }
      }
      if (thisAgent->visualizationManager->settings->launch_editor
              ->get_value()) {
        lSystemCommand = "open ";
        lSystemCommand += lFileName;
        lSystemCommand += ".gv";
      ...
      if (thisAgent->visualizationManager->settings->print_gv->get_value()) {
        PrintCLIMessage(
            thisAgent->visualizationManager->graphviz_output.c_str());
      }
      thisAgent->visualizationManager->clear_visualization();
    }
