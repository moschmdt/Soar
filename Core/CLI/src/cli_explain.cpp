...
bool CommandLineInterface::DoExplain(const std::string *pArg1,
                                     const std::string *pArg2, bool json) {
  ...
  if (!pArg1) {
    ...
    /* Operator explanation sub-commands */
    if (*pArg1 == "track-operator") {
      if (!pArg2) {
        thisAgent->operatorExplainer->list_tracked(thisAgent);
      } else {
        thisAgent->operatorExplainer->track_operator(*pArg2);
      }
      return true;
    } else if (*pArg1 == "untrack-operator") {
      if (!pArg2) {
        return SetError("Please specify the operator name to untrack.");
      }
      thisAgent->operatorExplainer->untrack_operator(*pArg2);
      return true;
    } else if (*pArg1 == "operator") {
      if (!pArg2) {
        return SetError("Please specify the operator name to explain. Usage: "
                        "explain operator <name> [-j|--json]");
      }

      if (json) {
        thisAgent->operatorExplainer->print_decisions_json(thisAgent, *pArg2);
      } else {
        thisAgent->operatorExplainer->print_decisions(thisAgent, *pArg2);
      }
      return true;
    }
    ...
    if (!my_param) {
      if (!thisAgent->explanationMemory->explain_chunk(pArg1)) {
        return SetError("Invalid explain sub-command.  Use 'explain ?' to see "
                        "a list of valid sub-commands and settings.");
      }
      ...
      if (!pArg2) {
        return SetError("Please specify the chunk using either the chunk's "
                        "name or its ID.");
      } else {
