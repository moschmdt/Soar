...
class TraceCommand : public cli::ParserCommand {
 public:
  ...
  virtual const char* GetString() const { return "trace"; }
  virtual const char* GetSyntax() const { return "Syntax: trace [options]"; }
    ...
    for (;;) {
      ...
      switch (opt.GetOption()) {
        ...
        case 'l': {
          ...
          if (!ProcessWatchLevelSettings(level, options, settings, wmeSetting,
                                         learnSetting)) {
            return cli.SetError(opt.GetError().c_str());
          }
          fromWatch = true;
 ...
 private:
  ...
  bool ProcessWatchLevelSettings(const int level, cli::WatchBitset& options,
                                 cli::WatchBitset& settings, int& wmeSetting,
                                 int& learnSetting) {
    if (level < 0) {
    ...
    switch (level) {
      case 0:  // none
        ...
        wmeSetting = 0;
        cli.PrintCLIMessage(
            "Trace level 0 enabled:  All trace messages disabled.");
        break;
      ...
      case 4:
        cli.PrintCLIMessage(
            "Trace level 4 enabled:  Working memory element additions and "
            "removals");
        settings.set(cli::WATCH_WMES);
        // falls through
      ...
      case 2:  // phases, gds
        cli.PrintCLIMessage(
            "Trace level 2 enabled:  All phases, state removals caused by "
            "operator selection\n"
            "                        or a GDS violation, and any learning "
            "issues detected");
        settings.set(cli::WATCH_PHASES);
        settings.set(cli::WATCH_CONSISTENCY);
        settings.set(cli::WATCH_GDS_STATE_REMOVAL);
      ...
      case 1:  // decisions
        cli.PrintCLIMessage(
            "Trace level 1 enabled:  Decision cycles, state creation and "
            "operator selection");
        settings.set(cli::WATCH_DECISIONS);
        break;
    }
  ...
  int ParseLearningOptarg(cli::Options& opt) {
    if (opt.GetOptionArgument() == "noprint" ||
        opt.GetOptionArgument() == "0") {
      return 0;
    }
    if (opt.GetOptionArgument() == "print" || opt.GetOptionArgument() == "1") {
      return 1;
    }
    if (opt.GetOptionArgument() == "fullprint" ||
        opt.GetOptionArgument() == "2") {
      return 2;
    }

    cli.SetError(
        "Invalid learn setting, expected noprint, print, fullprint, or 0-2. "
        "Got: " +
        opt.GetOptionArgument());
    return -1;
  }
  ...
  bool CheckOptargRemoveOrZero(cli::Options& opt) {
    ...
    return cli.SetError("Invalid argument, expected remove or 0. Got: " +
                        opt.GetOptionArgument());
  }
