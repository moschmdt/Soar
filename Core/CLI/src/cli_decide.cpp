...
bool CommandLineInterface::DoDecide(std::vector<std::string>& argv,
  ...
  soar_module::param* my_param =
      thisAgent->command_params->decide_params->get(pCmd.c_str());
  if (!my_param) {
    return SetError(
        "Invalid decide command.  Use 'decide ?' to see a list of valid "
        "settings.");
  }
  if (my_param ==
      thisAgent->command_params->decide_params->indifferent_selection_cmd) {
    return ParseIndifferentSelection(argv);
  } else if (my_param == thisAgent->command_params->decide_params
                             ->numeric_indifferent_mode_cmd) {
    return ParseNumericIndifferentMode(argv);
  } else if (my_param ==
             thisAgent->command_params->decide_params->predict_cmd) {
    return ParsePredict(argv);
  } else if (my_param == thisAgent->command_params->decide_params->select_cmd) {
    return ParseSelect(argv);
  } else if ((my_param ==
              thisAgent->command_params->decide_params->srand_cmd) ||
             (my_param ==
              thisAgent->command_params->decide_params->srand_bc_cmd)) {
    return ParseSRand(argv);
  } else if ((my_param == thisAgent->command_params->decide_params->help_cmd) ||
             (my_param ==
              thisAgent->command_params->decide_params->qhelp_cmd)) {
    thisAgent->command_params->decide_params->print_settings(thisAgent);
  }
  return false;
}
bool CommandLineInterface::ParseIndifferentSelection(
...
bool CommandLineInterface::ParseNumericIndifferentMode(
    std::vector<std::string>& argv) {
  cli::Options opt;
  OptionsData optionsData[] = {{'a', "average", OPTARG_NONE},
                               {'a', "avg", OPTARG_NONE},
                               {'s', "sum", OPTARG_NONE},
                               {0, 0, OPTARG_NONE}};
...
bool CommandLineInterface::DoIndifferentSelection(const char pOp,
                                                  const std::string* p1,
                                                  const std::string* p2,
                                                  const std::string* p3) {
  agent* thisAgent = m_pAgentSML->GetSoarAgent();
  ...
  if (!pOp) {
    const char* policy_name =
        exploration_convert_policy(exploration_get_policy(thisAgent));

    if (m_RawOutput) {
      m_Result << policy_name;
    } else {
      AppendArgTagFast(sml_Names::kParamIndifferentSelectionMode,
                       sml_Names::kTypeString, policy_name);
    }
  ...
  // selection policy parameter
    ...
    if (!p1) {
      double param_value =
          exploration_get_parameter_value(thisAgent, "epsilon");
      std::string temp;
    ...
    if (!p1) {
      double param_value =
          exploration_get_parameter_value(thisAgent, "temperature");
      std::string temp;
      to_string(param_value, temp);
      ...
      if (!exploration_valid_parameter_value(thisAgent, "temperature",
                                             new_val)) {
        return SetError("Invalid parameter value.");
      }
  ...
  // selection parameter reduction policy
    ...
    if (!p2) {
      ...
      const char* policy_name = exploration_convert_reduction_policy(
          exploration_get_reduction_policy(thisAgent, p1->c_str()));
      ...
      if (!exploration_valid_reduction_policy(thisAgent, p1->c_str(),
                                              p2->c_str())) {
        return SetError("Invalid parameter value.");
      }

      return exploration_set_reduction_policy(thisAgent, p1->c_str(),
                                              p2->c_str());
    }
  }
  ...
  // selection parameter reduction rate
    ...
    if (!p3) {
      double reduction_rate =
          exploration_get_reduction_rate(thisAgent, p1->c_str(), p2->c_str());
      std::string temp;
      ...
      if (!exploration_valid_reduction_rate(thisAgent, p1->c_str(), p2->c_str(),
                                            new_val)) {
        return SetError("Invalid parameter value.");
      }

      return exploration_set_reduction_rate(thisAgent, p1->c_str(), p2->c_str(),
                                            new_val);
    }
  }
  ...
  // stats
    ...
    temp +=
        (thisAgent->Decider->settings[DECIDER_AUTO_REDUCE] ? ("on") : ("off"));
    ...
    for (int i = 0; i < EXPLORATION_PARAMS; i++) {
      ...
      temp += exploration_convert_reduction_policy(
...
bool CommandLineInterface::DoNumericIndifferentMode(bool query,
                                                    bool usesAvgNIM) {
  agent* thisAgent = m_pAgentSML->GetSoarAgent();
  if (query) {
    if (m_RawOutput) {
      ...
      AppendArgTagFast(sml_Names::kParamNumericIndifferentMode,
                       sml_Names::kTypeInt, modeString.str());
    }
  } else  // !query
  {
    thisAgent->numeric_indifferent_mode = usesAvgNIM
  ...
  if (!pOp) {
    ...
    if (my_selection != NULL) {
      if (m_RawOutput) {
        ...
        AppendArgTagFast(sml_Names::kOperator_ID, sml_Names::kTypeID,
                         my_selection);
      }
    } else {
      if (m_RawOutput) {
