...
bool CommandLineInterface::DoRun(const RunBitset& options, int count,
                                 eRunInterleaveMode interleaveIn) {
  // Default run type is sml_DECIDE
  smlRunStepSize runType = sml_DECIDE;
  //// ... unless there is a count, then the default is a decision cycle:
  // if (count >= 0) runType = sml_DECIDE;
  ...
  runResult = pScheduler->RunScheduledAgents(forever, runType, count, runFlags,
                                             interleave, synchronizeAtStart);
  ...
  switch (runResult) {
    ...
    case sml_RUN_EXECUTING:
      if (m_RawOutput) {
        ...
        std::string temp;
        AppendArgTagFast(sml_Names::kParamRunResult, sml_Names::kTypeInt,
                         to_string(runResult, temp));
      }
      break;

    case sml_RUN_COMPLETED_AND_INTERRUPTED:  // an interrupt was requested, but
                                             // the run completed first
    // falls through
    ...
    case sml_RUN_COMPLETED:
      ...
      // retval is sml_RUN_COMPLETED, but agent m_RunState ==
      // gSKI_RUNSTATE_HALTED should only check the agents
      // m_pAgentSML->WasOnRunList()
      {
        std::string temp;
        thisAgent->Decider->get_run_result_string(temp);
        if (m_RawOutput)
