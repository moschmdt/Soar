...
class ProductionCommand : public cli::ParserCommand {
 public:
  ProductionCommand(cli::CommandLineInterface& cli)
      : cli(cli), ParserCommand() {}
  virtual ~ProductionCommand() {}
  virtual const char* GetString() const { return "production"; }
  virtual const char* GetSyntax() const {
    return "Use 'production ?' or 'help production' to learn more about the "
           "production command.";
  }

  virtual bool Parse(std::vector<std::string>& argv) {
    cli::Options opt;
    // Maintainer Note: because we parse the options once here and once in
    // DoProduction, we have to declare all options used by subcommands here as
    // well.
    // TODO: that's double-maintenance. We should only have to declare options
    // to the top-level command here.
    ...
    if (!opt.GetNonOptionArguments()) {
      ...
      //                    return cli.SetError("The 'production' commands
      //                    contains commands to manipulate Soar rules and
      //                    analyze their usage.\n\n"
      //                        "Use 'production ?' to see an overview of the
      //                        command or 'help production' to read the manual
      //                        page.");
    }
