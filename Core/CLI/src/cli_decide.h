...
class DecideCommand : public cli::ParserCommand {
 public:
  ...
  virtual const char* GetString() const { return "decide"; }
  virtual const char* GetSyntax() const {
    return "Use 'decide ?' or 'help decide' to learn more about the decide "
           "command.";
  }
  ...
  virtual bool Parse(std::vector<std::string>& argv) {
    ...
    //                {
    //                    re
    //                    return cli.SetError("Sub-command is required. Use
    //                    'decide ?' or 'help decide' to learn more about the
    //                    decide command.");
    //                }
    std::string lCmd;
