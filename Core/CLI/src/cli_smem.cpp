...
bool CommandLineInterface::DoSMem(const char pOp, const std::string* pArg1,
  ...
  if (!pOp) {
    ...
    if (!result) {
      SetError(
          "This parameter is protected while the semantic memory database is "
          "open.");
    } else {
    ...
    soar_module::param* my_param =
        thisAgent->SMem->settings->get(pArg1->c_str());
    if (!my_param) {
      return SetError(
          "Invalid semantic memory parameter.  Use 'help smem' to see list of "
          "valid settings.");
    }
    ...
    if (thisAgent->SMem->settings->append_db->get_value() == on) {
      PrintCLIMessage(
          "Note: Since append mode is on, smem --init does not clear the "
          "database.\n"
    ...
    bool result = thisAgent->SMem->CLI_query(pArg1->c_str(), &(err),
                                             &(retrieved), number_to_retrieve);
    ...
    if (pArg2) {
      force =
          (!strcmp(pArg2->c_str(), "f") || (!strcmp(pArg2->c_str(), "force")));
    }

    bool result = thisAgent->SMem->CLI_remove(pArg1->c_str(), &(err),
                                              &(retrieved), force);
    ...
    if (!strcmp(pArg1->c_str(), "spreading") &&
        thisAgent->SMem->settings->activation_mode->get_value() !=
            smem_param_container::act_base) {
      return SetError(
          "Spreading activation cannot be turned on until the "
          "'activation-mode' is also set to base-level.\n"
          "Run 'smem --set activation-mode base-level' first if you intend to "
          "use spreading.");
    }
    if (!strcmp(pArg1->c_str(), "activation-mode") &&
        thisAgent->SMem->settings->activation_mode->get_value() ==
            smem_param_container::act_base &&
        thisAgent->SMem->settings->spreading->get_value() == on) {
      return SetError(
          "activation-mode cannot be changed while spreading activation is "
          "on.");
    }
    if (thisAgent->SMem->settings->spreading->get_value() == on &&
      ...
      return SetError(
          "Some spreading activation settings cannot be changed once spreading "
          "activation has been turned on.");
    }

    smem_param_container::db_choices last_db_mode =
    ...
    if (!result) {
      ...
      PrintCLIMessage(&tempString);
      if (thisAgent->SMem->connected()) {
        if (((!strcmp(pArg1->c_str(), "database")) &&
             (thisAgent->SMem->settings->database->get_value() !=
              last_db_mode)) ||
            (!strcmp(pArg1->c_str(), "path"))) {
          PrintCLIMessage(
              "To finalize database switch, issue an smem --init command.\n");
        }
      }
    ...
    if (!pArg1) {
      ...
      PrintCLIMessage_Item(
          "SQLite Version:", thisAgent->SMem->statistics->db_lib_version, 40);
      PrintCLIMessage_Item(
          "Memory Usage:", thisAgent->SMem->statistics->mem_usage, 40);
      PrintCLIMessage_Item(
          "Memory Highwater:", thisAgent->SMem->statistics->mem_high, 40);
      PrintCLIMessage_Item(
          "Retrieves:", thisAgent->SMem->statistics->retrievals, 40);
      PrintCLIMessage_Item("Queries:", thisAgent->SMem->statistics->queries,
                           40);
      PrintCLIMessage_Item("Stores:", thisAgent->SMem->statistics->stores, 40);
      PrintCLIMessage_Item(
          "Activation Updates:", thisAgent->SMem->statistics->act_updates, 40);
      ...
      soar_module::statistic* my_stat =
          thisAgent->SMem->statistics->get(pArg1->c_str());
      if (!my_stat) {
    ...
    if (!pArg1) {
      ...
      soar_module::timer* my_timer =
          thisAgent->SMem->timers->get(pArg1->c_str());
      if (!my_timer) {
