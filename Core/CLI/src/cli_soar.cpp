...
bool CommandLineInterface::DoSoar(const char pOp, const std::string* pArg1,
                                  const std::string* pArg2,
                                  const std::string* pArg3) {
  ...
  if (!pOp) {
    ...
    soar_module::param* my_param =
        thisAgent->Decider->params->get(pArg1->c_str());
    if (!my_param) {
      return SetError(
          "Invalid soar sub-command.  Use 'soar ?' to see a list of valid "
          "sub-commands and settings.");
    }
    ...
    if (my_param == thisAgent->Decider->params->init_cmd) {
      ...
      // S1 gets created during Reinitialize, clear its output from the trace
      // buffers
      ...
      if (pArg2 && !pArg2->empty() && !strcmp(pArg2->c_str(), "self")) {
        ...
        // We used to call FireSystemStop() in this function, but that's no good
        // because it comes before the agent has stopped because interrupt only
        // stops at the next phase or similar boundary (so could be a long time
        // off). So instead we set a flag and allow system stop to fire at the
        // end of the run.
      ...
      if (m_RawOutput) {
        ...
        AppendArgTagFast(sml_Names::kParamVersionMajor, sml_Names::kTypeInt,
                         to_string(major, temp));
        AppendArgTagFast(sml_Names::kParamVersionMinor, sml_Names::kTypeInt,
                         to_string(minor, temp));
        AppendArgTagFast(sml_Names::kParamVersionMicro, sml_Names::kTypeInt,
                         to_string(micro, temp));
      }
      return true;
    } else if ((my_param == thisAgent->Decider->params->help_cmd) ||
               (my_param == thisAgent->Decider->params->qhelp_cmd)) {
      ...
      if (m_RawOutput) {
        ...
        buffer << stopPhase;
        AppendArgTagFast(sml_Names::kParamPhase, sml_Names::kTypeInt,
                         buffer.str());
      }
    } else {
    ...
    if (!my_param) {
      return SetError(
          "Invalid command.  Use 'soar ?' to see a list of valid settings.");
    }
    ...
    if (!my_param->validate_string(pArg2->c_str())) {
      if (my_param->validate_string(pArg3->c_str())) {
        ...
        return SetError(
            "Invalid argument. Use 'soar ?' to see a list of valid settings.");
      }
    }
    ...
    if (my_param == thisAgent->Decider->params->stop_phase) {
      thisAgent->Decider->settings[DECIDER_STOP_PHASE] =
          thisAgent->Decider->params->stop_phase->get_value();
      ...
      } else if (thisAgent->Decider->params->stop_phase->get_value() ==
                 DECIDE_PHASE) {
      ...
      thisAgent->Decider->settings[DECIDER_KEEP_TOP_OPREFS] =
          thisAgent->Decider->params->keep_all_top_oprefs->get_value();
      thisAgent->outputManager->sprint_sf(
          tempString,
          "Soar will now %s retain top level preferences for items that are "
          "already o-supported.",
          thisAgent->Decider->settings[DECIDER_KEEP_TOP_OPREFS] ? "always"
                                                                : "not");
      PrintCLIMessage(tempString.c_str());
    } else if (my_param == thisAgent->Decider->params->wait_snc) {
      thisAgent->Decider->settings[DECIDER_WAIT_SNC] =
          thisAgent->Decider->params->wait_snc->get_value();
      thisAgent->outputManager->sprint_sf(
          tempString, "Soar will now %s when a state doesn't change.",
          thisAgent->Decider->settings[DECIDER_WAIT_SNC]
              ? "wait instead of impassing"
              : "impasse");
      PrintCLIMessage(tempString.c_str());
    } else if (my_param == thisAgent->Decider->params->timers_enabled) {
      thisAgent->timers_enabled =
          thisAgent->Decider->params->timers_enabled->get_value();
      thisAgent->outputManager->sprint_sf(
          tempString, "Timers are now %s.",
          thisAgent->timers_enabled ? "enabled" : "disabled");
      PrintCLIMessage(tempString.c_str());
      ...
      if (thisAgent->Decider->params->tcl_enabled->get_value() == true) {
        ...
        if (Soar_Instance::Get_Soar_Instance().is_Tcl_on()) {
          PrintCLIMessage(
              "Soar does not currently support turning Tcl off after it has "
              "been enabled.");
      ...
      if (thisAgent->Decider->settings[DECIDER_MAX_GP] !=
          thisAgent->Decider->params->max_gp->get_value()) {
        m_GPMax = static_cast<size_t>(
            thisAgent->Decider->params->max_gp->get_value());
      }
      thisAgent->Decider->settings[DECIDER_MAX_GP] =
          thisAgent->Decider->params->max_gp->get_value();
      thisAgent->outputManager->sprint_sf(
          tempString, "The maximum number of rules gp can generate is now %u.",
          thisAgent->Decider->settings[DECIDER_MAX_GP]);
      PrintCLIMessage(tempString.c_str());
    } else if (my_param == thisAgent->Decider->params->max_dc_time) {
      thisAgent->Decider->settings[DECIDER_MAX_DC_TIME] =
          thisAgent->Decider->params->max_dc_time->get_value();
      if (thisAgent->Decider->settings[DECIDER_MAX_DC_TIME] > 0) {
        thisAgent->outputManager->sprint_sf(
            tempString, "Soar will now interrupt decisions after %u seconds.",
            thisAgent->Decider->settings[DECIDER_MAX_DC_TIME]);
        PrintCLIMessage(tempString.c_str());
      } else {
        PrintCLIMessage(
            "Soar will no longer interrupt based on how how long a decision "
            "takes. (default)");
      }
    } else if (my_param == thisAgent->Decider->params->max_elaborations) {
      thisAgent->Decider->settings[DECIDER_MAX_ELABORATIONS] =
          thisAgent->Decider->params->max_elaborations->get_value();
      thisAgent->outputManager->sprint_sf(
          tempString,
          "The maximum number of elaborations in a phase is now %u.",
          thisAgent->Decider->settings[DECIDER_MAX_ELABORATIONS]);
      PrintCLIMessage(tempString.c_str());
    } else if (my_param == thisAgent->Decider->params->max_goal_depth) {
      thisAgent->Decider->settings[DECIDER_MAX_GOAL_DEPTH] =
          thisAgent->Decider->params->max_goal_depth->get_value();
      thisAgent->outputManager->sprint_sf(
          tempString, "The maximum goal depth is now %u.",
          thisAgent->Decider->settings[DECIDER_MAX_GOAL_DEPTH]);
      PrintCLIMessage(tempString.c_str());
    } else if (my_param == thisAgent->Decider->params->max_memory_usage) {
      thisAgent->Decider->settings[DECIDER_MAX_MEMORY_USAGE] =
          thisAgent->Decider->params->max_memory_usage->get_value();
      if (thisAgent->Decider->settings[DECIDER_MAX_MEMORY_USAGE] > 0) {
        thisAgent->outputManager->sprint_sf(
            tempString,
            "Soar will now interrupt execution if more than %u bytes of memory "
            "are used.  (This requires a special build of Soar.  See manual "
            "for more information.)",
            thisAgent->Decider->settings[DECIDER_MAX_MEMORY_USAGE]);
        PrintCLIMessage(tempString.c_str());
      } else {
        PrintCLIMessage(
            "Soar will not interrupt execution based on memory usage. "
            "(default)");
      }
    } else if (my_param == thisAgent->Decider->params->max_nil_output_cycles) {
      thisAgent->Decider->settings[DECIDER_MAX_NIL_OUTPUT_CYCLES] =
          thisAgent->Decider->params->max_nil_output_cycles->get_value();
      thisAgent->outputManager->sprint_sf(
          tempString,
          "The maximum number of decision cycles without output before "
          "interrupting is now %u.  (used with run --output)",
          thisAgent->Decider->settings[DECIDER_MAX_NIL_OUTPUT_CYCLES]);
      PrintCLIMessage(tempString.c_str());
    }
    return result;
