...
class Parser : public soar::tokenizer_callback {
 public:
  virtual ~Parser() {
    for (std::map<std::string, ParserCommand*>::iterator iter = cmds.begin();
         iter != cmds.end(); ++iter) {
      delete iter->second;
    }
    cmds.clear();
  ...
  void AddCommand(ParserCommand* p) { cmds[p->GetString()] = p; }
  ...
  ParserCommand* PartialMatch(const std::vector<std::string>& argv) {
    ...
    for (unsigned index = 0; index < argv[0].size(); ++index) {
      if (index == 0) {
        ...
        std::map<std::string, ParserCommand*>::const_iterator citer =
            cmds.begin();
        ...
        std::vector<std::pair<std::string, ParserCommand*> >::iterator liter =
            possibilities.begin();
    ...
    if (possibilities.size() != 1) {
      ...
      for (std::vector<std::pair<std::string, ParserCommand*> >::const_iterator
               liter = possibilities.begin();
           liter != possibilities.end(); ++liter) {
  ...
  cli::Aliases& GetAliases() { return aliases; }
