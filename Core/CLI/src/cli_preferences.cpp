...
bool read_attribute_from_string(agent* thisAgent, Symbol* id, char* the_lexeme,
  ...
  soar::Lexeme lexeme =
      soar::Lexer::get_lexeme_from_string(thisAgent, the_lexeme);
...
int soar_ecPrintPreferences(agent* thisAgent, char* szId, char* szAttr,
                            bool object, bool print_prod, wme_trace_type wtt) {
  Symbol *id, *attr = 0;
  slot* s = 0;
  preference* p;
  wme* w;
  ...
  if (!id->id->isa_goal && !strcmp(szAttr, "operator")) {
    ...
    if (szAttr && !object)  // default ^attr is ^operator, unless specified
                            // --object on cmdline
    {
      if (!read_attribute_from_string(thisAgent, id, szAttr, &attr)) {
        ...
        if (attr) {
          thisAgent->outputManager->printa_sf(
              thisAgent,
              "  This is probably an io- or arch-wme and does not have "
              "preferences\n");
          return 0;
        }
        thisAgent->outputManager->printa_sf(
            thisAgent,
            "Could not find prefs for the id,attribute pair: %s %s\n", szId,
            szAttr);
        return -2;
      }
      s = find_slot(id, attr);
      if (!s && !object) {
        ...
        thisAgent->outputManager->printa_sf(
            thisAgent, "Could not find preferences for %s ^%s.", szId, szAttr);
        return -3;
      }
    }
  }
  ...
  if (object) {
    ...
    for (s = id->id->slots; s != NIL; s = s->next) {
      if (s->attr == thisAgent->symbolManager->soarSymbols.operator_symbol) {
        thisAgent->outputManager->printa_sf(
            thisAgent, "Preferences for %y ^%y:", s->id, s->attr);
      } else {
    ...
    for (w = thisAgent->all_wmes_in_rete; w != NIL; w = w->rete_next) {
      if (w->value == id) {
        if (w->value == thisAgent->symbolManager->soarSymbols.operator_symbol) {
          thisAgent->outputManager->printa_sf(
              thisAgent, "Preferences for (%u: ", w->timetag);
        } else {
          thisAgent->outputManager->printa_sf(thisAgent,
                                              "Support for (%u: ", w->timetag);
        }
        ...
        if (w->preference) {
          ...
          if (!s) {
            thisAgent->outputManager->printa_sf(
                thisAgent, "    This is an arch-wme and has no prefs.\n");
          } else {
            for (i = 0; i < NUM_PREFERENCE_TYPES; i++) {
              if (s->preferences[i]) {
                // print(thisAgent, "\n%ss:\n",
                ...
                for (p = s->preferences[i]; p; p = p->next) {
                  if (p->value == id) {
                    if (i == ACCEPTABLE_PREFERENCE_TYPE) {
                      double prob = exploration_probability_according_to_policy(
                          thisAgent, s, s->preferences[i], p);
                      print_preference_and_source(thisAgent, p, print_prod, wtt,
                                                  &prob);
                    } else {
                      print_preference_and_source(thisAgent, p, print_prod,
                                                  wtt);
                    }
                  }
                }
          ...
          thisAgent->outputManager->printa_sf(
              thisAgent, "    This is an input-wme and has no prefs.\n");
        }
      }
    }
    return 0;
  ...
  if (id->id->isa_goal && !strcmp(szAttr, "operator")) {
    ...
    // returns a list of candidates without deciding which one in the event of
    // indifference
    preference* cand = 0;
    byte impasse_type = run_preference_semantics(thisAgent, s, &cand, true);

    // if the impasse isn't NONE_IMPASSE_TYPE, there's an impasse and we don't
    // want to print anything if we have no candidates, we don't want to print
    // anything
    if ((impasse_type == NONE_IMPASSE_TYPE) && cand) {
      ...
      for (p = cand; p; p = p->next_candidate) {
        double prob =
            exploration_probability_according_to_policy(thisAgent, s, cand, p);
...
bool CommandLineInterface::DoPreferences(const ePreferencesDetail detail,
                                         bool object, const std::string* pId,
                                         const std::string* pAttribute) {
  ...
  thisAgent->symbolManager->soarSymbols.operator_symbol->to_string(
      true, false, attr, PREFERENCES_BUFFER_SIZE);
  ...
  if (soar_ecPrintPreferences(thisAgent, id, attr, object, print_productions,
                              wtt)) {
