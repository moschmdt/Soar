...
bool CommandLineInterface::DoSVS(const std::vector<std::string>& args) {
  ...
  if (args.size() == 1) {
    m_Result << "Spatial Visual System is "
             << (thisAgent->svs->is_enabled() ? "enabled." : "disabled.");
    return true;
  } else if (args.size() == 2) {
    if ((args[1] == "--enable") || (args[1] == "-e") || (args[1] == "--on")) {
      if (thisAgent->svs->is_enabled()) {
        ...
        if (thisAgent->svs->is_enabled_in_substates()) {
          ...
          for (Symbol* lState = thisAgent->top_goal; lState;
      ...
      return true;
    } else if ((args[1] == "--disable") || (args[1] == "-d") ||
               (args[1] == "--off")) {
      if (!thisAgent->svs->is_enabled()) {
        ...
        if (thisAgent->svs->is_in_substate()) {
          m_Result
              << "Cannot disable Spatial Visual System while in a substate. ";
          return false;
        }
      ...
      if (!thisAgent->svs->is_enabled_in_substates()) {
        ...
        if (thisAgent->svs->is_in_substate()) {
          m_Result << "Cannot disable Spatial Visual System in substates while "
                      "in a substate. ";
          return false;
        }
...
#endif
    m_Result << "Spatial Visual System is currently disabled.  Please enable "
                "to execute SVS commands.";
    return false;
#ifndef NO_SVS
  }
#endif
