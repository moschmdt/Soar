...
bool CommandLineInterface::DoProduction(std::vector<std::string>& argv,
  ...
  soar_module::param* my_param =
      thisAgent->command_params->production_params->get(pCmd.c_str());
  if (!my_param) {
    return SetError(
        "Invalid production command.  Use 'production ?' to see a list of "
        "valid settings.");
  }
  if (my_param == thisAgent->command_params->production_params->excise_cmd) {
    return ParseExcise(argv);
  } else if (my_param ==
             thisAgent->command_params->production_params->firing_counts_cmd) {
    return ParseFC(argv);
  } else if (my_param ==
             thisAgent->command_params->production_params->matches_cmd) {
    return ParseMatches(argv);
  } else if (my_param ==
             thisAgent->command_params->production_params->memories_cmd) {
    return ParseMemories(argv);
  } else if (my_param == thisAgent->command_params->production_params
                             ->multi_attributes_cmd) {
    return ParseMultiAttributes(argv);
  } else if (my_param ==
             thisAgent->command_params->production_params->break_cmd) {
    return ParsePBreak(argv);
  } else if (my_param ==
             thisAgent->command_params->production_params->find_cmd) {
    return ParsePFind(argv);
  } else if (my_param ==
             thisAgent->command_params->production_params->watch_cmd) {
    return ParsePWatch(argv);
  } else if ((my_param ==
              thisAgent->command_params->production_params->help_cmd) ||
             (my_param ==
              thisAgent->command_params->production_params->qhelp_cmd)) {
...
bool CommandLineInterface::ParseExcise(std::vector<std::string>& argv) {
  cli::Options opt;
  OptionsData optionsData[] = {{'a', "all", OPTARG_NONE},
  ...
  if (opt.GetNonOptionArguments() > 2) {
    return SetError(
        "Only one production name allowed, call excise multiple times to "
        "excise more than one specific production.");
  }

  // Pass the production to the DoExcise function
  return DoExcise(options,
                  &(argv[opt.GetArgument() - opt.GetNonOptionArguments() + 1]));
}
bool CommandLineInterface::ParseFC(std::vector<std::string>& argv) {
  ...
  if (opt.GetNonOptionArguments() > 1) {
    if (opt.GetNonOptionArguments() > 2) {
      ...
      for (size_t i = opt.GetArgument() - opt.GetNonOptionArguments() + 1;
           i < argv.size(); ++i) {
      ...
      // one argument, figure out if it is a non-negative integer or a
      // production
      if (from_string(numberToList, pProduction)) {
        ...
        if (hasOptions) {
          SetError(
              "Ignoring unexpected options when printing firing count for a "
              "single production.\n");
        }
      }
    }
...
bool CommandLineInterface::ParseMatches(std::vector<std::string>& argv) {
  cli::Options opt;
  OptionsData optionsData[] = {{'a', "assertions", OPTARG_NONE},
                               {'c', "count", OPTARG_NONE},
                               {'n', "names", OPTARG_NONE},
  ...
  if (opt.GetNonOptionArguments() == 2) {
    ...
    return DoMatches(
        cli::MATCHES_PRODUCTION, detail,
        &argv[opt.GetArgument() - opt.GetNonOptionArguments() + 1]);
  }

  return DoMatches(mode, detail);
}
bool CommandLineInterface::ParseMultiAttributes(
...
bool CommandLineInterface::ParsePBreak(std::vector<std::string>& argv) {
  ...
  switch (option) {
    ...
    case 's':
      ...
      return DoPbreak(
          option, argv[opt.GetArgument() - opt.GetNonOptionArguments() + 1]);
    ...
    default:
      if (argv.size() == 2) {
        ...
        return DoPbreak(
            's', argv[opt.GetArgument() - opt.GetNonOptionArguments() + 1]);
      } else {
...
bool CommandLineInterface::ParsePFind(std::vector<std::string>& argv) {
  cli::Options opt;
  OptionsData optionsData[] = {
      {'c', "chunks", OPTARG_NONE},        {'l', "lhs", OPTARG_NONE},
      {'n', "nochunks", OPTARG_NONE},      {'r', "rhs", OPTARG_NONE},
      {'s', "show-bindings", OPTARG_NONE}, {0, 0, OPTARG_NONE}};
  ...
  std::string pattern;
  for (unsigned i = opt.GetArgument() - opt.GetNonOptionArguments() + 1;
       i < argv.size(); ++i) {
    pattern += argv[i];
    pattern += ' ';
  }
...
bool CommandLineInterface::ParsePWatch(std::vector<std::string>& argv) {
  cli::Options opt;
  OptionsData optionsData[] = {{'d', "disable", OPTARG_NONE},
...
bool CommandLineInterface::DoPbreak(const char& mode,
                                    const std::string& production) {
  agent* thisAgent = m_pAgentSML->GetSoarAgent();

  if (mode == 's' || mode == 'c') {
    Symbol* sym =
        thisAgent->symbolManager->find_str_constant(production.c_str());
    rete_node* prod =
        (sym && sym->sc->production) ? sym->sc->production->p_node : 0;
    ...
    for (int i = 0; i != NUM_PRODUCTION_TYPES; ++i) {
      for (struct production_struct* prod =
               thisAgent->all_productions_of_type[i];
           prod; prod = prod->next) {
  ...
  // We pass back the name of the command we just executed which becomes the tag
  // name used in the resulting XML.
  if (!m_RawOutput) {
...
bool CommandLineInterface::DoExcise(const ExciseBitset& options,
                                    const std::string* pProduction) {
  ...
  if (options.test(EXCISE_CHUNKS)) {
    ...
    excise_all_productions_of_type(thisAgent, JUSTIFICATION_PRODUCTION_TYPE,
                                   false);
  }
  ...
  if (options.test(EXCISE_RL)) {
    for (production* prod =
             thisAgent->all_productions_of_type[DEFAULT_PRODUCTION_TYPE];
         prod != NIL; prod = nextProd) {
      nextProd = prod->next;
      if (prod->rl_rule) {
        exciseCount++;
    ...
    for (production* prod =
             thisAgent->all_productions_of_type[USER_PRODUCTION_TYPE];
         prod != NIL; prod = nextProd) {
      nextProd = prod->next;
      if (prod->rl_rule) {
        exciseCount++;
    ...
    for (production* prod =
             thisAgent->all_productions_of_type[CHUNK_PRODUCTION_TYPE];
         prod != NIL; prod = nextProd) {
      nextProd = prod->next;
      if (prod->rl_rule) {
        exciseCount++;
  ...
  if (options.test(EXCISE_NEVER_FIRED)) {
    for (int i = 0; i < NUM_PRODUCTION_TYPES; i++) {
      for (production* prod = thisAgent->all_productions_of_type[i];
           prod != NIL; prod = nextProd) {
        nextProd = prod->next;
  ...
  if (pProduction) {
    Symbol* sym =
        thisAgent->symbolManager->find_str_constant(pProduction->c_str());
    ...
    if (!m_RawOutput) {
      ...
      AppendArgTagFast(sml_Names::kParamName, sml_Names::kTypeString,
                       *pProduction);
    }
  ...
  if (m_RawOutput) {
    m_Result << exciseCount << " production" << (exciseCount == 1 ? " " : "s ")
             << "excised.\n";
  } else {
    // Add the count tag to the front
    std::string temp;
    PrependArgTag(sml_Names::kParamCount, sml_Names::kTypeInt,
                  to_string(exciseCount, temp));
  }

  return true;
}
struct FiringsSort {
  bool operator()(std::pair<std::string, uint64_t> a,
                  std::pair<std::string, uint64_t> b) const {
    return a.second < b.second;
  }
};

void add_prods_of_type_to_fc_list(
    agent* thisAgent, unsigned int productionType, bool printThisType,
    bool printRL, const int numberToList,
    std::vector<std::pair<std::string, uint64_t> >* firings) {
  if (printThisType || printRL) {
    for (production* pSoarProduction =
             thisAgent->all_productions_of_type[productionType];
         pSoarProduction != 0; pSoarProduction = pSoarProduction->next) {
      if (!numberToList) {
...
bool CommandLineInterface::DoFiringCounts(PrintBitset options,
  ...
  if (pProduction && !pProduction->empty()) {
    ...
    if (options.test(PRINT_ALL) ||
        (!options.test(PRINT_CHUNKS) && !options.test(PRINT_DEFAULTS) &&
         !options.test(PRINT_JUSTIFICATIONS) && !options.test(PRINT_USER) &&
         !printRL && !options.test(PRINT_TEMPLATE))) {
  ...
  for (std::vector<std::pair<std::string, uint64_t> >::reverse_iterator j =
           firings.rbegin();
    ...
    if (!onlyFired || (onlyFired && (j->second > 0))) {
      if (m_RawOutput) {
        ...
        AppendArgTagFast(sml_Names::kParamName, sml_Names::kTypeString,
                         j->first);
        AppendArgTagFast(sml_Names::kParamCount, sml_Names::kTypeInt,
                         to_string(j->second, temp));
      }
    }
  }
  return true;
...
bool CommandLineInterface::DoMatches(const eMatchesMode mode,
                                     const eWMEDetail detail,
                                     const std::string* pProduction) {
...
bool CommandLineInterface::DoMultiAttributes(const std::string* pAttribute,
                                             int n) {
  agent* thisAgent = m_pAgentSML->GetSoarAgent();
  ...
  if (!pAttribute && !n) {
    ...
    buffer << count;
    if (!m_RawOutput) {
      PrependArgTagFast(sml_Names::kParamCount, sml_Names::kTypeInt,
                        buffer.str());
    }
    return true;
  }
  ...
  maList =
      static_cast<multi_attribute*>(thisAgent->memoryManager->allocate_memory(
          sizeof(multi_attribute), MISCELLANEOUS_MEM_USAGE));
  assert(maList);
...
bool CommandLineInterface::DoPWatch(bool query, const std::string* pProduction,
                                    bool setting) {
  agent* thisAgent = m_pAgentSML->GetSoarAgent();
...
void print_binding_list(agent* thisAgent, cons* bindings) {
  ...
  for (c = bindings; c != NIL; c = c->rest) {
    thisAgent->outputManager->printa_sf(thisAgent, "   (%y -> %y)\n",
                                        static_cast<Binding*>(c->first)->from,
                                        static_cast<Binding*>(c->first)->to);
  }
}

void reset_old_binding_point(agent* thisAgent, cons** bindings,
                             cons** current_binding_point) {
  cons *c, *c_next;
...
bool symbols_are_equal_with_bindings(agent* thisAgent, Symbol* s1, Symbol* s2,
                                     cons** bindings) {
  Binding* b;
  Symbol* bvar;
  ...
  if (bvar == NIL) {
    b = static_cast<Binding*>(thisAgent->memoryManager->allocate_memory(
        sizeof(Binding), MISCELLANEOUS_MEM_USAGE));
    b->from = s1;
    b->to = s2;
...
bool actions_are_equal_with_bindings(agent* thisAgent, action* a1, action* a2,
                                     cons** bindings) {
  ...
  if (!symbols_are_equal_with_bindings(thisAgent, rhs_value_to_symbol(a1->id),
                                       rhs_value_to_symbol(a2->id), bindings)) {
    return false;
  }

  if ((rhs_value_is_symbol(a1->attr)) && (rhs_value_is_symbol(a2->attr))) {
    if (!symbols_are_equal_with_bindings(
            thisAgent, rhs_value_to_symbol(a1->attr),
            rhs_value_to_symbol(a2->attr), bindings)) {
      return false;
    }
  } else {
    //            if ((rhs_value_is_funcall(a1->attr)) &&
    //            (rhs_value_is_funcall(a2->attr)))
    //            {
  ...
  if ((rhs_value_is_symbol(a1->value)) && (rhs_value_is_symbol(a2->value))) {
    if (symbols_are_equal_with_bindings(
            thisAgent, rhs_value_to_symbol(a1->value),
...
#define dealloc_and_return(thisAgent, x, y) \
  {                                         \
    deallocate_test(thisAgent, x);          \
    return (y);                             \
  }

bool tests_are_equal_with_bindings(agent* thisAgent, test t1, test test2,
                                   cons** bindings) {
  cons *c1, *c2;

  /* DJP 4/3/96 -- The problem here is that sometimes test2 was being copied */
  /*               and sometimes it wasn't.  If it was copied, the copy was
   * never */
  ...
  /* t1 is from the pattern given to "pf"; t2 is from a production's condition
   * list. */
  if (!t1) {
    return (test2 != 0);
  }
  ...
  if ((!test_includes_goal_or_impasse_id_test(t1, true, false)) &&
    ...
    t2 = copy_test(thisAgent,
                   test2); /* DJP 4/3/96 -- Always make t2 into a copy */
  }
  if (t1->type == EQUALITY_TEST) {
    if (!(t2 && (t2->type == EQUALITY_TEST))) {
      dealloc_and_return(thisAgent, t2, false);
    } else {
      if (symbols_are_equal_with_bindings(thisAgent, t1->data.referent,
                                          t2->data.referent, bindings)) {
  ...
  switch (t1->type) {
    ...
    case DISJUNCTION_TEST:
      ...
      if (c1 == c2) {
        dealloc_and_return(thisAgent, t2,
                           true); /* make sure they both hit end-of-list */
      } else {
        dealloc_and_return(thisAgent, t2, false);
      }
      break;
    case CONJUNCTIVE_TEST:
      for (c1 = t1->data.conjunct_list, c2 = t2->data.conjunct_list;
           ((c1 != NIL) && (c2 != NIL)); c1 = c1->rest, c2 = c2->rest) {
        if (!tests_are_equal_with_bindings(
                thisAgent, static_cast<test>(c1->first),
                static_cast<test>(c2->first), bindings))
          dealloc_and_return(thisAgent, t2, false)
      }
      if (c1 == c2) {
        dealloc_and_return(thisAgent, t2,
...
bool conditions_are_equal_with_bindings(agent* thisAgent, condition* c1,
                                        condition* c2, cons** bindings) {
  if (c1->type != c2->type) {
    return false;
  }
  switch (c1->type) {
    ...
    case CONJUNCTIVE_NEGATION_CONDITION:
      for (c1 = c1->data.ncc.top, c2 = c2->data.ncc.top;
           ((c1 != NIL) && (c2 != NIL)); c1 = c1->next, c2 = c2->next)
...
void read_pattern_and_get_matching_productions(
    agent* thisAgent, const char* lhs_str, cons** current_pf_list,
  ...
  if (!clist) {
    thisAgent->outputManager->printa_sf(thisAgent,
                                        "Error: not a valid condition list.\n");
    current_pf_list = NIL;
    return;
  }
  /**
  ...
  */
  ...
  /* For the moment match against productions of all types (user,chunk,default,
   * justification).     Later on the type should be a parameter.
   */
  ...
  for (i = 0; i < NUM_PRODUCTION_TYPES; i++)
    if ((i == CHUNK_PRODUCTION_TYPE && !no_chunks) ||
      ...
      for (prod = thisAgent->all_productions_of_type[i]; prod != NIL;
        ...
        (but don't necc. need to save them -- maybe can just print them as we
        go). */

        match = true;
        p_node_to_conditions_and_rhs(thisAgent, prod->p_node, NIL, NIL, &top,
                                     &bottom, NIL);
        ...
        for (c = clist; c != NIL; c = c->next) {
          ...
          for (pc = top; pc != NIL; pc = pc->next) {
            if (conditions_are_equal_with_bindings(thisAgent, c, pc,
                                                   &bindings)) {
              match_this_c = true;
              break;
            } else {
              /* Remove new, incorrect bindings. */
              reset_old_binding_point(thisAgent, &bindings,
  ...
  if (bindings) {
    free_binding_list(
        thisAgent, bindings); /* DJP 4/3/96 -- To catch the last production */
  }
}

void read_rhs_pattern_and_get_matching_productions(
    agent* thisAgent, const char* rhs_string, cons** current_pf_list,
    bool show_bindings, bool just_chunks, bool no_chunks) {
  action *a, *alist, *pa;
  int i;
  production* prod;
  ...
  for (i = 0; i < NUM_PRODUCTION_TYPES; i++) {
    if ((i == CHUNK_PRODUCTION_TYPE && !no_chunks) ||
      ...
      for (prod = thisAgent->all_productions_of_type[i]; prod != NIL;
        ...
        p_node_to_conditions_and_rhs(thisAgent, prod->p_node, NIL, NIL,
                                     &top_cond, &bottom_cond, &rhs);
...
bool CommandLineInterface::DoProductionFind(const ProductionFindBitset& options,
  ...
  if (options.test(PRODUCTION_FIND_INCLUDE_LHS)) {
    ...
    read_pattern_and_get_matching_productions(
        thisAgent, pattern.c_str(), &current_pf_list,
...
bool CommandLineInterface::DoMemories(const MemoriesBitset options, int n,
                                      const std::string* pProduction) {
  ...
  if (options.none()) {
    ...
    for (unsigned int i = 0; i < NUM_PRODUCTION_TYPES; ++i) {
      ...
      for (production* pSoarProduction = thisAgent->all_productions_of_type[i];
        ...
        memory.second =
            count_rete_tokens_for_production(thisAgent, pSoarProduction);
        memories.push_back(memory);
      }
    }
...
bool CommandLineInterface::ParseMemories(std::vector<std::string>& argv) {
  cli::Options opt;
  OptionsData optionsData[] = {
      {'c', "chunks", OPTARG_NONE},         {'d', "defaults", OPTARG_NONE},
      {'j', "justifications", OPTARG_NONE}, {'T', "templates", OPTARG_NONE},
      {'u', "user", OPTARG_NONE},           {0, 0, OPTARG_NONE}};
  ...
  if (opt.GetNonOptionArguments() > 2) {
    return SetError(
        "Syntax: memories [options] [number]\nmemories production_name");
  }
  ...
  if (opt.GetNonOptionArguments() == 2) {
    ...
    if (from_string(n, argv[optind])) {
      ...
      if (options.any()) {
        return SetError(
            "Do not specify production type when specifying a production "
            "name.");
      }
