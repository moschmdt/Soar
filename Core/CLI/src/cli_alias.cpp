  ...
  if (!argv || argv->size() == 1) {
    ...
    while (iter != m_Parser.GetAliases().End()) {
      if (!argv || argv->front() == iter->first) {
        ...
        for (std::vector<std::string>::const_iterator j = iter->second.begin();
             j != iter->second.end(); ++j) {
          expansion += *j;
          expansion += ' ';
        }
        ...
        if (m_RawOutput) {
          ...
          AppendArgTagFast(sml_Names::kParamAlias, sml_Names::kTypeString,
                           iter->first);
          AppendArgTagFast(sml_Names::kParamAliasedCommand,
                           sml_Names::kTypeString, expansion);
        }
