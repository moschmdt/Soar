...
class LoadCommand : public cli::ParserCommand {
 public:
  ...
  virtual ~LoadCommand() {}
  virtual const char* GetString() const { return "load"; }
  ...
  virtual bool Parse(std::vector<std::string>& argv) {
    ...
    OptionsData optionsData[] = {
        {'c', "close", OPTARG_NONE},       {'f', "flush", OPTARG_NONE},
        {'o', "open", OPTARG_REQUIRED},    {'l', "load", OPTARG_REQUIRED},
        {'r', "restore", OPTARG_REQUIRED}, {'s', "save", OPTARG_REQUIRED},
        {'a', "all", OPTARG_NONE},         {'d', "disable", OPTARG_NONE},
        {'v', "verbose", OPTARG_NONE},     {0, 0, OPTARG_NONE}};
    ...
    if (!opt.GetNonOptionArguments()) {
      return cli.SetError(
          "File type is required. \n\nUse 'load ?' or 'help load' to learn "
          "more about the load command.");
    }
    std::string lCmd;
...
class SaveCommand : public cli::ParserCommand {
 public:
  ...
  virtual ~SaveCommand() {}
  virtual const char* GetString() const { return "save"; }
  virtual const char* GetSyntax() const {
  ...
  virtual bool Parse(std::vector<std::string>& argv) {
    ...
    if (!opt.GetNonOptionArguments()) {
      return cli.SetError(
          "File type is required. \n\nUse 'save ?' or 'help save' to learn "
