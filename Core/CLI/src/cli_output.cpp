...
bool CommandLineInterface::DoRedirectedOutputCommand(
    std::vector<std::string>& argv, bool& had_error) {
  if (argv.size() < 3) return false;

  if (m_pAgentSML == NULL || m_pAgentSML->GetSoarAgent() == NULL) return false;

  agent* thisAgent = m_pAgentSML->GetSoarAgent();
  soar_module::param* my_param =
      thisAgent->outputManager->m_params->get(argv.at(1).c_str());
...
bool CommandLineInterface::DoOutput(std::vector<std::string>& argv,
                                    const std::string* pArg1,
  ...
  soar_module::param* my_param =
      thisAgent->outputManager->m_params->get(pArg1->c_str());

  if (!my_param) {
    return SetError(
        "Invalid output sub-command.  Use 'output ?' to see a list of valid "
        "sub-commands and settings.");
  } else if (my_param == thisAgent->outputManager->m_params->agent_traces) {
    if (!pArg2) {
      PrintCLIMessage(thisAgent->outputManager->m_params
                          ->get_agent_channel_string(thisAgent)
                          .c_str());
      return true;
    } else {
      int lTraceLevel;
      if (!pArg3) {
        return SetError(
            "Wrong number of arguments to output agent-trace command.");
      }
      if (!my_param->validate_string(pArg3->c_str())) {
        return SetError(
            "Agent trace channel setting must be 'on' or 'off'. Use 'output ?' "
            "to see a list of valid sub-commands.");
      }
      if (pArg3) {
        if (!from_string(lTraceLevel, (*pArg2)) || (lTraceLevel < 1) ||
            (lTraceLevel > maxAgentTraces)) {
          tempStringStream
              << "Agent trace channel must be an integer between 1 and "
              << maxAgentTraces << ".";
          return SetError(tempStringStream.str().c_str());
        }
        thisAgent->output_settings->agent_traces_enabled[lTraceLevel - 1] =
            ((*pArg3) == "on");
      } else {
  ...
  } else if ((my_param == thisAgent->outputManager->m_params->help_cmd) ||
             (my_param == thisAgent->outputManager->m_params->qhelp_cmd)) {
    ...
    if (!pArg2) {
      ...
      if (!my_param->validate_string(pArg2->c_str())) {
        return SetError(
            "Invalid argument for output command. Use 'output ?' to see a list "
            "of valid sub-commands.");
      }
      ...
      if (my_param == thisAgent->outputManager->m_params->print_depth) {
        thisAgent->outputManager->m_params->update_int_setting(
            thisAgent, static_cast<soar_module::integer_param*>(my_param));
      } else {
        thisAgent->outputManager->m_params->update_bool_setting(
...
bool CommandLineInterface::ParseClog(std::vector<std::string>& argv) {
  cli::Options opt;
  OptionsData optionsData[] = {{'a', "add", OPTARG_NONE},
  ...
  switch (mode) {
    case cli::LOG_ADD: {
      ...
      for (int i = 0; i < (opt.GetArgument() - opt.GetNonOptionArguments());
           ++i) {
        ++iter;
      }
    ...
    case cli::LOG_NEW:
      ...
      if (opt.GetNonOptionArguments() > 1) {
        return SetError(
            "Filename or nothing expected, enclose filename in quotes if there "
    ...
    case cli::LOG_NEWAPPEND:
      ...
      if (opt.GetNonOptionArguments() > 1) {
        return SetError(
            "Filename expected, enclose filename in quotes if there are spaces "
            "in the path.");
      }
      ...
      return DoCLog(mode,
                    &argv[opt.GetArgument() - opt.GetNonOptionArguments()]);
...
bool CommandLineInterface::ParseCTF(std::vector<std::string>& argv) {
  // Not going to use normal option parsing in this case because I do not want
  // to disturb the other command on the line
  if (argv.size() < 3) {
    return SetError(
        "Syntax: output command-to-file [-a] <filename> command [args]");
  }
...
bool CommandLineInterface::DoCommandToFile(const eLogMode mode,
...
bool CommandLineInterface::DoCLog(const eLogMode mode,
                                  const std::string* pFilename,
                                  const std::string* pToAdd, bool silent) {
  ...
  if (!silent) {
    if (m_RawOutput) {
      ...
      AppendArgTagFast(sml_Names::kParamLogSetting, sml_Names::kTypeBoolean,
                       setting);
