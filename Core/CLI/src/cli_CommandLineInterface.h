...
struct CallData {
  CallData(sml::AgentSML *pAgent, bool rawOutput)
      : pAgent(pAgent), rawOutput(rawOutput) {}
...
public:
   ...
   * @param pKernelSML The pointer to the KernelSML object, optional, used to
   * disable print callbacks
   *************************************************************/
   ...
   *************************************************************/
  EXPORT bool DoCommand(sml::Connection *pConnection, sml::AgentSML *pAgent,
                        const char *pCommandLine, bool echoResults,
                        bool rawOutput, soarxml::ElementXML *pResponse);

  bool DoAlias(std::vector<std::string> *argv = 0, bool doRemove = false);
  bool DoCD(const std::string *pDirectory = nullptr);
  bool DoChunk(const std::string *pArg1 = 0, const std::string *pArg2 = 0,
               const std::string *pArg3 = 0, const std::string *pArg4 = 0,
               bool doRemove = false, bool doClear = false);
  ...
  bool DoLS();
  bool DoOutput(std::vector<std::string> &argv, const std::string *pArg1 = 0,
                const std::string *pArg2 = 0, const std::string *pArg3 = 0);
  bool DoRedirectedOutputCommand(std::vector<std::string> &argv,
                                 bool &had_error);
  bool DoPopD();
  bool DoPreferences(const ePreferencesDetail detail, const bool object,
                     const std::string *pId = 0,
                     const std::string *pAttribute = 0);
  ...
  bool DoPWD();
  bool DoRL(const char pOp = 0, const std::string *pAttr = 0,
            const std::string *pVal = 0);
  bool DoRun(const RunBitset &options, int count = 0,
             eRunInterleaveMode interleave = RUN_INTERLEAVE_DEFAULT);
  bool DoSave(std::vector<std::string> &argv, const std::string &pCmd);
  bool DoSMem(const char pOp = 0, const std::string *pArg1 = 0,
              const std::string *pArg2 = 0, const std::string *pArg3 = 0);
  bool DoSoar(const char pOp = 0, const std::string *pArg1 = 0,
              const std::string *pArg2 = 0, const std::string *pArg3 = 0);
  ...
  bool DoTrace(const WatchBitset &options, const WatchBitset &settings,
               const int wmeSetting, const int learnSetting, bool fromWatch);
  bool DoVisualize(const std::string *pArg = 0, const std::string *pArg2 = 0,
                   const std::string *pArg3 = 0);
  bool DoWM(std::vector<std::string> &argv, const std::string &pCmd);

  EXPORT std::string ExpandCommand(const char *pCommand);

  /**
   * @brief Returns the directory that was the current working directory when
   * this CLI was created
   */
  std::string GetHome() { return m_HomeDirectory; };

  /*************************************************************
   * @brief Returns true if the given command should always be echoed (to any
   * listeners) The current implementation doesn't support aliases or short
   * forms of the commands.
   ...
   *
   * NOTE: The attribute names must be compile time constants -- i.e. they
   * remain in scope at all times (so we don't have to copy them).
   *************************************************************/
  ...
  void AppendArgTag(const char *pParam, const char *pType,
                    const std::string &value);
  void AppendArgTagFast(const char *pParam, const char *pType,
                        const char *pValue);
  void AppendArgTagFast(const char *pParam, const char *pType,
                        const std::string &value);
  void PrependArgTag(const char *pParam, const char *pType, const char *pValue);
  void PrependArgTag(const char *pParam, const char *pType,
                     const std::string &value);
  void PrependArgTagFast(const char *pParam, const char *pType,
                         const char *pValue);
  void PrependArgTagFast(const char *pParam, const char *pType,
                         const std::string &value);
  ...
  void PrintCLIMessage_Justify(const char *prefixString,
                               const char *printString, int column_width,
                               const char *commentString = 0);
  void PrintCLIMessage_Item(const char *prefixString,
                            soar_module::named_object *printObject,
                            int column_width, bool add_raw_lf = true);
  void PrintCLIMessage_Header(const char *headerString, int column_width,
                              bool add_raw_lf = true);
  void PrintCLIMessage_Section(const char *headerString, int column_width,
                               bool add_raw_lf = true);
...
private:
  ...
  bool DoAddWME(const std::string &id, std::string attribute,
                const std::string &value, bool acceptable);
  bool DoAllocate(const std::string &pool, int blocks);
  bool DoCaptureInput(eCaptureInputMode mode, bool autoflush = false,
                      std::string *pathname = 0);
  bool DoCLog(const eLogMode mode = LOG_QUERY, const std::string *pFilename = 0,
              const std::string *pToAdd = 0, bool silent = false);
  bool DoCommandToFile(const eLogMode mode, const std::string &filename,
                       std::vector<std::string> &argv);
  bool DoExcise(const ExciseBitset &options,
                const std::string *pProduction = 0);
  bool DoFiringCounts(PrintBitset options, const int numberToList = -1,
                      const std::string *pProduction = 0);
  bool DoIndifferentSelection(const char pOp = 0, const std::string *p1 = 0,
                              const std::string *p2 = 0,
                              const std::string *p3 = 0);
  bool DoLoadLibrary(const std::string &libraryCommand);
  bool DoMatches(const eMatchesMode mode,
                 const eWMEDetail detail = WME_DETAIL_NONE,
                 const std::string *pProduction = 0);
  bool DoMemories(const MemoriesBitset options, int n = 0,
                  const std::string *pProduction = 0);
  ...
  bool DoPredict();
  bool DoProductionFind(const ProductionFindBitset &options,
                        const std::string &pattern);
  bool DoPWatch(bool query = true, const std::string *pProduction = 0,
                bool setting = false);
  ...
  bool DoCheck(std::string filename);
  bool DoTime(std::vector<std::string> &argv);
  bool DoWatchWMEs(const eWatchWMEsMode mode, WatchWMEsTypeBitset type,
                   const std::string *pIdString = 0,
                   const std::string *pAttributeString = 0,
                   const std::string *pValueString = 0);
  bool DoWMA(const char pOp = 0, const std::string *pAttr = 0,
             const std::string *pVal = 0);
  ...
  bool ParseCheck(std::vector<std::string> &argv);
  ...
  void GetLastResultSML(sml::Connection *pConnection,
                        soarxml::ElementXML *pResponse, bool echoResults);
  ...
  // For help system
  bool ListHelpTopics(const std::string &directory,
                      std::list<std::string> &topics);
  bool Print_9_4_Help_Mapping(std::string pCmd, const char *pNewCmd,
  ...
  void PrintSourceSummary(int sourced, const std::list<std::string> &excised,
                          int ignored);
  ...
  soarxml::XMLTrace *m_XMLResult; // Used to collect up XML output from commands
  ...
  std::stack<CallData>
      m_CallDataStack;          // Call data we're currently working with
  ...
  bool m_CheckMode; // True during "load check": parse only, no permanent
                    // production load
  std::stack<std::string>
      m_SourceFileStack; // Stack of source calls, if zero then command line
...
extern bool read_id_or_context_var_from_string(agent *thisAgent,
