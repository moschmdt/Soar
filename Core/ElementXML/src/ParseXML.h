...
// which is (a) using allocateString() calls (so we can just pass off the
// results) and (b) is optimized for appending (so we keep a pointer to the end
// of the string) and (c) optimized to likely lengths we'll need (e.g. start
// with 32 or 64 chars for identifiers and values) Let's start with std::string
// until we know what capabilities we need.
...
class ParseXML {
 ...
 protected:
  // Used to report an error.  We only report the first one we find and then we
  // abort the parse.
  bool m_Error;
  std::string m_ErrorMsg;

  // The current token is the symbol/string/identifier we're currently looking
  // at
  ...
  // Set to true when we go to read the next line after the last line in the
  // file. (So it's not true when we're reading the last, incomplete line).
  bool m_IsEOF;
  ...
  void MustBe(char const* pValue) {
    if (m_TokenValue.compare(pValue) != 0) {
      RecordError("Looking for " + std::string(pValue) + " instead found " +
                  GetTokenValue());
    }
   ...
   * @param value      Iss et to the value of the current token (if matches
   *type).
   *
   */
  void MustBe(TokenType type, ParseString& value) {
    if (GetTokenType() != type) {
      RecordError("Found incorrect type when parsing token '" +
                  GetTokenValue() + "'");
    }
  ...
  // the first token from the next stream at the end of the current document and
  // need to be able to backup.  (This has no impact if we're just reading one
  // XML document from a stream).
