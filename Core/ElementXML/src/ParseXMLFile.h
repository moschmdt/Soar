...
class ParseXMLFile : public ParseXML {
 ...
 protected:
  ...
  // Number of chars in the current line buffer (when pos reaches this, read
  // another line)
  size_t m_LineLength;

  // Set to true when we have read all there is in the file (but not parsed it
  // yet).
  bool m_ReachedEOF;
   ...
   *
   */
  virtual char GetCurrentChar() { return m_CurrentLine[m_Pos]; }
  ...
  virtual void StartingNewToken() {
    ...
    // Only add something here if we decide we want to read multiple docs from a
    // file.
  }
