...
//
// A client can send a stream of XML data which this class parses to create the
// object representation of the XML. Or the client can call to this library
// directly, creating the object representation without ever producing the
// actual XML output (this is just for improved efficiency when the client and
// the Soar kernel are embedded in the same process).
//
...
// This arrangement allows us to pass references to objects created in that
// library around safely.
//
// The implementation of this class is based around a handle (pointer) to an
// ElementXMLImpl object stored in the ElementXML DLL. Why do we have this
// abstraction rather than just storing the ElementXMLImpl object right here?
// The reason is that we're going to pass these objects between an executable
// (the client) and a DLL (the kernel). If both are compiled on the same day
// with the same compiler, there's no problem passing a pointer to an object
// between the two. However, if that's not the case (e.g. we want to use a new
// client with an existing Soar DLL) then we need a safe way to ensure that the
// class layout being pointed to in the executable is the same as that used in
// the DLL.  If we've used different versions of a compiler or changed the
// ElementXMLImpl class in any way, the class layout (where the fields are in
// memory) won't be the same and we'll get a crash as soon as the object is
// accessed on the other side of the EXE-DLL divide.
//
// The solution to this is to add a separate DLL (ElementXML DLL) and only pass
// around pointers to objects which it owns. That way, there is no way for the
// two sides of the equation to get "out of synch" because there's only one
// implementation of ElementXMLImpl in the system and it's inside ElementXML
// DLL.  If we change the class, we update the DLL and neither the client nor
// the Soar Kernel DLL has any idea that a change has occured as neither can
// directly access the data within an ElementXMLImpl object.
//
// We wouldn't normally make all of this effort unless we thought there's a real
// chance this sort of problem will arise and in the case of the Soar kernel
// it's quite possible (even likely) as systems are often tied to specific
// versions of Soar and so old libraries and clients are used.  Building in a
// requirement that both need to be compiled at the same time means we're
// building in requirements that the source is available for both client and
// kernel--which may not always be the case if you consider commercial
// applications of either side.
//
// JRV/RPM Update: The function definitions for this class' members are now
// included in the headers and there are no corresponding .cpp files. The reason
// for this is that the code that uses them needs to have these classes defined
// in that compilation unit so that the ElementXML objects these units create
// are done so on their heap, and the ElementXMLImpl objects that actually do
...
class EXPORT ElementXML {
  // Let Connection have access to Fast methods (which are protected because
  // they take care to use correctly).
  friend class sml::Connection;
  friend class XMLTrace;

 protected:
  ElementXML_Handle m_hXML;  // Reference to the underlying object which is
 ...
 public:
  ...
  virtual ~ElementXML(void) {
    if (m_hXML) {
      ...
      // This code should be unnecessary but harmless and it makes a useful
      // place to put a break point if there are ref-counting problems with an
      // object.
      if (refCount == 0) {
        m_hXML = NULL;
      }
   ...
   *�_�.
   * @param  copyName  If true, tagName will be copied.  If false, we take
   *ownership of tagName.
   ...
   * comment blocks before an XML element, nor can you have trailing comment
   *blocks where there is no XML element following the comment.  However, both
   *of these are unusual situations and would require a significantly more
   *complex API to support so it seems unnecessary.
   *
   ...
   * @param  copyName      If true, atttributeName will be copied.  If false, we
   *take ownership of attributeName
   * @param  copyValue     If true, atttributeName will be copied.  If false, we
   *take ownership of attributeValue
  ...
  bool AddAttribute(char* attributeName, char* attributeValue,
    ...
    return ::soarxml_AddAttribute(m_hXML, attributeName, attributeValue,
                                  copyName, copyValue);
  }
  ...
  bool AddAttribute(char const* attributeName, char const* attributeValue) {
    return AddAttribute(CopyString(attributeName), CopyString(attributeValue),
                        false, false);
  }
   ...
   * @returns The value of the named attribute (or null if this attribute
   *doesn't exist).
   */
   ...
   *special characters such as �<� and �&� with the XML escape sequences &lt;
   *etc. These values will be converted when the XML stream is created.
   * @param  copyData      If true, characterData will be copied.  If false, we
   *take ownership of characterData
   */
   ...
   * @param length         The length of the buffer (note: allocateString(len)
   *allocates len+1 bytes--and the len+1 needs to be passed in here.
   * @param copyData       If true, characterData will be copied.  If false, we
   *take ownership of characterData
   */
   ...
   *no character data, returns zero-length string. The character data returned
   *will not include any XML escape sequences (e.g. &lt;). It will include the
   *original special characters (e.g. "<").
   */
   ...
   * set of character data is encoded binary (converting it back from hex to
   *binary).
   *
   ...
   * character data will be encoded as binary (but that should be OK based on
   *the tag names).
   *
   * Calling this function on a buffer that has already been decoded has no
   *effect.
   *
   ...
   * If the data is a null terminated string this is the length of the string +
   *1 (for the null).
   */
   ...
   *false.
   *
   *        This value is ignored if the character data is marked as binary
   *data.
   *
   * @param useCData   true if this element�s character data should be stored in
   *a CDATA section.
   */
  ...
  int DetermineXMLStringLength(bool includeChildren,
                               bool insertNewLines = false) const {
    return ::soarxml_DetermineXMLStringLength(m_hXML, includeChildren,
                                              insertNewLines);
  }
  ...
  //
  // These operations allow a client to allocate memory that ElementXML will
  // later release, or similarly, allow a client to release memory that
  // ElementXML has allocated.
  //
   ...
   *
   * @param length     The length is the number of characters in the string, so
   *length+1 bytes will be allocated (so that a trailing null is always
   *included).  Thus passing length 0 is valid and will allocate a single byte.
   */
   ...
   *
   * @param string     The string to release.  Passing NULL is valid and does
   *nothing.
   */
   ...
   *
   * @param string     The string to copy.  Passing NULL is valid and returns
   *NULL.
   */
   ...
   *
   * @param string     The buffer to copy.  Passing NULL is valid and returns
   *NULL.
   * @param length     The length of the buffer to copy (this exact length will
   *be allocated--no trailing NULL is added).
   */
   ...
   * @returns NULL if parsing failed, otherwise the ElementXML representing XML
   *doc
   */
   ...
   *
   * @param  pString   The XML document stored in a string.
   * @param  startPos  We'll start parsing the current XML document from this
   *position (0 == beginning of the string)
   * @param  endPos    This value is filled in at the end of the parse and
   *indicates where the parse ended. (if endPos == strlen(pString) we're done)
   * @returns NULL if parsing failed, otherwise the ElementXML representing XML
   *doc
   */
  static ElementXML* ParseXMLFromStringSequence(char const* pString,
                                                size_t startPos,
                                                size_t* endPos) {
    ElementXML_Handle hXML =
        ::soarxml_ParseXMLFromStringSequence(pString, startPos, endPos);
 ...
 protected:
  ...
  bool AddAttributeFast(char const* attributeName, char* attributeValue,
                        bool copyValue = true) {
    return ::soarxml_AddAttributeFast(m_hXML, attributeName, attributeValue,
                                      copyValue);
  }
   ...
   *this object. In practice, this generally means it must be a static constant.
   *
   * @param attributeName  Attribute name can only contain letters, numbers, �.�
   *�-� and �_�.
   * @param attributeValue Can be any string.
  ...
  bool AddAttributeFastFast(char const* attributeName,
    ...
    return ::soarxml_AddAttributeFastFast(m_hXML, attributeName,
                                          attributeValue);
  }
