...
// This attribute is used to indicate when a block of character data is hex
// encoded binary. If the attribute is found, the character data is
// automatically decoded back to binary. There is no requirement to use this
// attribute, but having it allows the parser of the document to decode binary
// data without having to know in advance which blocks are encoded binary. (If
// you don't use this attribute, then the character data will still be in its
// encoded form
...
// Converts from an escaped character (e.g. &lt; for <) to the original
// character
char ParseXML::GetEscapeChar() {
  ...
  if (IsEOF()) {
    RecordError(
        "Error trying to parse an escape sequence.  Started with '&' but had "
        "no ending ';'");
    return ' ';
  }
...
void ParseXML::GetNextToken() {
  ...
  // Record where this token starts (matters when we're doing a sequence of XML
  // docs)
  StartingNewToken();
  ...
  if (IsSymbol(GetCurrentChar())) {
    ...
    if (!IsEOF() && currentChar == kOpenTagChar &&
        GetCurrentChar() == kEndMarkerChar) {
    ...
    else if (!IsEOF() && currentChar == kEndMarkerChar &&
             GetCurrentChar() == kCloseTagChar) {
      SetCurrentToken(kSingleTagEndMarkerString, kSymbol);
      GetNextChar();
      return;
    }

    // If this is the beginning of a comment <! then handle it differently from
    // a regular symbol
    else if (IsCommentStart(GetCurrentChar())) {
      std::stringstream comment;

      // Consume the ! and the two leading dashes (BADBAD, I guess we should
      // check that they are leading -- and report a better error if they're
      // not)
      GetNextChar();
      GetNextChar();
      GetNextChar();
      ...
      while (!IsEOF() && !done) {
        ...
        // No need to escape contents within a comment as we read looking for
        // one specific pattern ignoring the rest
        // if (ch == kEscapeChar)
        // ch = GetEscapeChar() ;
        ...
        done = (last == kCloseTagChar && len > 3 &&
                comment.str().at(len - 3) == '-' &&
                comment.str().at(len - 2) == '-' &&
                comment.str().at(len - 1) == '>');
      }

      // Store the comment (the part lying between <!-- and -->
      SetCurrentToken(comment.str().substr(0, comment.str().size() - 3),
                      kComment);
    }
...
ElementXMLImpl* ParseXML::ParseElement() {
  ...
  while (!IsError() && !endTag) {
    if (Have(kCharData)) {
      ...
      pElement->SetCharacterData(const_cast<char*>(contents.c_str()),
                                 true);  // FIXME: inelegant, if const, this
                                         // should assume false (force copy)
    ...
    if (Have(kComment)) {
      // We'll only allow comments before other XML elements.  That allows us to
      // "hide" the comment within the existing ElementXML structure and allow
      // the ultimate parser of the XML ignore comments completely.
      SetLastComment(GetTokenValue());
      GetNextToken();
      continue;
    }
    ...
    RecordError(
        "Looking for character data block, but failed to find it or tag "
        "marker");
  }
  ...
  if (!IsError() && !singleEndTag) {
    ...
    if (tagName.compare(pElement->GetTagName()) != 0) {
      RecordError("The closing tag for " + std::string(pElement->GetTagName()) +
                  " doesn't match the opening tag");
    }

    // Then we must have the close tag.
    SetInCharData(true);  // If we are inside a parent element the next string
                          // is character data
    MustBe(kCloseTagChar);
  }
