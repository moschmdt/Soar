...
//
// This class is closely related to the "ElementXML" class that a client would
// use. But the implementation of that class resides in this class, which is in
// a different DLL. (Passing a pointer to an object between DLLs is
// dangerous--both DLLs would need to have been
//  built at the same time, with the same class header file etc.  Passing a
...
// Used to store a list of ElementXMLImpl nodes (use a vector for rapid index
// access)
...
// I think we'll just use char* internally for strings, but having a typedef
// means we can change this decision later with less typing.
typedef char* xmlString;
typedef char const* xmlStringConst;

// Used to store a list of strings.  Switched to vector to improve performance
// (we only add one by one and then delete the lot)
...
typedef std::map<xmlStringConst, xmlStringConst, strCompareElementXMLImpl>
    xmlAttributeMap;
...
class EXPORT ElementXMLImpl {
  // Let MessageGenerator have access to Fast methods (which are protected
  // because they take care to use correctly).
  friend class MessageGenerator;

 protected:
  int m_ErrorCode;  // Used to report any errors.
  bool m_UseCData;  // If true, should store character data in a CDATA section
                    // when encoding as XML.
  xmlStringConst m_TagName;  // The tag name (e.g. in <name>...</name> the tag
                             // name is "name")
  xmlString
      m_CharacterData;  // The character data (e.g. in <name>Albert
                        // Einstein</name> the char data is "Albert Einstein")
  xmlAttributeMap
      m_AttributeMap;   // Mapping from attribute-name to attribute-value (e.g.
                        // in <name first="Albert"> first is an attribute with
                        // value "Albert")
  xmlList m_Children;   // List of children of this element
  xmlString m_Comment;  // Used to attach a comment to this object.  It will
                        // appear ahead of the element when stored/retrieved.
  volatile long m_RefCount;  // Reference count.  Set to 1 on initialization.
                             // When reaches 0 the object is deleted.
  bool m_DataIsBinary;       // If true, then the character data is treated as a
                        // binary buffer (can contain embedded nulls) and the
                        // binary length is needed
  int m_BinaryDataLength;  // Gives the length of the character data buffer,
                           // when it's being treated as a binary buffer.  (only
                           // valid if m_IsDataBinary is true).
 ...
 public:
   ...
   * If you wish to keep it, you can call addRef() (and then later
   *releaseRef()).
   *
   ...
   * NOTE: The characterData will be deleted by this object when it is deleted.
   * It should be allocated with either allocateString() or copyString().
   * Be careful with the lengths -- allocateString(len) allocates len+1 bytes
   *and the length of the entire buffer should be passed in here (i.e. len+1 in
   *this example).
   *
   ...
   *
   * @param depth              How deep we are into the XML tree (can be used
   *for indedentation)
   * @param includeChildren    Includes all children in the XML output.
   * @param insertNewlines     Add newlines to space out the tags to be more
   *human-readable
  ...
  //
  // These operations allow a client to allocate memory that ElementXMLImpl will
  // later release, or similarly, allow a client to release memory that
  // ElementXMLImpl has allocated.
  //
  ...
  static inline char* AllocateString(int length) {
    // Switching to malloc and free (from new[] and delete[]), specifically so
    // that we can use strdup() for CopyString which gets called a lot.  Using
    // the library implementation (which should be in assembler) will be a lot
 ...
 protected:
   ...
   * @param maxLength          The max length of the string (not counting
   *trailing null)
   * @param includeChildren    Includes all children in the XML output.
   * @param insertNewlines     Add newlines to space out the tags to be more
   *human-readable
   ...
   */
  char* GenerateXMLString(int depth, char* pStr, int maxLength,
                          bool includeChildren, bool insertNewLines) const;
};
