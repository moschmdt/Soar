...
#endif  // DEBUG_TRY
        // with DEBUG_TRY defined, setting a breakpoint on this line will stop
        // things right when two threads are trying to access the ref count at
        // the same time
...
static const int kLenCommentStartString =
    static_cast<int>(strlen(kCommentStartString));
static const int kLenCommentEndString =
    static_cast<int>(strlen(kCommentEndString));
...
ElementXMLImpl::~ElementXMLImpl(void) {
  ...
  */
  ...
  // Again, replacing this walking of the list with a faster use of
  // std::for_each. We'll use a static method and have it call the member
...
ElementXMLImpl* ElementXMLImpl::MakeCopy() const {
  ...
  // Copy the attributes
  for (xmlAttributeMapConstIter mapIter = m_AttributeMap.begin();
       mapIter != m_AttributeMap.end(); mapIter++) {
  ...
  // Copy all of the children, overwriting the parent field for them so that
  // everything connects up correctly.
  for (xmlListConstIter iterChildren = m_Children.begin();
       iterChildren != m_Children.end(); iterChildren++) {
    ElementXMLImpl* pChild = *iterChildren;
 ...
 * when it is destroyed.  It should be allocated with either allocateString() or
 *copyString().
 *
 ...
 * should generally be declared as a static constant.
 *
 * The attribute value will be copied and can be deleted by the caller
 *immediately.
 *
...
const char* ElementXMLImpl::GetAttribute(const char* attName) const {
  ...
  // exist before (which we don't want) and Second, there is no const version of
  // [] so we can't do this anyway (it's not const because of the first
  // behavior...)
...
void ElementXMLImpl::SetBinaryCharacterData(char* characterData, int length,
                                            bool copyData) {
  // Decide if we're taking ownership of this string or not.
  if (copyData) {
...
int ElementXMLImpl::DetermineXMLStringLength(int depth, bool includeChildren,
  ...
  // The comment
  if (m_Comment) {
    len += kLenCommentStartString + static_cast<int>(strlen(m_Comment)) +
           kLenCommentEndString;
  }
  ...
  // The start tag
  if (m_TagName) {
    len += kLenStartTagOpen + static_cast<int>(strlen(this->m_TagName)) +
           kLenStartTagClose;
  }
  ...
  if (m_CharacterData) {
    if (m_DataIsBinary) {
      ...
      len += kLenCDataStart + static_cast<int>(strlen(m_CharacterData)) +
             kLenCDataEnd;
    } else {
  ...
  if (includeChildren) {
    ...
    while (iter != m_Children.end()) {
      ...
      len += pChild->DetermineXMLStringLength(depth + 1, includeChildren,
                                              insertNewLines);
  ...
  // The end tag
  if (m_TagName) {
    len += kLenEndTagOpen + static_cast<int>(strlen(this->m_TagName)) +
           kLenEndTagClose;
  }
...
char* ElementXMLImpl::GenerateXMLString(int depth, char* pStart, int maxLength,
  ...
  if (includeChildren) {
    ...
    while (iter != m_Children.end()) {
      ElementXMLImpl const* pChild = *iter;
      pStr = pChild->GenerateXMLString(depth + 1, pStr, maxLength,
                                       includeChildren, insertNewLines);
