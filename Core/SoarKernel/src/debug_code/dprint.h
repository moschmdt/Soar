...
#define dprint(mode, format, ...) \
  Output_Manager::Get_OM().debug_print_sf(mode, format, ##__VA_ARGS__)
#define dprint_noprefix(mode, ...) \
  Output_Manager::Get_OM().debug_print_sf_noprefix(mode, ##__VA_ARGS__)
...
#define dprint_partial_matches(mode, p_p_node) \
  Output_Manager::Get_OM().print_partial_matches(mode, p_p_node)
#define dprint_variablization_table(mode) \
  thisAgent->explanationBasedChunker->print_variablization_table(mode)
#define dprint_id_to_identity_map(mode) \
  thisAgent->explanationBasedChunker->print_id_to_identity_map(mode)
#define dprint_instantiation_identities_map(mode) \
  thisAgent->explanationBasedChunker->print_instantiation_identities_map(mode)
