...
typedef struct EXPORT agent_struct {
  ...
  OperatorExplanationManager *operatorExplainer;
  ...
  soar_timer_accumulator timers_decision_cycle_phase
      [NUM_PHASE_TYPES]; // decision_cycle_phase_timers

  soar_timer_accumulator
      timers_monitors_cpu_time[NUM_PHASE_TYPES]; // monitors_cpu_time, uses
                                                 // timers_phase
  soar_timer_accumulator
      timers_input_function_cpu_time; // input_function_cpu_time,
                                      // uses timers_kernel
  soar_timer_accumulator
      timers_output_function_cpu_time; // output_function_cpu_time,
                                       // uses timers_kernel

  uint64_t last_derived_kernel_time_usec; // Total of the time spent in the
  ...
  uint64_t max_dc_time_cycle; // Holds cycle_count that maximum amount of
                              // decision cycle time happened

  double max_dc_epmem_time_sec;     // Holds maximum amount epmem time
  double total_dc_epmem_time_sec;   // Holds last amount epmem time, used to
                                    // calculate delta
  uint64_t max_dc_epmem_time_cycle; // Holds what cycle max_dc_epmem_time_sec
                                    // was acheived

  double max_dc_smem_time_sec;     // Holds maximum amount smem time
  double total_dc_smem_time_sec;   // Holds last amount smem time, used to
                                   // calculate delta
  uint64_t max_dc_smem_time_cycle; // Holds what cycle max_dc_smem_time_sec was
                                   // acheived
...
/* only used if DETAILED_TIMING_STATS is #def'd in kernel.h */
  ...
  soar_timer_accumulator
      timers_ownership_cpu_time[NUM_PHASE_TYPES]; // ownership_cpu_time
  soar_timer_accumulator
      timers_chunking_cpu_time[NUM_PHASE_TYPES]; // chunking_cpu_time
  soar_timer_accumulator
      timers_match_cpu_time[NUM_PHASE_TYPES];                  // match_cpu_time
  ...
  inst_list
      newly_deleted_instantiations; // instantiations from NCI that are flagged
                                    // for deallocation before they are asserted
  ...
  xml_handle
      xml_destination; // The current destination for all XML generation,
                       // essentially either == to xml_trace or xml_commands
  xml_handle
      xml_trace; // During a run, xml_destination will be set to this pointer.
  xml_handle xml_commands; // During commands, xml_destination will be set to
                           // this pointer.
...
inline void allocate_cons(agent *thisAgent, T *dest_cons_pointer) {
  thisAgent->memoryManager->allocate_with_pool(MP_cons_cell,
                                               (dest_cons_pointer));
}

template <typename T> inline void free_cons(agent *thisAgent, T *c) {
