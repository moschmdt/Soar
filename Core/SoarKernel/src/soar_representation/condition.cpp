...
condition* copy_condition(agent* thisAgent, condition* cond,
                          ...
                          bool pStripLiteralConjuncts, bool pCopyInstantiation,
  ...
  switch (cond->type) {
    case POSITIVE_CONDITION:
      New->bt = cond->bt;
      New->data.tests.id_test = copy_test(
          thisAgent, cond->data.tests.id_test, pUnify_variablization_identity,
          pStripLiteralConjuncts, pStripGoalImpasseTests);
      New->data.tests.attr_test = copy_test(
          thisAgent, cond->data.tests.attr_test, pUnify_variablization_identity,
          pStripLiteralConjuncts, pStripGoalImpasseTests);
      New->data.tests.value_test =
          copy_test(thisAgent, cond->data.tests.value_test,
                    pUnify_variablization_identity, pStripLiteralConjuncts,
                    pStripGoalImpasseTests);
      New->test_for_acceptable_preference =
    ...
    case CONJUNCTIVE_NEGATION_CONDITION:
      copy_condition_list(
          thisAgent, cond->data.ncc.top, &(New->data.ncc.top),
          &(New->data.ncc.bottom), pUnify_variablization_identity,
          pStripLiteralConjuncts, false,
          false);  // I don't think we'd want to strip state tests from NCCs
      break;
  }
  return New;
}
...
void copy_condition_list(agent* thisAgent, condition* top_cond,
  ...
  while (top_cond) {
    New = copy_condition(thisAgent, top_cond, pUnify_variablization_identity,
                         pStripLiteralConjuncts, pCopyInstantiation,
                         pStripGoalImpasseTests);
    if (prev) {
      prev->next = New;
    } else {
