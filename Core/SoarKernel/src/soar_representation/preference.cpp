...
preference* make_preference(agent* thisAgent, PreferenceType type, Symbol* id,
                            ...
                            const identity_quadruple& o_ids,
                            const bool_quadruple& pWas_unbound_vars) {
  preference* p;
  ...
  p->identities = {NULL_IDENTITY_SET, NULL_IDENTITY_SET, NULL_IDENTITY_SET,
                   NULL_IDENTITY_SET};
  p->rhs_func_inst_identities = {NULL, NULL, NULL, NULL};
  p->chunk_inst_identities = {LITERAL_VALUE, LITERAL_VALUE, LITERAL_VALUE,
                              LITERAL_VALUE};
...
/* This function just copies the elements of a preferences we need for the EBC
...
preference* shallow_copy_preference(agent* thisAgent, preference* pPref) {
  ...
  p->inst_identities = {pPref->inst_identities.id, pPref->inst_identities.attr,
                        pPref->inst_identities.value,
                        pPref->inst_identities.referent};
  p->chunk_inst_identities = {pPref->chunk_inst_identities.id,
                              pPref->chunk_inst_identities.attr,
                              pPref->chunk_inst_identities.value,
  ...
  p->rhs_func_inst_identities.id =
      copy_rhs_value(thisAgent, pPref->rhs_func_inst_identities.id);
  p->rhs_func_inst_identities.attr =
      copy_rhs_value(thisAgent, pPref->rhs_func_inst_identities.attr);
  p->rhs_func_inst_identities.value =
      copy_rhs_value(thisAgent, pPref->rhs_func_inst_identities.value);
  p->rhs_func_inst_identities.referent =
      copy_rhs_value(thisAgent, pPref->rhs_func_inst_identities.referent);
...
void cache_preference_if_necessary(agent* thisAgent, preference* pref) {
  if ((pref->inst->match_goal_level != TOP_GOAL_LEVEL) &&
      thisAgent->explanationMemory->is_any_enabled()) {
    preference* lNewPref = shallow_copy_preference(thisAgent, pref);
    insert_at_head_of_dll(pref->inst->preferences_cached, lNewPref, inst_next,
                          inst_prev);
  }
}
...
void deallocate_preference_contents(agent* thisAgent, preference* pref,
  ...
  if (pref->identities.id)
    IdentitySet_remove_ref(thisAgent, pref->identities.id);
  if (pref->identities.attr)
    IdentitySet_remove_ref(thisAgent, pref->identities.attr);
  if (pref->identities.value)
    IdentitySet_remove_ref(thisAgent, pref->identities.value);
  if (pref->identities.referent)
    IdentitySet_remove_ref(thisAgent, pref->identities.referent);

  if (pref->rhs_func_inst_identities.id)
    deallocate_rhs_value(thisAgent, pref->rhs_func_inst_identities.id);
  if (pref->rhs_func_inst_identities.attr)
    deallocate_rhs_value(thisAgent, pref->rhs_func_inst_identities.attr);
  if (pref->rhs_func_inst_identities.value)
    deallocate_rhs_value(thisAgent, pref->rhs_func_inst_identities.value);
  if (pref->rhs_func_inst_identities.referent)
    deallocate_rhs_value(thisAgent, pref->rhs_func_inst_identities.referent);
  if (pref->rhs_func_chunk_inst_identities.id)
    deallocate_rhs_value(thisAgent, pref->rhs_func_chunk_inst_identities.id);
  if (pref->rhs_func_chunk_inst_identities.attr)
    deallocate_rhs_value(thisAgent, pref->rhs_func_chunk_inst_identities.attr);
  ...
  if (pref->rhs_func_chunk_inst_identities.referent)
    deallocate_rhs_value(thisAgent,
                         pref->rhs_func_chunk_inst_identities.referent);

  thisAgent->memoryManager->free_with_pool(MP_preference, pref);
}

/* IMPORTANT: Any changes made to deallocate_preference should also be made to
 * corresponding code in deallocate_instantiation */
void deallocate_preference(agent* thisAgent, preference* pref,
  ...
  if (pref->on_goal_list)
    remove_from_dll(pref->inst->match_goal->id->preferences_from_goal, pref,
                    all_of_goal_next, all_of_goal_prev);

  if (pref->inst) {
    if (!dont_cache) cache_preference_if_necessary(thisAgent, pref);
    remove_from_dll(pref->inst->preferences_generated, pref, inst_next,
                    inst_prev);
...
bool add_preference_to_tm(agent* thisAgent, preference* pref) {
  ...
  if (!thisAgent->Decider->settings[DECIDER_KEEP_TOP_OPREFS] &&
      (pref->inst->match_goal == thisAgent->top_state) && pref->o_supported &&
      !s->isa_context_slot && (pref->type == ACCEPTABLE_PREFERENCE_TYPE)) {
    /* We could potentially cache a list of all o-supported values in the slot
     * so that we don't have to iterate through all of the preferences. We would
     * need to maintain the list as we decide non-context slots */
    bool already_top_o_supported = false;

    for (p2 = s->all_preferences; (p2 && !already_top_o_supported);
         p2 = p2->all_of_slot_next) {
      if ((p2->value == pref->value) && p2->o_supported &&
          (p2->inst->match_goal == thisAgent->top_state)) {
    ...
    if (already_top_o_supported) {
      if (thisAgent->trace_settings[TRACE_FIRINGS_PREFERENCES_SYSPARAM]) {
        ...
        thisAgent->outputManager->printa_sf(
            thisAgent, " (%y) ALREADY SUPPORTED ON TOP LEVEL.  IGNORING.\n",
            pref->inst->prod_name);
      }
      return false;
    }
  ...
  insert_at_head_of_dll(s->all_preferences, pref, all_of_slot_next,
                        all_of_slot_prev);

  /*  add to preference list of slot (according to match goal level of the
   * instantiations) */
  ...
  /* If the slot is unchanged but has some references laying around, we clear
   * them This does not lead to immediate memory deallocate/allocate but once
   * the WMEs are resolved, this should free the memory. */
  ...
  if (wma_enabled(thisAgent) && !s->isa_context_slot) {
    ...
    /* If wme exists, it should already have been updated during assertion of
     * new preferences */
    if (!exists) {
      if (s->wma_val_references == NIL) {
        thisAgent->memoryManager->allocate_with_pool(MP_wma_slot_refs,
                                                     &(s->wma_val_references));
#ifdef USE_MEM_POOL_ALLOCATORS
        s->wma_val_references =
            new (s->wma_val_references) wma_sym_reference_map(
                std::less<Symbol*>(), soar_module::soar_memory_pool_allocator<
                                          std::pair<Symbol*, uint64_t> >());
#else
        s->wma_val_references =
            new (s->wma_val_references) wma_sym_reference_map();
#endif
   ...
   * later */
  if ((s->isa_context_slot) && ((pref->type == ACCEPTABLE_PREFERENCE_TYPE) ||
...
void remove_preference_from_tm(agent* thisAgent, preference* pref) {
  ...
  pref->in_tm = false;
  pref->slot =
      NIL; /* BUG shouldn't we use pref->slot in place of pref->in_tm? */
  mark_slot_as_changed(thisAgent, s);
