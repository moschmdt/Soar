 ...
 *
 * fc, ic, sc, id, var:  These are cached pointers for each of the 5
 type-specific
 ...
 *
 * Note:  I've disabled the union when in debugging mode.  It makes it much
 easier
 * to see the type-specific variables in a debugger.  It can also help find some
 * bugs where some part of the kernel may be treating a symbol as the wrong
 type.
 *
...
typedef struct EXPORT symbol_struct {
  ...
  bool is_in_tc(tc_number tc) {
    if ((symbol_type == VARIABLE_SYMBOL_TYPE) ||
        (symbol_type == IDENTIFIER_SYMBOL_TYPE))
      return (tc_num == tc);
    else
      return false;
  }
  ...
  char* to_string(bool rereadable = false, bool showLTILink = false,
                  char* dest = NIL, size_t dest_size = 0,
                  const int decimal_precision = DEFAULT_DECIMAL_PRECISION);
...
inline bool Symbol::is_lti() {
  return ((symbol_type == IDENTIFIER_SYMBOL_TYPE) && (id->LTI_ID != NIL));
};
inline bool Symbol::is_state() {
  return ((symbol_type == IDENTIFIER_SYMBOL_TYPE) && id->isa_goal);
}
inline bool Symbol::is_operator() {
  return ((symbol_type == IDENTIFIER_SYMBOL_TYPE) && id->isa_operator);
}
...
inline void Symbol::mark_if_unmarked(agent* thisAgent, tc_number tc,
                                     cons** sym_list) {
  if (tc_num != tc) {
    tc_num = tc;
    if (sym_list) {
 ...
 * to figure out whether a given object is an operator. name_number, name_letter
 * Name and letter of the identifier slots                       DLL of all
 * slots this symbol is used in tc_num                      Unique numbers put
 ...
 * are to this id. promotion_level             Level to which this id is going
 ...
 * in the context stack impasse_wmes                For goals and impasses, DLL
 * of architecturally created wmes (e.g., (G37 ^object G36)) lower_goal For
 * goals, substate of this goal in the context stack operator_slot For goals,
