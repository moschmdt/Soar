...
rhs_value allocate_rhs_value_for_symbol_no_refcount(
    ...
    uint64_t pChunkIdentity, Identity* pIdentity = NULL_IDENTITY_SET,
    bool pWasUnbound = false);
rhs_value allocate_rhs_value_for_symbol(agent* thisAgent, Symbol* sym,
                                        uint64_t pInstIdentity,
                                        uint64_t pChunkIdentity,
                                        Identity* pIdentity = NULL_IDENTITY_SET,
                                        bool pWasUnbound = false);
...
rhs_value copy_rhs_value(agent* thisAgent, rhs_value rv,
                         bool get_identity_set = false,
                         bool get_cloned_identity = false);
...
inline uint64_t rhs_value_to_inst_identity(rhs_value rv) {
  return reinterpret_cast<rhs_symbol>(rv)->inst_identity;
}
inline bool rhs_value_to_was_unbound_var(rhs_value rv) {
  return reinterpret_cast<rhs_symbol>(rv)->was_unbound_var;
}
...
inline rhs_value reteloc_to_rhs_value(byte field_num,
...
inline bool rhs_values_equal(rhs_value rv1, rhs_value rv2) {
  if (rhs_value_is_symbol(rv1) && rhs_value_is_symbol(rv2))
    ...
    for (c = fl1->rest, c2 = fl2->rest; c != NIL && c2 != NIL;
         c = c->rest, c2 = c2->rest)
      if (!rhs_values_equal(static_cast<char*>(c->first),
                            static_cast<char*>(c2->first)))
        return false;
    return true;
  } else
    return (rv1 == rv2);
...
inline bool rhs_value_is_null(rhs_value rv) {
  ...
  if (rhs_value_is_funcall(rv)) {
    return (rhs_value_to_funcall_list(rv) == 0);
  };
