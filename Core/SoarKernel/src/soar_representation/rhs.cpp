  ...
  if (!rv) {
    ...
    for (c = fl->rest; c != NIL; c = c->rest) {
      ...
      new_c->first = copy_rhs_value(thisAgent, static_cast<char*>(c->first),
                                    get_identity_set, get_cloned_identity);
    ...
    if (get_identity_set) {
      if (l_identity) {
        if (l_identity->get_clone_identity()) {
          assert(false);
          l_identity = thisAgent->explanationBasedChunker->get_identity_for_id(
              l_identity->get_clone_identity());
        } else {
          l_identity = thisAgent->explanationBasedChunker->get_identity_for_id(
    ...
    if (l_identity && get_cloned_identity) {
      ...
      l_identity =
          NULL_IDENTITY_SET;  // Will be filled in later based when finalizing
    }

    return allocate_rhs_value_for_symbol(thisAgent, r->referent, lID, r->cv_id,
                                         l_identity, r->was_unbound_var);
  }
}
...
void add_all_variables_in_action(agent* thisAgent, action* a, tc_number tc,
                                 cons** var_list) {
  Symbol* id;
...
void add_all_variables_in_action_list(agent* thisAgent, action* actions,
                                      tc_number tc, cons** var_list) {
  action* a;
...
void add_bound_variables_in_action(agent* thisAgent, action* a, tc_number tc,
                                   cons** var_list) {
  Symbol* id;
...
void add_bound_variables_in_action_list(agent* thisAgent, action* actions,
                                        tc_number tc, cons** var_list) {
  action* a;
...
rhs_value create_RHS_value(agent* thisAgent, rhs_value rv, condition* cond,
  ...
  if (rhs_value_is_reteloc(rv)) {
     ...
     * This case seems to only be for identifiers or constants originally bound
     * to variables */

    t = var_test_bound_in_reconstructed_conds(
        thisAgent, cond, rhs_value_to_reteloc_field_num(rv),
        rhs_value_to_reteloc_levels_up(rv));
    return allocate_rhs_value_for_symbol(thisAgent, t->data.referent,
                                         t->inst_identity, 0, t->identity);
  }
    ...
    if (!*(thisAgent->rhs_variable_bindings + index)) {
      ...
      *(thisAgent->rhs_variable_bindings + index) = sym;
      ...
      if (ebcTraceType == Explanation_Trace) {
        lO_id = thisAgent->explanationBasedChunker
                    ->get_or_create_inst_identity_for_sym(sym);
      }
      /* generate will increment the refcount on the new variable, so don't need
       * to do it here. */
      return allocate_rhs_value_for_symbol_no_refcount(thisAgent, sym, lO_id, 0,
                                                       NULL, true);
    } else {
  ...
  if (rhs_value_is_funcall(rv)) {
    ...
    for (c = fl->rest; c != NIL; c = c->rest) {
      allocate_cons(thisAgent, &new_c);
      new_c->first = create_RHS_value(thisAgent, static_cast<char*>(c->first),
                                      cond, first_letter, ebcTraceType);
      prev_new_c->rest = new_c;
      prev_new_c = new_c;
    }
    prev_new_c->rest = NIL;
    ...
    if (ebcTraceType == Explanation_Trace)
      return allocate_rhs_value_for_symbol(thisAgent, rs->referent,
                                           rs->inst_identity, rs->cv_id,
                                           rs->identity, rs->was_unbound_var);
    else
      return allocate_rhs_value_for_symbol(
          thisAgent, rs->referent, LITERAL_VALUE, 0, NULL, rs->was_unbound_var);
  }
}

action* create_RHS_action_list(agent* thisAgent, action* actions,
                               condition* cond, ExplainTraceType ebcTraceType) {
  ...
  while (old) {
    ...
    if (old->type == FUNCALL_ACTION) {
      New->value =
          create_RHS_value(thisAgent, old->value, cond, 'v', ebcTraceType);
    } else {
      New->id = create_RHS_value(thisAgent, old->id, cond, 's', ebcTraceType);
      New->attr =
          create_RHS_value(thisAgent, old->attr, cond, 'a', ebcTraceType);
      first_letter = first_letter_from_rhs_value(New->attr);
      New->value = create_RHS_value(thisAgent, old->value, cond, first_letter,
                                    ebcTraceType);
      if (preference_is_binary(old->preference_type)) {
        New->referent = create_RHS_value(thisAgent, old->referent, cond,
                                         first_letter, ebcTraceType);
      }
    }
    old = old->next;
  }
...
rhs_value allocate_rhs_value_for_symbol_no_refcount(
    agent* thisAgent, Symbol* sym, uint64_t pInstIdentity,
    uint64_t pChunkIdentity, Identity* pIdentity, bool pWasUnbound) {
  ...
  new_rhs_symbol->cv_id = pChunkIdentity;
...
rhs_value allocate_rhs_value_for_symbol(agent* thisAgent, Symbol* sym,
                                        uint64_t pInstIdentity,
                                        uint64_t pChunkIdentity,
                                        Identity* pIdentity, bool pWasUnbound) {
  rhs_symbol new_rhs_symbol;
