...
/* The last argument to add_rhs_function determines whether EBC literalizes the
 * arguments of the function, i.e. forces the identity sets of any arguments
 * that are variables to map to the null identity set.  This will cause any
 * variables that map to that identity set to to not be variablized in the final
 * chunk.
 *
 * Currently, ONLY MATH FUNCTIONS cause literalization of their arguments.
 *
 *   - One could argue that make-constant-symbol, trim, capitalize_symbol should
 * also literalize arguments, but it seems unlikely that their products would
 * affect behavior.  Moreover, an agent that used those functions to print a lot
 * of trace messages could end up literalizing a lot of variables unnecessarily.
 *   - One could also argue that ifeq should be literalized since it could
 * return an identifier. It looks the original author of ifeq made it for pretty
 * printing and not a way to get conditional rhs actions, which we probably
 * don't want to support.  So I grouped this with the other text producing rhs
 * functions that don't cause literalization.
 *
 * Technically, we only need to literalize the arguments of a rhs function if
 * another condition later tests that the value produced meets a constraint.  If
 * they don't have constraints, we could actually compose the functions used
 * into the variablized rhs actions of the final chunk.  If we add that
 * capability to EBC down the line, the semantics of this parameter will change
 * to indicate whether to compose (if no constraints) or literalize (if later
 * constraints on value exist).  If we add that, we should definitely include
 * if-eq, make-constant-symbol, trim, and capitalize_symbol, so that they can be
 * composed into the learned rule rather than be hard-coded (possibly
 * inaccurate) strings. */

void add_rhs_function(agent* thisAgent, Symbol* name, rhs_function_routine f,
                      int num_args_expected, bool can_be_rhs_value,
                      bool can_be_stand_alone_action, void* user_data,
  ...
  if ((!can_be_rhs_value) && (!can_be_stand_alone_action)) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "Internal error: attempt to add_rhs_function that can't appear "
        "anywhere\n");
    return;
  }
  ...
  for (rf = thisAgent->rhs_functions; rf != NIL; rf = rf->next) {
    if (rf->name == name) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "Internal error: attempt to add_rhs_function that already exists: "
          "%y\n",
          name);
      return;
    }
  ...
  rf = static_cast<rhs_function_struct*>(
      thisAgent->memoryManager->allocate_memory(sizeof(rhs_function),
...
void remove_rhs_function(agent* thisAgent,
                         Symbol* name) /* code from Koss 8/00 */
{
  rhs_function *rf = NIL, *prev;

  /* Find registered function by name */
  for (prev = NIL, rf = thisAgent->rhs_functions; rf != NIL;
       prev = rf, rf = rf->next) {
    if (rf->name == name) {
      break;
    }
  ...
  if (rf == NIL) {
    fprintf(stderr,
            "Internal error: attempt to remove_rhs_function that does not "
            "exist.\n");
    thisAgent->outputManager->printa_sf(
        thisAgent,
  ...
  /* Else, remove it */
    ...
    //        if (rf->cached_print_str) free_memory_block_for_string(thisAgent,
    //        rf->cached_print_str);
...
Symbol* write_rhs_function_code(agent* thisAgent, cons* args,
  ...
  if (thisAgent->outputManager->settings[OM_AGENT_WRITES]) {
    ...
    char* string;
    growable_string gs =
        make_blank_growable_string(thisAgent);  // for XML generation
    ...
    xml_object(thisAgent, kTagRHS_write, kRHS_String,
...
Symbol* trace_rhs_function_code(agent* thisAgent, cons* args,
  ...
  if (thisAgent->outputManager->settings[OM_AGENT_WRITES]) {
    ...
    if (!lChannelSym->is_int() || (lChannelSym->ic->value < 0) ||
        (lChannelSym->ic->value > 5)) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "%eError: First argument of agent's (trace) rhs-function must be an "
          "integer trace level between 0 and 5.  %y is invalid.\n",
          lChannelSym);
      return NIL;
    }
    args = args->rest;
...
Symbol* log_rhs_function_code(agent* thisAgent, cons* args,
  ...
  if (thisAgent->outputManager->settings[OM_AGENT_WRITES]) {
    ...
    Symbol* lChannelSym = static_cast<Symbol*>(args->first);
    if (!lChannelSym->is_int() || (lChannelSym->ic->value < 1) ||
        (lChannelSym->ic->value > maxAgentTraces)) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "%eError: First argument of agent's (log) rhs-function must be an "
          "integer channel number between 1 and %d.  %y is invalid.\n",
          maxAgentTraces, lChannelSym);
      return NIL;
    }
    args = args->rest;
    if (thisAgent->output_settings
            ->agent_traces_enabled[(lChannelSym->ic->value - 1)]) {
      growable_string gs =
          make_blank_growable_string(thisAgent);  // for XML generation
...
Symbol* crlf_rhs_function_code(agent* thisAgent, cons* /*args*/,
                               void* /*user_data*/) {
  thisAgent->symbolManager->symbol_add_ref(
      thisAgent->symbolManager->soarSymbols.crlf_symbol);
...
Symbol* make_constant_symbol_rhs_function_code(agent* thisAgent, cons* args,
  ...
  if ((!args) &&
      (!thisAgent->symbolManager->find_str_constant(buf.str().c_str()))) {
    return thisAgent->symbolManager->make_str_constant(buf.str().c_str());
  }
  return thisAgent->symbolManager->generate_new_str_constant(
...
Symbol* timestamp_rhs_function_code(agent* thisAgent, cons* /*args*/,
                                    void* /*user_data*/) {
#define TIMESTAMP_BUFFER_SIZE 100
  ...
  SNPRINTF(buf, TIMESTAMP_BUFFER_SIZE, "%04u-%02u-%02u %02u:%02u:%02u.%03u",
...
Symbol* accept_rhs_function_code(agent* thisAgent, cons* /*args*/,
  ...
  release_io_symbol(thisAgent,
                    sym); /* because it was obtained using get_io_... */
  return sym;
}

Symbol* get_lti_id_rhs_function_code(agent* thisAgent, cons* args,
                                     void* /*user_data*/) {
  Symbol *sym, *returnSym;

  if (!args) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: '@' function called with no arguments.\n");
    return NIL;
  }
  ...
  if (!sym->is_lti()) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: %y is not linked to a semantic identifier.\n",
        sym);
    return NIL;
  }
  ...
  if (args->rest) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: '@' takes exactly 1 argument.\n");
    return NIL;
  }
...
Symbol* set_lti_id_rhs_function_code(agent* thisAgent, cons* args,
                                     void* /*user_data*/) {
  Symbol *sym, *ltiIDSym;

  if (!args) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: '@' rhs function called with no arguments.\n");
    return NIL;
  }
  ...
  if (!sym->is_sti()) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "%eError: '@' rhs function cannot accept %y because it is not a Soar "
        "identifier\n",
        sym);
    return NIL;
  }
  ...
  if (args->rest) {
    if (args->rest->rest) {
      thisAgent->outputManager->printa_sf(
          thisAgent, "%eError: '@' rhs function takes exactly 2 arguments.\n");
      return NIL;
    }
    ...
    if (!ltiIDSym->is_int()) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "%eError: '@' rhs function cannot accept %y as an LTI ID because it "
          "is not an integer\n",
          ltiIDSym);
      return NIL;
    }
  ...
  if (thisAgent->SMem->lti_exists(ltiIDSym->ic->value)) {
    ...
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "%eWarning: Long-term memory @%u does not exist.  Could not link "
        "short-term memory %y.\n",
...
Symbol* capitalize_symbol_rhs_function_code(agent* thisAgent, cons* args,
  ...
  if (!args) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "%eError: 'capitalize-symbol' function called with no arguments.\n");
    return NIL;
  }
  ...
  if (sym->symbol_type != STR_CONSTANT_SYMBOL_TYPE) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "%eError: non-symbol (%y) passed to capitalize-symbol function.\n",
        sym);
    return NIL;
  }
  ...
  if (args->rest) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'capitalize-symbol' takes exactly 1 argument.\n");
    return NIL;
  }
...
Symbol* ifeq_rhs_function_code(agent* thisAgent, cons* args,
  ...
  if (!args) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'ifeq' function called with no arguments\n");
    return NIL;
  }
  ...
  if (arg1 == arg2) {
    ...
    thisAgent->symbolManager->symbol_add_ref(
        static_cast<Symbol*>(c->rest->first));
...
Symbol* trim_rhs_function_code(agent* thisAgent, cons* args,
                               void* /*user_data*/) {
  char* symbol_to_trim;
  Symbol *sym, *returnSym;
  ...
  if (!args) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'trim' function called with no arguments.\n");
    return NIL;
  }
  ...
  if (sym->symbol_type != STR_CONSTANT_SYMBOL_TYPE) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: non-symbol (%y) passed to 'trim' function.\n",
        sym);
    return NIL;
  }
  ...
  if (args->rest) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'trim' takes exactly 1 argument.\n");
    return NIL;
  }
...
/**
 * Convert a symbol of any type to a string symbol representation. Uses
 * Symbol::to_string for all types except floats, which need special handling to
 * avoid losing precision.
 */
Symbol* string_rhs_function_code(agent* thisAgent, cons* args,
                                 void* /*user_data*/) {
  if (!args) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'string' function called with no arguments.\n");
    return NIL;
  }
  if (args->rest) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'string' takes exactly 1 argument.\n");
    return NIL;
  }
  ...
  Symbol* returnSym =
      thisAgent->symbolManager->make_str_constant(sym_to_stringify->to_string(
          false, false, NIL, 0, std::numeric_limits<double>::max_digits10));

  return returnSym;
}

Symbol* strlen_rhs_function_code(agent* thisAgent, cons* args,
                                 void* /*user_data*/) {
  Symbol* arg;
  char* string;
  ...
  return thisAgent->symbolManager->make_int_constant(
      static_cast<int64_t>(strlen(string)));
}
/* AGR 520     end */
...
Symbol* dont_learn_rhs_function_code(agent* thisAgent, cons* args,
  ...
  if (!args) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'dont-learn' function called with no arg.\n");
    return NIL;
  }
  ...
  if (state->symbol_type != IDENTIFIER_SYMBOL_TYPE) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "%eError: non-identifier (%y) passed to dont-learn function.\n", state);
    return NIL;
  } else if (!state->id->isa_goal) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "%eError: identifier passed to dont-learn is not a state: %y.\n",
        state);
  }

  if (args->rest) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'dont-learn' takes exactly 1 argument.\n");
    return NIL;
  }
  ...
  if (!member_of_list(
    ...
    push(thisAgent, state,
         thisAgent->explanationBasedChunker->chunk_free_problem_spaces);
    /* thisAgent->outputManager->printa_sf("State  %y  added to
     * chunk_free_list.\n",state); */
  }
  return NIL;
}
...
Symbol* force_learn_rhs_function_code(agent* thisAgent, cons* args,
  ...
  if (!args) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'force-learn' function called with no arg.\n");
    return NIL;
  }
  ...
  if (state->symbol_type != IDENTIFIER_SYMBOL_TYPE) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "%eError: non-identifier (%y) passed to force-learn function.\n",
        state);
    return NIL;
  } else if (!state->id->isa_goal) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "%eError: identifier passed to force-learn is not a state: %y.\n",
        state);
  }

  if (args->rest) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%eError: 'force-learn' takes exactly 1 argument.\n");
    return NIL;
  }

  if (!member_of_list(
          state, thisAgent->explanationBasedChunker->chunky_problem_spaces)) {
    push(thisAgent, state,
...
void recursive_wme_copy(
    agent* thisAgent, Symbol* parent_id, wme* curwme,
    std::unordered_map<Symbol*, Symbol*>& processedSymbols) {
  ...
  if (curwme->attr->is_sti()) {
    ...
    std::unordered_map<Symbol*, Symbol*>::iterator it =
        processedSymbols.find(curwme->attr);
    if (it != processedSymbols.end()) {
      /* Retrieve the previously created id symbol */
      new_attr = it->second;
    } else {
      /* Make a new id symbol */
      new_attr = thisAgent->symbolManager->make_new_identifier(
          curwme->attr->id->name_letter, 0, NIL);
      made_new_attr_symbol = true;
    }

    recursive_deep_copy_helper(thisAgent, curwme->attr, new_attr,
                               processedSymbols);
  }
  ...
  if (curwme->value->symbol_type == 1) {
    /* Have I already made a new identifier for this identifier */
    std::unordered_map<Symbol*, Symbol*>::iterator it =
        processedSymbols.find(curwme->value);
    if (it != processedSymbols.end()) {
      /* Retrieve the previously created id symbol */
      new_value = it->second;
    } else {
      /* Make a new id symbol */
      new_value = thisAgent->symbolManager->make_new_identifier(
          curwme->value->id->name_letter, 0, NIL);
      made_new_value_symbol = true;
    }

    recursive_deep_copy_helper(thisAgent, curwme->value, new_value,
                               processedSymbols);
  }

  /* Making the new wme (Note just reusing the wme data structure, these wme's
   * actually get converted into preferences later).*/
  ...
  thisAgent->WM->glbDeepCopyWMEs.push_back(
      new deep_copy_wme(new_id, new_attr, new_value, curwme));
}
...
void recursive_deep_copy_helper(
  ...
  processedSymbols.insert(
      std::pair<Symbol*, Symbol*>(id_to_process, parent_id));

  /* Iterating over the normal slot wmes */
  for (slot* curslot = id_to_process->id->slots; curslot != 0;
  ...
  for (wme* curwme = id_to_process->id->input_wmes; curwme != 0;
       curwme = curwme->next) {
    recursive_wme_copy(thisAgent, parent_id, curwme, processedSymbols);
  }
}
...
Symbol* deep_copy_rhs_function_code(agent* thisAgent, cons* args,
  ...
  /* Make the new root identifier symbol.  We'll set the level in
...
Symbol* xmltowme_from_xml_internal(agent* thisAgent, ElementXML_Handle element,
                                   Symbol* targetId, LinkMap& linkMap,
                                   Links& links);

Symbol* xmltowme_get_value(agent* thisAgent, ElementXML_Handle element,
                           LinkMap& linkMap, Links& links) {
  Symbol* result = NULL;
  ...
  if (value == NULL) value = soarxml_GetCharacterData(element);
...
Symbol* xmltowme_from_xml_internal(agent* thisAgent, ElementXML_Handle element,
  ...
  for (int i = 0; i < numChildren; i++) {
    ...
    if (tag == NULL) continue;  // Warning null tag on node
...
Symbol* xmltowme_from_xml(agent* thisAgent, ElementXML_Handle element,
                          Symbol* targetId) {
  LinkMap linkMap;
  Links links;
  Symbol* result =
      xmltowme_from_xml_internal(thisAgent, element, targetId, linkMap, links);

  for (Link link : links) {
    auto targetIt = linkMap.find(std::string(link.linkTo));
    if (targetIt != linkMap.end()) {
      soar_module::add_module_wme(thisAgent, link.from, link.attribute,
                                  targetIt->second, false);
    } else {
...
Symbol* xmltowme_rhs_function_code(agent* thisAgent, cons* args,
                                   void* /*user_data*/) {
  // No arguments
...
void init_built_in_rhs_functions(agent* thisAgent) {
   ...
   * sml_RhsFunction.cpp instead of here in the kernel:  concat, exec, cmd and
   * interrupt */

  /* Stand-alone RHS functions */
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("count"),
                   count_rhs_function_code, -1, false, true, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("dont-learn"),
                   dont_learn_rhs_function_code, 1, false, true, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("force-learn"),
                   force_learn_rhs_function_code, 1, false, true, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("halt"),
                   halt_rhs_function_code, 0, false, true, 0, false);
  add_rhs_function(
      thisAgent, thisAgent->symbolManager->make_str_constant("link-stm-to-ltm"),
      set_lti_id_rhs_function_code, 2, false, true, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("wait"),
                   wait_rhs_function_code, 1, false, true, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("write"),
                   write_rhs_function_code, -1, false, true, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("trace"),
                   trace_rhs_function_code, -1, false, true, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("log"),
                   log_rhs_function_code, -1, false, true, 0, false);

  /* RHS functions that return a simple value */
  add_rhs_function(thisAgent, thisAgent->symbolManager->make_str_constant("@"),
                   get_lti_id_rhs_function_code, 1, true, false, 0, false);
  add_rhs_function(
      thisAgent,
      thisAgent->symbolManager->make_str_constant("capitalize-symbol"),
      capitalize_symbol_rhs_function_code, 1, true, false, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("crlf"),
                   crlf_rhs_function_code, 0, true, false, 0, false);
  add_rhs_function(thisAgent, thisAgent->symbolManager->make_str_constant("dc"),
                   dc_rhs_function_code, 0, true, false, 0, false);
  add_rhs_function(
      thisAgent,
      thisAgent->symbolManager->make_str_constant("make-constant-symbol"),
      make_constant_symbol_rhs_function_code, -1, true, false, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("strlen"),
                   strlen_rhs_function_code, 1, true, false, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("timestamp"),
                   timestamp_rhs_function_code, 0, true, false, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("trim"),
                   trim_rhs_function_code, 1, true, false, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("string"),
                   string_rhs_function_code, 1, true, false, 0, true);

  /* RHS functions that are more elaborate */
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("accept"),
                   accept_rhs_function_code, 0, true, false, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("deep-copy"),
                   deep_copy_rhs_function_code, 1, true, false, 0, false);
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("ifeq"),
                   ifeq_rhs_function_code, 4, true, false, 0, false);

  // TODO: Not sure about some of the final params in this call?
  add_rhs_function(thisAgent,
                   thisAgent->symbolManager->make_str_constant("from-st-xml"),
                   xmltowme_rhs_function_code, 1, true, false, 0, false);

  /* EBC Manager caches these rhs functions since it may re-use them many times
   */
  thisAgent->explanationBasedChunker->lti_link_function = lookup_rhs_function(
      thisAgent,
      thisAgent->symbolManager->find_str_constant("link-stm-to-ltm"));
...
void remove_built_in_rhs_functions(agent* thisAgent) {
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("count"));
  remove_rhs_function(
      thisAgent, thisAgent->symbolManager->find_str_constant("dont-learn"));
  remove_rhs_function(
      thisAgent, thisAgent->symbolManager->find_str_constant("force-learn"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("halt"));
  remove_rhs_function(thisAgent, thisAgent->symbolManager->find_str_constant(
                                     "link-stm-to-ltm"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("wait"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("write"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("trace"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("log"));

  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->soarSymbols.at_symbol);
  remove_rhs_function(thisAgent, thisAgent->symbolManager->find_str_constant(
                                     "capitalize-symbol"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("crlf"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("dc"));
  remove_rhs_function(thisAgent, thisAgent->symbolManager->find_str_constant(
                                     "make-constant-symbol"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("strlen"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("timestamp"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("trim"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("string"));

  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("accept"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("deep-copy"));
  remove_rhs_function(thisAgent,
                      thisAgent->symbolManager->find_str_constant("ifeq"));
  remove_rhs_function(
      thisAgent, thisAgent->symbolManager->find_str_constant("from-st-xml"));
