...
void find_match_goal(agent* thisAgent, instantiation* inst) {
  ...
  if (lowest_goal_so_far) {
    ...
    inst->match_goal =
        find_goal_at_goal_stack_level(thisAgent, lowest_id_level_so_far);
...
void set_bt_and_find_match_goal(instantiation* inst) {
  ...
  for (cond = inst->top_of_instantiated_conditions; cond != NIL;
    ...
    if (cond->type == POSITIVE_CONDITION) {
      ...
      cond->bt.trace =
          cond->bt.wme_->preference;  // These are later changed to the correct
                                      // clone for the level
      id = cond->bt.wme_->id;
      if (id->id->isa_goal)
        if (cond->bt.level > lowest_level_so_far) {
...
uint64_t get_rhs_function_first_arg_identity(agent* thisAgent, rhs_value rv) {
  ...
  thisAgent->explanationBasedChunker->deep_copy_sym_expanded =
      rhs_value_to_symbol(static_cast<char*>(firstArg));
  return returnVal;
}
...
Symbol* instantiate_rhs_value(agent* thisAgent, rhs_value rv,
  ...
  /* We pass back whether this was an unbound var via wasUnboundVar so that the
   * re-orderer will not consider it unconnected to the LHS if it's a child of
   * an unconnected symbol */
  wasUnboundVar = false;
  ...
  if (rhs_value_is_unboundvar(rv)) {
    ...
    if (!sym) {
      sym = thisAgent->symbolManager->make_new_identifier(new_id_letter,
                                                          new_id_level);
  ...
  for (arg_cons = fl->rest; arg_cons != NIL; arg_cons = arg_cons->rest) {
    ...
    c->first = instantiate_rhs_value(
        thisAgent, static_cast<char*>(arg_cons->first), new_id_level,
        new_id_letter, tok, w, dummy_was_unbound_var);
    if (!c->first) {
      nil_arg_found = true;
    }
    if (prev_c) {
...
preference* execute_action(agent* thisAgent, action* a,
  ...
  if (!lId->is_sti()) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "Error: RHS of action %a makes a preference for %y (not an "
        "identifier)\n",
        a, lId);
    goto abort_execute_action;
  }
  ...
  lValue = instantiate_rhs_value(thisAgent, a->value, lId->id->level,
                                 first_letter, tok, w, was_unbound_vars.value);
  if (!lValue) {
    goto abort_execute_action;
  }
  ...
  if (preference_is_binary(a->preference_type)) {
    lReferent =
        instantiate_rhs_value(thisAgent, a->referent, lId->id->level,
                              first_letter, tok, w, was_unbound_vars.referent);
    if (!lReferent) {
      goto abort_execute_action;
    }
  }
  ...
  if (((a->preference_type != ACCEPTABLE_PREFERENCE_TYPE) &&
      ...
      (!(lId->id->isa_goal &&
         (lAttr == thisAgent->symbolManager->soarSymbols.operator_symbol)))) {
    thisAgent->outputManager->printa_sf(thisAgent,
                                        "\nError: attribute preference other "
                                        "than +/- for %y ^%y -- ignoring it.",
                                        lId, lAttr);
    goto abort_execute_action;
  }
  ...
  if (rule_action) {
    if (rule_action->id) {
      if (rhs_value_is_funcall(rule_action->id)) {
        f_id = rule_action->id;
        /* rule_action will get deallocated in create_instantiation, but we want
         * it in the preference for learning, so we just steal this copy and set
    ...
    if (rule_action->value) {
      if (rhs_value_is_funcall(rule_action->value)) {
        if (!thisAgent->WM->glbDeepCopyWMEs.empty()) {
          ...
          oid_value = get_rhs_function_first_arg_identity(thisAgent,
                                                          rule_action->value);
  ...
  newPref = make_preference(
      thisAgent, a->preference_type, lId, lAttr, lValue, lReferent,
      identity_quadruple(oid_id, oid_attr, oid_value, oid_referent),
      was_unbound_vars);

  /* We don't copy these rhs functions because update_preference_identities
   * needs to copy them to update identity set information later */
...
void calculate_support_for_instantiation_preferences(
  ...
  if (inst->prod->declared_support == DECLARED_O_SUPPORT) {
    ...
    /* check if the instantiation is proposing an operator.  if it is, then this
     * instantiation is i-supported.  */
    ...
    if (non_variabilized_inst->rete_wme) {
      for (act = non_variabilized_inst->prod->action_list; act != NIL;
           act = act->next) {
        if ((act->type == MAKE_ACTION) && (rhs_value_is_symbol(act->attr)) &&
            (rhs_value_to_rhs_symbol(act->attr)->referent ==
          ...
          if (rhs_value_is_reteloc(act->id) &&
              get_symbol_from_rete_loc(rhs_value_to_reteloc_levels_up(act->id),
                                       ...
                                       non_variabilized_inst->rete_wme)
                  ->is_state()) {
            ...
            if (pass == 0) {
              if (w->id->id->isa_goal == true) {
                if (lowest_goal_wme == NIL)
                  lowest_goal_wme = w;
                else if (w->id->id->level > lowest_goal_wme->id->id->level)
                  lowest_goal_wme = w;
              }
              ...
              if ((w->attr ==
                   thisAgent->symbolManager->soarSymbols.operator_symbol) &&
                  (w->acceptable == false) && (w->id == lowest_goal_wme->id)) {
                /* iff RHS has only operator elaborations then it's IE_PROD,
                 * otherwise PE_PROD, so look for non-op-elabs in the actions */
                for (act = inst->prod->action_list; act != NIL;
                     act = act->next) {
...
void finalize_instantiation(agent* thisAgent, instantiation* inst,
  ...
  bool lDoIdentities =
      (isSubGoalMatch && thisAgent->explanationBasedChunker
    ...
    if (cond->type == POSITIVE_CONDITION) {
      ...
      if (isSubGoalMatch && cond->bt.trace) {
        if (cond->bt.trace->level > inst->match_goal_level) {
          cond->bt.trace =
              find_clone_for_level(cond->bt.trace, inst->match_goal_level);
        }
        if (cond->bt.trace) {
          ...
          if (lDoIdentities) {
            ...
            if (cond->bt.trace->level == inst->match_goal_level) {
              if (lTest_id->inst_identity)
                set_test_identity(
                    thisAgent, lTest_id,
                    thisAgent->explanationBasedChunker->get_or_add_identity(
                        lTest_id->inst_identity, cond->bt.trace->identities.id,
                        inst->match_goal));
              if (lTest_attr->inst_identity)
                set_test_identity(
                    thisAgent, lTest_attr,
                    thisAgent->explanationBasedChunker->get_or_add_identity(
                        lTest_attr->inst_identity,
                        cond->bt.trace->identities.attr, inst->match_goal));
              if (lTest_value->inst_identity)
                set_test_identity(
                    thisAgent, lTest_value,
                    thisAgent->explanationBasedChunker->get_or_add_identity(
                        lTest_value->inst_identity,
                        cond->bt.trace->identities.value, inst->match_goal));
            } else if (inst->match_goal_level > TOP_GOAL_LEVEL) {
              if (lTest_id->inst_identity)
                set_test_identity(
                    thisAgent, lTest_id,
                    thisAgent->explanationBasedChunker->get_or_add_identity(
                        lTest_id->inst_identity, NULL_IDENTITY_SET,
                        inst->match_goal));
              if (lTest_attr->inst_identity)
                set_test_identity(
                    thisAgent, lTest_attr,
                    thisAgent->explanationBasedChunker->get_or_add_identity(
                        lTest_attr->inst_identity, NULL_IDENTITY_SET,
                        inst->match_goal));
              if (lTest_value->inst_identity)
                set_test_identity(
                    thisAgent, lTest_value,
                    thisAgent->explanationBasedChunker->get_or_add_identity(
                        lTest_value->inst_identity, NULL_IDENTITY_SET,
                        inst->match_goal));
            }
          }
        }
      ...
      if (cond->bt.wme_->local_singleton_value_identity_set && lDoIdentities &&
          (cond->bt.wme_->id == inst->match_goal)) {
        thisAgent->explanationBasedChunker->force_id_to_identity_mapping(
            cond->data.tests.id_test->eq_test->inst_identity,
            cond->bt.wme_->local_singleton_id_identity_set);
        thisAgent->explanationBasedChunker->force_id_to_identity_mapping(
            cond->data.tests.value_test->eq_test->inst_identity,
            cond->bt.wme_->local_singleton_value_identity_set);
        set_test_identity(thisAgent, cond->data.tests.id_test->eq_test,
                          cond->bt.wme_->local_singleton_id_identity_set);
        set_test_identity(thisAgent, cond->data.tests.value_test->eq_test,
                          cond->bt.wme_->local_singleton_value_identity_set);
        thisAgent->explanationMemory->increment_stat_identity_propagations();
      }
      if (lDoIdentities) {
        /* Architectural WME or we couldn't find a pref at the current level.
         * Start a new identity set */
        if (!cond->data.tests.id_test->eq_test->identity &&
            cond->data.tests.id_test->eq_test->inst_identity) {
          set_test_identity(
              thisAgent, cond->data.tests.id_test->eq_test,
              thisAgent->explanationBasedChunker->get_or_add_identity(
                  cond->data.tests.id_test->eq_test->inst_identity,
                  NULL_IDENTITY_SET, inst->match_goal));
        }
        if (!cond->data.tests.attr_test->eq_test->identity &&
            cond->data.tests.attr_test->eq_test->inst_identity) {
          set_test_identity(
              thisAgent, cond->data.tests.attr_test->eq_test,
              thisAgent->explanationBasedChunker->get_or_add_identity(
                  cond->data.tests.attr_test->eq_test->inst_identity,
                  NULL_IDENTITY_SET, inst->match_goal));
        }
        if (!cond->data.tests.value_test->eq_test->identity &&
            cond->data.tests.value_test->eq_test->inst_identity) {
          set_test_identity(
              thisAgent, cond->data.tests.value_test->eq_test,
              thisAgent->explanationBasedChunker->get_or_add_identity(
                  cond->data.tests.value_test->eq_test->inst_identity,
                  NULL_IDENTITY_SET, inst->match_goal));
        }
  ...
  if (lDoIdentities)
    thisAgent->explanationBasedChunker->update_identities_in_condlist(
        cond = inst->top_of_instantiated_conditions, inst);

  for (p = inst->preferences_generated; p != NIL; p = p->inst_next) {
    if (lDoIdentities) {
      thisAgent->explanationBasedChunker->update_identities_in_preferences(
          p, inst->match_goal, is_chunk_inst);
    }
    if (addToGoal) {
...
void add_pref_to_inst(agent* thisAgent, preference* pref, instantiation* inst) {
  ...
  /* The parser cannot determine if a rhs preference (<s> ^operator <o> = <x>)
   * is a relative operator indifferent preference or numeric indifferent
   * preference until it matches, so we set it here. */
...
void add_cond_to_arch_inst(agent* thisAgent, condition*& prev_cond,
                           instantiation* inst, wme* pWME,
                           bool addBTPref = true) {
  condition* cond;

  /* Add contents */
  cond =
      make_condition(thisAgent, make_test(thisAgent, pWME->id, EQUALITY_TEST),
  ...
  if (thisAgent->explanationBasedChunker
          ->ebc_settings[SETTING_EBC_LEARNING_ON]) {
    thisAgent->explanationBasedChunker->add_inst_identity_to_test(
...
void add_pref_to_arch_inst(agent* thisAgent, instantiation* inst, Symbol* pID,
                           Symbol* pAttr, Symbol* pValue) {
  preference* pref;

  pref = make_preference(thisAgent, ACCEPTABLE_PREFERENCE_TYPE, pID, pAttr,
                         pValue);
  ...
  if (thisAgent->explanationBasedChunker
          ->ebc_settings[SETTING_EBC_LEARNING_ON]) {
    pref->inst_identities.id =
        thisAgent->explanationBasedChunker->get_or_create_inst_identity_for_sym(
            pref->id);
    if (pref->attr->is_sti())
      pref->inst_identities.attr =
          thisAgent->explanationBasedChunker
              ->get_or_create_inst_identity_for_sym(pref->attr);
    if (pref->value->is_sti())
      pref->inst_identities.value =
          thisAgent->explanationBasedChunker
              ->get_or_create_inst_identity_for_sym(pref->value);
  }
  add_pref_to_inst(thisAgent, pref, inst);
}

void add_deep_copy_prefs_to_inst(agent* thisAgent, preference* pref,
                                 instantiation* inst) {
  ...
  for (auto it = thisAgent->WM->glbDeepCopyWMEs.begin();
       it != thisAgent->WM->glbDeepCopyWMEs.end(); it++) {
    lNewDC_WME = *it;
    ...
    if (lNewDC_WME->attr->is_sti() &&
        lNewDC_WME->attr->id->level == NO_WME_LEVEL) {
      lNewDC_WME->attr->id->level = glbDeepCopyWMELevel;
    }
    if (lNewDC_WME->value->is_sti() &&
        lNewDC_WME->value->id->level == NO_WME_LEVEL) {
      lNewDC_WME->value->id->level = glbDeepCopyWMELevel;
    }

    /* Add a condition that tests the WME that the deep copied preference is
     * based on */
    add_cond_to_arch_inst(thisAgent, inst->bottom_of_instantiated_conditions,
                          inst, lNewDC_WME->deep_copied_wme);

    /* Add the copied preference */
    lPref =
        make_preference(thisAgent, ACCEPTABLE_PREFERENCE_TYPE, lNewDC_WME->id,
                        lNewDC_WME->attr, lNewDC_WME->value);

    if (thisAgent->explanationBasedChunker
            ->ebc_settings[SETTING_EBC_LEARNING_ON]) {
      /* We set the identities of the preferences so that they are dependent on
       * the explanation behind the working memory elements that they were
       * copied from */
      lPref->inst_identities.id = thisAgent->explanationBasedChunker
                                      ->get_or_create_inst_identity_for_sym(
                                          lNewDC_WME->deep_copied_wme->id);
      lPref->inst_identities.attr = thisAgent->explanationBasedChunker
                                        ->get_or_create_inst_identity_for_sym(
                                            lNewDC_WME->deep_copied_wme->attr);
      lPref->inst_identities.value =
          thisAgent->explanationBasedChunker
              ->get_or_create_inst_identity_for_sym(
                  lNewDC_WME->deep_copied_wme->value);
    }

    /* Now add a preferences that will create the deep-copied WME */
    add_pref_to_inst(thisAgent, lPref, inst);

    /* Clean up the deep copy struct.  We didn't increase refcounts on their
     * symbols. */
    delete lNewDC_WME;
  }
...
void init_instantiation(agent* thisAgent, instantiation*& inst,
                        Symbol* backup_name, production* prod,
                        struct token_struct* tok, wme* w) {
...
void create_instantiation(agent* thisAgent, production* prod,
  ...
  init_instantiation(
      thisAgent, inst,
      thisAgent->symbolManager->soarSymbols.architecture_inst_symbol, prod, tok,
      w);
  ...
  if (thisAgent->trace_settings[TRACE_ASSERTIONS_SYSPARAM]) {
    ...
    thisAgent->outputManager->sprinta_sf(
        ...
        inst->prod_name, inst->match_goal,
        static_cast<int64_t>(inst->match_goal_level));
  ...
  p_node_to_conditions_and_rhs(
      thisAgent, prod->p_node, tok, w, &(inst->top_of_instantiated_conditions),
      &(inst->bottom_of_instantiated_conditions),
      (ebcTraceType != WM_Trace) ? &(rhs_vars) : NULL, ebcTraceType);
  ...
  if (!isSubGoalMatch && (prod->type != TEMPLATE_PRODUCTION_TYPE)) {
    /* We don't need identity information or the original vars on the top level,
     * so we clean up now */
 ...
 */
  ...
  for (a = prod->action_list; a != NIL; a = a->next) {
    ...
    if (pref) {
      ...
      if (!thisAgent->WM->glbDeepCopyWMEs.empty()) {
        ...
        if (isSubGoalMatch && thisAgent->explanationBasedChunker
                                  ->ebc_settings[SETTING_EBC_LEARNING_ON]) {
          thisAgent->explanationBasedChunker->force_add_inst_identity(
              pref->value, pref->inst_identities.value);
          thisAgent->explanationBasedChunker->force_add_inst_identity(
              thisAgent->explanationBasedChunker->deep_copy_sym_expanded,
              pref->inst_identities.value);
        }
  ...
  if (isSubGoalMatch) {
    /* Copy any operator selection knowledge preferences for conditions of this
     * instantiation */
    ...
    thisAgent->explanationBasedChunker->learn_rule_from_instance(
        inst, &(thisAgent->newly_created_instantiations));
  }
...
void deallocate_instantiation(agent* thisAgent, instantiation*& inst) {
  ...
  while (next_iter != l_instantiation_list.end()) {
    ...
    for (condition* cond = lInst->top_of_instantiated_conditions; cond != NIL;
         cond = cond->next) {
      if (cond->type == POSITIVE_CONDITION) {
        /*  voigtjr, nlderbin:
         We flattened out the following recursive loop in order to prevent a
         stack overflow that happens when the chain of backtrace instantiations
         is very long:
         ...
         Note:  If one of those functions that are flattened out here changes,
         this code may need updating. - Mazin */
 ...
 * */
...
#endif
          {
            if (lPref->reference_count != 0) lPref->reference_count--;
            if (lPref->reference_count == 0) {
              ...
              if (lPref->on_goal_list) {
                remove_from_dll(
                    lPref->inst->match_goal->id->preferences_from_goal, lPref,
                    all_of_goal_next, all_of_goal_prev);
  ...
  for (inst_list::reverse_iterator riter = l_instantiation_list.rbegin();
       riter != l_instantiation_list.rend(); ++riter) {
    ...
    deallocate_condition_list(thisAgent,
                              lDelInst->top_of_instantiated_conditions);
    ...
    if (lDelInst->OSK_proposal_prefs) {
      /* These prefs did not have their refcounts increased, so we don't want to
       * call clear_preference_list */
    ...
    if (lDelInst->prod) {
      if ((lDelInst->prod->type == JUSTIFICATION_PRODUCTION_TYPE) &&
          (lDelInst->prod->reference_count == 1)) {
        /* We are about to remove a justification that has not been excised from
         * the rete. Normally, justifications are excised as soon as they don't
         * have any matches in rete.cpp.  But if removing the preference will
         * remove the instantiation, we need to excise it now so that the rete
         * doesn't try to later */
...
void retract_instantiation(agent* thisAgent, instantiation* inst) {
      ...
      if (trace_it) {
        if (!retracted_a_preference) {
          ...
          print_instantiation_with_wmes(
              ...
              static_cast<wme_trace_type>(
                  thisAgent
                      ->trace_settings[TRACE_FIRINGS_WME_TRACE_TYPE_SYSPARAM]),
              1);
          if (thisAgent
                  ->trace_settings[TRACE_FIRINGS_WME_TRACE_TYPE_SYSPARAM] !=
              NONE_WME_TRACE) {
  ...
  if (inst->prod) {
    ...
    if (prod->type == CHUNK_PRODUCTION_TYPE) {
       ...
       * - An RL rule that has not been updated by RL AND is not in line to be
       * updated by RL
       */
...
instantiation* make_architectural_instantiation_for_memory_system(
    agent* thisAgent, Symbol* pState, wme_set* pConds,
    symbol_triple_list* pActions, bool forSMem) {
  instantiation* inst;

  init_instantiation(
      thisAgent, inst,
      thisAgent->symbolManager->soarSymbols.fake_instantiation_symbol);
  ...
  // create LHS
  {
    ...
    if (forSMem) {
      add_cond_to_arch_inst(thisAgent, prev_cond, inst,
                            pState->id->smem_info->smem_link_wme, false);
      add_cond_to_arch_inst(thisAgent, prev_cond, inst,
                            pState->id->smem_info->cmd_wme, false);
      add_cond_to_arch_inst(thisAgent, prev_cond, inst,
                            pState->id->smem_info->result_wme, false);
    } else {
      add_cond_to_arch_inst(thisAgent, prev_cond, inst,
                            pState->id->epmem_info->epmem_link_wme, false);
      add_cond_to_arch_inst(thisAgent, prev_cond, inst,
                            pState->id->epmem_info->cmd_wme, false);
      add_cond_to_arch_inst(thisAgent, prev_cond, inst,
                            pState->id->epmem_info->result_wme, false);
    }
    for (wme_set::iterator c_it = pConds->begin(); c_it != pConds->end();
         c_it++) {
  ...
  // create RHS
  {
    for (symbol_triple_list::iterator a_it = pActions->begin();
         a_it != pActions->end(); a_it++) {
      add_pref_to_arch_inst(thisAgent, inst, (*a_it)->id, (*a_it)->attr,
                            (*a_it)->value);
    }
  }
  ...
  /* Initialize levels, add refcounts to prefs/wmes, sets on_goal_list flag,
   * o-support calculation if needed */
...
preference* make_architectural_instantiation_for_impasse_item(
  ...
  /* Find the acceptable preference wme we want to backtrace to and the
   * superstate wme to link rule to dependent knowledge in superstate.  Now
   * needed for EBC. */
  s = cand->slot;
  for (ap_wme = s->acceptable_preference_wmes; ap_wme != NIL;
       ap_wme = ap_wme->next)
    if (ap_wme->value == cand->value) break;
  for (ss_link_wme = goal->id->impasse_wmes; ss_link_wme != NIL;
       ss_link_wme = ss_link_wme->next)
    if (ss_link_wme->attr ==
  ...
  // Create the RHS
  add_pref_to_arch_inst(thisAgent, inst, goal,
                        thisAgent->symbolManager->soarSymbols.item_symbol,
                        cand->value);
