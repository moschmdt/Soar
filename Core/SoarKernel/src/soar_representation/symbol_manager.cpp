...
uint32_t hash_identifier_raw_info(char name_letter, uint64_t name_number,
                                  short num_bits) {
  return compress(static_cast<uint32_t>(name_number) ^
                      (static_cast<uint32_t>(name_letter) << 24),
                  num_bits);  // FIXME: cast from 64 to 32 bits
}
...
uint32_t hash_identifier(void* item, short num_bits) {
  ...
  return compress(static_cast<uint32_t>(id->name_number) ^
                      (static_cast<uint32_t>(id->name_letter) << 24),
                  num_bits);  // FIXME: cast from 64 to 32 bits
}
...
void Symbol_Manager::init_symbol_tables() {
  ...
  float_constant_hash_table =
      make_hash_table(thisAgent, 0, hash_float_constant);

  thisAgent->memoryManager->init_memory_pool(MP_variable, sizeof(varSymbol),
                                             "variable");
  thisAgent->memoryManager->init_memory_pool(MP_identifier, sizeof(idSymbol),
                                             "identifier");
  thisAgent->memoryManager->init_memory_pool(MP_str_constant, sizeof(strSymbol),
                                             "str constant");
  thisAgent->memoryManager->init_memory_pool(MP_int_constant, sizeof(intSymbol),
                                             "int constant");
  thisAgent->memoryManager->init_memory_pool(
      MP_float_constant, sizeof(floatSymbol), "float constant");

  reset_id_counters();
}

bool retesave_symbol_and_assign_index(agent* thisAgent, void* item,
                                      void* userdata) {
  Symbol* sym;
...
void Symbol_Manager::retesave(FILE* f) {
  ...
  do_for_all_items_in_hash_table(thisAgent, str_constant_hash_table,
                                 retesave_symbol_and_assign_index, f);
  do_for_all_items_in_hash_table(thisAgent, variable_hash_table,
                                 retesave_symbol_and_assign_index, f);
  do_for_all_items_in_hash_table(thisAgent, int_constant_hash_table,
                                 retesave_symbol_and_assign_index, f);
  do_for_all_items_in_hash_table(thisAgent, float_constant_hash_table,
                                 retesave_symbol_and_assign_index, f);
}
Symbol* Symbol_Manager::find_variable(const char* name) {
  ...
  sym = reinterpret_cast<varSymbol*>(
      *(variable_hash_table->buckets + hash_value));
...
Symbol* Symbol_Manager::find_identifier(char name_letter,
  ...
  sym = reinterpret_cast<idSymbol*>(
      *(identifier_hash_table->buckets + hash_value));
...
Symbol* Symbol_Manager::find_str_constant(const char* name) {
  ...
  hash_value =
      hash_str_constant_raw_info(name, str_constant_hash_table->log2size);
  sym = reinterpret_cast<strSymbol*>(
      *(str_constant_hash_table->buckets + hash_value));
...
Symbol* Symbol_Manager::find_int_constant(int64_t value) {
  ...
  hash_value =
      hash_int_constant_raw_info(value, int_constant_hash_table->log2size);
  sym = reinterpret_cast<intSymbol*>(
      *(int_constant_hash_table->buckets + hash_value));
...
Symbol* Symbol_Manager::find_float_constant(double value) {
  ...
  hash_value =
      hash_float_constant_raw_info(value, float_constant_hash_table->log2size);
  sym = reinterpret_cast<floatSymbol*>(
      *(float_constant_hash_table->buckets + hash_value));
...
Symbol* Symbol_Manager::make_new_identifier(char name_letter,
...
void Symbol_Manager::create_predefined_symbols() {
  ...
  soarSymbols.constraint_failure_symbol =
      make_str_constant("constraint-failure");
  ...
  soarSymbols.fake_instantiation_symbol =
      make_str_constant("Memory System Recall");
  ...
  soarSymbols.epmem_sym_normalized_match_score =
      make_str_constant("normalized-match-score");
  soarSymbols.epmem_sym_match_cardinality =
      make_str_constant("match-cardinality");
  ...
  soarSymbols.smem_sym_math_query_less_or_equal =
      make_str_constant("less-or-equal");
  soarSymbols.smem_sym_math_query_greater_or_equal =
      make_str_constant("greater-or-equal");
  soarSymbols.smem_sym_math_query_max = make_str_constant("max");
  soarSymbols.smem_sym_math_query_min = make_str_constant("min");
}

void Symbol_Manager::create_variables_for_range(const char lChar, int lStart,
                                                int lEnd) {
  int lNum;
  char msg[256];
...
void Symbol_Manager::release_variables_for_range(const char lChar, int lStart,
                                                 int lEnd) {
  Symbol* lVar;
  char msg[256];
...
void Symbol_Manager::deallocate_symbol(Symbol*& sym) {
  switch (sym->symbol_type) {
    ...
    case IDENTIFIER_SYMBOL_TYPE:
      if (sym->id->cached_print_str)
        free_memory_block_for_string(thisAgent, sym->id->cached_print_str);
      if (sym->id->cached_lti_str)
        free_memory_block_for_string(thisAgent, sym->id->cached_lti_str);
      remove_from_hash_table(thisAgent, identifier_hash_table, sym);
      thisAgent->memoryManager->free_with_pool(MP_identifier, sym);
      break;
    case STR_CONSTANT_SYMBOL_TYPE:
      if (sym->sc->cached_rereadable_print_str &&
          (sym->sc->cached_rereadable_print_str != sym->sc->name))
        free_memory_block_for_string(thisAgent,
                                     sym->sc->cached_rereadable_print_str);
    ...
    case INT_CONSTANT_SYMBOL_TYPE:
      if (sym->ic->cached_print_str)
        free_memory_block_for_string(thisAgent, sym->ic->cached_print_str);
    ...
    case FLOAT_CONSTANT_SYMBOL_TYPE:
      if (sym->fc->cached_print_str)
        free_memory_block_for_string(thisAgent, sym->fc->cached_print_str);
...
bool print_identifier_ref_info(agent* thisAgent, void* item, void* userdata) {
  ...
  if (sym->symbol_type == IDENTIFIER_SYMBOL_TYPE) {
    if (sym->reference_count > 0) {
      SNPRINTF(msg, 256, "\t%c%" SCNu64 " --> %" SCNu64 "\n",
    ...
    thisAgent->outputManager->printa_sf(
        thisAgent, "\tERROR: HASHTABLE ITEM IS NOT AN IDENTIFIER!\n");
    return true;
  }
  return false;
}
...
bool print_sym(agent* thisAgent, void* item, void*) {
  thisAgent->outputManager->printa_sf(
      thisAgent, "%s (%u)\n", static_cast<symbol_struct*>(item)->to_string(),
      static_cast<symbol_struct*>(item)->reference_count);
  return false;
}

void Symbol_Manager::clear_variable_gensym_numbers() {
  do_for_all_items_in_hash_table(thisAgent, variable_hash_table,
                                 clear_gensym_number, 0);
}

void Symbol_Manager::print_internal_symbols() {
  thisAgent->outputManager->printa_sf(thisAgent,
                                      "\n--- Symbolic Constants: ---\n");
  do_for_all_items_in_hash_table(thisAgent, str_constant_hash_table, print_sym,
                                 0);
  thisAgent->outputManager->printa_sf(thisAgent,
                                      "\n--- Integer Constants: ---\n");
  do_for_all_items_in_hash_table(thisAgent, int_constant_hash_table, print_sym,
                                 0);
  thisAgent->outputManager->printa_sf(thisAgent,
  ...
  do_for_all_items_in_hash_table(thisAgent, float_constant_hash_table,
                                 print_sym, 0);
  thisAgent->outputManager->printa_sf(thisAgent, "\n--- Identifiers: ---\n");
  do_for_all_items_in_hash_table(thisAgent, identifier_hash_table, print_sym,
                                 0);
  thisAgent->outputManager->printa_sf(thisAgent, "\n--- Variables: ---\n");
...
/* If you #define CONFIGURE_SOAR_FOR_UNIT_TESTS and INIT_AFTER_RUN unit_tests.h,
 * the following detect refcount leaks in unit tests and print out a message
 * accordingly */
#ifndef SOAR_RELEASE_VERSION
        if (identifier_hash_table->count < 23)
          do_for_all_items_in_hash_table(thisAgent, identifier_hash_table,
                                         print_sym, 0);
        else
          std::cout << "Refcount leak of " << identifier_hash_table->count
      ...
      {
        thisAgent->outputManager->printa_sf(
            thisAgent, "%d identifiers still exist.  Forcing deletion.\n",
            identifier_hash_table->count);
/* Note:  The do_for_all_items_in_hash_table printing could cause a crash if
 * there's memory corruption, but usually prints out and is good for debugging.
 */
#ifndef SOAR_RELEASE_VERSION
        do_for_all_items_in_hash_table(thisAgent, identifier_hash_table,
                                       print_sym, 0);
...
void Symbol_Manager::reset_id_and_variable_tc_numbers() {
  do_for_all_items_in_hash_table(thisAgent, identifier_hash_table, reset_tc_num,
                                 0);
  do_for_all_items_in_hash_table(thisAgent, variable_hash_table, reset_tc_num,
                                 0);
}

Symbol* Symbol_Manager::generate_new_str_constant(const char* prefix,
                                                  uint64_t* counter) {
#define GENERATE_NEW_STR_CONSTANT_BUFFER_SIZE \
  2000 /* that ought to be long enough! */
  char name[GENERATE_NEW_STR_CONSTANT_BUFFER_SIZE];
  Symbol* New;

  while (true) {
    SNPRINTF(name, GENERATE_NEW_STR_CONSTANT_BUFFER_SIZE, "%s%" SCNu64, prefix,
             (*counter)++);
...
void Symbol_Manager::reset_variable_generator(
    condition* conds_with_vars_to_avoid, action* actions_with_vars_to_avoid) {
  tc_number tc;
  cons* var_list;
  cons* c;
  int i;
  ...
  add_all_variables_in_condition_list(thisAgent, conds_with_vars_to_avoid, tc,
                                      &var_list);
  add_all_variables_in_action_list(thisAgent, actions_with_vars_to_avoid, tc,
                                   &var_list);
  for (c = var_list; c != NIL; c = c->rest) {
    static_cast<Symbol*>(c->first)->var->gensym_number =
        current_variable_gensym_number;
  }
...
#define GENERATE_NEW_VARIABLE_BUFFER_SIZE \
  200 /* that ought to be long enough! */
  ...
  while (true) {
    ...
    name[GENERATE_NEW_VARIABLE_BUFFER_SIZE - 1] =
