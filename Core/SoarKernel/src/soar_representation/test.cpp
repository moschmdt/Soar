...
cons* copy_test_list(agent* thisAgent, cons* c, test* pEq_test,
  ...
  new_c->first =
      copy_test(thisAgent, static_cast<test>(c->first),
                pUnify_variablization_identity, pStripLiteralConjuncts);
  if (static_cast<test>(new_c->first)->type == EQUALITY_TEST) {
    *pEq_test = static_cast<test>(new_c->first);
  }
  new_c->rest =
      copy_test_list(thisAgent, c->rest, pEq_test,
                     pUnify_variablization_identity, pStripLiteralConjuncts);
  return new_c;
}

inline bool in_null_identity_set(test t) {
  if (t->identity) return t->identity->literalized();
  return true;
};

test copy_test(agent* thisAgent, test t, bool pUseUnifiedIdentitySet,
               bool pStripLiteralConjuncts, bool remove_state_impasse,
               bool* removed_goal, bool* removed_impasse) {
  test new_ct;
  ...
  switch (t->type) {
    ...
    case CONJUNCTIVE_TEST:
      if (pStripLiteralConjuncts &&
          thisAgent->explanationBasedChunker
              ->ebc_settings[SETTING_EBC_LEARNING_ON] &&
          in_null_identity_set(t->eq_test)) {
        new_ct =
            make_test(thisAgent, t->eq_test->data.referent, t->eq_test->type);
        if (pUseUnifiedIdentitySet) {
          if (t->eq_test->identity) {
            new_ct->inst_identity = t->eq_test->identity->get_identity();
            set_test_identity(thisAgent, new_ct,
                              t->eq_test->identity->joined_identity);
            new_ct->chunk_inst_identity =
                t->eq_test->identity->get_clone_identity();
          } else {
        ...
        for (c = t->data.conjunct_list; c != NIL; c = c->rest) {
          temp = copy_test(thisAgent, static_cast<test>(c->first),
                           pUseUnifiedIdentitySet, pStripLiteralConjuncts,
        ...
        new_ct = make_test(thisAgent, NIL, t->type);
        new_ct->data.conjunct_list =
            copy_test_list(thisAgent, t->data.conjunct_list, &(new_ct->eq_test),
                           pUseUnifiedIdentitySet, pStripLiteralConjuncts);
      }
      break;
    default:
      ...
      if (pUseUnifiedIdentitySet &&
        ...
        new_ct->chunk_inst_identity =
            get_joined_identity_chunk_inst_id(new_ct->identity);
        set_test_identity(thisAgent, new_ct,
                          get_joined_identity(new_ct->identity));
      } else {
...
void merge_disjunction_tests(agent* thisAgent, test destination,
                             test new_test) {
  ...
  for (c_new = destination->data.disjunction_list; c_new != NULL;
       c_new = c_new->rest) {
...
bool add_test_merge_disjunctions(agent* thisAgent, test* dest_test_address,
                                 test new_test) {
  test destination = 0;
  cons* c;
...
bool add_test(agent* thisAgent, test* dest_test_address, test new_test,
  ...
  /* Since this function is called frequently but merges infrequently, we call a
   * special version of this function instead */
...
void add_test_if_not_already_there(agent* thisAgent, test* t, test add_me,
  ...
  if (thisAgent->explanationBasedChunker->is_learning_chunk())
    thisAgent->explanationMemory->increment_stat_operational_constraints();
...
uint32_t hash_test(agent* thisAgent, test t) {
  ...
  switch (t->type) {
    ...
    case CONJUNCTIVE_TEST:
      ...
      //  result = result + hash_test (thisAgent,
      //  static_cast<constraint>(c->first));
      return result;
    case NOT_EQUAL_TEST:
    case LESS_TEST:
    case GREATER_TEST:
...
bool test_includes_goal_or_impasse_id_test(test t, bool look_for_goal,
  ...
  if (t->type == CONJUNCTIVE_TEST) {
    for (c = t->data.conjunct_list; c != NIL; c = c->rest)
      if (test_includes_goal_or_impasse_id_test(
              static_cast<test>(c->first), look_for_goal, look_for_impasse)) {
        return true;
      }
    return false;
  }
...
void add_all_variables_in_test(agent* thisAgent, test t, tc_number tc,
  ...
  switch (t->type) {
    ...
    case CONJUNCTIVE_TEST:
      for (c = t->data.conjunct_list; c != NIL; c = c->rest) {
        add_all_variables_in_test(thisAgent, static_cast<test>(c->first), tc,
                                  var_list);
      }
      break;
...
void add_bound_variables_in_test(agent* thisAgent, test t, tc_number tc,
                                 cons** var_list) {
  cons* c;
  ...
  switch (t->type) {
    ...
    case CONJUNCTIVE_TEST:
      for (c = t->data.conjunct_list; c != NIL; c = c->rest) {
        add_bound_variables_in_test(thisAgent, static_cast<test>(c->first), tc,
                                    var_list);
      }
      break;
      /* If you re-enable the next section, variables bound to lti-id's will be
       * legal on the rhs. Note that they will not work as expected and will
       * point to instance variable */
      //        case SMEM_LINK_TEST:
      //            referent = t->data.referent;
      //            break;
    case EQUALITY_TEST:
...
void add_bound_variable_with_identity(agent* thisAgent, Symbol* pSym,
                                      Symbol* pMatchedSym,
                                      uint64_t pInstIdentity, tc_number tc,
                                      matched_symbol_list* var_list) {
  if (pSym->is_variable()) {
    if (pSym->tc_num != tc) {
      pSym->tc_num = tc;
      if (var_list) {
...
void add_rete_test_list_to_tests(agent* thisAgent,
  ...
  for (; rt != NIL; rt = rt->next) {
    if (rt->type == ID_IS_GOAL_RETE_TEST) {
      ...
      if (!rt->data.variable_referent.levels_up) {
        ...
        if (rt->data.variable_referent.field_num == 0) {
          ...
          if (!cond->data.tests.attr_test ||
            ...
            add_gensymmed_equality_test(thisAgent,
                                        &(cond->data.tests.attr_test), 'a');
          }
        } else {
      ...
      referent = var_bound_in_reconstructed_conds(
          thisAgent, cond, rt->data.variable_referent.field_num,
...
void add_hash_info_to_id_test(agent* thisAgent, condition* cond, byte field_num,
  ...
  temp =
      var_bound_in_reconstructed_conds(thisAgent, cond, field_num, levels_up);
 ...
 */
void copy_non_identical_test(agent* thisAgent, test* t, test add_me,
                             bool considerIdentity = false) {
  test target_test;
  cons* c;
 ...
 * Requires:  two lists
 * Modifies:  t
 * Effect:    This function copies any tests from add_me that aren't already in
 * t
 *
 ...
 *          as non-identical.
 */
void copy_non_identical_tests(agent* thisAgent, test* t, test add_me,
                              bool considerIdentity) {
  cons* c;
