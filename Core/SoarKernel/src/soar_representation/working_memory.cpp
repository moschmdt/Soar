...
struct SpdlogInitializer {
  SpdlogInitializer() { spdlog::set_level(spdlog::level::debug); }
};
static SpdlogInitializer _spdlog_init;
} // namespace
...
void reset_wme_timetags(agent *thisAgent) {
  if (thisAgent->num_existing_wmes != 0) {
    ...
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "there are still %u wmes allocated. (Probably a memory leak.)\n",
        thisAgent->num_existing_wmes);
    thisAgent->outputManager->printa(thisAgent,
                                     "(Leaving timetag numbers alone.)\n");
    xml_generate_warning(
        thisAgent, "Internal warning:  wanted to reset wme timetag generator, "
                   "but\nthere are still some wmes allocated. (Probably a "
                   "memory leak.)\n(Leaving timetag numbers alone.)");
    return;
  }
...
// Helper to produce a human-readable representation of a Symbol.
static std::string symbol_to_string(Symbol *s) {
  if (!s)
    return "NIL";
  std::string tmp;
  if (s->get_id_name(tmp))
    return tmp;
  char *c = s->to_string(false);
  return c ? std::string(c) : std::string("NIL");
}
...
void add_wme_to_wm(agent *thisAgent, wme *w) {
  ...
  spdlog::debug(
      "add_wme_to_wm: timetag={} id={} attr={} value={} acceptable={}",
      w->timetag, symbol_to_string(w->id), symbol_to_string(w->attr),
      symbol_to_string(w->value), w->acceptable);

  if (w->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) {
    post_link_addition(thisAgent, w->id, w->value);
    if (w->id->is_state() &&
        (w->attr == thisAgent->symbolManager->soarSymbols.operator_symbol)) {
...
void remove_wme_from_wm(agent *thisAgent, wme *w) {
  ...
  spdlog::debug("remove_wme_from_wm: timetag={} id={} attr={} value={}",
                w->timetag, symbol_to_string(w->id), symbol_to_string(w->attr),
                symbol_to_string(w->value));
...
/* --- stuff wme changes through the rete net --- */
  ...
  for (c = thisAgent->wmes_to_add; c != NIL; c = c->rest) {
    ...
    if (w->id->symbol_type == IDENTIFIER_SYMBOL_TYPE &&
        w->id->id
            ->LTI_ID) { // We attempt to keep track of ltis currently in wmem.
      if (thisAgent->SMem->smem_in_wmem->find(w->id->id->LTI_ID) ==
          thisAgent->SMem->smem_in_wmem->end()) {
        ...
        if (thisAgent->SMem->smem_context_removals->find(w->id->id->LTI_ID) !=
            thisAgent->SMem->smem_context_removals->end()) {
          thisAgent->SMem->smem_context_removals->erase(w->id->id->LTI_ID);
        }
      } else {
        (*(thisAgent->SMem->smem_in_wmem))[w->id->id->LTI_ID] =
            (*(thisAgent->SMem->smem_in_wmem))[w->id->id->LTI_ID] + 1;
      }
    }
#endif
    if (thisAgent->explanationBasedChunker
            ->ebc_settings[SETTING_AUTOMATICALLY_CREATE_SINGLETONS] &&
        (w->attr->is_string() && !w->attr->sc->singleton.possible)) {
      thisAgent->explanationBasedChunker->add_new_singleton(ebc_any, w->attr,
                                                            ebc_any);
    }
  ...
  for (c = thisAgent->wmes_to_remove; c != NIL; c = c->rest) {
    ...
    if (w->id->symbol_type == IDENTIFIER_SYMBOL_TYPE && w->id->id->LTI_ID) {
      if (thisAgent->SMem->smem_in_wmem->find(w->id->id->LTI_ID) !=
          thisAgent->SMem->smem_in_wmem->end()) {
        if ((*(thisAgent->SMem->smem_in_wmem))[w->id->id->LTI_ID] ==
            1) { // We are removing the last instance = actual removal.
          thisAgent->SMem->smem_in_wmem->erase(w->id->id->LTI_ID);
          if (thisAgent->SMem->smem_context_additions->find(
                  w->id->id->LTI_ID) !=
              thisAgent->SMem->smem_context_additions->end()) {
            thisAgent->SMem->smem_context_additions->erase(w->id->id->LTI_ID);
          } else {
            thisAgent->SMem->smem_context_removals->insert(w->id->id->LTI_ID);
          }
        } else { // just reducing the number of instances.
          (*(thisAgent->SMem->smem_in_wmem))[w->id->id->LTI_ID] =
              (*(thisAgent->SMem->smem_in_wmem))[w->id->id->LTI_ID] - 1;
        }
      }
...
#ifdef DETAILED_TIMING_STATS
  local_timer.stop();
  thisAgent->timers_match_cpu_time[thisAgent->current_phase].update(
      local_timer);
#endif
#endif
  ...
  if (thisAgent->trace_settings[TRACE_WM_CHANGES_SYSPARAM]) {
    for (c = thisAgent->wmes_to_add; c != NIL; c = next_c) {
      ...
      for (cr = thisAgent->wmes_to_remove; cr != NIL; cr = next_c) {
        next_c = cr->rest;
        if (w == cr->first) {
          const char *const kWarningMessage =
              "WARNING: WME added and removed in same phase : ";
          thisAgent->outputManager->printa(thisAgent,
                                           const_cast<char *>(kWarningMessage));
...
Symbol *find_name_of_object(agent *thisAgent, Symbol *object) {
  if (object->symbol_type != IDENTIFIER_SYMBOL_TYPE)
    return NIL;
  slot *s =
      find_slot(object, thisAgent->symbolManager->soarSymbols.name_symbol);
  if (!s)
    return NIL;
  if (!s->wmes)
    return NIL;
