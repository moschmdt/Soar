...
void init_soar_agent(agent *thisAgent) {
  ...
  thisAgent->memoryManager->init_memory_pool(MP_chunk_cond, sizeof(chunk_cond),
                                             "chunk_condition");
  thisAgent->memoryManager->init_memory_pool(
      MP_constraints, sizeof(constraint_struct), "constraints");
  thisAgent->memoryManager->init_memory_pool(
      MP_sym_triple, sizeof(symbol_triple), "symbol_triple");
  thisAgent->memoryManager->init_memory_pool(
      MP_identity_mapping, sizeof(identity_mapping), "id_mapping");
  thisAgent->memoryManager->init_memory_pool(
      MP_chunk_element, sizeof(chunk_element), "chunk_element");
  thisAgent->memoryManager->init_memory_pool(MP_identity_sets, sizeof(Identity),
                                             "identities");

  thisAgent->memoryManager->init_memory_pool(
      MP_action_record, sizeof(action_record), "action_record");
  thisAgent->memoryManager->init_memory_pool(
      MP_condition_record, sizeof(condition_record), "cond_record");
  thisAgent->memoryManager->init_memory_pool(
      MP_instantiation_record, sizeof(instantiation_record), "inst_record");
  thisAgent->memoryManager->init_memory_pool(
      MP_chunk_record, sizeof(chunk_record), "chunk_record");
  thisAgent->memoryManager->init_memory_pool(
      MP_production_record, sizeof(production_record), "prod_record");
  thisAgent->memoryManager->init_memory_pool(
      MP_repair_path, sizeof(Repair_Path), "repair_paths");

  thisAgent->memoryManager->init_memory_pool(
      MP_gds, sizeof(goal_dependency_set), "gds");

  thisAgent->memoryManager->init_memory_pool(MP_rl_info, sizeof(rl_data),
                                             "rl_id_data");
  thisAgent->memoryManager->init_memory_pool(MP_rl_et, sizeof(rl_et_map),
                                             "rl_et");
  thisAgent->memoryManager->init_memory_pool(
      MP_rl_rule, sizeof(production_list), "rl_rules");

  thisAgent->memoryManager->init_memory_pool(
      MP_wma_decay_element, sizeof(wma_decay_element), "wma_decay");
  thisAgent->memoryManager->init_memory_pool(
      MP_wma_decay_set, sizeof(wma_decay_set), "wma_decay_set");
  thisAgent->memoryManager->init_memory_pool(MP_wma_wme_oset, sizeof(wme_set),
                                             "wma_oset");
  thisAgent->memoryManager->init_memory_pool(
      MP_wma_slot_refs, sizeof(wma_sym_reference_map), "wma_slot_ref");

  thisAgent->memoryManager->init_memory_pool(
      MP_smem_wmes, sizeof(preference_list), "smem_wmes");
  thisAgent->memoryManager->init_memory_pool(MP_smem_info, sizeof(smem_data),
                                             "smem_id_data");

  thisAgent->memoryManager->init_memory_pool(
      MP_epmem_wmes, sizeof(preference_list), "epmem_wmes");
  thisAgent->memoryManager->init_memory_pool(MP_epmem_info, sizeof(epmem_data),
                                             "epmem_id_data");
  thisAgent->memoryManager->init_memory_pool(
      MP_epmem_literal, sizeof(epmem_literal), "epmem_literals");
  thisAgent->memoryManager->init_memory_pool(
      MP_epmem_pedge, sizeof(epmem_pedge), "epmem_pedges");
  thisAgent->memoryManager->init_memory_pool(
      MP_epmem_uedge, sizeof(epmem_uedge), "epmem_uedges");
  thisAgent->memoryManager->init_memory_pool(
      MP_epmem_interval, sizeof(epmem_interval), "epmem_intervals");
                   ...
                   "%id %ifdef[(%v[attribute] %v[impasse])]");
  {
    ...
    evaluate_object_sym =
        thisAgent->symbolManager->make_str_constant("evaluate-object");
...
agent *create_soar_agent(char *agent_name) /* loop index */
{
  ...
  if (!getcwd(cur_path, MAXPATHLEN)) {
    ...
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "Unable to set current directory while initializing agent: %s\n",
        error);
  }

  for (int productionTypeCounter = 0;
       productionTypeCounter < NUM_PRODUCTION_TYPES; productionTypeCounter++) {
  ...
  thisAgent->operatorExplainer = new OperatorExplanationManager(thisAgent);
...
void destroy_soar_agent(agent *delete_agent) {
  ...
  for (multi_attribute *curmattr = delete_agent->multi_attributes;
       curmattr != 0; curmattr = curmattr->next) {
    delete_agent->symbolManager->symbol_remove_ref(&(curmattr->symbol));
    delete_agent->memoryManager->free_memory(lastmattr,
  ...
  delete delete_agent->operatorExplainer;
  delete_agent->operatorExplainer = NULL;

  delete_agent->symbolManager->release_predefined_symbols();
  delete_agent->symbolManager->release_common_variables_and_numbers();

  delete_agent->memoryManager->free_with_pool(MP_rete_node,
                                              delete_agent->dummy_top_node);
  delete_agent->memoryManager->free_with_pool(MP_token,
                                              delete_agent->dummy_top_token);

  soar_remove_all_monitorable_callbacks(delete_agent);

  delete_agent->memoryManager->free_memory(delete_agent->left_ht,
                                           HASH_TABLE_MEM_USAGE);
  delete_agent->memoryManager->free_memory(delete_agent->right_ht,
                                           HASH_TABLE_MEM_USAGE);
  delete_agent->memoryManager->free_memory(delete_agent->rhs_variable_bindings,
                                           MISCELLANEOUS_MEM_USAGE);

  /* Releasing trace formats (needs to happen before tracing hashtables are
   * released) */
  remove_trace_format(delete_agent, false, FOR_ANYTHING_TF, NIL);
  remove_trace_format(delete_agent, false, FOR_STATES_TF, NIL);
  Symbol *evaluate_object_sym =
      delete_agent->symbolManager->find_str_constant("evaluate-object");
  remove_trace_format(delete_agent, false, FOR_OPERATORS_TF,
                      evaluate_object_sym);
...
void reinitialize_agent(agent *thisAgent) {
  /* Clean up explanation-based chunking, episodic and semantic memory data
   * structures */
  epmem_reinit(thisAgent);
  thisAgent->SMem->reinit();
  thisAgent->explanationBasedChunker->reinit();

  /* Turn off WM activation and RL forgetting temporarily while clearing the
   * goal stack */
  ...
  thisAgent->RL->rl_params->apoptosis->set_value(
  ...
  /* Signal that everything should be retracted and allow all i-instantiations
   * to retract */
  thisAgent->active_level = 0;
  thisAgent->FIRING_TYPE = IE_PRODS;
   ...
   * Note:  reset_hash_table() is where refcount leaks in identifiers are
   * detected. */
