...
wma_activation_param::wma_activation_param(
...
wma_decay_param::wma_decay_param(const char* new_name, double new_value,
...
wma_param_container::wma_param_container(agent* new_agent)
  ...
  // WMA on/off
  activation = new wma_activation_param(
      "activation", off, new soar_module::f_predicate<boolean>(), new_agent);
  add(activation);

  // decay-rate
  decay_rate = new wma_decay_param(
      "decay-rate", -0.5, new soar_module::btw_predicate<double>(0, 1, true),
      new wma_activation_predicate<double>(thisAgent));
  add(decay_rate);

  // decay-thresh
  decay_thresh = new wma_decay_param(
      "decay-thresh", -2.0, new soar_module::gt_predicate<double>(0, false),
      new wma_activation_predicate<double>(thisAgent));
  add(decay_thresh);

  // do we compute an approximation of the distant references?
  petrov_approx = new soar_module::boolean_param(
      "petrov-approx", off, new wma_activation_predicate<boolean>(thisAgent));
  add(petrov_approx);

  // are WMEs removed from WM when activation gets too low?
  forgetting = new soar_module::constant_param<forgetting_choices>(
      "forgetting", disabled,
      new wma_activation_predicate<forgetting_choices>(thisAgent));
  ...
  forget_wme = new soar_module::constant_param<forget_wme_choices>(
      "forget-wme", all,
      new wma_activation_predicate<forget_wme_choices>(thisAgent));
  forget_wme->add_mapping(all, "all");
  forget_wme->add_mapping(lti, "lti");
  add(forget_wme);

  // fake forgetting?
  fake_forgetting = new soar_module::boolean_param(
      "fake-forgetting", off, new wma_activation_predicate<boolean>(thisAgent));
  add(fake_forgetting);
  ...
  max_pow_cache = new soar_module::integer_param(
      "max-pow-cache", 10, new soar_module::gt_predicate<int64_t>(0, false),
      new wma_activation_predicate<int64_t>(thisAgent));
  add(max_pow_cache);
};
...
wma_stat_container::wma_stat_container(agent* new_agent)
    : soar_module::stat_container(new_agent) {
  // forgotten-wmes
  forgotten_wmes = new soar_module::integer_stat(
      "forgotten-wmes", 0, new soar_module::f_predicate<int64_t>());
  add(forgotten_wmes);
};

// Timer Functions (wma::timers)

wma_timer_container::wma_timer_container(agent* new_agent)
  ...
  forgetting =
      new wma_timer("wma_forgetting", thisAgent, soar_module::timer::one);
  add(forgetting);
}

//

wma_timer_level_predicate::wma_timer_level_predicate(agent* new_agent)
    : soar_module::agent_predicate<soar_module::timer::timer_level>(new_agent) {
}

bool wma_timer_level_predicate::operator()(
...
wma_timer::wma_timer(const char* new_name, agent* new_agent,
                     soar_module::timer::timer_level new_level)
    : soar_module::timer(new_name, new_agent, new_level,
                         new wma_timer_level_predicate(new_agent)) {}
...
void wma_init(agent* thisAgent) {
  ...
  // repeated calls to pow() at runtime
  {
    // determine cache size
    {
      ...
      // basic idea: solve for the time that would just fall below the decay
      // threshold, given decay rate and assumption of max references/decision
      // t = e^( ( thresh - ln( max_refs ) ) / -decay_rate )
      double cache_full = static_cast<double>(
          exp((decay_thresh -
               log(static_cast<double>(WMA_REFERENCES_PER_DECISION))) /
              decay_rate));

      // we bound this by the max-pow-cache parameter to control the space vs.
      // time tradeoff the cache supports max-pow-cache is in MB, so do the
      // conversion: MB * 1024 bytes/KB * 1024 KB/MB
      double cache_bound =
          (static_cast<unsigned int>(max_pow_cache * 1024 * 1024) /
           static_cast<unsigned int>(sizeof(double)));

      thisAgent->WM->wma_power_size = static_cast<unsigned int>(
          ceil((cache_full > cache_bound) ? (cache_bound) : (cache_full)));
    }
    ...
    for (unsigned int i = 1; i < thisAgent->WM->wma_power_size; i++) {
      thisAgent->WM->wma_power_array[i] =
          pow(static_cast<double>(i), decay_rate);
    }
  }
  ...
  // approximation cache
  if (thisAgent->WM->wma_params->forgetting->get_value() ==
      wma_param_container::approx) {
    thisAgent->WM->wma_approx_array =
        new wma_d_cycle[WMA_REFERENCES_PER_DECISION];

    thisAgent->WM->wma_approx_array[0] = 0;
    for (int i = 1; i < WMA_REFERENCES_PER_DECISION; i++) {
      thisAgent->WM->wma_approx_array[i] = static_cast<wma_d_cycle>(ceil(
          exp(static_cast<double>(decay_thresh - log(static_cast<double>(i))) /
              static_cast<double>(decay_rate))));
    }
  }
...
void wma_deinit(agent* thisAgent) {
  ...
  for (wma_forget_p_queue::iterator pq_p =
           thisAgent->WM->wma_forget_pq->begin();
       pq_p != thisAgent->WM->wma_forget_pq->end(); pq_p++) {
...
inline bool wma_should_have_decay_element(wme* w) {
  return ((w->preference) && (w->preference->reference_count) &&
          (w->preference->o_supported));
}
...
inline double wma_pow(agent* thisAgent, wma_d_cycle cycle_diff) {
  if (cycle_diff < thisAgent->WM->wma_power_size) {
    ...
    return pow(static_cast<double>(cycle_diff),
               thisAgent->WM->wma_params->decay_rate->get_value());
  }
}

inline double wma_sum_history(agent* thisAgent, wma_history* history,
  ...
  while (counter) {
    ...
    return_val += (history->access_history[p].num_references *
                   wma_pow(thisAgent, cycle_diff));
    ...
    if (history->total_references > history->history_references) {
      ...
      return_val +=
          (((history->total_references - history->history_references) *
            (pow(static_cast<double>(current_cycle - history->first_reference),
                 d_inv) -
             pow(static_cast<double>(cycle_diff), d_inv))) /
...
inline double wma_calculate_decay_activation(agent* thisAgent,
...
inline wma_reference wma_calculate_initial_boost(agent* thisAgent, wme* w) {
  ...
  for (pref = w->preference->slot->preferences[ACCEPTABLE_PREFERENCE_TYPE];
       pref; pref = pref->next) {
    if ((pref->value == w->value) && (pref->o_supported)) {
      for (cond = pref->inst->top_of_instantiated_conditions; cond != NIL;
           cond = cond->next) {
        if ((cond->type == POSITIVE_CONDITION) &&
            (cond->bt.wme_->wma_tc_value != tc)) {
          ...
          if (cond_wme->wma_decay_el) {
            if (!cond_wme->wma_decay_el->just_created) {
              num_cond_wmes++;
              combined_time_sum +=
                  wma_get_wme_activation(thisAgent, cond_wme, false);
            }
          } else if (cond_wme->preference) {
            if (cond_wme->preference->wma_o_set) {
              for (wme_p = cond_wme->preference->wma_o_set->begin();
                   wme_p != cond_wme->preference->wma_o_set->end(); wme_p++) {
                if (((*wme_p)->wma_tc_value != tc) &&
                    (!(*wme_p)->wma_decay_el ||
                     !(*wme_p)->wma_decay_el->just_created)) {
                  num_cond_wmes++;
                  combined_time_sum +=
                      wma_get_wme_activation(thisAgent, (*wme_p), false);
...
void wma_activate_wme(agent* thisAgent, wme* w, wma_reference num_references,
                      wme_set* o_set, bool o_only) {
  ...
  if (wma_should_have_decay_element(w)) {
    ...
    if (!temp_el) {
      thisAgent->memoryManager->allocate_with_pool(MP_wma_decay_element,
                                                   &temp_el);
    ...
    if (thisAgent->SMem->edge_updating_on() &&
        w->id->symbol_type == IDENTIFIER_SYMBOL_TYPE && w->id->id->LTI_ID) {
      if (w->value->id && w->value->id->LTI_ID) {
        // thisAgent->SMem->invalidate_from_lti(w->id->id->LTI_ID);//This is a
        // HUGE time sink.
        thisAgent->SMem->add_to_invalidate_from_lti_table(w->id->id->LTI_ID);
        /*for (int i = 1; i < 11; i++)
        {//A changing edge weight is treated as an invalidation of cases that
        could have used that edge.
        ...
        // Now, we keep track of the wma update that invalidated this spread so
        // that we can change edge weights later.
        ...
        if (thisAgent->SMem->smem_edges_to_update->find(w->id->id->LTI_ID) ==
            thisAgent->SMem->smem_edges_to_update
                ->end()) {  // We create an entry for that parent.
          thisAgent->SMem->smem_edges_to_update->emplace(
              std::pair<uint64_t, std::list<smem_edge_update*>>(
                  w->id->id->LTI_ID,
                  std::initializer_list<smem_edge_update*>{}));
        }
        std::list<smem_edge_update*>* list_ptr_for_parent =
            &(thisAgent->SMem->smem_edges_to_update->find(w->id->id->LTI_ID)
                  ->second);
  ...
  // i-supported, non-architectural WME
    ...
    if (!my_o_set) {
      ...
      for (condition* c = w->preference->inst->top_of_instantiated_conditions;
           c; c = c->next) {
...
inline void wma_forgetting_remove_from_p_queue(agent* thisAgent,
                                               wma_decay_element* decay_el);
void wma_deactivate_element(agent* thisAgent, wme* w) {
  ...
  if (temp_el) {
    if (!temp_el->just_removed) {
      ...
      if ((thisAgent->WM->wma_params->forgetting->get_value() ==
           wma_param_container::approx) ||
          (thisAgent->WM->wma_params->forgetting->get_value() ==
           wma_param_container::bsearch)) {
...
inline void wma_forgetting_add_to_p_queue(agent* thisAgent,
                                          wma_decay_element* decay_el,
                                          wma_d_cycle new_cycle) {
  if (decay_el) {
    decay_el->forget_cycle = new_cycle;

    wma_forget_p_queue::iterator pq_p =
        thisAgent->WM->wma_forget_pq->find(new_cycle);
    if (pq_p == thisAgent->WM->wma_forget_pq->end()) {
      ...
      newbie = new (newbie) wma_decay_set(
          std::less<wma_decay_element*>(),
          soar_module::soar_memory_pool_allocator<wma_decay_element*>(
              thisAgent));
#else
      newbie = new (newbie) wma_decay_set();
#endif
...
inline void wma_forgetting_remove_from_p_queue(agent* thisAgent,
  ...
  if (decay_el) {
    ...
    wma_forget_p_queue::iterator pq_p =
        thisAgent->WM->wma_forget_pq->find(decay_el->forget_cycle);
    if (pq_p != thisAgent->WM->wma_forget_pq->end()) {
...
inline void wma_forgetting_move_in_p_queue(agent* thisAgent,
                                           wma_decay_element* decay_el,
                                           wma_d_cycle new_cycle) {
...
inline wma_d_cycle wma_forgetting_estimate_cycle(agent* thisAgent,
                                                 wma_decay_element* decay_el,
                                                 bool fresh_reference) {
  wma_d_cycle return_val =
      static_cast<wma_d_cycle>(thisAgent->WM->wma_d_cycle_count);
  wma_param_container::forgetting_choices forgetting =
      thisAgent->WM->wma_params->forgetting->get_value();
  ...
  if (fresh_reference && (forgetting == wma_param_container::approx)) {
    ...
    while (counter) {
      ...
      approx_ref = ((history->access_history[p].num_references <
                     WMA_REFERENCES_PER_DECISION)
  ...
  if (return_val ==
    ...
    // binary parameter search
    {
      ...
      double act = wma_calculate_decay_activation(thisAgent, decay_el,
                                                  (return_val + to_add), false);

      if (act >= my_thresh) {
        while (act >= my_thresh) {
          to_add *= 2;
          act = wma_calculate_decay_activation(thisAgent, decay_el,
                                               (return_val + to_add), false);
        }
...
inline bool wma_forgetting_update_p_queue(agent* thisAgent) {
  ...
  if (!thisAgent->WM->wma_forget_pq->empty()) {
    ...
    bool forget_only_lti =
        (thisAgent->WM->wma_params->forget_wme->get_value() ==
         wma_param_container::lti);
    ...
    if (pq_p->first == current_cycle) {
      ...
      while (d_p != pq_p->second->end()) {
        ...
        if (wma_calculate_decay_activation(
                thisAgent, (*current_p), current_cycle, false) < decay_thresh) {
          (*current_p)->forget_cycle = WMA_FORGOTTEN_CYCLE;

          if (!forget_only_lti ||
              ((*current_p)->this_wme->id->id->LTI_ID != NIL)) {
            do_forget = true;

            // implements all-or-nothing check for lti mode
            if (forget_only_lti) {
              for (s = (*current_p)->this_wme->id->id->slots; (s && do_forget);
                   s = s->next) {
                for (w = s->wmes; (w && do_forget); w = w->next) {
                  if (w->preference->o_supported &&
                      (!w->wma_decay_el || (w->wma_decay_el->forget_cycle !=
                                            WMA_FORGOTTEN_CYCLE))) {
                    do_forget = false;
                  }
                }
            ...
            if (do_forget) {
              if (forget_only_lti) {
                ...
                for (s = (*current_p)->this_wme->id->id->slots;
                     (s && do_forget); s = s->next) {
                  for (w = s->wmes; (w && do_forget); w = w->next) {
                ...
                if (wma_forgetting_forget_wme(thisAgent,
                                              (*current_p)->this_wme)) {
                  return_val = true;
                }
              }
          ...
          wma_forgetting_move_in_p_queue(
              thisAgent, (*current_p),
              wma_forgetting_estimate_cycle(thisAgent, (*current_p), false));
        }
      }
    ...
    for (wma_decay_cycle_set::iterator touched_it =
             thisAgent->WM->wma_touched_sets->begin();
         touched_it != thisAgent->WM->wma_touched_sets->end(); touched_it++) {
      pq_p = thisAgent->WM->wma_forget_pq->find(*touched_it);

      if ((pq_p != thisAgent->WM->wma_forget_pq->end()) &&
          (pq_p->second->empty())) {
        pq_p->second->~wma_decay_set();
        thisAgent->memoryManager->free_with_pool(MP_wma_decay_set,
                                                 pq_p->second);
...
inline bool wma_forgetting_naive_sweep(agent* thisAgent) {
  wma_d_cycle current_cycle = thisAgent->WM->wma_d_cycle_count;
  double decay_thresh = thisAgent->WM->wma_thresh_exp;
  bool forget_only_lti = (thisAgent->WM->wma_params->forget_wme->get_value() ==
                          wma_param_container::lti);
  bool return_val = false;
  ...
  for (wme* w = thisAgent->all_wmes_in_rete; w; w = w->rete_next) {
    if (w->wma_decay_el && (!forget_only_lti || (w->id->id->LTI_ID != NIL))) {
      ...
      if ((w->wma_decay_el->touches.total_references > 0) &&
          (w->wma_decay_el->touches
               .access_history[wma_history_prev(
                   w->wma_decay_el->touches.next_p)]
               .d_cycle < current_cycle) &&
          (wma_calculate_decay_activation(thisAgent, w->wma_decay_el,
                                          current_cycle,
                                          false) < decay_thresh)) {
        if (wma_forgetting_forget_wme(thisAgent, w)) {
          return_val = true;
...
inline void wma_update_decay_histories(agent* thisAgent) {
  ...
  wma_d_cycle current_cycle = thisAgent->WM->wma_d_cycle_count;
  bool forgetting = ((thisAgent->WM->wma_params->forgetting->get_value() ==
                      wma_param_container::approx) ||
                     (thisAgent->WM->wma_params->forgetting->get_value() ==
                      wma_param_container::bsearch));

  // add to history for changed elements
  for (wme_p = thisAgent->WM->wma_touched_elements->begin();
       wme_p != thisAgent->WM->wma_touched_elements->end(); wme_p++) {
    ...
    temp_el->touches.history_references +=
        (temp_el->num_references -
         temp_el->touches.access_history[temp_el->touches.next_p]
             .num_references);

    // set history
    temp_el->touches.access_history[temp_el->touches.next_p].d_cycle =
        current_cycle;
    ...
    if (forgetting) {
      if (temp_el->just_created) {
        wma_forgetting_add_to_p_queue(
            thisAgent, temp_el,
            wma_forgetting_estimate_cycle(thisAgent, temp_el, true));
      } else {
        wma_forgetting_move_in_p_queue(
            thisAgent, temp_el,
            wma_forgetting_estimate_cycle(thisAgent, temp_el, true));
...
double wma_get_wme_activation(agent* thisAgent, wme* w, bool log_result) {
  double return_val = static_cast<double>((log_result) ? (WMA_ACTIVATION_NONE)
                                                       : (WMA_TIME_SUM_NONE));

  if (w->wma_decay_el) {
    return_val = wma_calculate_decay_activation(
        thisAgent, w->wma_decay_el, thisAgent->WM->wma_d_cycle_count,
        log_result);
  }
...
inline void _wma_ref_to_str(wma_cycle_reference& ref, wma_d_cycle current_cycle,
                            std::string& str) {
  std::string temp;
...
void wma_get_wme_history(agent* thisAgent, wme* w, std::string& buffer) {
  if (w->wma_decay_el) {
    ...
    wma_param_container::forgetting_choices forget =
        thisAgent->WM->wma_params->forgetting->get_value();

    if ((forget == wma_param_container::bsearch) ||
        (forget == wma_param_container::approx)) {
      buffer.append("\n\n");
    ...
    if (forgetting != wma_param_container::disabled) {
      ...
      if (forgot_something) {
        ...
        if (wm_removal_diff > 0) {
          thisAgent->WM->wma_stats->forgotten_wmes->set_value(
              thisAgent->WM->wma_stats->forgotten_wmes->get_value() +
              static_cast<int64_t>(wm_removal_diff));
        }
