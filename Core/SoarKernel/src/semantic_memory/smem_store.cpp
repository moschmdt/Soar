    ...
    if (pLTM->slots) {
      ...
      while (!pLTM->slots->empty()) {
        ...
        if (s->second) {
          ...
          for (v = s->second->begin(); v != s->second->end();
               v = s->second->erase(v)) {
...
inline void SMem_Manager::count_child_connection(
    std::map<uint64_t, int64_t>* children, uint64_t child_lti_id) {
  std::map<uint64_t, int64_t>::iterator child_location =
      children->find(child_lti_id);
...
inline void SMem_Manager::count_child_connection(
    std::map<uint64_t, uint64_t>* children, uint64_t child_lti_id) {
  std::map<uint64_t, uint64_t>::iterator child_location =
      children->find(child_lti_id);
  if (child_location != children->end()) {
    (*children)[child_lti_id] = child_location->second + 1;
  } else {
...
void SMem_Manager::disconnect_ltm(
    uint64_t pLTI_ID, std::map<uint64_t, uint64_t>* old_children = NULL) {
  // adjust attr, attr/value counts
  {
    ...
    while (SQL->web_all->execute() == soar_module::row) {
      ...
      if (SQL->web_all->column_int(1) != SMEM_AUGMENTATIONS_NULL) {
        ...
        SQL->wmes_constant_frequency_update->bind_int(
            3, SQL->web_all->column_int(1));
        ...
        SQL->wmes_lti_frequency_update->bind_int(3,
                                                 SQL->web_all->column_int(2));
    ...
    // now attributes
    for (std::set<uint64_t>::iterator a = distinct_attr.begin();
...
void SMem_Manager::LTM_to_DB(uint64_t pLTI_ID, ltm_slot_map* children,
                             bool remove_old_children, bool activate,
                             smem_storage_type store_type) {
  assert(pLTI_ID);
  ...
  if (remove_old_children) {
    ...
    existing_edges =
        static_cast<uint64_t>(SQL->act_lti_child_ct_get->column_int(0));
    ...
    existing_lti_edges =
        static_cast<uint64_t>(SQL->act_lti_child_lti_ct_get->column_int(0));
  ...
  std::unordered_map<uint64_t, double> edge_weights;
  {
    ...
    for (s = children->begin(); s != children->end(); s++) {
      ...
      if (remove_old_children) {
        ...
        if (SQL->web_attr_child->execute(soar_module::op_reinit) !=
            soar_module::row) {
          attr_new.insert(attr_hash);
        }
      }
      ...
      for (v = s->second->begin(); v != s->second->end(); v++) {
        if ((*v)->val_const.val_type == value_const_t) {
          ...
          if (remove_old_children) {
            ...
            if (SQL->web_const_child->execute(soar_module::op_reinit) !=
                soar_module::row) {
              const_new.insert(std::make_pair(attr_hash, value_hash));
            }
          }
          ...
          if (thisAgent->trace_settings[TRACE_SMEM_SYSPARAM]) {
            ...
            thisAgent->outputManager->sprinta_sf_cstr(
                thisAgent, buf, 256, "=>SMEM: (@%u ^%y %y)\n", pLTI_ID,
                s->first, (*v)->val_const.val_value);
          ...
          if (remove_old_children) {
            ...
            if (SQL->web_lti_child->execute(soar_module::op_reinit) !=
                soar_module::row) {
              lti_new.insert(std::make_pair(attr_hash, value_lti));
              if (new_children != NULL) {
                count_child_connection(new_children, value_lti);
          ...
          // We have an edge_weight and a parent and a child. This is where we
          // set the edge_weight to the nonfan value.
          if (edge_weight != 0.0) {
          ...
          if (thisAgent->trace_settings[TRACE_SMEM_SYSPARAM]) {
            ...
            thisAgent->outputManager->sprinta_sf_cstr(
                thisAgent, buf, 256, "=>SMEM: (%u ^%y %u)\n", pLTI_ID, s->first,
                (*v)->val_lti.val_value->lti_id);
            thisAgent->outputManager->printa(thisAgent, buf);
            xml_generate_warning(thisAgent, buf);
          }
        }
  ...
  if (new_children != NULL) {
    if (remove_old_children) {  // This is when the delta needs to be
       ...
       * value. At the same time, erase the old children if it showed up (after
       * calculating the delta) then, loop through the remaining old children
       * and just add those values as negative.
       */
      assert(old_children != NULL);

      std::map<uint64_t, int64_t>::iterator new_child;
      for (new_child = new_children->begin(); new_child != new_children->end();
           ++new_child) {
        if (old_children->find(new_child->first) != old_children->end()) {
          (*new_children)[new_child->first] =
              (*new_children)[new_child->first] -
              (*old_children)[new_child->first];
          old_children->erase(new_child->first);
        }
      }
      std::map<uint64_t, uint64_t>::iterator old_child;
      for (old_child = old_children->begin(); old_child != old_children->end();
           ++old_child) {
        (*new_children)[old_child->first] = old_child->second;
      }
    }
    // new_children contains the set of changes to memory. We use those to
    // invalidate spreading trajectories.
  ...
  // This doesn't create a prohibt. It creates an entry in the prohibit tracking
  // table.
  {
  ...
  // insert new edges, update counters
  {
    // attr/const pairs
    {
      for (std::set<std::pair<smem_hash_id, smem_hash_id> >::iterator p =
               const_new.begin();
           p != const_new.end(); p++) {
        // insert
        {
        ...
        // update counter
        {
          ...
          if (SQL->wmes_constant_frequency_check->execute(
                  soar_module::op_reinit) != soar_module::row) {
            ...
            SQL->wmes_constant_frequency_update->execute(
    ...
    // attr/lti pairs
    {
      for (std::set<std::pair<smem_hash_id, uint64_t> >::iterator p =
               lti_new.begin();
           p != lti_new.end(); p++) {
        // insert
        {
          ...
          if (ever_updated_edge_weight &&
             ...
             * "fix" this, in that things won't "break", but the values will be
             * different than what the user presumably intended.
             */
        ...
        // update counter
        {
          ...
          if (SQL->wmes_lti_frequency_check->execute(soar_module::op_reinit) !=
              soar_module::row) {
    ...
    {
      for (std::set<smem_hash_id>::iterator a = attr_new.begin();
           a != attr_new.end(); a++) {
        // check if counter exists (and add if does not): attribute_s_id
        SQL->attribute_frequency_check->bind_int(1, *a);
        if (SQL->attribute_frequency_check->execute(soar_module::op_reinit) !=
            soar_module::row) {
    ...
    {
      statistics->edges->set_value(statistics->edges->get_value() +
                                   (const_new.size() + lti_new.size()));
    }
  }
...
void SMem_Manager::store_new(Symbol* pSTI, smem_storage_type store_type,
                             bool pOverwriteOldLinkToLTM, tc_number tc) {
  /* We only need to use lookup (3rd arg), if we're storing new and can't
   * overwrite the old lti_value */
  STM_to_LTM(pSTI, store_type, !pOverwriteOldLinkToLTM, pOverwriteOldLinkToLTM,
             tc);
}

void SMem_Manager::update(Symbol* pSTI, smem_storage_type store_type,
                          tc_number tc) {
  STM_to_LTM(pSTI, store_type, false, false, tc);
}

void SMem_Manager::STM_to_LTM(Symbol* pSTI, smem_storage_type store_type,
                              bool use_lookup, bool pOverwriteOldLinkToLTM,
                              tc_number tc) {
  ...
  uint64_t l_LTM_ID =
      get_current_LTI_for_iSTI(pSTI, use_lookup, pOverwriteOldLinkToLTM);
  ...
  // encode this level
  {
    ...
    for (w = children->begin(); w != children->end(); w++) {
      ...
      if ((*w)->value->is_constant()) {
        ...
        /* This seems like funky map usage.  Following line will create entry in
         * map if it doesn't exist.  This works because following code will use
         * created entry anyway.  Should use iterator and find, then create if
         * necessary. */
        ...
        if (!(*c)) {
          (*c) = new ltm_object;
          (*c)->slots = NULL;
          (*c)->lti_id = get_current_LTI_for_iSTI((*w)->value, use_lookup,
                                                  pOverwriteOldLinkToLTM);
  ...
  for (symbol_list::iterator shorty = shorties.begin();
       shorty != shorties.end(); shorty++) {
    STM_to_LTM((*shorty), store_recursive, use_lookup, pOverwriteOldLinkToLTM,
               tc);
  }
}
