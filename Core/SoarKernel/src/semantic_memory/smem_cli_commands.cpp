...
bool SMem_Manager::CLI_add(const char* ltms_str, std::string** err_msg) {
  ...
  while ((lexer.current_lexeme.type == L_PAREN_LEXEME) && (good_ltm)) {
    ...
    if (good_ltm) {
      ...
      for (c_new = newbies.begin(); c_new != newbies.end(); c_new++) {
        if ((*c_new)->slots != NIL) {
          /* Third parameter determines whether smem will update LTM based on
           * LTI_ID in an STI. For that to be useful here, parser must be
           * changed to also accepts STIs for smem -add. May want to change to
           * false until that is possible */
          LTM_to_DB((*c_new)->lti_id, (*c_new)->slots, false, false);
        }
      }

      // deallocate *contents* of all newbies (need to keep around name->id
      // association for future ltms)
      for (c_new = newbies.begin(); c_new != newbies.end(); c_new++) {
...
bool SMem_Manager::CLI_query(const char* ltms_str, std::string** err_msg,
                             std::string** result_message,
                             uint64_t number_to_retrieve) {
  uint64_t clause_count =
      0;  // This is counting up the number of parsed clauses
  ...
  bool good_cue = true;  // This is a success or failure flag that will be
                         // checked periodically
  // and indicates whether or not we can call smem_process_query.

  std::map<std::string, Symbol*>
      cue_ids;  // I want to keep track of previous references when adding a new
                // element to the cue.
  ...
  Symbol* root_cue_id =
      NIL;  // This is the id that gets passed to smem_process_query.
  // It's main purpose is to contain augmentations
  Symbol* negative_cues =
      NULL;  // This is supposed to contain the negative augmentations.
  ...
  bool first_attribute =
      true;  // Want to make sure there is a positive attribute to begin with.
  ...
  while ((lexer.current_lexeme.type == L_PAREN_LEXEME) && good_cue) {
    ...
    if (trigger_first) {
      ...
      if (good_cue) {
        root_cue_id = thisAgent->symbolManager->make_new_identifier(
            (char)lexer.current_lexeme.string()[1], 1);
        cue_ids[lexer.current_lexeme.string()] = root_cue_id;
        negative_cues = thisAgent->symbolManager->make_new_identifier(
            (char)lexer.current_lexeme.string()[1], 1);
      } else {
        (*err_msg)->append(
            "Error: The cue must be a variable.\n");  // Spit out that the cue
                                                      // must be a variable.
        break;
      }
      ...
      // If this isn't the first time around, then this better be the same as
      // the root_cue_id variable.
      good_cue = cue_ids[lexer.current_lexeme.string()] == root_cue_id;
      if (!good_cue) {
        (*err_msg)->append(
            "Error: Additional clauses must share same variable.\n");  // Spit
                                                                       // out
                                                                       // that
                                                                       // additional
                                                                       // clauses
    ...
    if (good_cue) {
      ...
      while (good_cue && (lexer.current_lexeme.type == UP_ARROW_LEXEME ||
                          lexer.current_lexeme.type == MINUS_LEXEME)) {
        ...
        lexer.get_lexeme();  // Consume the up arrow and move on to the
                             // attribute.
        ...
        // for the type of the lexeme so passing a "(" caused a segfault when
        // making the slot.
        ...
        if (minus) {
          ...
          temp_slot = make_slot(thisAgent, root_cue_id,
                                attribute);  // Make a slot for this attribute,
                                             // or return slot it already has.
        }
        ...
        do  // Add value by type
        {
          ...
          if (lexer.current_lexeme.type == STR_CONSTANT_LEXEME) {
            value = thisAgent->symbolManager->make_str_constant(
                static_cast<const char*>(lexer.current_lexeme.string()));
            lexer.get_lexeme();
          } else if (lexer.current_lexeme.type == INT_CONSTANT_LEXEME) {
            value = thisAgent->symbolManager->make_int_constant(
                lexer.current_lexeme.int_val);
            lexer.get_lexeme();
          } else if (lexer.current_lexeme.type == FLOAT_CONSTANT_LEXEME) {
            value = thisAgent->symbolManager->make_float_constant(
                lexer.current_lexeme.float_val);
            lexer.get_lexeme();
            ...
            if (lexer.current_lexeme.type != INT_CONSTANT_LEXEME) {
              good_cue = false;
              (*err_msg)->append(
                  "Error: @ must be followed by an integer to be a long-term "
                  "identifier.\n");
              break;
            }
            ...
            if (value_id == NIL) {
              ...
              /* Not sure what we'd want to use here.  Will create an identifier
               * for now with lti_id */
            ...
            lexer.get_lexeme();
          } else if (lexer.current_lexeme.type == VARIABLE_LEXEME ||
            ...
            if (value_iterator == cue_ids.end()) {
              value = thisAgent->symbolManager->make_new_identifier(
                  (char)lexer.current_lexeme.string()[0], 1);
              cue_ids[lexer.current_lexeme.string()] =
                  value;  // Keep track of created symbols for deletion later.
            }
            lexer.get_lexeme();
          } else {
          ...
          if (value != NIL && good_cue) {
            // Value might be nil, but R_paren or next attribute could make it a
            // good cue.
            hasAddedValue = true;
            if (minus) {
              temp_wme =
                  make_wme(thisAgent, negative_cues, attribute, value, false);
            } else {
              temp_wme =
                  make_wme(thisAgent, root_cue_id, attribute, value, false);
            }
            insert_at_head_of_dll(
                temp_slot->wmes, temp_wme, next,
                prev);  // Put the wme in the slow for the attribute.
          }
        } while (value != NIL);  // Loop until there are no more value lexemes
                                 // to add to that attribute.
  ...
  if (!good_cue) {
    ...
    process_query(NIL, root_cue_id_list, minus_ever ? negative_cues : NIL, NIL,
                  prohibit, cue_wmes, meta_wmes, retrieval_wmes, qry_search,
                  number_to_retrieve, &(match_ids), 1, fake_install);

    if (!match_ids.empty()) {
      for (std::list<uint64_t>::const_iterator id = match_ids.begin(),
                                               end = match_ids.end();
           id != end; ++id) {
  ...
  if (root_cue_id != NIL) {
    ...
    for (s = root_cue_id->id->slots; s != NIL; s = s->next) {
      ...
      wme* delete_wme;
      for (delete_wme = s->wmes; delete_wme != NIL;
           delete_wme = delete_wme->next) {
    ...
    thisAgent->symbolManager->symbol_remove_ref(
        &root_cue_id);  // gets rid of cue id.
    thisAgent->symbolManager->symbol_remove_ref(
        &negative_cues);  // gets rid of negative cues id.
  }

  return good_cue;
}

bool SMem_Manager::CLI_remove(const char* ltms_str, std::string** err_msg,
                              std::string** result_message, bool force) {
  ...
  if (good_command) {
    ...
    if (lexer.current_lexeme.type == UP_ARROW_LEXEME) {
      // Now that we know we have a good lti, we can do a NCBR so that we know
      // what attributes and values we can delete.
      //"--force" will ignore attempts to delete that which isn't there, while
      // the default will be to stop and report back.
      install_memory(NIL, lti_id, NULL, false, meta_wmes, retrieval_wmes,
                     fake_install);
      // First, we'll create the slot_map according to retrieval_wmes, then
      // we'll remove what we encounter during parsing.
      symbol_triple_list::iterator triple_ptr_iter;
      ltm_slot* temp_slot;
      for (triple_ptr_iter = retrieval_wmes.begin();
           triple_ptr_iter != retrieval_wmes.end(); triple_ptr_iter++) {
        if (children.count((*triple_ptr_iter)->attr))  // If the attribute is
                                                       // already in the map.
        {
          temp_slot = (children.find((*triple_ptr_iter)->attr)->second);
          ltm_value* temp_val = new ltm_value;
          if ((*triple_ptr_iter)->value->symbol_type ==
        ...
        } else  // If the attribute is not in the map and we need to make a
                // slot.
        {
      ...
      while (lexer.current_lexeme.type == UP_ARROW_LEXEME &&
        ...
        if (lexer.current_lexeme.type == STR_CONSTANT_LEXEME) {
          attribute = thisAgent->symbolManager->find_str_constant(
              static_cast<const char*>(lexer.current_lexeme.string()));
        } else if (lexer.current_lexeme.type == INT_CONSTANT_LEXEME) {
          attribute = thisAgent->symbolManager->find_int_constant(
              lexer.current_lexeme.int_val);
        } else if (lexer.current_lexeme.type == FLOAT_CONSTANT_LEXEME) {
          attribute = thisAgent->symbolManager->find_float_constant(
              lexer.current_lexeme.float_val);
        }
        ...
        if (good_command && (lexer.current_lexeme.type != UP_ARROW_LEXEME &&
                             lexer.current_lexeme.type !=
                                 R_PAREN_LEXEME))  // If there are values.
        {
          Symbol* value;
          do  // Add value by type
          {
            value = NIL;
            if (lexer.current_lexeme.type == STR_CONSTANT_LEXEME) {
              value = thisAgent->symbolManager->find_str_constant(
                  static_cast<const char*>(lexer.current_lexeme.string()));
              lexer.get_lexeme();
            } else if (lexer.current_lexeme.type == INT_CONSTANT_LEXEME) {
              value = thisAgent->symbolManager->find_int_constant(
                  lexer.current_lexeme.int_val);
              lexer.get_lexeme();
            } else if (lexer.current_lexeme.type == FLOAT_CONSTANT_LEXEME) {
              value = thisAgent->symbolManager->find_float_constant(
                  lexer.current_lexeme.float_val);
              lexer.get_lexeme();
            } else if (lexer.current_lexeme.type == AT_LEXEME) {
              lexer.get_lexeme();
              if (lexer.current_lexeme.type == INT_CONSTANT_LEXEME) {
                value = get_current_iSTI_for_LTI(lexer.current_lexeme.int_val,
                                                 NO_WME_LEVEL);
                lexer.get_lexeme();
              } else {
                (*err_msg)->append(
                    "Error: '@' should be followed by an integer lti id.\n");
                good_command = false;
                break;
              }
            } else {
              good_command = (lexer.current_lexeme.type == R_PAREN_LEXEME ||
                              lexer.current_lexeme.type == UP_ARROW_LEXEME);
              if (!good_command) {
                (*err_msg)->append(
                    "Error: Expected ')' or '^'.\n... Neither was found.\n");
            ...
            if (value != NIL &&
                good_command)  // Value might be nil, but that can be just fine.
            {
              // Given a value for this attribute, we have a symbol triple to
              // remove.
              ltm_slot::iterator values;
              for (values = (children.find(attribute))->second->begin();
                   values != (children.find(attribute))->second->end();
                   values++) {
                if (value->is_lti() &&
                    (*values)->val_lti.val_type == value_lti_t) {
                  if ((*values)->val_lti.val_value->lti_id ==
                      value->id->LTI_ID) {
                ...
                } else if (!value->is_sti() &&
                           (*values)->val_const.val_type == value_const_t) {
              ...
              if (values == (children.find(attribute))->second->end()) {
                (*err_msg)->append(
                    "Error: Value does not exist on attribute.\n");
              }
            } else {
              if ((good_command && !force) &&
                  (lexer.current_lexeme.type != R_PAREN_LEXEME &&
                   lexer.current_lexeme.type != UP_ARROW_LEXEME)) {
                (*err_msg)->append(
                    "Error: Attribute contained a value that could not be "
                    "found.\n");
                break;
              }
            }
                   ...
                   children.find(attribute) !=
                       children.end())  // If we didn't have any values, then we
                                        // just get rid of everything on the
                                        // attribute.
        {
        ...
        if (force) {
          while ((lexer.current_lexeme.type != EOF_LEXEME &&
                  lexer.current_lexeme.type != UP_ARROW_LEXEME) &&
                 lexer.current_lexeme.type !=
                     R_PAREN_LEXEME)  // Loop until the lexeme is EOF, another
                                      // ^, or ")".
          {
            lexer.get_lexeme();
          }
...
Symbol* SMem_Manager::parse_constant_attr(soar::Lexeme* lexeme) {
  ...
  if ((*lexeme).type == STR_CONSTANT_LEXEME) {
    return_val = thisAgent->symbolManager->make_str_constant(
        static_cast<const char*>((*lexeme).string()));
...
bool SMem_Manager::parse_add_clause(soar::Lexer* lexer,
                                    str_to_ltm_map* str_to_LTMs,
                                    ltm_set* newbies) {
  bool return_val = false;
  ...
  if ((lexer->current_lexeme.type == AT_LEXEME) ||
    ...
    if (good_at) {
      ...
      if (good_at) {
        ...
        while (lexer->current_lexeme.type == UP_ARROW_LEXEME) {
          ...
          if (l_ltm_attr != NIL) {
            ...
            while (lexer->current_lexeme.type == PERIOD_LEXEME) {
              ...
              l_ltm_slot =
                  make_ltm_slot(l_ltm_intermediate_parent->slots, l_ltm_attr);
              l_ltm_slot->push_back(l_ltm_value);

              // create a key guaranteed to be unique
              temp_key.assign("<");
              temp_key.append(
                  1, ((l_ltm_attr->symbol_type == STR_CONSTANT_SYMBOL_TYPE)
                          ? (static_cast<char>(
                                static_cast<int>(l_ltm_attr->sc->name[0])))
                          : ('X')));
              temp_key.append("#");
              ...
              // the new ltm is our parent for this set of values (or further
              // dots)
            ...
            if (l_ltm_attr != NIL) {
              ...
              do {
                ...
                if ((lexer->current_lexeme.type == STR_CONSTANT_LEXEME) ||
                  ...
                  l_ltm_value->val_const.val_value =
                      thisAgent->symbolManager->make_str_constant(
                  ...
                  l_ltm_value->val_const.val_type = value_const_t;
                  l_ltm_value->val_const.val_value =
                      thisAgent->symbolManager->make_int_constant(
                          lexer->current_lexeme.int_val);
                } else if (lexer->current_lexeme.type ==
                  ...
                  if (good_at && !mistakenLTI) {
                    ...
                    if (lexer->current_lexeme.type == VARIABLE_LEXEME) {
                      ...
                      temp_key2.clear();
                      get_lti_name(
                          static_cast<uint64_t>(lexer->current_lexeme.int_val),
                          temp_key2);
                    }
                    ...
                    if (p != str_to_LTMs->end()) {
                      ...
                      if (lexer->current_lexeme.type == INT_CONSTANT_LEXEME) {
                        l_ltm_temp->lti_id = static_cast<uint64_t>(
                            lexer->current_lexeme.int_val);
                        /* May want to verify that this is a legitimate id in
                         * the smem database */
                        // l_ltm_temp->lti_id =
                        // lti_exists(static_cast<uint64_t>(lexer->current_lexeme.int_val));
                    ...
                    thisAgent->outputManager->printa_sf(
                        thisAgent,
                        "Value of smem -add clause for @%u is invalid: %s\n",
                ...
                if (l_ltm_value != NIL) {
                  ...
                  if (lexer->current_lexeme.type == L_PAREN_LEXEME) {
                    ...
                    if (lexer->current_lexeme.type == FLOAT_CONSTANT_LEXEME) {
                      l_ltm_value->val_lti.edge_weight =
                          lexer->current_lexeme.float_val;
                    } else if (lexer->current_lexeme.type ==
                               INT_CONSTANT_LEXEME) {
                      l_ltm_value->val_lti.edge_weight =
                          static_cast<double>(lexer->current_lexeme.int_val);
                    } else {
                      thisAgent->outputManager->printa_sf(
                          thisAgent,
                          "Edge weight input for smem -add clause for @%u is "
                          "not float or integer: %s\n",
                          l_ltm->lti_id, lexer->current_lexeme.string());
                      // error: The syntax suggested that an edge-weight value
                      // was going to be provided but this was not encountered
                    }
                    lexer->get_lexeme();
                    if (lexer->current_lexeme.type == R_PAREN_LEXEME) {
                      lexer->get_lexeme();
                  ...
                  l_ltm_slot = make_ltm_slot(l_ltm_intermediate_parent->slots,
                                             l_ltm_attr);
                  if (first_value && !l_ltm_slot->empty()) {
                    // in the case of a repeated attribute, remove ref here to
                    // avoid leak
              ...
              thisAgent->outputManager->printa_sf(
                  thisAgent,
                  "Attribute of smem -add clause for @%u is invalid: %s\n",
                  l_ltm->lti_id, lexer->current_lexeme.string());
            }
          }
        }
        ...
        thisAgent->outputManager->printa_sf(
            thisAgent, "Identifier of smem -add clause is invalid: %s\n",
            lexer->current_lexeme.string());
        delete l_ltm;
      }
    } else {
  ...
  if (return_val) {
    ...
    if (!(*p)) {
      ...
      if (!(*p)->slots) {
        ...
        // for all slots
        for (ss_p = l_ltm->slots->begin(); ss_p != l_ltm->slots->end();
             ss_p++) {
...
uint64_t SMem_Manager::spread_size() {
  ...
  uint64_t number_spread_elements =
      SQL->calc_spread_size_debug_cmd->column_int(0);
