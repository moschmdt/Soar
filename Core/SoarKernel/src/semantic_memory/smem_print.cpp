...
bool SMem_Manager::export_smem(uint64_t lti_id, std::string& result_text,
                               std::string** err_msg) {
  ltm_set store_set;
  if (!connected()) {
  ...
  for (auto it = store_set.begin(); it != store_set.end(); ++it) {
    ...
    /* Skip if LTI has no augmentations.  Will be added by other smem --add
     * clause */
    if (current_ltm->slots->size() > 0) {
      thisAgent->outputManager->sprinta_sf(thisAgent, result_text, "(@%u",
                                           current_ltm->lti_id);
      ...
      for (auto map_it = current_ltm->slots->begin();
        ...
        thisAgent->outputManager->sprinta_sf(thisAgent, result_text, " ^%y",
                                             attr);

        for (auto slot_it = current_slot->begin();
             slot_it != current_slot->end(); ++slot_it) {
          if ((*slot_it)->val_lti.val_type == value_lti_t) {
            thisAgent->outputManager->sprinta_sf(
                thisAgent, result_text, " @%u",
                (*slot_it)->val_lti.val_value->lti_id);
          } else {
            thisAgent->outputManager->sprinta_sf(
                thisAgent, result_text, " %y", (*slot_it)->val_const.val_value);
          }
        }
      }
      thisAgent->outputManager->sprinta_sf(thisAgent, result_text, ")\n");
...
void SMem_Manager::create_store_set(ltm_set* store_set, uint64_t lti_id,
  ...
  while (!bfs.empty()) {
    ...
    while (expand_q->execute() == soar_module::row) {
      Symbol* attr = rhash_(static_cast<byte>(expand_q->column_int(0)),
      ...
      if (expand_q->column_int(4) != SMEM_AUGMENTATIONS_NULL) {
        ...
        new_value->val_const.val_value =
            rhash_(expand_q->column_int(2), expand_q->column_int(3));
...
void SMem_Manager::clear_store_set(ltm_set* store_set) {
  ...
  for (set_it = store_set->begin(); set_it != store_set->end(); ++set_it) {
    ...
    for (map_it = current_ltm_slot_map->begin();
         map_it != current_ltm_slot_map->end(); ++map_it) {
      Symbol* attr = map_it->first;
      ltm_slot* current_slot = map_it->second;
      ltm_slot::iterator slot_it;
      for (slot_it = current_slot->begin(); slot_it != current_slot->end();
           ++slot_it) {
        if ((*slot_it)->val_lti.val_type == value_lti_t) {
...
id_set SMem_Manager::print_LTM(uint64_t pLTI_ID, double lti_act,
  ...
  while (expand_q->execute() == soar_module::row) {
    ...
    if (expand_q->column_int(4) != SMEM_AUGMENTATIONS_NULL) {
      ...
      /* The following line prints the children indented.  It seems redundant
       * when printing the smem store, but perhaps it's useful for printing
       * something rooted in an lti? */
  ...
  {
    for (lti_slot = augmentations.begin(); lti_slot != augmentations.end();
         lti_slot++) {
      return_val->append(" ^");
      return_val->append(lti_slot->first);

      for (slot_val = lti_slot->second.begin();
           slot_val != lti_slot->second.end(); slot_val++) {
        return_val->append(" ");
  ...
  if (history != NIL) {
    ...
    return_val->append("[-");
    for (std::list<uint64_t>::iterator history_item = (*history).begin();
         history_item != (*history).end(); ++history_item) {
      if (history_item != (*history).begin()) {
        return_val->append(", -");
      }
      temp_string << ((int64_t)thisAgent->SMem->smem_max_cycle -
                      (int64_t)*history_item);
...
void SMem_Manager::print_smem_object(uint64_t pLTI_ID, uint64_t depth,
                                     std::string* return_val, bool history) {
  id_set visited;
  ...
  soar_module::sqlite_statement* lti_access_q =
      thisAgent->SMem->SQL->lti_access_get;
  unsigned int i;
    ...
    // get lti info
    {
      ...
      if (history) {
        ...
        for (int i = 0; i < n && i < 10;
             ++i)  // 10 because of the length of the history record kept for
                   // smem.
        {
