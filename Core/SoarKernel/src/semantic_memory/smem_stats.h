...
class smem_db_lib_version_stat
 ...
 public:
  smem_db_lib_version_stat(agent* new_agent, const char* new_name,
...
class smem_mem_usage_stat : public soar_module::integer_stat {
 ...
 public:
  smem_mem_usage_stat(agent* new_agent, const char* new_name, int64_t new_value,
...
class smem_mem_high_stat : public soar_module::integer_stat {
 ...
 public:
  smem_mem_high_stat(agent* new_agent, const char* new_name, int64_t new_value,
                     soar_module::predicate<int64_t>* new_prot_pred);
  int64_t get_value();
};
