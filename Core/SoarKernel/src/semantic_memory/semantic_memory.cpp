        ...
        wmes = get_direct_augs_of_id(parent_sym, tc);
        {
          for (w_p = wmes->begin(); w_p != wmes->end(); w_p++) {
            if (((store_only) &&
                 ((parent_level != 0) ||
                  ((*w_p)->attr ==
                   thisAgent->symbolManager->soarSymbols.smem_sym_store))) ||
                ((!store_only) &&
                 ((parent_level != 0) ||
                  ((*w_p)->attr !=
                   thisAgent->symbolManager->soarSymbols.smem_sym_store)))) {
              ...
              if ((*w_p)->timetag >
                  state->id->smem_info->last_cmd_time[time_slot]) {
                new_cue = true;
                state->id->smem_info->last_cmd_time[time_slot] =
                    (*w_p)->timetag;
              }
              ...
              if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
                  (parent_level == 0) &&
                  (((*w_p)->attr ==
                    thisAgent->symbolManager->soarSymbols.smem_sym_query) ||
                   ((*w_p)->attr ==
                    thisAgent->symbolManager->soarSymbols.smem_sym_store))) {
                syms.push((*w_p)->value);
    ...
    if (new_cue && wme_count) {
      ...
      for (w_p = cmds->begin(); w_p != cmds->end(); w_p++) {
        ...
        if (path != cmd_bad) {
          ...
          if ((*w_p)->attr ==
              thisAgent->symbolManager->soarSymbols.smem_sym_retrieve) {
            if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
                (path == blank_slate)) {
              retrieve = (*w_p)->value;
              path = cmd_retrieve;
              ...
              path = cmd_bad;
                     ...
                     thisAgent->symbolManager->soarSymbols.smem_sym_query) {
            if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
                ((path == blank_slate || path == cmd_prohibit) ||
                 (path == cmd_query)) &&
                (query == NIL))
            ...
            {
                //Since we have already received a query command and now we are
            receiving another, this must be an or-query.
                //This means that we will issue several queries and return the
            winner among the multiple queries.
                //The logic amounts to performing an or/union operation over the
            separate queries. orquery.push_back((*w_p)->value);
            }*/
            else {
              path = cmd_bad;
            }
          } else if ((*w_p)->attr ==
                     thisAgent->symbolManager->soarSymbols.smem_sym_negquery) {
            if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
                ((path == blank_slate || path == cmd_prohibit) ||
                 (path == cmd_query)) &&
                (negquery == NIL))

            {
              ...
              path = cmd_bad;
                     ...
                     thisAgent->symbolManager->soarSymbols.smem_sym_prohibit) {
            if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
                ((path == blank_slate || path == cmd_prohibit) ||
                 (path == cmd_query)) &&
          ...
          } else if ((*w_p)->attr == thisAgent->symbolManager->soarSymbols
                                         .smem_sym_math_query) {
            if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
                ((path == blank_slate || path == cmd_prohibit) ||
                 (path == cmd_query)) &&
            ...
            if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
              ...
              path = cmd_bad;
                     ...
                     thisAgent->symbolManager->soarSymbols.smem_sym_store_new) {
            if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
              ...
              path = cmd_bad;
                     ...
                     thisAgent->symbolManager->soarSymbols.smem_sym_overwrite) {
            ...
            if (((*w_p)->value->symbol_type == STR_CONSTANT_SYMBOL_TYPE) &&
              ...
              path = cmd_bad;
                     ...
                     thisAgent->symbolManager->soarSymbols.smem_sym_depth) {
            if ((*w_p)->value->symbol_type == INT_CONSTANT_SYMBOL_TYPE) {
              depth =
                  ((*w_p)->value->ic->value > 0) ? (*w_p)->value->ic->value : 1;
            } else {
              path = cmd_bad;
            }
          } else {
      ...
      if (path != cmd_bad) {
        ...
        if (path == cmd_retrieve) {
          if (retrieve->id->LTI_ID == NIL) {
            ...
            install_memory(state, retrieve->id->LTI_ID, NULL, true, meta_wmes,
                           retrieval_wmes, wm_install, depth);

            // add one to the expansions stat
            thisAgent->SMem->statistics->retrievals->set_value(
                thisAgent->SMem->statistics->retrievals->get_value() + 1);
          }
        }
        // query
          ...
          process_query(state, orquery, negquery, math, &(prohibit_lti),
                        cue_wmes, meta_wmes, retrieval_wmes, qry_full, 1, NIL,
                        depth, wm_install);

          // add one to the cbr stat
          thisAgent->SMem->statistics->queries->set_value(
              thisAgent->SMem->statistics->queries->get_value() + 1);
          ...
          for (sym_p = store.begin(); sym_p != store.end(); sym_p++) {
            ...
            thisAgent->SMem->statistics->stores->set_value(
                thisAgent->SMem->statistics->stores->get_value() + 1);
          }
...
prohibit_add = new soar_module::sqlite_statement(new_db, "INSERT OR IGNORE INTO
smem_prohibited (lti_id,prohibited,dirty) VALUES (?,0,0)"); add(prohibit_add);
 ...
 */
        ...
        add_triple_to_recall_buffer(
            meta_wmes, state->id->smem_info->result_wme->value,
            thisAgent->symbolManager->soarSymbols.smem_sym_bad_cmd,
            state->id->smem_info->cmd_wme->value);
      }
...
bool SMem_Manager::clear() {
  if (thisAgent->SMem->connected()) {
    boolean oldAppendSetting =
        thisAgent->SMem->settings->append_db->get_value();
...
void SMem_Manager::reinit() {
  if (thisAgent->SMem->connected() &&
      (thisAgent->SMem->settings->database->get_value() ==
       smem_param_container::file)) {
    close();
    init_db();
  }
}
...
SMem_Manager::SMem_Manager(agent* myAgent) {
  ...
  smem_recipients_of_source =
      new std::unordered_map<uint64_t, std::set<uint64_t>*>();
