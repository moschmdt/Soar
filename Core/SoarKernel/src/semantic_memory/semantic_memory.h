...
class SMem_Manager {
 ...
 public:
  ...
  void reset_id_counters() {
    lti_id_counter =
        (get_max_lti_id() > settings->initial_variable_id->get_value()
             ? get_max_lti_id()
             : settings->initial_variable_id->get_value() - 1);
  };
  bool backup_db(const char* file_name, std::string* err);
  bool export_smem(uint64_t lti_id, std::string& result_text,
                   std::string** err_msg);
  void close();
  ...
  bool CLI_query(const char* ltms, std::string** err_msg,
                 std::string** result_message, uint64_t number_to_retrieve);
  bool CLI_remove(const char* ltms, std::string** err_msg,
                  std::string** result_message, bool force = false);

  void calc_spread_trajectories();
  void invalidate_trajectories(uint64_t lti_parent_id,
                               std::map<uint64_t, int64_t>* delta_children);
  void calc_spread(std::set<uint64_t>* current_candidates, bool do_manual_crawl,
                   smem_weighted_cue_list::iterator* cand_set = NULL);
 ...
 private:
  ...
  void get_lti_name(uint64_t pLTI_ID, std::string& lti_name) {
    lti_name.append("@");
    lti_name.append(std::to_string(pLTI_ID));
  }
  uint64_t get_max_lti_id();
  double lti_activate(uint64_t pLTI_ID, bool add_access,
                      uint64_t num_edges = SMEM_ACT_MAX, double touches = 1,
                      bool increment_timer = true);
  double lti_calc_base(uint64_t pLTI_ID, int64_t time_now, uint64_t n = 0,
                       uint64_t activations_first = 0);
  ...
  void install_buffered_triple_list(Symbol* state, wme_set& cue_wmes,
                                    symbol_triple_list& my_list, bool meta,
                                    bool stripLTILinks = false);
  void install_memory(
      Symbol* state, uint64_t pLTI_ID, Symbol* lti, bool activate_lti,
      symbol_triple_list& meta_wmes, symbol_triple_list& retrieval_wmes,
      smem_install_type install_type = wm_install, uint64_t depth = 1,
      std::set<uint64_t>* visited = NULL,
      std::queue<std::pair<Symbol*, uint64_t>>* to_install = NULL);
  void install_recall_buffer(Symbol* state, wme_set& cue_wmes,
  ...
  inline void count_child_connection(std::map<uint64_t, uint64_t>* children,
                                     uint64_t child_lti_id);
  void disconnect_ltm(uint64_t pLTI_ID,
                      std::map<uint64_t, uint64_t>* old_children);
  ltm_slot* make_ltm_slot(ltm_slot_map* slots, Symbol* attr);
  bool parse_add_clause(soar::Lexer* lexer, str_to_ltm_map* ltms,
                        ltm_set* newbies);
  Symbol* parse_constant_attr(soar::Lexeme* lexeme);
  void store_new(Symbol* pSTI, smem_storage_type store_type,
                 bool pOverwriteOldLinkToLTM, tc_number tc = NIL);
  void update(Symbol* pSTI, smem_storage_type store_type, tc_number tc = NIL);
  void STM_to_LTM(Symbol* pSTI, smem_storage_type store_type,
                  bool pCreateNewLTM, bool pOverwriteOldLinkToLTM,
                  tc_number tc = NIL);
  void LTM_to_DB(uint64_t pLTI_ID, ltm_slot_map* children,
  ...
  bool process_cue_wme(wme* w, bool pos_cue,
                       ...
                       MathQuery* mathQuery);
  uint64_t process_query(Symbol* state, std::list<Symbol*> query,
                         Symbol* negquery, Symbol* mathQuery, id_set* prohibit,
                         wme_set& cue_wmes, symbol_triple_list& meta_wmes,
                         symbol_triple_list& retrieval_wmes,
                         smem_query_levels query_level = qry_full,
                         uint64_t number_to_retrieve = 1,
                         std::list<uint64_t>* match_ids = NIL,
                         uint64_t depth = 1,
                         smem_install_type install_type = wm_install);
  std::pair<bool, bool>* processMathQuery(
  ...
  soar_module::sqlite_statement* setup_web_crawl_without_spread(
      smem_weighted_cue_element* el);
  soar_module::sqlite_statement* setup_cheap_web_crawl(
      smem_weighted_cue_element* el);
  ...
  void child_spread(uint64_t lti_id,
                    std::map<uint64_t, std::list<std::pair<uint64_t, double>>*>&
                        lti_trajectories,
                    int depth);
  void trajectory_construction(
      uint64_t lti_id,
      std::map<uint64_t, std::list<std::pair<uint64_t, double>>*>&
          lti_trajectories,
      int depth, bool initial);
  // void calc_likelihoods_for_trajectories(uint64_t lti_id);
  inline soar_module::sqlite_statement* setup_manual_web_crawl(
      smem_weighted_cue_element* el, uint64_t lti_id);
  // void calc_spread(std::set<uint64_t>* current_candidates, bool
  // do_manual_crawl, smem_weighted_cue_list::iterator* cand_set=NULL);
};
