...
void identity_record::add_identity_mapping(uint64_t pI_ID,
                                           IDSet_Mapping_Type pType,
                                           Identity* pFromID, Identity* pToID) {
...
void identity_record::clean_up() {
  if (idset_to_var_map) {
    Symbol* lSym;
    for (auto it = idset_to_var_map->begin(); it != idset_to_var_map->end();
         ++it) {
      lSym = it->second;
...
void identity_record::clear_mappings() {
  for (auto it = instantiation_mappings->begin();
       it != instantiation_mappings->end(); ++it) {
    identity_mapping* lMapping;
...
void generate_identity_sets_from_test(agent* thisAgent, test pTest,
                                      uint64_t pInstID, id_set* pIdentity,
                                      id_to_sym_map* pID_Set_Map) {
...
void generate_identity_sets_from_conditions(agent* thisAgent, condition* lhs,
                                            uint64_t pInstID, id_set* pIdentity,
                                            id_to_sym_map* pID_Set_Map) {
  for (condition* lCond = lhs; lCond != NULL; lCond = lCond->next) {
    if (lCond->type == CONJUNCTIVE_NEGATION_CONDITION) {
      generate_identity_sets_from_conditions(thisAgent, lCond->data.ncc.top,
                                             pInstID, pIdentity, pID_Set_Map);
    } else {
      generate_identity_sets_from_test(thisAgent, lCond->data.tests.id_test,
                                       pInstID, pIdentity, pID_Set_Map);
      generate_identity_sets_from_test(thisAgent, lCond->data.tests.attr_test,
                                       pInstID, pIdentity, pID_Set_Map);
      generate_identity_sets_from_test(thisAgent, lCond->data.tests.value_test,
                                       pInstID, pIdentity, pID_Set_Map);
    }
  }
}

void identity_record::analyze_chunk_identities(uint64_t pInstID,
                                               condition* lhs) {
  /* Generate identity sets and add mappings for all conditions in chunk */
  generate_identity_sets_from_conditions(thisAgent, lhs, pInstID,
                                         identities_in_chunk, idset_to_var_map);

  for (auto it = identities_in_chunk->begin(); it != identities_in_chunk->end();
       ++it) {
    thisAgent->explanationMemory->increment_stat_identities_participated();
  }
}

void identity_record::print_identity_mappings_for_instantiation(
    instantiation_record* pInstRecord) {
...
void identity_record::print_identities_in_chunk() {
  thisAgent->outputManager->printa_sf(
      thisAgent, "\nLearned rule contained %u identities: ",
      static_cast<uint64_t>(identities_in_chunk->size()));
  for (auto it = identities_in_chunk->begin(); it != identities_in_chunk->end();
       ++it) {
...
void identity_record::print_instantiation_mappings(uint64_t pI_ID) {
  ...
  if (lIterInst == instantiation_mappings->end()) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "No identity set unifications for instantiation %u.\n",
        pI_ID);
  } else {
...
void identity_record::print_mappings() {
    ...
    if (it->second->size() != 0) {
      if (!unification_found) {
        thisAgent->outputManager->printa_sf(thisAgent,
                                            "Identity set unifications:\n\n");
        unification_found = true;
      }
      lInstString.clear();
      outputManager->sprinta_sf(thisAgent, lInstString, " in instantiation %u",
                                it->first);
  ...
  if (!unification_found) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "No identity unifications occurred during backtracing.\n");
  } else {
    for (auto it = instantiation_mappings->begin();
         it != instantiation_mappings->end(); ++it) {
      if (it->second->size() != 0) {
        lInstString.clear();
        outputManager->sprinta_sf(thisAgent, lInstString,
                                  " in instantiation %u", it->first);
...
void identity_record::print_mapping_list(identity_mapping_list* pMapList,
                                         bool pLiteralizationMode,
                                         std::string* pInstString) {
  Output_Manager* outputManager = thisAgent->outputManager;
  identity_mapping* lMapping;
  bool printOnlyChunkIdentities =
  ...
  for (auto it = pMapList->begin(); it != pMapList->end(); ++it) {
    ...
    if (printOnlyChunkIdentities && (lFindIter != idset_to_var_map->end()) &&
        !lFindIter->second)
      continue;

    if (lMapping->to_identity)
      outputManager->printa_sf(thisAgent, "%-%u merged with %u",
                               lMapping->from_identity, lMapping->to_identity);
    else
      outputManager->printa_sf(thisAgent, "%-%u literalized",
                               lMapping->from_identity);
    ...
    switch (lMapping->mappingType) {
      case IDS_join:
        outputManager->printa_sf(
            thisAgent, "%-| Two identities propagated into the same variable");
        break;
      case IDS_unified_with_singleton:
        outputManager->printa_sf(thisAgent,
                                 "%-| Tested a super-state singleton WME "
                                 "previously tested by another rule");
        break;
      case IDS_unified_child_result:
        outputManager->printa_sf(
            thisAgent,
            "%-| Identities joined to connected child result to parent result");
        break;
      case IDS_literalized_LHS_literal:
        outputManager->printa_sf(
            thisAgent,
            "%-| Literal value in another rule compared against RHS variable");
        break;
      case IDS_literalized_RHS_literal:
        outputManager->printa_sf(
            thisAgent,
            "%-| Variable in another rule compared against literal RHS value");
        break;
      case IDS_literalized_RHS_function_arg:
        outputManager->printa_sf(
            thisAgent, "%-| Variable was used as argument in a RHS function");
        break;
      case IDS_literalized_RHS_function_compare:
        outputManager->printa_sf(
            thisAgent,
            "%-| Variable in another rule tested result of RHS function");
        break;
      default:
        assert(false);
...
void identity_record::record_identity_sets(identity_set* identity_sets) {
  ...
  for (auto it = identity_sets->begin(); it != identity_sets->end(); it++) {
    l_inst_identity = (*it);
    (*identity_joins)[l_inst_identity->idset_id] =
        l_inst_identity->joined_identity->idset_id;
  }
}
  ...
  for (auto it = instantiation_mappings->begin();
    ...
    if (it->second->size() != 0) {
      for (auto it2 = it->second->begin(); it2 != it->second->end(); ++it2) {
        lMapping = *it2;
        if (lMapping->to_identity)
          thisAgent->visualizationManager->viz_connect_identities(
              lMapping->from_identity, lMapping->to_identity);
      }
    }
  }
