...
void chunk_record::record_chunk_contents(
    ...
    tc_number pBacktraceNumber, instantiation* pChunkInstantiation,
  ...
  original_productionID =
      thisAgent->explanationMemory->add_production_id_if_necessary(pProduction);
  ...
  chunkInstantiation = thisAgent->explanationMemory->add_instantiation(
      pChunkInstantiation, 0, true);

  instantiation_record *lResultInstRecord, *lNewInstRecord;
  instantiation* lNewInst;

  /* The following creates instantiations records for all instantiation involved
   * in the backtrace.
   *
   * Note that instantiation records are shared between multiple chunk
   * explanations, so we only create a new instantiation record for the first
   * chunk explanation that backtraces through that instantiation.  If an
   * instantiation already exists, we mark it for an update, since the backtrace
   * info will have changed for the new rule being explained. */

  for (auto it = backtraced_instantiations->begin();
       it != backtraced_instantiations->end(); it++) {
    lNewInst = (*it);
    lNewInstRecord =
        thisAgent->explanationMemory->add_instantiation((*it), chunkID);
    backtraced_inst_records->push_back(lNewInstRecord);
  }

  /* The following actually records the contents of the instantiation.
   *
   * Note that if step 1 detected that this is an instantiation that it already
   * recorded for a previous chunk, we call an update function . */

  for (auto it = backtraced_inst_records->begin();
       it != backtraced_inst_records->end(); it++) {
    lNewInstRecord = (*it);
    if (lNewInstRecord->cached_inst->explain_status == explain_recording) {
      lNewInstRecord->record_instantiation_contents();

    } else if (lNewInstRecord->cached_inst->explain_status ==
  ...
  baseInstantiation =
      thisAgent->explanationMemory->get_instantiation(pBaseInstantiation);

  for (auto it = result_instantiations->begin();
       it != result_instantiations->end(); ++it) {
  ...
  auto lGoalIter = thisAgent->explanationMemory->all_identities_in_goal->find(
      thisAgent->explanationBasedChunker->m_inst->match_goal);
...
void chunk_record::generate_dependency_paths() {
  ...
  for (auto it = result_inst_records->begin(); it != result_inst_records->end();
       it++) {
  ...
  for (condition_record_list::iterator it =
           chunkInstantiation->conditions->begin();
       it != chunkInstantiation->conditions->end(); it++) {
    l_cond = (*it);
    l_inst = l_cond->get_instantiation();
    if (l_inst) {
      l_path = l_inst->get_path_to_base();
      /* This is to handle the problem situation that can occur with partially
       * operational conditions that are repaired.  Described above in
       * record_chunk_contents. */
      if (l_path) {
...
void chunk_record::visualize() {
  ...
  thisAgent->visualizationManager->viz_connect_inst_to_chunk(
      baseInstantiation->get_instantiationID(),
      chunkInstantiation->get_instantiationID());
  for (auto it = result_inst_records->begin(); it != result_inst_records->end();
       ++it) {
    thisAgent->visualizationManager->viz_connect_inst_to_chunk(
...
/* This function is not currently used, but I'm leaving in case we ever add
 * something that needs to excise explanations, for example a command or
 * something to limit memory use.  This function has not been tested. */
void chunk_record::excise_chunk_record() {
  /* For this to work, store id of last chunk that created instantiation record.
   * If it's the same as this chunk being excised, no other chunk uses it.  This
   * assumes that this is only called when a chunk fails.  If we need to excise
   * records in general, we'll need refcounts */
  for (auto it = backtraced_inst_records->begin();
       it != backtraced_inst_records->end(); it++) {
    if ((*it)->get_chunk_creator() == chunkID) {
      (*it)->delete_instantiation();
      thisAgent->explanationMemory->delete_instantiation(
          (*it)->get_instantiationID());
    }
  }
}
