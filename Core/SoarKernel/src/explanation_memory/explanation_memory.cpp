...
void Explanation_Memory::clear_explanations() {
  ...
  for (auto it = (*instantiations).begin(); it != (*instantiations).end();
       ++it) {
    it->second->clean_up();
    thisAgent->memoryManager->free_with_pool(MP_instantiation_record,
                                             it->second);
  }
  instantiations->clear();

  for (auto it = (*all_conditions).begin(); it != (*all_conditions).end();
       ++it) {
    it->second->clean_up();
  ...
  for (auto it = (*all_actions).begin(); it != (*all_actions).end(); ++it) {
    ...
    thisAgent->memoryManager->free_with_pool(
        MP_action_record, static_cast<action_record*>(it->second));
  }
  all_actions->clear();

  for (auto it = (*cached_production).begin(); it != (*cached_production).end();
       ++it) {
    (*it)->clean_up();
...
void Explanation_Memory::add_chunk_record(instantiation* pBaseInstantiation) {
  if ((!m_all_enabled) && (!pBaseInstantiation->prod ||
                           !pBaseInstantiation->prod->explain_its_chunks)) {
    current_recording_chunk = NULL;
    return;
  }

  thisAgent->memoryManager->allocate_with_pool(MP_chunk_record,
                                               &current_recording_chunk);
...
void Explanation_Memory::cancel_chunk_record() {
  if (current_recording_chunk) {
    ...
    thisAgent->memoryManager->free_with_pool(MP_chunk_record,
                                             current_recording_chunk);
    current_recording_chunk = NULL;
  }
}
...
void Explanation_Memory::add_result_instantiations(instantiation* pBaseInst,
                                                   preference* pResults) {
  if (current_recording_chunk) {
    for (preference* lResult = pResults; lResult != NIL;
         lResult = lResult->next_result) {
...
void Explanation_Memory::record_chunk_contents(
    production* pProduction, condition* lhs, action* rhs, preference* results,
    id_to_join_map* pIdentitySetMappings, instantiation* pBaseInstantiation,
    instantiation* pChunkInstantiation, ProductionType prodType) {
  if (current_recording_chunk) {
    current_recording_chunk->record_chunk_contents(
        pProduction, lhs, rhs, results, pIdentitySetMappings,
        pBaseInstantiation, backtrace_number, pChunkInstantiation, prodType);
    chunks->insert({pProduction->name, current_recording_chunk});
    chunks_by_ID->insert(
        {current_recording_chunk->chunkID, current_recording_chunk});
    thisAgent->symbolManager->symbol_add_ref(pProduction->name);
  }
}

condition_record* Explanation_Memory::add_condition(
    condition_record_list* pCondList, condition* pCond,
    instantiation_record* pInst, bool pMakeNegative,
    bool isChunkInstantiation) {
  condition_record* lCondRecord;

  if (pCond->type != CONJUNCTIVE_NEGATION_CONDITION) {
    thisAgent->memoryManager->allocate_with_pool(MP_condition_record,
                                                 &lCondRecord);
    increment_counter(condition_id_count);
    lCondRecord->init(thisAgent, pCond, condition_id_count, pInst,
    ...
    for (condition* cond = pCond->data.ncc.top; cond != NIL;
         cond = cond->next) {
      new_cond_record =
          add_condition(pCondList, cond, pInst, true, isChunkInstantiation);
    }
    return new_cond_record;
  }
}

instantiation_record* Explanation_Memory::add_instantiation(
    instantiation* pInst, uint64_t pChunkID, bool isChunkInstantiation) {
  ...
  if (pInst->explain_status == explain_unrecorded) {
    /* Instantiations have their backtrace_number marked as the dependency
     * analysis is performed, so we can use that to determine if this
     * instantiation needs to be added.
     *
     * If it was not in the backtrace (created superstate item that was in bt)
     * or bt_depth == max, we add this instantiation but set it as a terminal
     * instantiation.  This puts a cap on the memory used, which can easily
     * cause issues for example in the count-and-die test agent that creates
     * thousands of instantiations */
    if ((pInst->backtrace_number != backtrace_number) ||
        (pInst->explain_depth == EXPLAIN_MAX_BT_DEPTH)) {
    ...
    thisAgent->memoryManager->allocate_with_pool(MP_instantiation_record,
                                                 &lInstRecord);
...
action_record* Explanation_Memory::add_result(preference* pPref,
                                              action* pAction,
                                              bool isChunkInstantiation) {
  increment_counter(action_id_count);
  action_record* lActionRecord;
  thisAgent->memoryManager->allocate_with_pool(MP_action_record,
                                               &lActionRecord);
...
instantiation_record* Explanation_Memory::get_instantiation(
    instantiation* pInst) {
  assert(pInst);
...
bool Explanation_Memory::toggle_production_watch(production* pProduction) {
  if (pProduction->explain_its_chunks) {
    ...
    outputManager->printa_sf(
        thisAgent, "No longer watching any chunks formed by rule '%y'\n",
        pProduction->name);
  } else {
    pProduction->explain_its_chunks = true;
    ++num_rules_watched;
    outputManager->printa_sf(thisAgent,
                             "%eNow watching any chunks formed by rule '%y'\n",
                             pProduction->name);
  }
  return true;
}
...
bool Explanation_Memory::watch_rule(const std::string* pStringParameter) {
  ...
  outputManager->printa_sf(thisAgent,
                           "Could not find a rule named %s to watch.\nType "
                           "'print' to see a list of all rules.\n",
...
bool Explanation_Memory::explain_chunk(const std::string* pStringParameter) {
  ...
  if (from_string(lObjectID, pStringParameter->c_str())) {
    if (!print_chunk_explanation_for_id(lObjectID)) {
      outputManager->printa_sf(
          thisAgent,
          "Could not find a rule name or id %s.\nType 'explain list-chunks' or "
          "'explain list-justifications' to see a list of rule formations Soar "
          "has recorded.\n",
          pStringParameter->c_str());
    } else {
    ...
    sym =
        thisAgent->symbolManager->find_str_constant(pStringParameter->c_str());
    if (sym && sym->sc->production) {
      ...
      outputManager->printa_sf(
          thisAgent,
          "Soar has not recorded an explanation for %s.\nType 'explain "
          "list-chunks' or 'explain list-justifications' to see a list of rule "
          "formations Soar has recorded.\n",
          pStringParameter->c_str());
      return false;
    }
  }
...
void Explanation_Memory::save_excised_production(production* pProd) {
  ...
  thisAgent->memoryManager->allocate_with_pool(MP_production_record,
                                               &lProductionRecord);
  lProductionRecord->init(thisAgent, pProd);
  if (lProductionRecord->was_generated()) {
    cached_production->insert(lProductionRecord);
  } else {
    thisAgent->memoryManager->free_with_pool(MP_production_record,
                                             lProductionRecord);
  }
}
...
bool Explanation_Memory::print_instantiation_explanation_for_id(
    uint64_t pInstID) {
  auto iter_inst = instantiations->find(pInstID);
  if (iter_inst == instantiations->end()) {
    outputManager->printa_sf(
        thisAgent, "Could not find an instantiation with ID %u.\n", pInstID);
    return false;
  }
...
bool Explanation_Memory::explain_instantiation(
    const std::string* pObjectIDString) {
  bool lSuccess = false;
  uint64_t lObjectID = 0;
  if (!from_string(lObjectID, pObjectIDString->c_str())) {
    outputManager->printa_sf(thisAgent,
                             "The instantiation ID must be a number.\n");
  }
...
void Explanation_Memory::clear_identity_sets() {
  ...
  for (auto it1 = all_identities_in_goal->begin();
       it1 != all_identities_in_goal->end(); ++it1) {
    lSym = it1->first;
...
void Explanation_Memory::add_identity_set_mapping(uint64_t pI_ID,
                                                  IDSet_Mapping_Type pType,
                                                  Identity* pFromJoinSet,
                                                  Identity* pToJoinSet) {
  if (current_recording_chunk)
    current_recording_chunk->identity_analysis.add_identity_mapping(
        pI_ID, pType, pFromJoinSet, pToJoinSet);
}
...
bool Explanation_Memory::visualize_instantiation_explanation_for_id(
    uint64_t pInstID) {
  auto iter_inst = instantiations->find(pInstID);
  if (iter_inst == instantiations->end()) {
    outputManager->printa_sf(
