...
void Explanation_Memory::switch_to_explanation_trace(
    bool pEnableExplanationTrace) {
...
void Explanation_Memory::print_formation_explanation() {
  ...
  if (current_discussed_chunk->result_inst_records->size() > 0) {
    outputManager->printa_sf(
        thisAgent,
        "The following %d instantiations fired to produce "
        "results...\n\n------\n\n",
        static_cast<int64_t>(
  ...
  outputManager->printa_sf(
      thisAgent,
      "Initial base instantiation (i %u) that fired when %y matched at level "
      "%d at time %u:\n\n",
  ...
  if (current_discussed_chunk->result_inst_records->size() > 0) {
    outputManager->printa_sf(
        thisAgent,
        "\n%d instantiation(s) that created extra results indirectly because "
        "they were connected to the results of the base instantiation:\n\n",
        static_cast<int64_t>(
            (current_discussed_chunk->result_inst_records->size() - 1)));
    for (auto it = current_discussed_chunk->result_inst_records->begin();
         it != current_discussed_chunk->result_inst_records->end(); ++it) {
...
void Explanation_Memory::print_footer(bool pPrintDiscussedChunkCommands) {
  ...
  if (print_explanation_trace) {
    outputManager->printa_sf(
        thisAgent,
        "- explain f %-Explain initial formation of chunk %-explain w %-Switch "
        "to working memory trace    -\n");
  } else {
    outputManager->printa_sf(
        thisAgent,
        "- explain f %-Explain initial formation of chunk %-explain e %-Switch "
        "to explanation trace       -\n");
  }
  outputManager->printa_sf(
      thisAgent,
      "- explain c %-Explain constraints required by problem-solving %-explain "
      "i %-Explain identity analysis         -\n");
  outputManager->printa_sf(thisAgent,
                           "- explain s %-Print chunk statistics %-chunk stats "
...
bool Explanation_Memory::is_condition_related(condition_record* pCondRecord) {
  //    if ((pCondRecord->condition_tests.id->eq_test->identity ==
  //    current_explained_ids.id) ||
  //        (pCondRecord->condition_tests.attr->eq_test->identity ==
  //        current_explained_ids.id) ||
  //        (pCondRecord->condition_tests.value->eq_test->identity ==
  //        current_explained_ids.id))
  //    {
  //    }
  return false;
...
void Explanation_Memory::print_path_to_base(const inst_record_list* pPathToBase,
                                            bool pPrintFinal,
                                            const char* pFailedStr,
  ...
  if (pPathToBase && (!pPathToBase->empty())) {
    ...
    if (pPathToBase->size() >= min_size) {
      ...
      bool notFirst = false;
      for (auto it = (pPathToBase)->rbegin(); it != (pPathToBase)->rend();
           it++) {
        if (notFirst) {
          thisAgent->outputManager->printa(thisAgent, " -> ");
        }
        thisAgent->outputManager->printa_sf(thisAgent, "i %u",
                                            (*it)->get_instantiationID());
        notFirst = true;
      }
    } else if (pFailedStr) {
...
void Explanation_Memory::print_chunk_actions(action_record_list* pActionRecords,
                                             production* pOriginalRule,
    ...
    if (print_explanation_trace) {
      ...
      if (!pOriginalRule || !pOriginalRule->p_node) {
        if (pExcisedRule) {
          ...
          outputManager->printa_sf(
              thisAgent, "No rule for this instantiation found in RETE\n");
          return;
        }
      } else {
        p_node_to_conditions_and_rhs(thisAgent, pOriginalRule->p_node, NIL, NIL,
                                     &top, &bottom, &rhs);
        pRhs = rhs;
      }
    }
...
void Explanation_Memory::print_instantiation_actions(
    action_record_list* pActionRecords, production* pOriginalRule,
    action* pRhs) {
  if (pActionRecords->empty()) {
    outputManager->printa(thisAgent, "No actions on right-hand-side\n");
  } else {
...
void Explanation_Memory::print_instantiation_explanation(
    instantiation_record* pInstRecord, bool printFooter) {
  if (print_explanation_trace) {
...
void Explanation_Memory::print_chunk_explanation() {
  ...
  if (print_explanation_trace) {
    current_discussed_chunk->chunkInstantiation->print_for_explanation_trace(
        true, false);
  } else {
    current_discussed_chunk->chunkInstantiation->print_for_wme_trace(true,
                                                                     false);
  }
...
void Explanation_Memory::print_explain_summary() {
  ...
  outputManager->printa(
      thisAgent, "=======================================================\n");
  outputManager->printa_sf(thisAgent,
                           "Watch all chunk formations        %-%s\n",
                           (m_all_enabled ? "Yes" : "No"));
  outputManager->printa_sf(thisAgent,
                           "Explain justifications            %-%s\n",
                           (m_justifications_enabled ? "Yes" : "No"));
  outputManager->printa_sf(thisAgent,
                           "Number of specific rules watched  %-%d\n",
  ...
  if (current_discussed_chunk) {
    outputManager->printa_sf(thisAgent,
                             "Current rule being explained: %-%s (c %u)\n\n",
                             current_discussed_chunk->name->sc->name,
                             current_discussed_chunk->chunkID);
  } else {
  ...
  outputManager->printa(
      thisAgent, "-------------------------------------------------------\n\n");
  outputManager->printa(thisAgent,
                        "Use 'explain chunk [ <chunk-name> | id ]' to explore "
                        "the formation of that chunk.\n");
  outputManager->printa_sf(thisAgent,
                           "Use 'explain ?' to learn more about explain's "
...
void Explanation_Memory::print_EBC_stats() {
  ...
  outputManager->printa_sf(thisAgent,
                           ...
                           "========================\n");
  outputManager->printa_sf(
      thisAgent, "                  Explanation-Based Chunking Statistics\n");
  outputManager->printa_sf(thisAgent,
                           "==================================================="
                           "========================\n");
  outputManager->printa_sf(
  ...
  outputManager->printa_sf(
      thisAgent,
      "Sub-states analyzed                                    %-%u\n",
      stats.chunks_attempted);
  outputManager->printa_sf(
      thisAgent,
      "Number of rules fired in substates analyzed            %-%u\n",
      thisAgent->explanationBasedChunker->get_instantiation_count());
  outputManager->printa_sf(
      thisAgent,
      "Number of rule firings analyzed during backtracing     %-%u\n",
  ...
  outputManager->printa_sf(
      thisAgent,
      "Disjunction tests merged                               %-%u\n",
      stats.merged_disjunctions);
  outputManager->printa_sf(
      thisAgent,
      "Operational constraints                                %-%u\n",
      stats.operational_constraints);
  outputManager->printa_sf(
      thisAgent,
      "Non-operational constraints detected                   %-%u\n",
      stats.constraints_collected);
  outputManager->printa_sf(
      thisAgent,
      "Non-operational constraints enforced                   %-%u\n",
      stats.constraints_attached);
  outputManager->printa_sf(thisAgent,
                           "\n-------------------------------------------------"
                           "--------------------------\n");
  outputManager->printa_sf(
      thisAgent, "                     Problem-Solving Characteristics\n");
  outputManager->printa_sf(thisAgent,
                           "---------------------------------------------------"
                           "------------------------\n");
  outputManager->printa_sf(thisAgent,
                           "Rules repaired that had unconnected conditions or "
                           "actions          %-%u\n",
                           stats.chunks_repaired);
  outputManager->printa_sf(thisAgent,
                           "Chunk used negated reasoning about sub-state       "
                           "                %-%u\n",
                           stats.tested_local_negation);
  outputManager->printa_sf(thisAgent,
                           "Chunk tested knowledge retrieved from long-term "
                           "memory             %-%u\n",
                           stats.tested_ltm_recall);
  outputManager->printa_sf(thisAgent,
                           "Justification used negated reasoning about "
                           "sub-state               %-%u\n",
                           stats.tested_local_negation_just);
  outputManager->printa_sf(thisAgent,
                           "Justification tested knowledge retrieved from "
                           "long-term memory     %-%u\n",
  ...
  outputManager->printa_sf(thisAgent,
                           "Distinct identities in learned rules               "
                           "                %-%u\n",
  ...
  outputManager->printa_sf(thisAgent,
                           ...
                           "--------------------------\n");
  outputManager->printa_sf(
      thisAgent, "                      Learning Skipped or Unsuccessful\n");
  outputManager->printa_sf(thisAgent,
                           "---------------------------------------------------"
                           "------------------------\n");
  outputManager->printa_sf(thisAgent,
                           "Ignored duplicate of existing rule                 "
                           "                %-%u\n",
                           stats.duplicates);
  outputManager->printa_sf(thisAgent,
                           "Skipped because problem-solving tested ^quiescence "
                           "true            %-%u\n",
                           stats.tested_quiescence);
  outputManager->printa_sf(thisAgent,
                           "Skipped because no super-state knowledge tested    "
                           "                %-%u\n",
                           stats.no_grounds);
  outputManager->printa_sf(thisAgent,
                           "Skipped because MAX-CHUNKS exceeded in a decision "
                           "cycle            %-%u\n",
                           stats.max_chunks);
  outputManager->printa_sf(thisAgent,
                           "Skipped because MAX-DUPES exceeded for rule this "
                           "decision cycle    %-%u\n",
                           stats.max_dupes);
}
...
void Explanation_Memory::print_chunk_stats(chunk_record* pChunkRecord,
  ...
  outputManager->printa_sf(
      thisAgent, "Number of conditions:           %-%u\n",
  ...
  outputManager->printa_sf(
      thisAgent, "Number of actions:              %-%u\n",
      static_cast<uint64_t>(pChunkRecord->chunkInstantiation->actions->size()));
  outputManager->printa_sf(thisAgent,
                           "Base instantiation:             %-i %u (%y)\n",
                           pChunkRecord->baseInstantiation->instantiationID,
                           pChunkRecord->baseInstantiation->production_name);
  if (pChunkRecord->result_inst_records->size() > 0) {
    outputManager->printa_sf(
        thisAgent, "Number of child result instantiations:  %-%u\n",
        static_cast<uint64_t>(pChunkRecord->result_inst_records->size()));
    outputManager->printa_sf(thisAgent, "Child result instantiations: ");
    for (auto it = pChunkRecord->result_inst_records->begin();
         it != pChunkRecord->result_inst_records->end(); ++it) {
  ...
  outputManager->printa_sf(thisAgent,
                           ...
                           "------------------------\n");
  outputManager->printa_sf(thisAgent,
                           "Identities created in rule's sub-state:            "
                           "                 %-%u\n",
  ...
  outputManager->printa_sf(thisAgent,
                           ...
                           "------------------------\n");
  outputManager->printa_sf(
      thisAgent, "Used negated reasoning about sub-state:              %-%s\n",
      (pChunkRecord->stats.tested_local_negation ? "Yes" : "No"));
  outputManager->printa_sf(
      thisAgent, "Tested ^quiescence true:                             %-%s\n",
      (pChunkRecord->stats.tested_quiescence ? "Yes" : "No"));
  outputManager->printa_sf(
      thisAgent, "Tested knowledge retrieved from long-term memory:    %-%s\n",
      (pChunkRecord->stats.tested_ltm_recall ? "Yes" : "No"));
  outputManager->printa_sf(
      thisAgent,
      "Added conditions to repair unconnected conditions or actions:    %-%s\n",
      (pChunkRecord->stats.repaired ? "Yes" : "No"));
}

void Explanation_Memory::list_explained_rules(short pNumToPrint, bool pChunks,
                                              bool pNewLine) {
  ...
  for (auto it = (*chunks).begin(); it != (*chunks).end(); ++it) {
    ...
    if (!lHeaderPrinted) {
      outputManager->printa_sf(
          thisAgent,
          pChunks ? "Chunks available for explanation:%s"
                  : "Justifications available for explanation:%s",
          pNewLine ? "\n" : " ");
      lHeaderPrinted = true;
    }
  ...
  if (!lHeaderPrinted) {
    outputManager->printa_sf(thisAgent, "No %s have been recorded.",
                             lRuleTypeString.c_str());
  } else {
    if (pNumToPrint && (lNumPrinted < chunks->size())) {
      outputManager->printa_sf(
          thisAgent,
          "\n* Note:  Only listed the first %d %s recorded.  Type 'explain "
          "list-%s' to see the other %d %s.\n",
          static_cast<int64_t>(pNumToPrint), lRuleTypeString.c_str(),
          lRuleTypeString.c_str(),
          static_cast<int64_t>(chunks->size() - lNumPrinted),
          lRuleTypeString.c_str());
    }
  }
}

bool Explanation_Memory::print_watched_rules_of_type(
    agent* thisAgent, unsigned int productionType, short& pNumToPrint) {
  short lNumPrinted = 0;
  bool lThereWasMore = false;

  for (production* prod = thisAgent->all_productions_of_type[productionType];
...
void Explanation_Memory::print_rules_watched(short pNumToPrint) {
  ...
  lThereWasMore = print_watched_rules_of_type(thisAgent, USER_PRODUCTION_TYPE,
                                              lNumLeftToPrint);
  if (!lThereWasMore) {
    lThereWasMore = print_watched_rules_of_type(
        thisAgent, CHUNK_PRODUCTION_TYPE, lNumLeftToPrint);
  }
  if (!lThereWasMore) {
    lThereWasMore = print_watched_rules_of_type(
        thisAgent, JUSTIFICATION_PRODUCTION_TYPE, lNumLeftToPrint);
  }
  if (!lThereWasMore) {
    lThereWasMore = print_watched_rules_of_type(
        thisAgent, DEFAULT_PRODUCTION_TYPE, lNumLeftToPrint);
  }
  if (!lThereWasMore) {
    lThereWasMore = print_watched_rules_of_type(
        thisAgent, TEMPLATE_PRODUCTION_TYPE, lNumLeftToPrint);
  }
  if (lThereWasMore) {
    outputManager->printa_sf(
        thisAgent,
        "\n* Note:  Only printed the first %d rules.  Type 'explain watch' to "
        "see the other %d rules.\n",
        static_cast<int64_t>(pNumToPrint),
        static_cast<int64_t>(lNumLeftToPrint));
  }
}
...
void Explanation_Memory::print_involved_instantiations() {
  ...
  //    struct cmp_iID
  //        {
  //            bool operator () (const instantiation_record& a, const
  //            instantiation_record& b)
  //            {
  //                  return (a.instantiationID <= b.instantiationID);
  //            }
  //        };
  //    std::set< instantiation_record*, cmp_iID > sorted_set;
  ////    { std::begin((*instantiations_for_current_chunk)),
  /// std::end((*instantiations_for_current_chunk)) };
  //    std::copy(std::begin(instantiations_for_current_chunk),
  //    std::end(instantiations_for_current_chunk), std::inserter(sorted_set));
  assert(current_discussed_chunk);

  outputManager->printa_sf(
      thisAgent,
      "This chunk summarizes the problem-solving involved in the following %d "
      "rule firings:\n\n",
      static_cast<int64_t>(
          current_discussed_chunk->backtraced_inst_records->size()));

  for (auto it = current_discussed_chunk->backtraced_inst_records->begin();
       it != current_discussed_chunk->backtraced_inst_records->end(); ++it) {
    outputManager->printa_sf(thisAgent, "   i %u (%y)\n",
                             (*it)->instantiationID, (*it)->production_name);
  }
  outputManager->printa(thisAgent, "\n");
