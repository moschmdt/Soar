...
class Explanation_Memory {
 ...
 public:
  ...
  void set_justifications_enabled(bool pEnabled) {
    m_justifications_enabled = pEnabled;
  };
  ...
  void add_bt_instantiation(instantiation* pInst, BTSourceType bt_type) {
    if (current_recording_chunk)
      current_recording_chunk->backtraced_instantiations->insert(pInst);
  };

  void add_chunk_record(instantiation* pBaseInstantiation);
  void add_result_instantiations(instantiation* pBaseInst,
                                 preference* pResults);
  void record_chunk_contents(production* pProduction, condition* lhs,
                             action* rhs, preference* results,
                             id_to_join_map* pIdentitySetMappings,
                             instantiation* pBaseInstantiation,
                             instantiation* pChunkInstantiation,
  ...
  void add_identity_set_mapping(uint64_t pI_ID, IDSet_Mapping_Type pType,
                                Identity* pFromJoinSet, Identity* pToJoinSet);

  instantiation_record* add_instantiation(instantiation* pInst,
                                          uint64_t pChunkID = 0,
                                          bool isChunkInstantiation = false);
  chunk_record* get_current_discussed_chunk() {
  ...
  void increment_stat_tested_local_negation(ebc_rule_type pType) {
    if (pType == ebc_chunk)
      stats.tested_local_negation++;
    else
      stats.tested_local_negation_just++;
    if (current_recording_chunk)
      current_recording_chunk->stats.tested_local_negation = true;
  };
  void increment_stat_tested_ltm_recall(ebc_rule_type pType) {
    if (pType == ebc_chunk)
      stats.tested_ltm_recall++;
    else
      stats.tested_ltm_recall_just++;
    if (current_recording_chunk)
      current_recording_chunk->stats.tested_ltm_recall = true;
  };
  void increment_stat_tested_quiescence() {
    stats.tested_quiescence++;
    if (current_recording_chunk)
      current_recording_chunk->stats.tested_quiescence = true;
  };
  void increment_stat_merged_conditions(int pCount = 1) {
    stats.merged_conditions += pCount;
    if (current_recording_chunk)
      current_recording_chunk->stats.merged_conditions += pCount;
  };
  void increment_stat_merged_disjunctions() {
    stats.merged_disjunctions++;
    if (current_recording_chunk)
      current_recording_chunk->stats.merged_disjunctions++;
  };
  void increment_stat_chunks_attempted() { stats.chunks_attempted++; };
  void increment_stat_chunks_succeeded() { stats.chunks_succeeded++; };
  void increment_stat_justifications_succeeded() {
    stats.justifications_succeeded++;
  };
  void increment_stat_instantations_backtraced() {
    stats.instantations_backtraced++;
    if (current_recording_chunk)
      current_recording_chunk->stats.instantations_backtraced++;
  };
  void increment_stat_constraints_attached() {
    stats.constraints_attached++;
    if (current_recording_chunk)
      current_recording_chunk->stats.constraints_attached++;
  };
  void increment_stat_constraints_collected() {
    stats.constraints_collected++;
    if (current_recording_chunk)
      current_recording_chunk->stats.constraints_collected++;
  };
  void increment_stat_chunks_repaired() {
    stats.chunks_repaired++;
    if (current_recording_chunk) current_recording_chunk->stats.repaired = true;
  };

  void increment_stat_identities_created() { stats.identities_created++; };
  void increment_stat_identity_propagations() {
    stats.identity_propagations++;
  };
  void increment_stat_identity_propagations_blocked() {
    stats.identity_propagations_blocked++;
  };

  void increment_stat_identities_joined() {
    stats.identities_joined++;
    if (current_recording_chunk)
      current_recording_chunk->stats.identities_joined++;
  };
  void increment_stat_operational_constraints() {
    stats.operational_constraints++;
    if (current_recording_chunk)
      current_recording_chunk->stats.operational_constraints++;
  };
  void increment_stat_identities_participated(int pCount = 1) {
    stats.identities_participated += pCount;
    if (current_recording_chunk)
  ...
  void increment_stat_identities_literalized() {
    ...
    if (current_recording_chunk)
      current_recording_chunk->stats.identities_literalized++;
  };
  ...
  //        bool explain_item(const std::string* pObjectTypeString, const
  //        std::string* pObjectIDString);
 ...
 private:
  ...
  condition_record* add_condition(condition_record_list* pCondList,
                                  condition* pCond,
                                  instantiation_record* pInst = NULL,
                                  bool pMakeNegative = false,
                                  bool isChunkInstantiation = false);
  action_record* add_result(preference* pPref, action* pAction,
  ...
  void list_explained_rules(short pNumToPrint = 0, bool pChunks = true,
                            bool pNewLine = false);
  void print_rules_watched(short pNumToPrint = 0);
  bool print_watched_rules_of_type(agent* thisAgent,
                                   unsigned int productionType,
                                   short& pNumToPrint);
  void print_action_list(action_record_list* pActionRecords,
                         production* pOriginalRule, action* pRhs = NULL,
                         production_record* pExcisedRule = NULL);
  void print_chunk_actions(action_record_list* pActionRecords,
                           production* pOriginalRule,
                           production_record* pExcisedRule);
  void print_instantiation_actions(action_record_list* pActionRecords,
  ...
  void print_path_to_base(const inst_record_list* pPathToBase,
                          bool pPrintFinal = true,
                          const char* pFailedStr = NULL,
