...
const std::string GraphViz_Visualizer::get_next_filename() {
  if (!thisAgent->visualizationManager->settings->use_same_file->get_value()) {
    std::string lFileNameBase =
        thisAgent->visualizationManager->settings->file_name->get_value();
    std::string lFileName;
    bool fileNameFound = false;
    while (!fileNameFound) {
      lFileName =
          lFileNameBase + std::to_string(++m_file_count) + std::string(".gv");
...
GraphViz_Visualizer::~GraphViz_Visualizer() { delete settings; }
...
void GraphViz_Visualizer::visualize_wm(Symbol* pSym, int pDepth) {
  ...
  if (thisAgent->visualizationManager->settings->memory_format->get_value() ==
      viz_node) {
...
void GraphViz_Visualizer::visualize_smem(uint64_t lti_id, int pDepth) {
  ...
  for (auto it = store_set.begin(); it != store_set.end(); ++it) {
    ...
    thisAgent->visualizationManager->viz_object_start_string(
        idStr, current_ltm->lti_id, viz_id_and_augs);
    lRecordStarted = false;

    for (auto map_it = current_ltm->slots->begin();
         map_it != current_ltm->slots->end(); ++map_it) {
      Symbol* attr = map_it->first;
      ltm_slot* current_slot = map_it->second;

      for (auto slot_it = current_slot->begin(); slot_it != current_slot->end();
           ++slot_it) {
...
void GraphViz_Visualizer::viz_graph_start(bool pLeftRight) {
  ...
  graphviz_output +=
      thisAgent->visualizationManager->settings->line_style->get_value();
  graphviz_output += "\"];\n";
}
...
void GraphViz_Visualizer::viz_object_start(Symbol* pName, uint64_t node_id,
                                           visObjectType objectType,
                                           std::string* pMakeUnique) {
  std::string pNameString = pName->to_string();
  viz_object_start_string(pNameString, node_id, objectType, pMakeUnique);
}

void GraphViz_Visualizer::viz_object_start_string(std::string& pName,
                                                  uint64_t node_id,
                                                  visObjectType objectType,
                                                  std::string* pMakeUnique) {
  ...
  switch (objectType) {
    case viz_inst_record:
      outputManager->sprinta_sf(thisAgent, graphviz_output,
                                ...
                                node_id);
      viz_table_start();
      outputManager->sprinta_sf(
          thisAgent, graphviz_output,
          "                \xF3TR\xF2 \xF3TD COLSPAN=\"3\" "
          "BGCOLOR=\"BLACK\"\xF2 \xF3"
          "FONTCOLOR=\"WHITE\"\xF2Instantiation (i %u) of "
          "rule\xF3\x42R/\xF2%s\xF3/FONT\xF2\xF3/TD\xF2\xF3/TR\xF2\n",
          node_id, pName.c_str());
      break;
    ...
    case viz_chunk_record:
      ...
      outputManager->sprinta_sf(thisAgent, graphviz_output,
                                "                \xF3TR\xF2 \xF3TD "
                                "COLSPAN=\"3\" BGCOLOR=\"BLUE\"\xF2 \xF3"
                                "FONTCOLOR=\"YELLOW\"\xF2%s (i "
                                "%u)\xF3/FONT\xF2\xF3/TD\xF2 \xF3/TR\xF2\n",
                                pName.c_str(), node_id);
      break;
    case viz_id_and_augs:
      ...
      outputManager->sprinta_sf(thisAgent, graphviz_output,
                                "                \xF3TR\xF2 \xF3TD "
                                "COLSPAN=\"3\"\xF2%s\xF3/TD\xF2 \xF3/TR\xF2\n",
                                pName.c_str());
      break;
...
void GraphViz_Visualizer::viz_table_start() {
  outputManager->sprinta_sf(thisAgent, graphviz_output,
                            "\xF3TABLE BORDER=\"0\" CELLBORDER=\"1\" "
                            "CELLSPACING=\"0\" CELLPADDING=\"4\"\xF2\n");
}
...
void GraphViz_Visualizer::viz_table_element_conj_start(uint64_t pNodeID,
                                                       char pTypeChar,
                                                       WME_Field pField,
                                                       bool pRowBorder,
                                                       const char* pModStr) {
  ...
  if (pRowBorder)
    borderString = " BORDER=\"3\"";
  else
    borderString = " ";
  if (pNodeID) {
    outputManager->sprinta_sf(
        thisAgent, graphviz_output,
        "\n                \xF3TD%s%sPORT=\"%c_%u%s\"\xF2 ", borderString,
        pModStr, pTypeChar, pNodeID, ((pField == ID_ELEMENT) ? "_l" : "_r"));
  } else {
    outputManager->sprinta_sf(thisAgent, graphviz_output,
                              "\n                \xF3TD%s%s\xF2 ", borderString,
                              pModStr);
  }
  viz_table_start();
}

void GraphViz_Visualizer::viz_table_element_start(uint64_t pNodeID,
                                                  char pTypeChar,
                                                  WME_Field pField,
                                                  bool pRowBorder,
                                                  const char* pModStr) {
  ...
  if (pNodeID) {
    outputManager->sprinta_sf(thisAgent, graphviz_output,
                              "\xF3TD%s%sPORT=\"%c_%u%s\"\xF2 ", borderString,
                              pModStr, pTypeChar, pNodeID,
                              ((pField == ID_ELEMENT) ? "_l" : "_r"));
  } else {
    outputManager->sprinta_sf(thisAgent, graphviz_output, "\xF3TD%s%s\xF2 ",
                              borderString, pModStr);
  }
}
...
void GraphViz_Visualizer::viz_text_record(const char* pMsg) {
  outputManager->sprinta_sf(thisAgent, graphviz_output,
                            "\xF3TD\xF2 %s \xF3/TD\xF2", pMsg);
}

void GraphViz_Visualizer::viz_NCC_start() {
  graphviz_output +=
      "                \xF3TR\xF2 \xF3TD COLSPAN=\"3\" ALIGN=\"LEFT\"\xF2 "
      "-\xE3 \xF3/TD\xF2 \xF3/TR\xF2\n";
}

void GraphViz_Visualizer::viz_NCC_end() {
  graphviz_output +=
      "                \xF3TR\xF2 \xF3TD COLSPAN=\"3\" ALIGN=\"LEFT\"\xF2 \xE1 "
      "\xF3/TD\xF2 \xF3/TR\xF2\n";
}

void GraphViz_Visualizer::viz_seperator() {
  graphviz_output +=
      "                \xF3TR\xF2 \xF3TD COLSPAN=\"3\"\xF2 ----> \xF3/TD\xF2 "
      "\xF3/TR\xF2";
}

void GraphViz_Visualizer::viz_endl() { graphviz_output += "\n"; }
...
void GraphViz_Visualizer::clear_visualization() { graphviz_output.clear(); }
void GraphViz_Visualizer::viz_connect_action_to_cond(uint64_t pSrcRuleID,
                                                     uint64_t pSrcActionID,
                                                     uint64_t pTargetRuleID,
                                                     uint64_t pTargetCondID) {
...
void GraphViz_Visualizer::viz_connect_inst_to_chunk(uint64_t pSrcRuleID,
                                                    uint64_t pTargetRuleID) {
  graphviz_output += "   rule";
  graphviz_output += std::to_string(pSrcRuleID);
...
void GraphViz_Visualizer::viz_connect_identities(uint64_t pSrcRuleID,
                                                 uint64_t pTargetRuleID) {
