...
bool Output_Manager::wme_to_string(agent* thisAgent, wme* w,
                                   std::string& destString) {
  sprinta_sf(thisAgent, destString,
             "(t%u: %y ^%y %y%s    [lvl = %d-%d, rc = %u]", w->timetag, w->id,
             w->attr, w->value, (w->acceptable ? " +)" : ")"),
...
void Output_Manager::test_to_string(test t, std::string& destString,
                                    bool show_equality) {
  cons* c;
  if (!t) {
  ...
  switch (t->type) {
    ...
    case CONJUNCTIVE_TEST:
      ...
      for (c = t->data.conjunct_list; c != NIL; c = c->rest) {
        this->test_to_string(static_cast<test>(c->first), destString,
                             show_equality);
        destString += ' ';
      }
      destString += '}';
...
void Output_Manager::condition_cons_to_string(agent* thisAgent, cons* c,
                                              std::string& destString) {
  while (c) {
    sprinta_sf(thisAgent, destString, "%s: %l\n", m_pre_string,
               static_cast<condition_struct*>(c->first));
    c = c->rest;
  }
  return;
}

void Output_Manager::condition_to_string(agent* thisAgent, condition* cond,
                                         std::string& destString) {
...
void Output_Manager::condition_list_to_string(agent* thisAgent,
                                              condition* top_cond,
                                              std::string& destString) {
  condition* cond;
  int64_t count = 0;
...
void Output_Manager::rhs_value_to_cstring(rhs_value rv, char* dest,
                                          size_t dest_size) {
  std::string lStr;
...
void Output_Manager::rhs_value_to_string(rhs_value rv, std::string& destString,
                                         bool rereadable,
                                         struct token_struct* tok, wme* w,
                                         bool pEmptyStringForNullIdentity) {
  rhs_symbol rsym = NIL;
  Symbol* sym = NIL;
  cons* c;
  cons* fl;
  ...
  if (rhs_value_is_null(rv)) {
    ...
    if (m_print_identity_effective &&
        (rsym->inst_identity || rsym->identity_id_unjoined)) {
      Identity* l_identity = rsym->identity;

      if (m_print_actual_effective) destString += ' ';
      if (l_identity) {
        if (l_identity->joined_identity != l_identity) {
          sprint_sf(destString, "[v%uc%ui%uj%u]", rsym->inst_identity,
                    rsym->cv_id, l_identity->get_sub_identity(),
                    l_identity->get_identity());
        } else {
          sprint_sf(destString, "[v%uc%ui%u]", rsym->inst_identity, rsym->cv_id,
                    l_identity->get_sub_identity());
        }
      } else if (rsym->identity_id_unjoined) {
        sprint_sf(destString, "[%u->v%uc%u]", rsym->identity_id_unjoined,
                  rsym->inst_identity, rsym->cv_id);
      } else {
    ...
    for (c = fl->rest; c != NIL; c = c->rest) {
      destString += ' ';
      rhs_value_to_string(static_cast<char*>(c->first), destString, rereadable,
                          tok, w, pEmptyStringForNullIdentity);
    }
    destString += ')';
  }
}

void Output_Manager::action_to_string(agent* thisAgent, action* a,
                                      std::string& destString) {
...
void Output_Manager::action_list_to_string(agent* thisAgent,
                                           action* action_list,
                                           std::string& destString) {
  action* a = NIL;
...
void Output_Manager::identity_to_string(agent* thisAgent, uint64_t pID,
                                        const Identity* pIdentity,
                                        std::string& destString) {
  destString += '[';
...
void Output_Manager::pref_to_string(agent* thisAgent, preference* pref,
                                    std::string& destString) {
  if (m_print_actual_effective) {
    sprinta_sf(thisAgent, destString, "(%y ^%y %y) %c", pref->id, pref->attr,
               pref->value, preference_to_char(pref->type));
  ...
  if (m_print_identity_effective) {
    ...
    if (pref->inst_identities.id || pref->identities.id)
      identity_to_string(thisAgent, pref->inst_identities.id,
                         pref->identities.id, lID);
    else
      lID = pref->id->to_string(true);
    if (pref->inst_identities.attr || pref->identities.attr)
      identity_to_string(thisAgent, pref->inst_identities.attr,
                         pref->identities.attr, lAttr);
    else
      lAttr = pref->attr->to_string(true);
    if (pref->inst_identities.value || pref->identities.value)
      identity_to_string(thisAgent, pref->inst_identities.value,
                         pref->identities.value, lValue);
    else
      lValue = pref->value->to_string(true);

    sprinta_sf(thisAgent, destString, "%s(%s ^%s %s) %c",
               (m_print_actual_effective) ? ", " : "", lID.c_str(),
               lAttr.c_str(), lValue.c_str(), preference_to_char(pref->type));

    if (preference_is_binary(pref->type)) {
      if (pref->inst_identities.referent && pref->identities.referent)
        identity_to_string(thisAgent, pref->inst_identities.referent,
                           pref->identities.referent, lReferent);
      else
        sprinta_sf(thisAgent, destString, " %y", pref->referent);
    }
    if (pref->o_supported) {
      sprinta_sf(thisAgent, destString, " (o-support at level %d)",
                 static_cast<int64_t>(pref->level));
    } else {
      sprinta_sf(thisAgent, destString, " (i-support at level %d)",
                 static_cast<int64_t>(pref->level));
    }
  }
}

void Output_Manager::preflist_inst_to_string(agent* thisAgent,
                                             preference* top_pref,
                                             std::string& destString) {
...
void Output_Manager::preflist_result_to_string(agent* thisAgent,
                                               preference* top_pref,
                                               std::string& destString) {
  for (preference* pref = top_pref; pref != NIL;) {
    sprinta_sf(thisAgent, destString, "%s%p\n", m_pre_string, pref);
...
void Output_Manager::cond_prefs_to_string(agent* thisAgent, condition* top_cond,
                                          preference* top_pref,
                                          std::string& destString) {
  if (m_print_actual) {
    if (m_print_identity) {
...
void Output_Manager::cond_results_to_string(agent* thisAgent,
                                            condition* top_cond,
                                            preference* top_pref,
                                            std::string& destString) {
  if (m_print_actual) {
...
void Output_Manager::cond_actions_to_string(agent* thisAgent,
                                            condition* top_cond,
                                            action* top_action,
                                            std::string& destString) {
  if (m_print_actual) {
...
void Output_Manager::instantiation_to_string(agent* thisAgent,
                                             instantiation* inst,
                                             std::string& destString) {
  sprinta_sf(thisAgent, destString,
             "%sInstantiation (i %u) matched %y in state %y (level %d)\n",
             m_pre_string, inst->i_id, inst->prod_name, inst->match_goal,
             static_cast<int64_t>(inst->match_goal_level));
  cond_prefs_to_string(thisAgent, inst->top_of_instantiated_conditions,
                       inst->preferences_generated, destString);
}

void add_inst_of_type(agent* thisAgent, unsigned int productionType,
  ...
  for (production* prod = thisAgent->all_productions_of_type[productionType];
    ...
    for (instantiation* inst = prod->instantiations; inst != NIL;
         inst = inst->next) {
...
void Output_Manager::print_all_inst(TraceMode mode) {
  ...
  add_inst_of_type(m_defaultAgent, JUSTIFICATION_PRODUCTION_TYPE,
                   instantiation_list);
  add_inst_of_type(m_defaultAgent, USER_PRODUCTION_TYPE, instantiation_list);
  add_inst_of_type(m_defaultAgent, TEMPLATE_PRODUCTION_TYPE,
                   instantiation_list);
...
inline const char* YesNoStrForMSC(ms_change* lPtr) {
  if (lPtr)
    return "Yes";
  else
    return "No";
...
void Output_Manager::print_msc(TraceMode mode, ms_change* p_ms_change) {
  ...
  if (p_ms_change->goal) {
    print_sf(" for %y at level %d", p_ms_change->goal,
             static_cast<int64_t>(p_ms_change->level));
  }
  print_sf(" (pnode %s):\n", p_ms_change->p_node ? "exists" : "is NULL");
  if (p_ms_change->w) print_sf("wme = %w\n", p_ms_change->w);
  if (p_ms_change->tok && p_ms_change->tok->w)
    print_sf("token wme = %w\n", p_ms_change->tok->w);
  if (p_ms_change->inst)
    print_sf("inst = %y (i %u)\n", p_ms_change->inst->prod_name,
             p_ms_change->inst->i_id);
  print_sf("List links:  Normal = %s/%s, of_node = %s/%s, in_level %s/%s\n",
           YesNoStrForMSC(p_ms_change->prev), YesNoStrForMSC(p_ms_change->next),
           YesNoStrForMSC(p_ms_change->prev_of_node),
           YesNoStrForMSC(p_ms_change->next_of_node),
           YesNoStrForMSC(p_ms_change->prev_in_level),
           YesNoStrForMSC(p_ms_change->next_in_level));
}
...
void Output_Manager::print_varnames_node(TraceMode mode,
                                         node_varnames* var_names_node) {
  if (!is_trace_enabled(mode)) return;
...
void Output_Manager::debug_find_and_print_sym(char* find_string) {
  ...
  if (find_string) {
    ...
    soar::Lexer::determine_possible_symbol_types_for_string(
        find_string, static_cast<size_t>(strlen(find_string)), &possible_id,
        &possible_var, &possible_sc, &possible_ic, &possible_fc, &rereadable);

    if (possible_id) {
      newSym = m_defaultAgent->symbolManager->find_identifier(
          toupper(find_string[0]), strtol(&find_string[1], NULL, 10));
      if (newSym) {
        found = true;
      }
    }
  ...
  if (newSym) {
    debug_print_sf(DT_DEBUG,
                   ...
                   "  refcount = %d\n"
                   "  tc_num   = %d\n",
                   newSym, static_cast<int64_t>(newSym->symbol_type),
...
bool om_print_sym(agent* thisAgent, void* item, void* vMode) {
  ...
  Output_Manager::Get_OM().printa_sf(
      thisAgent, "%y (%u)\n", static_cast<symbol_struct*>(item),
...
void Output_Manager::print_identifiers(TraceMode mode) {
  ...
  print("--- Identifiers: ---\n");
  do_for_all_items_in_hash_table(
      m_defaultAgent, m_defaultAgent->symbolManager->identifier_hash_table,
      om_print_sym, &mode);
}
...
void Output_Manager::print_variables(TraceMode mode) {
  ...
  print("--- Variables: ---\n");
  do_for_all_items_in_hash_table(
      m_defaultAgent, m_defaultAgent->symbolManager->variable_hash_table,
      om_print_sym, &mode);
}
...
void Output_Manager::print_identity_sets(TraceMode mode) {
  ...
  /* Not sure how to do this yet.  There's no central list of them.  Can we
   * somehow iterate through memory pool? */
}
