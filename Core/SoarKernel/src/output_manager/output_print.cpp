  ...
  if (pSoarAgent) {
    ...
    if (pSoarAgent->output_settings->callback_mode) {
      soar_invoke_callbacks(
          pSoarAgent, PRINT_CALLBACK,
          static_cast<soar_call_data>(const_cast<char*>(msg)));
    }
    if (stdout_mode) {
      fputs(msg, stdout);
    }
...
void Output_Manager::sprinta_sf_given_va(agent* thisAgent,
...
void Output_Manager::sprinta_sf(agent* thisAgent, std::string& destString,
                                const char* format, ...) {
  va_list args;
  va_start(args, format);
  vsnprint_sf(thisAgent, destString, format, args);
...
/* Same as above but changes the destination pointer to point to the next
 * character to write to.  Used by other output manager functions to build up a
 * string incrementally */

size_t Output_Manager::sprinta_sf_cstr(agent* thisAgent, char* dest,
                                       size_t dest_size, const char* format,
                                       ...) {
...
size_t Output_Manager::sprint_sf_cstr(char* dest, size_t dest_size,
                                      const char* format, ...) {
  if (!dest_size) return 0;
  std::string buf;
...
void Output_Manager::debug_print_header(TraceMode mode,
                                        Print_Header_Type whichHeaders,
                                        const char* format, ...) {
  if (!is_trace_enabled(mode)) return;
  if (!m_defaultAgent) {
    std::cout << format;
...
void Output_Manager::buffer_start_fresh_line(agent* thisAgent,
                                             std::string& destString) {
  if (!thisAgent) {
    std::cout << std::endl;
    return;
  ...
  if (destString.empty()) {
    if ((global_printer_output_column != 1) ||
        (thisAgent->output_settings->printer_output_column != 1)) {
      destString.append("\n");
    }
  } else {
    if (destString.back() != '\n') {
...
void Output_Manager::vsnprint_sf(agent* thisAgent, std::string& destString,
                                 const char* format, va_list pargs) {
  Symbol* sym;
  test t, ct;
  char ch = 0;
  char* ch2 = 0;
    ...
    if (ch == '%') {
      ...
      if (i < m) {
        ...
        switch (ch) {
          ...
          case '=': {
            indent_amount = 0;
            next_position = (this->get_printer_output_column(thisAgent) +
                             destString.length());
            ...
            if (t) {
              if (t->type != CONJUNCTIVE_TEST) {
                if (t->inst_identity) {
                  ...
                  identity_to_string(thisAgent, t->inst_identity, t->identity,
                                     destString);
                } else {
                  test_to_string(t, destString);
                }
                ...
                for (cons* c = t->data.conjunct_list; c != NIL; c = c->rest) {
                  ...
                  if (ct->inst_identity) {
                    ...
                    identity_to_string(thisAgent, ct->inst_identity,
                                       ct->identity, destString);
                  } else {
          ...
          case 'n': {
            ...
            if (la) {
              this->rhs_value_to_string(funcall_list_to_rhs_value(la),
                                        destString);
          ...
          case '4': {
            cond_actions_to_string(thisAgent, va_arg(args, condition*),
                                   va_arg(args, action*), destString);
          } break;

          case '5': {
            cond_prefs_to_string(thisAgent, va_arg(args, condition*),
                                 va_arg(args, preference*), destString);
          } break;

          case '6': {
            cond_results_to_string(thisAgent, va_arg(args, condition*),
                                   va_arg(args, preference*), destString);
          } break;
