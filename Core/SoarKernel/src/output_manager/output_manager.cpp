...
bool is_DT_mode_enabled(TraceMode mode) {
  return Output_Manager::Get_OM().is_trace_enabled(mode);
}
...
void Output_Manager::set_output_params_global(bool pDebugEnabled) {
  if (pDebugEnabled &&
      !(Soar_Instance::Get_Soar_Instance().was_run_from_unit_test())) {
    m_print_actual = true;
...
void Output_Manager::copy_output_modes(
    trace_mode_info mode_info_src[num_trace_modes],
...
void Output_Manager::print_output_modes(
    trace_mode_info mode_info_to_print[num_trace_modes]) {
  for (int i = 0; i < num_trace_modes; i++) {
    print_sf("%s: %s\n", mode_info_to_print[i].prefix,
             (mode_info_to_print[i].enabled ? "enabled" : "disabled"));
  }
}
...
void Output_Manager::set_printer_output_column(agent* thisAgent,
                                               int pOutputColumn) {
  if (thisAgent) {
...
void Output_Manager::update_printer_columns(agent* pSoarAgent,
                                            const char* msg) {
  const char* ch;
