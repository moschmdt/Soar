...
class Output_Manager {
 ...
 private:
  ...
  void action_list_to_string(agent* thisAgent, action* action_list,
                             std::string& destString);
  void condition_to_string(agent* thisAgent, condition* cond,
                           std::string& destString);
  void condition_cons_to_string(agent* thisAgent, cons* c,
                                std::string& destString);
  void condition_list_to_string(agent* thisAgent, condition* top_cond,
                                std::string& destString);
  void cond_prefs_to_string(agent* thisAgent, condition* top_cond,
                            preference* top_pref, std::string& destString);
  void cond_actions_to_string(agent* thisAgent, condition* top_cond,
                              action* top_action, std::string& destString);
  void cond_results_to_string(agent* thisAgent, condition* top_cond,
                              preference* top_pref, std::string& destString);
  void instantiation_to_string(agent* thisAgent, instantiation* inst,
                               std::string& destString);
  void pref_to_string(agent* thisAgent, preference* pref,
                      std::string& destString);
  void preflist_inst_to_string(agent* thisAgent, preference* top_pref,
                               std::string& destString);
  void preflist_result_to_string(agent* thisAgent, preference* top_pref,
                                 std::string& destString);
  ...
  void vsnprint_sf(agent* thisAgent, std::string& destString,
 ...
 public:
  ...
  void copy_output_modes(trace_mode_info mode_info_src[num_trace_modes],
                         trace_mode_info mode_info_dest[num_trace_modes]);
  ...
  void sprinta_sf(agent* thisAgent, std::string& destString, const char* format,
                  ...);
  size_t sprinta_sf_cstr(agent* thisAgent, char* dest, size_t dest_size,
                         const char* format, ...);
  void sprinta_sf_given_va(agent* thisAgent, std::string& destString,
                           const char* format, va_list args);
  ...
  void debug_print_header(TraceMode mode, Print_Header_Type whichHeaders,
                          const char* format, ...);

  void test_to_string(test t, std::string& destString,
                      bool show_equality = false);
  void identity_to_string(agent* thisAgent, uint64_t pID,
                          const Identity* pIdentity, std::string& destString);
  const char* phase_to_string(top_level_phase pPhase);
  void rhs_value_to_string(rhs_value rv, std::string& destString,
  ...
  void display_reorder_error(agent* thisAgent,
                             ProdReorderFailureType pErrorType,
                             const char* pString1 = NULL,
                             const char* pString2 = NULL);
...
inline const char* capitalizeOnOff(bool isEnabled) {
  return isEnabled ? "[ ON | off ]" : "[ on | OFF ]";
}
inline const char* capitalizeYesNo(bool isEnabled) {
  return isEnabled ? "[ YES | no ]" : "[ yes | NO ]";
}
inline const char* TorF(bool isTrue) {
  if (isTrue)
    return "true";
  else
    return "false";
}
inline const char* PassorFail(bool isTrue) {
  if (isTrue)
    return "Pass";
  else
    return "Fail";
...
inline std::string concatJustified(const char* left_string,
                                   std::string right_string, int pWidth) {
