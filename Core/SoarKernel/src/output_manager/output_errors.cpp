...
void Output_Manager::display_reorder_error(agent* thisAgent,
  ...
  switch (pErrorType) {
    case reorder_failed_reordering_rhs: {
      printa_sf(
          thisAgent,
          "%eAttempted to add rule with ungrounded action(s).\n"
    ...
    case reorder_failed_unconnected_conditions: {
      printa_sf(
          thisAgent,
          "%eConditions on the LHS contain tests that are not connected \n"
          "to a goal: %s\n\n",
          pString2);
      break;
    }
    case reorder_failed_no_roots: {
      printa_sf(
          thisAgent,
          "Error: production %s has no positive conditions that reference a "
          "goal state.\n"
    ...
    case reorder_failed_negative_relational_test_bindings: {
      thisAgent->explanationBasedChunker->print_current_built_rule(
          "Attempted to add an invalid rule:");
      printa(thisAgent,
             "   Unbound relational test in negative condition of rule \n");
      break;
    }
    default: {
      thisAgent->explanationBasedChunker->print_current_built_rule(
          "Attempted to add an invalid rule:");
      printa(thisAgent,
             "\nUnspecified reordering/validation failure. That's weird. "
             "Should report.\n\n");
...
void Output_Manager::display_soar_feedback(agent* thisAgent,
                                           SoarCannedMessageType pMessageType,
                                           bool shouldPrint) {
  if (!shouldPrint) {
    return;
  }
  ...
  switch (pMessageType) {
    case ebc_error_max_chunks: {
      printa_sf(thisAgent,
                "%eWarning: Maximum number of chunks reached.  Skipping "
                "opportunity to learn new rule.\n");
      break;
    }
    case ebc_error_max_dupes: {
      printa_sf(
          thisAgent,
          "%eWarning: Rule has produced maximum number of duplicate chunks "
          "this decision cycle.  Skipping opportunity to learn new rule.\n");
      break;
    }
    ...
    case ebc_error_invalid_justification: {
      printa_sf(thisAgent,
                "%eWarning:  Chunking produced an invalid justification.  "
                "Ignoring.\n");
      break;
    }
    ...
    case ebc_error_no_conditions: {
      printa(
          thisAgent,
          "\nWarning: Soar has created a chunk or justification with no "
          "conditions.  Ignoring.\n\n"
          "         Any results created will lose support when the sub-state "
          "disappears.\n"
          "         To avoid this issue, the problem-solving in the sub-state "
          "must\n"
    ...
    default: {
      printa(
          thisAgent,
          "Warning: Unspecified soar error. That's weird.  Should report.\n");
    }
  }
}

void Output_Manager::display_ambiguous_command_error(
    agent* thisAgent, const std::list<std::string> matched_objects_str) {
  std::string last_p;
