...
void mark_context_slot_as_acceptable_preference_changed(agent *thisAgent,
  ...
  insert_at_head_of_dll(thisAgent->context_slots_with_changed_accept_prefs, dc,
                        next, prev);
}
...
void promote_id_and_tc(agent *thisAgent, Symbol *id,
  ...
  for (s = id->id->slots; s != NIL; s = s->next) {
    for (pref = s->all_preferences; pref != NIL;
         pref = pref->all_of_slot_next) {
...
void mark_id_and_tc_as_unknown_level(agent *thisAgent, Symbol *root) {
  ...
  while (!ids_to_walk.empty()) {
    ...
    if (id->id->could_be_a_link_from_below) {
      thisAgent->lowest_level_anything_could_fall_to =
...
inline bool level_update_needed(agent *thisAgent, Symbol *sym) {
  return ((sym->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
          (sym->tc_num != thisAgent->walk_tc_number));
}
...
void do_demotion(agent *thisAgent) {
  ...
  while (thisAgent->ids_with_unknown_level) {
    ...
    thisAgent->ids_with_unknown_level = thisAgent->ids_with_unknown_level->next;
    id = static_cast<symbol_struct *>(dc->item);
    thisAgent->memoryManager->free_with_pool(MP_dl_cons, dc);
...
void do_buffered_link_changes(agent *thisAgent) {
  ...
  if (!(thisAgent->promoted_ids || thisAgent->ids_with_unknown_level ||
...
#ifdef DETAILED_TIMING_STATS
  local_timer.stop();
  thisAgent->timers_ownership_cpu_time[thisAgent->current_phase].update(
      local_timer);
#endif
#endif
}
...
void build_rl_trace(agent *const &thisAgent, preference *const &candidates,
                    preference *const &selected) ///< bazald
{
  ...
  for (preference *cand = candidates; cand; cand = cand->next_candidate) {
    if (cand->inst && cand->inst->prod) {
      //       std::cerr << "rl-trace: " << cand->inst->prod_name->sc->name <<
      //       std::endl;

      //       for(preference *pref =
      //       cand->inst->match_goal->id->operator_slot->preferences[NUMERIC_INDIFFERENT_PREFERENCE_TYPE];
      //       pref; pref = pref->next) {
      //         production * const &prod2 = pref->inst->prod;
      //         if(cand->value == pref->value && prod2->rl_rule) {
      //           std::cerr << "rl-trace: +" << prod2->name->sc->name <<
      //           std::endl;
      //         }
      //       }
      ...
      const double probability =
          cand->rl_contribution
              ? exploration_probability_according_to_policy(
                    thisAgent, candidates->slot, candidates, cand)
              : nan("");

      //       std::cerr << "rl-trace: =" << probability << std::endl;

      RL_Trace *const rl_trace =
          static_cast<RL_Trace *>(candidates->slot->id->id->rl_trace);
...
preference *run_non_context_preference_semantics(agent *thisAgent, slot *s) {
  ...
  if (!s->all_preferences) {
    ...
    /* Mark every acceptable preference as a possible candidate then unmark any
     * that has a reject*/
    for (p = s->preferences[ACCEPTABLE_PREFERENCE_TYPE]; p != NIL;
         p = p->next) {
      p->value->decider_flag = CANDIDATE_DECIDER_FLAG;
    }
...
byte run_preference_semantics(agent *thisAgent, slot *s,
                              preference **result_candidates, bool consistency,
                              bool predict) {
  preference *p, *p2, *cand, *prev_cand;
  bool match_found, not_all_indifferent, some_numeric, add_OSK,
      some_not_worst = false;
  preference *candidates;
  Symbol *value;

  /* Set a flag to determine if a context-dependent preference set makes sense
   * in this context. We can ignore OSK prefs when:
   * - Run_preference_semantics is called for a consistency check (don't want
   * side effects)
   * - For non-context slots (only makes sense for operators)
   * - For context-slots at the top level (will never be backtraced through)
   * - when the learning system parameter is set off (note, this is independent
   * of whether learning is on) */
  add_OSK =
      (thisAgent->explanationBasedChunker->ebc_settings[SETTING_EBC_ADD_OSK] &&
       !consistency && (s->id->id->level > TOP_GOAL_LEVEL));
  ...
  if (!s->all_preferences) {
    *result_candidates = NIL;
    if (add_OSK)
      thisAgent->explanationBasedChunker->update_proposal_OSK(s, NULL);
    return NONE_IMPASSE_TYPE;
  }

  /* If this is the true decision slot and selection has been made, attempt
   * force selection */

  if (!consistency) {
    if (select_get_operator(thisAgent) != NULL) {
      preference *force_result = select_force(
          thisAgent, s->preferences[ACCEPTABLE_PREFERENCE_TYPE], !predict);
      ...
      if (force_result) {
        ...
        *result_candidates = force_result;
        ...
        if (add_OSK)
          thisAgent->explanationBasedChunker->update_proposal_OSK(s,
                                                                  force_result);
        return NONE_IMPASSE_TYPE;
      }
    }
  ...
  /* If debugging a context-slot, print all preferences that we're deciding
   * through */
  ...
  //              "\n-------------------------------\nRUNNING PREFERENCE
  //              SEMANTICS...\n-------------------------------\n");
  //        thisAgent->outputManager->printa_sf(thisAgent, "All Preferences for
  //        slot:");
  //
  ...
  //            {
  //                thisAgent->outputManager->printa_sf(thisAgent, "\n %ss:\n",
  //                preference_name(i)); for (p = s->preferences[i]; p; p =
  //                p->next)
  //                {
  //                    thisAgent->outputManager->printa_sf(thisAgent, "   ");
  ...
  if (s->preferences[REQUIRE_PREFERENCE_TYPE]) {
    ...
    *result_candidates = candidates;
    ...
    /* Check if we have more than one required item. If so, return constraint
     * failure. */
     ...
     * constraint failure. Note that this is the one difference between prohibit
     * and reject preferences. */
    ...
    //        if (thisAgent->trace_settings[TRACE_BACKTRACING_SYSPARAM])
    //        {
    //            thisAgent->outputManager->printa_sf(thisAgent, "--> Adding
    //            preference to OSK prefs: "); print_preference(thisAgent,
    //            candidates);
    //        }
    if (add_OSK)
      thisAgent->explanationBasedChunker->update_proposal_OSK(s, candidates);
    return NONE_IMPASSE_TYPE;
  }
  ...
  if (add_OSK) {
    if (s->preferences[PROHIBIT_PREFERENCE_TYPE] ||
        s->preferences[REJECT_PREFERENCE_TYPE]) {
      for (p = s->preferences[PROHIBIT_PREFERENCE_TYPE]; p != NIL;
           p = p->next) {
    ...
    while (cand) {
      if (cand->value->decider_flag == CONFLICTED_DECIDER_FLAG) {
        ...
        if (add_OSK) {
          ...
          for (p = s->preferences[BETTER_PREFERENCE_TYPE]; p != NIL;
               p = p->next) {
            if (p->value == cand->value) {
              thisAgent->explanationBasedChunker->add_to_OSK(s, p);
            }
          }
          for (p = s->preferences[WORSE_PREFERENCE_TYPE]; p != NIL;
               p = p->next) {
  ...
  if (s->preferences[BEST_PREFERENCE_TYPE]) {
    ...
    /* Reduce candidates list to only those with best preference flag and add
     * pref to OSK prefs */
    prev_cand = NIL;
    for (cand = candidates; cand != NIL; cand = cand->next_candidate)
      if (cand->value->decider_flag == BEST_DECIDER_FLAG) {
        if (add_OSK) {
          for (p = s->preferences[BEST_PREFERENCE_TYPE]; p != NIL;
               p = p->next) {
            if (p->value == cand->value) {
              thisAgent->explanationBasedChunker->add_to_OSK(s, p);
            }
  ...
  if (s->preferences[WORST_PREFERENCE_TYPE]) {
    ...
    /* Because we only want to add worst preferences to the OSK prefs if they
     * actually have an impact on the candidate list, we must first see if
     * there's at least one non-worst candidate. */
    ...
    for (cand = candidates; cand != NIL; cand = cand->next_candidate) {
      if (cand->value->decider_flag != WORST_DECIDER_FLAG) {
        ...
        if (add_OSK && some_not_worst) {
          ...
          for (p = s->preferences[WORST_PREFERENCE_TYPE]; p != NIL;
               p = p->next) {
            if (p->value == cand->value) {
              thisAgent->explanationBasedChunker->add_to_OSK(s, p);
            }
  ...
  for (p = s->preferences[NUMERIC_INDIFFERENT_PREFERENCE_TYPE]; p;
       p = p->next) {
  ...
  for (cand = candidates; cand != NIL; cand = cand->next_candidate) {
    ...
    /* If this candidate has a unary indifferent preference, skip. Numeric
     * indifferent prefs are considered to have an implicit unary indifferent
     * pref, which is why they are skipped too. */

    if (cand->value->decider_flag == UNARY_INDIFFERENT_DECIDER_FLAG) {
      continue;
    } else if (cand->value->decider_flag ==
  ...
  if (!not_all_indifferent) {
    if (!consistency) {
      (*result_candidates) =
          exploration_choose_according_to_policy(thisAgent, s, candidates);
      if (!predict && rl_enabled(thisAgent)) {
      ...
      if (add_OSK) {
        ...
        /* Add all indifferent preferences associated with the chosen candidate
         * to the OSK prefs.*/

        if (some_numeric) {

          /* Note that numeric indifferent preferences are never considered
           * duplicates, so we pass an extra argument to add_to_OSK so that it
           * does not check for duplicates.*/

          for (p = s->preferences[NUMERIC_INDIFFERENT_PREFERENCE_TYPE];
               p != NIL; p = p->next) {
          ...
          /* Now add any binary preferences with a candidate that does NOT have
           * a numeric preference. */

          for (p = s->preferences[BINARY_INDIFFERENT_PREFERENCE_TYPE]; p != NIL;
               p = p->next) {
            if ((p->value == (*result_candidates)->value) ||
                (p->referent == (*result_candidates)->value)) {
              if ((p->referent->decider_flag !=
                   UNARY_INDIFFERENT_CONSTANT_DECIDER_FLAG) ||
                  (p->value->decider_flag !=
                   UNARY_INDIFFERENT_CONSTANT_DECIDER_FLAG)) {
                thisAgent->explanationBasedChunker->add_to_OSK(s, p);
              }
            }
          ...
          /* This decision was non-numeric, so add all non-numeric preferences
           * associated with the chosen candidate to the OSK prefs.*/

          /* Note:  We've disabled unary indifferents because they were causing
           * problems in certain demo agents All of the OSK prefs that involve
           * uncertainty now seem weird. Will need to reconsider how we handle
           * them now  that we have a better handle for correctness issues and
           * are thinking about the possibility of probabilistic chunks.*/

          //                    for (p =
          //                    s->preferences[UNARY_INDIFFERENT_PREFERENCE_TYPE];
          //                    p != NIL; p = p->next)
          //                    {
          //                        if (p->value == (*result_candidates)->value)
          //                        {
          //                            thisAgent->explanationBasedChunker->add_to_OSK(s,
          //                            p);
          //                        }
          //                    }
          for (p = s->preferences[BINARY_INDIFFERENT_PREFERENCE_TYPE]; p != NIL;
    ...
    if (add_OSK)
      thisAgent->explanationBasedChunker->update_proposal_OSK(
          s, *result_candidates);
    return NONE_IMPASSE_TYPE;
  }
...
wme *add_impasse_wme(agent *thisAgent, Symbol *id, Symbol *attr, Symbol *value,
                     preference *p, bool isSingleton = false) {
  wme *w;
...
Symbol *create_new_impasse(agent *thisAgent, bool isa_goal, Symbol *object,
                           Symbol *attr, byte impasse_type,
                           goal_stack_level level) {
  Symbol *impasseID;

  impasseID = thisAgent->symbolManager->make_new_identifier(
      (isa_goal ? 'S' : 'I'), level, 0, false);
  post_link_addition(thisAgent, NIL, impasseID); /* add the special link */

  add_impasse_wme(
      thisAgent, impasseID, thisAgent->symbolManager->soarSymbols.type_symbol,
      isa_goal ? thisAgent->symbolManager->soarSymbols.state_symbol
               : thisAgent->symbolManager->soarSymbols.impasse_symbol,
      NIL, true);

  if (isa_goal) {
    thisAgent->memoryManager->allocate_with_pool(MP_rl_info,
                                                 &(impasseID->id->rl_info));
    thisAgent->memoryManager->allocate_with_pool(MP_smem_info,
                                                 &(impasseID->id->smem_info));
    thisAgent->memoryManager->allocate_with_pool(MP_epmem_info,
                                                 &(impasseID->id->epmem_info));

    wme *lSSWME =
        add_impasse_wme(thisAgent, impasseID,
                        thisAgent->symbolManager->soarSymbols.superstate_symbol,
                        object, NIL, true);
    if (thisAgent->explanationBasedChunker
            ->ebc_settings[SETTING_EBC_LEARNING_ON]) {
      if (level == (TOP_GOAL_LEVEL + 1)) {
        thisAgent->explanationBasedChunker->reset_identity_counter();
      }
      if (level > TOP_GOAL_LEVEL) {
        lSSWME->local_singleton_id_identity_set =
            thisAgent->explanationBasedChunker->get_floating_identity(
                impasseID);
        lSSWME->local_singleton_value_identity_set =
            thisAgent->explanationBasedChunker->get_floating_identity(
                impasseID);
      }
    }
    Symbol *lreward_header =
        thisAgent->symbolManager->make_new_identifier('R', level);
    impasseID->id->rl_info->rl_link_wme = soar_module::add_module_wme(
        thisAgent, impasseID,
        thisAgent->symbolManager->soarSymbols.rl_sym_reward_link,
        lreward_header, true);

    Symbol *lepmem_header =
        thisAgent->symbolManager->make_new_identifier('E', level);
    impasseID->id->epmem_info->epmem_link_wme = soar_module::add_module_wme(
        thisAgent, impasseID, thisAgent->symbolManager->soarSymbols.epmem_sym,
        lepmem_header, true);
    Symbol *lepmem_cmd_header =
        thisAgent->symbolManager->make_new_identifier('C', level);
    impasseID->id->epmem_info->cmd_wme = soar_module::add_module_wme(
        thisAgent, lepmem_header,
        thisAgent->symbolManager->soarSymbols.epmem_sym_cmd, lepmem_cmd_header);
    Symbol *lepmem_result_header =
        thisAgent->symbolManager->make_new_identifier('R', level);
    impasseID->id->epmem_info->result_wme = soar_module::add_module_wme(
        thisAgent, lepmem_header,
        thisAgent->symbolManager->soarSymbols.epmem_sym_result,
        lepmem_result_header);

    {
      int64_t my_time = static_cast<int64_t>(
          thisAgent->EpMem->epmem_stats->time->get_value());
      if (my_time == 0)
        my_time = 1;

      Symbol *my_time_sym =
          thisAgent->symbolManager->make_int_constant(my_time);
      impasseID->id->epmem_info->epmem_time_wme = soar_module::add_module_wme(
          thisAgent, lepmem_header,
    ...
    Symbol *lsmem_header =
        thisAgent->symbolManager->make_new_identifier('L', level);
    Symbol *lsmem_cmd_header =
        thisAgent->symbolManager->make_new_identifier('C', level);
    Symbol *lsmem_result_header =
        thisAgent->symbolManager->make_new_identifier('R', level);
    impasseID->id->smem_info->smem_link_wme = soar_module::add_module_wme(
        thisAgent, impasseID, thisAgent->symbolManager->soarSymbols.smem_sym,
        lsmem_header, true);
    impasseID->id->smem_info->cmd_wme = soar_module::add_module_wme(
        thisAgent, lsmem_header,
        thisAgent->symbolManager->soarSymbols.smem_sym_cmd, lsmem_cmd_header);
    impasseID->id->smem_info->result_wme = soar_module::add_module_wme(
        thisAgent, lsmem_header,
    ...
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.object_symbol, object,
                    NIL, true);
  }

  if (attr) {
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.attribute_symbol,
                    attr, NIL, true);
  }
  ...
  case CONSTRAINT_FAILURE_IMPASSE_TYPE:
    add_impasse_wme(
        thisAgent, impasseID,
        thisAgent->symbolManager->soarSymbols.impasse_symbol,
        thisAgent->symbolManager->soarSymbols.constraint_failure_symbol, NIL,
        true);
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.choices_symbol,
                    thisAgent->symbolManager->soarSymbols.none_symbol, NIL);
    break;
  case CONFLICT_IMPASSE_TYPE:
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.impasse_symbol,
                    thisAgent->symbolManager->soarSymbols.conflict_symbol, NIL,
                    true);
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.choices_symbol,
                    thisAgent->symbolManager->soarSymbols.multiple_symbol, NIL);
    break;
  case TIE_IMPASSE_TYPE:
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.impasse_symbol,
                    thisAgent->symbolManager->soarSymbols.tie_symbol, NIL,
                    true);
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.choices_symbol,
                    thisAgent->symbolManager->soarSymbols.multiple_symbol, NIL);
  ...
  case NO_CHANGE_IMPASSE_TYPE:
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.impasse_symbol,
                    thisAgent->symbolManager->soarSymbols.no_change_symbol, NIL,
                    true);
    add_impasse_wme(thisAgent, impasseID,
                    thisAgent->symbolManager->soarSymbols.choices_symbol,
                    thisAgent->symbolManager->soarSymbols.none_symbol, NIL);
...
void update_impasse_items(agent *thisAgent, Symbol *id, preference *items) {
  ...
  for (int it = regular; it <= numeric; it++) {
    if (it == regular) {
      ...
      loop_count_sym =
          thisAgent->symbolManager->soarSymbols.non_numeric_count_symbol;
    }
    ...
    if ((it == numeric) && items) {
      for (cand = items->slot->preferences[NUMERIC_INDIFFERENT_PREFERENCE_TYPE];
           cand; cand = cand->next) {
        cand->value->decider_flag = NOTHING_DECIDER_FLAG;
      }
    }
    ...
    // for each existing item: if supposed to be there, ALREADY EXISTING;
    // otherwise remove
    ...
    for (cand = items; cand != NIL; cand = cand->next_candidate) {
      ...
      if (id->id->isa_goal) {
        bt_pref = make_architectural_instantiation_for_impasse_item(thisAgent,
                                                                    id, cand);
      } else {
        bt_pref = cand;
      }
      ...
      if (cand->value->decider_flag == ALREADY_EXISTING_WME_DECIDER_FLAG) {
        if (id->id->isa_goal) {
          /* Remove fake preference for goal item */
          preference_remove_ref(thisAgent,
                                cand->value->decider_wme->preference);
        }
    ...
    if (item_count > 0) {
      count_sym = thisAgent->symbolManager->make_int_constant(
  ...
  for (cand = candidates; cand != NIL; cand = cand->next_candidate) {
    if (cand->value->decider_flag == ALREADY_EXISTING_WME_DECIDER_FLAG) {
      ...
      if ((s->wma_val_references != NIL) && wma_enabled(thisAgent)) {
        wma_sym_reference_map::iterator it =
            s->wma_val_references->find(w->value);
        if (it != s->wma_val_references->end()) {
          ...
          if (s->wma_val_references->empty()) {
            ...
            thisAgent->memoryManager->free_with_pool(MP_wma_slot_refs,
                                                     s->wma_val_references);
       ...
       *   If the instantiation that lead to the preference has not been already
       * explored OR If the instantiation is not an subgoal instantiation for a
       * chunk instantiation we are already exploring Then Add the instantiation
       * to a list of instantiations that will be explored in elaborate_gds().
       */

      for (pref = w->preference; pref != NIL; pref = pref->next) {
        if ((pref->inst->GDS_evaluated_already == false) &&
            (pref->inst->match_goal_level == current_highest_level)) {
      ...
      thisAgent->timers_gds_cpu_time[thisAgent->current_phase].update(
          thisAgent->timers_gds);
#endif
#endif
...
void remove_existing_context_and_descendents(agent *thisAgent, Symbol *goal) {
  ...
  soar_invoke_callbacks(thisAgent, POP_CONTEXT_STACK_CALLBACK,
                        static_cast<soar_call_data>(goal));

  if (goal != thisAgent->top_goal) {
    if (thisAgent->explanationBasedChunker
            ->ebc_settings[SETTING_EBC_LEARNING_ON] &&
        thisAgent->explanationMemory->is_any_enabled()) {
    ...
    if (rl_enabled(thisAgent)) {
      ...
      rl_perform_update(
          thisAgent, 0, true, goal,
          false); // this update only sees reward - there is no next state
      rl_clear_refs(goal);
    }
  }
  ...
  while (goal->id->preferences_from_goal) {
    ...
    remove_from_dll(goal->id->preferences_from_goal, p, all_of_goal_next,
                    all_of_goal_prev);
  ...
  if (goal->id->preferences_from_goal) {
    ...
    while (p) {
      preference *p_next =
          p->all_of_goal_prev; // RPM 10/06 we need to save this because p may
                               // be freed by the end of the loop
      remove_from_dll(goal->id->preferences_from_goal, p, all_of_goal_next,
                      all_of_goal_prev);
      p->on_goal_list = false;
      if (!remove_preference_from_clones_and_deallocate(thisAgent, p))
  ...
  update_impasse_items(thisAgent, goal,
                       NIL); /* causes items & fake pref's to go away */
  ...
  thisAgent->memoryManager->free_with_pool(
      MP_rl_et, goal->id->rl_info->eligibility_traces);
  goal->id->rl_info->prev_op_rl_rules->~production_list();
  thisAgent->memoryManager->free_with_pool(MP_rl_rule,
                                           goal->id->rl_info->prev_op_rl_rules);
  thisAgent->memoryManager->free_with_pool(MP_rl_info, goal->id->rl_info);

  goal->id->epmem_info->epmem_wmes->~preference_list();
  thisAgent->memoryManager->free_with_pool(MP_epmem_wmes,
                                           goal->id->epmem_info->epmem_wmes);
  thisAgent->memoryManager->free_with_pool(MP_epmem_info, goal->id->epmem_info);

  goal->id->smem_info->smem_wmes->~preference_list();
  thisAgent->memoryManager->free_with_pool(MP_smem_wmes,
                                           goal->id->smem_info->smem_wmes);
  ...
  free_list(thisAgent,
            extract_list_elements(
                ...
                cons_equality_fn, reinterpret_cast<void *>(goal)));
  free_list(
      thisAgent,
      extract_list_elements(
          thisAgent,
  ...
  if (goal->id->level <= thisAgent->substate_break_level) {
    ...
    thisAgent->reason_for_stopping =
        "Stopped due to substate (goal) retraction.";
  }
...
void create_new_context(agent *thisAgent, Symbol *attr_of_impasse,
                        byte impasse_type, bool isONC = false) {
  Symbol *id;

  if (thisAgent->bottom_goal) {
    /* Creating a sub-goal (or substate) */
    id = create_new_impasse(
        thisAgent, true, thisAgent->bottom_goal, attr_of_impasse, impasse_type,
    ...
    add_impasse_wme(thisAgent, id,
                    thisAgent->symbolManager->soarSymbols.quiescence_symbol,
    ...
    if (NO_CHANGE_IMPASSE_TYPE == impasse_type) {
      ...
      if (thisAgent->Decider->settings[DECIDER_MAX_GOAL_DEPTH] <
          thisAgent->bottom_goal->id->level) {
        // appear to be SNC'ing deep in goalstack, so interrupt and warn user
        // KJC note: we actually halt, because there is no interrupt function in
        // SoarKernel in the gSKI Agent code, if system_halted, MAX_GOAL_DEPTH
        // is checked and if exceeded then the interrupt is generated and
        // system_halted is set to false so the user can recover.
    ...
    id = create_new_impasse(thisAgent, true,
                            thisAgent->symbolManager->soarSymbols.nil_symbol,
                            NIL, NONE_IMPASSE_TYPE, TOP_GOAL_LEVEL);
  ...
  id->id->isa_goal = true;
  id->id->operator_slot = make_slot(
      thisAgent, id, thisAgent->symbolManager->soarSymbols.operator_symbol);
  ...
  thisAgent->memoryManager->allocate_with_pool(
      MP_rl_et, &(id->id->rl_info->eligibility_traces));
  id->id->rl_info->eligibility_traces =
      new (id->id->rl_info->eligibility_traces) rl_et_map();
  thisAgent->memoryManager->allocate_with_pool(
      MP_rl_rule, &(id->id->rl_info->prev_op_rl_rules));
  id->id->rl_info->prev_op_rl_rules =
      new (id->id->rl_info->prev_op_rl_rules) production_list();
  ...
  thisAgent->memoryManager->allocate_with_pool(
      MP_epmem_wmes, &(id->id->epmem_info->epmem_wmes));
  id->id->epmem_info->epmem_wmes =
      new (id->id->epmem_info->epmem_wmes) preference_list();
  ...
  thisAgent->memoryManager->allocate_with_pool(MP_smem_wmes,
                                               &(id->id->smem_info->smem_wmes));
  id->id->smem_info->smem_wmes =
      new (id->id->smem_info->smem_wmes) preference_list();

  /* --- invoke callback routine --- */
  soar_invoke_callbacks(thisAgent, CREATE_NEW_CONTEXT_CALLBACK,
...
byte type_of_existing_impasse(agent *thisAgent, Symbol *goal) {
  ...
  for (w = goal->id->lower_goal->id->impasse_wmes; w != NIL; w = w->next)
    if (w->attr == thisAgent->symbolManager->soarSymbols.impasse_symbol) {
      ...
      if (w->value ==
          thisAgent->symbolManager->soarSymbols.constraint_failure_symbol) {
        return CONSTRAINT_FAILURE_IMPASSE_TYPE;
      }
...
Symbol *attribute_of_existing_impasse(agent *thisAgent, Symbol *goal) {
    ...
    }
  {
    char msg[BUFFER_MSG_SIZE];
    strncpy(msg,
            "decide.c: Internal error: couldn't find attribute of existing "
            "impasse.\n",
            BUFFER_MSG_SIZE);
    msg[BUFFER_MSG_SIZE - 1] = 0; /* ensure null termination */
    abort_with_fatal_error(thisAgent, msg);
...
bool decide_context_slot(agent *thisAgent, Symbol *goal, slot *s,
  ...
  if (!context_slot_is_decidable(s)) {
    ...
    if (predict) {
      ...
      default:
        if (!candidates ||
            (candidates->value->symbol_type != IDENTIFIER_SYMBOL_TYPE)) {
          predict_set(thisAgent, "none");
        } else {
          std::string temp = "";
    ...
    remove_wmes_for_context_slot(thisAgent, s); /* must remove old wme before
  ...
  /* --- record operator selection explanation (if tracking is active) --- */
  {
    preference *winner_pref =
        (impasse_type == NONE_IMPASSE_TYPE) ? candidates : NIL;
    thisAgent->operatorExplainer->record_decision(thisAgent, s, goal,
                                                  winner_pref, impasse_type);
  }
  ...
  if (impasse_type == NONE_IMPASSE_TYPE) {
    ...
    if (goal->id->lower_goal) {
      if (thisAgent->trace_settings[TRACE_CONSISTENCY_CHANGES_SYSPARAM]) {
        thisAgent->outputManager->printa_sf(
            thisAgent, "Removing state %y because of a decision.\n",
            goal->id->lower_goal);
      }
  ...
  if ((impasse_type == type_of_existing_impasse(thisAgent, goal)) &&
      (attribute_of_impasse ==
       attribute_of_existing_impasse(thisAgent, goal))) {
    update_impasse_items(thisAgent, goal->id->lower_goal, candidates);
    return false;
  }

  /* --- no impasse already existed, or an impasse of the wrong type already
   * existed --- */
  for (temp = candidates; temp; temp = temp->next_candidate) {
    preference_add_ref(temp);
  }
  ...
  if (goal->id->lower_goal) {
    if (thisAgent->trace_settings[TRACE_CONSISTENCY_CHANGES_SYSPARAM]) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "Removing state %y because it's the wrong type of impasse.\n",
          goal->id->lower_goal);
    }
  ...
  if (!thisAgent->Decider->settings[DECIDER_WAIT_SNC] ||
      !(impasse_type == NO_CHANGE_IMPASSE_TYPE) ||
      !(attribute_of_impasse ==
        thisAgent->symbolManager->soarSymbols.state_symbol)) {
...
void init_decider(agent *thisAgent) {
  ...
  thisAgent->memoryManager->init_memory_pool(MP_preference, sizeof(preference),
                                             "preference");
}
...
void assert_new_preferences(agent *thisAgent, preference_list &bufdeallo) {
  ...
  }
  {
    ...
    for (inst = thisAgent->newly_created_instantiations; inst != NIL;
         inst = next_inst) {
      next_inst = inst->next;
  ...
  if (!thisAgent->Decider->settings[DECIDER_KEEP_TOP_OPREFS] &&
      !thisAgent->newly_deleted_instantiations.empty()) {
    instantiation *lDeleteInst;
    for (auto iter = thisAgent->newly_deleted_instantiations.begin();
         iter != thisAgent->newly_deleted_instantiations.end(); iter++) {
      lDeleteInst = *iter;
...
/**
 * New waterfall model:
 * Returns true if the function create_instantiation should run for this
 * production. Used to delay firing of matches in the inner preference loop.
 */
bool shouldCreateInstantiation(agent *thisAgent, production *prod,
  ...
  // Scan RHS identifiers for their levels, don't fire those at or higher than
  // the change level
  action *a = NIL;
  for (a = prod->action_list; a != NIL; a = a->next) {
    ...
    if (sym->id->level <= thisAgent->change_level) {
      if (thisAgent->trace_settings[TRACE_WATERFALL_SYSPARAM]) {
        thisAgent->outputManager->printa_sf(
            thisAgent, "*** Waterfall: aborting firing because (%y * *)", sym);
        thisAgent->outputManager->printa_sf(
            thisAgent,
            " level %d is on or higher (lower int) than change level %d\n",
            static_cast<int64_t>(sym->id->level),
            static_cast<int64_t>(thisAgent->change_level));
      }
      return false;
    }
  }
...
void do_preference_phase(agent *thisAgent) {
  ...
  if (thisAgent->trace_settings[TRACE_PHASES_SYSPARAM]) {
    if (thisAgent->current_phase ==
        APPLY_PHASE) /* it's always IE for PROPOSE */
    {
      ...
      xml_att_val(
          thisAgent, kPhase_LevelNum,
          levelString
              .c_str()); // SBW 8/4/2008: active_level for XML output mode
  ...
  // Temporary list to buffer deallocation of some preferences until the inner
  // elaboration loop is over.
  ...
  for (;;) {
    ...
    if (thisAgent->current_phase == APPLY_PHASE) {
      thisAgent->active_goal = highest_active_goal_apply(
          thisAgent, thisAgent->active_goal->id->lower_goal, true);
    } else {
      thisAgent->active_goal = highest_active_goal_propose(
          thisAgent, thisAgent->active_goal->id->lower_goal, true);
    }
...
void do_working_memory_phase(agent *thisAgent) {
  ...
  if (thisAgent->trace_settings[TRACE_PHASES_SYSPARAM]) {
        ...
        APPLY_PHASE) /* it's always IE for PROPOSE */
    {
      ...
      case PE_PRODS:
        thisAgent->outputManager->printa_sf(
            thisAgent, "\t--- Change Working Memory (PE) ---\n", 0);
        xml_att_val(thisAgent, kPhase_FiringType, kPhaseFiringType_PE);
        break;
      case IE_PRODS:
        thisAgent->outputManager->printa_sf(
            thisAgent, "\t--- Change Working Memory (IE) ---\n", 0);
        xml_att_val(thisAgent, kPhase_FiringType, kPhaseFiringType_IE);
        break;
      }
      xml_end_tag(thisAgent, kTagSubphase);
...
void clear_goal_stack(agent *thisAgent) {
  ...
  do_input_cycle(
      thisAgent); /* tell input functions that the top state is gone */
  do_output_cycle(
      thisAgent); /* tell output functions that output commands are gone */
}

void print_lowest_slot_in_context_stack(agent *thisAgent) {

  if (thisAgent->bottom_goal->id->operator_slot->wmes) {
    print_stack_trace(thisAgent,
                      thisAgent->bottom_goal->id->operator_slot->wmes->value,
...
void uniquely_add_to_head_of_dll(agent *thisAgent, instantiation *inst) {
  ...
  for (curr_pi = thisAgent->parent_list_head; curr_pi;
...
void add_wme_to_gds(agent *thisAgent, goal_dependency_set *gds,
                    wme *wme_to_add) {
  ...
  if (thisAgent->trace_settings[TRACE_GDS_WMES_SYSPARAM]) {
    ...
    thisAgent->outputManager->sprinta_sf_cstr(
        thisAgent, msgbuf, 255,
        "Adding to GDS for %y: ", wme_to_add->gds->goal);
...
void elaborate_gds(agent *thisAgent) {
  ...
  for (curr_pi = thisAgent->parent_list_head; curr_pi; curr_pi = temp_pi) {
    ...
    for (cond = inst->top_of_instantiated_conditions; cond != NIL;
      ...
      /* This following was changed to better handle WMEs that change levels
       * since their instantiations were created.  If there's a preference for
       * the wme at the instantiation level, I think we want the GDS code to
       * backtrace through that so that it can pick up any other instantiations
       * that may have other wme's to add to the GDS.  If it doesn't exist, then
       * it's a superstate WME, and we should process it normally. */
      preference *clone_for_this_level;

      if (pref_for_this_wme &&
          (pref_for_this_wme->level != inst->match_goal_level)) {
        clone_for_this_level =
            find_clone_for_level(pref_for_this_wme, inst->match_goal_level);
        if (clone_for_this_level)
          pref_for_this_wme = clone_for_this_level;
      }
      /* WME is in a supergoal or is architecturally created
       *
       * Note:  architectural instantiations for retrievals and impasse items do
       * have prefs, and get handled in clause for "wme is local and
       * i-supported")         */

      if ((pref_for_this_wme == NIL) ||
          (wme_goal_level < inst->match_goal_level)) {
        if (wme_matching_this_cond->gds != NIL) {
          /* Then we want to check and see if the old GDS value should be
           * changed */
          if (wme_matching_this_cond->gds->goal == NIL) {
            /* The goal is NIL: meaning that the goal for the GDS is no longer
             * around */
            fast_remove_from_dll(wme_matching_this_cond->gds->wmes_in_gds,
                                 wme_matching_this_cond, wme, gds_next,
                                 gds_prev);

            /* Must check for GDS removal every time we take a WME off the GDS
             * wme list */
            if (!wme_matching_this_cond->gds->wmes_in_gds) {
              ...
              thisAgent->memoryManager->free_with_pool(
                  MP_gds, wme_matching_this_cond->gds);
            }

            add_wme_to_gds(thisAgent, inst->match_goal->id->gds,
                           wme_matching_this_cond);
          } else if (wme_matching_this_cond->gds->goal->id->level >
                     inst->match_goal_level) {
            /* This WME currently belongs to the GDS of a goal below the current
             * one */
          ...
          /* WME should be in the GDS of the current goal if the WME's GDS does
           * not already exist. (i.e., if NIL GDS) */
          add_wme_to_gds(thisAgent, inst->match_goal->id->gds,
                         wme_matching_this_cond);

          if (wme_matching_this_cond->gds->wmes_in_gds->gds_prev) {
            thisAgent->outputManager->printa_sf(
                thisAgent, "\nDEBUG DEBUG : The new header should never have a "
                           "prev value.\n");
          }
        } /* end else clause for "if wme_matching_this_cond->gds != NIL" */
      } /* end "wme in supergoal or arch-supported" */
      else {
        /* WME must be local. If wme's pref is o-supported, then just ignore it
         * and move to next condition */
        if (pref_for_this_wme->o_supported == true) {
          ...
          else /* (inst->match_goal_level != 1) */
          {
            ...
            if (s == NIL) {
               ...
               * differently-named pointer.  it probably should be a subroutine
               */
              {
                wme *fake_inst_wme_cond;

                fake_inst_wme_cond =
                    pref_for_this_wme->inst->top_of_instantiated_conditions->bt
                        .wme_;
                if (fake_inst_wme_cond->gds != NIL) {
                  /* Then we want to check and see if the old GDS value should
                   * be changed */
                  if (fake_inst_wme_cond->gds->goal == NIL) {
                    /* The goal is NIL: meaning that the goal for the GDS is no
                     * longer around */

                    fast_remove_from_dll(fake_inst_wme_cond->gds->wmes_in_gds,
                                         fake_inst_wme_cond, wme, gds_next,
                                         gds_prev);

                    /* We have to check for GDS removal anytime we take a WME
                     * off the GDS wme list, not just when a WME is removed from
                     * memory. */
                   ...
                   * goal if the WME's GDS does not already exist. (i.e., if NIL
                   * GDS) */
                  ...
                  if (fake_inst_wme_cond->gds->wmes_in_gds->gds_prev) {
                    thisAgent->outputManager->printa_sf(
                        thisAgent, "\nDEBUG DEBUG : The new header should "
                                   "never have a prev value.\n");
                  }
                }
              } /* matches { wme *fake_inst_wme_cond  */
            } else {
              /* this was the original "local & i-supported" action */
              for (pref = s->preferences[ACCEPTABLE_PREFERENCE_TYPE]; pref;
                   pref = pref->next) {
                /* Check that the value with acceptable pref for the slot is the
                   same as the value for the wme in the condition, since
                       operators can have acceptable preferences for values
                   other than the WME value.  We dont want to backtrack thru
                   acceptable prefs for other operators */

                if (pref->value == wme_matching_this_cond->value) {
                  /* REW BUG: may have to go over all insts regardless of this
                   * visited_already flag... */
                  ...
                  if (pref->inst->GDS_evaluated_already == false) {
                    ...
                    if (pref->level <= inst->match_goal_level) {
                      ...
                      /* This was added to follow up on above comment. This
                       * looks for the pref at the current level. If EBC fails
                       * to learn a chunk or justification, it's possible that
                       * it cannot find a pref for this level.*/
                      preference *clone_for_this_pref =
                          find_clone_for_level(pref, inst->match_goal_level);
...
void gds_invalid_so_remove_goal(agent *thisAgent, wme *w) {
  if (thisAgent->trace_settings[TRACE_GDS_STATE_REMOVAL_SYSPARAM]) {
    ...
    thisAgent->outputManager->sprinta_sf_cstr(
        thisAgent, msgbuf, 255,
        "Removing state %y because element in GDS changed. WME: ",
        w->gds->goal);
    thisAgent->outputManager->printa(thisAgent, msgbuf);
  ...
  if (thisAgent->highest_goal_whose_context_changed) {
    ...
    /* If nothing has yet changed (highest_ ... = NIL) then set the goal
     * automatically */
    thisAgent->highest_goal_whose_context_changed =
        w->gds->goal->id->higher_goal;
    ...
    // bug 1011
    for (slot *s = thisAgent->highest_goal_whose_context_changed->id->slots;
         s != 0; s = s->next) {
      if (s->isa_context_slot && !s->changed) {
        s->changed = reinterpret_cast<dl_cons *>(
            1); // use non-zero value to indicate change, see definition of
                // slot::changed
      }
    }
  }
