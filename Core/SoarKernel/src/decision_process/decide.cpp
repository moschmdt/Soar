...
Symbol* create_new_impasse(agent* thisAgent, bool isa_goal, Symbol* object,
  ...
  switch (impasse_type) {
    ...
    case CONFLICT_IMPASSE_TYPE:
      add_impasse_wme(thisAgent, impasseID,
                      thisAgent->symbolManager->soarSymbols.impasse_symbol,
                      thisAgent->symbolManager->soarSymbols.conflict_symbol,
                      NIL, true);
      add_impasse_wme(thisAgent, impasseID,
                      thisAgent->symbolManager->soarSymbols.choices_symbol,
                      thisAgent->symbolManager->soarSymbols.multiple_symbol,
                      NIL);
      break;
    ...
    case NO_CHANGE_IMPASSE_TYPE:
      add_impasse_wme(thisAgent, impasseID,
                      thisAgent->symbolManager->soarSymbols.impasse_symbol,
                      thisAgent->symbolManager->soarSymbols.no_change_symbol,
                      NIL, true);
      add_impasse_wme(thisAgent, impasseID,
                      thisAgent->symbolManager->soarSymbols.choices_symbol,
                      thisAgent->symbolManager->soarSymbols.none_symbol, NIL);
...
#ifndef NO_SVS
  if (thisAgent->svs->is_enabled()) thisAgent->svs->state_creation_callback(id);
#endif
}
      ...
      switch (thisAgent->FIRING_TYPE) {
        case PE_PRODS:
          thisAgent->outputManager->printa_sf(
              ...
              static_cast<int64_t>(
                  thisAgent
                      ->active_level));  // SBW 8/4/2008: added active_level
      ...
      if ((pref_for_this_wme == NIL) ||
        ...
        if (wme_matching_this_cond->gds != NIL) {
          ...
          if (wme_matching_this_cond->gds->wmes_in_gds->gds_prev) {
            thisAgent->outputManager->printa_sf(
                thisAgent,
                "\nDEBUG DEBUG : The new header should never have a prev "
                "value.\n");
          }
