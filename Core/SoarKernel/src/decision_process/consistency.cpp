...
void remove_operator_if_necessary(agent* thisAgent, slot* s, wme* w) {
  ...
  {
    if (s->wmes->value ==
        w->value) /* The WME in the context slot is WME whose pref changed */
    {
      if (thisAgent->trace_settings[TRACE_CONSISTENCY_CHANGES_SYSPARAM]) {
        thisAgent->outputManager->printa_sf(
            thisAgent,
            "\n        REMOVING: Operator from context slot (proposal no "
            "longer matches): ");
      ...
      if (s->id->id->lower_goal) {
        if (thisAgent->trace_settings[TRACE_CONSISTENCY_CHANGES_SYSPARAM]) {
          thisAgent->outputManager->printa_sf(
              thisAgent, "Removing state %y because of an operator removal.\n",
              s->id->id->lower_goal);
        }

        remove_existing_context_and_descendents(thisAgent,
                                                s->id->id->lower_goal);
      }
    }
  }
...
bool decision_consistent_with_current_preferences(agent* thisAgent,
  ...
  if (s->isa_context_slot) {
    ...
    thisAgent->outputManager->printa_sf(
        thisAgent, "    Slot Identifier [%y] and attribute [%y]\n", s->id,
        s->attr);
  }
...
#ifdef DEBUG_CONSISTENCY_CHECK
    printf(
        "    Impasse types are different: Returning false, preferences are not "
        "consistent with prior decision.\n");
#endif
    return false;
  }

  /* in these cases, we know that the new impasse and the old impasse *TYPES*
     are the same.  We just want to check and make the actual impasses/decisions
     are the same. */
  switch (new_impasse_type) {
    case NONE_IMPASSE_TYPE:
      /* There are four cases to consider when NONE_IMPASSE_TYPE is returned: */
      /* 1.  Previous operator and operator returned by run_pref_sem are the
         same. In this case, return true (decision remains consistent) */

      /* This next if is meant to test that there actually is something in the
         slot but I'm nut quite certain that it will not always be true? */
      if (operator_in_slot) {
#ifdef DEBUG_CONSISTENCY_CHECK
        ...
        /* Because of indifferent preferences, we need to compare all possible
        candidates with the current decision */
        for (cand = candidates; cand; cand = cand->next_candidate) {
          if (current_operator->value == cand->value) {
#ifdef DEBUG_CONSISTENCY_CHECK
            thisAgent->outputManager->printa_sf(
                thisAgent,
                "       Operator slot ID [%y] and candidate ID [%y] are the "
                "same.\n",
        ...
        /* 2.  A different operator is indicated for the slot than the one that
           is currently installed.  In this case, we return false (the decision
           is not consistent with the preferences). */

        /* Now we know that the decision is inconsistent */
        return false;

        /* 3.  A single operator is suggested when an impasse existed
           previously. In this case, return false so that the impasse can be
           removed. */
      ...
      } else /* There is no operator in the slot */
      {
        if (goal->id->lower_goal) /* But there is an impasse */
        {
          if (goal->id->lower_goal->id->isa_impasse) {
            printf("This goal is an impasse\n");
          }
          printf(
              "      No Impasse Needed but Impasse exists: remove impasse "
              "now\n");
          printf(
...
void remove_current_decision(agent* thisAgent, slot* s) {
  if (!s->wmes) {
    if (thisAgent->trace_settings[TRACE_CONSISTENCY_CHANGES_SYSPARAM]) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "\n       REMOVING CONTEXT SLOT: Slot Identifier [%y] and attribute "
          "[%y]\n",
          s->id, s->attr);
    }
  }
  ...
  if (s->id) {
    if (thisAgent->trace_settings[TRACE_CONSISTENCY_CHANGES_SYSPARAM]) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "\n          Decision for goal [%y] is inconsistent.  Replacing it "
          "with....\n",
          s->id);
    }
  }
...
bool check_context_slot_decisions(agent* thisAgent, goal_stack_level level) {
  ...
  if (thisAgent->highest_goal_whose_context_changed) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "    Highest goal with changed context: [%y]\n",
        thisAgent->highest_goal_whose_context_changed);
  }
#endif

  /* REW: begin 05.05.97 */
  /* Check only those goals where preferences have changes that are at or above
     the level of the consistency check */
  for (goal = thisAgent->highest_goal_whose_context_changed;
       goal && goal->id->level <= level; goal = goal->id->lower_goal) {
    /* REW: end   05.05.97 */
#ifdef DEBUG_CONSISTENCY_CHECK
    thisAgent->outputManager->printa_sf(
        thisAgent,
    ...
    if ((goal->id->lower_goal) || (s->wmes)) {
      /* If we are not at the bottom goal or if there is an operator in the
          bottom goal's operator slot */
#ifdef DEBUG_CONSISTENCY_CHECK
      printf(
          "      This is a goal that either has subgoals or, if the bottom "
          "goal, has an operator in the slot\n");
#endif
      if (s->changed) /* Only need to check a goal if its prefs have changed */
      {
        ...
        if (!decision_consistent_with_current_preferences(thisAgent, goal, s)) {
          ...
          thisAgent->outputManager->printa_sf(
              thisAgent,
              "   The current preferences indicate that the decision at [%y] "
              "needs to be removed.\n",
              goal);
#endif
          if (thisAgent->trace_settings[TRACE_CONSISTENCY_CHANGES_SYSPARAM]) {
            thisAgent->outputManager->printa_sf(
                thisAgent,
                "Removing substates of %y because the operator last selected "
                "in %y is not\n",
                goal, goal);
            thisAgent->outputManager->printa_sf(
                thisAgent, "consistent with the current preferences.\n");
          }
          /* This doesn't seem like it should be necessary but evidently it is:
           * see 2.008 */
...
bool i_activity_at_goal(Symbol* goal) {
  /* thisAgent->outputManager->printa_sf("\nLooking for I-activity at goal:
   * %y\n", goal); */
...
bool minor_quiescence_at_goal(agent* thisAgent, Symbol* goal) {
  if ((thisAgent->FIRING_TYPE == IE_PRODS) && (!i_activity_at_goal(goal)))
...
void print_assertion(agent* thisAgent, ms_change* msc) {
  if (msc->p_node) {
    thisAgent->outputManager->printa_sf(thisAgent, "%eAssertion: %y",
                                        msc->p_node->b.p.prod->name);
  } else {
    thisAgent->outputManager->printa_sf(thisAgent,
                                        "%eAssertion exists but has no p_node");
  }
}

void print_retraction(agent* thisAgent, ms_change* msc) {
  if (msc->p_node) {
    thisAgent->outputManager->printa_sf(thisAgent, "%eRetraction: %y",
                                        msc->p_node->b.p.prod->name);
  } else {
    thisAgent->outputManager->printa_sf(
...
Symbol* highest_active_goal_propose(agent* thisAgent, Symbol* start_goal,
                                    bool noneOk) {
  Symbol* goal;
  ...
  thisAgent->outputManager->printa_sf(
      thisAgent,
      "WARNING: Returning NIL active goal because only NIL goal retractions "
      "are active.");
  xml_generate_warning(thisAgent,
                       "WARNING: Returning NIL active goal because only NIL "
                       "goal retractions are active.");
#endif
  ...
  if (!noneOk) {
    ...
    strncpy(msg,
            "\n consistency.c: Error: Unable to find an active goal when not "
            "at quiescence.\n",
            BUFFER_MSG_SIZE);
...
Symbol* highest_active_goal_apply(agent* thisAgent, Symbol* start_goal,
  ...
  if (!noneOk) {
    char msg[BUFFER_MSG_SIZE];
    strncpy(msg,
            "\nconsistency.c: Error: Unable to find an active goal when not at "
            "quiescence.\n",
            BUFFER_MSG_SIZE);
    msg[BUFFER_MSG_SIZE - 1] = 0; /* ensure null termination */
    abort_with_fatal_error(thisAgent, msg);
...
bool goal_stack_consistent_through_goal(agent* thisAgent, Symbol* goal) {
  ...
  if (thisAgent->highest_goal_whose_context_changed) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "current_agent(highest_goal_whose_context_changed) = [%y]\n",
        thisAgent->highest_goal_whose_context_changed);
  } else {
...
void determine_highest_active_production_level_in_stack_apply(
  ...
  /* KJC 04/05 - moved phase printing to init_soar: do_one_top_level_phase, case
   * APPLY */
  ...
  /* Save the old goal and level (must save level explicitly in case goal is
   * NIL) */
  ...
  thisAgent->active_goal =
      highest_active_goal_apply(thisAgent, thisAgent->top_goal, false);
...
#ifdef DEBUG_DETERMINE_LEVEL_PHASE
  printf("\nHighest level of activity is....%d",
         static_cast<int64_t>(thisAgent->active_level));
  printf("\n   Previous level of activity is....%d",
         static_cast<int64_t>(thisAgent->previous_active_level));
#endif
  ...
  switch (level_change_type) {
    ...
    case NEW_DECISION:
      ...
      thisAgent->FIRING_TYPE =
          active_production_type_at_goal(thisAgent->active_goal);
    ...
    case LOWER_LEVEL:
      ...
      if (minor_quiescence_at_goal(thisAgent,
                                   thisAgent->previous_active_goal)) {
#ifdef DEBUG_DETERMINE_LEVEL_PHASE
        printf("\nMinor quiescence at level %d",
               static_cast<int64_t>(thisAgent->previous_active_level));
#endif
        if (!goal_stack_consistent_through_goal(
                thisAgent, thisAgent->previous_active_goal)) {
          thisAgent->current_phase = OUTPUT_PHASE;
          break;
        }
      }
      ...
      if (goal->id->saved_firing_type != NO_SAVED_PRODS) {
        ...
        thisAgent->outputManager->printa_sf(
            thisAgent,
            "\nRestoring production type from previous processing at this "
            "level");
#endif
        thisAgent->FIRING_TYPE = goal->id->saved_firing_type;
        // KJC 04.05 commented the next line after reworking the phases in
        // init_soar.cpp thisAgent->current_phase = DETERMINE_LEVEL_PHASE;
        // Reluctant to make this a recursive call, but somehow we need to go
        // thru and determine which level we should start with now that we've
        // returned from a lower level (solved a subgoal or changed the
        // conditions). We could return a flag instead and test it everytime
        // thru loop in APPLY.
    ...
    case SAME_LEVEL:
      ...
      if (minor_quiescence_at_goal(thisAgent, thisAgent->active_goal)) {
        ...
        printf("\nMinor quiescence at level %d",
               static_cast<int64_t>(thisAgent->active_level));
#endif
        if (!goal_stack_consistent_through_goal(thisAgent,
                                                thisAgent->active_goal)) {
          thisAgent->current_phase = OUTPUT_PHASE;
          break;
        }
...
void determine_highest_active_production_level_in_stack_propose(
  ...
  /* Determine the new highest level of activity */
  thisAgent->active_goal =
      highest_active_goal_propose(thisAgent, thisAgent->top_goal, false);
  if (thisAgent->active_goal) {
    thisAgent->active_level = thisAgent->active_goal->id->level;
  } else {
    thisAgent->active_level = 0; /* Necessary for get_next_retraction */
