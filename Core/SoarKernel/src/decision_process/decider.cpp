...
void SoarDecider::get_enabled_module_strings(std::string &enabledStr,
                                             std::string &disabledStr) {
  bool ebcEnabled = (thisAgent->explanationBasedChunker->ebc_params
                         ->chunk_in_states->get_value() != ebc_never);
  ...
  if (ebcEnabled) {
    if (notFirstEnabledItem)
      enabledStr += ", ";
    else
      notFirstEnabledItem = true;
    enabledStr += "EBC";
  } else {
    if (notFirstDisabledItem)
      disabledStr += ", ";
    else
      notFirstDisabledItem = true;
    disabledStr += "EBC";
  }
...
void SoarDecider::clean_up_for_agent_deletion() { delete params; }

void SoarDecider::before_run() {
  if (thisAgent->d_cycle_count > 1)
    last_dc = thisAgent->d_cycle_count;
  else
    last_dc = 0;
...
void SoarDecider::get_run_result_string(std::string &runResultStr) {
  ...
  runResultStr += (dc_delta > 1 ? " decision cycles executed. "
                                : " decision cycle executed. ");
  if (fc_delta) {
    runResultStr.append(std::to_string(fc_delta));
    runResultStr += (fc_delta > 1 ? " rules fired. " : " rule fired. ");
  } else
    runResultStr += "No rules fired. ";
  if (cl_delta) {
    runResultStr.append(std::to_string(cl_delta));
    runResultStr +=
        (cl_delta > 1 ? " new rules learned." : " new rule learned.");
  }
  last_dc = now_dc;
  last_fc = now_fc;
  last_cl = now_cl;
