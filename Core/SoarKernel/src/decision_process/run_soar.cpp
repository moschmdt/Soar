...
void abort_with_fatal_error(agent* thisAgent, const char* msg) {
  const char* warning =
      ...
      "inspection, but may be corrupt.\nYou will have to restart Soar to run "
...
void set_trace_setting(agent* thisAgent, int param_number, int64_t new_value) {
  if ((param_number < 0) || (param_number > HIGHEST_SYSPARAM_NUMBER)) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "Internal error: tried to set bad trace param #: %d\n",
        static_cast<int64_t>(param_number));
    return;
  }
  thisAgent->trace_settings[param_number] = new_value;

  soar_invoke_callbacks(thisAgent, SYSTEM_PARAMETER_CHANGED_CALLBACK,
...
void remove_pwatch(agent* thisAgent, production* prod) {
  ...
  free_list(thisAgent, extract_list_elements(
                           thisAgent, &thisAgent->productions_being_traced,
...
void reset_production_firing_counts(agent* thisAgent) {
  ...
  for (t = 0; t < NUM_PRODUCTION_TYPES; t++) {
    for (p = thisAgent->all_productions_of_type[t]; p != NIL; p = p->next) {
      p->firing_count = 0;
    }
  }
}
...
void reinitialize_soar(agent* thisAgent) {
  ...
  thisAgent->stop_soar =
      false;  // voigtjr:  this line doesn't exist in other kernel
...
void do_one_top_level_phase(agent* thisAgent) {
  ...
  switch (thisAgent->current_phase) {
    case INPUT_PHASE:
      ...
      /* we check e_cycle_count because Soar 7 runs multiple input cycles per
       * decision */
      /* always true for Soar 8 */
      if (thisAgent->e_cycles_this_d_cycle == 0) {
        soar_invoke_callbacks(thisAgent, BEFORE_DECISION_CYCLE_CALLBACK,
                              reinterpret_cast<soar_call_data>(INPUT_PHASE));
      } /* end if e_cycles_this_d_cycle == 0 */

      if (thisAgent->input_cycle_flag ==
          true) /* Soar 7 flag, but always true for Soar8 */
      {
        soar_invoke_callbacks(thisAgent, BEFORE_INPUT_PHASE_CALLBACK,
                              reinterpret_cast<soar_call_data>(INPUT_PHASE));
        ...
        soar_invoke_callbacks(thisAgent, AFTER_INPUT_PHASE_CALLBACK,
                              reinterpret_cast<soar_call_data>(INPUT_PHASE));

        if (thisAgent->input_period) {
          thisAgent->input_cycle_flag = false;
        }
      } /* END if (input_cycle_flag==true) AGR REW1 this line and 1 previous
           line */
      ...
      thisAgent->timers_decision_cycle_phase[INPUT_PHASE].update(
    ...
    case PROPOSE_PHASE: /* added in 8.6 to clarify Soar8 decision cycle */
      ...
      if (thisAgent->e_cycles_this_d_cycle < 1) {
        ...
        soar_invoke_callbacks(thisAgent, BEFORE_PROPOSE_PHASE_CALLBACK,
                              reinterpret_cast<soar_call_data>(PROPOSE_PHASE));

        // We need to generate this event here in case no elaborations fire...
        // FIXME return the correct enum top_level_phase constant in
        // soar_call_data?
        /*(soar_call_data)((thisAgent->applyPhase == true)? gSKI_K_APPLY_PHASE:
         * gSKI_K_PROPOSE_PHASE)*/
        soar_invoke_callbacks(thisAgent, BEFORE_ELABORATION_CALLBACK, NULL);

        /* 'Prime the decision for a new round of production firings at the end
         * of REW:   05.05.97   */  /*  KJC 04.05 moved here from INPUT_PHASE for 8.6.0 */
        initialize_consistency_calculations_for_new_decision(thisAgent);

        thisAgent->FIRING_TYPE = IE_PRODS;
        thisAgent->applyPhase =
            false; /* KJC 04/05: do we still need this line?  gSKI does*/
        ...
        if (thisAgent->current_phase == DECIDE_PHASE) {
          ...
          thisAgent->run_elaboration_count++;  // All phases count as a run
                                               // elaboration
          // FIXME return the correct enum top_level_phase constant in
          // soar_call_data?
          /*(soar_call_data)((thisAgent->applyPhase == true)?
           * gSKI_K_APPLY_PHASE: gSKI_K_PROPOSE_PHASE)*/
      ...
      while (thisAgent->current_phase != DECIDE_PHASE) {
        if (thisAgent->e_cycles_this_d_cycle) {
          ...
          // FIXME return the correct enum top_level_phase constant in
          // soar_call_data? /*(soar_call_data)((thisAgent->applyPhase == true)?
          // gSKI_K_APPLY_PHASE: gSKI_K_PROPOSE_PHASE)*/
          soar_invoke_callbacks(thisAgent, BEFORE_ELABORATION_CALLBACK, NULL);
        }
       ...
       *  otherwise, we're only stopping because we're running by ELABORATIONS,
       * so don't do the end-of-phase updating in that case.
       */
      if (thisAgent->current_phase == DECIDE_PHASE) {
        ...
        thisAgent->run_phase_count++;
        soar_invoke_callbacks(thisAgent, AFTER_PROPOSE_PHASE_CALLBACK,
                              reinterpret_cast<soar_call_data>(PROPOSE_PHASE));
...
#ifndef NO_TIMING_STUFF
      thisAgent->timers_phase.stop();
      thisAgent->timers_decision_cycle_phase[PROPOSE_PHASE].update(
          thisAgent->timers_phase);
#endif
    ...
    case PREFERENCE_PHASE:
       ...
       * not valid here */
      /* needs to be updated for gSKI interface, and gSKI needs to accommodate
       * Soar 7 */
      ...
      soar_invoke_callbacks(thisAgent, BEFORE_PREFERENCE_PHASE_CALLBACK,
      ...
      thisAgent
          ->run_elaboration_count++;  // All phases count as a run elaboration
      soar_invoke_callbacks(thisAgent, AFTER_PREFERENCE_PHASE_CALLBACK,
                            reinterpret_cast<soar_call_data>(PREFERENCE_PHASE));
      thisAgent->current_phase = WM_PHASE;

#ifndef NO_TIMING_STUFF /* REW:  28.07.96 */
      thisAgent->timers_phase.stop();
      thisAgent->timers_decision_cycle_phase[PREFERENCE_PHASE].update(
          thisAgent->timers_phase);
#endif
    ...
    case WM_PHASE:
      ...
      soar_invoke_callbacks(thisAgent, BEFORE_WM_PHASE_CALLBACK,
      ...
      thisAgent
          ->run_elaboration_count++;  // All phases count as a run elaboration
      soar_invoke_callbacks(thisAgent, AFTER_WM_PHASE_CALLBACK,
                            reinterpret_cast<soar_call_data>(WM_PHASE));
...
#ifndef NO_TIMING_STUFF /* REW:  28.07.96 */
      thisAgent->timers_phase.stop();
      thisAgent->timers_decision_cycle_phase[WM_PHASE].update(
          thisAgent->timers_phase);
#endif
    ...
    case APPLY_PHASE: /* added in 8.6 to clarify Soar8 decision cycle */
      ...
      /* e_cycle_count will always be zero UNLESS we are running by
       * ELABORATIONS. We only want to do the following if we've just finished
       * DECIDE and are starting APPLY.  If this is the second elaboration for
       * APPLY, then just do the while loop below.   KJC  June 05
       */
      if (thisAgent->e_cycles_this_d_cycle < 1) {
        if (thisAgent->trace_settings[TRACE_PHASES_SYSPARAM]) {
          print_phase(thisAgent, "\n--- Application Phase ---\n", 0);
        ...
        soar_invoke_callbacks(thisAgent, BEFORE_APPLY_PHASE_CALLBACK,
        ...
        thisAgent->FIRING_TYPE =
            PE_PRODS; /* might get reset in det_high_active_prod_level... */
        thisAgent->applyPhase =
            true; /* KJC 04/05: do we still need this line?  gSKI does*/
      ...
      if (thisAgent->current_phase == OUTPUT_PHASE) {
         ...
         * Set phase back to APPLY, do print_phase, callbacks and reset phase to
         * OUTPUT
         */
        ...
        thisAgent->run_phase_count++;
        soar_invoke_callbacks(thisAgent, AFTER_APPLY_PHASE_CALLBACK,
                              reinterpret_cast<soar_call_data>(APPLY_PHASE));
...
#ifndef NO_TIMING_STUFF
      thisAgent->timers_phase.stop();
      thisAgent->timers_decision_cycle_phase[APPLY_PHASE].update(
          thisAgent->timers_phase);
#endif
    ...
    case OUTPUT_PHASE:
      ...
      soar_invoke_callbacks(thisAgent, BEFORE_OUTPUT_PHASE_CALLBACK,
      ...
      if (epmem_enabled(thisAgent) &&
          (thisAgent->EpMem->epmem_params->phase->get_value() ==
      ...
      // RL apoptosis
      {
        rl_param_container::apoptosis_choices rl_apoptosis =
        ...
        if (rl_apoptosis != rl_param_container::apoptosis_none) {
          ...
          for (rl_production_memory::object_set::iterator p =
                   thisAgent->RL->rl_prods->forgotten_begin();
               p != thisAgent->RL->rl_prods->forgotten_end(); p++) {
            // conditions:
            // - no matched instantiations AND
            // - if RL...
            //   - no update count
            //   - not in some state's prev_op_rl_rules list
            if (((*p)->instantiations == NIL) &&
                (!(*p)->rl_rule ||
                 ((static_cast<int64_t>((*p)->rl_update_count) == 0) &&
                  ((*p)->rl_ref_count == 0)))) {
      ...
      // Count the outputs the agent generates (or times reaching
      // max-nil-outputs without sending output)
      if (thisAgent->output_link_changed ||
          ((++(thisAgent->run_last_output_count)) >=
           thisAgent->Decider->settings[DECIDER_MAX_NIL_OUTPUT_CYCLES])) {
        thisAgent->run_last_output_count = 0;
      ...
      thisAgent
          ->run_elaboration_count++;  // All phases count as a run elaboration
      soar_invoke_callbacks(thisAgent, AFTER_OUTPUT_PHASE_CALLBACK,
                            reinterpret_cast<soar_call_data>(OUTPUT_PHASE));

      /* REW: begin 09.15.96 */
      // JRV: Get rid of the cached XML after every decision but before the
      // after-decide-phase callback
      xml_invoke_callback(
          thisAgent);  // invokes XML_GENERATION_CALLBACK, clears XML state

      /* KJC June 05:  moved here from DECIDE Phase */
      soar_invoke_callbacks(thisAgent, AFTER_DECISION_CYCLE_CALLBACK,
                            reinterpret_cast<soar_call_data>(OUTPUT_PHASE));
#ifndef NO_TIMING_STUFF /* timers stopped KJC 10-04-98 */
      thisAgent->timers_phase.stop();
      thisAgent->timers_decision_cycle_phase[OUTPUT_PHASE].update(
          thisAgent->timers_phase);
#endif
      ...
      // Update per-cycle statistics
      {
...
#ifndef NO_TIMING_STUFF
        uint64_t derived_kernel_time_usec =
            get_derived_kernel_time_usec(thisAgent);
        dc_time_usec =
            derived_kernel_time_usec - thisAgent->last_derived_kernel_time_usec;
        ...
        if (thisAgent->Decider->settings[DECIDER_MAX_DC_TIME] > 0) {
          if (dc_time_usec >=
              static_cast<uint64_t>(
                  thisAgent->Decider->settings[DECIDER_MAX_DC_TIME])) {
            thisAgent->stop_soar = true;
            thisAgent->reason_for_stopping =
                "decision cycle time greater than interrupt threshold";
          }
        }
        thisAgent->last_derived_kernel_time_usec = derived_kernel_time_usec;

        double total_epmem_time =
            thisAgent->EpMem->epmem_timers->total->value();
        if (thisAgent->total_dc_epmem_time_sec >= 0) {
          double delta_epmem_time =
              total_epmem_time - thisAgent->total_dc_epmem_time_sec;
        ...
        if (thisAgent->total_dc_smem_time_sec >= 0) {
          double delta_smem_time =
              total_smem_time - thisAgent->total_dc_smem_time_sec;
        ...
        uint64_t dc_wm_changes = thisAgent->wme_addition_count -
                                 thisAgent->start_dc_wme_addition_count;
        dc_wm_changes += thisAgent->wme_removal_count -
                         thisAgent->start_dc_wme_removal_count;
        ...
        uint64_t dc_firing_counts = thisAgent->production_firing_count -
                                    thisAgent->start_dc_production_firing_count;
        if (thisAgent->max_dc_production_firing_count_value <
            dc_firing_counts) {
          thisAgent->max_dc_production_firing_count_value = dc_firing_counts;
          thisAgent->max_dc_production_firing_count_cycle =
        ...
        if (thisAgent->dc_stat_tracking) {
          stats_db_store(thisAgent, dc_time_usec, dc_wm_changes,
                         dc_firing_counts);
        }
      }
    ...
    case DECIDE_PHASE:
      ...
      thisAgent->decide_phases_count++; /* counts decisions, not cycles, for
                                           more accurate stats */
      ...
      soar_invoke_callbacks(thisAgent, BEFORE_DECIDE_PHASE_CALLBACK,
      ...
      soar_invoke_callbacks(thisAgent, AFTER_DECIDE_PHASE_CALLBACK,
                            reinterpret_cast<soar_call_data>(DECIDE_PHASE));
...
#ifndef NO_TIMING_STUFF
      thisAgent->timers_phase.stop();
      thisAgent->timers_decision_cycle_phase[DECIDE_PHASE].update(
          thisAgent->timers_phase);
#endif
      /* REW: end 28.07.96 */
  ...
  if (thisAgent->system_halted) {
    ...
    soar_invoke_callbacks(
        thisAgent, AFTER_HALT_SOAR_CALLBACK,
        reinterpret_cast<soar_call_data>(thisAgent->current_phase));

    // To model episodic task, after halt, perform RL update with next-state
    // value 0
...
void run_for_n_modifications_of_output(agent* thisAgent, int64_t n) {
  ...
  while (!thisAgent->stop_soar && n) {
    ...
    if (count >= thisAgent->Decider->settings[DECIDER_MAX_NIL_OUTPUT_CYCLES]) {
      break;
      // thisAgent->stop_soar = true;
      // thisAgent->reason_for_stopping = "exceeded max_nil_output_cycles with
      // no output";
    }
  }
#ifndef NO_TIMING_STUFF
...
void run_for_n_decision_cycles(agent* thisAgent, int64_t n) {
  ...
  while (!thisAgent->stop_soar) {
    if (n ==
        static_cast<int64_t>(thisAgent->d_cycle_count - d_cycles_at_start)) {
      break;
    }
    do_one_top_level_phase(thisAgent);
  }
...
void run_for_n_selections_of_slot(agent* thisAgent, int64_t n,
                                  Symbol* attr_of_slot) {
  int64_t count;
...
void init_agent_memory(agent* thisAgent) {
  ...
  thisAgent->io_header_link = add_input_wme(
      thisAgent, thisAgent->top_state,
      thisAgent->symbolManager->soarSymbols.io_symbol, thisAgent->io_header);
  ...
  //  is already being saved (above), and when we release it they will
  ...
  // KJC & RPM 10/06
  // A lot of stuff isn't initialized properly until the input and output cycles
  // are run the first time. Because of this, SW had to put in a hack to work
  // around changes made to the output-link in the first
  //   dc not being visible. (see comment near end of
  //   update_for_top_state_wme_addition).  This change added an item to the
  ...
  //   ol->ids_in_tc gets initialized, so remove_output_link_tc_info -- but it
  //   only cleans up the first copy of the item.
  // All of these problems come back to things not being initialized properly,
  // so we run the input and output
  //   phases here to force proper initialization (and have commented out SW's
  //   changes to update_for_top_state_wme_addition).  This will cause
  //   somecallbacks to be triggered, but we don't think this is a problem for
  //   two reasons: 1) these events are currently not exposed through SML, so no
  //   clients will see them 2) even if these events were exposed, they are
  //   being fired during agent creation.  Since the agent
  ...
  // call which attempted to do some
