...
void add_input_function(agent* thisAgent, soar_callback_fn f,
  ...
  soar_add_callback(thisAgent, INPUT_PHASE_CALLBACK, f, INPUT_PHASE_CALLBACK,
                    cb_data, free_fn, name);
}
...
void add_output_function(agent* thisAgent, soar_callback_fn f,
                         soar_callback_data cb_data,
                         soar_callback_free_fn free_fn, int eventID,
                         const char* output_link_name) {
  if (soar_exists_callback_id(thisAgent, OUTPUT_PHASE_CALLBACK,
                              output_link_name) != NULL) {
    ...
    soar_add_callback(thisAgent, OUTPUT_PHASE_CALLBACK, f, eventID, cb_data,
                      free_fn, output_link_name);
  }
}
...
Symbol* get_new_io_identifier(agent* thisAgent, char first_letter) {
  return thisAgent->symbolManager->make_new_identifier(first_letter,
                                                       TOP_GOAL_LEVEL, NIL);
}

Symbol* get_io_identifier(agent* thisAgent, char first_letter,
                          uint64_t number) {
  Symbol* id = thisAgent->symbolManager->find_identifier(first_letter, number);

  // DJP: The other "make_<type>" methods either make a new object or
  // incremenent the refence on an existing object.  So I'm going to make this
  // method function the same way for identifiers.
  if (id) {
    thisAgent->symbolManager->symbol_add_ref(id);
  } else {
    id = thisAgent->symbolManager->make_new_identifier(first_letter,
                                                       TOP_GOAL_LEVEL, NIL);
  }
...
wme* add_input_wme(agent* thisAgent, Symbol* id, Symbol* attr, Symbol* value) {
  ...
  // PrintDebugFormat("Added wme with timetag %d to id %c%d
  // ",w->timetag,id->id->name_letter,id->id->name_number) ;

  return w;
}

wme* find_input_wme_by_timetag_from_id(agent* thisAgent, Symbol* idSym,
                                       uint64_t timetag, tc_number tc) {
  wme *pWME, *w;

  // PrintDebugFormat("Scanning id %c%d", idSym->id->name_letter,
  // idSym->id->name_number) ;

  // Mark this id as having been visited (the key here is that tc numbers always
  // increase so tc_num must be < tc until it's marked)
  idSym->id->tc_num = tc;
  ...
  for (pWME = idSym->id->input_wmes; pWME != NIL; pWME = pWME->next) {
    ...
    // NOTE: The test for the tc_num keeps us from getting stuck in loops within
    // graphs
    if (pWME->value->symbol_type == IDENTIFIER_SYMBOL_TYPE &&
        pWME->value->id->tc_num != tc) {
...
void do_input_cycle(agent* thisAgent) {
  if (thisAgent->prev_top_state && (!thisAgent->top_state)) {
    ...
    soar_invoke_callbacks(
        thisAgent, INPUT_PHASE_CALLBACK,
...
void update_for_top_state_wme_addition(agent* thisAgent, wme* w) {
  ...
  // push(thisAgent, thisAgent->output_link_for_tc,
  // ol->link_wme->value->id->associated_output_links);
}
...
void inform_output_module_of_wm_changes(agent* thisAgent,
  ...
  for (c = wmes_being_added; c != NIL; c = c->rest) {
    ...
    if (w->id == thisAgent->io_header) {
      ...
      thisAgent->d_cycle_last_output =
          thisAgent->d_cycle_count; /* KJC 11/17/05 */
    }
...
void add_id_to_output_link_tc(agent* thisAgent, Symbol* id) {
  ...
  thisAgent->symbolManager->symbol_add_ref(
      id); /* make sure the id doesn't get deallocated before we
have a chance to free the cons cell we just added */

  /* --- add output_link to id's list --- */
  push(thisAgent, thisAgent->output_link_for_tc,
       id->id->associated_output_links);
  ...
  for (ol = thisAgent->existing_output_links; ol != NIL; ol = next_ol) {
    ...
    switch (ol->status) {
...
#endif
        if (ol->cb) {
          (ol->cb->function)(thisAgent, ol->cb->eventid, ol->cb->data,
                             &output_call_data);
        }
#ifndef NO_TIMING_STUFF
        thisAgent->timers_kernel.stop();
        thisAgent->timers_output_function_cpu_time.update(
            thisAgent->timers_kernel);
        thisAgent->timers_kernel.start();
      ...
      case REMOVED_OL_STATUS:
        ...
        iw_list = get_io_wmes_for_output_link(thisAgent,
                                              ol); /* gives just the link wme */
...
Symbol* get_io_symbol_from_tio_constituent_string(agent* thisAgent,
  ...
  soar::Lexer::determine_possible_symbol_types_for_string(
      input_string, strlen(input_string), &possible_id, &possible_var,
      &possible_sc, &possible_ic, &possible_fc, &rereadable);
  ...
  if (possible_ic) {
    ...
    if (errno) {
      thisAgent->outputManager->printa_sf(
          thisAgent, "Text Input Error: bad integer (probably too large)\n");
      return NIL;
    }
  ...
  if (possible_fc) {
    ...
    if (errno) {
      thisAgent->outputManager->printa_sf(
          thisAgent, "Text Input Error: bad floating point number\n");
      return NIL;
    }
...
void init_soar_io(agent* thisAgent) {
  ...
  thisAgent->memoryManager->init_memory_pool(
      MP_output_link, sizeof(output_link), "output link");
  thisAgent->memoryManager->init_memory_pool(MP_io_wme, sizeof(io_wme),
                                             "io wme");
