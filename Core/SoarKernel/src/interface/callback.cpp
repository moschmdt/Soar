...
void soar_init_callbacks(agent* the_agent) {
  int ct;  // ct was originally of type SOAR_CALLBACK_TYPE, changed for c++
           // compatibility (5/1/02)
...
int callback_count(agent* the_agent, SOAR_CALLBACK_TYPE callback_type) {
  int count = 0;
  for (cons* c = the_agent->soar_callbacks[callback_type]; c != NIL;
       c = c->rest) {
    ++count;
  }
...
#include <iostream>
void soar_add_callback(agent* thisAgent, SOAR_CALLBACK_TYPE callback_type,
                       soar_callback_fn fn, soar_callback_event_id eventid,
                       soar_callback_data data, soar_callback_free_fn free_fn,
                       soar_callback_id id) {
  soar_callback* cb;
...
//{
//   for (std::map<SOAR_CALLBACK_TYPE, soar_timer_accumulator>::iterator iter =
//   callback_timers.begin();
...
void soar_callback_data_free_string(soar_callback_data data) { free(data); }
...
SOAR_CALLBACK_TYPE soar_callback_name_to_enum(char* name,
                                              bool monitorable_only) {
  int limit;
  int i;  // i was originally of type SOAR_CALLBACK_TYPE, changed for c++
          // compatibility (5/1/02)
...
soar_callback* soar_exists_callback_id(agent* the_agent,
  ...
  for (c = the_agent->soar_callbacks[callback_type]; c != NIL; c = c->rest) {
    soar_callback* cb;
...
void soar_invoke_callbacks(agent* thisAgent, SOAR_CALLBACK_TYPE callback_type,
  ...
  switch (callback_type) {
    ...
    case AFTER_DECISION_CYCLE_CALLBACK:
      /* for soar7: thisAgent->current_phase = DECIDE_PHASE; for soar8 it's
       * OUTPUT_PHASE */
  ...
  for (c = thisAgent->soar_callbacks[callback_type]; c != NIL; c = c->rest) {
    soar_callback* cb;
  ...
  switch (callback_type) {
    ...
    case AFTER_DECISION_CYCLE_CALLBACK:
       ...
       * OUTPUT_PHASE */
      thisAgent->timers_phase.stop();
      thisAgent->timers_monitors_cpu_time[thisAgent->current_phase].update(
          thisAgent->timers_phase);
    ...
    case INPUT_PHASE_CALLBACK:
      ...
      thisAgent->timers_kernel.stop();
      thisAgent->timers_input_function_cpu_time.update(
          thisAgent->timers_kernel);
      thisAgent->callback_timers[callback_type].update(
          thisAgent->timers_kernel);
      thisAgent->timers_kernel.start();
      thisAgent->timers_phase.start();
      break;
...
void soar_list_all_callbacks(agent* thisAgent, bool monitorable_only) {
  ...
  for (ct = 1; ct < limit; ct++) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "%s: ",
        soar_callback_enum_to_name(static_cast<SOAR_CALLBACK_TYPE>(ct), false));
    soar_list_all_callbacks_for_event(thisAgent,
                                      static_cast<SOAR_CALLBACK_TYPE>(ct));
...
void soar_list_all_callbacks_for_event(agent* thisAgent,
  ...
  for (c = thisAgent->soar_callbacks[ct]; c != NIL; c = c->rest) {
    soar_callback* cb;
...
void soar_pop_callback(agent* thisAgent, SOAR_CALLBACK_TYPE callback_type) {
  ...
  if (head == NULL) {
    thisAgent->outputManager->printa(
        thisAgent, "Attempt to remove non-existant callback.\n");
    return;
  }

  if ((callback_type == PRINT_CALLBACK) && (head->rest == NULL)) {
    thisAgent->outputManager->printa(
        thisAgent, "Attempt to remove last print callback. Ignored.\n");
    return;
  }
...
void soar_push_callback(agent* thisAgent, SOAR_CALLBACK_TYPE callback_type,
                        soar_callback_fn fn, soar_callback_event_id eventid,
...
void soar_remove_all_monitorable_callbacks(agent* thisAgent) {
  ...
  for (ct = 1; ct < NUMBER_OF_MONITORABLE_CALLBACKS; ct++) {
    soar_remove_all_callbacks_for_event(thisAgent,
                                        static_cast<SOAR_CALLBACK_TYPE>(ct));
  }
}
...
void soar_remove_callback(agent* thisAgent, SOAR_CALLBACK_TYPE callback_type,
                          soar_callback_id id) {
  cons* c;
...
void soar_callback_test_callback(agent* /*the_agent*/, soar_callback_data data,
                                 soar_call_data /*call_data*/) {
...
void soar_test_all_monitorable_callbacks(agent* thisAgent) {
  ...
  for (i = 1; i < NUMBER_OF_MONITORABLE_CALLBACKS; i++) {
    soar_add_callback(
        thisAgent, static_cast<SOAR_CALLBACK_TYPE>(i),
        reinterpret_cast<soar_callback_fn>(soar_callback_test_callback), i,
        static_cast<void*>(const_cast<char*>(soar_callback_enum_to_name(
            static_cast<SOAR_CALLBACK_TYPE>(i), true))),
