...
bool Lexer::determine_type_of_constituent_string() {
  ...
  determine_possible_symbol_types_for_string(
      current_lexeme.string(), current_lexeme.length(), &possible_id,
  ...
  if (possible_ic) {
    ...
    if (errno) {
      thisAgent->outputManager->printa(
          thisAgent, "Error: bad integer (probably too large)\n");
  ...
  if (possible_id) {
    errno = 0;
    current_lexeme.id_letter =
        static_cast<char>(toupper(current_lexeme.lex_string[0]));
    current_lexeme.type = IDENTIFIER_LEXEME;
    if (!from_c_string(current_lexeme.id_number,
                       &(current_lexeme.lex_string[1]))) {
      thisAgent->outputManager->printa(
          thisAgent, "Error: bad number for identifier (probably too large)\n");
    ...
    if (thisAgent->outputManager->settings[OM_WARNINGS]) {
      if ((current_lexeme.lex_string[0] == '<') ||
        ...
        thisAgent->outputManager->printa_sf(
            thisAgent, "Warning: Suspicious string constant \"%s\"\n",
...
void Lexer::lex_equal() {
  ...
  if (current_lexeme.length() == 1) {
    current_lexeme.type = EQUAL_LEXEME;
    return;
  }
...
void Lexer::lex_ampersand() {
  ...
  read_constituent_string();
  if (current_lexeme.length() == 1) {
    current_lexeme.type = AMPERSAND_LEXEME;
    return;
  }
  determine_type_of_constituent_string();
}
...
void Lexer::lex_greater() {
  ...
  read_constituent_string();
  if (current_lexeme.length() == 1) {
    current_lexeme.type = GREATER_LEXEME;
    return;
  }
  if (current_lexeme.length() == 2) {
    if (current_lexeme.lex_string[1] == '>') {
      current_lexeme.type = GREATER_GREATER_LEXEME;
      return;
    }
    if (current_lexeme.lex_string[1] == '=') {
      current_lexeme.type = GREATER_EQUAL_LEXEME;
      return;
    }
  }
  determine_type_of_constituent_string();
...
void Lexer::lex_less() {
  ...
  read_constituent_string();
  if (current_lexeme.length() == 1) {
    current_lexeme.type = LESS_LEXEME;
    return;
  }
  if (current_lexeme.length() == 2) {
    if (current_lexeme.lex_string[1] == '>') {
      current_lexeme.type = NOT_EQUAL_LEXEME;
      return;
    }
    if (current_lexeme.lex_string[1] == '=') {
      current_lexeme.type = LESS_EQUAL_LEXEME;
      return;
    }
    if (current_lexeme.lex_string[1] == '<') {
      current_lexeme.type = LESS_LESS_LEXEME;
      return;
    }
  }
  if (current_lexeme.length() == 3) {
    if ((current_lexeme.lex_string[1] == '=') &&
        (current_lexeme.lex_string[2] == '>')) {
      current_lexeme.type = LESS_EQUAL_GREATER_LEXEME;
      return;
    }
  }
  determine_type_of_constituent_string();
...
void Lexer::lex_period() {
  ...
  bool float_disallowed =
      !isspace(prev_char) && (current_lexeme.type == STR_CONSTANT_LEXEME ||
                              current_lexeme.type == VARIABLE_LEXEME);
...
void Lexer::lex_minus() {
  ...
  if (current_lexeme.length() == 1) {
    current_lexeme.type = MINUS_LEXEME;
    return;
  }
  if (current_lexeme.length() == 3) {
    if ((current_lexeme.lex_string[1] == '-') &&
        (current_lexeme.lex_string[2] == '>')) {
      current_lexeme.type = RIGHT_ARROW_LEXEME;
      return;
    }
  }
  determine_type_of_constituent_string();
...
void Lexer::lex_vbar() {
  ...
  do {
    if (current_char == EOF) {
      thisAgent->outputManager->printa(
          thisAgent, "Error:  opening '|' without closing '|'\n");
...
void Lexer::consume_whitespace_and_comments() {
  ...
  while (true) {
    ...
    if (current_char == '#') {
      while ((current_char != '\n') && (current_char != EOF)) get_next_char();
...
int Lexer::current_parentheses_level() const { return parentheses_level; }

const char *Lexer::current_remaining_string() const {
  return production_string;
}

const char *Lexer::current_orig_string() const { return orig_string.c_str(); }
...
void Lexer::determine_possible_symbol_types_for_string(
    const char *s, size_t length_of_s, bool *possible_id, bool *possible_var,
    bool *possible_sc, bool *possible_ic, bool *possible_fc, bool *rereadable) {
  const char *ch;
  bool all_alphanum;
  ...
  *rereadable = false;
  ...
  if (number_starters[static_cast<unsigned char>(*s)]) {
    ...
    if ((*ch == 0) && (isdigit(*(ch - 1)))) *possible_ic = true;
    if (*ch == '.') {
      ch++;                      /* decimal point */
      while (isdigit(*ch)) ch++; /* string of digits */
      if ((*ch == 'e') || (*ch == 'E')) {
  ...
  if (all_alphanum || (length_of_s > length_of_longest_special_lexeme) ||
      ((length_of_s == 1) && (*s == '*'))) {
    *rereadable = true;
  }
...
void Lexer::addExec() {
  ...
  while (std::string("exec") != current_lexeme.string() ||
         strlen(production_string) > beg)
    get_lexeme();
}
