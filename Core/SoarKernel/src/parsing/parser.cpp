...
void substitute_for_placeholders_in_symbol(agent* thisAgent, Symbol** sym) {
  ...
  if (!(*sym)->var->current_binding_value) {
    ...
    prefix[1] = '*';
    prefix[2] = 0;
    (*sym)->var->current_binding_value =
        thisAgent->symbolManager->generate_new_variable(prefix);
    just_created = true;
  }
    ...
    case CONJUNCTIVE_TEST:
      for (c = ct->data.conjunct_list; c != NIL; c = c->rest) {
        substitute_for_placeholders_in_test(
            thisAgent, reinterpret_cast<test*>(&(c->first)));
      }
      return;
    default:
  ...
  for (; cond != NIL; cond = cond->next) {
    switch (cond->type) {
      ...
      case NEGATIVE_CONDITION:
        substitute_for_placeholders_in_test(thisAgent,
      ...
      case CONJUNCTIVE_NEGATION_CONDITION:
        substitute_for_placeholders_in_condition_list(thisAgent,
                                                      cond->data.ncc.top);
        break;
    }
  }
...
Symbol* make_symbol_for_lexeme(agent* thisAgent, Lexeme* lexeme,
  ...
  switch (lexeme->type) {
    ...
    case IDENTIFIER_LEXEME: {
      //            thisAgent->outputManager->printa_sf(thisAgent, "Found
      //            potential Soar identifier that would be invalid.  Adding as
      //            string.\n", lexeme->id_letter, lexeme->id_number);
      char buf[30];
      SNPRINTF(buf, sizeof(buf) - 1, "%c%" SCNu64, lexeme->id_letter,
               lexeme->id_number);
      buf[sizeof(buf) - 1] = '\0';
    ...
    default: {
      ...
      SNPRINTF(msg, BUFFER_MSG_SIZE,
               "Internal error:  Illegal lexeme type found in "
               "make_symbol_for_lexeme: %s\n",
               lexeme->string());
...
test parse_relational_test(agent* thisAgent, Lexer* lexer) {
  ...
  switch (lexer->current_lexeme.type) {
    ...
    case IDENTIFIER_LEXEME:
      referent =
          make_symbol_for_lexeme(thisAgent, &(lexer->current_lexeme), false);
      if (!lexer->get_lexeme()) {
    ...
    default:
      thisAgent->outputManager->printa_sf(
          thisAgent, "Expected variable or constant for test\n");
...
test parse_disjunction_test(agent* thisAgent, Lexer* lexer) {
  ...
  if (lexer->current_lexeme.type != LESS_LESS_LEXEME) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "Expected << to begin disjunction test\n");
  ...
  while (lexer->current_lexeme.type != GREATER_GREATER_LEXEME) {
    switch (lexer->current_lexeme.type) {
      ...
      case FLOAT_CONSTANT_LEXEME:
      case IDENTIFIER_LEXEME:
        /* make_symbol_for_lexeme will convert an identifier lexeme into a
         * string symbol */
        push(thisAgent,
             make_symbol_for_lexeme(thisAgent, &(lexer->current_lexeme), false),
...
test parse_test(agent* thisAgent, Lexer* lexer) {
  ...
  do {
    ...
    if (t && t->eq_test && temp->eq_test) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "Soar does not support having two equality tests in one conjunctive "
          "test!\n");
      if (t->type == EQUALITY_TEST && temp->type == EQUALITY_TEST) {
        if (!t->data.referent->is_constant() &&
            temp->data.referent->is_constant()) {
          thisAgent->outputManager->printa_sf(
              thisAgent,
              "Ignoring %t in favor of constant %t.  Rule semantics may have "
              "changed!\n",
              t, temp);
          deallocate_test(thisAgent, t);
          t = temp;
          ...
          thisAgent->outputManager->printa_sf(
              thisAgent,
              "Ignoring %t in favor of existing %t.  Rule semantics may have "
              "changed!\n",
              temp, t->eq_test);
  ...
  if (t->type == CONJUNCTIVE_TEST) {
    t->data.conjunct_list = destructively_reverse_list(t->data.conjunct_list);
  }
...
condition* parse_value_test_star(agent* thisAgent, Lexer* lexer,
                                 char first_letter) {
...
auto parse_head_of_conds_for_one_id(agent* thisAgent, Lexer* lexer,
  ...
  if (lexer->current_lexeme.type != L_PAREN_LEXEME) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "Expected ( to begin condition element\n");
  ...
  if ((lexer->current_lexeme.type != MINUS_LEXEME) &&
    ...
    if (!id_test->eq_test) {
      add_test(thisAgent, &id_test,
               make_placeholder_test(thisAgent, first_letter_if_no_id_given));
    } else {
      ...
      // Symbol type can only be IDENTIFIER_SYMBOL_TYPE if it is a long term
      // identifier (lti), Otherwise, it isn't possible to have an
      // IDENTIFIER_SYMBOL_TYPE here.
      if ((sym->symbol_type != VARIABLE_SYMBOL_TYPE) &&
          (sym->symbol_type != IDENTIFIER_SYMBOL_TYPE)) {
        thisAgent->outputManager->printa_sf(
            thisAgent, "Warning: Constant %y in id field test.\n", sym);
        thisAgent->outputManager->printa_sf(
            thisAgent, "         This will never match.\n");
        ...
        // TODO: should we append this to the previous XML message or create a
        // new message for it?
...
condition* parse_tail_of_conds_for_one_id(agent* thisAgent, Lexer* lexer,
  ...
  if (lexer->current_lexeme.type == R_PAREN_LEXEME) {
    if (is_state_or_impasse_cond) {
      thisAgent->outputManager->printa_sf(
          thisAgent,
          "Error: Expected attribute-value test "
          "after state/impasse test. Did you forget to add \"^type state\" or "
          "\"^superstate nil\"?\n");
      return nullptr;
    }
    ...
    if (!lexer->get_lexeme()) return NULL;
    c = make_condition(thisAgent, NULL, make_placeholder_test(thisAgent, 'a'),
...
condition* parse_cond(agent* thisAgent, Lexer* lexer) {
  ...
  if (lexer->current_lexeme.type == L_BRACE_LEXEME) {
    ...
    if (lexer->current_lexeme.type != R_BRACE_LEXEME) {
      thisAgent->outputManager->printa_sf(
          thisAgent, "Expected } to end conjunctive condition\n");
...
const char* help_on_rhs_grammar[] = {
    ...
    "   <naturally-unary-preference> ::= + | - | ! | ~ | @",
    "   <binary-preference> ::= > | = | < | &",
    "   <any-preference> ::= <naturally-unary-preference> | "
    "<binary-preference>",
    "   <forced-unary-preference> ::= <binary-preference> ",
    "                                 {<any-preference> | , | ) | ^}",
    "     ;but the parser shouldn't consume the <any-preference>, \")\" or "
...
rhs_value parse_function_call_after_lparen(agent* thisAgent, Lexer* lexer,
  ...
  if (lexer->current_lexeme.type == PLUS_LEXEME) {
    ...
    if (std::string(lexer->current_lexeme.string()) == "succeeded" ||
        std::string(lexer->current_lexeme.string()) == "failed") {
      // unit tests
    } else {
      if (fun_name) {
  ...
  if (!fun_name &&
      (std::string(lexer->current_lexeme.string()) == "succeeded" ||
       std::string(lexer->current_lexeme.string()) == "failed")) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "WARNING: Replacing function named %s with halt since this is a unit "
        "test but running in a non-unit testing environment.\n",
        lexer->current_lexeme.string());
  ...
  if (!rf && (std::string(lexer->current_lexeme.string()) == "succeeded" ||
              std::string(lexer->current_lexeme.string()) == "failed")) {
    thisAgent->outputManager->printa_sf(
        thisAgent,
        "WARNING: Replacing function named %s with halt since this is a unit "
        "test but running in a non-unit testing environment.\n",
        lexer->current_lexeme.string());
    rf = lookup_rhs_function(
        thisAgent, thisAgent->symbolManager->find_str_constant("halt"));
  }
  ...
  if ((!is_stand_alone_action) && (!rf->can_be_rhs_value)) {
    thisAgent->outputManager->printa_sf(
        thisAgent, "Function %s can only be used as a stand-alone action\n",
        lexer->current_lexeme.string());
    return NIL;
  }
...
action* parse_attr_value_make(agent* thisAgent, Lexer* lexer, Symbol* id) {
  ...
  while (lexer->current_lexeme.type == PERIOD_LEXEME) {
    ...
    new_var =
        make_placeholder_var(thisAgent, first_letter_from_rhs_value(attr));
    ...
    if (attr_str != "operator") {
      ...
      new_actions = parse_preferences_soar8_non_operator(thisAgent, lexer, id,
                                                         attr, lNewRHSValue);
    } else {
      lNewRHSValue = allocate_rhs_value_for_symbol(thisAgent, new_var, 0, 0);
...
void abort_parse_production(agent* thisAgent, Symbol*& name,
                            char** documentation = NULL,
                            condition** lhs_top = NULL, action** rhs = NULL) {
  if (name) {
    thisAgent->outputManager->printa_sf(thisAgent,
                                        "(Ignoring production %y)\n\n", name);
...
production* parse_production(agent* thisAgent, const char* prod_string,
  ...
  name = thisAgent->symbolManager->make_str_constant(
      lexer.current_lexeme.string());
  if (!lexer.get_lexeme()) {
  ...
  p = make_production(thisAgent, prod_type, name, name->sc->name, &lhs_top,
                      &rhs, true, NULL);

  if (prod_type == TEMPLATE_PRODUCTION_TYPE) {
    if (!rl_valid_template(p)) {
      thisAgent->outputManager->printa_sf(
          thisAgent, "Invalid Soar-RL template (%y)\n\n", name);
  ...
  *rete_addition_result =
      add_production_to_rete(thisAgent, p, lhs_top, NIL, true, duplicate_rule);
  deallocate_condition_list(thisAgent, lhs_top);
