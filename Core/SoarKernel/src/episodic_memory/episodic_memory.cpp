...
epmem_param_container::epmem_param_container(agent* new_agent)
  ...
  // learning
  learning = new soar_module::boolean_param(
  ...
  phase = new soar_module::constant_param<phase_choices>(
      "phase", phase_output, new soar_module::f_predicate<phase_choices>());
  phase->add_mapping(phase_output, "output");
  phase->add_mapping(phase_selection, "selection");
  add(phase);

  // trigger
  trigger = new soar_module::constant_param<trigger_choices>(
      "trigger", dc, new soar_module::f_predicate<trigger_choices>());
  ...
  // force
  force = new soar_module::constant_param<force_choices>(
      "force", force_off, new soar_module::f_predicate<force_choices>());
  ...
  exclusions = new soar_module::sym_set_param(
  ...
  database = new soar_module::constant_param<db_choices>(
      "database", memory, new soar_module::f_predicate<db_choices>());
  database->add_mapping(memory, "memory");
  database->add_mapping(file, "file");
  add(database);

  // append database or dump data on init
  append_db = new soar_module::boolean_param(
      "append", off, new soar_module::f_predicate<boolean>());
  add(append_db);

  // path
  path = new epmem_path_param(
      "path", "", new soar_module::predicate<const char*>(),
      new soar_module::f_predicate<const char*>(), thisAgent);
  add(path);

  // auto-commit
  lazy_commit = new soar_module::boolean_param(
  ...
  graph_match = new soar_module::boolean_param(
      "graph-match", on, new soar_module::f_predicate<boolean>());
  add(graph_match);

  // balance
  balance = new soar_module::decimal_param(
      "balance", 1, new soar_module::btw_predicate<double>(0, 1, true),
  ...
  timers = new soar_module::constant_param<soar_module::timer::timer_level>(
      "timers", soar_module::timer::zero,
      new soar_module::f_predicate<soar_module::timer::timer_level>());
  ...
  // page_size
  page_size = new soar_module::constant_param<page_choices>(
      "page-size", page_8k, new epmem_db_predicate<page_choices>(thisAgent));
  ...
  // cache_size
  cache_size = new soar_module::integer_param(
      "cache-size", 10000, new soar_module::gt_predicate<int64_t>(1, true),
      new epmem_db_predicate<int64_t>(thisAgent));
  add(cache_size);

  // opt
  opt = new soar_module::constant_param<opt_choices>(
      "optimization", epmem_param_container::opt_speed,
      new epmem_db_predicate<opt_choices>(thisAgent));
  ...
  gm_ordering = new soar_module::constant_param<gm_ordering_choices>(
      ...
      new soar_module::f_predicate<gm_ordering_choices>());
  ...
  // merge
  merge = new soar_module::constant_param<merge_choices>(
      "merge", merge_none, new soar_module::f_predicate<merge_choices>());
...
epmem_path_param::epmem_path_param(
    const char* new_name, const char* new_value,
    soar_module::predicate<const char*>* new_val_pred,
    soar_module::predicate<const char*>* new_prot_pred, agent* new_agent)
    : soar_module::string_param(new_name, new_value, new_val_pred,
                                new_prot_pred),
      thisAgent(new_agent) {}

void epmem_path_param::set_value(const char* new_value) {
  /* Removed automatic switching to disk database mode when first setting path.
     Now that switching databases and database modes on the fly seems to work,
     there's no need to attach special significance to the first time the path
     is set. MMA 2013 */
...
epmem_db_predicate<T>::epmem_db_predicate(agent* new_agent)
    : soar_module::agent_predicate<T>(new_agent) {}

template <typename T>
bool epmem_db_predicate<T>::operator()(T /*val*/) {
  return (this->thisAgent->EpMem->epmem_db->get_status() ==
...
inline epmem_hash_id epmem_temporal_hash_add_type(agent* thisAgent,
  ...
  thisAgent->EpMem->epmem_stmts_common->hash_add_type->execute(
      soar_module::op_reinit);
  return static_cast<epmem_hash_id>(
      thisAgent->EpMem->epmem_db->last_insert_rowid());
}

inline epmem_hash_id epmem_temporal_hash_int(agent* thisAgent, int64_t val,
                                             bool add_on_fail = true) {
  epmem_hash_id return_val = NIL;

  // search first
  thisAgent->EpMem->epmem_stmts_common->hash_get_int->bind_int(1, val);
  if (thisAgent->EpMem->epmem_stmts_common->hash_get_int->execute() ==
      soar_module::row) {
    return_val = static_cast<epmem_hash_id>(
        thisAgent->EpMem->epmem_stmts_common->hash_get_int->column_int(0));
  }
  ...
  if (!return_val && add_on_fail) {
    // type first
    return_val =
        epmem_temporal_hash_add_type(thisAgent, INT_CONSTANT_SYMBOL_TYPE);
    ...
    thisAgent->EpMem->epmem_stmts_common->hash_add_int->execute(
        soar_module::op_reinit);
  }
...
inline epmem_hash_id epmem_temporal_hash_float(agent* thisAgent, double val,
                                               bool add_on_fail = true) {
  epmem_hash_id return_val = NIL;

  // search first
  thisAgent->EpMem->epmem_stmts_common->hash_get_float->bind_double(1, val);
  if (thisAgent->EpMem->epmem_stmts_common->hash_get_float->execute() ==
      soar_module::row) {
    return_val = static_cast<epmem_hash_id>(
        thisAgent->EpMem->epmem_stmts_common->hash_get_float->column_int(0));
  }
  ...
  if (!return_val && add_on_fail) {
    // type first
    return_val =
        epmem_temporal_hash_add_type(thisAgent, FLOAT_CONSTANT_SYMBOL_TYPE);

    // then content
    thisAgent->EpMem->epmem_stmts_common->hash_add_float->bind_int(1,
                                                                   return_val);
    thisAgent->EpMem->epmem_stmts_common->hash_add_float->bind_double(2, val);
    thisAgent->EpMem->epmem_stmts_common->hash_add_float->execute(
        soar_module::op_reinit);
  }
...
inline epmem_hash_id epmem_temporal_hash_str(agent* thisAgent, char* val,
                                             bool add_on_fail = true) {
  epmem_hash_id return_val = NIL;

  // search first
  thisAgent->EpMem->epmem_stmts_common->hash_get_str->bind_text(
      1, static_cast<const char*>(val));
  if (thisAgent->EpMem->epmem_stmts_common->hash_get_str->execute() ==
      soar_module::row) {
    return_val = static_cast<epmem_hash_id>(
        thisAgent->EpMem->epmem_stmts_common->hash_get_str->column_int(0));
  }
  ...
  if (!return_val && add_on_fail) {
    // type first
    return_val =
        epmem_temporal_hash_add_type(thisAgent, STR_CONSTANT_SYMBOL_TYPE);

    // then content
    thisAgent->EpMem->epmem_stmts_common->hash_add_str->bind_int(1, return_val);
    thisAgent->EpMem->epmem_stmts_common->hash_add_str->bind_text(
        2, static_cast<const char*>(val));
    thisAgent->EpMem->epmem_stmts_common->hash_add_str->execute(
        soar_module::op_reinit);
  }
...
inline int64_t epmem_reverse_hash_int(agent* thisAgent,
  ...
  soar_module::exec_result res =
      thisAgent->EpMem->epmem_stmts_common->hash_rev_int->execute();
  (void)res;  // quells compiler warning
  assert(res == soar_module::row);
  return_val =
      thisAgent->EpMem->epmem_stmts_common->hash_rev_int->column_int(0);
  thisAgent->EpMem->epmem_stmts_common->hash_rev_int->reinitialize();

  return return_val;
}

inline double epmem_reverse_hash_float(agent* thisAgent,
                                       epmem_hash_id s_id_lookup) {
  double return_val = NIL;

  thisAgent->EpMem->epmem_stmts_common->hash_rev_float->bind_int(1,
                                                                 s_id_lookup);
  soar_module::exec_result res =
      thisAgent->EpMem->epmem_stmts_common->hash_rev_float->execute();
  (void)res;  // quells compiler warning
  assert(res == soar_module::row);
  return_val =
      thisAgent->EpMem->epmem_stmts_common->hash_rev_float->column_double(0);
  thisAgent->EpMem->epmem_stmts_common->hash_rev_float->reinitialize();

  return return_val;
}

inline void epmem_reverse_hash_str(agent* thisAgent, epmem_hash_id s_id_lookup,
                                   std::string& dest) {
  soar_module::exec_result res;
  soar_module::sqlite_statement* sql_hash_rev_str =
      thisAgent->EpMem->epmem_stmts_common->hash_rev_str;
...
inline Symbol* epmem_reverse_hash(agent* thisAgent, epmem_hash_id s_id_lookup,
                                  byte sym_type = 255) {
  Symbol* return_val = NULL;
  std::string dest;

  if (sym_type == 255) {
    thisAgent->EpMem->epmem_stmts_common->hash_get_type->bind_int(1,
                                                                  s_id_lookup);
    soar_module::exec_result res =
        thisAgent->EpMem->epmem_stmts_common->hash_get_type->execute();
    (void)res;  // quells compiler warning
    assert(res == soar_module::row);
    sym_type = static_cast<byte>(
        thisAgent->EpMem->epmem_stmts_common->hash_get_type->column_int(0));
  ...
  switch (sym_type) {
    case STR_CONSTANT_SYMBOL_TYPE:
      ...
      return_val = thisAgent->symbolManager->make_str_constant(
          const_cast<char*>(dest.c_str()));
      break;

    case INT_CONSTANT_SYMBOL_TYPE:
      return_val = thisAgent->symbolManager->make_int_constant(
          epmem_reverse_hash_int(thisAgent, s_id_lookup));
      break;

    case FLOAT_CONSTANT_SYMBOL_TYPE:
      return_val = thisAgent->symbolManager->make_float_constant(
          epmem_reverse_hash_float(thisAgent, s_id_lookup));
      break;
...
inline void epmem_reverse_hash_print(agent* thisAgent,
                                     epmem_hash_id s_id_lookup,
                                     std::string& dest, byte sym_type = 255) {
  Symbol* return_val = NULL;

  // This may be faster than including type lookup in edges?  Might want to
  // check later.
...
epmem_hash_id epmem_temporal_hash(agent* thisAgent, Symbol* sym,
                                  bool add_on_fail) {
  epmem_hash_id return_val = NIL;
  ...
  if (sym->is_constant()) {
    if ((!sym->epmem_hash) ||
      ...
      switch (sym->symbol_type) {
        case STR_CONSTANT_SYMBOL_TYPE:
          return_val =
              epmem_temporal_hash_str(thisAgent, sym->sc->name, add_on_fail);
          break;

        case INT_CONSTANT_SYMBOL_TYPE:
          return_val =
              epmem_temporal_hash_int(thisAgent, sym->ic->value, add_on_fail);
          break;

        case FLOAT_CONSTANT_SYMBOL_TYPE:
          return_val =
              epmem_temporal_hash_float(thisAgent, sym->fc->value, add_on_fail);
          break;
      }
...
epmem_stat_container::epmem_stat_container(agent* new_agent)
  ...
  // time
  time = new epmem_time_id_stat(
      "time", 0, new epmem_db_predicate<epmem_time_id>(thisAgent));
  add(time);

  // db-lib-version
  db_lib_version =
      new epmem_db_lib_version_stat(thisAgent, "db-lib-version", NULL,
                                    new soar_module::predicate<const char*>());
  add(db_lib_version);

  // mem-usage
  mem_usage = new epmem_mem_usage_stat(thisAgent, "mem-usage", 0,
                                       new soar_module::predicate<int64_t>());
  add(mem_usage);

  // mem-high
  mem_high = new epmem_mem_high_stat(thisAgent, "mem-high", 0,
                                     new soar_module::predicate<int64_t>());
  add(mem_high);

  // non-cue-based-retrievals
  ncbr = new soar_module::integer_stat("retrievals", 0,
                                       new soar_module::f_predicate<int64_t>());
  add(ncbr);

  // cue-based-retrievals
  cbr = new soar_module::integer_stat("queries", 0,
                                      new soar_module::f_predicate<int64_t>());
  add(cbr);

  // nexts
  nexts = new soar_module::integer_stat(
      "nexts", 0, new soar_module::f_predicate<int64_t>());
  add(nexts);

  // prev's
  prevs = new soar_module::integer_stat(
      "prevs", 0, new soar_module::f_predicate<int64_t>());
  add(prevs);

  // ncb-wmes
  ncb_wmes = new soar_module::integer_stat(
      "ncb-wmes", 0, new soar_module::f_predicate<int64_t>());
  add(ncb_wmes);

  // qry-pos
  qry_pos = new soar_module::integer_stat(
      "qry-pos", 0, new soar_module::f_predicate<int64_t>());
  add(qry_pos);

  // qry-neg
  qry_neg = new soar_module::integer_stat(
      "qry-neg", 0, new soar_module::f_predicate<int64_t>());
  add(qry_neg);

  // qry-ret
  qry_ret = new epmem_time_id_stat(
      "qry-ret", 0, new soar_module::f_predicate<epmem_time_id>());
  add(qry_ret);

  // qry-card
  qry_card = new soar_module::integer_stat(
      "qry-card", 0, new soar_module::f_predicate<int64_t>());
  add(qry_card);

  // qry-lits
  qry_lits = new soar_module::integer_stat(
      "qry-lits", 0, new soar_module::f_predicate<int64_t>());
  add(qry_lits);

  // next-id
  next_id = new epmem_node_id_stat(
      "next-id", 0, new epmem_db_predicate<epmem_node_id>(thisAgent));
  add(next_id);

  // rit-offset-1
  rit_offset_1 = new soar_module::integer_stat(
      "rit-offset-1", 0, new epmem_db_predicate<int64_t>(thisAgent));
  add(rit_offset_1);

  // rit-left-root-1
  rit_left_root_1 = new soar_module::integer_stat(
      "rit-left-root-1", 0, new epmem_db_predicate<int64_t>(thisAgent));
  add(rit_left_root_1);

  // rit-right-root-1
  rit_right_root_1 = new soar_module::integer_stat(
      "rit-right-root-1", 0, new epmem_db_predicate<int64_t>(thisAgent));
  add(rit_right_root_1);

  // rit-min-step-1
  rit_min_step_1 = new soar_module::integer_stat(
      "rit-min-step-1", 0, new epmem_db_predicate<int64_t>(thisAgent));
  add(rit_min_step_1);

  // rit-offset-2
  rit_offset_2 = new soar_module::integer_stat(
      "rit-offset-2", 0, new epmem_db_predicate<int64_t>(thisAgent));
  add(rit_offset_2);

  // rit-left-root-2
  rit_left_root_2 = new soar_module::integer_stat(
      "rit-left-root-2", 0, new epmem_db_predicate<int64_t>(thisAgent));
  add(rit_left_root_2);

  // rit-right-root-2
  rit_right_root_2 = new soar_module::integer_stat(
      "rit-right-root-2", 0, new epmem_db_predicate<int64_t>(thisAgent));
  add(rit_right_root_2);

  // rit-min-step-2
  rit_min_step_2 = new soar_module::integer_stat(
  ...
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].offset.stat =
      rit_offset_1;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].offset.var_key =
      var_rit_offset_1;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].leftroot.stat =
      rit_left_root_1;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE]
      .leftroot.var_key = var_rit_leftroot_1;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].rightroot.stat =
      rit_right_root_1;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE]
      .rightroot.var_key = var_rit_rightroot_1;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].minstep.stat =
      rit_min_step_1;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE]
      .minstep.var_key = var_rit_minstep_1;

  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].offset.stat =
      rit_offset_2;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].offset.var_key =
      var_rit_offset_2;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].leftroot.stat =
      rit_left_root_2;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE]
      .leftroot.var_key = var_rit_leftroot_2;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].rightroot.stat =
      rit_right_root_2;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE]
      .rightroot.var_key = var_rit_rightroot_2;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].minstep.stat =
      rit_min_step_2;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE]
      .minstep.var_key = var_rit_minstep_2;
}

//

epmem_db_lib_version_stat::epmem_db_lib_version_stat(
    agent* new_agent, const char* new_name, const char* new_value,
    soar_module::predicate<const char*>* new_prot_pred)
    : soar_module::primitive_stat<const char*>(new_name, new_value,
                                               new_prot_pred),
      thisAgent(new_agent) {}
...
epmem_mem_usage_stat::epmem_mem_usage_stat(
    agent* new_agent, const char* new_name, int64_t new_value,
    soar_module::predicate<int64_t>* new_prot_pred)
    : soar_module::integer_stat(new_name, new_value, new_prot_pred),
      thisAgent(new_agent) {}
...
epmem_mem_high_stat::epmem_mem_high_stat(
    agent* new_agent, const char* new_name, int64_t new_value,
    soar_module::predicate<int64_t>* new_prot_pred)
    : soar_module::integer_stat(new_name, new_value, new_prot_pred),
      thisAgent(new_agent) {}
...
epmem_timer_container::epmem_timer_container(agent* new_agent)
  ...
  storage =
      new epmem_timer("epmem_storage", thisAgent, soar_module::timer::two);
  add(storage);

  ncb_retrieval = new epmem_timer("epmem_ncb_retrieval", thisAgent,
                                  soar_module::timer::two);
  add(ncb_retrieval);
  ...
  trigger =
      new epmem_timer("epmem_trigger", thisAgent, soar_module::timer::two);
  add(trigger);
  ...
  wm_phase =
      new epmem_timer("epmem_wm_phase", thisAgent, soar_module::timer::two);
  add(wm_phase);
  ...
  ncb_edge_rit =
      new epmem_timer("ncb_edge_rit", thisAgent, soar_module::timer::three);
  add(ncb_edge_rit);
  ...
  ncb_node_rit =
      new epmem_timer("ncb_node_rit", thisAgent, soar_module::timer::three);
  add(ncb_node_rit);

  query_dnf =
      new epmem_timer("query_dnf", thisAgent, soar_module::timer::three);
  add(query_dnf);

  query_walk =
      new epmem_timer("query_walk", thisAgent, soar_module::timer::three);
  add(query_walk);

  query_walk_edge =
      new epmem_timer("query_walk_edge", thisAgent, soar_module::timer::three);
  add(query_walk_edge);

  query_walk_interval = new epmem_timer("query_walk_interval", thisAgent,
                                        soar_module::timer::three);
  add(query_walk_interval);

  query_graph_match = new epmem_timer("query_graph_match", thisAgent,
                                      soar_module::timer::three);
  add(query_graph_match);

  query_result =
      new epmem_timer("query_result", thisAgent, soar_module::timer::three);
  add(query_result);

  query_cleanup =
      new epmem_timer("query_cleanup", thisAgent, soar_module::timer::three);
  add(query_cleanup);

  query_sql_edge =
      new epmem_timer("query_sql_edge", thisAgent, soar_module::timer::three);
  add(query_sql_edge);

  query_sql_start_ep = new epmem_timer("query_sql_start_ep", thisAgent,
                                       soar_module::timer::three);
  add(query_sql_start_ep);

  query_sql_start_now = new epmem_timer("query_sql_start_now", thisAgent,
                                        soar_module::timer::three);
  add(query_sql_start_now);

  query_sql_start_point = new epmem_timer("query_sql_start_point", thisAgent,
                                          soar_module::timer::three);
  add(query_sql_start_point);

  query_sql_end_ep =
      new epmem_timer("query_sql_end_ep", thisAgent, soar_module::timer::three);
  add(query_sql_end_ep);

  query_sql_end_now = new epmem_timer("query_sql_end_now", thisAgent,
                                      soar_module::timer::three);
  add(query_sql_end_now);

  query_sql_end_point = new epmem_timer("query_sql_end_point", thisAgent,
                                        soar_module::timer::three);
  ...
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].timer =
      ncb_node_rit;
  thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].timer =
      ncb_edge_rit;
}

//

epmem_timer_level_predicate::epmem_timer_level_predicate(agent* new_agent)
    : soar_module::agent_predicate<soar_module::timer::timer_level>(new_agent) {
}

bool epmem_timer_level_predicate::operator()(
    soar_module::timer::timer_level val) {
  return (thisAgent->EpMem->epmem_params->timers->get_value() >= val);
}

//

epmem_timer::epmem_timer(const char* new_name, agent* new_agent,
...
void epmem_common_statement_container::create_graph_tables() {
  add_structure(
      "CREATE TABLE IF NOT EXISTS versions (system TEXT PRIMARY "
      "KEY,version_number TEXT)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_persistent_variables (variable_id "
      "INTEGER PRIMARY KEY,variable_value NONE)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_rit_left_nodes (rit_min INTEGER, "
      "rit_max INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_rit_right_nodes (rit_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_symbols_type (s_id INTEGER PRIMARY "
      "KEY, symbol_type INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_symbols_integer (s_id INTEGER PRIMARY "
      "KEY, symbol_value INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_symbols_float (s_id INTEGER PRIMARY "
      "KEY, symbol_value REAL)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_symbols_string (s_id INTEGER PRIMARY "
      "KEY, symbol_value TEXT)");
}

void epmem_common_statement_container::create_graph_indices() {
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS symbols_int_const ON "
      "epmem_symbols_integer (symbol_value)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS symbols_float_const ON "
      "epmem_symbols_float (symbol_value)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS symbols_str_const ON "
      "epmem_symbols_string (symbol_value)");
}

void epmem_common_statement_container::drop_graph_tables() {
  // Note: We don't want to dump versions database because it might also contain
  // other version information if we ever combine epmem and smem into one
  // database, which is something that has been discussed
...
epmem_common_statement_container::epmem_common_statement_container(
    agent* new_agent)
    : soar_module::sqlite_statement_container(new_agent->EpMem->epmem_db) {
  soar_module::sqlite_database* new_db = new_agent->EpMem->epmem_db;

  // Drop tables in the database if append setting is off.  (Tried DELETE
  // before, but it had problems.)
  if ((new_agent->EpMem->epmem_params->database->get_value() !=
       epmem_param_container::memory) &&
  ...
  add_structure(
      "INSERT OR REPLACE INTO versions (system, version_number) VALUES "
      "('epmem_schema'," EPMEM_SCHEMA_VERSION ")");

  // Add symbol lookups for special cases

  // Root node of tree
  // Note:  I don't think root node string is ever actually looked up.  Set to
  // root instead of
  //        previous NULL for compatibility with other db systems.
  add_structure(
      "INSERT OR IGNORE INTO epmem_symbols_type (s_id,symbol_type) VALUES "
      "(0,2)");
  add_structure(
      "INSERT OR IGNORE INTO epmem_symbols_string (s_id,symbol_value) VALUES "
      "(0,'root')");
  ...
  var_get = new soar_module::sqlite_statement(
      new_db,
      "SELECT variable_value FROM epmem_persistent_variables WHERE "
      "variable_id=?");
  add(var_get);

  var_set = new soar_module::sqlite_statement(
      new_db,
      "REPLACE INTO epmem_persistent_variables (variable_id,variable_value) "
      "VALUES (?,?)");
  add(var_set);

  //

  rit_add_left = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_rit_left_nodes (rit_min,rit_max) VALUES (?,?)");
  add(rit_add_left);

  rit_truncate_left = new soar_module::sqlite_statement(
      new_db, "DELETE FROM epmem_rit_left_nodes");
  add(rit_truncate_left);

  rit_add_right = new soar_module::sqlite_statement(
      new_db, "INSERT INTO epmem_rit_right_nodes (rit_id) VALUES (?)");
  add(rit_add_right);

  rit_truncate_right = new soar_module::sqlite_statement(
      new_db, "DELETE FROM epmem_rit_right_nodes");
  add(rit_truncate_right);

  //

  hash_rev_int = new soar_module::sqlite_statement(
      new_db, "SELECT symbol_value FROM epmem_symbols_integer WHERE s_id=?");
  add(hash_rev_int);

  hash_rev_float = new soar_module::sqlite_statement(
      new_db, "SELECT symbol_value FROM epmem_symbols_float WHERE s_id=?");
  add(hash_rev_float);

  hash_rev_str = new soar_module::sqlite_statement(
      new_db, "SELECT symbol_value FROM epmem_symbols_string WHERE s_id=?");
  add(hash_rev_str);

  hash_get_int = new soar_module::sqlite_statement(
      new_db, "SELECT s_id FROM epmem_symbols_integer WHERE symbol_value=?");
  add(hash_get_int);

  hash_get_float = new soar_module::sqlite_statement(
      new_db, "SELECT s_id FROM epmem_symbols_float WHERE symbol_value=?");
  add(hash_get_float);

  hash_get_str = new soar_module::sqlite_statement(
      new_db, "SELECT s_id FROM epmem_symbols_string WHERE symbol_value=?");
  add(hash_get_str);

  hash_get_type = new soar_module::sqlite_statement(
      new_db, "SELECT symbol_type FROM epmem_symbols_type WHERE s_id=?");
  add(hash_get_type);

  hash_add_type = new soar_module::sqlite_statement(
      new_db, "INSERT INTO epmem_symbols_type (symbol_type) VALUES (?)");
  add(hash_add_type);

  hash_add_int = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_symbols_integer (s_id,symbol_value) VALUES (?,?)");
  add(hash_add_int);

  hash_add_float = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_symbols_float (s_id,symbol_value) VALUES (?,?)");
  add(hash_add_float);

  hash_add_str = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_symbols_string (s_id,symbol_value) VALUES (?,?)");
  add(hash_add_str);
}

void epmem_graph_statement_container::create_graph_tables() {
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_nodes (n_id INTEGER, lti_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_episodes (episode_id INTEGER PRIMARY "
      "KEY)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_wmes_constant_now (wc_id "
      "INTEGER,start_episode_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_wmes_identifier_now (wi_id "
      "INTEGER,start_episode_id INTEGER, lti_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_wmes_constant_point (wc_id "
      "INTEGER,episode_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_wmes_identifier_point (wi_id "
      "INTEGER,episode_id INTEGER, lti_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_wmes_constant_range (rit_id "
      "INTEGER,start_episode_id INTEGER,end_episode_id INTEGER,wc_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_wmes_identifier_range (rit_id "
      "INTEGER,start_episode_id INTEGER,end_episode_id INTEGER,wi_id INTEGER, "
      "lti_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_wmes_constant (wc_id INTEGER PRIMARY "
      "KEY AUTOINCREMENT,parent_n_id INTEGER,attribute_s_id INTEGER, "
      "value_s_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_wmes_identifier (wi_id INTEGER PRIMARY "
      "KEY AUTOINCREMENT,parent_n_id INTEGER,attribute_s_id INTEGER,child_n_id "
      "INTEGER, last_episode_id INTEGER)");
  add_structure(
      "CREATE TABLE IF NOT EXISTS epmem_ascii (ascii_num INTEGER PRIMARY KEY, "
      "ascii_chr TEXT)");
}

void epmem_graph_statement_container::create_graph_indices() {
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS epmem_node_lti ON epmem_nodes (n_id, "
      "lti_id)");
  add_structure("CREATE INDEX IF NOT EXISTS epmem_lti ON epmem_nodes (lti_id)");

  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_constant_now_start ON "
      "epmem_wmes_constant_now (start_episode_id)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS epmem_wmes_constant_now_id_start ON "
      "epmem_wmes_constant_now (wc_id,start_episode_id DESC)");

  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_identifier_now_start ON "
      "epmem_wmes_identifier_now (start_episode_id)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS epmem_wmes_identifier_now_id_start ON "
      "epmem_wmes_identifier_now (wi_id,start_episode_id DESC)");

  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS epmem_wmes_constant_point_id_start ON "
      "epmem_wmes_constant_point (wc_id,episode_id DESC)");
  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_constant_point_start ON "
      "epmem_wmes_constant_point (episode_id)");

  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS epmem_wmes_identifier_point_id_start "
      "ON epmem_wmes_identifier_point (wi_id,episode_id DESC)");
  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_identifier_point_start ON "
      "epmem_wmes_identifier_point (episode_id)");

  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_constant_range_lower ON "
      "epmem_wmes_constant_range (rit_id,start_episode_id)");
  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_constant_range_upper ON "
      "epmem_wmes_constant_range (rit_id,end_episode_id)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS epmem_wmes_constant_range_id_start ON "
      "epmem_wmes_constant_range (wc_id,start_episode_id DESC)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS "
      "epmem_wmes_constant_range_id_end_start ON epmem_wmes_constant_range "
      "(wc_id,end_episode_id DESC,start_episode_id)");

  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_identifier_range_lower ON "
      "epmem_wmes_identifier_range (rit_id,start_episode_id)");
  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_identifier_range_upper ON "
      "epmem_wmes_identifier_range (rit_id,end_episode_id)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS epmem_wmes_identifier_range_id_start "
      "ON epmem_wmes_identifier_range (wi_id,start_episode_id DESC)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS "
      "epmem_wmes_identifier_range_id_end_start ON epmem_wmes_identifier_range "
      "(wi_id,end_episode_id DESC,start_episode_id)");

  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS "
      "epmem_wmes_constant_parent_attribute_value ON epmem_wmes_constant "
      "(parent_n_id,attribute_s_id,value_s_id)");

  add_structure(
      "CREATE INDEX IF NOT EXISTS epmem_wmes_identifier_parent_attribute_last "
      "ON epmem_wmes_identifier (parent_n_id,attribute_s_id,last_episode_id)");
  add_structure(
      "CREATE UNIQUE INDEX IF NOT EXISTS "
      "epmem_wmes_identifier_parent_attribute_child ON epmem_wmes_identifier "
...
epmem_graph_statement_container::epmem_graph_statement_container(
    agent* new_agent)
    : soar_module::sqlite_statement_container(new_agent->EpMem->epmem_db) {
  soar_module::sqlite_database* new_db = new_agent->EpMem->epmem_db;
  ...
  {
    add_structure(
        "INSERT OR IGNORE INTO epmem_ascii (ascii_num, ascii_chr) VALUES "
        "(65,'A')");
    add_structure(
        "INSERT OR IGNORE INTO epmem_ascii (ascii_num, ascii_chr) VALUES "
        "(66,'B')");
  ...
  add_time = new soar_module::sqlite_statement(
      new_db, "INSERT INTO epmem_episodes (episode_id) VALUES (?)");
  add(add_time);

  add_node = new soar_module::sqlite_statement(
      new_db, "INSERT INTO epmem_nodes (n_id,lti_id) VALUES (?,?)");
  add(add_node);

  update_node = new soar_module::sqlite_statement(
      new_db,
      "INSERT OR REPLACE INTO epmem_nodes (n_id, lti_id) "
      "VALUES (?,?)");  //"UPDATE epmem_nodes SET lti_id=?
                        // where n_id=?");
  add(update_node);

  add_epmem_wmes_constant_now = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_wmes_constant_now (wc_id,start_episode_id) VALUES "
      "(?,?)");
  add(add_epmem_wmes_constant_now);

  delete_epmem_wmes_constant_now = new soar_module::sqlite_statement(
      new_db, "DELETE FROM epmem_wmes_constant_now WHERE wc_id=?");
  add(delete_epmem_wmes_constant_now);

  add_epmem_wmes_constant_point = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_wmes_constant_point (wc_id,episode_id) VALUES (?,?)");
  add(add_epmem_wmes_constant_point);

  add_epmem_wmes_constant_range = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_wmes_constant_range "
      "(rit_id,start_episode_id,end_episode_id,wc_id) VALUES (?,?,?,?)");
  add(add_epmem_wmes_constant_range);

  add_epmem_wmes_constant = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_wmes_constant (parent_n_id,attribute_s_id,value_s_id) "
      "VALUES (?,?,?)");
  add(add_epmem_wmes_constant);

  find_epmem_wmes_constant = new soar_module::sqlite_statement(
      new_db,
      "SELECT wc_id FROM epmem_wmes_constant WHERE parent_n_id=? AND "
      "attribute_s_id=? AND value_s_id=?");
  add(find_epmem_wmes_constant);

  add_epmem_wmes_identifier_now = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_wmes_identifier_now (wi_id,start_episode_id,lti_id) "
      "VALUES (?,?,?)");
  add(add_epmem_wmes_identifier_now);

  delete_epmem_wmes_identifier_now = new soar_module::sqlite_statement(
      new_db, "DELETE FROM epmem_wmes_identifier_now WHERE wi_id=?");
  add(delete_epmem_wmes_identifier_now);

  add_epmem_wmes_identifier_point = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_wmes_identifier_point (wi_id,episode_id,lti_id) "
      "VALUES (?,?,?)");
  add(add_epmem_wmes_identifier_point);

  add_epmem_wmes_identifier_range = new soar_module::sqlite_statement(
      new_db,
      "INSERT INTO epmem_wmes_identifier_range "
      "(rit_id,start_episode_id,end_episode_id,wi_id,lti_id) VALUES "
      "(?,?,?,?,?)");
  add(add_epmem_wmes_identifier_range);

  add_epmem_wmes_identifier =
      new soar_module::sqlite_statement(new_db,
                                        "INSERT INTO epmem_wmes_identifier "
                                        "(parent_n_id,attribute_s_id,child_n_"
                                        "id,last_episode_id) VALUES (?,?,?,?)");
  add(add_epmem_wmes_identifier);

  find_epmem_wmes_identifier = new soar_module::sqlite_statement(
      new_db,
      "SELECT wi_id, child_n_id FROM epmem_wmes_identifier WHERE parent_n_id=? "
      "AND attribute_s_id=?");
  add(find_epmem_wmes_identifier);

  find_epmem_wmes_identifier_shared = new soar_module::sqlite_statement(
      new_db,
      "SELECT wi_id, child_n_id FROM epmem_wmes_identifier WHERE parent_n_id=? "
      "AND attribute_s_id=? AND child_n_id=?");
  add(find_epmem_wmes_identifier_shared);

  valid_episode = new soar_module::sqlite_statement(
      new_db, "SELECT COUNT(*) AS ct FROM epmem_episodes WHERE episode_id=?");
  add(valid_episode);

  next_episode = new soar_module::sqlite_statement(
      new_db,
      "SELECT episode_id FROM epmem_episodes WHERE episode_id>? ORDER BY "
      "episode_id ASC LIMIT 1");
  add(next_episode);

  prev_episode = new soar_module::sqlite_statement(
      new_db,
      "SELECT episode_id FROM epmem_episodes WHERE episode_id<? ORDER BY "
      "episode_id DESC LIMIT 1");
  add(prev_episode);

  get_wmes_with_constant_values = new soar_module::sqlite_statement(
      new_db,
      "SELECT f.wc_id, f.parent_n_id, f.attribute_s_id, f.value_s_id "
      "FROM epmem_wmes_constant f "
      "WHERE f.wc_id IN "
      "(SELECT n.wc_id FROM epmem_wmes_constant_now n WHERE "
      "n.start_episode_id<= ? UNION ALL "
      "SELECT p.wc_id FROM epmem_wmes_constant_point p WHERE p.episode_id=? "
      "UNION ALL "
      "SELECT e1.wc_id FROM epmem_wmes_constant_range e1, epmem_rit_left_nodes "
      "lt WHERE e1.rit_id=lt.rit_min AND e1.end_episode_id >= ? UNION ALL "
      "SELECT e2.wc_id FROM epmem_wmes_constant_range e2, "
      "epmem_rit_right_nodes rt WHERE e2.rit_id = rt.rit_id AND "
      "e2.start_episode_id <= ?) "
      "ORDER BY f.wc_id ASC",
      new_agent->EpMem->epmem_timers->ncb_node);
  add(get_wmes_with_constant_values);

  get_wmes_with_identifier_values = new soar_module::sqlite_statement(
      new_db,
      "WITH timetables AS ( "
      "SELECT n.wi_id, n.lti_id FROM epmem_wmes_identifier_now n WHERE "
      "n.start_episode_id<= ? UNION ALL "
      "SELECT p.wi_id, p.lti_id FROM epmem_wmes_identifier_point p WHERE "
      "p.episode_id = ? UNION ALL "
      "SELECT e1.wi_id, e1.lti_id FROM epmem_wmes_identifier_range e1, "
      "epmem_rit_left_nodes lt WHERE e1.rit_id=lt.rit_min AND "
      "e1.end_episode_id >= ? UNION ALL "
      "SELECT e2.wi_id, e2.lti_id FROM epmem_wmes_identifier_range e2, "
      "epmem_rit_right_nodes rt WHERE e2.rit_id = rt.rit_id AND "
      "e2.start_episode_id <= ?) "
      "SELECT f.parent_n_id, f.attribute_s_id, f.child_n_id, n.lti_id FROM "
      "epmem_wmes_identifier f, timetables n WHERE f.wi_id=n.wi_id "
      "ORDER BY f.parent_n_id ASC, f.child_n_id ASC",
      new_agent->EpMem->epmem_timers->ncb_edge);
  ...
  "(SELECT n.wi_id FROM epmem_wmes_identifier_now n WHERE n.start_episode_id<= ?
  UNION ALL " "SELECT p.wi_id FROM epmem_wmes_identifier_point p WHERE
  p.episode_id = ? UNION ALL " "SELECT e1.wi_id FROM epmem_wmes_identifier_range
  e1, epmem_rit_left_nodes lt WHERE e1.rit_id=lt.rit_min AND e1.end_episode_id
  >= ? UNION ALL " "SELECT e2.wi_id FROM epmem_wmes_identifier_range e2,
  epmem_rit_right_nodes rt WHERE e2.rit_id = rt.rit_id AND e2.start_episode_id
  <= ?) " "ORDER BY f.parent_n_id ASC, f.child_n_id ASC",
  new_agent->EpMem->epmem_timers->ncb_edge);*/
  ...
  update_epmem_wmes_identifier_last_episode_id =
      new soar_module::sqlite_statement(
          new_db,
          "UPDATE epmem_wmes_identifier SET last_episode_id=? WHERE wi_id=?");
  ...
  // init statement pools
  {
    ...
    const char* epmem_find_edge_queries[2][2] = {
        {"SELECT wc_id, value_s_id, ? FROM epmem_wmes_constant WHERE "
         "parent_n_id=? AND attribute_s_id=?",
         "SELECT wc_id, value_s_id, ? FROM epmem_wmes_constant WHERE "
         "parent_n_id=? AND attribute_s_id=? AND value_s_id=?"},
        {"SELECT wi_id, child_n_id, last_episode_id FROM epmem_wmes_identifier "
         "WHERE parent_n_id=? AND attribute_s_id=? AND ?<last_episode_id ORDER "
         "BY last_episode_id DESC",
         "SELECT wi_id, child_n_id, last_episode_id FROM epmem_wmes_identifier "
         "WHERE parent_n_id=? AND attribute_s_id=? AND child_n_id=? AND "
         "?<last_episode_id"}};

    for (j = EPMEM_RIT_STATE_NODE; j <= EPMEM_RIT_STATE_EDGE; j++) {
      for (k = 0; k <= 1; k++) {
        pool_find_edge_queries[j][k] = new soar_module::sqlite_statement_pool(
            new_agent, new_db, epmem_find_edge_queries[j][k]);
      }
    }
    ...
    const char* epmem_find_interval_queries[2][2][3] = {
        {{"SELECT (e.start_episode_id - 1) AS start FROM "
          "epmem_wmes_constant_range e WHERE e.wc_id=? AND "
          "e.start_episode_id<=? ORDER BY e.start_episode_id DESC",
          "SELECT (e.start_episode_id - 1) AS start FROM "
          "epmem_wmes_constant_now e WHERE e.wc_id=? AND e.start_episode_id<=? "
          "ORDER BY e.start_episode_id DESC",
          "SELECT (e.episode_id - 1) AS start FROM epmem_wmes_constant_point e "
          "WHERE e.wc_id=? AND e.episode_id<=? ORDER BY e.episode_id DESC"},
         {"SELECT e.end_episode_id AS end FROM epmem_wmes_constant_range e "
          "WHERE e.wc_id=? AND e.end_episode_id>0 AND e.start_episode_id<=? "
          "ORDER BY e.end_episode_id DESC",
          "SELECT ? AS end FROM epmem_wmes_constant_now e WHERE e.wc_id=? AND "
          "e.start_episode_id<=? ORDER BY e.start_episode_id DESC",
          "SELECT e.episode_id AS end FROM epmem_wmes_constant_point e WHERE "
          "e.wc_id=? AND e.episode_id<=? ORDER BY e.episode_id DESC"}},
        {{"SELECT (e.start_episode_id - 1) AS start FROM "
          "epmem_wmes_identifier_range e WHERE e.wi_id=? AND "
          "e.start_episode_id<=? ORDER BY e.start_episode_id DESC",
          "SELECT (e.start_episode_id - 1) AS start FROM "
          "epmem_wmes_identifier_now e WHERE e.wi_id=? AND "
          "e.start_episode_id<=? ORDER BY e.start_episode_id DESC",
          "SELECT (e.episode_id - 1) AS start FROM epmem_wmes_identifier_point "
          "e WHERE e.wi_id=? AND e.episode_id<=? ORDER BY e.episode_id DESC"},
         {"SELECT e.end_episode_id AS end FROM epmem_wmes_identifier_range e "
          "WHERE e.wi_id=? AND e.end_episode_id>0 AND e.start_episode_id<=? "
          "ORDER BY e.end_episode_id DESC",
          "SELECT ? AS end FROM epmem_wmes_identifier_now e WHERE e.wi_id=? "
          "AND e.start_episode_id<=? ORDER BY e.start_episode_id DESC",
          "SELECT e.episode_id AS end FROM epmem_wmes_identifier_point e WHERE "
          "e.wi_id=? AND e.episode_id<=? ORDER BY e.episode_id DESC"}},
    };
      ...
      for (k = EPMEM_RANGE_START; k <= EPMEM_RANGE_END; k++) {
        for (m = EPMEM_RANGE_EP; m <= EPMEM_RANGE_POINT; m++) {
          pool_find_interval_queries[j][k][m] =
              new soar_module::sqlite_statement_pool(
                  new_agent, new_db, epmem_find_interval_queries[j][k][m]);
        }
      }
    }
    ...
    pool_dummy = new soar_module::sqlite_statement_pool(new_agent, new_db,
                                                        "SELECT ? as start");
  }
}
...
inline void _epmem_process_buffered_wme_list(agent* thisAgent, Symbol* state,
                                             wme_set& cue_wmes,
                                             symbol_triple_list& my_list,
                                             preference_list* epmem_wmes) {
  if (my_list.empty()) {
    return;
  }

  instantiation* inst = make_architectural_instantiation_for_memory_system(
      thisAgent, state, &cue_wmes, &my_list, false);
  ...
  for (preference* pref = inst->preferences_generated; pref;) {
    ...
    if (add_preference_to_tm(thisAgent, pref)) {
      ...
      insert_at_head_of_dll(state->id->preferences_from_goal, pref,
  ...
  if (!epmem_wmes) {
    ...
    if (my_justification_list != NIL) {
      ...
      for (instantiation* my_justification = my_justification_list;
           my_justification != NIL; my_justification = next_justification) {
        next_justification = my_justification->next;

        if (my_justification->in_ms) {
          insert_at_head_of_dll(my_justification->prod->instantiations,
                                my_justification, next, prev);
        }

        for (just_pref = my_justification->preferences_generated;
             just_pref != NIL; just_pref = just_pref->inst_next) {
...
inline void epmem_process_buffered_wmes(agent* thisAgent, Symbol* state,
                                        wme_set& cue_wmes,
                                        symbol_triple_list& meta_wmes,
                                        symbol_triple_list& retrieval_wmes) {
  _epmem_process_buffered_wme_list(thisAgent, state, cue_wmes, meta_wmes,
                                   state->id->epmem_info->epmem_wmes);
  _epmem_process_buffered_wme_list(thisAgent, state, cue_wmes, retrieval_wmes,
                                   NULL);
}

inline void epmem_buffer_add_wme(agent* thisAgent, symbol_triple_list& my_list,
...
bool epmem_get_variable(agent* thisAgent, epmem_variable_key variable_id,
  ...
  soar_module::sqlite_statement* var_get =
      thisAgent->EpMem->epmem_stmts_common->var_get;
...
void epmem_set_variable(agent* thisAgent, epmem_variable_key variable_id,
  ...
  soar_module::sqlite_statement* var_set =
      thisAgent->EpMem->epmem_stmts_common->var_set;
...
int64_t epmem_rit_fork_node(int64_t lower, int64_t upper,
  ...
  int64_t step;
  for (step = (((node >= 0) ? (node) : (-1 * node)) / 2); step >= 1;
       step /= 2) {
    if (upper < node) {
      node -= step;
    } else if (node < lower) {
...
void epmem_rit_clear_left_right(agent* thisAgent) {
  thisAgent->EpMem->epmem_stmts_common->rit_truncate_left->execute(
      soar_module::op_reinit);
  thisAgent->EpMem->epmem_stmts_common->rit_truncate_right->execute(
      soar_module::op_reinit);
}
...
void epmem_rit_add_left(agent* thisAgent, epmem_time_id min,
  ...
  thisAgent->EpMem->epmem_stmts_common->rit_add_left->execute(
      soar_module::op_reinit);
}
...
void epmem_rit_insert_interval(agent* thisAgent, int64_t lower, int64_t upper,
  ...
  int64_t node;
  {
    ...
    if ((u < EPMEM_RIT_ROOT) && (l <= (2 * left_root))) {
      left_root = static_cast<int64_t>(
          pow(-2.0, floor(log(static_cast<double>(-l)) / EPMEM_LN_2)));
    ...
    if ((l > EPMEM_RIT_ROOT) && (u >= (2 * right_root))) {
      right_root = static_cast<int64_t>(
          pow(2.0, floor(log(static_cast<double>(u)) / EPMEM_LN_2)));
  ...
  // if (lti_id)
  // A horrible error was occuring where instead of defaulting to null in the
  // absense of a given lti_id, another value was used, which ended up
  // effectively assigning lti status to absurd things when they were recorded
  // into the history for episodic memory.
...
void epmem_clear_transient_structures(agent* thisAgent) {
  ...
  for (p = thisAgent->EpMem->epmem_id_repository->begin();
       p != thisAgent->EpMem->epmem_id_repository->end(); p++) {
  ...
  for (epmem_id_ref_counter::iterator rf_it =
           thisAgent->EpMem->epmem_id_ref_counts->begin();
       rf_it != thisAgent->EpMem->epmem_id_ref_counts->end(); rf_it++) {
    delete rf_it->second;
  }
  ...
  if (thisAgent->EpMem->epmem_db->get_status() == soar_module::connected) {
    if ((thisAgent->EpMem->epmem_params->database->get_value() ==
      ...
      if (thisAgent->EpMem->epmem_params->append_db->get_value() == off) {
        print_sysparam_trace(thisAgent, 0,
                             "Episodic memory re-initializing.\n");
      } else {
        print_sysparam_trace(
            thisAgent, 0,
            "Note: Episodic memory can currently only append to an an on-disk "
            "database.  Ignoring append = on.\n");
        print_sysparam_trace(thisAgent, 0,
                             "Episodic memory re-initializing.\n");
      }
    } else {
...
void epmem_switch_db_mode(agent* thisAgent, std::string& buf, bool readonly) {
  ...
  thisAgent->EpMem->epmem_db->disconnect();
  thisAgent->EpMem->epmem_params->database->set_value(
      epmem_param_container::memory);
...
void epmem_init_db(agent* thisAgent, bool readonly) {
  if (thisAgent->EpMem->epmem_db->get_status() != soar_module::disconnected) {
    print_sysparam_trace(thisAgent, TRACE_EPMEM_SYSPARAM,
                         "Cannot initialize episodic memory database.  It is "
                         "already connected!");
    return;
  }
  ...
  if (thisAgent->EpMem->epmem_db->get_status() == soar_module::problem) {
    ...
    // If the database is on file, make sure the database contents use the
    // current schema If it does not, switch to memory-based database

    if (strcmp(db_path, ":memory:"))  // Only worry about database version if
                                      // writing to disk
    {
      ...
      if (thisAgent->EpMem->epmem_db->sql_is_new_db(sql_is_new)) {
        if (sql_is_new) {
          ...
          print_sysparam_trace(thisAgent, TRACE_EPMEM_SYSPARAM,
                               "...episodic memory database is new.\n");
        } else {
          // Check if table exists already
          temp_q = new soar_module::sqlite_statement(
              thisAgent->EpMem->epmem_db,
              "CREATE TABLE IF NOT EXISTS versions (system TEXT PRIMARY "
              "KEY,version_number TEXT)");
          temp_q->prepare();
          if (temp_q->get_status() == soar_module::ready) {
            if (thisAgent->EpMem->epmem_db->sql_simple_get_string(
                    "SELECT version_number FROM versions WHERE system = "
                    "'epmem_schema'",
                    schema_version)) {
              if (schema_version !=
                  EPMEM_SCHEMA_VERSION)  // Incompatible version
              {
                version_error_message.assign(
                    "...Error:  Cannot load episodic memory database with "
                    "schema version ");
                version_error_message.append(schema_version.c_str());
                version_error_message.append(
                    ".\n...Please convert old database or start a new database "
                    "by "
              ...
              } else  // Version is OK
              {
                print_sysparam_trace(
                    thisAgent, TRACE_EPMEM_SYSPARAM,
                    "...version of episodic memory database ok.\n");
                switch_to_memory = false;
              }

            } else  // Some sort of error reading version info from version
                    // database
            {
              version_error_message.assign(
                  "...Error:  Cannot read version number from file-based "
                  "episodic memory database.\n"
                  "...Switching to memory-based database.\n");
            }
          } else  // Non-empty database exists with no version table.  Probably
                  // schema 1.0
          {
            version_error_message.assign(
                "...Error:  Cannot load an episodic memory database with an "
                "old schema version.\n...Please convert "
                "old database or start a new database by setting a new "
                "database file path.\n...Switching "
        ...
        version_error_message.assign(
            "...Error:  Cannot read database meta info from file-based "
            "episodic memory database.\n"
            "...Switching to memory-based database.\n");
      }
      if (switch_to_memory) {
        // Memory mode will be set on, database will be disconnected to and then
        // init_db will be called again to reinitialize database.
    ...
    // apply performance options
    {
      ...
      // optimization
      if (thisAgent->EpMem->epmem_params->opt->get_value() ==
          epmem_param_container::opt_speed) {
        // synchronous - don't wait for writes to complete (can corrupt the db
        // in case unexpected crash during transaction)
        thisAgent->EpMem->epmem_db->sql_execute("PRAGMA synchronous = OFF");

        // journal_mode - no atomic transactions (can result in database
        // corruption if crash during transaction)
        thisAgent->EpMem->epmem_db->sql_execute("PRAGMA journal_mode = OFF");

        // locking_mode - no one else can view the database after our first
        // write
        thisAgent->EpMem->epmem_db->sql_execute(
    ...
    thisAgent->EpMem->epmem_stmts_common =
        new epmem_common_statement_container(thisAgent);
    thisAgent->EpMem->epmem_stmts_common->structure();
    thisAgent->EpMem->epmem_stmts_common->prepare();

    {
      // setup graph structures/queries
      thisAgent->EpMem->epmem_stmts_graph =
          new epmem_graph_statement_container(thisAgent);
      ...
      (*thisAgent->EpMem->epmem_id_repository)[EPMEM_NODEID_ROOT] =
          new epmem_hashed_id_pool;
      {
#ifdef USE_MEM_POOL_ALLOCATORS
        epmem_wme_set* wms_temp = new epmem_wme_set(
            std::less<wme*>(), soar_module::soar_memory_pool_allocator<wme*>());
#else
        epmem_wme_set* wms_temp = new epmem_wme_set();
#endif

        // voigtjr: Cast to wme* is necessary for compilation in VS10
        // Without it, it picks insert(int) instead of insert(wme*) and does not
        // compile.
      ...
      thisAgent->EpMem->epmem_stats->next_id->set_value(1);
      {
        ...
        if (epmem_get_variable(thisAgent, var_next_id, &stored_id)) {
          ...
          epmem_set_variable(
              thisAgent, var_next_id,
              thisAgent->EpMem->epmem_stats->next_id->get_value());
        }
      }

      // initialize rit state
      for (int i = EPMEM_RIT_STATE_NODE; i <= EPMEM_RIT_STATE_EDGE; i++) {
        thisAgent->EpMem->epmem_rit_state_graph[i].offset.stat->set_value(
            EPMEM_RIT_OFFSET_INIT);
        ...
        thisAgent->EpMem->epmem_rit_state_graph[i].minstep.stat->set_value(
            LONG_MAX);
      }
      thisAgent->EpMem->epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].add_query =
          thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant_range;
      ...
      // get/set RIT variables
      {
        ...
        for (int i = EPMEM_RIT_STATE_NODE; i <= EPMEM_RIT_STATE_EDGE; i++) {
          // offset
          if (epmem_get_variable(
                  thisAgent,
                  thisAgent->EpMem->epmem_rit_state_graph[i].offset.var_key,
                  &var_val)) {
            thisAgent->EpMem->epmem_rit_state_graph[i].offset.stat->set_value(
                var_val);
          } else {
            epmem_set_variable(
                thisAgent,
                thisAgent->EpMem->epmem_rit_state_graph[i].offset.var_key,
                thisAgent->EpMem->epmem_rit_state_graph[i]
                    .offset.stat->get_value());
          }

          // leftroot
          if (epmem_get_variable(
                  thisAgent,
                  thisAgent->EpMem->epmem_rit_state_graph[i].leftroot.var_key,
                  &var_val)) {
            thisAgent->EpMem->epmem_rit_state_graph[i].leftroot.stat->set_value(
                var_val);
          } else {
            epmem_set_variable(
                thisAgent,
                thisAgent->EpMem->epmem_rit_state_graph[i].leftroot.var_key,
                thisAgent->EpMem->epmem_rit_state_graph[i]
                    .leftroot.stat->get_value());
          }

          // rightroot
          if (epmem_get_variable(
                  thisAgent,
                  thisAgent->EpMem->epmem_rit_state_graph[i].rightroot.var_key,
                  &var_val)) {
            thisAgent->EpMem->epmem_rit_state_graph[i]
                .rightroot.stat->set_value(var_val);
          } else {
            epmem_set_variable(
                thisAgent,
                thisAgent->EpMem->epmem_rit_state_graph[i].rightroot.var_key,
                thisAgent->EpMem->epmem_rit_state_graph[i]
                    .rightroot.stat->get_value());
          }

          // minstep
          if (epmem_get_variable(
                  thisAgent,
                  thisAgent->EpMem->epmem_rit_state_graph[i].minstep.var_key,
                  &var_val)) {
            thisAgent->EpMem->epmem_rit_state_graph[i].minstep.stat->set_value(
                var_val);
          } else {
      ...
      // get max time
      {
        temp_q = new soar_module::sqlite_statement(
            thisAgent->EpMem->epmem_db,
            "SELECT MAX(episode_id) FROM epmem_episodes");
        temp_q->prepare();
        if (temp_q->execute() == soar_module::row) {
          thisAgent->EpMem->epmem_stats->time->set_value(temp_q->column_int(0) +
                                                         1);
        }
      ...
      if (!readonly) {
        ...
        const char* now_select[] = {
            "SELECT wc_id,start_episode_id FROM epmem_wmes_constant_now",
            "SELECT wi_id,start_episode_id,lti_id FROM "
            "epmem_wmes_identifier_now"};
        soar_module::sqlite_statement* now_add[] = {
            thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant_point,
            thisAgent->EpMem->epmem_stmts_graph
                ->add_epmem_wmes_identifier_point};
        const char* now_delete[] = {"DELETE FROM epmem_wmes_constant_now",
                                    "DELETE FROM epmem_wmes_identifier_now"};

        for (int i = EPMEM_RIT_STATE_NODE; i <= EPMEM_RIT_STATE_EDGE; i++) {
          temp_q = now_add[i];
          temp_q->bind_int(2, time_last);

          temp_q2 = new soar_module::sqlite_statement(
              thisAgent->EpMem->epmem_db, now_select[i]);
          temp_q2->prepare();
          while (temp_q2->execute() == soar_module::row) {
            ...
            if (range_start == time_last) {
              ...
              epmem_rit_insert_interval(
                  thisAgent, range_start, time_last, temp_q2->column_int(0),
                  &(thisAgent->EpMem->epmem_rit_state_graph[i]),
                  temp_q2->column_int(2));
            }

            if (i == EPMEM_RIT_STATE_EDGE) {
              thisAgent->EpMem->epmem_stmts_graph
                  ->update_epmem_wmes_identifier_last_episode_id->bind_int(
                      1, time_last);
              thisAgent->EpMem->epmem_stmts_graph
                  ->update_epmem_wmes_identifier_last_episode_id->bind_int(
                      2, temp_q2->column_int(0));
              thisAgent->EpMem->epmem_stmts_graph
                  ->update_epmem_wmes_identifier_last_episode_id->execute(
                      soar_module::op_reinit);
            }
          }
          delete temp_q2;
          ...
          temp_q = new soar_module::sqlite_statement(thisAgent->EpMem->epmem_db,
                                                     now_delete[i]);
          temp_q->prepare();
          temp_q->execute();
          delete temp_q;
      ...
      // get max id + max list
      {
        const char* minmax_select[] = {
            "SELECT MAX(wc_id) FROM epmem_wmes_constant",
            "SELECT MAX(wi_id) FROM epmem_wmes_identifier"};
        std::vector<bool>* minmax_max[] = {thisAgent->EpMem->epmem_node_maxes,
                                           thisAgent->EpMem->epmem_edge_maxes};
        std::vector<epmem_time_id>* minmax_min[] = {
            thisAgent->EpMem->epmem_node_mins,
            thisAgent->EpMem->epmem_edge_mins};
        ...
        for (int i = EPMEM_RIT_STATE_NODE; i <= EPMEM_RIT_STATE_EDGE; i++) {
          ...
          if (temp_q->column_type(0) != soar_module::null_t) {
            std::vector<bool>::size_type num_ids =
                static_cast<size_t>(temp_q->column_int(0));
      ...
      // get id pools
      {
        ...
        temp_q = new soar_module::sqlite_statement(
            thisAgent->EpMem->epmem_db,
            "SELECT parent_n_id, attribute_s_id, child_n_id, wi_id FROM "
            "epmem_wmes_identifier");
        temp_q->prepare();
    ...
    // if lazy commit, then we encapsulate the entire lifetime of the agent in a
    // single transaction
    if (thisAgent->EpMem->epmem_params->lazy_commit->get_value() == on) {
      thisAgent->EpMem->epmem_stmts_common->begin->execute(
          soar_module::op_reinit);
    }
  }
...
/* **************************************************************************
   ...
   - The identifier symbol of each wme isn't set and is ignored. parent_id
     ...
     2. value unknown in phase one, but known at phase two (try assignment
adhering to constraint)
     3. value unknown in phase one/two (if anything is left, unconstrained
assignment)

************************************************************************** */

inline void _epmem_store_level(
    agent* thisAgent, std::queue<Symbol*>& parent_syms,
    std::queue<epmem_node_id>& parent_ids, tc_number tc,
    epmem_wme_list::iterator w_b, epmem_wme_list::iterator w_e,
    epmem_node_id parent_id, epmem_time_id time_counter,
    std::map<wme*, epmem_id_reservation*>& id_reservations,
    std::set<Symbol*>& new_identifiers, std::queue<epmem_node_id>& epmem_node,
  ...
  fprintf(stderr,
          "==================================================\nDEBUG "
          "_epmem_store_level called for parent_id "
          "%d\n==================================================\n",
  ...
  for (w_p = w_b; w_p != w_e; w_p++) {
    //      #ifdef DEBUG_EPMEM_WME_ADD
    //      fprintf(stderr, "DEBUG epmem.2132: _epmem_store_level processing wme
    //      (types: %d %d %d)\n",
    //              (*w_p)->id->symbol_type,  (*w_p)->attr->var->symbol_type,
    //              (*w_p)->value->symbol_type);
    //      #endif
    ...
    if (((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) &&
      ...
      fprintf(stderr,
              ...
              "^%s %s\n",
              (unsigned int)parent_id,
              symbol_to_string(thisAgent, (*w_p)->attr, true, NIL, 0),
              symbol_to_string(thisAgent, (*w_p)->value, true, NIL, 0));
#endif
      ...
      if ((*w_p)->acceptable) {
        ...
        new_id_reservation->my_hash =
            epmem_temporal_hash(thisAgent, (*w_p)->attr);
      }

      // try to find appropriate reservation
      my_id_repo = &(*(
          *thisAgent->EpMem
               ->epmem_id_repository)[parent_id])[new_id_reservation->my_hash];
      if ((*my_id_repo)) {
#ifdef DEBUG_EPMEM_WME_ADD
        fprintf(stderr, "   id repository exists.  reserving id\n");
#endif
        for (pool_p = (*my_id_repo)->begin(); pool_p != (*my_id_repo)->end();
             pool_p++) {
...
#ifdef DEBUG_EPMEM_WME_ADD
        fprintf(stderr,
                "   no id repository found.  creating new id repository.\n");
#endif
        // add repository
  ...
  for (w_p = w_b; w_p != w_e; w_p++) {
    ...
    if ((*w_p)->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) {
      ...
      // if the value already has an epmem_id, the WME ID would have already
      // been assigned above (ie. the epmem_id of the VALUE is KNOWN APRIORI
      // [sic]) however, it's also possible that the value is known but no WME
      // ID is given (eg. (<s> ^foo <a> ^bar <a>)); this is case 2
      value_known_apriori =
          (((*w_p)->value->id->epmem_id != EPMEM_NODEID_BAD) &&
           ((*w_p)->value->id->epmem_valid ==
            thisAgent->EpMem->epmem_validation));
      ...
      if ((*w_p)->value->id->LTI_ID &&
          ((*w_p)->value->id->LTI_epmem_valid !=
           thisAgent->EpMem->epmem_validation) &&
          ((*w_p)->value->id->epmem_id != EPMEM_NODEID_BAD)) {
        // Update the node database with the new lti_id
        thisAgent->EpMem->epmem_stmts_graph->update_node->bind_int(
            2, (*w_p)->value->id->LTI_ID);
        thisAgent->EpMem->epmem_stmts_graph->update_node->bind_int(
            1, (*w_p)->value->id->epmem_id);
        thisAgent->EpMem->epmem_stmts_graph->update_node->execute(
            soar_module::op_reinit);
        (*w_p)->value->id->LTI_epmem_valid = thisAgent->EpMem->epmem_validation;
      }  // Steven - This is an important point - It's where we know we're
         // adding an lti for which there wasn't already a known epmem id
         // correspondence assigned to it.
      {
        ...
        if (value_known_apriori) {
          ...
          if (r_p != id_reservations.end()) {
            ...
            if (r_p->second->my_id != EPMEM_NODEID_BAD) {
              ...
              (*thisAgent->EpMem->epmem_id_replacement)[(*w_p)->epmem_id] =
                  my_id_repo2;
#ifdef DEBUG_EPMEM_WME_ADD
          ...
          // OR a shared identifier at the same level, in which case we need an
          // exact match (case 2)
          else {
#ifdef DEBUG_EPMEM_WME_ADD
            fprintf(stderr,
                    "   No reservation found.  Looking for shared identifier "
                    "at same level.\n");
#endif
            // get temporal hash
            if ((*w_p)->acceptable) {
            ...
            my_id_repo = &(
                *(*thisAgent->EpMem->epmem_id_repository)[parent_id])[my_hash];
            if ((*my_id_repo)) {
              if (!(*my_id_repo)->empty()) {
                for (pool_p = (*my_id_repo)->begin();
                     pool_p != (*my_id_repo)->end(); pool_p++) {
                  if (pool_p->first == (*w_p)->value->id->epmem_id) {
                    (*w_p)->epmem_id = pool_p->second;
                    (*my_id_repo)->erase(pool_p);
                    (*thisAgent->EpMem
                          ->epmem_id_replacement)[(*w_p)->epmem_id] =
                        (*my_id_repo);
#ifdef DEBUG_EPMEM_WME_ADD
                    fprintf(stderr, "   Assigning id from existing pool: %d\n",
                            (unsigned int)(*w_p)->epmem_id);
        ...
        // case 3
          ...
          if ((*my_id_repo)) {
            ...
            if (!(*my_id_repo)->empty()) {
              for (pool_p = (*my_id_repo)->begin();
                   pool_p != (*my_id_repo)->end(); pool_p++) {
                // the ref set for this epmem_id may not be there if the pools
                // were regenerated from a previous DB a non-existant ref set is
                // the equivalent of a ref count of 0 (ie. an empty ref set) so
                // we allow the identifier from the pool to be used
                if ((thisAgent->EpMem->epmem_id_ref_counts->count(
                         pool_p->first) == 0) ||
                    ((*thisAgent->EpMem->epmem_id_ref_counts)[pool_p->first]
                         ->empty()))

                {
                  ...
                  fprintf(
                      stderr,
                      "   Found unused id. Setting wme id for VALUE to %d\n",
      ...
      if ((*w_p)->epmem_id == EPMEM_NODEID_BAD) {
...
#endif
        // can't use value_known_apriori, since value may have been assigned
        // (lti, id repository via case 3)
        if (((*w_p)->value->id->epmem_id == EPMEM_NODEID_BAD) ||
          ...
          thisAgent->EpMem->epmem_stats->next_id->set_value(
              (*w_p)->value->id->epmem_id + 1);
          epmem_set_variable(thisAgent, var_next_id,
                             (*w_p)->value->id->epmem_id + 1);

#ifdef DEBUG_EPMEM_WME_ADD
          fprintf(stderr,
                  "   Adding new n_id and setting wme id for VALUE to %d\n",
                  (unsigned int)(*w_p)->value->id->epmem_id);
#endif
          ...
          (*thisAgent->EpMem
                ->epmem_id_repository)[(*w_p)->value->id->epmem_id] =
              new epmem_hashed_id_pool;

          // add ref set
#ifdef USE_MEM_POOL_ALLOCATORS
          (*thisAgent->EpMem
                ->epmem_id_ref_counts)[(*w_p)->value->id->epmem_id] =
              new epmem_wme_set(
                  std::less<wme*>(),
...
#endif
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier
            ->bind_int(1, parent_id);
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier
            ->bind_int(2, my_hash);
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier
            ->bind_int(3, (*w_p)->value->id->epmem_id);
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier
            ->bind_int(4, LLONG_MAX);
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier->execute(
            soar_module::op_reinit);

        (*w_p)->epmem_id = static_cast<epmem_node_id>(
            thisAgent->EpMem->epmem_db->last_insert_rowid());
#ifdef DEBUG_EPMEM_WME_ADD
        ...
        epmem_edge.emplace((*w_p)->epmem_id,
                           static_cast<int64_t>((*w_p)->value->id->is_lti()
                                                    ? (*w_p)->value->id->LTI_ID
                                                    : 0));
        ...
        (*thisAgent->EpMem->epmem_edge_removals)[std::make_pair(
            (*w_p)->epmem_id,
            static_cast<int64_t>(
                (*w_p)->value->id->is_lti() ? (*w_p)->value->id->LTI_ID : 0))] =
            false;

        // we add ONLY if the last thing we did was remove
        if ((*thisAgent->EpMem->epmem_edge_maxes)[static_cast<size_t>(
                (*w_p)->epmem_id - 1)]) {
          epmem_edge.emplace(
              (*w_p)->epmem_id,
              static_cast<int64_t>(
                  (*w_p)->value->id->is_lti() ? (*w_p)->value->id->LTI_ID : 0));
          (*thisAgent->EpMem
                ->epmem_edge_maxes)[static_cast<size_t>((*w_p)->epmem_id - 1)] =
              false;
        }
      }
      ...
      if (new_identifiers.find((*w_p)->value) != new_identifiers.end()) {
        // because we could have bypassed the ref set before, we need to create
        // it here
        if (thisAgent->EpMem->epmem_id_ref_counts->count(
                (*w_p)->value->id->epmem_id) == 0) {
#ifdef USE_MEM_POOL_ALLOCATORS
          (*thisAgent->EpMem
                ->epmem_id_ref_counts)[(*w_p)->value->id->epmem_id] =
              new epmem_wme_set(
        ...
        (*thisAgent->EpMem->epmem_id_ref_counts)[(*w_p)->value->id->epmem_id]
            ->insert((*w_p));
#ifdef DEBUG_EPMEM_WME_ADD
        fprintf(stderr,
                "   increasing ref_count of value in %d %d %d; new ref_count "
                "is %d\n",
                (unsigned int)(*w_p)->id->id->epmem_id,
                (unsigned int)epmem_temporal_hash(thisAgent, (*w_p)->attr),
                (unsigned int)(*w_p)->value->id->epmem_id,
                (unsigned int)(*thisAgent->EpMem->epmem_id_ref_counts)
                    [(*w_p)->value->id->epmem_id]
                        ->size());
#endif
      }
      ...
      if (((*w_p)->epmem_id == EPMEM_NODEID_BAD) ||
        ...
        // try to get node id
        {
...
#ifdef DEBUG_EPMEM_WME_ADD
          fprintf(stderr,
                  "   Looking for id of a duplicate entry in "
                  "epmem_wmes_constant.\n");
#endif
          thisAgent->EpMem->epmem_stmts_graph->find_epmem_wmes_constant
              ->bind_int(1, parent_id);
          thisAgent->EpMem->epmem_stmts_graph->find_epmem_wmes_constant
              ->bind_int(2, my_hash);
          thisAgent->EpMem->epmem_stmts_graph->find_epmem_wmes_constant
          ...
          if (thisAgent->EpMem->epmem_stmts_graph->find_epmem_wmes_constant
                  ->execute() == soar_module::row) {
            (*w_p)->epmem_id = thisAgent->EpMem->epmem_stmts_graph
                                   ->find_epmem_wmes_constant->column_int(0);
          }
        ...
        if ((*w_p)->epmem_id == EPMEM_NODEID_BAD) {
#ifdef DEBUG_EPMEM_WME_ADD
          fprintf(stderr,
                  "   No duplicate wme found in epmem_wmes_constant.  Adding "
                  "wme to table!!!!\n");
          fprintf(stderr, "   Performing database insertion: %d %d %d\n",
                  (unsigned int)parent_id, (unsigned int)my_hash,
                  (unsigned int)my_hash2);
#endif
          // insert (parent_n_id, attribute_s_id, value_s_id)
          thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant
              ->bind_int(1, parent_id);
          thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant
              ->bind_int(2, my_hash);
          thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant
              ->bind_int(3, my_hash2);
          thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant->execute(
              soar_module::op_reinit);

          (*w_p)->epmem_id =
              (epmem_node_id)thisAgent->EpMem->epmem_db->last_insert_rowid();
#ifdef DEBUG_EPMEM_WME_ADD
...
#ifdef DEBUG_EPMEM_WME_ADD
          fprintf(stderr,
                  "   Node found in database, definitely don't remove.\n");
          ...
          if ((*thisAgent->EpMem->epmem_node_maxes)[static_cast<size_t>(
                  (*w_p)->epmem_id - 1)]) {
            epmem_node.push((*w_p)->epmem_id);
            (*thisAgent->EpMem->epmem_node_maxes)[static_cast<size_t>(
                (*w_p)->epmem_id - 1)] = false;
          }
        }
      }
    }
...
void epmem_new_episode(agent* thisAgent) {
  ...
  print_sysparam_trace(thisAgent, TRACE_EPMEM_SYSPARAM,
                       "New episodic memory recorded for time %u.\n",
  ...
  // perform storage
  {
    ...
    std::queue<std::pair<epmem_node_id, int64_t>>
        epmem_edge;  // epmem_edge now needs to keep track of the lti
                     // status/identity of the wmenode/epmemedge
    ...
    // walk appropriate levels
    {
      ...
      for (epmem_symbol_set::iterator id_p =
               thisAgent->EpMem->epmem_wme_adds->begin();
           id_p != thisAgent->EpMem->epmem_wme_adds->end(); id_p++) {
        // make sure the WME is valid
        // it can be invalid if a child WME was added, but then the parent was
        // removed, setting the epmem_id to EPMEM_NODEID_BAD
        if ((*id_p)->id->epmem_id != EPMEM_NODEID_BAD) {
          ...
          while (!parent_syms.empty()) {
            ...
            if (!wmes->empty()) {
              _epmem_store_level(thisAgent, parent_syms, parent_ids, tc,
                                 wmes->begin(), wmes->end(), parent_id,
                                 time_counter, id_reservations, new_identifiers,
                                 epmem_node, epmem_edge);
            }
            delete wmes;
          }
        }
    ...
    // all inserts
    {
      ...
      while (!epmem_node.empty()) {
        ...
        // add NOW entry
        // id = ?, start_episode_id = ?
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant_now
            ->bind_int(1, (*temp_node));
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant_now
            ->bind_int(2, time_counter);
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_constant_now
            ->execute(soar_module::op_reinit);

        // update min
        (*thisAgent->EpMem
              ->epmem_node_mins)[static_cast<size_t>((*temp_node) - 1)] =
            time_counter;
      ...
      while (
          !epmem_edge
               .empty()) {  // For the identifiers that are lti instances and
                            // for which they previously were not stored with
                            // the lti metadata they currently possess, we need
                            // to make a new interval. The lti instance metadata
                            // will be stored as a field on the interval. The
                            // most recent lti will be treated as the "default"
                            // and splitting of intervals will happen only if a
                            // change from the most recent is detected. It will
                            // function similarly to a removal and replacement
                            // with respect to the relational interval tree.
        temp_node = &epmem_edge.front().first;
        lti_id = &epmem_edge.front().second;

        // add NOW entry
        // id = ?, start_episode_id = ?
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier_now
            ->bind_int(1, (*temp_node));
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier_now
            ->bind_int(2, time_counter);
        thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier_now
        ...
        // update min
        (*thisAgent->EpMem
              ->epmem_edge_mins)[static_cast<size_t>((*temp_node) - 1)] =
            time_counter;

        thisAgent->EpMem->epmem_stmts_graph
            ->update_epmem_wmes_identifier_last_episode_id->bind_int(1,
                                                                     LLONG_MAX);
        thisAgent->EpMem->epmem_stmts_graph
            ->update_epmem_wmes_identifier_last_episode_id->bind_int(
                2, *temp_node);
    ...
    // all removals
    {
      ...
      // wme's with constant values
      {
        ...
        while (r != thisAgent->EpMem->epmem_node_removals->end()) {
          if (r->second) {
            // remove NOW entry
            // id = ?
            thisAgent->EpMem->epmem_stmts_graph->delete_epmem_wmes_constant_now
                ->bind_int(1, r->first);
            thisAgent->EpMem->epmem_stmts_graph->delete_epmem_wmes_constant_now
                ->execute(soar_module::op_reinit);
            ...
            range_start =
                (*thisAgent->EpMem
                      ->epmem_node_mins)[static_cast<size_t>(r->first - 1)];
            ...
            // node
              ...
              epmem_rit_insert_interval(
                  thisAgent, range_start, range_end, r->first,
                  &(thisAgent->EpMem
            ...
            // update max
            (*thisAgent->EpMem
                  ->epmem_node_maxes)[static_cast<size_t>(r->first - 1)] = true;
          }
      ...
      epmem_edge_removal_map::iterator r =
          thisAgent->EpMem->epmem_edge_removals->begin();
      while (r != thisAgent->EpMem->epmem_edge_removals->end()) {
        if (r->second) {
          // remove NOW entry
          // id = ?
          thisAgent->EpMem->epmem_stmts_graph->delete_epmem_wmes_identifier_now
              ->bind_int(1, r->first.first);
          thisAgent->EpMem->epmem_stmts_graph->delete_epmem_wmes_identifier_now
              ->execute(soar_module::op_reinit);

          range_start =
              (*thisAgent->EpMem
                    ->epmem_edge_mins)[static_cast<size_t>(r->first.first - 1)];
          range_end = (time_counter - 1);
          ...
          // point (id, start_episode_id)
          if (range_start == range_end) {
            thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier_point
                ->bind_int(1, r->first.first);
            thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier_point
                ->bind_int(2, range_start);
            thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier_point
                ->bind_int(3, r->first.second);
            thisAgent->EpMem->epmem_stmts_graph->add_epmem_wmes_identifier_point
                ->execute(soar_module::op_reinit);
          }
          // node
            ...
            epmem_rit_insert_interval(
                thisAgent, range_start, range_end, r->first.first,
                &(thisAgent->EpMem
                      ->epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE]),
                r->first.second);
          }

          // update max
          (*thisAgent->EpMem
                ->epmem_edge_maxes)[static_cast<size_t>(r->first.first - 1)] =
              true;
        }
    ...
    thisAgent->EpMem->epmem_stmts_graph->add_time->execute(
        soar_module::op_reinit);
    ...
    // update time wme on all states
    {
      ...
      Symbol* my_time_sym =
          thisAgent->symbolManager->make_int_constant(time_counter + 1);

      while (state != NULL) {
        if (state->id->epmem_info->epmem_time_wme != NIL) {
          soar_module::remove_module_wme(thisAgent,
                                         state->id->epmem_info->epmem_time_wme);
        }

        state->id->epmem_info->epmem_time_wme = soar_module::add_module_wme(
            thisAgent, state->id->epmem_info->epmem_link_wme->value,
...
bool epmem_valid_episode(agent* thisAgent, epmem_time_id memory_id) {
...

  {
    soar_module::sqlite_statement* my_q =
        thisAgent->EpMem->epmem_stmts_graph->valid_episode;
...
inline void _epmem_install_id_wme(
    agent* thisAgent, Symbol* parent, Symbol* attr,
    std::map<epmem_node_id, std::pair<Symbol*, bool>>* ids,
    epmem_node_id child_n_id, uint64_t val_num, epmem_id_mapping* id_record,
    symbol_triple_list& retrieval_wmes) {
    ...
    if (val_num) {
      id_p = ids
                 ->insert(std::make_pair(
                     child_n_id,
                     std::make_pair(
                         thisAgent->symbolManager->make_new_identifier(
                             ((attr->symbol_type == STR_CONSTANT_SYMBOL_TYPE)
                                  ? (attr->sc->name[0])
                                  : ('L')),
                             parent->id->level),
                         true)))
      ...
      if (thisAgent->SMem->lti_exists(val_num)) {
        ...
        id_p->second.first->id->LTI_epmem_valid =
            thisAgent->EpMem->epmem_validation;
      }
    } else {
      id_p = ids
  ...
  epmem_buffer_add_wme(thisAgent, retrieval_wmes, parent, attr,
                       id_p->second.first);
...
void epmem_install_memory(agent* thisAgent, Symbol* state,
  ...
  if ((memory_id == EPMEM_MEMID_NONE) ||
    ...
    epmem_buffer_add_wme(
        thisAgent, meta_wmes, result_header,
        thisAgent->symbolManager->soarSymbols.epmem_sym_retrieved,
  ...
  retrieved_header = thisAgent->symbolManager->make_new_identifier(
      'R', result_header->id->level);
  if (id_record) {
  ...
  // add *-id wme's
  {
    ...
    my_meta = thisAgent->symbolManager->make_int_constant(
        static_cast<int64_t>(memory_id));
    epmem_buffer_add_wme(
        thisAgent, meta_wmes, result_header,
        thisAgent->symbolManager->soarSymbols.epmem_sym_memory_id, my_meta);
    thisAgent->symbolManager->symbol_remove_ref(&my_meta);

    my_meta = thisAgent->symbolManager->make_int_constant(
        static_cast<int64_t>(thisAgent->EpMem->epmem_stats->time->get_value()));
    epmem_buffer_add_wme(
        thisAgent, meta_wmes, result_header,
        thisAgent->symbolManager->soarSymbols.epmem_sym_present_id, my_meta);
    thisAgent->symbolManager->symbol_remove_ref(&my_meta);
  }
  ...
  // install memory
  {
    ...
    //
    // Because of shared WMEs at different levels of the storage breadth-first
    // search, there is the possibility that the unique database id of an
    // identifier can be greater than that of its parent.  Because the retrieval
    // query sorts by unique id ascending, it is thus possible to have an
    // "orphan" - a child with no current parent.  We keep track of orphans and
    // add them later, hoping their parents have shown up.  I *suppose* there
    // could be a really evil case in which the ordering of the unique ids is
    // exactly opposite of their insertion order. I just hope this isn't a
    // common case...

    // shared identifier lookup table
    std::map<epmem_node_id, std::pair<Symbol*, bool>> ids;
    bool dont_abide_by_ids_second =
        (thisAgent->EpMem->epmem_params->merge->get_value() ==
         epmem_param_container::merge_add);
    ...
    my_q = thisAgent->EpMem->epmem_stmts_graph->get_wmes_with_identifier_values;
    {
      ...
      // the bool in the pair refers to if children are allowed on this id (re:
      // lti)
      std::map<epmem_node_id, std::pair<Symbol*, bool>>::iterator id_p;
      ...
      while (my_q->execute() == soar_module::row) {
        ...
        val_lti_id = (my_q->column_type(3) == soar_module::null_t
                          ? 0
                          : static_cast<uint64_t>(my_q->column_int(3)));
        ...
        if (id_p != ids.end()) {
          ...
          if (dont_abide_by_ids_second || id_p->second.second) {
            _epmem_install_id_wme(thisAgent, id_p->second.first, attr, &(ids),
                                  child_n_id, val_lti_id, id_record,
                                  retrieval_wmes);
            num_wmes++;
          }
      ...
      if (!orphans.empty()) {
        ...
        do {
          ...
          while (!orphans.empty()) {
            ...
            if (id_p != ids.end()) {
              if (dont_abide_by_ids_second || id_p->second.second) {
                _epmem_install_id_wme(thisAgent, id_p->second.first,
                                      orphan->attribute, &(ids),
                                      orphan->child_n_id, orphan->child_lti_id,
                                      id_record, retrieval_wmes);
                num_wmes++;
              }
    ...
    my_q = thisAgent->EpMem->epmem_stmts_graph->get_wmes_with_constant_values;
    {
      ...
      while (my_q->execute() == soar_module::row) {
        ...
        if (dont_abide_by_ids_second || parent.second) {
          ...
          epmem_buffer_add_wme(thisAgent, retrieval_wmes, parent.first, attr,
                               value);
          num_wmes++;
...
epmem_time_id epmem_next_episode(agent* thisAgent, epmem_time_id memory_id) {
  ...
  if (memory_id != EPMEM_MEMID_NONE) {
    soar_module::sqlite_statement* my_q =
        thisAgent->EpMem->epmem_stmts_graph->next_episode;
    my_q->bind_int(1, memory_id);
...
epmem_time_id epmem_previous_episode(agent* thisAgent,
  ...
  if (memory_id != EPMEM_MEMID_NONE) {
    soar_module::sqlite_statement* my_q =
        thisAgent->EpMem->epmem_stmts_graph->prev_episode;
    my_q->bind_int(1, memory_id);
    if (my_q->execute() == soar_module::row) {
      return_val = (epmem_time_id)my_q->column_int(0);
    }
...
void epmem_print_retrieval_state(epmem_wme_literal_map& literals,
  ...
  for (epmem_wme_literal_map::iterator lit_iter = literals.begin();
       lit_iter != literals.end(); lit_iter++) {
    ...
    if (literal->id_sym) {
      ...
      std::cout << "];" << std::endl;
      std::cout << "\"" << literal->id_sym << "\" -> \"" << literal->value_sym
                << "\" [label=\"";
  ...
  for (int type = EPMEM_RIT_STATE_NODE; type <= EPMEM_RIT_STATE_EDGE; type++) {
    ...
    for (epmem_triple_uedge_map::iterator uedge_iter = uedge_cache->begin();
         uedge_iter != uedge_cache->end(); uedge_iter++) {
      epmem_triple triple = (*uedge_iter).first;
      if (triple.child_n_id != EPMEM_NODEID_ROOT) {
        if (type == EPMEM_RIT_STATE_NODE) {
          std::cout << "\"n" << triple.child_n_id << "\" [shape=\"rect\"];"
                    << std::endl;
        }
        std::cout << "\"e" << triple.parent_n_id << "\" -> \""
                  << (type == EPMEM_RIT_STATE_NODE ? "n" : "e")
                  << triple.child_n_id << "\" [label=\""
                  << triple.attribute_s_id << "\"];" << std::endl;
      }
    }
  }
  std::cout << "};" << std::endl;
  ...
  for (int type = EPMEM_RIT_STATE_NODE; type <= EPMEM_RIT_STATE_EDGE; type++) {
    for (epmem_triple_pedge_map::iterator pedge_iter =
             pedge_caches[type].begin();
         pedge_iter != pedge_caches[type].end(); pedge_iter++) {
      ...
      if (triple.attribute_s_id != EPMEM_NODEID_BAD) {
        ...
        std::cout << "];" << std::endl;
        for (epmem_literal_set::iterator lit_iter = pedge->literals.begin();
             lit_iter != pedge->literals.end(); lit_iter++) {
          epmem_literal* literal = *lit_iter;
          std::cout << "\"" << literal->value_sym << "\" -> \"" << pedge
                    << "\";" << std::endl;
        }
  ...
  for (int type = EPMEM_RIT_STATE_NODE; type <= EPMEM_RIT_STATE_EDGE; type++) {
    ...
    for (epmem_triple_uedge_map::iterator uedge_iter = uedge_cache->begin();
      ...
      if (triple.attribute_s_id != EPMEM_NODEID_BAD) {
        for (epmem_pedge_set::iterator pedge_iter = uedge->pedges.begin();
             pedge_iter != uedge->pedges.end(); pedge_iter++) {
          epmem_pedge* pedge = *pedge_iter;
          std::pair<epmem_pedge*, epmem_node_id> pair =
              std::make_pair(pedge, triple.parent_n_id);
          if (!drawn.count(pair)) {
            drawn.insert(pair);
            std::cout << "\"" << pedge << "\" -> \"e" << triple.parent_n_id
                      << "\";" << std::endl;
          }
          std::cout << "\"" << pedge << "\" -> \""
                    << (pedge->value_is_id ? "e" : "n") << triple.child_n_id
                    << "\" [style=\"dashed\"];" << std::endl;
          std::pair<std::multimap<epmem_node_id, epmem_pedge*>::iterator,
                    std::multimap<epmem_node_id, epmem_pedge*>::iterator>
              pedge_iters = parent_pedge_map.equal_range(triple.child_n_id);
          for (std::multimap<epmem_node_id, epmem_pedge*>::iterator pedge_iter =
                   pedge_iters.first;
               pedge_iter != pedge_iters.second; pedge_iter++) {
            std::cout << "\"" << pedge << "\" -> \"" << (*pedge_iter).second
                      << "\";" << std::endl;
          }
        }
      }
...
epmem_literal* epmem_build_dnf(wme* cue_wme,
                               ...
                               epmem_literal_deque& gm_ordering, int query_type,
                               std::set<Symbol*>& visiting, wme_set& cue_wmes,
                               agent* thisAgent) {
  // if the value is being visited, this is part of a loop; return NULL
  // remove this check (and in fact, the entire visiting parameter) if cyclic
  // cues are allowed
  if (visiting.count(cue_wme->value)) {
    return NULL;
  }
  // if the value is an identifier and we've been here before, we can return the
  // previous literal
  ...
  //{
  //  This is an attempt to reintegrate matching of ltis into epmem queries
  //  despite the change to make ltis into instances. The idea is that ltis
  //  require both a structure match like normal identifiers, and also a direct
  //  match of the lti in question. Treating ltis purely like normal identifiers
   ...
   * instance-based.
   */
  // The first step is to find the LTI at all in the first place. If it's not
  // present, we can just return failure.
  ...
  else  // WME is a normal identifier
  {
    ...
    epmem_wme_list* children =
        epmem_get_augs_of_id(value, get_new_tc_number(thisAgent));
    ...
    if (children->empty()) {
      ...
      for (epmem_wme_list::iterator wme_iter = children->begin();
           wme_iter != children->end(); wme_iter++) {
        // check to see if this child forms a cycle
        // if it does, we skip over it
        epmem_literal* child = epmem_build_dnf(
            *wme_iter, literal_cache, leaf_literals, symbol_num_incoming,
            gm_ordering, query_type, visiting, cue_wmes, thisAgent);
        if (child) {
      ...
      // if all children of this WME lead to cycles, then we don't need to walk
      // this path in essence, this forces the DNF graph to be acyclic this
      // results in savings in not walking edges and intervals
  ...
  if (query_type == EPMEM_NODE_POS) {
    ...
    thisAgent->EpMem->epmem_stats->qry_pos->set_value(
        thisAgent->EpMem->epmem_stats->qry_pos->get_value() + 1);
  } else {
    thisAgent->EpMem->epmem_stats->qry_neg->set_value(
        thisAgent->EpMem->epmem_stats->qry_neg->get_value() + 1);
  }
  ...
  literal->weight =
      (literal->is_neg_q ? -1 : 1) *
      (thisAgent->EpMem->epmem_params->balance->get_value() >= 1.0 - 1.0e-8
           ? 1.0
           : wma_get_wme_activation(thisAgent, cue_wme, true));
#ifdef USE_MEM_POOL_ALLOCATORS
  new (&(literal->matches)) epmem_node_pair_set(
      std::less<epmem_node_pair>(),
      soar_module::soar_memory_pool_allocator<epmem_node_pair>());
#else
  new (&(literal->matches)) epmem_node_pair_set();
#endif
...
bool epmem_register_pedges(epmem_node_id parent, epmem_literal* literal,
                           epmem_pedge_pq& pedge_pq, epmem_time_id after,
                           epmem_triple_pedge_map pedge_caches[],
                           epmem_triple_uedge_map uedge_caches[],
                           agent* thisAgent) {
  // we don't need to keep track of visited literals/nodes because the literals
  // are guaranteed to be acyclic that is, the expansion to the literal's
  // children will eventually bottom out select the query
  epmem_triple triple = {parent, literal->attribute_s_id, literal->child_n_id};
  int is_edge = literal->value_is_id;
  if (QUERY_DEBUG >= 1) {
    std::cout << "		RECURSING ON " << parent << " " << literal
              << std::endl;
  }
  ...
  if (pedge_iter == pedge_cache->end() || (*pedge_iter).second == NULL) {
    ...
    soar_module::pooled_sqlite_statement* pedge_sql =
        thisAgent->EpMem->epmem_stmts_graph
            ->pool_find_edge_queries[is_edge][has_value]
            ->request(thisAgent->EpMem->epmem_timers->query_sql_edge);
    int bind_pos = 1;
    if (!is_edge) {
    ...
    if (pedge_sql->execute() == soar_module::row) {
      thisAgent->memoryManager->allocate_with_pool(MP_epmem_pedge,
                                                   &child_pedge);
    ...
    if (!child_pedge->literals.count(literal)) {
      child_pedge->literals.insert(literal);
      // if the literal is an edge with no specified value, add the literal to
      // all potential pedges
      if (!literal->is_leaf && literal->child_n_id == EPMEM_NODEID_BAD) {
        bool created = false;
        epmem_triple_uedge_map* uedge_cache = &uedge_caches[is_edge];
        for (epmem_triple_uedge_map::iterator uedge_iter =
                 uedge_cache->lower_bound(triple);
             uedge_iter != uedge_cache->end(); uedge_iter++) {
          ...
          if (child_triple.child_n_id != EPMEM_NODEID_BAD &&
              child_uedge->value_is_id) {
            for (epmem_literal_set::iterator child_iter =
                     literal->children.begin();
                 child_iter != literal->children.end(); child_iter++) {
              created |= epmem_register_pedges(
                  child_triple.child_n_id, *child_iter, pedge_pq, after,
                  pedge_caches, uedge_caches, thisAgent);
            }
          }
        }
        return created;
...
bool epmem_satisfy_literal(epmem_literal* literal, epmem_node_id parent,
                           epmem_node_id child, double& current_score,
  ...
  if (QUERY_DEBUG >= 1) {
    std::cout << "		RECURSING ON " << parent << " " << child << " "
              << literal << std::endl;
  }
  ...
  if (!parents_satisfied) {
    ...
    match_iter =
        symbol_node_count.find(std::make_pair(literal->id_sym, parent));
    // since, by definition, if a node satisfies all incoming literals, all
    // incoming literals are satisfied
    parents_satisfied = (match_iter != symbol_node_count.end()) &&
                        ((*match_iter).second == num_incoming);
  }
  // if yes
  if (parents_satisfied) {
    ...
    if (values_iter == literal->values.end()) {
      ...
      if (literal->is_leaf) {
        if (literal->matches.size() == 1) {
          ...
          if (QUERY_DEBUG >= 1) {
            std::cout << "			NEW SCORE: " << current_score
        ...
        epmem_symbol_node_pair match =
            std::make_pair(literal->value_sym, child);
        ...
        for (epmem_literal_set::iterator child_iter = literal->children.begin();
             child_iter != literal->children.end(); child_iter++) {
          epmem_literal* child_lit = *child_iter;
          epmem_triple_uedge_map* uedge_cache =
              &uedge_caches[child_lit->value_is_id];
          epmem_triple child_triple = {child, child_lit->attribute_s_id,
                                       child_lit->child_n_id};
          ...
          if (child_lit->child_n_id == EPMEM_NODEID_BAD) {
            ...
            while (uedge_iter != uedge_cache->end()) {
              ...
              if (child_uedge->activated &&
                  child_uedge->activation_count == 1) {
                changed_score |= epmem_satisfy_literal(
                    child_lit, child_triple.parent_n_id,
                    child_triple.child_n_id, current_score, current_cardinality,
                    symbol_node_count, uedge_caches, symbol_num_incoming);
              }
              uedge_iter++;
            }
          } else {
...
bool epmem_unsatisfy_literal(
    epmem_literal* literal, epmem_node_id parent, epmem_node_id child,
    double& current_score, long int& current_cardinality,
    epmem_symbol_node_pair_int_map& symbol_node_count) {
  ...
  epmem_node_pair_set::iterator lit_match_iter =
      literal->matches.find(std::make_pair(parent, child));
    ...
    if ((*values_iter).second == 0) {
      ...
      if (literal->is_leaf) {
        ...
        epmem_symbol_node_pair match =
            std::make_pair(literal->value_sym, child);
        epmem_symbol_node_pair_int_map::iterator match_iter =
            symbol_node_count.find(match);
        (*match_iter).second--;
        if ((*match_iter).second == 0) {
          symbol_node_count.erase(match_iter);
        }
        // if this literal is no longer satisfied, recurse on all children
        // if this literal is still satisfied, recurse on children who is
        // matching on descendants of this edge
        if (literal->matches.size() == 0) {
          for (epmem_literal_set::iterator child_iter =
                   literal->children.begin();
               child_iter != literal->children.end(); child_iter++) {
            epmem_literal* child_lit = *child_iter;
            epmem_node_pair_set::iterator node_iter =
                child_lit->matches.begin();
            while (node_iter != child_lit->matches.end()) {
              changed_score |= epmem_unsatisfy_literal(
                  child_lit, (*node_iter).first, (*node_iter).second,
                  current_score, current_cardinality, symbol_node_count);
          ...
          for (epmem_literal_set::iterator child_iter =
            ...
            epmem_literal* child_lit = *child_iter;
            epmem_node_pair_set::iterator node_iter =
                child_lit->matches.lower_bound(node_pair);
            if (node_iter != child_lit->matches.end() &&
                (*node_iter).first == child) {
              changed_score |= epmem_unsatisfy_literal(
                  child_lit, (*node_iter).first, (*node_iter).second,
                  current_score, current_cardinality, symbol_node_count);
            }
...
bool epmem_graph_match(epmem_literal_deque::iterator& dnf_iter,
                       ...
                       epmem_node_symbol_map bound_nodes[], agent* thisAgent,
                       int depth = 0) {
  ...
  epmem_node_set failed_parents =
      epmem_node_set(std::less<epmem_node_id>(),
                     soar_module::soar_memory_pool_allocator<epmem_node_id>());
  ...
  for (epmem_node_pair_set::iterator match_iter = literal->matches.begin();
       match_iter != literal->matches.end(); match_iter++) {
    ...
    // for all parents
    for (epmem_literal_set::iterator parent_iter = literal->parents.begin();
         relations_okay && parent_iter != literal->parents.end();
         parent_iter++) {
      epmem_literal* parent = *parent_iter;
      epmem_literal_node_pair_map::iterator bind_iter = bindings.find(parent);
      if (bind_iter != bindings.end() &&
          (*bind_iter).second.second != parent_n_id) {
    ...
    // if the node has already been bound, make sure it's bound to the same
    // thing
    epmem_node_symbol_map::iterator binder =
        bound_nodes[literal->value_is_id].find(child_n_id);
    if (binder != bound_nodes[literal->value_is_id].end() &&
        (*binder).second != literal->value_sym) {
    ...
    if (QUERY_DEBUG >= 2) {
      ...
      std::cout << "TRYING " << literal << " " << parent_n_id << " "
                << child_n_id << std::endl;
    }
    if (literal->child_n_id != EPMEM_NODEID_BAD &&
        literal->child_n_id != child_n_id) {
      relations_okay = false;
    }
    // for all children
    for (epmem_literal_set::iterator child_iter = literal->children.begin();
         relations_okay && child_iter != literal->children.end();
         child_iter++) {
      epmem_literal* child = *child_iter;
      epmem_literal_node_pair_map::iterator bind_iter = bindings.find(child);
      if (bind_iter != bindings.end() &&
          (*bind_iter).second.first != child_n_id) {
        relations_okay = false;
      }
    }
    if (!relations_okay) {
    ...
    if (QUERY_DEBUG >= 2) {
      ...
      std::cout << literal << " " << parent_n_id << " " << child_n_id
                << std::endl;
    }
    ...
    bool list_satisfied = epmem_graph_match(next_iter, iter_end, bindings,
                                            bound_nodes, thisAgent, depth + 1);
  ...
  // this means we've tried everything and this whole exercise was a waste of
  // time EPIC FAIL
  if (QUERY_DEBUG >= 2) {
    for (int i = 0; i < depth; i++) {
      std::cout << "\t";
...
void epmem_process_query(agent* thisAgent, Symbol* state, Symbol* pos_query,
                         Symbol* neg_query, epmem_time_list& prohibits,
                         epmem_time_id before, epmem_time_id after,
                         wme_set& cue_wmes, symbol_triple_list& meta_wmes,
                         symbol_triple_list& retrieval_wmes, int level = 3) {
  // a query must contain a positive cue
  if (pos_query == NULL) {
    epmem_buffer_add_wme(
        thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
        thisAgent->symbolManager->soarSymbols.epmem_sym_status,
        thisAgent->symbolManager->soarSymbols.epmem_sym_bad_cmd);
    return;
  }

  // before and after, if specified, must be valid relative to each other
  if (before != EPMEM_MEMID_NONE && after != EPMEM_MEMID_NONE &&
      before <= after) {
    epmem_buffer_add_wme(
        thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
        thisAgent->symbolManager->soarSymbols.epmem_sym_status,
  ...
  // epmem options
  bool do_graph_match =
      (thisAgent->EpMem->epmem_params->graph_match->get_value() == on);
  epmem_param_container::gm_ordering_choices gm_order =
      thisAgent->EpMem->epmem_params->gm_ordering->get_value();
  ...
  epmem_triple_uedge_map uedge_caches[2] = {
      epmem_triple_uedge_map(
          std::less<epmem_triple>(),
          soar_module::soar_memory_pool_allocator<
              std::pair<const epmem_triple, epmem_uedge*>>()),
      epmem_triple_uedge_map(
          std::less<epmem_triple>(),
          soar_module::soar_memory_pool_allocator<
              std::pair<const epmem_triple, epmem_uedge*>>())};
  epmem_interval_set interval_cleanup = epmem_interval_set(
      std::less<epmem_interval*>(),
      soar_module::soar_memory_pool_allocator<epmem_interval*>());
#else
  ...
  epmem_symbol_int_map symbol_num_incoming;  // number of literals with a
                                             // certain symbol as its value
  epmem_symbol_node_pair_int_map
      symbol_node_count;  // number of times a symbol is matched by a node
  ...
  if (level > 1) {
    // build the DNF graph while checking for leaf WMEs
    {
      ...
      new (&(root_literal->matches)) epmem_node_pair_set(
          std::less<epmem_node_pair>(),
          soar_module::soar_memory_pool_allocator<epmem_node_pair>(thisAgent));
#else
      new (&(root_literal->matches)) epmem_node_pair_set();
#endif
      ...
      for (int query_type = EPMEM_NODE_POS; query_type <= EPMEM_NODE_NEG;
           query_type++) {
        ...
        epmem_wme_list* children =
            epmem_get_augs_of_id(query_root, get_new_tc_number(thisAgent));
      ...
      thisAgent->EpMem->epmem_stats->qry_lits->set_value(
          thisAgent->EpMem->epmem_stats->qry_pos->get_value() +
          thisAgent->EpMem->epmem_stats->qry_neg->get_value());
    }

    // calculate the highest possible score and cardinality score
    double perfect_score = 0;
    int perfect_cardinality = 0;
    for (epmem_literal_set::iterator iter = leaf_literals.begin();
         iter != leaf_literals.end(); iter++) {
      if (!(*iter)->is_neg_q) {
    ...
    // create dummy edges and intervals
    {
      ...
      // we make an SQL statement just so we don't have to do anything special
      // at cleanup
      epmem_triple triple = {EPMEM_NODEID_BAD, EPMEM_NODEID_BAD,
                             EPMEM_NODEID_ROOT};
      ...
      root_pedge->sql =
          thisAgent->EpMem->epmem_stmts_graph->pool_dummy->request();
      ...
      thisAgent->memoryManager->allocate_with_pool(MP_epmem_interval,
                                                   &root_interval);
    ...
    while (pedge_pq.size() && current_episode > after) {
      ...
      while (pedge_pq.size() && (pedge_pq.top()->time == next_edge ||
                                 pedge_pq.top()->time >= current_episode)) {
        ...
        if (QUERY_DEBUG >= 1) {
          std::cout << "	EDGE " << triple.parent_n_id << "-"
                    << triple.attribute_s_id << "-" << triple.child_n_id
                    << std::endl;
        }
        ...
        if (pedge->value_is_id) {
          bool created = false;
          for (epmem_literal_set::iterator literal_iter =
                   pedge->literals.begin();
               literal_iter != pedge->literals.end(); literal_iter++) {
            epmem_literal* literal = *literal_iter;
            for (epmem_literal_set::iterator child_iter =
                     literal->children.begin();
                 child_iter != literal->children.end(); child_iter++) {
              created |= epmem_register_pedges(triple.child_n_id, *child_iter,
                                               pedge_pq, after, pedge_caches,
                                               uedge_caches, thisAgent);
            }
          }
        }
        // TODO JUSTIN what I want to do here is, if there is no children which
        // leads to a leaf, retract everything I'm not sure how to properly test
        // for this though

        // look for uedge with triple; if none exist, create one
        // otherwise, link up the uedge with the pedge and consider score
        // changes
        ...
        if (uedge_iter == uedge_cache->end()) {
          ...
          for (int interval_type = EPMEM_RANGE_EP;
               interval_type <= EPMEM_RANGE_POINT; interval_type++) {
            for (int point_type = EPMEM_RANGE_START;
                 point_type <= EPMEM_RANGE_END; point_type++) {
              ...
              switch (interval_type) {
                case EPMEM_RANGE_EP:
                  if (point_type == EPMEM_RANGE_START) {
                    sql_timer =
                        thisAgent->EpMem->epmem_timers->query_sql_start_ep;
                  } else {
                    sql_timer =
                        thisAgent->EpMem->epmem_timers->query_sql_end_ep;
                  }
                  break;
                case EPMEM_RANGE_NOW:
                  if (point_type == EPMEM_RANGE_START) {
                    sql_timer =
                        thisAgent->EpMem->epmem_timers->query_sql_start_now;
                  } else {
                    sql_timer =
                        thisAgent->EpMem->epmem_timers->query_sql_end_now;
                  }
                  break;
                case EPMEM_RANGE_POINT:
                  if (point_type == EPMEM_RANGE_START) {
                    sql_timer =
                        thisAgent->EpMem->epmem_timers->query_sql_start_point;
                  } else {
                    sql_timer =
                        thisAgent->EpMem->epmem_timers->query_sql_end_point;
                  }
                  break;
              }
              // create the SQL query and bind it
              // try to find an existing query first; if none exist, allocate a
              // new one from the memory pools
              soar_module::pooled_sqlite_statement* interval_sql = NULL;
              interval_sql =
                  thisAgent->EpMem->epmem_stmts_graph
                      ->pool_find_interval_queries[pedge->value_is_id]
                                                  [point_type][interval_type]
                      ->request(sql_timer);
              int bind_pos = 1;
              if (point_type == EPMEM_RANGE_END &&
                  interval_type == EPMEM_RANGE_NOW) {
              ...
              if (interval_sql->execute() == soar_module::row) {
                ...
                // If it's an start point of a range (ie. not a point) and it's
                // before the promo time (this is possible if a the promotion is
                // in the middle of a range) trim it to the promo time. This
                // will only happen if the LTI is promoted in the last interval
                // it appeared in (since otherwise the start point would not be
                // before its promotion). We don't care about the remaining
          ...
          if (uedge->activated && uedge->activation_count == 1) {
            for (epmem_literal_set::iterator lit_iter = pedge->literals.begin();
              ...
              epmem_literal* literal = (*lit_iter);
              changed_score |= epmem_satisfy_literal(
                  literal, triple.parent_n_id, triple.child_n_id, current_score,
                  current_cardinality, symbol_node_count, uedge_caches,
                  symbol_num_incoming);
            }
          }
        }
      ...
      while (interval_pq.size() && interval_pq.top()->time > next_edge &&
        ...
        while (interval_pq.size() &&
               interval_pq.top()->time >= current_episode) {
          ...
          if (QUERY_DEBUG >= 1) {
            std::cout << "	INTERVAL ("
                      << (interval->is_end_point ? "end" : "start")
          ...
          if (interval->is_end_point) {
            ...
            if (uedge->activation_count == 1) {
              for (epmem_pedge_set::iterator pedge_iter = uedge->pedges.begin();
                   pedge_iter != uedge->pedges.end(); pedge_iter++) {
                epmem_pedge* pedge = *pedge_iter;
                for (epmem_literal_set::iterator lit_iter =
                         pedge->literals.begin();
                     lit_iter != pedge->literals.end(); lit_iter++) {
                  epmem_literal* literal = *lit_iter;
                  changed_score |= epmem_satisfy_literal(
                      literal, triple.parent_n_id, triple.child_n_id,
                      current_score, current_cardinality, symbol_node_count,
            ...
            for (epmem_pedge_set::iterator pedge_iter = uedge->pedges.begin();
              ...
              for (epmem_literal_set::iterator lit_iter =
                   ...
                   lit_iter != pedge->literals.end(); lit_iter++) {
                changed_score |= epmem_unsatisfy_literal(
                    *lit_iter, triple.parent_n_id, triple.child_n_id,
                    current_score, current_cardinality, symbol_node_count);
              }
            }
          }
          // put the interval query back into the queue if there's more and some
          // literal cares otherwise, reinitialize the query and put it in a
          // pool
          if (interval->sql && interval->sql->execute() == soar_module::row) {
            ...
            if (uedge->intervals) {
              ...
              thisAgent->memoryManager->free_with_pool(MP_epmem_interval,
                                                       interval);
            } else {
        ...
        while (prohibits.size() && current_episode > next_episode &&
               current_episode == prohibits.back()) {
          current_episode--;
          prohibits.pop_back();
        }

        if (QUERY_DEBUG >= 2) {
          epmem_print_retrieval_state(literal_cache, pedge_caches,
                                      uedge_caches);
        }

        print_sysparam_trace(
            thisAgent, TRACE_EPMEM_SYSPARAM,
            "Considering episode (time, cardinality, score) (%u, %u, %f)\n",
            static_cast<uint64_t>(current_episode),
            static_cast<uint64_t>(current_cardinality), current_score);
        ...
        if (current_episode > next_episode && changed_score &&
            (best_episode == EPMEM_MEMID_NONE || current_score > best_score ||
             (do_graph_match && current_score == best_score &&
              !best_graph_matched))) {
          bool new_king = false;
          ...
          // we should graph match if the option is set and all leaf literals
          // are satisfied
          if (current_cardinality == perfect_cardinality) {
            ...
            if (do_graph_match) {
              ...
              graph_matched = epmem_graph_match(begin, end, best_bindings,
                                                bound_nodes, thisAgent, 2);
          ...
          if (new_king && thisAgent->trace_settings[TRACE_EPMEM_SYSPARAM]) {
            char buf[256];
            SNPRINTF(
                buf, 254, "NEW KING (perfect, graph-match): (%s, %s)\n",
                (current_cardinality == perfect_cardinality ? "true" : "false"),
                (best_graph_matched ? "true" : "false"));
    ...
    if (best_episode == EPMEM_MEMID_NONE) {
      epmem_buffer_add_wme(
          thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
          thisAgent->symbolManager->soarSymbols.epmem_sym_failure, pos_query);
      if (neg_query) {
        epmem_buffer_add_wme(
            thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
            thisAgent->symbolManager->soarSymbols.epmem_sym_failure, neg_query);
      }
    } else {
      ...
      // cue size
      temp_sym =
          thisAgent->symbolManager->make_int_constant(leaf_literals.size());
      epmem_buffer_add_wme(
          thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
          thisAgent->symbolManager->soarSymbols.epmem_sym_cue_size, temp_sym);
      thisAgent->symbolManager->symbol_remove_ref(&temp_sym);
      // match cardinality
      temp_sym = thisAgent->symbolManager->make_int_constant(best_cardinality);
      epmem_buffer_add_wme(
          thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
          thisAgent->symbolManager->soarSymbols.epmem_sym_match_cardinality,
          temp_sym);
      thisAgent->symbolManager->symbol_remove_ref(&temp_sym);
      // match score
      ...
      epmem_buffer_add_wme(
          thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
          thisAgent->symbolManager->soarSymbols.epmem_sym_match_score,
          temp_sym);
      thisAgent->symbolManager->symbol_remove_ref(&temp_sym);
      // normalized match score
      temp_sym = thisAgent->symbolManager->make_float_constant(best_score /
                                                               perfect_score);
      epmem_buffer_add_wme(thisAgent, meta_wmes,
                           state->id->epmem_info->result_wme->value,
                           thisAgent->symbolManager->soarSymbols
      ...
      epmem_buffer_add_wme(
          thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
          thisAgent->symbolManager->soarSymbols.epmem_sym_success, pos_query);
      if (neg_query) {
        epmem_buffer_add_wme(
            thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
            thisAgent->symbolManager->soarSymbols.epmem_sym_success, neg_query);
      }
      // give more metadata if graph match is turned on
      if (do_graph_match) {
        // graph match
        temp_sym = thisAgent->symbolManager->make_int_constant(
            (best_graph_matched ? 1 : 0));
        epmem_buffer_add_wme(
            thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
            thisAgent->symbolManager->soarSymbols.epmem_sym_graph_match,
            temp_sym);
        thisAgent->symbolManager->symbol_remove_ref(&temp_sym);
        ...
        if (best_graph_matched) {
          goal_stack_level level =
              state->id->epmem_info->result_wme->value->id->level;
          // mapping identifier
          Symbol* mapping =
              thisAgent->symbolManager->make_new_identifier('M', level);
          epmem_buffer_add_wme(thisAgent, meta_wmes,
                               state->id->epmem_info->result_wme->value,
                               thisAgent->symbolManager->soarSymbols
                                   .epmem_sym_graph_match_mapping,
                               mapping);
          thisAgent->symbolManager->symbol_remove_ref(&mapping);

          for (epmem_literal_node_pair_map::iterator iter =
                   best_bindings.begin();
               iter != best_bindings.end(); iter++) {
            if ((*iter).first->value_is_id) {
              // create the node
              temp_sym =
                  thisAgent->symbolManager->make_new_identifier('N', level);
              epmem_buffer_add_wme(thisAgent, meta_wmes, mapping,
                                   thisAgent->symbolManager->soarSymbols
                                       .epmem_sym_graph_match_mapping_node,
                                   temp_sym);
              thisAgent->symbolManager->symbol_remove_ref(&temp_sym);
              // point to the cue identifier
              epmem_buffer_add_wme(thisAgent, meta_wmes, temp_sym,
                                   thisAgent->symbolManager->soarSymbols
                                       .epmem_sym_graph_match_mapping_cue,
                                   (*iter).first->value_sym);
      ...
      if (level > 2) {
        epmem_install_memory(thisAgent, state, best_episode, meta_wmes,
                             retrieval_wmes, &node_mem_map);
      }
      if (best_graph_matched) {
        for (epmem_id_mapping::iterator iter = node_mem_map.begin();
             iter != node_mem_map.end(); iter++) {
          epmem_id_mapping::iterator map_iter =
              node_map_map.find((*iter).first);
          if (map_iter != node_map_map.end() && (*iter).second) {
            epmem_buffer_add_wme(
                thisAgent, meta_wmes, (*map_iter).second,
                thisAgent->symbolManager->soarSymbols.epmem_sym_retrieved,
                (*iter).second);
          }
        }
  ...
  for (epmem_interval_set::iterator iter = interval_cleanup.begin();
       iter != interval_cleanup.end(); iter++) {
  ...
  for (int type = EPMEM_RIT_STATE_NODE; type <= EPMEM_RIT_STATE_EDGE; type++) {
    for (epmem_triple_pedge_map::iterator iter = pedge_caches[type].begin();
         iter != pedge_caches[type].end(); iter++) {
    ...
    for (epmem_triple_uedge_map::iterator iter = uedge_caches[type].begin();
         iter != uedge_caches[type].end(); iter++) {
      epmem_uedge* uedge = (*iter).second;
      uedge->pedges.~epmem_pedge_set();
      thisAgent->memoryManager->free_with_pool(MP_epmem_uedge, uedge);
    }
  }
  for (epmem_wme_literal_map::iterator iter = literal_cache.begin();
       iter != literal_cache.end(); iter++) {
...
void epmem_print_episode(agent* thisAgent, epmem_time_id memory_id,
                         std::string* buf) {
  epmem_attach(thisAgent);
  ...
  std::map<epmem_node_id, std::map<std::string, std::list<std::string>>> ep;
  {
    ...
    my_q = thisAgent->EpMem->epmem_stmts_graph->get_wmes_with_constant_values;
    {
      ...
      while (my_q->execute() == soar_module::row) {
        ...
        //              parent_n_id, (unsigned int) my_q->column_int( 2 ),
        //              (unsigned int) my_q->column_int( 3 ));
  ...
  // output
  {
    ...
    std::map<epmem_node_id,
             std::map<std::string, std::list<std::string>>>::iterator ep_it;
    ...
    for (ep_it = ep.begin(); ep_it != ep.end(); ep_it++) {
      ...
      // attr
      for (slot_it = ep_it->second.begin(); slot_it != ep_it->second.end();
           slot_it++) {
        buf->append(" ^");
        buf->append(slot_it->first);

        for (val_it = slot_it->second.begin(); val_it != slot_it->second.end();
             val_it++) {
          buf->append(" ");
          buf->append(*val_it);
        }
...
void epmem_visualize_episode(agent* thisAgent, epmem_time_id memory_id,
                             std::string* buf) {
  epmem_attach(thisAgent);
  ...
  // taken heavily from install
  {
    ...
    my_q = thisAgent->EpMem->epmem_stmts_graph->get_wmes_with_identifier_values;
    {
      ...
      std::map<epmem_node_id, std::pair<std::string, std::string>>::iterator
          lti_p;
      ...
      while (my_q->execute() == soar_module::row) {
        ...
        if (val_is_short_term) {
          ...
          if (lti_p == ltis.end()) {
            ...
            ltis.insert(
                std::make_pair(child_n_id, std::make_pair(temp3, temp4)));
          }
        }
...
bool epmem_consider_new_episode(agent* thisAgent) {
  ...
  if (force == epmem_param_container::force_off) {
    const int64_t trigger =
        thisAgent->EpMem->epmem_params->trigger->get_value();
    ...
    thisAgent->EpMem->epmem_params->force->set_value(
...
void inline _epmem_respond_to_cmd_parse(
    agent* thisAgent, epmem_wme_list* cmds, bool& good_cue, int& path,
    epmem_time_id& retrieve, Symbol*& next, Symbol*& previous, Symbol*& query,
    Symbol*& neg_query, epmem_time_list& prohibit, epmem_time_id& before,
    epmem_time_id& after, wme_set& cue_wmes) {
  cue_wmes.clear();
  ...
  for (epmem_wme_list::iterator w_p = cmds->begin(); w_p != cmds->end();
       w_p++) {
    cue_wmes.insert((*w_p));
    ...
    if (good_cue) {
      ...
      if ((*w_p)->attr ==
          thisAgent->symbolManager->soarSymbols.epmem_sym_retrieve) {
        if (((*w_p)->value->symbol_type == INT_CONSTANT_SYMBOL_TYPE) &&
            (path == 0) && ((*w_p)->value->ic->value > 0)) {
          ...
          good_cue = false;
                 ...
                 thisAgent->symbolManager->soarSymbols.epmem_sym_next) {
        if (((*w_p)->value->is_sti()) && (path == 0)) {
          next = (*w_p)->value;
          path = 2;
        } else {
          good_cue = false;
                 ...
                 thisAgent->symbolManager->soarSymbols.epmem_sym_prev) {
        if (((*w_p)->value->is_sti()) && (path == 0)) {
          previous = (*w_p)->value;
          path = 2;
        } else {
          good_cue = false;
                 ...
                 thisAgent->symbolManager->soarSymbols.epmem_sym_query) {
        if (((*w_p)->value->is_sti()) && ((path == 0) || (path == 3)) &&
            (query == NULL))

        {
          ...
          good_cue = false;
                 ...
                 thisAgent->symbolManager->soarSymbols.epmem_sym_negquery) {
        if (((*w_p)->value->is_sti()) && ((path == 0) || (path == 3)) &&
            (neg_query == NULL))

        {
          ...
          good_cue = false;
                 ...
                 thisAgent->symbolManager->soarSymbols.epmem_sym_before) {
        if (((*w_p)->value->symbol_type == INT_CONSTANT_SYMBOL_TYPE) &&
            ((path == 0) || (path == 3))) {
          if ((before == EPMEM_MEMID_NONE) ||
              (static_cast<epmem_time_id>((*w_p)->value->ic->value) < before)) {
          ...
          good_cue = false;
                 ...
                 thisAgent->symbolManager->soarSymbols.epmem_sym_after) {
        if (((*w_p)->value->symbol_type == INT_CONSTANT_SYMBOL_TYPE) &&
            ((path == 0) || (path == 3))) {
          ...
          good_cue = false;
                 ...
                 thisAgent->symbolManager->soarSymbols.epmem_sym_prohibit) {
        if (((*w_p)->value->symbol_type == INT_CONSTANT_SYMBOL_TYPE) &&
            ((path == 0) || (path == 3))) {
...
void epmem_respond_to_cmd(agent* thisAgent) {
  ...
  while (state != NULL) {
    ...
    if (new_cue && wme_count) {
      _epmem_respond_to_cmd_parse(thisAgent, cmds, good_cue, path, retrieve,
      ...
      if (good_cue) {
        ...
        // retrieve
        if (path == 1) {
          epmem_install_memory(thisAgent, state, retrieve, meta_wmes,
                               retrieval_wmes);

          // add one to the ncbr stat
          thisAgent->EpMem->epmem_stats->ncbr->set_value(
              thisAgent->EpMem->epmem_stats->ncbr->get_value() + 1);
        }
        // previous or next
        else if (path == 2) {
          if (next) {
            epmem_install_memory(
                thisAgent, state,
                epmem_next_episode(thisAgent,
                                   state->id->epmem_info->last_memory),
                meta_wmes, retrieval_wmes);

            // add one to the next stat
            thisAgent->EpMem->epmem_stats->nexts->set_value(
                thisAgent->EpMem->epmem_stats->nexts->get_value() + 1);
          } else {
            epmem_install_memory(
                thisAgent, state,
                epmem_previous_episode(thisAgent,
                                       state->id->epmem_info->last_memory),
                meta_wmes, retrieval_wmes);

            // add one to the prev stat
            thisAgent->EpMem->epmem_stats->prevs->set_value(
                thisAgent->EpMem->epmem_stats->prevs->get_value() + 1);
          }

          if (state->id->epmem_info->last_memory == EPMEM_MEMID_NONE) {
            epmem_buffer_add_wme(
                thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
                thisAgent->symbolManager->soarSymbols.epmem_sym_failure,
                ((next) ? (next) : (previous)));
          } else {
            epmem_buffer_add_wme(
                thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
                thisAgent->symbolManager->soarSymbols.epmem_sym_success,
                ((next) ? (next) : (previous)));
          }
        }
        // query
        else if (path == 3) {
          epmem_process_query(thisAgent, state, query, neg_query, prohibit,
                              before, after, cue_wmes, meta_wmes,
                              retrieval_wmes);

          // add one to the cbr stat
          thisAgent->EpMem->epmem_stats->cbr->set_value(
              thisAgent->EpMem->epmem_stats->cbr->get_value() + 1);
        }
      } else {
        epmem_buffer_add_wme(
            thisAgent, meta_wmes, state->id->epmem_info->result_wme->value,
      ...
      if (!retrieval_wmes.empty() || !meta_wmes.empty()) {
        ...
        epmem_process_buffered_wmes(thisAgent, state, cue_wmes, meta_wmes,
                                    retrieval_wmes);
        ...
        // clear cache
        {
          ...
          for (mw_it = retrieval_wmes.begin(); mw_it != retrieval_wmes.end();
               mw_it++) {
...
bool epmem_backup_db(agent* thisAgent, const char* file_name,
                     std::string* err) {
  bool return_val = false;
...
EpMem_Manager::EpMem_Manager(agent* myAgent) {
  ...
  epmem_node_removals = new epmem_id_removal_map(
      ...
      soar_module::soar_memory_pool_allocator<
          std::pair<std::pair<epmem_node_id const, int64_t> const, bool>>(
          thisAgent));
  epmem_edge_removals = new epmem_edge_removal_map(
      std::less<std::pair<epmem_node_id const, int64_t>>(),
      soar_module::soar_memory_pool_allocator<
          std::pair<epmem_node_id const, bool>>(thisAgent));
  epmem_wme_adds = new epmem_symbol_set(
      std::less<Symbol*>(),
      soar_module::soar_memory_pool_allocator<Symbol*>(thisAgent));
