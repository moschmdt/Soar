...
action* Explanation_Based_Chunker::variablize_rl_action(
    action* pRLAction, struct token_struct* tok, wme* w,
  ...
  id_sym = instantiate_rhs_value(thisAgent, pRLAction->id, -1, 's', tok, w,
                                 was_unbound_vars.id);
  attr_sym =
      instantiate_rhs_value(thisAgent, pRLAction->attr, id_sym->id->level, 'a',
                            tok, w, was_unbound_vars.attr);
  first_letter = first_letter_from_symbol(attr_sym);
  val_sym =
      instantiate_rhs_value(thisAgent, pRLAction->value, id_sym->id->level,
                            first_letter, tok, w, was_unbound_vars.value);
  ref_sym =
      instantiate_rhs_value(thisAgent, pRLAction->referent, id_sym->id->level,
  ...
  rhs->id =
      allocate_rhs_value_for_symbol(thisAgent, id_sym, lRS->inst_identity,
                                    lRS->cv_id, NULL, was_unbound_vars.id);
  lRS = rhs_value_to_rhs_symbol(pRLAction->attr);
  rhs->attr =
      allocate_rhs_value_for_symbol(thisAgent, attr_sym, lRS->inst_identity,
                                    lRS->cv_id, NULL, was_unbound_vars.attr);
  lRS = rhs_value_to_rhs_symbol(pRLAction->value);
  rhs->value =
      allocate_rhs_value_for_symbol(thisAgent, val_sym, lRS->inst_identity,
                                    lRS->cv_id, NULL, was_unbound_vars.value);
  lRS = rhs_value_to_rhs_symbol(pRLAction->referent);
  rhs->referent = allocate_rhs_value_for_symbol(
      thisAgent, ref_sym, lRS->inst_identity, lRS->cv_id, NULL,
      was_unbound_vars.referent);

  /* instantiate and allocate both increased refcount by 1.  Decrease one here.
   * Variablize may decrease also */
...
void Explanation_Based_Chunker::variablize_rl_condition_list(
    condition* top_cond) {
