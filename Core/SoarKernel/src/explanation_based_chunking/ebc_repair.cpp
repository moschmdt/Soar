...
void delete_ungrounded_symbol_list(agent* thisAgent,
                                   matched_symbol_list** unconnected_syms) {
  matched_symbol_list* lSyms = *unconnected_syms;
  chunk_element* lSym;

  // for(matched_symbol_list::iterator it = lSyms->begin(), end = lSyms->end();
  // it != end; ++it)
...
wme_list* Repair_Manager::find_path_to_goal_for_symbol(
    Symbol* pNonOperationalSym) {
  ...
  while (!ids_to_walk.empty()) {
    ...
    { /* we can delete the rest of the sym_grounding_path objects */
      for (slot* s = lCurrentPath->get_root()->id->slots; s != NIL;
           s = s->next) {
        for (wme* w = s->wmes; w != NIL; w = w->next) {
          if (w->preference && w->value->is_sti() &&
              (w->value->tc_num != ground_lti_tc)) {
            w->value->tc_num = ground_lti_tc;
            thisAgent->memoryManager->allocate_with_pool(MP_repair_path,
                                                         &lNewPath);
...
inline void add_cond_to_lists(condition** c, condition** prev,
                              condition** first) {
  if (*prev) {
    (*c)->prev = *prev;
    (*prev)->next = *c;
...
void Repair_Manager::add_state_link_WMEs(goal_stack_level pTargetGoal,
                                         tc_number pSeenTC) {
...
void Repair_Manager::add_path_to_goal_WMEs(chunk_element* pTargetSym,
                                           tc_number cond_tc) {
  wme_list* l_WMEPath =
      find_path_to_goal_for_symbol(pTargetSym->instantiated_sym);
...
condition* Repair_Manager::make_condition_from_wme(wme* lWME) {
  ...
  new_cond =
      make_condition(thisAgent, make_test(thisAgent, lWME->id, EQUALITY_TEST),
...
void Repair_Manager::mark_states_WMEs_and_store_variablizations(
    condition* pCondList, tc_number tc) {
  condition* lCond;
  ...
  for (lCond = pCondList; lCond; lCond = lCond->next) {
    if (lCond->type == POSITIVE_CONDITION) {
      ...
      if (lSym->is_sti() && lSym->id->isa_goal) {
        lSym->tc_num = tc;
      } else if (lMatchedSym && lMatchedSym->is_sti() &&
                 lMatchedSym->id->isa_goal) {
        lMatchedSym->tc_num = tc;
      }
      if (lMatchedSym) {
        thisAgent->explanationBasedChunker->add_sti_variablization(
            lMatchedSym, lSym,
            lCond->data.tests.id_test->eq_test->inst_identity,
            lCond->data.tests.id_test->eq_test->chunk_inst_identity);
      }
      ...
      if (lMatchedSym && lMatchedSym->is_sti()) {
        thisAgent->explanationBasedChunker->add_sti_variablization(
            lMatchedSym, lSym,
            lCond->data.tests.value_test->eq_test->inst_identity,
            lCond->data.tests.id_test->eq_test->chunk_inst_identity);
      }
    }
  }
...
void Repair_Manager::repair_rule(condition*& p_lhs_top,
                                 matched_symbol_list* p_dangling_syms) {
  ...
  for (auto it = p_dangling_syms->begin(); it != p_dangling_syms->end(); it++) {
    ...
    thisAgent->explanationBasedChunker->add_sti_variablization(
        ...
        lDanglingSymInfo->inst_identity, lDanglingSymInfo->cv_id);
  }
  ...
  for (auto it = m_repair_WMEs.begin(); it != m_repair_WMEs.end(); it++) {
    ...
    thisAgent->explanationBasedChunker->sti_variablize_test(
        new_cond->data.tests.id_test);
    thisAgent->explanationBasedChunker->sti_variablize_test(
        new_cond->data.tests.value_test);
...
bool Explanation_Based_Chunker::reorder_and_validate_chunk() {
  ...
  auto reorder_result = reorder_and_validate_lhs_and_rhs(
      thisAgent, &m_lhs, &m_rhs, false, unconnected_syms, true, true);

  if (reorder_result != reorder_success) {
    report_reorder_errors(reorder_result);
    if ((reorder_result == reorder_failed_unconnected_conditions) ||
        (reorder_result == reorder_failed_reordering_rhs)) {
      thisAgent->outputManager->display_soar_feedback(
          thisAgent, ebc_progress_repairing,
          thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]);

      Repair_Manager* lRepairManager = new Repair_Manager(
          thisAgent, m_results_match_goal_level, m_chunk_inst->i_id);
      ...
      thisAgent->outputManager->display_soar_feedback(
          thisAgent, ebc_progress_validating,
          thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]);
      if (reorder_and_validate_lhs_and_rhs(thisAgent, &m_lhs, &m_rhs, false,
                                           unconnected_syms, false,
                                           false) == reorder_success) {
        delete_ungrounded_symbol_list(thisAgent, &unconnected_syms);
        if (thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]) {
          thisAgent->outputManager->display_soar_feedback(
              thisAgent, ebc_progress_repaired);
    ...
    thisAgent->outputManager->display_soar_feedback(
        thisAgent, ebc_error_invalid_chunk,
        thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]);
    delete_ungrounded_symbol_list(thisAgent, &unconnected_syms);
    return false;
  }
...
void Explanation_Based_Chunker::report_reorder_errors(
    ProdReorderFailureType reorder_result) {
  const char* message;
  switch (reorder_result) {
    case reorder_failed_reordering_rhs: {
      message =
          "Chunking issue detected. Soar has learned a rule with ungrounded "
          "action(s). Repair required.";
      break;
    }
    case reorder_failed_unconnected_conditions: {
      message =
          "Chunking issue detected. Soar has learned a rule with with "
          "ungrounded condition(s). Repair required.\n"
          "   This is likely caused by a condition that tested a working "
          "memory element \n"
          "   that was created in the sub-state but later became connected to "
          "the \n"
          "   super-state because it was a child of an identifier that was an "
          "element\n"
    ...
    case reorder_failed_no_roots: {
      message =
          "Chunking issue detected. Soar has learned a rule with no conditions "
          "that match a goal state.";
      break;
    }
    case reorder_failed_negative_relational_test_bindings: {
      message =
          "Chunking issue detected. Soar has learned a rule with negative "
          "relational test bindings.";
      break;
    }
