...
Identity* Explanation_Based_Chunker::create_new_identity(Symbol* pGoal) {
  ...
  if (thisAgent->explanationMemory->is_any_enabled())
    thisAgent->explanationMemory->add_identity(l_identity, pGoal);
...
void set_pref_identity(agent* thisAgent, preference* pPref, WME_Field pField,
                       Identity* pIdentity) {
  ...
  if (pField == ID_ELEMENT) {
    pPref->identities.id = pIdentity;
  } else if (pField == ATTR_ELEMENT) {
    pPref->identities.attr = pIdentity;
  } else if (pField == VALUE_ELEMENT) {
    pPref->identities.value = pIdentity;
  } else if (pField == REFERENT_ELEMENT) {
    pPref->identities.referent = pIdentity;
  }
}
...
Identity* Explanation_Based_Chunker::get_or_add_identity(uint64_t pID,
                                                         Identity* pIdentity,
                                                         Symbol* pGoal) {
  auto iter = (*inst_id_to_identity_map).find(pID);
  if (iter != (*inst_id_to_identity_map).end()) {
    ...
    if (pIdentity)
      thisAgent->explanationMemory
          ->increment_stat_identity_propagations_blocked();
    return l_identity;
  }
...
void Explanation_Based_Chunker::clean_up_identities() {
  Identity* lJoin_set;
  for (auto it = identities_to_clean_up.begin();
       it != identities_to_clean_up.end(); it++) {
    lJoin_set = (*it);
...
void Explanation_Based_Chunker::join_identities(Identity* lFromJoinSet,
                                                Identity* lToJoinSet) {
  ...
  uint64_t lFromSize = lFromJoinSet->merged_identities
                           ? lFromJoinSet->merged_identities->size()
                           : 0;
  uint64_t lToSize =
      lToJoinSet->merged_identities ? lToJoinSet->merged_identities->size() : 0;
  ...
  /* Iterate through identity sets in lFromJoinSet and set their super join set
   * point to lToJoinSet */
  Identity* lPreviouslyJoinedIdentity;
  if (lFromJoinSet->merged_identities) {
    for (auto it = lFromJoinSet->merged_identities->begin();
         it != lFromJoinSet->merged_identities->end(); it++) {
    ...
    lToJoinSet->merged_identities->splice(
        lToJoinSet->merged_identities->begin(),
        (*lFromJoinSet->merged_identities));
    delete lFromJoinSet->merged_identities;
    lFromJoinSet->merged_identities = NULL;
  }
  /* The identity set being joined is not on its child identity_sets list, so we
   * add it to other identity set here*/
...
void Explanation_Based_Chunker::update_identities_in_test(
    test t, instantiation* pInst) {
  cons* c;
  switch (t->type) {
    case GOAL_ID_TEST:
...
void Explanation_Based_Chunker::update_identities_in_cond(
    condition* pCond, instantiation* pInst) {
...
void Explanation_Based_Chunker::update_identities_in_condlist(
    condition* pCondTop, instantiation* pInst) {
  condition* pCond;
...
void Explanation_Based_Chunker::update_identities_in_preferences(
    preference* lPref, Symbol* pGoal, bool is_chunk_inst) {
  if (lPref->inst_identities.id)
    set_pref_identity(
        thisAgent, lPref, ID_ELEMENT,
        get_or_add_identity(lPref->inst_identities.id, NULL, pGoal));
  if (lPref->inst_identities.attr)
    set_pref_identity(
        thisAgent, lPref, ATTR_ELEMENT,
        get_or_add_identity(lPref->inst_identities.attr, NULL, pGoal));
  if (lPref->inst_identities.value)
    set_pref_identity(
        thisAgent, lPref, VALUE_ELEMENT,
        get_or_add_identity(lPref->inst_identities.value, NULL, pGoal));
  if (lPref->inst_identities.referent)
    set_pref_identity(
        thisAgent, lPref, REFERENT_ELEMENT,
        get_or_add_identity(lPref->inst_identities.referent, NULL, pGoal));

  if (is_chunk_inst) {
    if (lPref->rhs_func_inst_identities.id)
      update_identities_in_rhs_value(lPref->rhs_func_inst_identities.id);
    if (lPref->rhs_func_inst_identities.attr)
      update_identities_in_rhs_value(lPref->rhs_func_inst_identities.attr);
    if (lPref->rhs_func_inst_identities.value)
      update_identities_in_rhs_value(lPref->rhs_func_inst_identities.value);
    if (lPref->rhs_func_inst_identities.referent)
      update_identities_in_rhs_value(lPref->rhs_func_inst_identities.referent);
  } else {
    /* For instantiations we don't deallocate the existing rhs_funcs before
     * replacing them because are created in execute_action() which doesn't have
     * ownership of these rhs functions and previously made copies for the
     * preference that it's creating. We now just moved it here so that it can
     * be done after identity sets have been determined. */
    rhs_value lNewRHSValue;
    if (lPref->rhs_func_inst_identities.id) {
      lNewRHSValue =
          copy_rhs_value(thisAgent, lPref->rhs_func_inst_identities.id, true);
      deallocate_rhs_value(thisAgent, lPref->rhs_func_inst_identities.id);
      lPref->rhs_func_inst_identities.id = lNewRHSValue;
    }
    if (lPref->rhs_func_inst_identities.attr) {
      lNewRHSValue =
          copy_rhs_value(thisAgent, lPref->rhs_func_inst_identities.attr, true);
      deallocate_rhs_value(thisAgent, lPref->rhs_func_inst_identities.attr);
      lPref->rhs_func_inst_identities.attr = lNewRHSValue;
    }
    if (lPref->rhs_func_inst_identities.value) {
      lNewRHSValue = copy_rhs_value(
          thisAgent, lPref->rhs_func_inst_identities.value, true);
      deallocate_rhs_value(thisAgent, lPref->rhs_func_inst_identities.value);
      lPref->rhs_func_inst_identities.value = lNewRHSValue;
    }
    if (lPref->rhs_func_inst_identities.referent) {
      lNewRHSValue = copy_rhs_value(
          thisAgent, lPref->rhs_func_inst_identities.referent, true);
