...
uint64_t Explanation_Based_Chunker::variablize_rhs_value(
    rhs_value& pRhs_val, tc_number lti_link_tc) {
  char prefix[2];
  Symbol* var;
  ...
  if (has_variablization) {
    ...
    if (rs->referent->is_lti() && lti_link_tc &&
        (rs->referent->id->level == m_inst->match_goal_level) &&
    ...
    if (pTest->eq_test->identity && !pTest->eq_test->identity->literalized()) {
      ...
      if (new_var) {
        ...
        pTest->eq_test->chunk_inst_identity =
            pTest->eq_test->identity->get_clone_identity();
        clear_test_identity(thisAgent, pTest->eq_test);
      } else {
        lOldSym = pTest->eq_test->data.referent;
        if (lOldSym->is_sti()) {
          char prefix_char =
              static_cast<char>(tolower(lOldSym->id->name_letter));
          if ((((prefix_char == 's') || (prefix_char == 'S')) &&
               !lOldSym->id->isa_goal) ||
              (((prefix_char == 'o') || (prefix_char == 'O')) &&
               !lOldSym->id->isa_operator)) {
            prefix[0] = 'c';
          } else {
        ...
        pTest->eq_test->identity->store_variablization(
...
bool Explanation_Based_Chunker::variablize_test_by_lookup(
...
void Explanation_Based_Chunker::variablize_tests_by_lookup(
    test t, bool pSkipTopLevelEqualities) {
  cons* c;
  test tt;
...
void Explanation_Based_Chunker::variablize_condition_list(
  ...
  for (condition* cond = top_cond; cond != NIL; cond = cond->next) {
    if (cond->type == POSITIVE_CONDITION) {
      if ((cond->data.tests.id_test->type == CONJUNCTIVE_TEST) ||
        ...
        variablize_tests_by_lookup(cond->data.tests.id_test,
                                   !pInNegativeCondition);
      if ((cond->data.tests.attr_test->type == CONJUNCTIVE_TEST) ||
          pInNegativeCondition)
        variablize_tests_by_lookup(cond->data.tests.attr_test,
                                   !pInNegativeCondition);
      if ((cond->data.tests.value_test->type == CONJUNCTIVE_TEST) ||
          pInNegativeCondition)
        variablize_tests_by_lookup(cond->data.tests.value_test,
                                   !pInNegativeCondition);
...
action* Explanation_Based_Chunker::variablize_result_into_action(
    preference* result, tc_number lti_link_tc) {
  ...
  if (!result->rhs_func_inst_identities.id) {
    a->id = allocate_rhs_value_for_symbol(
        thisAgent, result->id, result->inst_identities.id,
        result->chunk_inst_identities.id, result->identities.id,
        result->was_unbound_vars.id);
  } else {
  ...
  if (!result->rhs_func_inst_identities.attr) {
    a->attr = allocate_rhs_value_for_symbol(
        thisAgent, result->attr, result->inst_identities.attr,
        result->chunk_inst_identities.attr, result->identities.attr,
        result->was_unbound_vars.attr);
  } else {
  ...
  if (!result->rhs_func_inst_identities.value) {
    a->value = allocate_rhs_value_for_symbol(
        thisAgent, result->value, result->inst_identities.value,
        result->chunk_inst_identities.value, result->identities.value,
        result->was_unbound_vars.value);
  } else {
    a->value =
        copy_rhs_value(thisAgent, result->rhs_func_inst_identities.value);
  }
  if (preference_is_binary(result->type)) {
    if (!result->rhs_func_inst_identities.referent) {
      a->referent = allocate_rhs_value_for_symbol(
          thisAgent, result->referent, result->inst_identities.referent,
          result->chunk_inst_identities.referent, result->identities.referent,
          result->was_unbound_vars.referent);
    } else {
      a->referent =
          copy_rhs_value(thisAgent, result->rhs_func_inst_identities.referent);
    }
  }
  ...
  if (!result->rhs_func_inst_identities.attr) {
    ...
    a->attr = copy_rhs_value(
        thisAgent, result->rhs_func_chunk_inst_identities.attr, false, true);
  ...
  if (!result->rhs_func_inst_identities.value) {
    ...
    a->value = copy_rhs_value(
        thisAgent, result->rhs_func_chunk_inst_identities.value, false, true);
  ...
  if (preference_is_binary(result->type)) {
    ...
    if (!result->rhs_func_inst_identities.referent) {
      ...
      a->referent = copy_rhs_value(
          thisAgent, result->rhs_func_chunk_inst_identities.referent, false,
          true);
...
void Explanation_Based_Chunker::add_LTM_linking_actions(action* pLastAction) {
  ...
  for (auto it = local_linked_STIs->begin(); it != local_linked_STIs->end();
       it++) {
    lRSym = rhs_value_to_rhs_symbol(*it);

    lIntRV = allocate_rhs_value_for_symbol_no_refcount(
        thisAgent,
        thisAgent->symbolManager->make_int_constant(
            lRSym->referent->var->instantiated_sym->id->LTI_ID),
        lRSym->inst_identity, lRSym->cv_id, lRSym->identity, false);
