...
void Explanation_Based_Chunker::report_local_negation(condition* c) {
  ...
  thisAgent->outputManager->printa(thisAgent,
                                   "\n*** Rule learned that used negative "
                                   "reasoning about local sub-state.***\n");
...
void Explanation_Based_Chunker::print_merge_map(TraceMode mode) {
  ...
  for (iter_id = cond_merge_map->begin(); iter_id != cond_merge_map->end();
       ++iter_id) {
    outputManager->printa_sf(thisAgent, "%y conditions: \n", iter_id->first);
    for (iter_attr = iter_id->second.begin();
         iter_attr != iter_id->second.end(); ++iter_attr) {
      for (iter_value = iter_attr->second.begin();
           iter_value != iter_attr->second.end(); ++iter_value) {
...
void Explanation_Based_Chunker::print_instantiation_identities_map(
    TraceMode mode) {
  if (!thisAgent->outputManager->is_trace_enabled(mode)) return;
  outputManager->printa_sf(thisAgent, "------------------------------------\n");
  ...
  for (auto iter_sym = instantiation_identities->begin();
       iter_sym != instantiation_identities->end(); ++iter_sym) {
...
void Explanation_Based_Chunker::print_id_to_identity_map(TraceMode mode) {
  ...
  for (iter = inst_id_to_identity_map->begin();
       iter != inst_id_to_identity_map->end(); ++iter) {
    outputManager->printa_sf(thisAgent, "   %u = %u\n", iter->first,
                             iter->second->get_identity());
  }
...
void Explanation_Based_Chunker::print_variablization_table(TraceMode mode) {
  ...
  //        it->second->variable_sym, it->second->identity);
  outputManager->printa_sf(thisAgent, "------------------------------------\n");
}
...
const char* Explanation_Based_Chunker::singletonTypeToString(
    singleton_element_type pType) {
  switch (pType) {
    case ebc_any:
      return "<any>";
    case ebc_state:
...
void Explanation_Based_Chunker::print_singleton_summary() {
  ...
  outputManager->printa(thisAgent,
                        "==== Singleton WME Unification Patterns ====\n");
  outputManager->printa(thisAgent,
                        "----------------- Local --------------------\n");
  ...
  outputManager->printa_sf(
      thisAgent,
      "   (<state> ^operator   <operator>)             %-(unless condition "
      "only tests operator proposal)\n");
  ...
  if (singletons->empty()) {
    ...
    for (auto it = singletons->begin(); it != singletons->end(); it++) {
      ...
      thisAgent->outputManager->printa_sf(
          thisAgent, "   (%s ^%y %s)\n",
          singletonTypeToString(lSym->sc->singleton.id_type), lSym,
          singletonTypeToString(lSym->sc->singleton.value_type));
    }
  }
  outputManager->printa(
      ...
      "   Valid types:          [ any | constant | identifier | operator | "
      "state ]  \n");
}
