...
extern Symbol* find_goal_at_goal_stack_level(agent* thisAgent,
...
Explanation_Based_Chunker::Explanation_Based_Chunker(agent* myAgent) {
  ...
  ebc_params =
      new ebc_param_container(thisAgent, ebc_settings, max_chunks, max_dupes);
  ...
  justification_name_prefix =
      make_memory_block_for_string(thisAgent, "justify");
...
bool Explanation_Based_Chunker::set_learning_for_instantiation(
    instantiation* inst) {
  if (!ebc_settings[SETTING_EBC_LEARNING_ON] ||
      (inst->match_goal_level == TOP_GOAL_LEVEL)) {
  ...
  if (ebc_settings[SETTING_EBC_EXCEPT] &&
    ...
    if (thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]) {
      ...
      message << "\nWill not attempt to learn a chunk for match of "
              << inst->prod_name->to_string() << " because state "
              << inst->match_goal->to_string()
              << " was flagged to prevent learning";
  ...
  /* Allow_bottom_up_chunks will be false if a chunk was already learned in a
   * lower goal */
  if (ebc_settings[SETTING_EBC_BOTTOM_ONLY] &&
    ...
    if (thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]) {
      ...
      message << "\nWill not attempt to learn a chunk for match of "
              << inst->prod_name->to_string() << " because state "
              << inst->match_goal->to_string() << " is not the bottom state";
      thisAgent->outputManager->printa_sf(thisAgent, message.str().c_str());
      xml_generate_verbose(thisAgent, message.str().c_str());
    }
    m_learning_on_for_instantiation = false;
...
Symbol* Explanation_Based_Chunker::generate_name_for_new_rule() {
  ...
  if (!ebc_settings[SETTING_EBC_LEARNING_ON] ||
      (ebc_params->naming_style->get_value() == numberedFormat)) {
    increment_counter((*rule_naming_counter));
    return thisAgent->symbolManager->generate_new_str_constant(
        rule_prefix, rule_naming_counter);
  }

  new_rule_name += rule_prefix;

  /* Step 2: Add learning depth to indicate learned rule based on another
   * learned rule */
  if (m_inst->prod) {
  ...
  if (thisAgent->symbolManager->find_str_constant(new_rule_name.c_str())) {
    ...
    return thisAgent->symbolManager->generate_new_str_constant(
        new_rule_name.c_str(), &dummy_counter);
  } else
    return thisAgent->symbolManager->make_str_constant_no_find(
        new_rule_name.c_str());
}
...
void Explanation_Based_Chunker::set_up_rule_name() {
  ...
  if (m_rule_type == ebc_chunk) {
    ...
    m_should_print_name =
        (thisAgent->trace_settings[TRACE_CHUNK_NAMES_SYSPARAM] != 0);
    m_should_print_prod =
        (thisAgent->trace_settings[TRACE_CHUNKS_SYSPARAM] != 0);
  } else {
    justifications_this_d_cycle++;
    m_prod_name = generate_name_for_new_rule();
    m_prod_type = JUSTIFICATION_PRODUCTION_TYPE;
    m_should_print_name =
        (thisAgent->trace_settings[TRACE_JUSTIFICATION_NAMES_SYSPARAM] != 0);
    m_should_print_prod =
        (thisAgent->trace_settings[TRACE_JUSTIFICATIONS_SYSPARAM] != 0);
  }
