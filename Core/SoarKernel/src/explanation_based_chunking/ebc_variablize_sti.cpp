...
void Explanation_Based_Chunker::add_sti_variablization(
    Symbol* pSym, Symbol* pVar, uint64_t pInstIdentity,
    uint64_t pInstCIdentity) {
...
void Explanation_Based_Chunker::sti_variablize_test(test pTest,
  ...
  /* Copy in any identities for the unconnected identifier that was used in the
  ...
  if (iter_sym == m_sym_to_var_map->end()) {
    ...
    if (generate_identity)
      lMatchedIdentity = thisAgent->explanationBasedChunker
                             ->get_or_create_inst_identity_for_sym(lNewVar);
...
void Explanation_Based_Chunker::sti_variablize_rhs_symbol(
    rhs_value& pRhs_val, bool generate_identity) {
  char prefix[2];
  Symbol* var;
  ...
  if (!has_variablization && rs->referent->is_sti()) {
    ...
    if (generate_identity)
      lMatchedIdentity = thisAgent->explanationBasedChunker
                             ->get_or_create_inst_identity_for_sym(var);
