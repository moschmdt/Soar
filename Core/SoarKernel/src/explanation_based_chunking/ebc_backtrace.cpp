...
void Explanation_Based_Chunker::backtrace_through_instantiation(
  ...
  if (inst->backtrace_number == backtrace_number) {
    if (thisAgent->trace_settings[TRACE_BACKTRACING_SYSPARAM]) {
      thisAgent->outputManager->printa(
          thisAgent, "(We already backtraced through this instantiation.)\n");
  ...
  /* We must backtrace through OSK even if we're not adding OSK because
   * prohibits also use this mechanism */
  if (inst->OSK_prefs) {
  ...
  for (c = inst->top_of_instantiated_conditions; c != NIL; c = c->next) {
    if (c->type == POSITIVE_CONDITION) {
      ...
      if (c->data.tests.id_test->eq_test->data.referent->id->level <=
          m_goal_level) {
        if (c->bt.wme_->tc !=
            grounds_tc) /* First time we've seen something matching this wme*/
        {
          add_to_grounds(c);
        } else if (ebc_settings[SETTING_EBC_LEARNING_ON]) /* Another condition
                                                             that matches the
                                                             same wme */
        {
          add_to_grounds(c);
        ...
        if (ebc_settings[SETTING_EBC_LEARNING_ON]) cache_constraints_in_cond(c);
        add_to_locals(c);
      }
    } else {
...
void Explanation_Based_Chunker::trace_locals() {
  ...
  while (locals) {
    ...
    if (cond->bt.trace) {
      bt_pref =
          (cond->bt.trace->level != m_results_match_goal_level)
              ? find_clone_for_level(cond->bt.trace, m_results_match_goal_level)
              : cond->bt.trace;
    }
    if (bt_pref) {
      backtrace_through_instantiation(bt_pref, cond, cond->inst->explain_depth,
                                      BT_Normal);
...
void Explanation_Based_Chunker::perform_dependency_analysis() {
  ...
  for (pref = m_results; pref != NIL; pref = pref->next_result) {
    ...
    backtrace_through_instantiation(
        pref, NULL, 0,
        (pref->inst == m_inst) ? BT_BaseInstantiation : BT_ExtraResults);
