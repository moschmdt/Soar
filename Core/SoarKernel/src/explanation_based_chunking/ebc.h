...
class Explanation_Based_Chunker {
 ...
 public:
  ...
  void learn_rule_from_instance(instantiation* inst,
                                instantiation** new_inst_list);

  /* Methods used during instantiation creation to generate identities used by
   * the explanation trace. */
  void add_explanation_to_condition(rete_node* node, condition* cond,
                                    node_varnames* nvn,
                                    ExplainTraceType additional_tests,
                                    bool inNegativeNodes);
  uint64_t get_new_inst_id() {
    increment_counter(inst_id_counter);
    return inst_id_counter;
  };
  uint64_t get_new_prod_id() {
    increment_counter(prod_id_counter);
    return prod_id_counter;
  };
  uint64_t get_instantiation_count() { return inst_id_counter; };
  uint64_t get_new_inst_identity_id() {
    increment_counter(inst_identity_counter);
    return inst_identity_counter;
  };
  uint64_t get_new_identity_id() {
    increment_counter(identity_counter);
  ...
  void update_identities_in_preferences(preference* lPref, Symbol* pGoal,
  ...
  Identity* get_or_add_identity(uint64_t pID, Identity* pIdentity,
                                Symbol* pGoal);
  Identity* get_floating_identity(Symbol* pGoal);
  void force_id_to_identity_mapping(uint64_t pID, Identity* pIdentity) {
  ...
  const std::string add_new_singleton(singleton_element_type id_type,
                                      Symbol* attrSym,
                                      singleton_element_type value_type);
  const std::string remove_singleton(singleton_element_type id_type,
                                     Symbol* attrSym,
                                     singleton_element_type value_type);
  ...
  void reset_chunks_this_d_cycle() {
    chunks_this_d_cycle = 0;
    justifications_this_d_cycle = 0;
  };
  ...
  void add_sti_variablization(Symbol* pSym, Symbol* pVar,
                              uint64_t pInstIdentity, uint64_t pInstCIdentity);
  void sti_variablize_test(test pTest, bool generate_identity = true);
  void sti_variablize_rhs_symbol(rhs_value& pRhs_val,
                                 bool generate_identity = true);
  void clear_sti_variablization_map() { m_sym_to_var_map->clear(); };

  void variablize_rl_condition_list(condition* top_cond);
  action* variablize_rl_action(action* pRLAction, struct token_struct* tok,
                               wme* w, double& initial_value);
 ...
 private:
  ...
  void add_var_test_bound_identity_to_id_test(condition* cond, byte field_num,
                                              rete_node_level levels_up);
  void add_constraint_to_explanation(test* dest_test_address, test new_test,
                                     bool has_referent = true);
  ...
  void add_pref_to_results(preference* pref, preference* pPref,
                           WME_Field pField);
  ...
  void trace_locals();
  void backtrace_through_instantiation(preference* pPref, condition* trace_cond,
                                       uint64_t bt_depth, BTSourceType bt_type);
  ...
  void unify_lhs_rhs_connection(condition* lhs_cond,
                                identity_set_quadruple& rhs_id_sets,
  ...
  void attach_relational_test(test pRelational_test, condition* pCond,
                              WME_Field pField);
  ...
  void variablize_condition_list(condition* top_cond,
                                 bool pInNegativeCondition = false);

  uint64_t variablize_rhs_value(rhs_value& pRhs_val, tc_number lti_link_tc = 0);
  action* variablize_result_into_action(preference* result,
                                        tc_number lti_link_tc);
  ...
  void reinstantiate_condition_list(condition* top_cond,
                                    bool pIsInstantiationCond,
                                    bool pIsNCC = false);
  void reinstantiate_condition(condition* cond, bool pIsInstantiationCond,
                               bool pIsNCC = false);
  ...
  void update_identities_in_tests_by_lookup(test t,
                                            bool pSkipTopLevelEqualities);
  bool update_identities_in_test_by_lookup(test t,
                                           bool pSkipTopLevelEqualities);
  void update_identities_in_condition_list(condition* top_cond,
                                           bool pInNegativeCondition = false);
