...
void Explanation_Based_Chunker::reinstantiate_test(test pTest,
  ...
  if (pTest->type == CONJUNCTIVE_TEST) {
    for (cons* c = pTest->data.conjunct_list; c != NIL; c = c->rest) {
      reinstantiate_test(static_cast<test>(c->first), pIsInstantiationCond);
             ...
             pTest->inst_identity &&
             pTest->data.referent->var->instantiated_sym) {
  ...
  if (test_has_referent(pTest) && pTest->inst_identity &&
      pIsInstantiationCond) {
    uint64_t temp;
...
void Explanation_Based_Chunker::reinstantiate_condition_list(
    condition* top_cond, bool pIsInstantiationCond, bool pIsNCC) {
  for (condition* cond = top_cond; cond != NIL; cond = cond->next) {
    reinstantiate_condition(cond, pIsInstantiationCond, pIsNCC);
  }
}

void Explanation_Based_Chunker::reinstantiate_condition(
    condition* cond, bool pIsInstantiationCond, bool pIsNCC) {
  if (cond->type != CONJUNCTIVE_NEGATION_CONDITION) {
    ...
    reinstantiate_condition_list(cond->data.ncc.top, pIsInstantiationCond,
                                 true);
  }
}
...
condition* Explanation_Based_Chunker::reinstantiate_lhs(condition* top_cond) {
  ...
  for (condition* cond = m_lhs; cond != NIL; cond = cond->next) {
    if (m_rule_type == ebc_justification) {
      /* This case is rare (if it should happen at all any more).  It occurs
       * when a learning attempt fails and is being reverted to a justification
       * instead */
...
void Explanation_Based_Chunker::reinstantiate_actions(action* pActionList) {
  for (action* lAction = pActionList; lAction != NULL;
