...
class Identity {
 public:
  ...
  condition* get_operational_cond() {
    return joined_identity->operational_cond;
  }
  WME_Field get_operational_field() {
    return joined_identity->operational_field;
  }
  uint64_t get_refcount() { return refcount; }

  void literalize() {
    joined_identity->m_literalized = true;
    joined_identity->touch();
  }
  void set_clone_identity(uint64_t pID) {
    joined_identity->chunk_inst_identity = pID;
    joined_identity->touch();
  }
  void set_var(Symbol* pVar) {
    joined_identity->new_var = pVar;
    joined_identity->touch();
  }
