...
/* ------------------------------------------------------------------
   ...
   there. If an operator The slot's OSK prefs is copied to conditions' bt
...
void Explanation_Based_Chunker::add_to_OSK(slot* pSlot, preference* pPref,
  ...
  for (lOSK_pref = pSlot->OSK_prefs; lOSK_pref != NIL;
       lOSK_pref = lOSK_pref->rest) {
    ...
    if (unique_value) {
      /* Checking if a preference is unique differs depending on the preference
       * type */

      /* Binary preferences can be considered equivalent if they point to the
       * same operators in the correct relative spots */
      if (((pPref->type == BETTER_PREFERENCE_TYPE) ||
           (pPref->type == WORSE_PREFERENCE_TYPE)) &&
          ((lPref->type == BETTER_PREFERENCE_TYPE) ||
           (lPref->type == WORSE_PREFERENCE_TYPE))) {
        if (pPref->type == lPref->type) {
          already_exists = ((pPref->value == lPref->value) &&
                            (pPref->referent == lPref->referent));
        } else {
          already_exists = ((pPref->value == lPref->referent) &&
                            (pPref->referent == lPref->value));
        }
      } else if ((pPref->type == BINARY_INDIFFERENT_PREFERENCE_TYPE) &&
                 (lPref->type == BINARY_INDIFFERENT_PREFERENCE_TYPE)) {
        already_exists = (((pPref->value == lPref->value) &&
                           (pPref->referent == lPref->referent)) ||
                          ((pPref->value == lPref->referent) &&
                           (pPref->referent == lPref->value)));
      } else {
        /* Otherwise they are equivalent if they have the same value and type */
        already_exists =
            (pPref->value == lPref->value) && (pPref->type == lPref->type);
      }
      if (already_exists) {
        break;
      }
...
/* --------------------------------------------------------------------------
 ...
 the corresponding condition.  The copied OSK prefs will later be backtraced
 through.

  Note: Until prohibits are included explicitly as part of the OSK prefs, we
...
void Explanation_Based_Chunker::copy_proposal_OSK(instantiation* inst,
  ...
  if (ebc_settings[SETTING_EBC_ADD_OSK]) {
    for (l_OSK_prefs = newOSK; l_OSK_prefs != NIL;
         l_OSK_prefs = l_OSK_prefs->rest) {
    ...
    if (cond->type == POSITIVE_CONDITION && cond->bt.trace &&
      ...
      if (ebc_settings[SETTING_EBC_ADD_OSK]) {
        if (cond->bt.trace->slot->OSK_prefs &&
            (cond->data.tests.id_test->eq_test->data.referent->id->level ==
             inst->match_goal_level) &&
            !cond->test_for_acceptable_preference) {
          for (l_OSK_prefs = cond->bt.trace->slot->OSK_prefs;
               l_OSK_prefs != NIL; l_OSK_prefs = l_OSK_prefs->rest) {
            pref = static_cast<preference*>(l_OSK_prefs->first);
...
void Explanation_Based_Chunker::update_proposal_OSK(slot* s,
                                                    preference* winner) {
...
void Explanation_Based_Chunker::generate_relevant_OSK(preference* winner,
