...
inline bool pref_has_identity_set_in_field(preference* pPref,
  ...
  if (pField == ID_ELEMENT) {
    ...
    if (pPref->identities.attr)
      ...
      return false;
  } else if (pField == VALUE_ELEMENT) {
    if (pPref->identities.value)
      return true;
    else
      return false;
  } else if (pField == REFERENT_ELEMENT) {
...
inline bool pref_has_same_identity_sets_in_2_fields(preference* pPref1,
                                                    WME_Field pField1,
                                                    preference* pPref2,
                                                    WME_Field pField2) {
  if (pField1 == ID_ELEMENT) {
    if (pField2 == ID_ELEMENT) {
      if (pPref1->identities.id == pPref2->identities.id)
        return true;
      else
        return false;
    }
    if (pField2 == ATTR_ELEMENT) {
      if (pPref1->identities.id == pPref2->identities.attr)
        return true;
      else
        return false;
    }
    if (pField2 == VALUE_ELEMENT) {
      if (pPref1->identities.id == pPref2->identities.value)
        return true;
      else
        return false;
    }
    if (pField2 == REFERENT_ELEMENT) {
      if (pPref1->identities.id == pPref2->identities.referent)
        return true;
      else
        return false;
    }
    ...
    if (pField2 == ID_ELEMENT) {
      if (pPref1->identities.attr == pPref2->identities.id)
        return true;
      else
        return false;
    }
    if (pField2 == ATTR_ELEMENT) {
      if (pPref1->identities.attr == pPref2->identities.attr)
        return true;
      else
        return false;
    }
    if (pField2 == VALUE_ELEMENT) {
      if (pPref1->identities.attr == pPref2->identities.value)
        return true;
      else
        return false;
    }
    if (pField2 == REFERENT_ELEMENT) {
      if (pPref1->identities.attr == pPref2->identities.referent)
        return true;
      else
        return false;
    }
    ...
    if (pField2 == ID_ELEMENT) {
      if (pPref1->identities.value == pPref2->identities.id)
        return true;
      else
        return false;
    }
    if (pField2 == ATTR_ELEMENT) {
      if (pPref1->identities.value == pPref2->identities.attr)
        return true;
      else
        return false;
    }
    if (pField2 == VALUE_ELEMENT) {
      if (pPref1->identities.value == pPref2->identities.value)
        return true;
      else
        return false;
    }
    if (pField2 == REFERENT_ELEMENT) {
      if (pPref1->identities.value == pPref2->identities.referent)
        return true;
      else
        return false;
    }
...
void Explanation_Based_Chunker::add_pref_to_results(preference* pref,
  ...
  if (pref->identities.id &&
      pref_has_identity_set_in_field(pLinkPref, pField)) {
    if (!pref_has_same_identity_sets_in_2_fields(pref, ID_ELEMENT, pLinkPref,
                                                 pField)) {
      if (pField == ID_ELEMENT) {
        join_identities(pref->identities.id, pLinkPref->identities.id);
        thisAgent->explanationMemory->add_identity_set_mapping(
            pref->inst->i_id, IDS_unified_child_result, pref->identities.id,
            pLinkPref->identities.id);
      }
      if (pField == ATTR_ELEMENT) {
        join_identities(pref->identities.id, pLinkPref->identities.attr);
        thisAgent->explanationMemory->add_identity_set_mapping(
            pref->inst->i_id, IDS_unified_child_result, pref->identities.id,
            pLinkPref->identities.attr);
      }
      if (pField == VALUE_ELEMENT) {
        join_identities(pref->identities.id, pLinkPref->identities.value);
        thisAgent->explanationMemory->add_identity_set_mapping(
            pref->inst->i_id, IDS_unified_child_result, pref->identities.id,
            pLinkPref->identities.value);
      }
    }
  }
...
void Explanation_Based_Chunker::add_results_if_needed(Symbol* pSym,
                                                      preference* pPref,
                                                      WME_Field pField) {
  slot* s;
  preference* pref;
  ...
  if ((pSym)->symbol_type == IDENTIFIER_SYMBOL_TYPE) {
    if (((pSym)->id->level >= m_results_match_goal_level) &&
...
bool Explanation_Based_Chunker::add_to_chunk_cond_set(chunk_cond_set* set,
                                                      chunk_cond* new_cc) {
  chunk_cond* old;
...
void Explanation_Based_Chunker::remove_from_chunk_cond_set(chunk_cond_set* set,
                                                           chunk_cond* cc) {
  remove_from_dll(set->all, cc, next, prev);
  remove_from_dll(set->table[cc->compressed_hash_value], cc, next_in_bucket,
                  prev_in_bucket);
}
  ...
  while (grounds) {
    ...
    if (ebc_settings[SETTING_EBC_LEARNING_ON]) {
      if (c_vrblz->data.tests.value_test->eq_test->identity &&
          !c_vrblz->data.tests.value_test->eq_test->identity
               ->get_operational_cond()) {
        c_vrblz->data.tests.value_test->eq_test->identity->set_operational_cond(
            c_vrblz, VALUE_ELEMENT);
      }
      if (c_vrblz->data.tests.attr_test->eq_test->identity &&
          !c_vrblz->data.tests.attr_test->eq_test->identity
               ->get_operational_cond()) {
        c_vrblz->data.tests.attr_test->eq_test->identity->set_operational_cond(
            c_vrblz, ATTR_ELEMENT);
      }
      if (c_vrblz->data.tests.id_test->eq_test->identity &&
          !c_vrblz->data.tests.id_test->eq_test->identity
               ->get_operational_cond()) {
        c_vrblz->data.tests.id_test->eq_test->identity->set_operational_cond(
            c_vrblz, ID_ELEMENT);
      }
    }
  ...
  if (has_local_negation) {
    ...
    if (ebc_settings[SETTING_EBC_INTERRUPT_WARNING] &&
      ...
      thisAgent->reason_for_stopping =
          "Chunking issue detected:  Problem-solving contained negated "
          "reasoning about sub-state structures.";
    }
  }
...
void Explanation_Based_Chunker::add_goal_or_impasse_tests() {
  ...
  for (cc = m_lhs; cc != NIL; cc = cc->next) {
    ...
    isa_goal = idSym->is_variable() ? idSym->var->instantiated_sym->id->isa_goal
                                    : idSym->id->isa_goal;
    isa_impasse = idSym->is_variable()
                      ? idSym->var->instantiated_sym->id->isa_impasse
                      : idSym->id->isa_impasse;
    if ((isa_goal || isa_impasse) && (idSym->tc_num != tc)) {
      t = make_test(thisAgent, NULL,
                    (isa_goal ? GOAL_ID_TEST : IMPASSE_ID_TEST));
...
void Explanation_Based_Chunker::make_clones_of_results() {
  ...
  for (lResultPref = m_results; lResultPref != NIL;
    ...
    lClonedPref = make_preference(
        ...
        lResultPref->chunk_inst_identities, lResultPref->was_unbound_vars);
    ...
    if (lResultPref->rhs_func_chunk_inst_identities.id) {
      lClonedPref->rhs_func_inst_identities.id =
          lResultPref->rhs_func_chunk_inst_identities.id;
      lResultPref->rhs_func_chunk_inst_identities.id = NULL;
    }
    if (lResultPref->rhs_func_chunk_inst_identities.attr) {
      lClonedPref->rhs_func_inst_identities.attr =
          lResultPref->rhs_func_chunk_inst_identities.attr;
      lResultPref->rhs_func_chunk_inst_identities.attr = NULL;
    }
    if (lResultPref->rhs_func_chunk_inst_identities.value) {
      lClonedPref->rhs_func_inst_identities.value =
          lResultPref->rhs_func_chunk_inst_identities.value;
      lResultPref->rhs_func_chunk_inst_identities.value = NULL;
    }
    if (lResultPref->rhs_func_chunk_inst_identities.referent) {
      lClonedPref->rhs_func_inst_identities.referent =
          lResultPref->rhs_func_chunk_inst_identities.referent;
      lResultPref->rhs_func_chunk_inst_identities.referent = NULL;
    }

    /* We record the identity set ids for the explainer so they match up with
     * identity sets on this level */
    if (thisAgent->explanationMemory->is_any_enabled()) {
      if (lResultPref->identities.id)
        lClonedPref->chunk_inst_identities.id =
            lResultPref->identities.id->get_identity();
      if (lResultPref->identities.attr)
        lClonedPref->chunk_inst_identities.attr =
            lResultPref->identities.attr->get_identity();
      if (lResultPref->identities.value)
        lClonedPref->chunk_inst_identities.value =
            lResultPref->identities.value->get_identity();
      if (lResultPref->identities.referent)
        lClonedPref->chunk_inst_identities.referent =
...
void Explanation_Based_Chunker::remove_chunk_instantiation() {
  ...
  for (lResultPref = m_chunk_inst->preferences_generated; lResultPref != NIL;
       lResultPref = lNext) {
...
bool Explanation_Based_Chunker::add_chunk_to_rete() {
  ...
  rete_addition_result =
      add_production_to_rete(thisAgent, m_prod, m_lhs, m_chunk_inst,
                             m_should_print_name, duplicate_rule);
  ...
  if (rete_addition_result == REFRACTED_INST_MATCHED) {
    thisAgent->explanationMemory->record_chunk_contents(
        m_prod, m_lhs, m_rhs, m_results, inst_id_to_identity_map, m_inst,
        m_chunk_inst, m_prod_type);
    if (m_prod_type == JUSTIFICATION_PRODUCTION_TYPE) {
      thisAgent->explanationMemory->increment_stat_justifications_succeeded();
      /* We'll interrupt on justification learning only if explainer is
       * recording justifications.  In most cases I think we wouldn't want to
       * interrupt on every justification learned */
      ...
      if (ebc_settings[SETTING_EBC_INTERRUPT_WATCHED] &&
          thisAgent->explanationMemory->isCurrentlyRecording()) {
        thisAgent->stop_soar = true;
        thisAgent->reason_for_stopping =
            "Soar learned a new rule from a watched production.";
      }
    }
    return true;
    ...
    if (m_prod_type == JUSTIFICATION_PRODUCTION_TYPE) {
      ...
      if (ebc_settings[SETTING_EBC_INTERRUPT_WARNING]) {
        thisAgent->stop_soar = true;
        thisAgent->reason_for_stopping =
            "Warning:  Justification did not match working memory.  Potential "
            "issue.";
      ...
      if (ebc_settings[SETTING_EBC_INTERRUPT_WARNING]) {
        thisAgent->stop_soar = true;
        thisAgent->reason_for_stopping =
            "Warning:  Chunk did not match working memory.  Potential issue.";
...
void Explanation_Based_Chunker::learn_rule_from_instance(
    instantiation* inst, instantiation** new_inst_list) {
  ...
  if (chunks_this_d_cycle >= max_chunks) {
    thisAgent->outputManager->display_soar_feedback(
        ...
        thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]);
  ...
  if (!can_learn_from_instantiation()) {
    m_inst = NULL;
    return;
  }
  ...
  if (m_inst->prod &&
      (thisAgent->d_cycle_count == m_inst->prod->last_duplicate_dc) &&
      (m_inst->prod->duplicate_chunks_this_cycle >= max_dupes)) {
    if (thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]) {
      thisAgent->outputManager->display_soar_feedback(
          thisAgent, ebc_error_max_dupes,
          thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]);
      thisAgent->outputManager->printa_sf(
          thisAgent, "         Rule that has reached the max-dupes limit: %y\n",
          m_inst->prod_name);
    }
  ...
  /* Set allow_bottom_up_chunks to false for all higher goals to prevent
   * chunking */
  {
    Symbol* g;
    for (g = m_inst->match_goal->id->higher_goal;
         g && g->id->allow_bottom_up_chunks; g = g->id->higher_goal) {
  ...
  if (!m_lhs) {
    if (thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]) {
      thisAgent->outputManager->display_soar_feedback(
          thisAgent, ebc_error_no_conditions,
          thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]);
      thisAgent->outputManager->printa_sf(
    ...
    if (ebc_settings[SETTING_EBC_INTERRUPT_WARNING]) {
      thisAgent->stop_soar = true;
      thisAgent->reason_for_stopping =
          "Chunking issue detected:  Rule learned had no conditions.";
    }
  ...
  /* Apply EBC correctness and (<s> ^quiescence t) filters that prevents rule
   * learning */
  if ((m_tested_local_negation &&
       !ebc_settings[SETTING_EBC_ALLOW_LOCAL_NEGATIONS]) ||
  ...
  if (m_tested_local_negation)
    thisAgent->explanationMemory->increment_stat_tested_local_negation(
        m_rule_type);
  if (m_tested_ltm_recall)
    thisAgent->explanationMemory->increment_stat_tested_ltm_recall(m_rule_type);
  if (m_tested_quiescence)
    thisAgent->explanationMemory->increment_stat_tested_quiescence();

  /* Create the name of the rule based on the type and circumstances of the
   * problem-solving */
  set_up_rule_name();
  ...
  /* Validate connectedness of chunk, repair if necessary and then re-order
   * conditions to reduce match costs */
  thisAgent->name_of_production_being_reordered = m_prod_name->sc->name;
  if (m_rule_type == ebc_chunk) {
    lChunkValidated = reorder_and_validate_chunk();
  }

  /* Handle rule learning failure.  With the addition of rule repair, this
   * should only happen when there is a repair failure.  Unless there's a bug in
   * the repair code, all rules should be reparable. */
  if (!lChunkValidated) {
    if (m_rule_type == ebc_chunk) {
      /* Could not re-order chunk, so we create a justification for the results
       * instead */
      m_rule_type = ebc_justification;
      lRevertedChunk = true;
      ...
      if (thisAgent->trace_settings[TRACE_CHUNKS_WARNINGS_SYSPARAM]) {
        thisAgent->outputManager->printa_sf(
            thisAgent,
            "Soar will learn a justification instead of a variablized rule.");
      }
    }
  }
   ...
   *
   * The next step in EBC will create an instantiated list of conditions based
   * on the variablized conditions.  This is used as the instantiation for the
   * chunk at the goal level (m_chunk_inst).  After being submitted to the RETE,
   * it is then chunked over if necessary for bottom up chunking. */

  if (ebc_settings[SETTING_EBC_LEARNING_ON] &&
      ((m_rule_type == ebc_chunk) || lRevertedChunk)) {
    l_inst_top = reinstantiate_current_rule();
    l_inst_bottom = l_inst_top;
    while (l_inst_bottom->next) l_inst_bottom = l_inst_bottom->next;
  } else {
    copy_condition_list(thisAgent, m_lhs, &l_inst_top, &l_inst_bottom, false,
                        false, false, false);
  }

  /* Create the production that will be added to the RETE */
  m_prod = make_production(thisAgent, m_prod_type, m_prod_name,
                           m_inst->prod ? m_inst->prod->original_rule_name
                                        : m_inst->prod_name->sc->name,
                           &m_lhs, &m_rhs, false, NULL);
  ...
  m_prod_name =
      NULL; /* Production struct is now responsible for the production name */
  ...
  if (lAddedSuccessfully) {
    ...
    /* Clean up.  (Now that m_chunk_inst s on the list of insts to be asserted,
     * we set it to to null because so that clean_up() won't delete it.) */
    m_chunk_inst = NULL;
    clean_up(l_clean_up_id, lLocalTimerPtr);

    if ((*new_inst_list)->match_goal_level > TOP_GOAL_LEVEL) {
      /* Initiate bottom-up chunking, i.e. tell EBC to learn a rule for the
       * chunk instantiation, which is what would have been created if the chunk
       * had fired on its goal level */
...
void Explanation_Based_Chunker::clean_up(uint64_t pClean_up_id,
                                         soar_timer* pTimer) {
