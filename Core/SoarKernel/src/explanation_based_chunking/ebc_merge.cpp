...
void Explanation_Based_Chunker::clear_merge_map() { cond_merge_map->clear(); }

void Explanation_Based_Chunker::merge_values_in_conds(condition* pDestCond,
                                                      condition* pSrcCond) {
  copy_non_identical_tests(thisAgent, &(pDestCond->data.tests.attr_test),
                           pSrcCond->data.tests.attr_test);
  copy_non_identical_tests(thisAgent, &(pDestCond->data.tests.value_test),
                           pSrcCond->data.tests.value_test);
  thisAgent->explanationMemory->increment_stat_merged_conditions();
}

condition* Explanation_Based_Chunker::get_previously_seen_cond(
    condition* pCond) {
  ...
  iter_id =
      cond_merge_map->find(pCond->data.tests.id_test->eq_test->data.referent);
  if (iter_id != cond_merge_map->end()) {
    iter_attr = iter_id->second.find(
        pCond->data.tests.attr_test->eq_test->data.referent);
    if (iter_attr != iter_id->second.end()) {
      iter_value = iter_attr->second.find(
...
void Explanation_Based_Chunker::merge_conditions() {
  int64_t current_cond = 1, cond_diff, new_num_conds,
          old_num_conds = count_conditions(m_lhs);
  ...
  for (condition* cond = m_lhs; cond; ++current_cond) {
    ...
    if (cond->type == POSITIVE_CONDITION) {
      ...
      if (found_cond) {
        ...
        (*cond_merge_map)[cond->data.tests.id_test->eq_test->data.referent]
                         [cond->data.tests.attr_test->eq_test->data.referent]
                         [cond->data.tests.value_test->eq_test->data.referent] =
                             cond;
      }
    } else {
      /* Search previous conditions for identical NC or NCC.
       *
       * I think identical NCCs might be impossible since collecting the grounds
       * uses a hash table to avoid adding the same NCC twice.
       *
       * We might be able to handle NCs with positive conditions if somehow
       * store a flag that indicates the type of the condition in the merge map.
       * Would need to use a more complex data structure.
       */
    }
    last_cond = cond;
    cond = next_cond;
