...
uint64_t Explanation_Based_Chunker::get_or_create_inst_identity_for_sym(
    Symbol* pSym) {
...
void Explanation_Based_Chunker::force_add_inst_identity(Symbol* pSym,
                                                        uint64_t pID) {
...
void Explanation_Based_Chunker::add_inst_identity_to_test(test pTest) {
  if (!pTest->inst_identity)
    pTest->inst_identity =
        get_or_create_inst_identity_for_sym(pTest->data.referent);
}

void Explanation_Based_Chunker::add_var_test_bound_identity_to_id_test(
    condition* cond, byte field_num, rete_node_level levels_up) {
  test t = var_test_bound_in_reconstructed_conds(thisAgent, cond, field_num,
                                                 levels_up);
  cond->data.tests.id_test->inst_identity = t->inst_identity;
}

void Explanation_Based_Chunker::add_explanation_to_condition(
    rete_node* node, condition* cond, node_varnames* nvn,
    ExplainTraceType ebcTraceType, bool inNegativeNodes) {
  ...
  if ((node->node_type == MP_BNODE) || (node->node_type == NEGATIVE_BNODE)) {
    ...
    add_var_test_bound_identity_to_id_test(
        cond, node->parent->left_hash_loc_field_num,
        node->parent->left_hash_loc_levels_up);
  }

  bool needs_eq_tests =
      ((!am->id && cond->data.tests.id_test &&
        !cond->data.tests.id_test->inst_identity &&
        !cond->data.tests.id_test->data.referent->is_variable()) ||
       (!am->attr && cond->data.tests.attr_test &&
        !cond->data.tests.attr_test->inst_identity &&
        !cond->data.tests.attr_test->data.referent->is_variable()) ||
       (!am->value && cond->data.tests.value_test &&
        !cond->data.tests.value_test->inst_identity &&
  ...
  if (needs_eq_tests) {
    ...
    for (; rt != NIL; rt = rt->next) {
      if (test_is_variable_relational_test(rt->type) &&
          (relational_test_type_to_test_type(
               kind_of_relational_test(rt->type)) == EQUALITY_TEST)) {
        ref_test = var_test_bound_in_reconstructed_conds(
    ...
    if (!am->id && cond->data.tests.id_test &&
        !cond->data.tests.id_test->inst_identity &&
        !cond->data.tests.id_test->data.referent->is_variable()) {
      cond->data.tests.id_test->inst_identity =
          thisAgent->explanationBasedChunker->get_new_inst_identity_id();
    }
    if (!am->attr && cond->data.tests.attr_test &&
        !cond->data.tests.attr_test->inst_identity &&
        !cond->data.tests.attr_test->data.referent->is_variable()) {
      cond->data.tests.attr_test->inst_identity =
          thisAgent->explanationBasedChunker->get_new_inst_identity_id();
    }
    if (!am->value && cond->data.tests.value_test &&
        !cond->data.tests.value_test->inst_identity &&
        !cond->data.tests.value_test->data.referent->is_variable()) {
      cond->data.tests.value_test->inst_identity =
          thisAgent->explanationBasedChunker->get_new_inst_identity_id();
  ...
  for (; rt != NIL; rt = rt->next) {
    ...
    if (rt->type == DISJUNCTION_RETE_TEST) {
      ...
      chunk_test->data.disjunction_list =
      ...
      if (test_is_constant_relational_test(rt->type)) {
        chunk_test =
            make_test(thisAgent, rt->data.constant_referent, test_type);
    ...
    if (chunk_test) {
      if (rt->right_field_num == 0)
        add_constraint_to_explanation(&(cond->data.tests.id_test), chunk_test,
                                      has_referent);
      else if (rt->right_field_num == 1)
        add_constraint_to_explanation(&(cond->data.tests.attr_test), chunk_test,
                                      has_referent);
      else
        add_constraint_to_explanation(&(cond->data.tests.value_test),
                                      chunk_test, has_referent);
    }
  }

  /* Make sure there's an equality test in each of the three fields.  These are
   * necessary for NCCs and NCs */
  if (!nvn) {
    ...
    if (!cond->data.tests.value_test || !cond->data.tests.value_test->eq_test) {
      add_new_chunk_variable(&(cond->data.tests.value_test),
...
void Explanation_Based_Chunker::add_new_chunk_variable(test* pTest, char pChar,
  ...
  if (*pTest)
    add_test(thisAgent, pTest, newTest);
  else
    *pTest = newTest;
  (*pTest)->eq_test->inst_identity = LITERAL_VALUE;
}

void Explanation_Based_Chunker::add_constraint_to_explanation(
    test* dest_test_address, test new_test, bool has_referent) {
  if (has_referent) {
    if ((*dest_test_address) && new_test && (new_test->type == EQUALITY_TEST)) {
      ...
      if (((destination->type == EQUALITY_TEST) &&
           (destination->data.referent == new_test->data.referent)) ||
          ((destination->type == CONJUNCTIVE_TEST) &&
           (destination->eq_test->data.referent == new_test->data.referent))) {
...
void Explanation_Based_Chunker::add_explanation_to_RL_condition(
    rete_node* node, condition* cond) {
  rete_test* rt = node->b.posneg.other_tests;
  ...
  for (; rt != NIL; rt = rt->next) {
    ...
    if (test_is_variable_relational_test(rt->type)) {
      ...
      if ((test_type == EQUALITY_TEST) || (test_type == NOT_EQUAL_TEST)) {
        test ref_test = var_test_bound_in_reconstructed_conds(
            thisAgent, cond, rt->data.variable_referent.field_num,
            rt->data.variable_referent.levels_up);
        if (ref_test->data.referent->is_sti() ||
            ref_test->data.referent->is_variable()) {
          chunk_test = make_test(thisAgent, ref_test->data.referent, test_type);
        }
      }
    }
