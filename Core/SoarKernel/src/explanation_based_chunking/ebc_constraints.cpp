...
void Explanation_Based_Chunker::clear_cached_constraints() {
  for (constraint_list::iterator it = constraints->begin();
       it != constraints->end(); ++it) {
    /* We intentionally used the tests in the conditions backtraced through
     * instead of copying them, so we don't need to deallocate the tests in the
     * constraint. We just delete the constraint struct that contains the two
     * pointers.*/
...
void Explanation_Based_Chunker::cache_constraints_in_test(test t) {
  ...
  for (cons* c = t->data.conjunct_list; c != NIL; c = c->rest) {
    ...
    if (test_can_be_transitive_constraint(ctest)) {
      thisAgent->memoryManager->allocate_with_pool(MP_constraints,
                                                   &new_constraint);
...
void Explanation_Based_Chunker::invert_relational_test(test* pEq_test,
                                                       test* pRelational_test) {
...
void Explanation_Based_Chunker::attach_relational_test(test pRelational_test,
                                                       condition* pCond,
                                                       WME_Field pField) {
  if (pField == VALUE_ELEMENT) {
    add_test(thisAgent, &(pCond->data.tests.value_test), pRelational_test,
             true);
...
void Explanation_Based_Chunker::add_additional_constraints() {
  ...
  for (constraint_list::iterator iter = constraints->begin();
       iter != constraints->end();) {
    lConstraint = *iter;
    condition* lOperationalCond =
        lConstraint->eq_test->identity
            ? lConstraint->eq_test->identity->get_operational_cond()
            : NULL;
    condition* lOperationalConstraintCond =
        lConstraint->constraint_test->identity
            ? lConstraint->constraint_test->identity->get_operational_cond()
            : NULL;

    if (lOperationalCond && !lConstraint->eq_test->identity->literalized()) {
      constraint_test =
          copy_test(thisAgent, lConstraint->constraint_test, true);
      attach_relational_test(
          constraint_test, lOperationalCond,
          lConstraint->eq_test->identity->get_operational_field());
    } else if (lOperationalConstraintCond &&
               !lConstraint->constraint_test->identity->literalized()) {
      eq_copy = copy_test(thisAgent, lConstraint->eq_test, true);
      constraint_test =
          copy_test(thisAgent, lConstraint->constraint_test, true);
      invert_relational_test(&eq_copy, &constraint_test);
      attach_relational_test(
          constraint_test, lOperationalConstraintCond,
          lConstraint->constraint_test->identity->get_operational_field());
