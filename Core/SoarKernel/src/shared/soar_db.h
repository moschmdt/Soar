...
template <typename T> class status_object {
protected:
  T my_status;
  int my_errno;
  char *my_errmsg;
...
public:
  ...
  virtual ~status_object() { set_errmsg(NULL); }
...
public:
  ...
  inline void reinitialize() { _reinit(); }
...
public:
  ...
  virtual ~statement_container() {
    for (std::list<statement *>::iterator p = statements->begin();
         p != statements->end(); p++) {
      delete (*p);
    }
  ...
  void add(statement *new_statement) { statements->push_back(new_statement); }
...
public:
  sqlite_database() : database(), my_db(NULL) { set_errno(SQLITE_OK); }
  virtual ~sqlite_database() {}

  inline sqlite3 *get_db() { return my_db; }

  void connect(const char *file_name,
...
protected:
  ...
  inline void sqlite_err(int sqlite_res) {
    ...
    fprintf(stderr,
            "SoarDB| Unexpected sqlite result!  result = %d. error = %d (%s)\n",
  ...
  virtual bool _prep() {
    ...
    int sqlite_res = sqlite3_prepare_v2(my_db->get_db(), sql,
                                        SQLITE_PREP_STR_MAX, &(my_stmt), &tail);
  ...
  virtual void _reinit() { sqlite3_reset(my_stmt); }
  ...
  virtual exec_result _exec() {
    ...
    if ((sqlite_res != SQLITE_OK) && (sqlite_res != SQLITE_DONE) &&
...
public:
  sqlite_statement(sqlite_database *new_db, const char *new_sql,
  ...
  inline void bind_null(int param) { sqlite3_bind_null(my_stmt, param); }

  inline void bind_text(int param, const char *val) {
    sqlite3_bind_text(my_stmt, param, val, SQLITE_PREP_STR_MAX, SQLITE_STATIC);
  }

  inline void bind_text_transient(int param, const char *val) {
    if (val) {
      sqlite3_bind_text(my_stmt, param, val, SQLITE_PREP_STR_MAX,
                        SQLITE_TRANSIENT);
    } else {
...
public:
  sqlite_statement_container(sqlite_database *new_db)
      : statement_container(), structures(new std::list<const char *>()),
        my_db(new_db) {}

  virtual ~sqlite_statement_container() { delete structures; }
  ...
  void structure() {
    ...
    for (std::list<const char *>::iterator p = structures->begin();
         p != structures->end(); p++) {
      ...
      if (execute_result == err) {
        fprintf(stderr,
                "SoarDB| Unexpected sqlite result in structure!  result = %d. "
                "error = %d (%s)\n",
                execute_result, temp_stmt->get_errno(),
                temp_stmt->get_errmsg());
        fprintf(stderr, "SoarDB|...in SQL statement: %s\n", (*p));
      }
#endif
      delete temp_stmt;
...
public:
  pooled_sqlite_statement(sqlite_statement_pool *new_pool,
                          sqlite_database *new_db, const char *new_sql,
                          timer *new_query_timer = NULL)
      : sqlite_statement(new_db, new_sql, new_query_timer),
        stmt_pool(new_pool) {}

  inline sqlite_statement_pool *get_pool() { return stmt_pool; }
};

class sqlite_statement_pool {
protected:
#ifdef USE_MEM_POOL_ALLOCATORS
  typedef std::list<pooled_sqlite_statement *,
                    soar_memory_pool_allocator<pooled_sqlite_statement *>>
...
public:
  sqlite_statement_pool(agent *thisAgent, sqlite_database *new_db,
                        const char *new_sql)
  ...
  ~sqlite_statement_pool() {
    for (sqlite_statement_pool_pool::iterator it = statements->begin();
  ...
  pooled_sqlite_statement *request(timer *query_timer = NULL) {
    ...
    if (statements->empty()) {
      ...
      return_val =
          new pooled_sqlite_statement(this, my_db, my_sql, query_timer);
...
inline bool sqlite_database::sql_simple_get_int(const char *sql,
                                                int64_t &return_value) {
  soar_module::sqlite_statement *temp_q =
      new soar_module::sqlite_statement(this, sql);
  temp_q->prepare();
...
inline bool sqlite_database::sql_execute(const char *sql) {
  ...
  if (execute_result == err) {
    fprintf(stderr,
            "SoarDB| Unexpected sqlite result in sql_execute!  result = %d. "
            "error = %d (%s)\n",
...
inline bool sqlite_database::sql_simple_get_float(const char *sql,
                                                  double &return_value) {
  soar_module::sqlite_statement *temp_q =
      new soar_module::sqlite_statement(this, sql);
  temp_q->prepare();
...
inline bool sqlite_database::sql_simple_get_string(const char *sql,
...
inline bool sqlite_database::sql_is_new_db(bool &return_value) {
  ...
  value_retrieved = sql_simple_get_int(
      "SELECT count(*) FROM sqlite_master WHERE type='table'", numTables);
  if (value_retrieved) {
