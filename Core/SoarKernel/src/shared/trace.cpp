...
trace_format* parse_item_from_format_string(agent* thisAgent) {
  ...
  if (*format != '%') {
    ...
    *ch = 0;
    tf = static_cast<trace_format_struct*>(
        thisAgent->memoryManager->allocate_memory(sizeof(trace_format),
  ...
  for (; tf != NIL; tf = tf->next) {
    switch (tf->type) {
      case STRING_TFT: {
        ...
        //                size_t i, len;
        //
        //                s = string_to_escaped_string(tf->data.string, '"',
        //                NULL); len = strlen(s); for (i = 1; i < len - 1; i++)
        //                {
        ...
        if (tf->data.attribute_path) {
          for (c = tf->data.attribute_path; c != NIL; c = c->rest) {
            thisAgent->outputManager->printa(
                thisAgent, static_cast<Symbol*>(c->first)->sc->name);
            if (c->rest) {
              thisAgent->outputManager->printa(thisAgent, ".");
            }
          }
...
void init_tracing(agent* thisAgent) {
  ...
  for (i = 0; i < 3; i++) {
    thisAgent->object_tr_ht[i] =
        make_hash_table(thisAgent, 0, tracing_rule_hash_function);
    thisAgent->stack_tr_ht[i] =
        make_hash_table(thisAgent, 0, tracing_rule_hash_function);
...
trace_format* lookup_trace_format(agent* thisAgent, bool stack_trace,
...
bool remove_trace_format(agent* thisAgent, bool stack_trace,
...
bool add_trace_format(agent* thisAgent, bool stack_trace, int type_restriction,
  ...
  remove_trace_format(thisAgent, stack_trace, type_restriction,
                      name_restriction);
  ...
  if (name_restriction) {
    ...
    tr = static_cast<tracing_rule_struct*>(
        thisAgent->memoryManager->allocate_memory(sizeof(tracing_rule),
                                                  MISCELLANEOUS_MEM_USAGE));
...
void print_tracing_rule(agent* thisAgent, int type_restriction,
                        Symbol* name_restriction, trace_format* format) {
  ...
  thisAgent->outputManager->printa_sf(thisAgent, " :add %c ",
                                      tracing_object_letters[type_restriction]);
  if (name_restriction) {
...
void print_tracing_rule_tcl(agent* thisAgent, int type_restriction,
                            Symbol* name_restriction, trace_format* format) {
  thisAgent->outputManager->printa_sf(thisAgent, "%c ",
                                      tracing_object_letters[type_restriction]);
  if (name_restriction) {
    thisAgent->outputManager->printa_sf(thisAgent, "%y ", name_restriction);
  }
...
bool print_trace_callback_fn(agent* thisAgent, void* item, void*) {
  ...
  tr = static_cast<tracing_rule_struct*>(item);
  print_tracing_rule(thisAgent, tr->type_restriction, tr->name_restriction,
                     tr->format);
  return false;
}
  ...
  if (stack_trace) {
    for (i = 0; i < 3; i++) {
      if (thisAgent->stack_tf_for_anything[i]) {
        print_tracing_rule(thisAgent, i, NIL,
                           thisAgent->stack_tf_for_anything[i]);
      }
      do_for_all_items_in_hash_table(thisAgent, thisAgent->stack_tr_ht[i],
                                     print_trace_callback_fn,
    ...
    for (i = 0; i < 3; i++) {
      if (thisAgent->object_tf_for_anything[i]) {
        print_tracing_rule(thisAgent, i, NIL,
                           thisAgent->object_tf_for_anything[i]);
      }
      do_for_all_items_in_hash_table(thisAgent, thisAgent->object_tr_ht[i],
                                     print_trace_callback_fn,
                                     reinterpret_cast<void*>(f));
    }
  }
...
void print_all_trace_formats_tcl(agent* thisAgent, bool stack_trace, FILE* f) {
  ...
  if (stack_trace) {
    for (i = 0; i < 3; i++) {
      if (thisAgent->stack_tf_for_anything[i]) {
        print_tracing_rule_tcl(thisAgent, i, NIL,
                               thisAgent->stack_tf_for_anything[i]);
      }
      do_for_all_items_in_hash_table(thisAgent, thisAgent->stack_tr_ht[i],
                                     print_trace_callback_fn_tcl,
                                     reinterpret_cast<void*>(f));
    }
  } else {
    for (i = 0; i < 3; i++) {
...
inline void set_tagged_trace_formats(agent* thisAgent) {
  ...
  add_trace_format(thisAgent, false, FOR_STATES_TF, NIL,
                   "%id\" %ifdef[impasse_object=\"%v[attribute]\" "
                   "impasse_type=\"%v[impasse]\"]");
  /***********  enable when determine tagged output format
  { Symbol *evaluate_object_sym;
    evaluate_object_sym = make_str_constant (thisAgent, "evaluate-object");
    add_trace_format (thisAgent, false, FOR_OPERATORS_TF, evaluate_object_sym,
  ...
  *************/
  ...
  add_trace_format(thisAgent, true, FOR_STATES_TF, NIL,
                   "<state stack_level=\"%sd\" decision_cycle_count=\"%dc\" "
                   "current_state_id=\"%cs>");
  add_trace_format(thisAgent, true, FOR_OPERATORS_TF, NIL,
                   "<operator stack_level=\"%sd\" decision_cycle_count=\"%dc\" "
                   "current_operator_id=\"%co>");
}
...
void add_values_of_attribute_path(agent* thisAgent, Symbol* object, cons* path,
                                  growable_string* result, bool recursive,
                                  int* count) {
  slot* s;
  wme* w;
  char* ch;
  ...
  for (w = object->id->impasse_wmes; w != NIL; w = w->next)
    if (w->attr == path->first)
      add_values_of_attribute_path(thisAgent, w->value, path->rest, result,
                                   recursive, count);
...
void add_trace_for_wme(agent* thisAgent, growable_string* result, wme* w,
                       bool print_attribute, bool recursive) {
  char* ch;
  growable_string gs;
...
void add_trace_for_attribute_path(agent* thisAgent, Symbol* object, cons* path,
                                  growable_string* result,
                                  bool print_attributes, bool recursive) {
  ...
  if (!path) {
    ...
    if (length_of_growable_string(values) > 0) {
      add_to_growable_string(thisAgent, result,
                             text_of_growable_string(values) + 1);
    }
  ...
  count = 0;
  add_values_of_attribute_path(thisAgent, object, path, &values, recursive,
                               &count);
  if (!count) {
...
growable_string trace_format_list_to_string(agent* thisAgent, trace_format* tf,
                                            Symbol* object) {
#define GROWABLE_STRING_TRACE_FORMAT_LIST_TO_STRING_BUFFER_SIZE 50
  ...
  for (; tf != NIL; tf = tf->next) {
    switch (tf->type) {
      ...
      case VALUES_TFT:
        add_trace_for_attribute_path(thisAgent, object, tf->data.attribute_path,
                                     &result, false, false);
        break;
      case VALUES_RECURSIVELY_TFT:
        add_trace_for_attribute_path(thisAgent, object, tf->data.attribute_path,
      ...
      case CURRENT_STATE_TFT:
        if (!tparams.current_s) {
          ...
          add_to_growable_string(thisAgent, &result,
                                 text_of_growable_string(temp_gs));
      ...
      case DECISION_CYCLE_COUNT_TFT:
        if (tparams.allow_cycle_counts) {
          SNPRINTF(buf, GROWABLE_STRING_TRACE_FORMAT_LIST_TO_STRING_BUFFER_SIZE,
                   "%" SCNu64, thisAgent->d_cycle_count);
          buf[GROWABLE_STRING_TRACE_FORMAT_LIST_TO_STRING_BUFFER_SIZE - 1] =
      ...
      case ELABORATION_CYCLE_COUNT_TFT:
        if (tparams.allow_cycle_counts) {
          SNPRINTF(buf, GROWABLE_STRING_TRACE_FORMAT_LIST_TO_STRING_BUFFER_SIZE,
                   "%" SCNu64, thisAgent->e_cycle_count);
          buf[GROWABLE_STRING_TRACE_FORMAT_LIST_TO_STRING_BUFFER_SIZE - 1] =
              0; /* ensure null termination */
          add_to_growable_string(thisAgent, &result, buf);
        } else {
      ...
      case IF_ALL_DEFINED_TFT: {
        ...
        temp_gs =
            trace_format_list_to_string(thisAgent, tf->data.subformat, object);
        if (!found_undefined) {
          add_to_growable_string(thisAgent, &result,
                                 text_of_growable_string(temp_gs));
        }
      ...
      case SUBGOAL_DEPTH_TFT:
        if (tparams.current_s) {
          SNPRINTF(buf, GROWABLE_STRING_TRACE_FORMAT_LIST_TO_STRING_BUFFER_SIZE,
                   "%u", tparams.current_s->id->level - 1);
          buf[GROWABLE_STRING_TRACE_FORMAT_LIST_TO_STRING_BUFFER_SIZE - 1] =
              0; /* ensure null termination */
          add_to_growable_string(thisAgent, &result, buf);
        } else {
      ...
      case REPEAT_SUBGOAL_DEPTH_TFT:
        if (tparams.current_s) {
          temp_gs = trace_format_list_to_string(thisAgent, tf->data.subformat,
                                                object);
...
trace_format* find_appropriate_trace_format(agent* thisAgent, bool stack_trace,
...
growable_string selection_to_trace_string(agent* thisAgent, Symbol* object,
...
void print_stack_trace_xml(agent* thisAgent, Symbol* object, Symbol* state,
  ...
  switch (slot_type) {
    ...
    case FOR_OPERATORS_TF:
       ...
       *    context, since the level of the state should also be the level of
       * the operator.  (Could be wrong about this.) Leaving this comment
       * because it's possible that the real problem is that the level of the
       * operator isn't being set somewhere else to the proper value. - MMA
       * 9-2014*/
      xml_att_val(thisAgent, kState_StackLevel, state->id->level - 1);
      xml_att_val(thisAgent, kOperator_DecisionCycleCt,
    ...
    default:
      // technically it looks like we should try to print a name here, but I
      // think that is an artifact (at least it doesn't make sense for the XML)
      break;
  }
  ...
  add_trace_format (thisAgent, true, FOR_STATES_TF, NIL,
                  "<state stack_level=\"%sd\" decision_cycle_count=\"%dc\"
  current_state_id=\"%cs>"); add_trace_format (thisAgent, true,
  FOR_OPERATORS_TF, NIL,
                  "<operator stack_level=\"%sd\" decision_cycle_count=\"%dc\"
  current_operator_id=\"%co>");
...
void print_stack_trace(agent* thisAgent, Symbol* object, Symbol* state,
                       int slot_type, bool allow_cycle_counts) {
  growable_string gs;

  thisAgent->tf_printing_tc = get_new_tc_number(thisAgent);
  gs = selection_to_trace_string(thisAgent, object, state, slot_type,
                                 allow_cycle_counts);
  thisAgent->outputManager->printa(thisAgent, text_of_growable_string(gs));
  free_growable_string(thisAgent, gs);

  // RPM 5/05
  print_stack_trace_xml(thisAgent, object, state, slot_type,
                        allow_cycle_counts);
}
