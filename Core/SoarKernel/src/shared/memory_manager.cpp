...
Memory_Manager::~Memory_Manager() {
  ...
  for (std::unordered_map<size_t, memory_pool*>::iterator it =
           dyn_memory_pools.begin();
       it != dyn_memory_pools.end(); it++) {
    delete it->second;
  }
  dyn_memory_pools.clear();
}

void Memory_Manager::init_memory_pool_by_ptr(memory_pool* pThisPool,
                                             size_t item_size,
                                             const char* name) {
  ...
  if (strlen(name) > MAX_POOL_NAME_LENGTH) {
    ...
    SNPRINTF(
        msg, msg_size,
        "memory_manager.cpp: Internal error: memory pool name too long: %s\n",
        name);
...
void Memory_Manager::init_memory_pool(MemoryPoolType mempool_index,
...
memory_pool* Memory_Manager::get_memory_pool(size_t size) {
  ...
  std::unordered_map<size_t, memory_pool*>::iterator it =
...
void Memory_Manager::free_memory_pool_by_ptr(memory_pool* pThisPool) {
  //    std::cout << "Free memory pool called for" << pThisPool->name <<
  //    std::endl;
...
bool Memory_Manager::add_block_to_memory_pool_by_name(
    const std::string& pool_name, int blocks) {
  //    char* new_block;
  ...
  for (lThisPool = memory_pools_in_use; lThisPool != NIL;
       lThisPool = lThisPool->next) {
    if (pool_name == lThisPool->name) break;
  }
  if (!lThisPool) return false;
...
void Memory_Manager::add_block_to_memory_pool(memory_pool* pThisPool) {
  ...
  *(char**)new_block = static_cast<char*>(pThisPool->first_block);
  ...
  /* somewhere in here, need to check if total mem usage exceeds limit set by
user we only check when increasing pools, because the other memories are small
by comparison, we shouldn't check for every block added to any pool, since that
...
void* Memory_Manager::allocate_memory(size_t size, int usage_code) {
  ...
  if (p == NULL) {
    ...
    SNPRINTF(msg, BUFFER_MSG_SIZE,
             "\nmemory_manager.cpp: Error:  Tried but failed to allocate %zu "
             "bytes of memory.\n",
             size);
  ...
  if (reinterpret_cast<uintptr_t>(p) & 3) {
    char msg[BUFFER_MSG_SIZE];
    strncpy(msg,
            "\nmemory_manager.cpp: Error:  Memory allocator returned an "
            "address that's not a multiple of 4.\n",
            BUFFER_MSG_SIZE);
    msg[BUFFER_MSG_SIZE - 1] = 0; /* ensure null termination */
    abort_with_fatal_error_noagent(msg);
  }
...
void* Memory_Manager::allocate_memory_and_zerofill(size_t size,
                                                   int usage_code) {
  void* p;
...
void Memory_Manager::debug_print_memory_stats(agent* thisAgent) {
  ...
  std::cout << "Soar " << sml::sml_Names::kSoarVersionValue << " on "
            << hostname << " at " << ctime(&current_time) << "\n";
  ...
  std::cout << "                                                        |   "
               "Computed\n";
  std::cout << "Phases:      Input   Propose   Decide   Apply    Output |     "
               "Totals\n";
  std::cout << "========================================================|======"
               "=====\n";

  std::cout << "Kernel:   " << std::setw(8)
            << thisAgent->timers_decision_cycle_phase[INPUT_PHASE].get_sec()
            << " " << std::setw(8)
            << thisAgent->timers_decision_cycle_phase[PROPOSE_PHASE].get_sec()
            << " " << std::setw(8)
  ...
  std::cout << "Input fn: " << std::setw(8) << input_function_time
            << "                                      | " << std::setw(10)
            << input_function_time << "\n";

  std::cout << "========================================================|======"
               "=====\n";
  std::cout << "Outpt fn:                                     " << std::setw(8)
  ...
  std::cout << "Computed------------------------------------------------+------"
               "-----\n";
  std::cout << "Totals:   " << std::setw(8) << input_phase_total_time << " "
  ...
  std::cout << "==============================================================="
               "==============|===========\n";
  std::cout << "Phases:        Input   Propose   Decide   Apply    Pref     "
               "WrkMem    Output |     Totals\n";
  std::cout << "==============================================================="
               "==============|===========\n";
  std::cout << "Match:      " << std::setw(8)
            << thisAgent->timers_match_cpu_time[INPUT_PHASE].get_sec() << " "
            << std::setw(8)
            << thisAgent->timers_match_cpu_time[PROPOSE_PHASE].get_sec() << " "
            << std::setw(8)
  ...
  std::cout << " Kernel CPU Time: " << std::setw(11) << total_kernel_time
            << " sec. \n";

  std::cout << " Total  CPU Time: " << std::setw(11)
  ...
  std::cout << wme_changes << " wme changes (" << thisAgent->wme_addition_count
            << " additions, " << thisAgent->wme_removal_count << " removals)\n";

  std::cout << "WM size: " << thisAgent->num_wmes_in_rete << " current, "
...
#ifdef MEMORY_POOL_STATS
  std::cout << "Pool Name        Used Items  Free Items  Item Size  Itm/Blk  "
               "Blocks  Total Bytes\n";
  std::cout << "---------------  ----------  ----------  ---------  -------  "
               "------  -----------\n";
#else
