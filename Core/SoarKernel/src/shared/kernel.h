 ...
 * 3 -  Memory pools and memory pool allocators will be turned on (if disabled
 * below) 4 -  Debugging flags below will be disabled
 *
 * Note:  The default build is optimized, so NDEBUG will be true, which will
 * automatically make SOAR_RELEASE_VERSION also true.
 *
 *        (use --dbg for debug build)
 *
 * Note:  Debug printing will prevent Soar from sending output to the
 * SoarJavaDebugger.
 * */

/* BUG_139_WORKAROUND:  Print a warning whenever we are ignoring a situation
 * when there's no instance to retract for a justification.  We can't find
 * documentation on what the original bug was.  We have seen the warning pop up
 * in agents still.  */
...
/* Spreading Activation Switch: Spreading can incur some small cost even when
 * off because of record-keeping in case it is later turned on. To reduce this
...
#ifdef MEM_POOLS_ENABLED
#define USE_MEM_POOL_ALLOCATORS \
  1 /* Whether to use custom STL allocators that use memory pools */
#endif
