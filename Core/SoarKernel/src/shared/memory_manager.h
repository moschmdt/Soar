 ...
 * is causing a problem, you could use that option to test it.
 */
...
class Memory_Manager {
 ...
 public:
  ...
  void init_memory_pool(MemoryPoolType mempool_index, size_t item_size,
                        const char* name);
  void init_memory_pool_by_ptr(memory_pool* pThisPool, size_t item_size,
                               const char* name);
  ...
  bool add_block_to_memory_pool_by_name(const std::string& pool_name,
                                        int blocks);
 ...
 public:
  template <typename T>
  inline void allocate_with_pool(MemoryPoolType mempool_index,
                                 T** dest_item_pointer) {
    ...
    *(dest_item_pointer) = static_cast<T*>(lThisPool->free_list);
    // we think this line increments free_list to the next available memory
    // block we thought it took advantage of the fact that free_list is the
    // first
    //  member of memory_pool, but I tried changing that and it still works, so
    //  now I'm at a loss
    ...
    // this is for debugging -- it disables the memory pool usage and just
    // allocates
    //  new memory every time.  If you want to use it, be sure to make the
    //  corresponding change to free_with_pool below
  ...
  template <typename T>
  inline void allocate_with_pool_ptr(memory_pool* pThisPool,
                                     T** dest_item_pointer) {
#if MEM_POOLS_ENABLED
    // if there's no memory blocks left in the pool, then allocate a new one
  ...
  inline void free_with_pool(MemoryPoolType mempool_index, T* item) {
    ...
    *(void**)(item) = lThisPool->free_list;
    ...
    // this is for debugging -- it disables the memory pool usage and just
    // deallocates
    //  the memory every time.  If you want to use it, be sure to make the
    //  corresponding change to allocate_with_pool above
    free(item);
