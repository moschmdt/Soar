...
/* UITODO| Make this preference type to string.  Maybe move to preference
 * struct? */
  ...
  while (conds_not_yet_printed) {
    ...
    while (conds_for_this_id) {
...

      {
        ...
        Output_Manager::Get_OM().sprinta_sf_cstr(
            thisAgent, ch, PRINT_CONDITION_LIST_TEMP_SIZE - (ch - temp), "%t",
  ...
  while (actions_not_yet_printed) {
    ...
    while (actions_for_this_id) {
...

      {
        ...
        thisAgent->outputManager->rhs_value_to_cstring(
            a->attr, ch, PRINT_ACTION_LIST_TEMP_SIZE - (ch - temp));
        while (*ch) {
          ch++;
        }
        *(ch++) = ' ';
        thisAgent->outputManager->rhs_value_to_cstring(
            a->value, ch, PRINT_ACTION_LIST_TEMP_SIZE - (ch - temp));
        while (*ch) {
          ch++;
        }
        if ((a->preference_type == REJECT_PREFERENCE_TYPE) ||
            (rhs_value_is_symbol(a->attr) &&
             (rhs_value_to_symbol(a->attr) ==
          ...
          *(ch++) = preference_to_char(a->preference_type);
          if (preference_is_binary(a->preference_type)) {
            *(ch++) = ' ';
            thisAgent->outputManager->rhs_value_to_cstring(
                a->referent, ch, PRINT_ACTION_LIST_TEMP_SIZE - (ch - temp));
            while (*ch) {
              ch++;
            }
          }
  ...
  switch (p->type) {
    ...
    case JUSTIFICATION_PRODUCTION_TYPE:
      thisAgent->outputManager->printa(
          thisAgent, "    :justification ;# not reloadable\n");
...
void print_preference(agent* thisAgent, preference* pref, bool add_lf) {
  ...
  thisAgent->outputManager->printa_sf(thisAgent, "(%y ^%y %y ", pref->id,
                                      pref->attr, pref->value);
...
void print_consed_list_of_condition_wmes(agent* thisAgent, cons* c,
                                         int indent) {
  for (; c != NIL; c = c->rest) {
    if (thisAgent->outputManager->get_printer_output_column(thisAgent) >=
        COLUMNS_PER_LINE - 20) {
...
void print_wme(agent* thisAgent, wme* w) {
  thisAgent->outputManager->printa_sf(thisAgent, "(%u: %y ^%y %y", w->timetag,
                                      w->id, w->attr, w->value);
  ...
  // <wme tag="123" id="s1" attr="foo" attrtype="string" val="123"
  // valtype="string"></wme>
  xml_object(thisAgent, w);
}

void print_wme_without_timetag(agent* thisAgent, wme* w) {
  thisAgent->outputManager->printa_sf(thisAgent, "(%y ^%y %y", w->id, w->attr,
                                      w->value);
  if (w->acceptable) {
    thisAgent->outputManager->printa(thisAgent, " +");
  }
...
extern "C" bool passes_wme_filtering(agent* thisAgent, wme* w, bool isAdd) {
  ...
  for (c = thisAgent->wme_filter_list; c != NIL; c = c->rest) {
    ...
    /*     thisAgent->outputManager->printa_sf(thisAgent, "  trying filter: %y
...
extern void print_sysparam_trace(agent* thisAgent, int64_t sysParamIndex,
  ...
  if ((sysParamIndex == INVALID_SYSPARAM) ||
      thisAgent->trace_settings[sysParamIndex]) {
    va_start(args, format);
    thisAgent->outputManager->sprinta_sf_given_va(
        thisAgent, buf, format,
        args);  // thisAgent->outputManager->sprinta_sf(thisAgent,
                // buf, format, args);
    // sjj - in response to github issue number 304, I have added a different
    // function as a pure kernel hack. It seems you cannot pass va_lists as
    // variadic arguments. However, you can pass them as va_lists, so I just did
    // that instead. I have left the rest as it was in case this was the wrong
    // decision.
    va_end(args);
