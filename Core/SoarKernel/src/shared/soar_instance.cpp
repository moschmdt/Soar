...
Soar_Instance::Soar_Instance() {
  ...
  m_tcl_enabled = false;
  m_loadedLibraries =
      new std::unordered_map<std::string, Soar_Loaded_Library*>();
...
Soar_Instance::~Soar_Instance() {
  ...
  for (std::unordered_map<std::string, Soar_Loaded_Library*>::iterator it =
           (*m_loadedLibraries).begin();
       it != (*m_loadedLibraries).end(); ++it) {
...
void Soar_Instance::Register_Library(sml::Kernel* pKernel, const char* pLibName,
  ...
  if (iter == (*m_loadedLibraries).end()) {
    if (!pMessageFunction) {
      m_Output_Manager->print(
          "Library did not pass in a message function.  Not registering.\n");
      return;
    }
...
std::string Soar_Instance::Message_Library(std::string& pMessage) {
  ...
  std::unordered_map<std::string, Soar_Loaded_Library*>::iterator iter =
      (*m_loadedLibraries).find(lCLIExtensionName.c_str());
  if (iter == (*m_loadedLibraries).end()) {
    // load library
    std::string result =
        m_Kernel->LoadExternalLibrary(lCLIExtensionName.c_str());

    // zero length is success
    if (result.size() != 0) {
      resultString =
          "Could not load library " + lCLIExtensionName + ": " + result;
      return resultString;
    }
  }
  ...
  if (((lMessage == "on") && libraryInfo->isOn) ||
      ((lMessage == "off") && !libraryInfo->isOn)) {
    resultString = "CLI extension " + lCLIExtensionName + "is already " +
                   lMessage + ".  Ignoring command.";
    return resultString;
  }
  if (lMessage == "off") {
    resultString =
        "Turning off CLI modules is currently disabled. Will be fixed in "
        "future version.  Restart Soar to turn off for now.";
    return resultString;
  }
  void* success = libraryInfo->libMessageFunction(lMessage.c_str(), NULL);
  if (!success) {
    resultString = "Message " + lMessage + " to CLI library " +
                   lCLIExtensionName + " returned unsuccessful.";
    return resultString;
  } else {
    ...
    if (lMessage == "on") {
      ...
      m_tcl_enabled = true;  // This was intended to be general purpose, but
                             // currently only used for tcl
    } else if (lMessage == "off") {
      resultString = lCLIExtensionName + " has been deactivated.\n";
      m_Output_Manager->print(resultString.c_str());
      libraryInfo->isOn = false;
      m_tcl_enabled = false;  // This was intended to be general purpose, but
                              // currently only used for tcl
    }
  }
...
void Soar_Instance::Register_Soar_AgentSML(char* pAgentName,
                                           sml::AgentSML* pSoarAgentSML) {
  ...
  if (iter != (*m_agent_table).end()) {
    ...
    if (update_OM) {
      if (m_agent_table->size() > 0) {
        m_Output_Manager->set_default_agent(
            (*m_agent_table).begin()->second->GetSoarAgent());
      } else {
...
void Soar_Instance::Print_Agent_Table() {
  ...
  m_Output_Manager->print("------------------------------------\n");
  for (std::unordered_map<std::string, sml::AgentSML*>::iterator it =
           (*m_agent_table).begin();
       it != (*m_agent_table).end(); ++it) {
    m_Output_Manager->print_sf("%s -> %s\n", it->first.c_str(),
                               it->second->GetSoarAgent()->name);
  }
}
...
void configure_agent_for_unit_tests(agent* testAgent) {
  agent* thisAgent =
      testAgent ? testAgent : Output_Manager::Get_OM().get_default_agent();
  if (thisAgent) {
...
Output_Manager* getOM() { return &(Output_Manager::Get_OM()); }
