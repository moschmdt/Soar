...
char* make_memory_block_for_string(agent* thisAgent, char const* s) {
  ...
  p = static_cast<char*>(
      thisAgent->memoryManager->allocate_memory(size, STRING_MEM_USAGE));
  strncpy(p, s, size);
...
growable_string make_blank_growable_string(agent* thisAgent) {
  ...
  gs = thisAgent->memoryManager->allocate_memory(
      2 * sizeof(int*) + INITIAL_GROWABLE_STRING_SIZE, STRING_MEM_USAGE);
...
void add_to_growable_string(agent* thisAgent, growable_string* gs,
  ...
  if (new_length + 1 > static_cast<size_t>(memsize_of_growable_string(*gs))) {
    ...
    New = thisAgent->memoryManager->allocate_memory(
        new_memsize + 2 * sizeof(int*), STRING_MEM_USAGE);
...
cons* extract_list_elements(agent* thisAgent, cons** header, cons_test_fn f,
                            void* data) {
...
dl_list* extract_dl_list_elements(agent* thisAgent, dl_list** header,
                                  dl_cons_test_fn f) {
