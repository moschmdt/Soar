 ...
 * to generating string output.  In the past we added code to some of those
 * functions so they'd generate string output and also XML output (as a string:
 * <aaa>...</aaa>).  To capture the XML output a caller would register for the
 * XML callback, generate the XML as a string, parse the XML back to an object
 * and return it.
 *
 * These methods generate XML as an object, so there are no strings being
 * created and subsequently parsed and no need to "intercept" the XML callback
 * channel (which is really for XML trace output to the debugger, not for
...
void xml_reset(agent* pAgent) {
  ...
  soarxml::XMLTrace* pTrace =
      static_cast<soarxml::XMLTrace*>(pAgent->xml_trace);
  soarxml::XMLTrace* pCommands =
      static_cast<soarxml::XMLTrace*>(pAgent->xml_commands);
...
void xml_begin_tag(agent* pAgent, char const* pTag) {
  soarxml::XMLTrace* pXML =
      static_cast<soarxml::XMLTrace*>(pAgent->xml_destination);
  pXML->BeginTag(pTag);
}
...
// around in the existing structure.  That's not often required but
// occassionally is helpful.
...
void xml_att_val(agent* pAgent, char const* pAttribute, Symbol* pSymbol) {
  // Passing 0, 0 as buffer to symbol to string causes it to use internal,
  // temporary buffers which is fine because we immediately copy that string in
  // XMLAddAttribute.
  soarxml::XMLTrace* pXML =
      static_cast<soarxml::XMLTrace*>(pAgent->xml_destination);
...
void xml_object(agent* pAgent, char const* pTag, char const* pAttribute,
                char const* pValue) {
  soarxml::XMLTrace* pXML =
      static_cast<soarxml::XMLTrace*>(pAgent->xml_destination);
...
void xml_object(agent* pAgent, wme* pWME, bool printTimetag) {
  ...
  // BADBAD: These calls are redundantly converting pXML again. Possibly
  // optimize.
  if (printTimetag) {
  ...
  xml_att_val(pAgent, kWME_ValueType,
              symbol_type_to_string(pWME->value->symbol_type));
...
void xml_invoke_callback(agent* pAgent) {
  ...
  // We need to call the handler explicitly here instead of using
  // soar_invoke_callbacks because we need to create a new ElementXML object for
  // each handler that gets called.
  for (cons* c = pAgent->soar_callbacks[XML_GENERATION_CALLBACK]; c != NIL;
       c = c->rest) {
    soarxml::ElementXML* pCallbackData =
    ...
    cb->function(pAgent, cb->eventid, cb->data,
                 static_cast<soar_call_data>(pCallbackData));
  }
