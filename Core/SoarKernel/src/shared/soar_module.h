...
class btw_predicate : public predicate<T> {
 ...
 public:
  ...
  bool operator()(T val) {
    return ((inclusive) ? ((val >= my_min) && (val <= my_max))
                        : ((val > my_min) && (val < my_max)));
  }
};
...
class lt_predicate : public predicate<T> {
 ...
 public:
  lt_predicate(T new_max, bool new_inclusive)
      : my_max(new_max), inclusive(new_inclusive) {}
...
class object_container {
 ...
 public:
  object_container(agent* new_agent)
      : thisAgent(new_agent), objects(new std::map<std::string, T*>) {}
...
class primitive_param : public param {
 ...
 public:
  primitive_param(const char* new_name, T new_value, predicate<T>* new_val_pred,
                  predicate<T>* new_prot_pred)
      : param(new_name),
        value(new_value),
        val_pred(new_val_pred),
        prot_pred(new_prot_pred) {}
...
class string_param : public param {
 ...
 public:
  string_param(const char* new_name, const char* new_value,
  ...
  virtual const std::string get_string() { return (*value); }
...
class primitive_set_param : public param {
 ...
 public:
  ...
  virtual typename std::set<T>::iterator set_begin() { return my_set->begin(); }

  virtual typename std::set<T>::iterator set_end() { return my_set->end(); }
};
...
class sym_set_param : public param {
 ...
 public:
  sym_set_param(const char* new_name, predicate<const char*>* new_prot_pred,
                agent* new_agent)
      : param(new_name),
        my_set(new std::set<Symbol*>()),
        value(new std::string),
        prot_pred(new_prot_pred),
        thisAgent(new_agent) {}

  virtual ~sym_set_param() {
    for (std::set<Symbol*>::iterator p = my_set->begin(); p != my_set->end();
         p++) {
      Symbol* lSym = (*p);
...
class boolean_param : public constant_param<boolean> {
 public:
  boolean_param(const char* new_name, boolean new_value,
...
class path_param : public soar_module::string_param {
 ...
 public:
  path_param(const char* new_name, const char* new_value,
...
class primitive_stat : public statistic {
 ...
 public:
  ...
  virtual ~primitive_stat() { delete prot_pred; }
...
class stat_container : public object_container<statistic> {
 public:
  ...
  void reset() {
    for (std::map<std::string, statistic*>::iterator p = objects->begin();
         p != objects->end(); p++) {
      p->second->reset();
    }
  }
...
class timer : public named_object {
 ...
 public:
  timer(const char* new_name, agent* new_agent, timer_level new_level,
        predicate<timer_level>* new_pred, bool soar_control = true);

  virtual ~timer() { delete pred; }
  ...
  virtual double value() { return accumulator.get_sec(); }
...
class timer_container : public object_container<timer> {
 public:
  ...
  void reset() {
    for (std::map<std::string, timer*>::iterator p = objects->begin();
         p != objects->end(); p++) {
      p->second->reset();
    }
  }
...
class object_memory {
 ...
 protected:
  ...
  time_step get_current_time() { return step_count; }

  bool is_initialized() { return initialized; }

  virtual time_step estimate_forgetting_time(const object_history* h,
                                             time_step t,
                                             bool fresh_reference) = 0;
 ...
 public:
  ...
  bool reference_object(const T* obj, object_reference num) {
    ...
    if (p != object_histories.end()) {
      ...
      std::pair<typename object_history_map::iterator, bool> ip =
          object_histories.insert(std::make_pair(obj, object_history(obj)));
      assert(ip.second);
  ...
  void process_buffered_references() {
    ...
    for (h_p = touched_histories.begin(); h_p != touched_histories.end();
         h_p++) {
      h = *h_p;

      // update number of references in the current history
      // (has to come before history overwrite)
      h->history_references += (h->buffered_references -
                                h->reference_history[h->next_p].num_references);
  ...
  typename object_set::iterator forgotten_begin() { return forgotten.begin(); }

  typename object_set::iterator forgotten_end() { return forgotten.end(); }
  ...
  void forget() {
    ...
    if (!forgetting_pq.empty()) {
      ...
      if (pq_p->first == step_count) {
        ...
        while (d_p != pq_p->second.end()) {
          ...
          if (should_forget(*current_p, step_count)) {
            ...
            move_in_pq(*current_p,
                       estimate_forgetting_time(*current_p, step_count, false));
          }
        }
      ...
      for (time_set::iterator t_p = touched_times.begin();
           t_p != touched_times.end(); t_p++) {
  ...
  void time_forward() { step_count++; }

  void time_back() { step_count--; }
...
class bla_object_memory : public object_memory<T, N> {
 ...
 public:
  bla_object_memory()
      : activation_none(1.0),
        activation_low(-1000000000),
        time_sum_none(2.71828182845905),
        use_petrov(true),
        decay_rate(-0.5),
        decay_thresh(-2.0),
        pow_cache_bound(10) {}
  ...
  bool set_decay_rate(double new_decay_rate) {
    if ((new_decay_rate > 0) && (new_decay_rate < 1) &&
        !this->is_initialized()) {
  ...
  double get_object_activation(T* obj, bool log_result) {
    return compute_history_activation(get_history(obj),
                                      this->get_current_time(), log_result);
  }
 ...
 protected:
  void _init() {
    ...
    // repeated calls to pow() at runtime
    {
      // determine cache size
      {
        ...
        // threshold, given decay rate and assumption of max references/time
        // step t = e^( ( thresh - ln( max_refs ) ) / -decay_rate )
        double cache_full = static_cast<double>(
            exp((decay_thresh - log(static_cast<double>(R))) / decay_rate));
        ...
        double cache_bound_unit =
            (static_cast<unsigned int>(pow_cache_bound * 1024 * 1024) /
             static_cast<unsigned int>(sizeof(double)));

        pow_cache_size = static_cast<unsigned int>(
            ceil((cache_full > cache_bound_unit) ? (cache_bound_unit)
                                                 : (cache_full)));
      }
    ...
    // approximation cache
    {
      approx_cache[0] = 0;
      for (int i = 1; i < R; i++) {
        approx_cache[i] = static_cast<time_step>(ceil(exp(
            static_cast<double>(decay_thresh - log(static_cast<double>(i))) /
  ...
  time_step estimate_forgetting_time(const object_history* h, time_step t,
    ...
    if (fresh_reference) {
      ...
      while (counter) {
        ...
        approx_ref = ((h->reference_history[p].num_references < R)
                          ? (h->reference_history[p].num_references)
                          : (R - 1));
 ...
 private:
  ...
  double compute_history_activation(const object_history* h, time_step t,
                                    bool log_result) {
      ...
      // sum history
      {
        ...
        if (use_petrov) {
          ...
          if (h->total_references > h->history_references) {
            ...
            return_val +=
                (((h->total_references - h->history_references) *
                  (pow(static_cast<double>(t - h->first_reference), d_inv) -
                   pow(static_cast<double>(t_diff), d_inv))) /
