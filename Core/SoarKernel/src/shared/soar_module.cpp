...
timer::timer(const char* new_name, agent* new_agent, timer_level new_level,
             predicate<timer_level>* new_pred, bool soar_control)
...
void print_ambiguous_commands(agent* thisAgent, const std::string badCommand,
  ...
  thisAgent->outputManager->printa_sf(thisAgent, "%?  Did you mean",
                                      badCommand.c_str());
  thisAgent->outputManager->display_ambiguous_command_error(thisAgent,
                                                            matched_name_list);
}
