...
extern exploration_parameter* exploration_add_parameter(
    double value, bool (*val_func)(double), const char* name);

// convert parameter name
extern const int exploration_convert_parameter(agent* thisAgent,
                                               const char* name);
extern const char* exploration_convert_parameter(agent* thisAgent,
                                                 const int parameter);

// validate parameter name
extern const bool exploration_valid_parameter(agent* thisAgent,
                                              const char* name);
extern const bool exploration_valid_parameter(agent* thisAgent,
                                              const int parameter);

// get parameter value
extern double exploration_get_parameter_value(agent* thisAgent,
...
// validate parameter value
extern bool exploration_valid_parameter_value(agent* thisAgent,
...
// set parameter value
extern bool exploration_set_parameter_value(agent* thisAgent, const char* name,
                                            double value);
extern bool exploration_set_parameter_value(agent* thisAgent,
                                            const int parameter, double value);
...
extern const int exploration_get_reduction_policy(agent* thisAgent,
                                                  const char* parameter);
extern const int exploration_get_reduction_policy(agent* thisAgent,
                                                  const int parameter);

// validate reduction policy per parameter
extern bool exploration_valid_reduction_policy(agent* thisAgent,
                                               const char* parameter,
...
extern bool exploration_set_reduction_policy(agent* thisAgent,
                                             const char* parameter,
                                             const char* policy_name);
extern bool exploration_set_reduction_policy(agent* thisAgent,
                                             const int parameter,
...
// get reduction rate
extern double exploration_get_reduction_rate(agent* thisAgent,
                                             const char* parameter,
                                             const char* policy_name);
extern double exploration_get_reduction_rate(agent* thisAgent,
                                             const int parameter,
...
// set reduction rate
extern bool exploration_set_reduction_rate(agent* thisAgent,
                                           const char* parameter,
                                           const char* policy_name,
                                           double reduction_rate);
extern bool exploration_set_reduction_rate(agent* thisAgent,
...
extern preference* exploration_choose_according_to_policy(
    agent* thisAgent, slot* s, preference* candidates);

// calculate the probability of a selection given the current exploration mode
extern double exploration_probability_according_to_policy(
    agent* thisAgent, slot* s, preference* candidates, preference* selection);

// selects a candidate in a random fashion
extern preference* exploration_randomly_select(preference* candidates,
                                               const bool& update_rho = true);
...
extern preference* exploration_boltzmann_select(agent* thisAgent,
                                                preference* candidates);

// selects a candidate based upon an epsilon-greedy distribution
extern preference* exploration_epsilon_greedy_select(agent* thisAgent,
                                                     preference* candidates);

// returns candidate with highest q-value (random amongst ties), assumes
// computed values
extern preference* exploration_get_highest_q_value_pref(preference* candidates);

// computes total contribution for a candidate from each preference, as well as
// number of contributions
extern void exploration_compute_value_of_candidate(agent* thisAgent,
                                                   preference* cand, slot* s,
