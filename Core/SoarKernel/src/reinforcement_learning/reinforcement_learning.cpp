...
RL_Manager::RL_Manager(agent* myAgent) {
  ...
  exploration_params[EXPLORATION_PARAM_EPSILON] =
      exploration_add_parameter(0.1, &exploration_validate_epsilon, "epsilon");
  exploration_params[EXPLORATION_PARAM_TEMPERATURE] = exploration_add_parameter(
      25, &exploration_validate_temperature, "temperature");
...
void RL_Manager::clean_up_for_agent_deletion() {
  ...
  delete rl_params;
  delete rl_stats;
  rl_params = NULL;  // apoptosis needs to know this for excise_all_productions
                     // later in agent deletion process
...
rl_param_container::get_documentation_params() {
  static std::vector<std::pair<std::string, param_accessor<double>*> >
      documentation_params;
  static bool initted = false;
  if (!initted) {
    initted = true;
    // Is it okay to use new here, because this is a static variable anyway,
    // so it's not going to happen more than once and shouldn't ever be cleaned
    // up?
    documentation_params.push_back(
        std::make_pair("rl-updates", new rl_updates_accessor()));
    documentation_params.push_back(
        std::make_pair("delta-bar-delta-h", new rl_dbd_h_accessor()));
  }
...
rl_param_container::rl_param_container(agent* new_agent)
  ...
  // learning
  learning = new rl_learning_param(
      "learning", off, new soar_module::f_predicate<boolean>(), new_agent);
  add(learning);

  // discount-rate
  discount_rate = new soar_module::decimal_param(
      "discount-rate", 0.9, new soar_module::btw_predicate<double>(0, 1, true),
      new soar_module::f_predicate<double>());
  add(discount_rate);

  // learning-rate
  learning_rate = new soar_module::decimal_param(
      "learning-rate", 0.3, new soar_module::btw_predicate<double>(0, 1, true),
      new soar_module::f_predicate<double>());
  add(learning_rate);

  // step-size-parameter
  step_size_parameter = new soar_module::decimal_param(
      "step-size-parameter", 1.0,
      new soar_module::btw_predicate<double>(0, 1, true),
      new soar_module::f_predicate<double>());
  add(step_size_parameter);

  // meta-learning-rate
  meta_learning_rate = new soar_module::decimal_param(
  ...
  // learning-policy
  learning_policy = new soar_module::constant_param<learning_choices>(
      "learning-policy", sarsa,
      new soar_module::f_predicate<learning_choices>());
  ...
  // decay-mode
  decay_mode = new soar_module::constant_param<decay_choices>(
      "decay-mode", normal_decay,
      new soar_module::f_predicate<decay_choices>());
  ...
  et_decay_rate = new soar_module::decimal_param(
  ...
  et_tolerance = new soar_module::decimal_param(
  ...
  // temporal-extension
  temporal_extension = new soar_module::boolean_param(
      "temporal-extension", on, new soar_module::f_predicate<boolean>());
  add(temporal_extension);

  // hrl-discount
  hrl_discount = new soar_module::boolean_param(
      "hrl-discount", off, new soar_module::f_predicate<boolean>());
  add(hrl_discount);

  // temporal-discount
  temporal_discount = new soar_module::boolean_param(
      "temporal-discount", on, new soar_module::f_predicate<boolean>());
  add(temporal_discount);

  // chunk-stop
  chunk_stop = new soar_module::boolean_param(
      "chunk-stop", on, new soar_module::f_predicate<boolean>());
  add(chunk_stop);

  // meta
  meta = new soar_module::boolean_param(
      "meta", off, new soar_module::f_predicate<boolean>());
  add(meta);

  // update-log-path
  update_log_path = new soar_module::string_param(
      "update-log-path", "", new soar_module::predicate<const char*>(),
      new soar_module::f_predicate<const char*>());
  add(update_log_path);

  // apoptosis
  apoptosis = new rl_apoptosis_param(
      "apoptosis", apoptosis_none,
      new soar_module::f_predicate<apoptosis_choices>(), thisAgent);
  ...
  // apoptosis-decay
  apoptosis_decay = new soar_module::decimal_param(
      "apoptosis-decay", 0.5,
      new soar_module::btw_predicate<double>(0, 1, true),
      new rl_apoptosis_predicate<double>(thisAgent));
  add(apoptosis_decay);

  // apoptosis-thresh
  apoptosis_thresh = new rl_apoptosis_thresh_param(
      "apoptosis-thresh", -2.0, new soar_module::gt_predicate<double>(0, false),
      new rl_apoptosis_predicate<double>(thisAgent));
  add(apoptosis_thresh);

  // trace
  trace = new soar_module::boolean_param(
      "trace", off, new soar_module::f_predicate<boolean>());
  add(trace);
};
...
rl_learning_param::rl_learning_param(
    const char* new_name, boolean new_value,
    soar_module::predicate<boolean>* new_prot_pred, agent* new_agent)
    : soar_module::boolean_param(new_name, new_value, new_prot_pred),
      thisAgent(new_agent) {}
...
rl_apoptosis_param::rl_apoptosis_param(
    const char* new_name, rl_param_container::apoptosis_choices new_value,
    soar_module::predicate<rl_param_container::apoptosis_choices>*
        new_prot_pred,
    agent* new_agent)
    : soar_module::constant_param<rl_param_container::apoptosis_choices>(
          new_name, new_value, new_prot_pred),
      thisAgent(new_agent) {}

void rl_apoptosis_param::set_value(
  ...
  if (value != new_value) {
    ...
    if (value == rl_param_container::apoptosis_none) {
      thisAgent->RL->rl_prods->set_decay_rate(
          thisAgent->RL->rl_params->apoptosis_decay->get_value());
      thisAgent->RL->rl_prods->set_decay_thresh(
          thisAgent->RL->rl_params->apoptosis_thresh->get_value());
...
rl_apoptosis_thresh_param::rl_apoptosis_thresh_param(
    const char* new_name, double new_value,
    soar_module::predicate<double>* new_val_pred,
    soar_module::predicate<double>* new_prot_pred)
    : soar_module::decimal_param(new_name, new_value, new_val_pred,
                                 new_prot_pred) {}
...
bool rl_apoptosis_predicate<T>::operator()(T /*val*/) {
  return (this->thisAgent->RL->rl_params->apoptosis->get_value() !=
...
rl_stat_container::rl_stat_container(agent* new_agent)
  ...
  // update-error
  update_error = new soar_module::decimal_stat(
      "update-error", 0, new soar_module::f_predicate<double>());
  add(update_error);

  // total-reward
  total_reward = new soar_module::decimal_stat(
      "total-reward", 0, new soar_module::f_predicate<double>());
  add(total_reward);

  // global-reward
  global_reward = new soar_module::decimal_stat(
      "global-reward", 0, new soar_module::f_predicate<double>());
  add(global_reward);
};
...
inline void rl_remove_ref(Symbol* goal, production* prod) {
  ...
  rules->remove(prod);
  /* For use when we try using a vector with a new memory allocator that can
   * handle variable size allocations */
  // production_list::iterator newEnd = std::remove(rules->begin(),
  // rules->end(), prod); rules->erase(newEnd, rules->end());
}
...
void rl_remove_refs_for_prod(agent* thisAgent, production* prod) {
  for (Symbol* state = thisAgent->top_state; state;
    ...
    if (a->type == MAKE_ACTION) {
      if (a->preference_type == NUMERIC_INDIFFERENT_PREFERENCE_TYPE) {
        ...
        if (rhs_value_is_symbol(a->referent) &&
            (rhs_value_to_symbol(a->referent)->is_variable())) {
          var_pref = true;
        }
      }
    }
...
void rl_rule_meta(agent* thisAgent, production* prod) {
  if (prod->documentation &&
      (thisAgent->RL->rl_params->meta->get_value() == on)) {
    std::string doc(prod->documentation);

    const std::vector<std::pair<std::string, param_accessor<double>*> >&
        documentation_params =
            thisAgent->RL->rl_params->get_documentation_params();
    for (std::vector<std::pair<std::string, param_accessor<double>*> >::
             const_iterator doc_params_it = documentation_params.begin();
...
Symbol* rl_build_template_instantiation(agent* thisAgent,
  ...
  if (my_template_instance->prod->rl_template_conds == NIL) {
    ...
    p_node_to_conditions_and_rhs(thisAgent, my_template_instance->prod->p_node,
                                 NIL, NIL, &(c_top), &(c_bottom), NIL,
  ...
  do {
    ...
    new_name =
        ("rl*" + empty_string + my_template->name->sc->name + "*" + temp_id);
  } while (thisAgent->symbolManager->find_str_constant(new_name.c_str()) !=
           NIL);
  new_name_symbol =
      thisAgent->symbolManager->make_str_constant(new_name.c_str());

  copy_condition_list(thisAgent,
                      my_template_instance->top_of_instantiated_conditions,
                      &cond_top, &cond_bottom, false, false, true);
  ...
  action* new_action = thisAgent->explanationBasedChunker->variablize_rl_action(
      rhs_actions, tok, w, init_value);

  // make new production
  thisAgent->name_of_production_being_reordered = new_name_symbol->sc->name;
  if (new_action &&
      reorder_and_validate_lhs_and_rhs(thisAgent, &cond_top, &new_action,
                                       false) == reorder_success) {
    production* new_production = make_production(
        thisAgent, USER_PRODUCTION_TYPE, new_name_symbol,
        my_template->name->sc->name, &cond_top, &new_action, false, NULL);
    ...
    if (add_production_to_rete(thisAgent, new_production, cond_top, NULL, false,
                               duplicate_rule, true) == DUPLICATE_PRODUCTION) {
...
void rl_add_goal_or_impasse_tests_to_conds(agent* thisAgent,
                                           condition* all_conds) {
  // mark each id as we add a test for it, so we don't add a test for the same
  // id in two different places
  Symbol* id_sym;
  test t;
  ...
  for (condition* cond = all_conds; cond != NIL; cond = cond->next) {
    ...
    if ((id_sym->id->isa_goal || id_sym->id->isa_impasse) &&
        (id_sym->tc_num != tc)) {
      t = make_test(thisAgent, NIL,
                    ((id_sym->id->isa_goal) ? GOAL_ID_TEST : IMPASSE_ID_TEST));
...
void rl_tabulate_reward_value_for_goal(agent* thisAgent, Symbol* goal) {
  ...
  if (!data->prev_op_rl_rules->empty()) {
    slot* s = find_slot(goal->id->rl_info->rl_link_wme->value,
                        thisAgent->symbolManager->soarSymbols.rl_sym_reward);
    slot* t;
    wme *w, *x;
    ...
    if (s) {
      for (w = s->wmes; w; w = w->next) {
        if (w->value->symbol_type == IDENTIFIER_SYMBOL_TYPE) {
          t = find_slot(w->value,
                        thisAgent->symbolManager->soarSymbols.rl_sym_value);
          if (t) {
            for (x = t->wmes; x; x = x->next) {
              if ((x->value->symbol_type == FLOAT_CONSTANT_SYMBOL_TYPE) ||
                  (x->value->symbol_type == INT_CONSTANT_SYMBOL_TYPE)) {
      ...
      data->reward +=
          (reward * pow(discount_rate, static_cast<double>(effective_age)));
    }
    ...
    if ((goal != thisAgent->bottom_goal) &&
        (thisAgent->RL->rl_params->hrl_discount->get_value() == on)) {
      data->hrl_age++;
    }
  }
}
...
void rl_store_data(agent* thisAgent, Symbol* goal, preference* cand) {
  ...
  bool using_gaps =
      (thisAgent->RL->rl_params->temporal_extension->get_value() == on);

  // Make list of just-fired prods
  unsigned int just_fired = 0;
  for (preference* pref =
           goal->id->operator_slot
               ->preferences[NUMERIC_INDIFFERENT_PREFERENCE_TYPE];
       pref; pref = pref->next) {
  ...
  if (just_fired) {
    ...
    if (thisAgent->trace_settings[TRACE_RL_SYSPARAM] && using_gaps &&
      ...
      char buf[256];
      SNPRINTF(buf, 254, "gap started (%c%" SCNu64 ")", goal->id->name_letter,
               goal->id->name_number);
...
void rl_perform_update(agent* thisAgent, double op_value, bool op_rl,
                       Symbol* goal, bool update_efr) {
  bool using_gaps =
      (thisAgent->RL->rl_params->temporal_extension->get_value() == on);
  ...
  if (!using_gaps || op_rl) {
    ...
    if (!data->prev_op_rl_rules->empty()) {
      ...
      if (data->gap_age && using_gaps &&
          thisAgent->trace_settings[TRACE_RL_SYSPARAM]) {
        char buf[256];
        SNPRINTF(buf, 254, "gap ended (%c%" SCNu64 ")", goal->id->name_letter,
                 goal->id->name_number);
      ...
      // Iterate through eligibility_traces, decay traces. If less than
      // TOLERANCE, remove from map.
      if (lambda == 0) {
        if (!data->eligibility_traces->empty()) {
          data->eligibility_traces->clear();
        }
      } else {
        for (iter = data->eligibility_traces->begin();
             iter != data->eligibility_traces->end();) {
          iter->second *= lambda;
      ...
      if (thisAgent->RL->rl_params->learning_policy->get_value() &
          rl_param_container::gql) {
        for (iter = data->eligibility_traces->begin();
             iter != data->eligibility_traces->end(); iter++) {
      ...
      if (!data->prev_op_rl_rules->empty()) {
        /// I = 0.0 for non-terminal states when using hierarchical
        /// reinforcement learning
        const double I = goal->id->lower_goal ? 0.0 : 1.0;
        double trace_increment =
            I * (1.0 / static_cast<double>(data->prev_op_rl_rules->size()));
        production_list::iterator p;

        for (p = data->prev_op_rl_rules->begin();
             p != data->prev_op_rl_rules->end(); p++) {
      ...
      // For each prod with a trace, perform update
      {
        ...
        double delta_t =
            (data->reward + discount * op_value) - (sum_old_ecr + sum_old_efr);
        ...
        for (iter = data->eligibility_traces->begin();
          ...
          switch (thisAgent->RL->rl_params->decay_mode->get_value()) {
            ...
            case rl_param_container::delta_bar_delta_decay: {
              // Note that in this case, x_i = 1.0 for all productions that are
              // being updated. Those values have been included here for
              // consistency with the algorithm as described in the delta bar
              // delta paper.
              prod->rl_delta_bar_delta_beta =
                  prod->rl_delta_bar_delta_beta +
                  theta * delta_t * 1.0 * prod->rl_delta_bar_delta_h;
              adjusted_alpha = exp(prod->rl_delta_bar_delta_beta);
              double decay_term = 1.0 - adjusted_alpha * 1.0 * 1.0;
              if (decay_term < 0.0) {
                decay_term = 0.0;
              }
              prod->rl_delta_bar_delta_h =
                  prod->rl_delta_bar_delta_h * decay_term +
          ...
          // calculate updates
          delta_ecr =
              (adjusted_alpha * iter->second * (data->reward - sum_old_ecr));

          if (update_efr) {
            delta_efr = (adjusted_alpha * iter->second *
                         ((discount * op_value) - sum_old_efr));
          } else {
            delta_efr = 0.0;
          }
          ...
          if (thisAgent->trace_settings[TRACE_RL_SYSPARAM]) {
            ...
            ss << "RL update " << prod->name->sc->name << " " << old_ecr << " "
               << old_efr << " " << old_ecr + old_efr << " -> " << new_ecr
               << " " << new_efr << " " << new_combined;
            ...
            std::string log_path =
                thisAgent->RL->rl_params->update_log_path->get_value();
            if (!log_path.empty()) {
          ...
          prod->action_list->referent =
              allocate_rhs_value_for_symbol_no_refcount(
                  thisAgent,
                  thisAgent->symbolManager->make_float_constant(new_combined),
                  0, 0);
        ...
        if (thisAgent->RL->rl_params->learning_policy->get_value() &
          ...
          for (production_list::iterator p = data->prev_op_rl_rules->begin();
               p != data->prev_op_rl_rules->end(); p++) {
          ...
          if (thisAgent->RL->rl_params->meta->get_value() == on) {
            ...
            const std::vector<std::pair<std::string, param_accessor<double>*> >&
                documentation_params =
                    thisAgent->RL->rl_params->get_documentation_params();
            for (std::vector<std::pair<std::string, param_accessor<double>*> >::
                     const_iterator doc_params_it =
                         documentation_params.begin();
                 doc_params_it != documentation_params.end(); ++doc_params_it) {
              doc_ss << doc_params_it->first << "="
            ...
            prod->documentation = make_memory_block_for_string( thisAgent,
            rlupdates.c_str() );
            */
          }

          // Change value of preferences generated by current instantiations of
          // this rule
          if (prod->instantiations) {
            for (instantiation* inst = prod->instantiations; inst;
                 inst = inst->next) {
