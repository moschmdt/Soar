...
// goal stack level, WMEs attached to the operator, and the probability of
// selection
struct RL_Trace {
  struct Entry {
...
class rl_param_container : public soar_module::param_container {
 public:
  enum learning_choices {
    off_policy = 0x01,  ///< For testing if learning is off-policy
  ...
  soar_module::boolean_param*
      meta;  // Whether doc strings are used for storing metadata.
  soar_module::string_param* update_log_path;  // If non-null and size > 0, log
                                               // all RL updates to this file.
  ...
  static const std::vector<std::pair<std::string, param_accessor<double>*> >&
...
class rl_learning_param : public soar_module::boolean_param {
 ...
 public:
  rl_learning_param(const char* new_name, boolean new_value,
                    soar_module::predicate<boolean>* new_prot_pred,
                    agent* new_agent);
  void set_value(boolean new_value);
};

class rl_apoptosis_param : public soar_module::constant_param<
...
class rl_apoptosis_thresh_param : public soar_module::decimal_param {
 public:
  rl_apoptosis_thresh_param(const char* new_name, double new_value,
...
typedef struct rl_data_struct {
  ...
  production_list*
      prev_op_rl_rules;  // rl rules associated with the previous operator
  wme* rl_link_wme;
  ...
  unsigned int
      gap_age;  // the number of steps since a cycle containing rl rules
...
extern void rl_update_template_tracking(agent* thisAgent,
...
extern Symbol* rl_build_template_instantiation(
    agent* thisAgent, instantiation* my_template_instance,
    struct token_struct* tok, wme* w, action* a2);

// adds a test to a condition list for goals or impasses contained within the
// condition list
...
extern void rl_perform_update(agent* thisAgent, double op_value, bool op_rl,
                              Symbol* goal, bool update_efr = true);
