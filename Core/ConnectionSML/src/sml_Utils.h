...
#define CHECK_RET_ZERO(x) \
  {                       \
    assert(x);            \
    if (!(x)) return 0;   \
  }
#define CHECK_RET_NULL(x)  \
  {                        \
    assert(x);             \
    if (!(x)) return NULL; \
  }
...
#define CTDEBUG_ENTER_METHOD(methodName) \
  CTDebugEnterMethod myEnterMethod3PNY(methodName)
...
class EXPORT CTDebugEnterMethod {
 public:
  ...
  ~CTDebugEnterMethod() {
    if (m_stackTraceDisplayPos > 0 &&
        m_stackTraceDisplayPos >= (long)(m_stackTrace.size())) {
 ...
 protected:
  ...
  // Pointer into stack trace to last item already printed.  Points to end() if
