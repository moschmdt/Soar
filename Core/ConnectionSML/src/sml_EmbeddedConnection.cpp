...
ElementXML_Handle LocalProcessMessage(
  ...
  // This is the connection object we created in this class, passed to the
  // kernel and have now received back.
  EmbeddedConnection* pConnection =
      reinterpret_cast<EmbeddedConnection*>(hReceiverConnection);
  ...
  if (action == SML_MESSAGE_ACTION_SYNCH) {
    ...
    // For a synchronous connection, immediately execute the incoming message,
    // generating a response which is immediately passed back to the caller.
...
bool EmbeddedConnection::AttachConnection(bool optimized, int portToListenOn) {
  ...
  // We only use the creation function once to create a connection object (which
  // we'll pass back with each call).
  int connectionType =
      this->IsAsynchronous() ? SML_ASYNCH_CONNECTION : SML_SYNCH_CONNECTION;
  m_hConnection = sml_CreateEmbeddedConnection(
      Dangerous_Pointer_Cast<Connection_Sender_Handle>::from(this),
      LocalProcessMessage, connectionType, portToListenOn);
  ...
  m_pKernelSML = reinterpret_cast<KernelSML*>(
      reinterpret_cast<EmbeddedConnection*>(m_hConnection)->GetUserData());
  return true;
}
...
void EmbeddedConnection::SetTraceCommunications(bool state) {
  ...
  if (m_hConnection) {
    ...
    m_pProcessMessageFunction(
        m_hConnection, 0,
        state ? SML_MESSAGE_ACTION_TRACE_ON : SML_MESSAGE_ACTION_TRACE_OFF);
  }
}

void EmbeddedConnection::DirectAddWME_String(Direct_AgentSML_Handle pAgentSML,
                                             char const* pId,
...
void EmbeddedConnection::DirectAddWME_Int(Direct_AgentSML_Handle pAgentSML,
                                          char const* pId,
...
void EmbeddedConnection::DirectAddWME_Double(Direct_AgentSML_Handle pAgentSMLIn,
                                             char const* pId,
                                             char const* pAttribute,
                                             double value,
                                             int64_t clientTimetag) {
...
void EmbeddedConnection::DirectRemoveWME(Direct_AgentSML_Handle pAgentSML,
                                         int64_t clientTimetag) {
  AgentSML* a = reinterpret_cast<AgentSML*>(pAgentSML);
  assert(a);
...
void EmbeddedConnection::DirectAddID(Direct_AgentSML_Handle pAgentSML,
...
void EmbeddedConnection::DirectRun(char const* pAgentName, bool forever,
                                   int stepSize, int interleaveSize,
                                   uint64_t count) {
