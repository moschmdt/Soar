...
//
// A client then connects to that listener data sender through the "client data
// sender" class.
//
...
class DataSender {
 protected:
  ...
  // These objects are created through the ListenerDataSender or
  // ClientDataSender classes.
 protected:
 ...
 public:
  ...
  // Returns true if data sender is closed--but then receiveMsg will know it's
  // closed. The timeout for waiting for data is secondsWait + millisecondsWait,
  // where millisecondsWait < 1000
  virtual bool IsReadDataAvailable(int secondsWait = 0,
                                   int millisecondsWait = 0) = 0;
 ...
 protected:
  ...
  // Specific kinds of locks have different ways of shutting themselves down,
  // they must do it here
