...
class WindowsMutex : public OSSpecificMutex {
 ...
 public:
  ...
  //
  // public:
  //	WindowsMutex()			{ mutex = CreateMutex(NULL, FALSE,
  // NULL); } 	virtual ~WindowsMutex() { CloseHandle(mutex); } 	void
  // Lock() { WaitForSingleObject(mutex, INFINITE); } 	void Unlock() {
  // ReleaseMutex(mutex) ; } 	bool TryToLock()		{ DWORD res =
  // WaitForSingleObject(mutex, 0) ; return (res == WAIT_OBJECT_0) ; }
};

OSSpecificMutex* soar_thread::MakeMutex() { return new WindowsMutex(); }
 ...
 public:
  ...
  bool WaitForEvent(int sec, int milli) {
    assert(milli < 1000 &&
           ...
           "part of time >= 1000 milliseconds");
...
void soar_thread::BeginThread(ThreadFuncPtr inThreadFuncPtr, void* inParam) {
  ...
  /*    static char mtid='a';
      ...
      new_name += mtid;
      mtid++;
      std::cout << "BeginThread creating thread named " << new_name <<
     std::endl; pthread_setname_np(new_name.c_str());
  */
...
#if (defined(__APPLE__) && defined(__MACH__)) || (__LSB_VERSION__ >= 12) || \
    defined(__FreeBSD__)
...
OSSpecificMutex* soar_thread::MakeMutex() { return new LinuxMutex(); }
...
class LinuxEvent : public OSSpecificEvent {
 ...
 public:
  ...
  bool WaitForEvent(int sec, int milli) {
    ...
    // make sure the nanoseconds field doesn't exceed 1 second (this is an error
    // on OS X)
...
OSSpecificEvent* soar_thread::MakeEvent() { return new LinuxEvent(); }
