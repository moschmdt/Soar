...
//
// A client then connects to that listener socket (it needs to know the IP
// address and port to connect to) through the "client socket" class.
//
...
void ignore_sigpipe(void) {
  ...
  if (r) std::cerr << "Sig Action Failed: " << strerror(r) << std::endl;
}
#endif
...
uint32_t sock::GetLocalIP() {
  ...
  if (stLclAddr.sin_addr.s_addr == INADDR_ANY) {
    ...
    if (hSock != INVALID_SOCKET) {
      ...
      nRet = connect(hSock, (SOCKADDR*)&stRmtAddr, sizeof(SOCKADDR));
      if (nRet != SOCKET_ERROR) {
        /* Get local address */
#ifdef _WIN32
        // The Windows call takes a signed int, the Linux an unsigned int for
        // nAddrSize.
...
bool Socket::SendBuffer(char const* pSendBuffer, uint32_t bufferSize) {
  ...
  while (bytesSent < bufferSize) {
    do {
      thisSend =
          send(hSock, pSendBuffer, static_cast<int>(bufferSize - bytesSent), 0);

      // Check if there was an error
      if (thisSend == SOCKET_ERROR) {
#ifdef NON_BLOCKING
        // On a non-blocking socket, the socket can return "would block" -- in
        // which case we need to wait for it to clear.  A blocking socket would
        // not return in this case so this would always be an error.
...
// in secs (0 is default) Argument       : int millisecondssecondsWait --
// Milliseconds part of how long to wait for data in usecs (0 is default, must
// be < 1000) Return type    : bool
//
...
bool Socket::IsReadDataAvailable(int secondsWait, int millisecondsWait) {
  ...
  if (!hSock) {
    if (m_bTraceCommunications) {
      sml::PrintDebug(
          "Error: Can't check for read data because this socket is closed");
    }
    return false;
  }
...
bool Socket::ReceiveBuffer(char* pRecvBuffer, uint32_t bufferSize) {
  ...
  while (bytesRead < bufferSize) {
    do {
      thisRead =
          recv(hSock, pRecvBuffer, static_cast<int>(bufferSize - bytesRead), 0);
      ...
      if (thisRead == SOCKET_ERROR) {
        ...
        if (IsErrorWouldBlock()) {
          // sml::PrintDebug("Waiting for socket to unblock") ;
          sml::Sleep(0, 0);  // BADBAD: Should have a proper way to pass control
                             // back to the caller while we're blocked.
        } else
#endif
        {
          if (m_bTraceCommunications) {
          ...
          // We treat these errors as all being fatal, which they all appear to
          // be. If we later decide we can survive certain ones, we should test
          // for them here and not always close the socket.
          if (m_bTraceCommunications) {
    ...
    if (m_bTraceCommunications) {
      sml::PrintDebugFormat("Received %d bytes",
                            static_cast<int64_t>(thisRead));
    }
