...
// DeleteString method
char* AnalyzeXML::GenerateXMLString(bool includeChildren,
                                    bool insertNewLines) const {
  return ::soarxml_GenerateXMLString(m_hRootObject, includeChildren,
                                     insertNewLines);
}
...
char const* AnalyzeXML::GetResultString() const {
  ...
  return NULL;  // this should never happen, since GetCharacterData will return
                // a zero-length string if there is no character data
}
...
int64_t AnalyzeXML::GetArgInt(char const* pArgName, int argPos,
...
void AnalyzeXML::Analyze(ElementXML const* pRootXML) {
  ...
  // (this is not really recommended as creating an AnalyzeXML object is pretty
  // quick and painless)
  if (m_hRootObject) {
    ::soarxml_ReleaseRef(m_hRootObject);
