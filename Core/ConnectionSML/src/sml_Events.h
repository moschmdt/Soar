...
enum smlSystemEventId {
  ...
  smlEVENT_INTERRUPT_CHECK,  // Chance for client to interrupt a run (designed
                             // to be low bandwidth)
  smlEVENT_SYSTEM_PROPERTY_CHANGED,  // A kernel-level parameter has been
                                     // changed (note: no longer includes
                                     // sysparams which are agent-level)
...
enum smlAgentEventId {
  ...
  // smlEVENT_BEFORE_AGENTS_RUN_STEP,      // Moved to system events because not
  // specific to one agent
  smlEVENT_BEFORE_AGENT_REINITIALIZED,  // DJP: I think these init-soar events
                                        // should move to run events.  When
                                        // they're hear you listen to the kernel
                                        // for these not the agent, which seems
                                        // wrong.
  smlEVENT_AFTER_AGENT_REINITIALIZED,   // DJP: I'm not making the change today
                                        // as I'm removing gSKI and want to
                                        // minimize other changes, but it should
                                        // be a simple change once gSKI has been
                                        // fully removed.
...
enum smlWorkingMemoryEventId {
  ...
  smlEVENT_INPUT_PHASE_CALLBACK,  // This event is not currently available to
                                  // clients -- listen for before/after input
                                  // phase instead
...
enum smlPrintEventId {
  ...
  smlEVENT_FIRST_PRINT_EVENT =
      smlEVENT_ECHO,  // This is a "clever" way of reducing the print event
                      // number to 0..n for array indexing, see PrintListener
                      // buffers
  smlEVENT_PRINT,
...
enum smlRhsEventId {
  ...
  // fired within Soar productions.  This is different from normal events in
  // that the handler is executing the function and returning a value, not just
  // being notified that something has happened.
  smlEVENT_RHS_USER_FUNCTION = smlEVENT_LAST_PRINT_EVENT + 1,
  smlEVENT_FILTER,  // This event can be used to filter (modify) command lines
                    // before they are processed by the kernel
  smlEVENT_CLIENT_MESSAGE,  // A generic message from one client to another (not
                            // really involving Soar/kernel directly)
...
enum smlXMLEventId {
  ...
  smlEVENT_XML_INPUT_RECEIVED,  // Echo event for input wmes added by a client
                                // (so others can listen in)
  smlEVENT_LAST_XML_EVENT = smlEVENT_XML_INPUT_RECEIVED
};

// Events that can be used by environments to trigger when the world should
// update
enum smlUpdateEventId {
  smlUPDATE_EVENT_BAD = -1,
  smlEVENT_AFTER_ALL_OUTPUT_PHASES =
      smlEVENT_LAST_XML_EVENT + 1,  // All agents have completed output phase
  smlEVENT_AFTER_ALL_GENERATED_OUTPUT,  // All agents have generated output
                                        // (since run began)
...
enum smlStringEventId {
  ...
  smlEVENT_TCL_LIBRARY_MESSAGE =
      smlEVENT_LAST_UPDATE_EVENT +
      1,  // Arg is "char const*" -- the name of the production to edit
...
static inline bool IsStringEventID(int id) {
  return (id >= smlEVENT_TCL_LIBRARY_MESSAGE &&
          id <= smlEVENT_LAST_STRING_EVENT);
}
...
static inline bool IsPhaseEventID(int id) {
  return (id > smlEVENT_BEFORE_PHASE_EXECUTED &&
          id < smlEVENT_AFTER_PHASE_EXECUTED);
}
static inline bool IsBEFOREPhaseEventID(int id) {
  return (id > smlEVENT_BEFORE_PHASE_EXECUTED &&
          id <= smlEVENT_BEFORE_WM_PHASE);
}
static inline bool IsAFTERPhaseEventID(int id) {
  return (id >= smlEVENT_AFTER_INPUT_PHASE &&
          id < smlEVENT_AFTER_PHASE_EXECUTED);
}

static inline bool IsProductionEventID(int id) {
  return (id >= smlEVENT_AFTER_PRODUCTION_ADDED &&
          id <= smlEVENT_LAST_PRODUCTION_EVENT);
}

static inline bool IsAgentEventID(int id) {
  return (id >= smlEVENT_AFTER_AGENT_CREATED &&
          id <= smlEVENT_LAST_AGENT_EVENT);
}
...
static inline bool IsUpdateEventID(int id) {
  return (id >= smlEVENT_AFTER_ALL_OUTPUT_PHASES &&
          id <= smlEVENT_LAST_UPDATE_EVENT);
}

enum smlPhase {
  sml_INPUT_PHASE,  // NOTE: This enum MUST be kept in synch with egSKIPhaseType
                    // defined in gSKI_Enumerations.h
  sml_PROPOSE_PHASE,
  sml_DECIDE_PHASE,
  sml_APPLY_PHASE,
  sml_OUTPUT_PHASE,
...
enum smlRunStepSize {
  // Note: phase moved before elaboration because that's how gSKI was doing
  // things
...
enum smlRunFlags {
  ...
  sml_DONT_UPDATE_WORLD = 1
                          << 3  // User explicitly requested world to not update
};
...
enum smlRunResult {
  ...
  sml_RUN_INTERRUPTED,
  sml_RUN_COMPLETED,
  sml_RUN_COMPLETED_AND_INTERRUPTED,  // Stop was requested but run completed
                                      // before agent was interrupted.
...
enum smlStopLocationFlags {
  sml_STOP_AFTER_SMALLEST_STEP = 1
                                 << 0,  // Keep in synch with egSKIStopLocation
...
class EXPORT Events {
 ...
 public:
  // Construction is expensive, so do this once and then use the methods
  // repeatedly
  Events();

  // Methods to convert event IDs from int to string values and back.
  // We do this because the list of IDs will likely change from one version of
  // Soar to another but we'd like the list of names to remain largely constant
  // (and just be added to). This will allow clients built with an earlier
