...
// kernel.
//
// The connection can be "embedded" which means both the client and the kernel
// are in the same process or it can be "remote" which means the client and the
// kernel are in different processes and possibly on different machines.
//
...
// and some user data.  The result can be NULL or a message that is sent back
// over the connection.
...
class Callback {
 ...
 public:
  ...
  soarxml::ElementXML* Invoke(soarxml::ElementXML* pIncomingMessage) {
    soarxml::ElementXML* pResult =
        (*m_pCallback)(m_pConnection, pIncomingMessage, m_pUserData);
    return pResult;
  }
};
...
class EXPORT Connection {
 ...
 protected:
  // Maps from SML document types (e.g. "call") to a list of functions to call
  // when that type of message is received.
  CallbackMap m_CallbackMap;

  // The client or kernel may wish to keep state information (e.g. a pointer
  // into gSKI for the kernel) with the connection.  If so, it can store it
  // here.  The caller retains ownership of this object, so it won't be deleted
  ...
  // A list of messages that have been received on this connection and are
  // waiting to be executed. This queue may not be in use for a given type of
  // connection
  MessageQueue m_IncomingMessageQueue;

  // We use this mutex to serialize acccess to the incoming message queue (for
  ...
  std::string
      m_Status;  // Status, optionally set by client from fixed set of values
  std::string m_AgentStatus;  // Agent status, referring to last created agent.
                              // Similar to connection status above.

  // The value to use for this connection's client side time tags (so each
  // connection can have its own part of the id space)
 ...
 public:
   ...
   *thread.
   * @param port           The port number the server should use to receive
   *remote connections.  The default port for SML is 12121 (picked at random).
   *
   * @param pError         Pass in a pointer to an int and receive back an error
   ...
   *"202.55.12.54"). Pass "127.0.0.1" to create a connection between two
   *processes on the same machine.
   * @param port       The port number to connect to.  The default port for SML
   *is 12121 (picked at random).
   * @param pError     Pass in a pointer to an int and receive back an error
   *code if there is a problem.  (Can pass NULL).
   *
   * @returns A RemoteConnection instance.
   */
  static Connection* CreateRemoteConnection(bool sharedFileSystem,
 ...
 public:
   ...
   *all. The direct connection is only true if this is a synchronous embedded
   *connection.
   */
   ...
   *by Soar.
   *
   * @param pMsg   The message (as an object representing XML) that is to be
   *sent. The caller should release this message object after making the send
   *call once it if finished using it.
   */
   ...
   *
   *        This call never blocks.  In an embedded situation, this does nothing
   *as incoming messages are sent directly to the callback functions. In a
   *remote situation, the client must call this function periodically, to read
   *incoming messages on the socket.
   *
   *        We use a callback model (rather than retrieving each message in turn
   *here) so that the embedded model and the remote model are closer to each
   *other.
   *
   * @param allMessages    If false, only retrieves at most one message before
   *returning, otherwise gets all messages.
   ...
   *available and the "wait" parameter is ignored. In a remote situation, if
   *wait is false and the result is not immediately available this call returns
   *false.
   *
   *        The ID is only required when the client is remote (because then
   *there might be many responses waiting on the socket). A message can only be
   *retrieved once, so a second call with the same ID will return NULL. Only the
   *response to the last call message can be retrieved.
   *
   *        The client is not required to call to get the result of a command it
   *has sent.
   *
   *        The implementation of this function will call ReceiveMessages() to
   *get messages one at a time and process them.  Thus callbacks may be invoked
   *while the client is blocked waiting for the particular response they
   *requested.
   *
   *        A response that is returned to the client through
   *GetResultOfMessage() will not be passed to a callback function registered
   *for response messages.  This allows a client to register a general function
   *to check for any error messages and yet retrieve specific responses to calls
   *that it is particularly interested in.
   *
   * @param pID    The id of the original SML message (the id is a attribute in
   *the top level [sml] tag)
   * @param wait   If true wait until the result is received (or we time out and
   *report an error).
   *
   ...
   *false.
   *
   *        The message is only required when the client is remote (because then
   *there might be many responses waiting on the socket). A message can only be
   *retrieved once, so a second call with the same ID will return NULL. Only the
   *response to the last call message can be retrieved.
   *
   ...
   *        A call is always paired to a response (think of this as a remote
   *function call that returns a value) while a notify does not receive a
   *response (think of this as a remote function call that does not return a
   *value). This type is stored in the "doctype" attribute of the top level SML
   *node in the message. NOTE: doctype's are case sensitive.
   *
   *        You MUST register a callback for the "call" type of message.  This
   *callback must return a "response" message which is then sent back over the
   *connection.  Other callbacks should not return a message. Once the returned
   *message has been sent it will be deleted.
   *
   *        We will maintain a list of callbacks for a given type of SML
   *document and call each in turn. Each callback on the list will be called in
   *turn until one returns a non-NULL response.  No further callbacks will be
   *called for that message.  This ensures that only one response is sent to a
   *message.
   *
   * @param callback   The function to call when an incoming message is received
   *(of the right type)
   * @param pUserData  This data is passed to the callback.  It allows the
   *callback to have some context to work in.  Can be NULL.
   * @param pType      The type of message to register for (currently one of
   *"call", "response" or "notify").
   * @param addToEnd   If true add the callback to the end of the list (called
   *last).  If false, add to front where it will be called first.
   */
   ...
   *of incoming message.
   *
   * @param callback   The function that was previously registered.  If NULL
   *removes all callbacks for this type of message.
   * @param pType      The type of message to unregister from (currently one of
   *"call", "response" or "notify").
   */
   ...
   *message.
   *
   * @param pIncomingMsg   The SML message that should be passed to the
   *callbacks.
   *
   * @returns The response message (or NULL if there is no response from any
   *calback).
   */
   ...
   *
   * The client or kernel may wish to keep state information (e.g. a pointer
   *into gSKI for the kernel) with the connection.  If so, it can store it here.
   *The caller retains ownership of this object, so it won't be deleted when the
   ...
   *client (e.g. debugger/java-toh etc.) Status - optional, set by client from
   *fixed list of values Agent status - optional, set by client and refers to
   *last created agent (set to "created" initially by kernel).
   */
   ...
   * @param pAgentName     The name of the agent this command is going to (can
   *be NULL -> implies going to top level of kernel)
   ...
   * @param rawOuput       If true, sends back a simple string form for the
   *result which the caller will probably just print. If false, sendds back a
   *structured XML object that the caller can analyze and do more with.
   * @returns  True if command was sent and received back without any errors
   *(either in sending or in executing the command).
   */
  bool SendAgentCommand(AnalyzeXML* pResponse, char const* pCommandName,
                        bool rawOutput = false);

  bool SendAgentCommand(AnalyzeXML* pResponse, char const* pCommandName,
                        char const* pAgentName, bool rawOutput = false);

  bool SendAgentCommand(AnalyzeXML* pResponse, char const* pCommandName,
                        char const* pAgentName, char const* pParamName1,
                        char const* pParamVal1, bool rawOutput = false);

  bool SendAgentCommand(AnalyzeXML* pResponse, char const* pCommandName,
                        char const* pAgentName, char const* pParamName1,
                        char const* pParamVal1, char const* pParamName2,
                        char const* pParamVal2, bool rawOutput = false);
  ...
  bool SendClassCommand(AnalyzeXML* pResponse, char const* pCommandName,
                        char const* pThisID, char const* pParamName1,
                        char const* pParamVal1);

  bool SendClassCommand(AnalyzeXML* pResponse, char const* pCommandName,
                        char const* pThisID, char const* pParamName1,
                        char const* pParamVal1, char const* pParamName2,
                        char const* pParamVal2);
   ...
   *
   * @param pType  The type of message (currently one of "call", "response" or
   *"notify"). Think of a call as a remote function call that returns a value
   *(the response). Think of a notify as a remote function call that does not
   *return a value.
   *
   ...
   *name of the command is "excise". Then add parameters to this.
   *
   * @param pName  The name of the command (the meaning depends on whoever
   *receives this command).
   * @param rawOutput  If true, results from command will be a string wrapped in
   *a <raw> tag, rather than full structured XML. (Defaults to false).
   *
   ...
   * @param pValue     The value of this parameter (represented as a string).
   *Can be empty, can't be NULL.
   * @param pValueType The type of the value (e.g. "int" or "string".  Anything
   *can go here as long as the recipient understands it) (usually will be NULL).
   *
   * @returns Pointer to the ElementXML_Handle for the <command> tag (not the
   *full message, just the <command> part) This is rarely needed, but could be
   *used to optimize the code.  DO NOT release this handle.
   */
  virtual ElementXML_Handle AddParameterToSMLCommand(
   ...
   * @param pErrorMsg  A description of the error in a form presentable to the
   *user
   * @param errorCode  An optional numeric code for the error (to support
   *programmatic responses to the error)
   */
   ...
   */
  virtual void AddSimpleResultToSMLResponse(soarxml::ElementXML* pResponse,
                                            char const* pResult);

  void SetInitialTimeTagCounter(int64_t value) {
    m_InitialTimeTagCounter = value;
  }
