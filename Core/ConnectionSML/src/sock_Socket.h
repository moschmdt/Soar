...
// By doing this in the code like this, each client doesn't need to add the
// winsock library as an explicit dependency.  When it includes this header, it
// gets the winsock library linked in.
...
class Socket : public DataSender {
 ...
 protected:
  ...
  // These objects are created through the ListenerSocket or ClientSocket
  // classes.
 protected:
  Socket();
 ...
 public:
  ...
  // Returns true if socket is closed--but then receiveMsg will know it's
  // closed. The timeout for waiting for data is secondsWait + millisecondsWait,
  // where millisecondsWait < 1000
  bool IsReadDataAvailable(int secondsWait = 0, int millisecondsWait = 0);

 public:
  // Print out debug information about the messages we are sending and
  // receiving. NOTE: We still print out information about start up/shut down,
  // errors etc. without this flag being true.
  void SetTraceCommunications(bool state) { m_bTraceCommunications = state; }

  // Send a string of characters.  Outgoing format will be "<4-byte
  // length>"+string data
  bool SendString(char const* pString);

  // Receive a string of characters.  Incoming format on socket should be
