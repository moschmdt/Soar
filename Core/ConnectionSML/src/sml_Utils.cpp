...
void sml::Sleep(int secs, int msecs) {
  ...
  // Use the Windows API Sleep function (need to specify the Global namespace to
  // distinguish from sml::Sleep)
  ::Sleep((secs * 1000) + msecs);

#else  // not _WIN32

  // if sleep 0 is requested, then don't sleep at all (an actual sleep 0 is very
  // slow on Linux, and probably OS X, too)
  if (msecs || secs) {
...
void sml::PrintDebugFormat(char const* pFormat, ...) {
  ...
  if (nBuf < 0) {
    strcpy(szBuffer,
           "** Debug message too long for PrintDebugFormat's buffer **");
  }
