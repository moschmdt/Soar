...
class RemoteConnection : public Connection {
 ...
 protected:
  // Clients should not use this.  Use Connection::CreateRemoteConnection
 ...
 protected:
  // The data sender we use to send and receive messages (data senders are
  // always 2-way)
  sock::DataSender* m_DataSender;

  // Whether both sides of the socket have access to the same file system
  // (i.e. whether sending a filename makes sense or if we need to send the file
  // contents). As of today (Oct 2004) this flag is always assumed to be true,
  // but later on we may start to support situations where it's false and having
