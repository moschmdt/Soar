...
class EXPORT AnalyzeXML {
 ...
 public:
  ...
  // Return the handle to the XML we're analyzing.  You can wrap then this into
  // an ElementXML* object if you wish. Be sure to call AddRef on this handle if
  // you keep it or wrap it in another ElementXML object.
  ElementXML_Handle GetElementXMLHandle() { return m_hRootObject; }

  // Each of these either returns a reference to the tag or NULL (if this
  // document doesn't contain that tag)
  ...
  // Returns the name of the command (or NULL if no command tag or no name in
  // that tag)
  char const* GetCommandName() const;

  // Returns the character data from the result tag (or NULL if no result tag or
  // no character data in that tag)
  char const* GetResultString() const;

  // Returns the character data from the result tag as an int (or default value
  // if there is no character data in the tag)
  int GetResultInt(int defaultValue) const;

  // Returns the character data from the result tag as an int64_t (or default
  // value if there is no character data in the tag)
  ...
  // Returns the string form of the XML.  Must be released with the static
  // DeleteString method
  ...
  //
  // BADBAD: We no longer use these argument position methods so this class
 ...
 protected:
   ...
   *in the name of the argument and its position in the argument order.  We look
   *for it by name and if that fails, look for it by position. You either get
   *back the right arg or NULL if this document doesn't contain that argument.
