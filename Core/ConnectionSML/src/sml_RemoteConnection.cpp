...
// and the Soar kernel.
//
// NOTE: This class is VERY similar to the EmbeddedConnectionAsynch class.  In
// fact we should probably fold them together at some point with a common base
...
RemoteConnection::RemoteConnection(bool sharedFileSystem,
                                   sock::DataSender* pDataSender) {
...
RemoteConnection::~RemoteConnection() {
  ...
  for (MessageListIter iter = m_ReceivedMessageList.begin();
       iter != m_ReceivedMessageList.end(); iter++) {
    ElementXML* xml = (*iter);
    delete xml;
  }
}
...
void RemoteConnection::AddResponseToList(ElementXML* pResponse) {
  ...
  if (m_bTraceCommunications) {
    sml::PrintDebugFormat("!! Adding ack for id %s to the pending message list",
                          pAckID);
  }

  // We keep the received message list from growing indefinitely.  This is
  // because a client may send a command and choose not to listen for the
  // response. (I don't believe this happens today, but it is allowed by the
  // API). In that case the message would remain on this list forever and if we
  // allowed it to grow over time we could be searching an ever increasingly
  // large list of dead messages that will never be retrieved.  I believe (but
  // haven't conclusively proved to my satisfaction yet) that we will never have
  // more messages pending here, for which the client is interested in the
  // response, than there are threads sending commands, so a small max list size
  // should be fine.
  while (m_ReceivedMessageList.size() > kMaxListSize) {
    if (m_bTraceCommunications) {
      sml::PrintDebugFormat(
          "Had to clean a message from the pending message list");
    }
...
ElementXML* RemoteConnection::IsResponseInList(char const* pID) {
  ...
  for (MessageListIter iter = m_ReceivedMessageList.begin();
    ...
    if (DoesResponseMatch(pXML, pID)) {
      if (m_bTraceCommunications) {
        sml::PrintDebugFormat("Found match for %s in pending message list",
                              pID);
      }
...
bool RemoteConnection::DoesResponseMatch(ElementXML* pResponse,
  ...
  if (m_bTraceCommunications) {
    sml::PrintDebugFormat("Received ack for message %s while looking for %s",
                          pMsgID, pID);
  }
...
void RemoteConnection::SendMsg(ElementXML* pMsg) {
  ...
  if (!ok) {
    // sml::PrintDebug("Socket has closed down abruptly (during send), so we'll
...
ElementXML* RemoteConnection::GetResponseForID(char const* pID, bool wait) {
  ...
  //  on one thread, but if we are using multiple threads it can come into
  //  play).
  ...
  int sleepTimeMillisecs =
      0;  // How long we sleep in milliseconds each pass through

  // How long we sleep on the socket waiting for data in msecs
  // We want to wait for a long time.  We used to set this to 0 and just poll
  // the socket, but that means we're consuming all of the CPU.  Setting a long
  // wait doesn't impact performance because we're not trying to do anything
  // else other than get a response here.
  ...
  do {
    ...
    while (ReceiveMessages(false, waitForMessageTimeSeconds,
                           waitForMessageTimeMilliseconds)) {
...
bool RemoteConnection::ReceiveMessages(bool allMessages, int secondsWait,
  ...
  while (haveData) {
    ...
    // Only check for read data after we've checked that the socket is still
    // alive. (This is because IsReadData can't signal the difference between a
    // dead connection and no data)
    ...
    ElementXML* pIncomingMsg =
        ElementXML::ParseXMLFromString(xmlString.c_str());
...
void RemoteConnection::SetTraceCommunications(bool state) {
  ...
  if (m_DataSender) {
    m_DataSender->SetTraceCommunications(state);
  }
}

void RemoteConnection::CloseConnection() { m_DataSender->Close(); }

bool RemoteConnection::IsClosed() { return !m_DataSender->IsAlive(); }
