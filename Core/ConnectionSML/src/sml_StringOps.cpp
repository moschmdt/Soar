...
int sml::Tokenize(std::string cmdline,
  ...
  for (;;) {
    ...
    ++argc;
    arg.str(std::string());
    // Use space as a delimiter unless inside quotes or brackets (nestable) or
    ...
    while (!isspace(*iter) || quotes || pipes || brackets || parens) {
      ...
      // Add to argument (if we eat quotes, this has to be moved into the else
      // above
      arg << (*iter);
      ...
      if (iter == cmdline.end()) {
        ...
        if (quotes || pipes || brackets || parens) {
          // FIXME: note that Trim will fail with bad pipes before parsing gets
          // here, so I don't think pipes will ever be true at this point.
          int code = 0;
          if (quotes) {
            code += -4;
          }
...
bool sml::TrimComments(std::string& line) {
  if (!line.size()) {
    return true;  // nothing on the line
  }
  ...
  const char* targets = "\\#|\"";
  for (std::string::size_type pos = line.find_first_of(targets, searchpos);
       pos != std::string::npos; pos = line.find_first_of(targets, searchpos)) {
    switch (line[pos]) {
      case '\\':  // skip backslashes
        searchpos = pos + 2;
        break;

      case '#':  // if not inside pipes or quotes, erase from pound to end or
                 // newline encountered
        if (pipe || quote) {
          searchpos = pos + 1;
        } else {
