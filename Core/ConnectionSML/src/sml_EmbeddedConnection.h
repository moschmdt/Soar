...
class EXPORT EmbeddedConnection : public Connection {
 public:
  // Clients should not use this.  Use Connection::CreateEmbeddedConnection
  // instead which creates a two-way connection.  This just creates a one-way
  // object.
  //  static Connection* CreateEmbeddedConnection() ;

 protected:
  // Clients should not use this.  Use Connection::CreateEmbeddedConnection
  // instead. Making it protected so you can't accidentally create one like
  // this.
  EmbeddedConnection();
 ...
 public:
  ...
  virtual void AttachConnectionInternal(Connection_Receiver_Handle hConnection,
  ...
  virtual bool IsAsynchronous() = 0;  // Returns true if messages are queued and
                                      // executed on receiver's thread
  ...
  // Direct methods, only supported for embedded connections and only used to
  // optimize the speed when doing I/O over an embedded connection (where speed
  // is most critical)
  void DirectAddWME_String(Direct_AgentSML_Handle pAgentSML, char const* pId,
                           char const* pAttribute, char const* pValue,
                           int64_t clientTimetag);
  void DirectAddWME_Int(Direct_AgentSML_Handle pAgentSML, char const* pId,
                        char const* pAttribute, int64_t value,
                        int64_t clientTimetag);
  void DirectAddWME_Double(Direct_AgentSML_Handle pAgentSML, char const* pId,
                           char const* pAttribute, double value,
                           int64_t clientTimetag);
  void DirectRemoveWME(Direct_AgentSML_Handle pAgentSML, int64_t clientTimetag);
  void DirectAddID(Direct_AgentSML_Handle pAgentSML, char const* pId,
                   char const* pAttribute, char const* pValueId,
                   int64_t clientTimetag);
  ...
  void DirectRun(char const* pAgentName, bool forever, int stepSize,
                 int interleaveSize, uint64_t count);
};
