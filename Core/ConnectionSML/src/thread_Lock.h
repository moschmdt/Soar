...
// The more primitive reentrant mutex class which does the work for the Lock
// class. You create one of these in the object that requires locking and then
// create "lock" objects as necessary to actually do the locking. This is safer
// than exposing the "Lock" and "Unlock" methods which is dangerous because it's
// east to forget to Unlock (or to have an exception or other unexcepted exit
// out of a function) causing a deadlock.
class EXPORT Mutex {
  friend class Lock;
...
// It can be useful in certain situations.  If in doubt, just use the "Lock"
// class.
class ConditionalLock {
 protected:
  Mutex* m_Mutex;
 ...
 public:
  ...
  // If the lock is obtained, the destructor will release it, as usual for a
  // lock.
