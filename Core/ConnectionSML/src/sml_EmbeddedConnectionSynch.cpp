...
// represents, both entities are within the same process.  For the "Synch"
...
void EmbeddedConnectionSynch::SendMsg(ElementXML* pMsg) {
  ...
  // Make the call to the kernel, passing the message over and getting an
  // immediate response since this is an embedded synchronous (in thread) call.
  hResponse = m_pProcessMessageFunction(m_hConnection, hSendMsg,
                                        SML_MESSAGE_ACTION_SYNCH);
...
ElementXML* EmbeddedConnectionSynch::GetResponseForID(char const* /*pID*/,
  ...
  // call. There's also no need to wait, we always have the result on hand.
  ...
  // (If we returned a pointer to m_LastResponse it could change when new
  // messages come in).
