...
bool NamedPipe::SendBuffer(char const* pSendBuffer, uint32_t bufferSize) {
  ...
  while (bytesSent < bufferSize) {
    do {
      success = WriteFile(hPipe,        // handle to pipe
      ...
      if (!success) {
#ifdef PIPE_NON_BLOCKING
        // On a non-blocking pipe, the pipe can return "pipe closing" -- in
        // which case we need to wait for it to clear.  A blocking pipe would
...
bool NamedPipe::IsReadDataAvailable(int /*secondsWait*/,
  ...
  if (hPipe == INVALID_HANDLE_VALUE) {
    sml::PrintDebug(
        "Error: Can't check for read data because this pipe is closed");
    return false;
  }
  ...
  if (res == 0) {
    ...
    // If we later decide we can survive certain ones, we should test for them
    // here and not always close the socket.
...
bool NamedPipe::ReceiveBuffer(char* pRecvBuffer, uint32_t bufferSize) {
  ...
  while (bytesRead < bufferSize) {
    ...
    do {
      ...
      success = ReadFile(hPipe,        // handle to pipe
      ...
      if (!success) {
#ifdef PIPE_NON_BLOCKING
        // On a non-blocking pipe, the socket may return "pipe closing" -- in
        // which case we need to wait for it to clear.  A blocking pipe would
        // not return in this case so this would always be an error.
        if (IsErrorWouldBlock()) {
          // sml::PrintDebug("Waiting for pipe to unblock") ;
    ...
    // if(tries>1)   sml::PrintDebugFormat("Number tries %d",
    // static_cast<int64_t>(tries));
