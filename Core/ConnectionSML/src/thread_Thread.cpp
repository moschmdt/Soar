...
void Thread::Start() {
  ...
  // turn calls back to the "Run" method of this class, so the user of the
  // thread has a nice object wrapped in a class to work with for their thread.
...
void Thread::Stop(bool waitTillStopped) {
  ...
  // generally not a very safe thing to do.  If you feel you need it, add a
  // "ForceStop" method.
  m_QuitNow = true;
  ...
  // If we've been asked to wait for the thread to quit then pause for a moment
  // while it terminates.
