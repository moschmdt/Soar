...
// represents, both entities are within the same process.  For the "Asynch"
// variant, the two entities execute in different threads.
//
// NOTE: This class is VERY similar to the RemoteConnection class.  In fact we
// should probably fold them together at some point with a common base class.
...
void EmbeddedConnectionAsynch::SendMsg(ElementXML* pMsg) {
  ...
  // Add a reference to this object, which will then be released by the receiver
  // of this message when they are done with it.
  pMsg->AddRefOnHandle();
  ...
  // Make the call to the kernel, passing the message over with the ASYNCH flag,
  // which means there will be no immediate response.
  ElementXML_Handle hResponse = m_pProcessMessageFunction(
      m_hConnection, hSendMsg, SML_MESSAGE_ACTION_ASYNCH);
...
ElementXML* EmbeddedConnectionAsynch::GetResponseForID(char const* pID,
  ...
  do {
    ...
    m_WaitEvent.WaitForEvent(maximumWaitTimeSeconds,
