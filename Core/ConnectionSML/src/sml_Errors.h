 ...
 public:
  ...
  static char const* GetErrorDescription(::ErrorCode code) {
    switch (code) {
      ...
      case kNoCallback:
        return "No callback is registered for this type of incoming message "
               "(that may be ok)";
      case kNoResponseToCall:
        return "This type of message requires a response but no callback "
               "created a response";
      case kNoEmbeddedLink:
        return "This embedded connection has not been set up correctly, so "
               "there is nowhere to send the messages to";
      case kArgumentIsNotSML:
        return "The ElementXML object passed in is not a pointer to a valid "
               "SML message";
      case kFunctionsNotFound:
        return "Loaded the shared library, but could not find the necessary "
               "exported functions.  Check their names and types.";
      case kCreationFailed:
        return "Loaded the shared library, but the attempt to create an "
               "embedded connection failed.";
      ...
      case kInvalidResponse:
        return "The connection sent a response when none should have been "
               "sent.";
      case kResponseIsNotSML:
        return "The kernel sent back a response that is not formatted as an "
               "SML message.";
      case kFailedToGetResponse:
        return "No response came back for the command we sent.";
      case kSMLErrorMessage:
        return "The SML message contained an error tag.  We should display "
               "that instead of this generic error.";
      case kDetailedError:
        return "There should be a detailed error message to accompany this and "
               "we should show that instead of this!";
      case kAgentExists:
