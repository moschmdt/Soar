...
// issue commands etc.
//
// This class is actually an abstract base class, with specific implementations
// of this class being used to provide the different types of connections.
//
...
#ifdef _DEBUG
  // It's useful to start somewhere other than 0 in debug, especially when
  // dealing with remote connections, so it's clear which id's come from which
  // client.
 ...
 *thread.
 * @param port           The port number the server should use to receive remote
 ...
 */
Connection* Connection::CreateEmbeddedConnection(bool clientThread,
                                                 bool optimized,
                                                 int portToListenOn,
                                                 ErrorCode* pError) {
  // Set an initial error code and then replace it if something goes wrong.
  if (pError) {
    *pError = Error::kNoError;
  }

  // We also use the terms "synchronous" and "asynchronous" for client thread or
  // kernel thread.
  EmbeddedConnection* pConnection =
      clientThread ? EmbeddedConnectionSynch::CreateEmbeddedConnectionSynch()
  ...
  // Report any errors
  if (pError) {
    *pError = pConnection->GetLastError();
  }

  return pConnection;
}

Connection* Connection::CreateRemoteConnection(bool sharedFileSystem,
...
Connection* Connection::CreateRemoteConnection(sock::DataSender* pDataSender) {
  ...
  // if the client and it share the same file system, so just set it to true by
  // default.
 ...
 *
 *        In an embedded situation, this result is always immediately available
 *and the "wait" parameter is ignored. In a remote situation, if wait is false
 *and the result is not immediately available this call returns false.
 *
 ...
 *has sent.
 *
 *        The implementation of this function will call ReceiveMessages() to get
 *messages one at a time and process them.  Thus callbacks may be invoked while
 *the client is blocked waiting for the particular response they requested.
 *
 *        A response that is returned to the client through GetResultOfMessage()
 *will not be passed to a callback function registered for response messages.
 *This allows a client to register a general function to check for any error
 *messages and yet retrieve specific responses to calls that it is particularly
 *interested in.
 *
 ...
 */
ElementXML* Connection::GetResponse(ElementXML const* pXML,
                                    bool wait /* == true */) {
  if (!pXML) {
 ...
 *function call that returns a value) while a notify does not receive a response
 *(think of this as a remote function call that does not return a value). This
 *type is stored in the "doctype" attribute of the top level SML node in the
 *message. NOTE: doctype's are case sensitive.
 *
 ...
 *
 *        We will maintain a list of callbacks for a given type of SML document
 *and call each in turn. Each callback on the list will be called in turn until
 *one returns a non-NULL response.  No further callbacks will be called for that
 *message.  This ensures that only one response is sent to a message.
 *
 ...
 *
 * @returns 0 if successful, otherwise an error code to indicate what went
 *wrong.
 */
void Connection::RegisterCallback(IncomingCallback callback, void* pUserData,
...
/**
 * @brief Removes a callback from the list of callbacks for a particular type of
 *incoming message.
 *
 ...
 *wrong.
 */
void Connection::UnregisterCallback(IncomingCallback callback,
                                    char const* pType) {
  ClearError();
...
CallbackList* Connection::GetCallbackList(char const* pType) {
  ...
  // doing this on a map has the nasty side-effect of adding an object to the
  // map. So the preferred method is to instead use find.
  CallbackMapIter iter = m_CallbackMap.find(pType);
...
ElementXML* Connection::InvokeCallbacks(ElementXML* pIncomingMsg) {
  ...
  MessageSML* pIncomingSML = (MessageSML*)
      pIncomingMsg;  // MessageSML is a soarxml::ElementXML, not sml::ElementXML
 ...
 */
bool Connection::SendMessageGetResponse(AnalyzeXML* pAnalysis,
                                        ElementXML* pMsg) {
  ...
  //  when the client is sleeping, but we don't want them both to be
 ...
 * @param pAgentName     The name of the agent this command is going to (can be
 *NULL -> implies going to top level of kernel)
 ...
 * @param rawOuput       If true, sends back a simple string form for the result
 *which the caller will probably just print. If false, sendds back a structured
 *XML object that the caller can analyze and do more with.
 * @returns  True if command was sent and received back without any errors
 *(either in sending or in executing the command).
 */
bool Connection::SendAgentCommand(AnalyzeXML* pResponse,
                                  char const* pCommandName, bool rawOutput) {
...
bool Connection::SendAgentCommand(AnalyzeXML* pResponse,
                                  ...
                                  char const* pAgentName, bool rawOutput) {
  ElementXML* pMsg = CreateSMLCommand(pCommandName, rawOutput);
...
bool Connection::SendAgentCommand(AnalyzeXML* pResponse,
                                  ...
                                  char const* pParamName1,
                                  char const* pParamVal1, bool rawOutput) {
  ElementXML* pMsg = CreateSMLCommand(pCommandName, rawOutput);
 ...
 * See SendAgentCommand() above.
 */
bool Connection::SendAgentCommand(
    AnalyzeXML* pResponse, char const* pCommandName, char const* pAgentName,
    char const* pParamName1, char const* pParamVal1, char const* pParamName2,
    char const* pParamVal2, bool rawOutput) {
  ElementXML* pMsg = CreateSMLCommand(pCommandName, rawOutput);
...
bool Connection::SendAgentCommand(
    AnalyzeXML* pResponse, char const* pCommandName, char const* pAgentName,
    char const* pParamName1, char const* pParamVal1, char const* pParamName2,
    char const* pParamVal2, char const* pParamName3, char const* pParamVal3,
    bool rawOutput) {
  ElementXML* pMsg = CreateSMLCommand(pCommandName, rawOutput);
 ...
 */
bool Connection::SendClassCommand(AnalyzeXML* pResponse,
                                  char const* pCommandName) {
 ...
 * @param pThisID        The id of the object (e.g. IAgent) whose method we are
 *calling.
 ...
 *(can be NULL if an optional param)
 * @returns  An analyzed version of the reply
 */
bool Connection::SendClassCommand(AnalyzeXML* pResponse,
                                  char const* pCommandName, char const* pThisID,
                                  char const* pParamName1,
...
bool Connection::SendClassCommand(AnalyzeXML* pResponse,
  ...
  // Note: If first param is missing, second must be ommitted too (normal
  // optional param syntax)
  if (pParamVal1) {
    AddParameterToSMLCommand(pMsg, pParamName1, pParamVal1);
  }
  if (pParamVal1 && pParamVal2) {
    AddParameterToSMLCommand(pMsg, pParamName2, pParamVal2);
  }
 ...
 * @returns  An analyzed version of the reply
 */
bool Connection::SendClassCommand(
    AnalyzeXML* pResponse, char const* pCommandName, char const* pThisID,
    char const* pParamName1, char const* pParamVal1, char const* pParamName2,
    char const* pParamVal2, char const* pParamName3, char const* pParamVal3) {
  ElementXML* pMsg = CreateSMLCommand(pCommandName);
  ...
  if (pParamVal1 && pParamVal2 && pParamVal3) {
    AddParameterToSMLCommand(pMsg, pParamName3, pParamVal3);
  }
 ...
 *"notify"). Think of a call as a remote function call that returns a value (the
 *response). Think of a notify as a remote function call that does not return a
 *value.
 *
...
/**
 * @brief Create a basic SML command message.  You should then add parameters to
 *this command. This function calls CreateSMLMessage() and then adds a <command>
 *tag to it. E.g. the command might be "excise -production" so the name of the
 *command is "excise". Then add parameters to this.
 *
 * @param pName      The name of the command (the meaning depends on whoever
 *receives this command).
 * @param rawOutput  If true, results from command will be a string wrapped in a
 *<raw> tag, rather than full structured XML.
 *
 * @returns The new SML command message.
 */
ElementXML* Connection::CreateSMLCommand(char const* pName,
  ...
  if (rawOutput) {
    pCommand->AddAttributeFastFast(sml_Names::kCommandOutput,
                                   sml_Names::kRawOutput);
  }
 ...
 * @param pValue     The value of this parameter (represented as a string).  Can
 ...
 *
 * @returns Pointer to the ElementXML_Handle for the <command> tag (not the full
 *message, just the <command> part) This is rarely needed, but could be used to
 *optimize the code.  DO NOT release this handle.
 */
ElementXML_Handle Connection::AddParameterToSMLCommand(ElementXML* pMsg,
                                                       char const* pName,
                                                       char const* pValue,
                                                       char const* pValueType) {
  ClearError();
...
ElementXML* Connection::CreateSMLResponse(ElementXML const* pIncomingMsg) {
  ...
  char const* pAck = ((MessageSML*)pIncomingMsg)
                         ->GetID();  // MessageSML is a soarxml::ElementXML, not
                                     // sml::ElementXML
 ...
 */
void Connection::AddErrorToSMLResponse(ElementXML* pResponse,
                                       char const* pErrorMsg,
                                       int errorCode /* = -1 */) {
  ClearError();
 ...
 */
void Connection::AddSimpleResultToSMLResponse(ElementXML* pResponse,
                                              char const* pResult) {
  ClearError();
...
ElementXML* Connection::PopIncomingMessageQueue() {
  ...
  // The guy receiving this expects to delete this object anyway when they're
  // done
  return pIncomingMsg;
}
