...
class RotationBase {
 ...
 public:
  ...
  friend EIGEN_DEVICE_FUNC inline RotationMatrixType operator*(
      const EigenBase<OtherDerived>& l, const Derived& r) {
  ...
  EIGEN_DEVICE_FUNC inline VectorType _transformVector(
...
struct rotation_base_generic_product_selector<
  ...
  EIGEN_DEVICE_FUNC static inline ReturnType run(
      const RotationDerived& r, const DiagonalMatrix<Scalar, Dim, MaxDim>& m) {
    ReturnType res(r);
    res.linear() *= m;
    return res;
  }
...
EIGEN_DEVICE_FUNC static inline Matrix<Scalar, 2, 2> toRotationMatrix(
    const Scalar& s) {
...
EIGEN_DEVICE_FUNC static inline Matrix<Scalar, Dim, Dim> toRotationMatrix(
