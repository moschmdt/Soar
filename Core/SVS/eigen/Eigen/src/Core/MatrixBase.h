...
#undef EIGEN_DOC_UNARY_ADDONS
  ...
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC const Product<Derived, OtherDerived> operator*(
  ...
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC inline PlainObject cross3(
      const MatrixBase<OtherDerived>& other) const;
  ...
  template <typename EssentialPart>
  EIGEN_DEVICE_FUNC void applyHouseholderOnTheLeft(
...
#define EIGEN_MATRIX_FUNCTION_1(ReturnType, Name, Description, Argument)     \
 ...
 protected:
  ...
  Derived& operator+=(const ArrayBase<OtherDerived>&) {
    EIGEN_STATIC_ASSERT(
        std::ptrdiff_t(sizeof(typename OtherDerived::Scalar)) == -1,
...
template <typename Derived>
template <typename OtherDerived>
inline Derived& MatrixBase<Derived>::operator*=(
...
template <typename Derived>
template <typename OtherDerived>
inline void MatrixBase<Derived>::applyOnTheRight(
    const EigenBase<OtherDerived>& other) {
  other.derived().applyThisOnTheRight(derived());
}
