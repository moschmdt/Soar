...
class variable_if_dynamicindex<T, DynamicIndex> {
 ...
 public:
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE explicit variable_if_dynamicindex(
      T value)
      : m_value(value) {}
...
struct nested_eval {
  enum {
    ...
    CoeffReadCost = evaluator<
        T>::CoeffReadCost,  // NOTE What if an evaluator evaluate itself into a
                            // temporary?
                            //      Then CoeffReadCost will be small (e.g., 1)
                            //      but we still have to evaluate, especially if
                            //      n>1. This situation is already taken care by
                            //      the EvalBeforeNestingBit flag, which is
                            //      turned ON for all evaluator creating a
                            //      temporary. This flag is then propagated by
                            //      the parent evaluators. Another solution
                            //      could be to count the number of temps?
