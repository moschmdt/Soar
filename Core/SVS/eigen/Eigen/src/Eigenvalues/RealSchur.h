 ...
 public:
  ...
  const MatrixType& matrixU() const {
    ...
    eigen_assert(m_matUisUptodate &&
                 "The matrix U has not been computed during the RealSchur "
                 "decomposition.");
    return m_matU;
  }
...
template <typename InputType>
RealSchur<MatrixType>& RealSchur<MatrixType>::compute(
...
template <typename MatrixType>
template <typename HessMatrixType, typename OrthMatrixType>
RealSchur<MatrixType>& RealSchur<MatrixType>::computeFromHessenberg(
    const HessMatrixType& matrixH, const OrthMatrixType& matrixQ,
    bool computeU) {
  using std::abs;
...
inline Index RealSchur<MatrixType>::findSmallSubdiagEntry(
    Index iu, const Scalar& considerAsZero) {
  using std::abs;
  Index res = iu;
  while (res > 0) {
    Scalar s =
...
inline void RealSchur<MatrixType>::initFrancisQRStep(
    Index il, Index iu, const Vector3s& shiftInfo, Index& im,
