...
template <typename Scalar>
template <typename Derived>
EIGEN_DEVICE_FUNC inline bool JacobiRotation<Scalar>::makeJacobi(
    const MatrixBase<Derived>& m, Index p, Index q) {
...
template <typename Scalar>
EIGEN_DEVICE_FUNC void JacobiRotation<Scalar>::makeGivens(
    const Scalar& p, const Scalar& q, Scalar* r, internal::false_type) {
  using std::abs;
  using std::sqrt;
  if (q == Scalar(0)) {
...
EIGEN_DEVICE_FUNC void apply_rotation_in_the_plane(
...
EIGEN_DEVICE_FUNC inline void MatrixBase<Derived>::applyOnTheLeft(
...
template <typename Derived>
template <typename OtherScalar>
EIGEN_DEVICE_FUNC inline void MatrixBase<Derived>::applyOnTheRight(
    Index p, Index q, const JacobiRotation<OtherScalar>& j) {
  ColXpr x(this->col(p));
  ColXpr y(this->col(q));
