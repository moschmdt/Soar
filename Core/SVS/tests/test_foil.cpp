...
test_spec tests[] = {
    {
        "foil_tests/1.test",
        true,
        {{"intersect(0,1,2) & ramp(0,2)", 1.0}, {NULL, 0.0}}  // sentinel
    },
    {"foil_tests/neg.test", true, {{"~indicator(0,1)", 1.0}, {NULL, 0.0}}},
    {"foil_tests/ramp.test", true, {{"intersect(0,1,2)", 0.975}, {NULL, 0.0}}},
...
bool run_foil(const char* path, bool prune, clause_vec& clauses, relation& pos,
              relation& neg, relation_table& all_rels, double& time);
void standalone(const char* path, bool prune);
bool test();
void test_clauses(clause_vec& clauses, relation& pos, relation& neg,
...
bool run_foil(const char* path, bool prune, clause_vec& clauses, relation& pos,
              relation& neg, relation_table& all_rels, double& t) {
  FOIL foil;
  ifstream input(path);
    ...
    for (j = 0; tests[i].clauses[j].clause != NULL; ++j) {
      ...
      if (learned == spec.clause) {
        ...
        if (!close(success, spec.success_rate)) {
          std::cout << "Expected success rate " << spec.success_rate << ", got "
                    << success << std::endl;
          result = false;
        }
      } else {
        std::cout << "Test " << j << " expected " << spec.clause << ", got "
                  << learned << std::endl;
        result = false;
      }
    }
...
bool close(double a, double b) { return fabs(a - b) < EQUAL_MARGIN; }

int test_clause_vec(const vector<clause>& clauses, const relation_table& rels,
                    var_domains& d) {
  for (int i = 0, iend = clauses.size(); i < iend; ++i) {
...
void test_clauses(clause_vec& clauses, relation& pos, relation& neg,
  ...
  std::cout << std::endl
            << success_rate << " success, " << correct << " correct, "
            << false_negs + false_pos << " incorrect, " << false_negs
            << " false negs, " << false_pos << " false pos" << std::endl;
}
