...
void get_rels_at_time(const relation_table& rels, int time,
                      relation_table& out) {
  tuple tt(1);
  tt[0] = time;
...
void learn(const relation& pos, const relation& neg, const relation_table& rels,
           clause_vec& clauses) {
  FOIL foil;
...
bool classify(const std::vector<clause>& clauses, const relation_table& rels,
              int target) {
  var_domains d;
  d[0].insert(0);
  d[1].insert(target);
...
int multi_classify(const multi_classifier& c, const relation_table& rels,
                   int target) {
  map<int, int> votes;

  map<pair<int, int>, clause_vec>::const_iterator k, kend;
  for (k = c.pair_clauses.begin(), kend = c.pair_clauses.end(); k != kend;
       ++k) {
...
void print_multi(const multi_classifier& mc) {
  ...
  for (i = mc.pair_clauses.begin(), iend = mc.pair_clauses.end(); i != iend;
       ++i) {
    std::cout << "For classes " << i->first.first << ", " << i->first.second
              << std::endl;
...
void parse_args(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc;) {
    if (strcmp(argv[i], "-train") == 0) {
      if (i + 2 >= argc || !parse_int(argv[i + 1], train_start) ||
          !parse_int(argv[i + 2], train_end)) {
        cerr << "invalid training range" << std::endl;
        exit(1);
      }
      i += 3;
    } else if (strcmp(argv[i], "-test") == 0) {
      if (i + 2 >= argc || !parse_int(argv[i + 1], test_start) ||
          !parse_int(argv[i + 2], test_end)) {
  ...
  if (argc - opt_end < 3) {
    cerr << "usage: " << argv[0]
         << " <options> <train_target> <train_rels> <train_classes> "
            "[<test_rels> ... ]"
         << std::endl;
