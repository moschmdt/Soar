 ...
 *calculate wrt Returns: vec3 - a position at which to place node a such that
 *the given axis separation constraints are satisfied Notes on separations: The
 *separation is a constraint on what the distance between the two nodes should
 *be along the given axis (using bounding boxes) A positive distance indicates
 *node a is higher up on the axis, A negative distance indicates node a is lower
...
vec3 calculate_placement(const sgnode* a, const sgnode* b, const sgnode* ref,
  ...
  for (int dim = 0; dim < 3; dim++) {
    ...
    if (sep[dim] > 0) {
      // Pos of a += axis * (max bound of b + separation distance + distance to
      // centroid of a) (mina is negative)
      pos = pos + axes[dim] * (maxb[dim] + sep[dim] - mina[dim]);
    } else if (sep[dim] < 0) {
      // Pos of a -= axis * (min bound of b + separation distance + distance to
      // centroid of a) (note separation distance is negative, so we subtract
      // it)
...
class calculate_placement_filter : public map_filter<vec3> {
 public:
  calculate_placement_filter(Symbol* root, soar_interface* si,
                             filter_input* input)
