 ...
 *    sgnode b
 *    axis << x y z >>
 *    sgnode reference [optional - default=world], frame of reference to
 *calculate wrt Returns: double - axis_distance between a and b
 *
 ...
 *    sgnode reference [optional - default=world], frame of reference to
 *calculate wrt Returns: sgnode b if min <= dist(a, b) <= max
 *
 */
#include <string>
...
double compare_axis_distance(sgnode* a, sgnode* b, const filter_params* p) {
  ...
  if (get_filter_param(0, p, "reference", ref)) {
    ...
    axis[dim] = 1.0;
    vec3 dir =
        ref->get_world_trans()(axis) - ref->get_world_trans()(vec3(0, 0, 0));
...
///// filter axis_distance filter* make_axis_distance_filter(Symbol* root,
/// soar_interface* si, scene* scn,
                                  filter_input* input) {
                                    return new node_comparison_filter(
                                        root, si, input,
                                        &compare_axis_distance);
                                  }

                                  filter_table_entry*
                                  axis_distance_filter_entry() {
                                    filter_table_entry* e =
                                        new filter_table_entry();
                                    e->name = "axis_distance";
                                    e->description =
                                        "Output distance between a and b on "
                                        "given axis";
                                    e->parameters["a"] = "Sgnode a";
                                    e->parameters["b"] = "Sgnode b";
                                    e->parameters["axis"] =
                                        "Axis to measure distance on (xyz)";
                                    e->parameters["reference"] =
                                        "[Optional] - base axis on coord frame "
                                        "of sgnode";
                                    e->create = &make_axis_distance_filter;
                                    return e;
                                  }

                                  ///// filter axis_distance_select filter*
                                  /// make_axis_distance_select_filter(Symbol*
                                  /// root, soar_interface* si,
                                         scene* scn, filter_input* input) {
                                           return new node_comparison_select_filter(
                                               root, si, input,
                                               &compare_axis_distance);
                                         }

                                         filter_table_entry*
                                         axis_distance_select_filter_entry() {
                                           filter_table_entry* e =
                                               new filter_table_entry();
                                           e->name = "axis_distance_select";
                                           e->description =
                                               "Select b if min <= dist(a, b, "
                                               "axis) <= max";
                                           e->parameters["a"] = "Sgnode a";
                                           e->parameters["b"] = "Sgnode b";
                                           e->parameters["axis"] =
                                               "Axis to measure distance on "
                                               "(xyz)";
                                           e->parameters["reference"] =
                                               "[Optional] - base axis on "
                                               "coord frame of sgnode";
                                           e->parameters["min"] =
                                               "minimum distance to select";
                                           e->parameters["max"] =
                                               "maximum distance to select";
                                           e->create =
                                               &make_axis_distance_select_filter;
                                           return e;
                                         }
