...
filter* make_axis_distance_filter(Symbol* root, soar_interface* si, scene* scn,
                                  filter_input* input) {
...
filter_table_entry* axis_distance_filter_entry() {
  ...
  e->parameters["reference"] =
      "[Optional] - base axis on coord frame of sgnode";
  e->create = &make_axis_distance_filter;
  return e;
}

///// filter axis_distance_select //////
filter* make_axis_distance_select_filter(Symbol* root, soar_interface* si,
                                         scene* scn, filter_input* input) {
  return new node_comparison_select_filter(root, si, input,
