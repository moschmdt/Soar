...
///// filter overlap filter* make_overlap_filter(Symbol* root, soar_interface*
/// si, scene* scn,
                            filter_input* input) {
                              return new node_comparison_filter(
                                  root, si, input, &compare_overlap);
                            }
                            ...
                            ///// filter overlap_select filter*
                            /// make_overlap_select_filter(Symbol* root,
                            /// soar_interface* si, scene* scn,
                                   ...
                                   filter_table_entry*
                                   overlap_select_filter_entry() {
                                     filter_table_entry* e =
                                         new filter_table_entry();
                                     e->name = "overlap_select";
                                     e->description =
                                         "Selects b if min <= overlap(a, b) <= "
                                         "max";
                                     e->parameters["a"] = "Sgnode a";
                                     e->parameters["b"] = "Sgnode b";
                                     e->parameters["min"] =
                                         "minimum overlap to select";
                                     e->parameters["max"] =
                                         "maximum overlap to select";
