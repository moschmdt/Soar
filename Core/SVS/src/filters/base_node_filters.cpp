...
bool node_select_range_filter::falls_in_range(double val) {
  if (include_min && val < range_min) return false;
  if (!include_min && val <= range_min) return false;
  if (include_max && val > range_max) return false;
  if (!include_max && val >= range_max) return false;
  return true;
}
...
bool node_test_select_filter::compute(const filter_params* p, sgnode*& out,
                                      bool& select) {
  sgnode* a = NULL;
  sgnode* b = NULL;
  if (!get_filter_param(this, p, "a", a) ||
...
bool node_comparison_select_filter::compute(const filter_params* p,
                                            sgnode*& out, bool& select) {
  sgnode* a = NULL;
  sgnode* b = NULL;
  if (!get_filter_param(this, p, "a", a) ||
      !get_filter_param(this, p, "b", b)) {
...
bool node_evaluation_select_filter::compute(const filter_params* p,
                                            sgnode*& out, bool& select) {
  sgnode* a = NULL;
  if (!get_filter_param(this, p, "a", a)) {
    set_status("Need node a input");
