...
///// filter volume filter* make_volume_filter(Symbol* root, soar_interface* si,
/// scene* scn,
                           filter_input* input) {
                             return new node_evaluation_filter(
                                 root, si, input, &evaluate_volume);
                           }
                           ...
                           filter_table_entry* volume_filter_entry() {
                             ...
                             e->parameters["volume_type"] =
                                 "Either bbox or scale";
                             e->create = &make_volume_filter;
                             return e;
                           }

                           ///// filter volume_select filter*
                           /// make_volume_select_filter(Symbol* root,
                           /// soar_interface* si, scene* scn,
                                  filter_input* input) {
                                    return new node_evaluation_select_filter(
                                        root, si, input, &evaluate_volume);
                                  }

                                  filter_table_entry*
                                  volume_select_filter_entry() {
                                    filter_table_entry* e =
                                        new filter_table_entry();
                                    e->name = "volume_select";
                                    e->description =
                                        "Select a if min <= volume(a) <= max";
                                    e->parameters["a"] = "Sgnode a";
                                    e->parameters["volume_type"] =
                                        "Either bbox or scale";
                                    e->parameters["min"] =
                                        "minimum volume to select";
                                    e->parameters["max"] =
                                        "maximum volume to select";
                                    e->create = &make_volume_select_filter;
                                    return e;
                                  }

                                  ////// filter smallest filter*
                                  /// make_smallest_filter(Symbol* root,
                                  /// soar_interface* si, scene* scn,
                             filter_input* input) {
                               node_evaluation_rank_filter* f =
                                   new node_evaluation_rank_filter(
                                       root, si, input, &evaluate_volume);
                             ...
                             filter_table_entry* smallest_filter_entry() {
                               ...
                               e->name = "smallest";
                               e->description =
                                   "Select node a with the smallest volume";
                               e->parameters["a"] = "Sgnode a";
                               e->parameters["volume_type"] =
                                   "Either bbox or scale";
                             ...
                             ////// filter largest  filter*
                             /// make_largest_filter(Symbol* root,
                             /// soar_interface* si, scene* scn,
                            filter_input* input) {
                              return new node_evaluation_rank_filter(
                                  root, si, input, &evaluate_volume);
                            }
                            ...
                            filter_table_entry* largest_filter_entry() {
                              ...
                              e->name = "largest";
                              e->description =
                                  "Select node a with the largest volume";
                              e->parameters["a"] = "Sgnode a";
                              e->parameters["volume_type"] =
                                  "Either bbox or scale";
                            ...
                            ////// filter larger filter*
                            /// make_larger_filter(Symbol* root, soar_interface*
                            /// si, scene* scn,
                           filter_input* input) {
                           ...
                           filter_table_entry* larger_filter_entry() {
                             ...
                             e->name = "larger";
                             e->description =
                                 "Returns true if volume(a) > volume(b)";
                             e->parameters["a"] = "Sgnode a";
                           ...
                           ////// filter smaller filter*
                           /// make_smaller_filter(Symbol* root, soar_interface*
                           /// si, scene* scn,
                            filter_input* input) {
                              return new node_test_filter(root, si, input,
                                                          &smaller_test);
                            ...
                            ////// filter larger_select filter*
                            /// make_larger_select_filter(Symbol* root,
                            /// soar_interface* si, scene* scn,
                                  filter_input* input) {
                                    return new node_test_select_filter(
                                        root, si, input, &larger_test);
                                  }

                                  filter_table_entry*
                                  larger_select_filter_entry() {
                                    filter_table_entry* e =
                                        new filter_table_entry();
                                    e->name = "larger_select";
                                    e->description =
                                        "Select b if volume(a) > volume(b)";
                                    e->parameters["a"] = "Sgnode a";
                                    e->parameters["b"] = "Sgnode b";
                                    e->parameters["volume_type"] =
                                        "Either bbox or scale";
                                  ...
                                  ////// filter smaller_select filter*
                                  /// make_smaller_select_filter(Symbol* root,
                                  /// soar_interface* si, scene* scn,
                                   filter_input* input) {
                                     return new node_test_select_filter(
                                         root, si, input, &smaller_test);
                                   }

                                   filter_table_entry*
                                   smaller_select_filter_entry() {
                                     filter_table_entry* e =
                                         new filter_table_entry();
