...
double axis_distance(const sgnode* a, const sgnode* b, const vec3& axis) {
  ...
  // cout << "======= axis_distance(" << a->get_id() << ", " << b->get_id() <<
  // ") along [" <<
  //   unit_axis[0] << ", " << unit_axis[1] << ", " << unit_axis[2] << "]" <<
  //   std::endl;
...
bool convex_intersects(const sgnode* n,
                       const std::vector<const sgnode*>& intersectors) {
 ...
 *
 * Result = # Samples that are contained in n1 and n2 / # Samples contained in
 * n1 (where it tries to get the requested number of samples)
 */
...
view_line create_view_line(const std::string& name, const vec3& p1,
                           const vec3& p2) {
...
// Build up a list of view lines that go from the eye point to each vertex in
// the target object view_line.first is a convex_node that actually represents
// the line view_line.second is a bool which is T if that view line is occluded
// by another object
void calc_view_lines(const sgnode* target, const sgnode* eye,
                     view_line_list& view_lines) {
  vec3 eyePos = eye->get_centroid();
  //	std::cout << "EYE: " << eyePos[0] << ", " << eyePos[1] << ", " <<
  // eyePos[2] << std::endl;

  // Create a view_line for the centroid
  // std::string name = "_centroid_line_";
  // view_lines.push_back(create_view_line(name, eyePos,
  // target->get_centroid()));
  ...
  for (c_geom_node_list::const_iterator i = geom_nodes.begin();
       i != geom_nodes.end(); i++) {
    ...
    if (c_node) {
      ...
      for (ptlist::const_iterator i = verts.begin(); i != verts.end(); i++) {
        // std::cout << "Point " << view_lines.size() << ": " << (*i)[0] << ", "
        // << (*i)[1] << ", " << (*i)[2] << std::endl;
...
// Returns the percentage of given view lines are intersected by an object in
// occludingNodes
double convex_occlusion(view_line_list& view_lines,
                        const c_sgnode_list& occluders) {
  ...
  // Go through every other object in the given set and see if it occludes any
  // view lines
  int num_occluded = 0;

  for (view_line_list::iterator i = view_lines.begin(); i != view_lines.end();
       i++) {
    i->second = false;
  }

  for (c_sgnode_list::const_iterator i = occluders.begin();
       i != occluders.end(); i++) {
    const sgnode* n = *i;
    // std::cout << "Testing Object " << n->get_id() << std::endl;
    for (view_line_list::iterator j = view_lines.begin(); j != view_lines.end();
         j++) {
...
double convex_occlusion(const sgnode* a, const sgnode* eye,
                        const std::vector<const sgnode*>& occluders) {
  view_line_list view_lines;
...
vec3 adjust_on_dims(sgnode* n, std::vector<const sgnode*> targets, int d1,
  ...
  // cout << "Adjusting on dims: " << d1 << ", " << d2 << ", " << d3 <<
  // std::endl;
...
vec3 adjust_two_dims(sgnode* n, std::vector<const sgnode*> targets, int dim) {
  return adjust_on_dims(n, targets, (dim + 1) % 3, (dim + 2) % 3,
                        (dim + 2) % 3);
}
...
void adjust_sgnode_size(sgnode* n, std::vector<const sgnode*> targets) {
  ...
  if (convex_intersects(centroid, intersectors)) {
    ...
    //  Something is very wrong, the centroid is inside another object, just
    //  quit
    delete centroid;
    return;
  }
  ...
  // cout << "Old Scale: " << scale[0] << ", " << scale[1] << ", " << scale[2]
  // << std::endl;
  ...
  // cout << "Old Scale: " << scale[0] << ", " << scale[1] << ", " << scale[2]
  // << std::endl; cout << "New Scale: " << newScale[0] << ", " << newScale[1]
  // << ", " << newScale[2] << std::endl;
...
void adjust_sgnode_size(sgnode* n, scene* scn) {
  ...
  for (std::vector<const sgnode*>::const_iterator i = all.begin();
       i != all.end(); i++) {
    if (*i == n) {
      continue;
    }
