...
class filter_val_c : public filter_val  // c for concrete
{
 public:
  ...
  filter_val* clone() const { return new filter_val_c<T>(v); }
...
template <>
void inline filter_val_c<vec3>::get_rep(
...
template <>
void inline filter_val_c<bbox>::get_rep(
    std::map<std::string, std::string>& rep) const {
  rep.clear();
  vec3 min = v.get_min();
  vec3 max = v.get_max();
...
class filter_val_c<sgnode*> : public filter_val {
 public:
  ...
  filter_val* clone() const { return new sgnode_filter_val(v); }
  ...
  sgnode* get_value() const { return v; }
