...
void serialize(const serializable& v, std::ostream& os) { v.serialize(os); }

void unserialize(serializable& v, std::istream& is) { v.unserialize(is); }
...
void serialize(bool b, std::ostream& os) { os << (b ? 't' : 'f'); }
...
void serialize(int v, std::ostream& os) { os << v; }

void serialize(long v, std::ostream& os) { os << v; }

void serialize(size_t v, std::ostream& os) { os << v; }
...
void unserialize(std::string& s, std::istream& is) {
  ...
  while (is.get(c)) {
    if ((quoted && c == '"' && is.get() != '"') || (!quoted && isspace(c))) {
      is.unget();
      break;
    }
    ss << c;
