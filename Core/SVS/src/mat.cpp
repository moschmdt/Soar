...
dyn_mat::dyn_mat(size_t nrows, size_t ncols, size_t init_row_capacity,
                 size_t init_col_capacity)
    : buf(init_row_capacity, init_col_capacity),
      r(nrows),
      c(ncols),
      released(false) {}

dyn_mat::dyn_mat(const_mat_view m) : r(m.rows()), c(m.cols()), released(false) {
  buf = m;
}
...
void serialize(const_mat_view m, std::ostream& os) {
  serializer sr(os);
  sr << std::string("MAT") << static_cast<int>(m.rows())
...
transform3::transform3(const vec3& p, const vec3& r, const vec3& s) {
  trans = Eigen::Translation<double, 3>(p) *
          ...
          Eigen::AngleAxisd(r(0), Eigen::Vector3d::UnitX()) * Eigen::Scaling(s);
}
