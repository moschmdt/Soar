...
struct command_entry {
  ...
  command* cmd;
  wme* cmd_wme;
  command_entry(std::string id, command* cmd, wme* cmd_wme)
      : id(id), cmd(cmd), cmd_wme(cmd_wme) {}
  bool operator<(const command_entry e) const { return id.compare(e.id) < 0; }
};
...
class svs : public svs_interface, public cliproxy {
 public:
  ...
  bool do_cli_command(const std::vector<std::string>& args,
                      std::string& output);

  bool is_enabled() { return enabled; }
  void set_enabled(bool is_enabled) { enabled = is_enabled; }

  bool is_enabled_in_substates() { return enabled_in_substates; }
  ...
  bool is_in_substate() { return state_stack.size() > 1; }
 ...
 private:
  ...
  void proxy_get_children(std::map<std::string, cliproxy*>& c);
  void cli_connect_viewer(const std::vector<std::string>& args,
                          std::ostream& os);
  void cli_disconnect_viewer(const std::vector<std::string>& args,
                             std::ostream& os);
  ...
  bool enabled;
  // when enabled is true but enabled_in_substates is false, only the top state
  // is enabled
