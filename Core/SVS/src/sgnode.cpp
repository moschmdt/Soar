...
void sgnode::send_update(sgnode::change_type t,
...
void sgnode::listen(sgnode_listener* o) { listeners.push_back(o); }

void sgnode::unlisten(sgnode_listener* o) { listeners.remove(o); }
...
group_node* sgnode::as_group() { return dynamic_cast<group_node*>(this); }
...
void sgnode::proxy_use_sub(const std::vector<std::string>& args,
                           std::ostream& os) {
  vec3 lp, ls, wp, ws;
  vec4 lr, wr;
...
void convex_node::update_shape() { set_bounds(bbox(get_world_verts())); }

void convex_node::set_transform_dirty_sub() { world_verts_dirty = true; }
...
const ptlist& convex_node::get_world_verts() const {
  if (world_verts_dirty) {
    ...
    transform(verts.begin(), verts.end(), world_verts.begin(),
              get_world_trans());
...
void convex_node::proxy_use_sub(const std::vector<std::string>& args,
                                std::ostream& os) {
...
void ball_node::proxy_use_sub(const std::vector<std::string>& args,
                              std::ostream& os) {
  sgnode::proxy_use_sub(args, os);
...
const tag_map& sgnode::get_all_tags() const { return tags; }

bool sgnode::get_tag(const std::string& tag_name,
                     std::string& tag_value) const {
  return map_get(tags, tag_name, tag_value);
}

void sgnode::set_tag(const std::string& tag_name,
