...
std::ostream& histogram(const std::vector<double>& vals, size_t nbins,
                        std::ostream& os) {
  assert(nbins > 0);
  ...
  for (i = 0; i < nbins; ++i) {
    os << min + binsize * i << "|";
    os << std::setfill('#') << std::setw((int)(hashes_per * counts[i])) << '/'
       << counts[i] << std::endl;
  }
  os.precision(p);
  os.flags(f);
...
bool is_inf(double x) { return x == std::numeric_limits<double>::infinity(); }

bool approx_equal(double a, double b, double thresh) {
  return (fabs(a - b) <= thresh);
}

table_printer::table_printer() : spacer_width(1) {}
...
void table_printer::set_precision(int p) { ss.precision(p); }
...
void table_printer::set_spacer_width(int w) { spacer_width = w; }
