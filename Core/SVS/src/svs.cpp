...
svs_interface* make_svs(agent* a) { return new svs(a); }
...
void sgwme::node_update(sgnode* n, sgnode::change_type t,
...
void svs_state::clear_scene() { scn->clear(); }
...
svs::svs(agent* a) : scn_cache(NULL), enabled(false) {
...
*/
void svs::add_input(const std::string& in) { split(in, "\n", env_inputs); }
...
void svs::proxy_get_children(std::map<std::string, cliproxy*>& c) {
  ...
  c["connect_viewer"]
      ->set_help("Connect to a running viewer.")
      .add_arg("PORT",
               "TCP port (or file socket path in Linux) to connect to.");

  c["disconnect_viewer"] =
      new memfunc_proxy<svs>(this, &svs::cli_disconnect_viewer);
...
bool svs::do_cli_command(const std::vector<std::string>& args,
                         std::string& output) {
  std::stringstream ss;
...
void svs::cli_connect_viewer(const std::vector<std::string>& args,
                             std::ostream& os) {
  if (args.empty()) {
...
void svs::cli_disconnect_viewer(const std::vector<std::string>& args,
                                std::ostream& os) {
  draw->disconnect();
}
