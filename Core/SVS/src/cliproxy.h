...
class cliproxy {
 public:
  virtual ~cliproxy();
  void proxy_use(const std::string& path, const std::vector<std::string>& args,
                 std::ostream& os);
 ...
 private:
  ...
  virtual void proxy_use_sub(const std::vector<std::string>& args,
                             std::ostream& os) {
    list_children(0, os);
  }
...
class memfunc_proxy : public cliproxy {
 public:
  typedef void (T::*farg_ptr)(const std::vector<std::string>& args,
                              std::ostream& os);
  typedef void (T::*fout_ptr)(std::ostream& os);
  typedef void (T::*fargc_ptr)(const std::vector<std::string>& args,
                               std::ostream& os) const;
  typedef void (T::*foutc_ptr)(std::ostream& os) const;

  memfunc_proxy(T* p, farg_ptr f)
      : p(p), pc(NULL), farg(f), fout(NULL), fargc(NULL), foutc(NULL) {}
  memfunc_proxy(T* p, fout_ptr f)
      : p(p), pc(NULL), farg(NULL), fargc(NULL), fout(f), foutc(NULL) {}
  memfunc_proxy(const T* p, fargc_ptr f)
      : p(NULL), pc(p), farg(NULL), fargc(f), fout(NULL), foutc(NULL) {}
  memfunc_proxy(const T* p, foutc_ptr f)
      : p(NULL), pc(p), farg(NULL), fout(NULL), fargc(NULL), foutc(f) {}
