...
enum GeometryType { BOX_GEOM, SPHERE_GEOM, NONE_GEOM, POINT_GEOM };
...
class add_node_command : public command {
 public:
  ...
  std::string description() { return std::string("add_node"); }
 ...
 private:
  ...
  bool add_node() {
    ...
    for (std::map<char, vec3>::iterator i = transforms.begin();
         i != transforms.end(); i++) {
