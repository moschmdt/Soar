...
class copy_node_command : public command {
 public:
  ...
  std::string description() { return std::string("copy-node"); }
  ...
  bool parse() {
    ...
    copy_tags = false;
    std::string copy_str;
    if (si->get_const_attr(root, "copy_tags", copy_str) && copy_str == "true") {
      copy_tags = true;
    }
    ...
    if (si->find_child_wme(root, "tags", tags_wme)) {
      ...
      if (si->get_child_wmes(tags_root, tag_wmes)) {
        for (std::vector<wme*>::const_iterator tag_it = tag_wmes.begin();
             tag_it != tag_wmes.end(); tag_it++) {
          ...
          if (get_symbol_value(tag_attr, tag_attr_str) &&
              get_symbol_value(tag_value, tag_value_str)) {
  ...
  bool copy_node() {
    ...
    const convex_node* sourceConvex =
        dynamic_cast<const convex_node*>(source_node);
    if (sourceBall) {
    ...
    if (copy_tags) {
      ...
      for (tag_map::const_iterator tag_it = src_tags.begin();
           tag_it != src_tags.end(); tag_it++) {
        dest_node->set_tag(tag_it->first, tag_it->second);
      }
    }

    for (tag_map::const_iterator tag_it = tags.begin(); tag_it != tags.end();
         tag_it++) {
      dest_node->set_tag(tag_it->first, tag_it->second);
    }
...
command_table_entry* copy_node_command_entry() {
  ...
  e->parameters["copy_tags"] =
      "[Optional] - true/false to copy tags from source node";
  e->parameters["tags"] =
      "[Optional] - any att/val pairs underneath added as tags";
