...
class extract_command : public command, public filter_input::listener {
 public:
  ...
  std::string description() { return std::string("extract"); }
  ...
  bool update_sub() {
    ...
    if (changed()) {
      ...
      fltr = parse_filter_spec(state->get_svs()->get_soar_interface(), root,
                               state->get_scene());
      if (!fltr) {
  ...
  void update_results() {
    ...
    for (size_t i = out->first_added(), iend = out->num_current(); i < iend;
         ++i) {
      handle_output(out->get_current(i));
    }
