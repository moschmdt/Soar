...
void cliproxy::proxy_use(const std::string& path,
...
int_proxy::int_proxy(int* p, const std::string& description) : p(p) {
  set_help(description);
  add_arg("[VALUE]", "New value. Must be an integer.");
}

void int_proxy::proxy_use_sub(const std::vector<std::string>& args,
                              std::ostream& os) {
  if (args.empty()) {
    os << *p << std::endl;
    return;
...
float_proxy::float_proxy(double* p, const std::string& description) : p(p) {
  set_help(description);
  add_arg("[VALUE]", "New value. Must be a float.");
}

void float_proxy::proxy_use_sub(const std::vector<std::string>& args,
                                std::ostream& os) {
  if (args.empty()) {
    os << *p << std::endl;
    return;
...
bool_proxy::bool_proxy(bool* p, const std::string& description) : p(p) {
  set_help(description);
  add_arg("[VALUE]", "New value. Must be (0|1|true|false|on|off)");
}

void bool_proxy::proxy_use_sub(const std::vector<std::string>& args,
                               std::ostream& os) {
  if (args.empty()) {
