...
typedef Eigen::Block<mat, Eigen::Dynamic, Eigen::Dynamic, true>
    mat_iblock;  // i means inner_panel template arg = true
...
typedef Eigen::Block<const mat, Eigen::Dynamic, Eigen::Dynamic, true>
    const_mat_iblock;
typedef Eigen::Block<const mat, Eigen::Dynamic, Eigen::Dynamic, false>
    const_mat_block;
...
typedef Eigen::Block<const_mat_map, Eigen::Dynamic, Eigen::Dynamic, true>
    const_map_iblock;
typedef Eigen::Block<const_mat_map, Eigen::Dynamic, Eigen::Dynamic, false>
    const_map_block;
typedef Eigen::Block<const_mat_map, Eigen::Dynamic, 1, false>
    const_map_col_block;
...
class mat_view : public mat_map {
 public:
  mat_view(mat& m)
      : mat_map(m.data(), m.rows(), m.cols(), mat_stride(m.rowStride(), 1)) {}
  mat_view(mat& m, size_t r, size_t c)
      : mat_map(m.data(), r, c, mat_stride(m.rowStride(), 1)) {}
...
class const_mat_view : public const_mat_map {
 public:
  ...
  // standard types
  const_mat_view(const mat& m)
      : const_mat_map(m.data(), m.rows(), m.cols(),
                      mat_stride(m.rowStride(), 1)) {}
  const_mat_view(const rvec& v)
      : const_mat_map(v.data(), 1, v.size(), mat_stride(1, 1)) {}
  const_mat_view(const cvec& v)
      : const_mat_map(v.data(), v.size(), 1, mat_stride(1, 1)) {}
  const_mat_view(const mat& m, size_t r, size_t c)
      : const_mat_map(m.data(), r, c, mat_stride(m.rowStride(), 1)) {}

  // for things like m.block
  const_mat_view(const mat_block& b)
      : const_mat_map(b.data(), b.rows(), b.cols(),
                      mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const mat_iblock& b)
      : const_mat_map(b.data(), b.rows(), b.cols(),
                      mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const const_mat_block& b)
      : const_mat_map(b.data(), b.rows(), b.cols(),
                      mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const const_mat_iblock& b)
      : const_mat_map(b.data(), b.rows(), b.cols(),
                      mat_stride(b.rowStride(), 1)) {}

  // for things like m.row and m.col
  const_mat_view(const row_block& b)
      : const_mat_map(b.data(), 1, b.cols(), mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const const_row_block& b)
      : const_mat_map(b.data(), 1, b.cols(), mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const col_block& b)
      : const_mat_map(b.data(), b.rows(), 1, mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const const_col_block& b)
      : const_mat_map(b.data(), b.rows(), 1, mat_stride(b.rowStride(), 1)) {}

  const_mat_view(const const_mat_map& m) : const_mat_map(m) {}
  const_mat_view(const mat_map& m)
      : const_mat_map(m.data(), m.rows(), m.cols(),
                      mat_stride(m.rowStride(), 1)) {}
  const_mat_view(const map_block& b)
      : const_mat_map(b.data(), b.rows(), b.cols(),
                      mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const const_map_block& b)
      : const_mat_map(b.data(), b.rows(), b.cols(),
                      mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const const_map_col_block& b)
      : const_mat_map(b.data(), b.rows(), 1, mat_stride(b.rowStride(), 1)) {}
  const_mat_view(const const_map_row_block& b)
      : const_mat_map(b.data(), 1, b.cols(), mat_stride(b.rowStride(), 1)) {}
};
...
class dyn_mat : public serializable {
 public:
  ...
  dyn_mat(size_t nrows, size_t ncols, size_t init_row_capacity,
          size_t init_col_capacity);
...
std::ostream& output_rvec(std::ostream& os, const rvec& v,
                          const std::string& sep = " ");
std::ostream& output_cvec(std::ostream& os, const cvec& v,
                          const std::string& sep = " ");
...
class bbox {
 public:
  ...
  vec3 get_centroid() const { return (max_pt + min_pt) / 2.0; }

  const vec3& get_min() const { return min_pt; }

  const vec3& get_max() const { return max_pt; }
  ...
  double get_volume() const {
    return (max_pt[0] - min_pt[0]) * (max_pt[1] - min_pt[1]) *
           (max_pt[2] - min_pt[2]);
  }
...
class transform3 {
 public:
  ...
  vec3 operator()(const vec3& v) const { return trans * v; }
  ...
  void get_matrix(mat& m) const { m = trans.matrix(); }
