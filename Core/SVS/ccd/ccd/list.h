...
#define ccdListForEach(list, item)                                       \
  for (item = (list)->next; _ccd_prefetch((item)->next), item != (list); \
       item = (item)->next)
...
_ccd_inline ccd_list_t *ccdListNext(ccd_list_t *l) { return l->next; }

_ccd_inline ccd_list_t *ccdListPrev(ccd_list_t *l) { return l->prev; }
