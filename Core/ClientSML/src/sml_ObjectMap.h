...
class ObjectMap {
  ...
  // The name should be owned by the object being stored (so we only delete the
  // object, never the name)

  typedef std::map<std::string, DataType> InternalMap;
  // typedef typename std::map<std::string, T>::iterator InternalMapIter ;
  // typedef typename std::map<std::string, T>::const_iterator
  // InternalMapConstIter ;
 ...
 public:
  ...
  void keep(std::list<DataType>* pKeepList) {
    for (InternalMapIter mapIter = m_Map.begin(); mapIter != m_Map.end();) {
      ...
      if (std::find(pKeepList->begin(), pKeepList->end(), pObject) !=
          pKeepList->end()) {
        mapIter++;
      } else {
