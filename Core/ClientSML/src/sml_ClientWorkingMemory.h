...
class EXPORT WorkingMemory {
 ...
 protected:
  ...
  enum ChangeListMode {
    CHANGE_LIST_AUTO_DISABLED = -1,
    CHANGE_LIST_USER_DISABLED = -2
  };
  int m_changeListHandlerId;
  static void ClearHandlerStatic(sml::smlRunEventId id, void* pUserData,
  ...
  WMElement* SearchWmeListForID(WmeList* pWmeList, char const* pID,
                                bool deleteFromList);
  ...
  bool m_Deleting;  // used when we're being deleted and the maps shouldn't be
                    // updated

  // Create a new WME of the appropriate type based on this information.
  WMElement* CreateWME(IdentifierSymbol* pParentSymbol, char const* pID,
                       char const* pAttribute, char const* pValue,
                       char const* pType, long long timeTag);
 ...
 public:
  ...
  void Refresh();
  void InvalidateOutputLink();  // During init-soar, the output-link identifier
                                // could change
