...
const char *DEBUGGER_NAME = "SoarDebugger";
#ifdef _WIN32
const char *DEBUGGER_EXE = "SoarDebugger.exe";
#else
const char *DEBUGGER_EXE = "SoarDebugger";
#endif
...
{
    if (processInformation && processInformation->process)
    {
        PrintDebugFormat("Debugger process information: pid = %d", processInformation->process->id());
    }
}
...
bool debugger_is_running(DebuggerProcessInformation *processInformation)
{
    ...
    return processInformation->process->running();
}
...
// True
// ```
bool Agent::SpawnDebugger(int port, const char *debuggerPath)
{
    // Helper lambda to append path separator if needed
    auto append_path_separator = [](std::string &path)
    {
    ...
    // Helper lambda to build and check path - returns empty string if not found
    auto try_path = [&](const std::string &base_path) -> std::string
    {
        if (base_path.empty())
            return {};

        std::string candidate = base_path;
        append_path_separator(candidate);
        candidate += DEBUGGER_EXE;

        return isfile(candidate.c_str()) ? candidate : std::string{};
    };

    // Find debugger executable path
    std::string found_path;

    // 1. Check explicit path if provided
    if (debuggerPath && *debuggerPath)
    {
        if (!isfile(debuggerPath))
        {
            std::cerr << "SpawnDebugger: provided path '" << debuggerPath
                      << "' is not a file\n";
            return false;
        }
        found_path = debuggerPath;
    }
    else
    {
    ...
    if (m_pDPI)
    {
        ...
        {
            std::cerr << "SpawnDebugger: A debugger process is already running\n";
    ...
    try
    {
        ...
        // Launch the debugger process using Boost.Process
        m_pDPI->process = std::make_unique<bp::child>(
            found_path,
            bp::args(args),
            bp::std_out > bp::null,
            bp::std_err > bp::null);

        std::cout << "SpawnDebugger: Qt debugger launched successfully" << std::endl;
        print_debugger_process_information(m_pDPI);
        return true;
    }
    catch (const std::exception &e)
    {
        std::cerr << "SpawnDebugger: Failed to launch debugger: " << e.what() << std::endl;
        ClearDebuggerProcessInformation();
        return false;
    }
}
...
bool Agent::KillDebugger(bool ignoreNonExistent)
{
    ...
    try
    {
        if (m_pDPI->process->running())
        {
            m_pDPI->process->terminate();
            m_pDPI->process->wait();
            successful = true;
        }
        else
    ...
    {
        std::cerr << "KillDebugger: Failed to terminate debugger: " << e.what() << std::endl;
        successful = false;
    }
