...
class dynamic_library_load_unload_handler {
 public:
  ...
  ~dynamic_library_load_unload_handler() { libraryPath = ""; }
...
void print_debugger_process_information(
    DebuggerProcessInformation* processInformation) {
#ifdef _WIN32
  PrintDebugFormat(
      "Debugger process information: hProcess = %p, hThread = %p, dwProcessId "
      "= %d, dwThreadId = %d",
...
#else   // _WIN32
  PrintDebugFormat("Debugger process information: pid = %d",
                   processInformation->debuggerPid);
#endif  // not _WIN32
}

/**
 * Returns true if a debugger process was previously started via SpawnDebugger
...
bool debugger_is_running(DebuggerProcessInformation* processInformation) {
  ...
  if (GetExitCodeProcess(
          processInformation->debuggerProcessInformation.hProcess, &exitCode)) {
...
Agent::~Agent() { KillDebugger(); }

Connection* Agent::GetConnection() const { return m_Kernel->GetConnection(); }
...
void Agent::ReceivedRunEvent(smlRunEventId id, AnalyzeXML* pIncoming,
  ...
  for (RunEventMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end();) {
...
void Agent::FireOutputNotification() {
  ...
  // Look up the handler(s) from the map
  OutputNotificationMap::ValueList* pHandlers =
      m_OutputNotificationMap.getList(id);
  ...
  // Go through the list of event handlers calling each in turn
  for (OutputNotificationMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end();) {
...
void Agent::ReceivedPrintEvent(smlPrintEventId id, AnalyzeXML* pIncoming,
                               ElementXML* /*pResponse*/) {
  ...
  // Go through the list of event handlers calling each in turn
  for (PrintEventMap::ValueListIter iter = pHandlers->begin();
    ...
    // If this is an echo event triggered by a command issued by ourselves
    // ignore it.
...
void Agent::ReceivedProductionEvent(smlProductionEventId id,
  ...
  // Go through the list of event handlers calling each in turn
  for (ProductionEventMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end(); iter++) {
...
bool Agent::LoadProductions(char const* pFilename, bool echoResults) {
  ...
  if ((pFilename[0] == '\"' && pFilename[len - 1] == '\"') ||
      (pFilename[0] == '{' && pFilename[len - 1] == '}')) {
...
// These are little utility classes we define to help with searching the event
// maps
...
class Agent::TestOutputNotificationCallback
    : public OutputNotificationMap::ValueTest {
 private:
  int m_ID;
...
class Agent::TestOutputNotificationCallbackFull
    : public OutputNotificationMap::ValueTest {
 private:
  int m_EventID;
 ...
 public:
  TestOutputNotificationCallbackFull(int id, OutputNotificationHandler handler,
                                     void* pUserData) {
    m_EventID = id;
    m_Handler = handler;
    m_UserData = pUserData;
...
class Agent::TestOutputCallbackFull : public OutputEventMap::ValueTest {
 ...
 public:
  TestOutputCallbackFull(char const* attributeName, OutputEventHandler handler,
                         void* pUserData) {
...
class Agent::TestProductionCallbackFull : public ProductionEventMap::ValueTest {
 ...
 public:
  TestProductionCallbackFull(int id, ProductionEventHandler handler,
                             void* pUserData) {
    m_EventID = id;
    m_Handler = handler;
    m_UserData = pUserData;
...
int Agent::RegisterForRunEvent(smlRunEventId id, RunEventHandler handler,
  ...
  RunEventHandlerPlusData handlerPlus(id, handler, pUserData,
                                      m_CallbackIDCounter);
...
int Agent::RegisterForOutputNotification(OutputNotificationHandler handler,
                                         void* pUserData, bool addToBack) {
  smlWorkingMemoryEventId id = smlEVENT_OUTPUT_PHASE_CALLBACK;
  ...
  // If we have no handlers registered with the kernel, then we need
  // to register for this event.  No need to do this multiple times.
  // (Only do this if we were ignoring output from the kernel already--otherwise
  // we'll get two copies of everything
  //  because we're already registered for this event)
  if (GetKernel()->m_bIgnoreOutput &&
      m_OutputNotificationMap.getListSize(id) == 0) {
    GetKernel()->RegisterForEventWithKernel(id, GetAgentName());
  }
  ...
  // We use a struct rather than a pointer to a struct, so there's no need to
  // new/delete everything as the objects are added and deleted.
  OutputNotificationHandlerPlusData handlerPlus(id, handler, pUserData,
                                                m_CallbackIDCounter);
...
bool Agent::UnregisterForOutputNotification(int callbackID) {
  ...
  // Find the event ID for this callbackID
  smlWorkingMemoryEventId id = m_OutputNotificationMap.findFirstKeyByTest(
      &test, smlWORKING_MEMORY_EVENT_BAD);
...
int Agent::AddOutputHandler(char const* pAttributeName,
                            OutputEventHandler handler, void* pUserData,
                            bool addToBack) {
  m_WorkingMemory.SetOutputLinkChangeTracking(true);

  // Start by checking if this attributeName, handler, pUSerData combination has
  ...
  // Record the handler
  m_CallbackIDCounter++;
  OutputEventHandlerPlusData handlerPlus(0, pAttributeName, handler, pUserData,
                                         m_CallbackIDCounter);
...
void Agent::ReceivedOutputEvent(WMElement* pWmeAdded) {
  ...
  // Look up the handler(s) from the map
  OutputEventMap::ValueList* pHandlers =
      m_OutputEventMap.getList(pAttributeName);
  ...
  // Go through the list of event handlers calling each in turn
  for (OutputEventMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end();) {
...
int Agent::RegisterForProductionEvent(smlProductionEventId id,
  ...
  // Record the handler
  m_CallbackIDCounter++;
  ProductionEventHandlerPlusData handlerPlus(id, handler, pUserData,
                                             m_CallbackIDCounter);
...
bool Agent::UnregisterForProductionEvent(int callbackID) {
  ...
  // Find the event ID for this callbackID
  smlProductionEventId id =
      m_ProductionEventMap.findFirstKeyByTest(&test, smlPRODUCTION_EVENT_BAD);
...
int Agent::RegisterForPrintEvent(smlPrintEventId id, PrintEventHandler handler,
                                 void* pUserData, bool ignoreOwnEchos,
                                 bool addToBack) {
  // Start by checking if this id, handler, pUSerData combination has already
  // been registered
  ...
  PrintEventHandlerPlusData handlerPlus(id, handler, pUserData, ignoreOwnEchos,
                                        m_CallbackIDCounter);
...
bool Agent::UnregisterForPrintEvent(int callbackID) {
  ...
  // Find the event ID for this callbackID
  smlPrintEventId id =
      m_PrintEventMap.findFirstKeyByTest(&test, smlPRINT_EVENT_BAD);
...
void Agent::ReceivedXMLEvent(smlXMLEventId id, AnalyzeXML* pIncoming,
                             ElementXML* /*pResponse*/) {
  ...
  // NOTE: This object needs to stay in scope for as long as we're calling
  // clients and then when it is deleted it will delete pXMLMessage.
  ...
  // Go through the list of event handlers calling each in turn
  for (XMLEventMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end(); iter++) {
...
void Agent::ReceivedXMLTraceEvent(smlXMLEventId id, ElementXML* pIncoming,
                                  ElementXML* /*pResponse*/) {
#ifdef _DEBUG
  ...
  // look for the correct parts of the original message.  This makes these
  // messages a bit more brittle than the rest of the system but speed really
  // counts on these messages and this saves the analysis step (which is
  // significant). If this assert fails we've changed the structure of the XML
  // event messages and we'll need to update the code to match.
  ...
  // NOTE: This object needs to stay in scope for as long as we're calling
  // clients and then when it is deleted it will delete pTrace.
  ...
  for (XMLEventMap::ValueListIter iter = pHandlers->begin();
    ...
    // the callback deletes the current handler from the list, invalidating the
    // iterator.
    iter++;
...
int Agent::RegisterForXMLEvent(smlXMLEventId id, XMLEventHandler handler,
                               void* pUserData, bool addToBack) {
  // Start by checking if this id, handler, pUSerData combination has already
  // been registered
  ...
  // If we have no handlers registered with the kernel, then we need
  // to register for the print event (which we'll then parse to create XML
  // objects). No need to do this multiple times.
  ...
  XMLEventHandlerPlusData handlerPlus(id, handler, pUserData,
                                      m_CallbackIDCounter);
...
Identifier* Agent::GetInputLink() { return GetWM()->GetInputLink(); }

Identifier* Agent::GetOutputLink() { return GetWM()->GetOutputLink(); }
...
void Agent::ClearOutputLinkChanges() { GetWM()->ClearOutputLinkChanges(); }
...
int Agent::GetNumberCommands() {
  ...
  for (Identifier::ChildrenIter iter = pOutputLink->GetChildrenBegin();
       iter != pOutputLink->GetChildrenEnd(); iter++) {
    WMElement* pWME = *iter;
...
StringElement* Agent::CreateStringWME(Identifier* parent,
...
Identifier* Agent::CreateSharedIdWME(Identifier* parent, char const* pAttribute,
...
IntElement* Agent::CreateIntWME(Identifier* parent, char const* pAttribute,
                                long long value) {
  if (!parent || parent->GetAgent() != this) {
    return NULL;
  }
...
FloatElement* Agent::CreateFloatWME(Identifier* parent, char const* pAttribute,
                                    double value) {
  if (!parent || parent->GetAgent() != this) {
    return NULL;
  }
...
bool Agent::Commit() { return GetWM()->Commit(); }

bool Agent::IsCommitRequired() { return GetWM()->IsCommitRequired(); }

bool Agent::IsAutoCommitEnabled() { return m_Kernel->IsAutoCommitEnabled(); }
...
char const* Agent::InitSoar() {
  ...
  // we get that, we'll queue up the input link information to be sent over
  // again and also erase our output link information. (See the InitSoarHandler
  // in ClientKernel.cpp)
...
bool Agent::WasAgentOnRunList() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_WasAgentOnRunList, GetAgentName());
...
smlRunResult Agent::GetResultOfLastRun() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_GetResultOfLastRun, GetAgentName());
...
bool Agent::SetStopSelfOnOutput(bool state)
{
    ...
    bool ok = GetConnection()->SendAgentCommand(&response,
sml_Names::kCommand_StopOnOutput, GetAgentName(), sml_Names::kParamValue, state
? sml_Names::kTrue : sml_Names::kFalse) ; return ok ;
}
*/
...
char const* Agent::RunSelfTilOutput() {
  ...
  // For now, maxDecisions is being ignored.  We should make this a separate
  // call to set this parameter.
...
void Agent::Refresh() {
  ...
  // as all working memory changes should be committed before other user-input
  // (e.g. init-soar) can be called.
...
smlPhase Agent::GetCurrentPhase() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_GetRunState, GetAgentName(),
      sml_Names::kParamValue, sml_Names::kParamPhase);
...
int Agent::GetDecisionCycleCounter() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_GetRunState, GetAgentName(),
      sml_Names::kParamValue, sml_Names::kParamDecision);
...
smlRunState Agent::GetRunState() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_GetRunState, GetAgentName(),
      sml_Names::kParamValue, sml_Names::kParamRunState);
...
char const* Agent::ExecuteCommandLine(char const* pCommandLine,
                                      bool echoResults, bool noFilter) {
  return GetKernel()->ExecuteCommandLine(pCommandLine, GetAgentName(),
                                         echoResults, noFilter);
}

bool Agent::ExecuteCommandLineXML(char const* pCommandLine,
                                  ClientAnalyzedXML* pResponse) {
  return GetKernel()->ExecuteCommandLineXML(pCommandLine, GetAgentName(),
                                            pResponse);
}
...
bool Agent::IsProductionLoaded(char const* pProductionName) {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_IsProductionLoaded, GetAgentName(),
      sml_Names::kParamName, pProductionName);
...
bool Agent::SynchronizeInputLink() { return GetWM()->SynchronizeInputLink(); }

bool Agent::SynchronizeOutputLink() { return GetWM()->SynchronizeOutputLink(); }
...
// There's no great way to unit-test this, but testing from Python you should be
// able to see the debugger open and return values given as below:
// ```python
...
bool Agent::SpawnDebugger(int port, const char* jarpath) {
  ...
  if (jarpath) {
    if (!isfile(jarpath)) {
      std::cerr << "SpawnDebugger: jarparth '" << jarpath << "'is not a file"
                << std::endl;
      return false;
    }
    p = jarpath;
  ...
  if (p.length() == 0) {
    ...
    if (!GetCurrentDirectory(4096, buffer)) {
      std::cerr << "SpawnDebugger: GetCurrentDirectory failed: "
    ...
    if (isfile(debuggerPath.c_str())) {
      ...
      std::cerr << "SpawnDebugger: Calculated debugger path '" << debuggerPath
  ...
  if (m_pDPI) {
    if (!debugger_is_running(m_pDPI)) {
      ...
      std::cerr << "SpawnDebugger: Previously-spawned debugger process is "
                   "still running"
                << std::endl;
  ...
  if (!CreatePipe(&pipes[ParentWrite], &pipes[ChildRead], &sa, 0)) {
    std::cerr << "SpawnDebugger: CreatePipe (parent write, child read) failed: "
              << GetLastError() << std::endl;
    return false;
  }
  if (!CreatePipe(&pipes[ParentRead], &pipes[ChildWrite], &sa, 0)) {
    std::cerr << "SpawnDebugger: CreatePipe (parent read, child write) failed: "
              << GetLastError() << std::endl;
    return false;
  }
  ...
  ZeroMemory(&m_pDPI->debuggerProcessInformation,
             sizeof(m_pDPI->debuggerProcessInformation));
  ...
  commandLine << "java.exe -Djava.library.path=\"" << path << "\" -jar \"" << p
              << "\" -remote -port " << port << " -agent \""
              << this->GetAgentName() << "\"";
  ...
  BOOL ret = CreateProcess(
      ...
      &m_pDPI->debuggerProcessInformation);  // Pointer to PROCESS_INFORMATION
                                             // structure
  ...
  if (ret == 0) {
    std::cout << "SpawnDebugger: CreateProcess failed: " << GetLastError()
              << std::endl;
    ClearDebuggerProcessInformation();
    return false;
  }
  ...
  while (wait) {
    auto success = ReadFile(pipes[ParentRead], ReadBuff, sizeof(ReadBuff) - 1,
                            &ReadNum, NULL);
    if (!success || !ReadNum) {
      break;
    }
...
#ifdef __APPLE__
    execlp("java", "java", ("-Djava.library.path=\"" + path + "\"").c_str(),
           "-cp", ("\"" + path + "\"").c_str(), "-XstartOnFirstThread", "-jar",
           p.c_str(), "-remote", "-port", portstring.c_str(), "-agent",
           this->GetAgentName(), NULL);
#else
    execlp("java", "java", ("-Djava.library.path=" + path).c_str(), "-jar",
           p.c_str(), "-remote", "-port", portstring.c_str(), "-agent",
           this->GetAgentName(), NULL);
#endif
...
bool Agent::KillDebugger() {
  if (!m_pDPI) {
    std::cerr << "KillDebugger: No existing debugger process information"
              << std::endl;
    return false;
  }
  ...
  if (ret) {
    ...
    std::cerr << "KillDebugger: TerminateProcess failed: " << GetLastError()
              << std::endl;
  }
...
char const* Agent::ConvertIdentifier(char const* pClientIdentifier) {
  ...
  bool ret = m_Kernel->GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_ConvertIdentifier, GetAgentName(),
      sml_Names::kParamName, pClientIdentifier);
