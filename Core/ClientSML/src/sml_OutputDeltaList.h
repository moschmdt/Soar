...
class EXPORT OutputDeltaList {
 ...
 public:
  ...
  ~OutputDeltaList() { Clear(true); }

  void Clear(bool deleteContents, bool clearJustAdded = false,
             bool clearChildrenModified = false) {
    // std::cout << "Clearing delta list " << deleteContents << clearJustAdded
    // << clearChildrenModified;
    ...
    if (clearJustAdded || clearChildrenModified) {
      for (std::vector<WMDelta*>::iterator iter = m_DeltaList.begin();
           iter != m_DeltaList.end(); ++iter) {
