...
// Two identifiers (two wmes) can have the same value because this is a
// graph not a tree so we need to represent that separately.
class EXPORT IdentifierSymbol {
  friend class Identifier;  // Provide direct access to children.

 protected:
  // The value for this id, which is a string identifier (e.g. I3) We'll
  // use upper case for Soar IDs and lower case for client IDs (sometimes
  // the client has to generate these before they are assigned by the
  // kernel)
  std::string m_Symbol;
  ...
  // This is true if the list of children of this identifier was changed.
 ...
 public:
  ...
  // Have this identifier take ownership of this WME. So when the identifier is
  // deleted it will delete the WME.
...
class EXPORT Identifier : public WMElement {
 ...
 protected:
  ...
  // So each identifier has a pointer to a symbol object, but two could share
  // the same object.
 ...
 public:
   ...
   * @param pAttribute
   * @return Identifier*
   * @throws std::logic_error when child is not an identifier
   */
  Identifier *GetChildIdentifier(char const *pAttribute) const {
    WMElement *pWME = FindByAttribute(pAttribute, 0);
    if (!pWME->IsIdentifier()) {
      std::string err_msg = std::string(pAttribute) + " is no identifier of " +
                            std::string(this->GetAttribute());
      throw std::logic_error(err_msg);
    }

    return pWME->ConvertToIdentifier();
  }

  double GetFloatChildValue(char const *pAttribute) const {
    return FindByAttribute(pAttribute, 0)->ConvertToFloatElement()->GetValue();
  }

  int GetIntChildValue(char const *pAttribute) const {
    return FindByAttribute(pAttribute, 0)->ConvertToIntElement()->GetValue();
  }

  std::string GetStringChildValue(char const *pAttribute) const {
    return std::string(
        FindByAttribute(pAttribute, 0)->ConvertToStringElement()->GetValue());
  }

  // IDEA: Provice tempalte fucntion: GetValue<FloatElement>("floatValue")
  // template <typename T>
  // T GetChildValue(const char * pAttribute)const
  // {
  //     auto pWMe = FindByAttribute(pAttribute, 0);
  //     pWMe->GetValueType()
  // }
  ...
  Identifier *CreateSharedIdWME(char const *pAttribute,
                                Identifier *pSharedValue);

 protected:
  // This version is only needed at the top of the tree (e.g. the input link)
  Identifier(Agent *pAgent, char const *pAttributeName, char const *pIdentifier,
             long long timeTag);

  // The normal case (where there is a parent id)
  Identifier(Agent *pAgent, Identifier *pParent, char const *pID,
             char const *pAttributeName, char const *pIdentifier,
             long long timeTag);
  Identifier(Agent *pAgent, IdentifierSymbol *pParentSymbol, char const *pID,
             char const *pAttributeName, char const *pIdentifier,
             long long timeTag);

  // The shared id case (where there is a parent id and value is an identifier
  // that already exists)
  Identifier(Agent *pAgent, Identifier *pParent, char const *pID,
             char const *pAttributeName, Identifier *pLinkedIdentifier,
             long long timeTag);
  Identifier(Agent *pAgent, IdentifierSymbol *pParentSymbol, char const *pID,
             char const *pAttributeName,
             IdentifierSymbol *pLinkedIdentifierSymbol, long long timeTag);
