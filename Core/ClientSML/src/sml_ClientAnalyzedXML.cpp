...
ClientAnalyzedXML::ClientAnalyzedXML() { m_pAnalyzeXML = new AnalyzeXML(); }

ClientAnalyzedXML::~ClientAnalyzedXML() { delete m_pAnalyzeXML; }
...
// Each of these either returns a reference to the tag or NULL (if this document
// doesn't contain that tag)
...
// Returns the name of the command (or NULL if no command tag or no name in that
// tag)
...
// Returns the character data from the result tag as an int (or default value if
// there is no character data in the tag)
...
// DeleteString method
char* ClientAnalyzedXML::GenerateXMLString(bool includeChildren,
...
long long ClientAnalyzedXML::GetArgInt(char const* pArgName,
