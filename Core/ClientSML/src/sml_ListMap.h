...
class ListMap {
 public:
  ...
  //  without having to rebuild the complete ValueType.  In practice this is
  //  helpful).
  class ValueTest {
   public:
    virtual ~ValueTest() {}
 ...
 public:
  ...
  virtual ~ListMap() { clear(); }

  void clear() {
    // Delete the contents of the map
    for (KeyMapIter mapIter = m_Map.begin(); mapIter != m_Map.end();
         ++mapIter) {
  ...
  // The way we identify which one(s) to remove is by passing in an object which
  // implements "isEqual". When it's true we remove the object.  Returns true if
  ...
  // Search all values to find a matching value and return the key for that
  // value. The "match" is based on implementing "isEqual" in a ValueTest class.
  // The first match is returned.
  // If there is no match, returns "notFoundKey" that you pass in (so you can
  // choose an appropriate value that's not a key)
  ...
  int getSize() { return (int)m_Map.size(); }
};
