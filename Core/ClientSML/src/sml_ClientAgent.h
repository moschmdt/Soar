...
class RunEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  RunEventHandlerPlusData(int eventID, RunEventHandler handler, void* userData,
                          int callbackID)
...
class ProductionEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  ProductionEventHandlerPlusData(int eventID, ProductionEventHandler handler,
...
class PrintEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  PrintEventHandlerPlusData() { m_IgnoreOwnEchos = true; }

  PrintEventHandlerPlusData(int eventID, PrintEventHandler handler,
                            void* userData, bool ignoreOwnEchos, int callbackID)
      : EventHandlerPlusData(eventID, userData, callbackID) {
    m_Handler = handler;
...
class XMLEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  XMLEventHandlerPlusData(int eventID, XMLEventHandler handler, void* userData,
                          int callbackID)
      : EventHandlerPlusData(eventID, userData, callbackID) {
    m_Handler = handler;
  }
...
class OutputEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  OutputEventHandlerPlusData(int eventID, char const* pAttributeName,
                             OutputEventHandler handler, void* userData,
                             int callbackID)
      : EventHandlerPlusData(eventID, userData, callbackID) {
    m_Handler = handler;
...
class OutputNotificationHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  OutputNotificationHandlerPlusData(int eventID,
...
class EXPORT Agent : public ClientErrors {
 ...
 protected:
  ...
  typedef sml::ListMap<smlProductionEventId, ProductionEventHandlerPlusData>
      ProductionEventMap;
  typedef sml::ListMap<smlPrintEventId, PrintEventHandlerPlusData>
      PrintEventMap;
  ...
  typedef sml::ListMap<smlWorkingMemoryEventId,
                       OutputNotificationHandlerPlusData>
      OutputNotificationMap;
 ...
 protected:
  ...
  // These are little utility classes we define in the .cpp file to help with
  // searching the event maps
  ...
  // When true, if a wme is updated to the same value as before we "blink" the
  // wme by removing the old wme and adding a new one, causing rules to rematch
  // in Soar.
 ...
 protected:
  ...
  // Client should just delete the kernel object (or call Kernel::DestroyAgent()
  // if just want to destroy this agent).
  virtual ~Agent();
   ...
   *
   * @param pIncoming  The output command (list of wmes added/removed from
   *output link)
  ...
  void ReceivedRunEvent(smlRunEventId id, AnalyzeXML* pIncoming,
                        soarxml::ElementXML* pResponse);
  void ReceivedProductionEvent(smlProductionEventId id, AnalyzeXML* pIncoming,
                               soarxml::ElementXML* pResponse);
  void ReceivedPrintEvent(smlPrintEventId id, AnalyzeXML* pIncoming,
                          soarxml::ElementXML* pResponse);
 ...
 public:
   ...
   *        deleted too (assuming it's the only parent -- i.e. part of a tree
   *not a full graph).
   *
   ...
   *attribute is added to the output link the handler you have registered for
   *that name is called.
   * @param pAttributeName The attribute which will trigger this callback
   *("move" in the example).
   ...
   *can examine the current contents of the output link (GetOutputLink() etc.)
   *and it will be up to date.
   */
  int RegisterForOutputNotification(OutputNotificationHandler handler,
                                    void* pUserData, bool addToBack = true);
   ...
   *later)
   */
  int RegisterForProductionEvent(smlProductionEventId id,
   ...
   * @param pUserData      Arbitrary data that will be passed back to the
   *handler function when the event happens.
   * @param ignoreOwnEchos If true and registering for echo event, commands
   *issued through this connection won't echo.  If false, echos all commands.
   *Ignored for non-echo events.
   * @param addToBack      If true add this handler is called after existing
   *handlers.  If false, called before existing handlers.
   *
   * Current set is:
   ...
   *later)
   */
  int RegisterForPrintEvent(smlPrintEventId id, PrintEventHandler handler,
   ...
   *         b) Call "Run(n)".  If the target attribute is added the output
   * function is called. Method 1 is the closest to the original SGIO and should
   * be sufficient in almost all cases.  However, Methods 2 & 3 provide complete
   * access to the output-link, while Method 1 only allows access to
   * top level wmes with this format: (I1 ^output-link I3) (I3 ^move M3) (M3
   * ^position 10). i.e. All commands are added as identifiers at the top level.
   * Method 4 is closest to the gSKI model for I/O and is more event driven but
   * like Method 1 assumes that commands are added by name to the top level and
   * that the list of commands is known in advance (usually not a problem).
   */
  ...
  */
   ...
   *stop soar. If so, call to this method inside that handler (this ensures
   *you're calling on the same thread that Soar is running on so you don't get
   *blocked).
   *
   ...
   *        There should be no reason for the client to call this method
   *directly.
   */
  void Refresh();
   ...
   */
  bool ExecuteCommandLineXML(char const* pCommandLine,
   ...
   *
   *        NOTE: This is the reverse of how a client normally uses the input
   *link but can be useful for tools that wish to debug or monitor changes in
   *the input link.
   *
   ...
   *with the agent. It's here in case a client connects to an existing kernel
   *and agent and wants to get up to date on the current state of the output
   *link.
   */
