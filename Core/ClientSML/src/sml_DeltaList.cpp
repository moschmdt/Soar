...
void DeltaList::RemoveWME(long long timeTag) {
  // BADBAD: We should scan the existing list of tags and if we are adding this
  // value just delete that tag and don't add anything to the delta list. (This
  // will happen if we change a value twice within a commit cycle). We probably
  // shouldn't do this if the object being removed is an identifier as we might
  // leave pending adds that are children of the object. (Then again, that might
  // be ok as presumably those adds would fail when we
