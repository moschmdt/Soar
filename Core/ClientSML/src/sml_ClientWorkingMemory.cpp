...
void WorkingMemory::SetOutputLinkChangeTracking(bool setting) {
  ...
  if (IsTrackingOutputLinkChanges()) {
    ...
    // turning on
    m_changeListHandlerId = GetAgent()->RegisterForRunEvent(
        smlEVENT_AFTER_APPLY_PHASE, ClearHandlerStatic, this);
  }
}

void WorkingMemory::ClearHandlerStatic(sml::smlRunEventId /*id*/,
...
WMElement* WorkingMemory::SearchWmeListForID(WmeList* pWmeList, char const* pID,
                                             bool deleteFromList) {
...
WMElement* WorkingMemory::CreateWME(IdentifierSymbol* pParentSymbol,
                                    char const* pID, char const* pAttribute,
                                    char const* pValue, char const* pType,
                                    long long timeTag) {
  // Value is an identifier
  if (strcmp(pType, sml_Names::kTypeID) == 0) {
    IdentifierSymbol* pSharedIdentifierSymbol =
        this->FindIdentifierSymbol(pValue);
    Identifier* pNewIdentifier = 0;
    if (pSharedIdentifierSymbol != NULL) {
      pNewIdentifier =
          new Identifier(GetAgent(), pParentSymbol, pID, pAttribute,
                         pSharedIdentifierSymbol, timeTag);
    } else {
      pNewIdentifier = new Identifier(GetAgent(), pParentSymbol, pID,
                                      pAttribute, pValue, timeTag);
    }
  ...
  if (strcmp(pType, sml_Names::kTypeString) == 0) {
    return new StringElement(GetAgent(), pParentSymbol, pID, pAttribute, pValue,
                             timeTag);
  }
  ...
  if (strcmp(pType, sml_Names::kTypeInt) == 0) {
    ...
    return new IntElement(GetAgent(), pParentSymbol, pID, pAttribute, value,
                          timeTag);
  }
  ...
  if (strcmp(pType, sml_Names::kTypeDouble) == 0) {
    double value = 0;
    from_c_string(value, pValue);
    return new FloatElement(GetAgent(), pParentSymbol, pID, pAttribute, value,
                            timeTag);
  }
...
void WorkingMemory::RecordDeletion(WMElement* pWME) {
  ...
  // If we're not tracking changes, this list is cleared after all deletions are
  // recorded in ReceivedOutput
...
void WorkingMemory::ClearOutputLinkChanges() {
  ...
  //  Call this even if m_TrackingOutputLinkChanges false in case there was
  //  something on it before false
...
bool WorkingMemory::ReceivedOutputAddition(ElementXML* pWmeXML, bool tracing) {
  ...
  char const* pID = pWmeXML->GetAttribute(
      sml_Names::kWME_Id);  // These IDs will be kernel side ids (e.g. "I3" not
                            // "i3")
  ...
  char const* pType = pWmeXML->GetAttribute(
      sml_Names::kWME_ValueType);  // Can be NULL (=> string)
  char const* pTimeTag = pWmeXML->GetAttribute(
      sml_Names::kWME_TimeTag);  // These will be kernel side time tags (e.g. +5
                                 // not -7)
  ...
  if (tracing) {
    sml::PrintDebugFormat("Received output wme: %s ^%s %s (time tag %s)", pID,
                          pAttribute, pValue, pTimeTag);
  }
  ...
  if (pParentSymbol) {
    ...
    if (!pAddWme) {
      ...
      // our tree of objects.
      pAddWme =
          CreateWME(pParentSymbol, pID, pAttribute, pValue, pType, timeTag);
      if (pAddWme) {
        pParentSymbol->AddChild(pAddWme);

        // Make a record that this wme was added so we can alert the client to
        // this change.
        RecordAddition(pAddWme);
      } else {
        sml::PrintDebugFormat(
            "Unable to create an output wme -- type was not recognized");
        GetAgent()->SetDetailedError(
            Error::kOutputError,
            "Unable to create an output wme -- type was not recognized");
      }
    } else {
    ...
    // See if this is the output-link itself (we want to keep a handle to that
    // specially)
    if (!m_OutputLink &&
        IsStringEqualIgnoreCase(pAttribute, sml_Names::kOutputLinkName)) {
      m_OutputLink = new Identifier(GetAgent(), "output-link", pValue, timeTag);

    } else if (m_OutputLink &&
               (IsStringEqual(m_OutputLink->GetValueAsString(), pValue) &&
                IsStringEqualIgnoreCase(pAttribute,
                                        sml_Names::kOutputLinkName))) {
      // Adding output link again but we already have it so ignored
    } else {
      // If we reach here we've received output which is out of order (e.g. (Y
      // ^att value) before (X ^att Y)) so there's no parent to connect it to.
      // We'll create the wme, keep it on a special list of orphans and try to
      // reconnect it later.
      pAddWme = CreateWME(NULL, pID, pAttribute, pValue, pType, timeTag);

      if (tracing) {
        sml::PrintDebugFormat(
            "Received output wme (orphaned): %s ^%s %s (time tag %s)", pID,
            pAttribute, pValue, pTimeTag);
      }
  ...
  // and we get in a new wme that is creating an identifier see if they match
  // up.
...
bool WorkingMemory::TryToAttachOrphanedChildren(Identifier* pPossibleParent) {
  ...
  WMElement* pWme = SearchWmeListForID(
      &m_OutputOrphans, pPossibleParent->GetValueAsString(), deleteFromList);
  ...
  while (pWme) {
    ...
    if (this->GetAgent()->GetKernel()->IsTracingCommunications()) {
      sml::PrintDebugFormat(
          "Adding orphaned child to this ID: %s ^%s %s (time tag %d)",
          pWme->GetIdentifierName(), pWme->GetAttribute(),
          pWme->GetValueAsString(), static_cast<int64_t>(pWme->GetTimeTag()));
    }

    // If the wme being attached is itself an identifier, we have to check in
    // turn to see if it has any orphaned children
    if (pWme->IsIdentifier()) {
      TryToAttachOrphanedChildren(pWme->ConvertToIdentifier());
    }

    // Make a record that this wme was added so we can alert the client to this
    // change.
    RecordAddition(pWme);

    pWme = SearchWmeListForID(
        &m_OutputOrphans, pPossibleParent->GetValueAsString(), deleteFromList);
  }
...
bool WorkingMemory::ReceivedOutputRemoval(ElementXML* pWmeXML, bool tracing) {
  ...
  char const* pTimeTag = pWmeXML->GetAttribute(
      sml_Names::kWME_TimeTag);  // These will usually be kernel side time tags
                                 // (e.g. +5 not -7)
  ...
  // This may fail as we may have removed the parent of this WME already in the
  // series of remove commands.
  // WMElement* pWME = m_OutputLink->FindFromTimeTag(timeTag) ;
  TimeTagWMEMapIter pWMEIter = m_TimeTagWMEMap.find(timeTag);

  // Delete the WME
  if (pWMEIter != m_TimeTagWMEMap.end() && pWMEIter->second &&
      pWMEIter->second->GetParent()) {
    if (tracing) {
    ...
    // Make a record that this wme was removed, so we can tell the client about
    // it. This recording will also involve deleting the wme.
    RecordDeletion(pWMEIter->second);
  } else {
    if (tracing) {
      sml::PrintDebugFormat(
          "Remove output wme request (seems to already be gone): time tag %s",
          pTimeTag);
    }
    return false;
  }
...
bool WorkingMemory::ReceivedOutput(AnalyzeXML* pIncoming,
  ...
  // (this is rare but can happen if the kernel generates wmes in an unusual
  // order)
  ...
  if (!m_OutputOrphans.empty()) {
    ...
    if (tracing) {
      sml::PrintDebugFormat(
          "Some output WMEs have no matching parent IDs -- they are ophans.  "
          "This is bad.");
    }

    GetAgent()->SetDetailedError(Error::kOutputError,
                                 "Some output WMEs have no matching parent IDs "
                                 "-- they are ophans.  This is bad.");
    m_OutputOrphans.clear();  // Have to discard them.
  }

  if (IsTrackingOutputLinkChanges()) {
    // Let anyone listening for the output notification know that output was
    // just received.
    GetAgent()->FireOutputNotification();

    // Call any handlers registered to listen for output
    // (This is one way to retrieve output).
    // We do this at the end of the output handler so that all of the children
    // of the wme have been received and recorded before we call the handler.
    if (GetAgent()->IsRegisteredForOutputEvent() && m_OutputLink) {
      ...
      for (int i = 0; i < nWmes; i++) {
        ...
        if (pWme->GetParent() == NULL ||
            pWme->GetParent()->GetIdentifierSymbol() == NULL) {
          continue;
        }

        // We're only looking for top-level wmes on the output link so check if
        // our identifier's symbol matches the output link's value
        if (strcmp(pWme->GetParent()->GetIdentifierSymbol(),
                   m_OutputLink->GetValueAsString()) == 0) {
    ...
    // ReceivedOutputRemoval above doesn't actually delete the wmes because of
    // possible ordering issues, instead caching them on the change list (which
    // is not being used for other output-link changes. Deleting the wmes here
    // is safer.
    ClearOutputLinkChanges();
  }
...
void WorkingMemory::SetAgent(Agent* agent) {
  ...
  if (GetConnection()->IsDirectConnection()) {
    EmbeddedConnection* pConnection =
        static_cast<EmbeddedConnection*>(GetConnection());
...
Identifier* WorkingMemory::GetInputLink() {
  if (!m_InputLink) {
    ...
    if (GetConnection()->SendAgentCommand(
            &response, sml_Names::kCommand_GetInputLink, GetAgentName())) {
      m_InputLink =
          new Identifier(GetAgent(), "input-link", response.GetResultString(),
                         GenerateTimeTag());
    }
  }
 ...
 *        but can be useful for tools that wish to debug or monitor changes in
 *the input link.
 *
...
bool WorkingMemory::SynchronizeInputLink() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_GetAllInput, GetAgentName());
  ...
  for (int i = 0; i < nChildren; i++) {
    ...
    char const* pID = pWmeXML->GetAttribute(
        sml_Names::kWME_Id);  // These IDs will be kernel side ids (e.g. "I3"
                              // not "i3")
    char const* pAttribute = pWmeXML->GetAttribute(sml_Names::kWME_Attribute);
    char const* pValue = pWmeXML->GetAttribute(sml_Names::kWME_Value);
    char const* pType = pWmeXML->GetAttribute(
        sml_Names::kWME_ValueType);  // Can be NULL (=> string)
    char const* pTimeTag = pWmeXML->GetAttribute(
        sml_Names::kWME_TimeTag);  // These will be kernel side time tags (e.g.
                                   // +5 not -7)
    ...
    if (tracing) {
      sml::PrintDebugFormat("Received input wme: %s ^%s %s (time tag %s)", pID,
                            pAttribute, pValue, pTimeTag);
    }
    ...
    if (pParentSymbol) {
      ...
      if (pAddWme) {
        ...
        sml::PrintDebugFormat(
            "Unable to create an input wme -- type was not recognized");
        GetAgent()->SetDetailedError(
            Error::kOutputError,
            "Unable to create an input wme -- type was not recognized");
      }
      ...
      if (tracing) {
        sml::PrintDebugFormat(
            "Received input wme (orphaned): %s ^%s %s (time tag %s)", pID,
            pAttribute, pValue, pTimeTag);
      }
    }
  }
 ...
 *
 *        NOTE: Calling this method shouldn't generally be necessary as the
...
bool WorkingMemory::SynchronizeOutputLink() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &incoming, sml_Names::kCommand_GetAllOutput, GetAgentName());
  ...
  // Process the new list of output -- as if it had just occurred in the agent
  // (when in fact we're just synching with it)
...
StringElement* WorkingMemory::CreateStringWME(Identifier* parent,
  ...
  StringElement* pWME =
      new StringElement(GetAgent(), parent, parent->GetValueAsString(),
                        pAttribute, pValue, GenerateTimeTag());
  ...
  if (GetConnection()->IsDirectConnection()) {
    EmbeddedConnection* pConnection =
        static_cast<EmbeddedConnection*>(GetConnection());
    pConnection->DirectAddWME_String(m_AgentSMLHandle,
                                     parent->GetValueAsString(), pAttribute,
                                     pValue, pWME->GetTimeTag());
  ...
  // Commit immediately if we're configured that way (makes life simpler for the
  // client)
...
IntElement* WorkingMemory::CreateIntWME(Identifier* parent,
  ...
  IntElement* pWME =
      new IntElement(GetAgent(), parent, parent->GetValueAsString(), pAttribute,
                     value, GenerateTimeTag());
  ...
  if (GetConnection()->IsDirectConnection()) {
    EmbeddedConnection* pConnection =
        static_cast<EmbeddedConnection*>(GetConnection());
    pConnection->DirectAddWME_Int(m_AgentSMLHandle, parent->GetValueAsString(),
                                  pAttribute, value, pWME->GetTimeTag());
...
FloatElement* WorkingMemory::CreateFloatWME(Identifier* parent,
  ...
  FloatElement* pWME =
      new FloatElement(GetAgent(), parent, parent->GetValueAsString(),
                       pAttribute, value, GenerateTimeTag());
  ...
  if (GetConnection()->IsDirectConnection()) {
    EmbeddedConnection* pConnection =
        static_cast<EmbeddedConnection*>(GetConnection());
    pConnection->DirectAddWME_Double(m_AgentSMLHandle,
                                     parent->GetValueAsString(), pAttribute,
                                     value, pWME->GetTimeTag());
...
void WorkingMemory::UpdateString(StringElement* pWME, char const* pValue) {
  ...
  // If the value hasn't changed and we're set to not blink the wme (remove/add
  // it again) then there's no work to do.
  ...
  // Change the value and the time tag (this is equivalent to us deleting the
  // old object and then creating a new one).
  ...
  if (GetConnection()->IsDirectConnection()) {
    ...
    pConnection->DirectAddWME_String(
        m_AgentSMLHandle, pWME->GetIdentifierName(), pWME->GetAttribute(),
        pValue, pWME->GetTimeTag());
...
void WorkingMemory::UpdateInt(IntElement* pWME, long long value) {
  ...
  if (GetConnection()->IsDirectConnection()) {
    ...
    pConnection->DirectAddWME_Int(m_AgentSMLHandle, pWME->GetIdentifierName(),
                                  pWME->GetAttribute(), value,
                                  pWME->GetTimeTag());
...
void WorkingMemory::UpdateFloat(FloatElement* pWME, double value) {
  ...
  // it again) then there's no work to do.
  if (!m_Agent->IsBlinkIfNoChange()) {
    // Note: There's no error margin allowed on this, so the value must match
    // exactly or the wme will blink.
    if (pWME->GetValue() == value) {
      return;
    }
  }
  ...
  if (GetConnection()->IsDirectConnection()) {
    ...
    pConnection->DirectAddWME_Double(
        m_AgentSMLHandle, pWME->GetIdentifierName(), pWME->GetAttribute(),
        value, pWME->GetTimeTag());
...
Identifier* WorkingMemory::CreateIdWME(Identifier* parent,
  ...
  // Create a new, unique id (e.g. "i3").  This id will be mapped to a different
  // id in the kernel.
  std::string id;
  GenerateNewID(pAttribute, &id);

  Identifier* pWME =
      new Identifier(GetAgent(), parent, parent->GetValueAsString(), pAttribute,
                     id.c_str(), GenerateTimeTag());
  ...
  if (GetConnection()->IsDirectConnection()) {
    EmbeddedConnection* pConnection =
        static_cast<EmbeddedConnection*>(GetConnection());
    pConnection->DirectAddID(m_AgentSMLHandle, parent->GetValueAsString(),
                             pAttribute, id.c_str(), pWME->GetTimeTag());
...
bool WorkingMemory::DestroyWME(WMElement* pWME) {
  ...
  // We can't delete top level WMEs (e.g. the WME that represents the
  // input-link's ID) Those are architecturally created.
  if (parent == NULL) {
    return false;
  }
...
bool WorkingMemory::Commit() {
  ...
  // as later calls will be ignored if the current set of changes has been sent
  // already.
  if (deltas == 0) {
    return true;
  }
  ...
  ElementXML* pMsg =
      GetConnection()->CreateSMLCommand(sml_Names::kCommand_Input);

  // Add the agent parameter and as a side-effect, get a pointer to the
  // <command> tag.  This is an optimization.
  ElementXML_Handle hCommand = GetConnection()->AddParameterToSMLCommand(
      pMsg, sml_Names::kParamAgent, GetAgentName());
...
void WorkingMemory::Refresh() {
  if (m_InputLink) {
    ...
    if (GetConnection()->SendAgentCommand(
            &response, sml_Names::kCommand_GetInputLink, GetAgentName())) {
      // Old comment: Technically we should reset the value of the input link
      // identifier itself, but it should never change value (since it's
      // architecturally created) and also adding a method to set the identifier
      // value is a bad idea if we only need it here.
