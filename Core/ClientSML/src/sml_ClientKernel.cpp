...
Kernel::Kernel(Connection* pConnection) {
  ...
  // We're turning on auto commit by default, so clients are a bit slower but
  // easier to write. Power users are free to turn it off and use explicit
  // commit calls.
  m_bAutoCommit = true;
  ...
  if (pConnection) {
    ...
    // We start the event thread for asynch connections (remote and embedded on
    // a new thread). Synchronous ones don't need it as the kernel can simply
    // call right over to the client directly for those.
  ...
  //  see sml_ProcessMessage and the other methods that are exported from
  //  KernelSML because they are only referenced within EmbeddedConnection
  //  (inside ConnectionSML.lib) which has to come later on the linker's command
  //  line than KernelSML. A way to resolve this is to make an access from here
  //  in ClientSML (which appears before KernelSML on the command line for the
  //  linker) to the methods in KernelSML, which will force the linker to pull
  //  the code into the final executable.
  //
  //  If we're in Windows this is not an issue (because the Windows linker
  //  supports these cyclical references) and if we're loading KernelSML
  //  dynamically (the normal fashion) this is also not a problem.
...
void Kernel::InitEvents() {
  ...
  RegisterForAgentEvent(smlEVENT_AFTER_AGENT_REINITIALIZED, &InitSoarHandler,
                        NULL);
  ...
  if (!GetConnection()->IsRemoteConnection()) {
    ...
    RegisterForStringEvent(smlEVENT_TCL_LIBRARY_MESSAGE,
                           &TclLibraryMessageHandler, NULL);
  }
}
...
Kernel::~Kernel(void) {
  ...
  // When the agent map is deleted, it will delete its contents (the Agent
  // objects) Do this before we delete the connection, in case we need to send
  // things to the kernel during clean up.
  m_AgentMap.clear();
  ...
  for (ConnectionListIter iter = m_ConnectionInfoList.begin();
       iter != m_ConnectionInfoList.end(); iter++) {
...
Agent* Kernel::IsXMLTraceEvent(ElementXML* pIncomingMsg) {
  ...
  if (trace.IsTag(sml_Names::kTagTrace) &&
      command.IsTag(sml_Names::kTagCommand) &&
      command.GetNumberChildren() > 0) {
...
ElementXML* Kernel::ProcessIncomingSML(Connection* pConnection,
  ...
  // Special case.  We want to intercept XML trace messages and pass them
  // directly to the handler without analyzing them.  This is just to boost
  // performance for these messages as speed is critical here as they're used
  // for trace output.
  Agent* pAgent = IsXMLTraceEvent(pIncomingMsg);
  if (pAgent) {
    pAgent->ReceivedXMLTraceEvent(smlEVENT_XML_TRACE_OUTPUT, pIncomingMsg,
                                  pResponse);
    return pResponse;
  }
...
void Kernel::ReceivedSystemEvent(smlSystemEventId id, AnalyzeXML* /*pIncoming*/,
  ...
  for (SystemEventMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end();) {
...
void Kernel::ReceivedUpdateEvent(smlUpdateEventId id, AnalyzeXML* pIncoming,
  ...
  smlRunFlags runFlags =
      smlRunFlags(pIncoming->GetArgInt(sml_Names::kParamValue, 0));
  ...
  // Go through the list of event handlers calling each in turn
  for (UpdateEventMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end();) {
...
void Kernel::ReceivedStringEvent(smlStringEventId id, AnalyzeXML* pIncoming,
  ...
  // Go through the list of event handlers calling each in turn
  for (StringEventMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end();) {
...
void Kernel::ReceivedAgentEvent(smlAgentEventId id, AnalyzeXML* pIncoming,
  ...
  // We may not, because "agent created" events are included in the list that
  // come here.
  Agent* pAgent = GetAgent(pAgentName);
  ...
  if (!pAgent && pAgentName) {
    ...
    // We have to do this now because we'll be passing it back to the caller in
    // a minute
    pAgent = MakeAgent(pAgentName);
  }

  // Go through the list of event handlers calling each in turn
  for (AgentEventMap::ValueListIter iter = pHandlers->begin();
       iter != pHandlers->end(); iter++) {
...
void Kernel::ReceivedRhsEvent(smlRhsEventId id, AnalyzeXML* pIncoming,
  ...
  // (We pass a single string but it could be parsed further to other values by
  // the client)
  char const* pFunctionName =
      pIncoming->GetArgString(sml_Names::kParamFunction);
  ...
  // we only execute the first handler (registering multipler handlers for the
  // same RHS function is not supported because these functions return a value
  // -- it wouldn't be clear which to use.  We could change this to call all
...
Kernel* Kernel::CreateKernelInCurrentThread(bool optimized,
                                            int portToListenOn) {
 ...
 *that are more explicitly named).
 *
 * @param ClientThread   If true, Soar will run in the client's thread and the
 *client must periodically call over to the kernel to check for incoming
 *messages on remote sockets. If false, Soar will run in a thread within the
 *kernel and that thread will check the incoming sockets itself. However, this
 *kernel thread model requires a context switch whenever commands are sent
 *to/from the kernel.
 * @param Optimized      If this is a client thread connection, we can
 *short-circuit parts of the messaging system for sending input and running
 *Soar.  If this flag is true we use those short cuts.  If you're trying to
 *debug the SML libraries you may wish to disable this option (so everything
 *goes through the standard paths).  Has no affect if not running on client
 *thread.
 * @param port           The port number the kernel should use to receive remote
 *connections.  The default port for SML is 12121 (picked at random).
 *
...
Kernel* Kernel::CreateEmbeddedConnection(bool clientThread, bool optimized,
  ...
  // Transfer any errors over to the kernel object, so the caller can retrieve
  // them.
  ...
  if (pConnection) {
    ...
    pConnection->RegisterCallback(ReceivedCall, pKernel,
                                  sml_Names::kDocType_Call, true);
...
int Kernel::GetListenerPort() {
  if (GetConnection()) {
    ...
    if (GetConnection()->SendAgentCommand(
            &response, sml_Names::kCommand_GetListenerPort)) {
...
Kernel* Kernel::CreateRemoteConnection(bool sharedFileSystem,
                                       char const* pIPaddress, int port,
                                       bool ignoreOutput) {
  ErrorCode errorCode = 0;
  ...
  Connection* pConnection = Connection::CreateRemoteConnection(
      sharedFileSystem, pIPaddress, port, &errorCode);
  ...
  // If this client isn't interested in getting output we can speed things up a
  // bit for them.
  ...
  // Register for "calls" from the kernel.
  pConnection->RegisterCallback(ReceivedCall, pKernel, sml_Names::kDocType_Call,
                                true);
 ...
 *        confused.  The current solution is to ask the kernel to assign a
 *starting point for the time tag counter--so we can keep each client in a
...
void Kernel::InitializeTimeTagCounter() {
  AnalyzeXML response;
  if (GetConnection()->SendAgentCommand(
          &response, sml_Names::kCommand_GetInitialTimeTag)) {
    long long initialTimeTag = response.GetResultInt(0);

    // Client side time tags are negative (to distinguish them from kernel side
    // ones)
...
void Kernel::UpdateAgentList() {
  ...
  if (GetConnection()->SendAgentCommand(&response,
                                        sml_Names::kCommand_GetAgentList)) {
    ElementXML const* pResult = response.GetResultTag();
    ElementXML child(NULL);

    // Keep a record of the agents we find, so we can delete any that have been
    // removed.
    ...
    // Any agents that are in our map but not in the "inuse" list we should
    // delete as they no longer exist.
...
bool Kernel::GetAllConnectionInfo() {
  ...
  AnalyzeXML response;
  if (GetConnection()->SendAgentCommand(&response,
                                        sml_Names::kCommand_GetConnections)) {
    ElementXML const* pResult = response.GetResultTag();
    ElementXML child(NULL);
    ...
    for (int i = 0; i < pResult->GetNumberChildren(); i++) {
      ...
      if (child.IsTag(sml_Names::kTagConnection)) {
        ...
        for (ConnectionListIter iter = previousList.begin();
             iter != previousList.end(); iter++) {
        ...
        if (!foundMatch) {
          ConnectionInfo* info =
              new ConnectionInfo(pID, pName, pStatus, pAgentStatus);
  ...
  // If we deleted all of the items from the previous list, then each connection
  // we found matched an existing one.
  if (previousList.size() == 0 &&
      previousConnectionCount == m_ConnectionInfoList.size()) {
    m_ConnectionInfoChanged = false;
  } else {
...
bool Kernel::HasConnectionInfoChanged() { return m_ConnectionInfoChanged; }
...
bool Kernel::SetConnectionInfo(char const* pName, char const* pConnectionStatus,
  ...
  AnalyzeXML response;
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_SetConnectionInfo, NULL,
      sml_Names::kConnectionName, pName, sml_Names::kConnectionStatus,
      pConnectionStatus, sml_Names::kAgentStatus, pAgentStatus);
  return ok;
}
...
Agent* Kernel::CreateAgent(char const* pAgentName) {
  ...
  if (GetConnection()->SendAgentCommand(&response,
                                        sml_Names::kCommand_CreateAgent, NULL,
                                        sml_Names::kParamName, pAgentName)) {
...
Agent* Kernel::MakeAgent(char const* pAgentName) {
  ...
  // Register to get output link events.  These won't come back as standard
  // events. Instead we'll get "output" messages which are handled in a special
  // manner.
  if (!m_bIgnoreOutput) {
    RegisterForEventWithKernel(smlEVENT_OUTPUT_PHASE_CALLBACK,
                               agent->GetAgentName());
  }
...
bool Kernel::DestroyAgent(Agent* pAgent) {
  ...
  if (GetConnection()->SendAgentCommand(&response,
                                        sml_Names::kCommand_DestroyAgent,
                                        pAgent->GetAgentName())) {
 ...
 * @param echoResults  If true the results are also echoed through the
 *smlEVENT_ECHO event, so they can appear in a debugger (or other listener)
...
char const* Kernel::ExecuteCommandLine(char const* pCommandLine,
  ...
  m_CommandLineSucceeded = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_CommandLine, pAgentName,
      sml_Names::kParamLine, pCommandLine, sml_Names::kParamEcho,
      echoResults ? sml_Names::kTrue : sml_Names::kFalse,
      sml_Names::kParamNoFiltering,
      !m_FilteringEnabled || noFilter ? sml_Names::kTrue : sml_Names::kFalse,
      wantRawOutput);
...
bool Kernel::ExecuteCommandLineXML(char const* pCommandLine,
  ...
  m_CommandLineSucceeded = GetConnection()->SendAgentCommand(
      pResponse->GetAnalyzeXML(), sml_Names::kCommand_CommandLine, pAgentName,
      sml_Names::kParamLine, pCommandLine, sml_Names::kParamNoFiltering,
      sml_Names::kTrue);
...
char const* Kernel::RunAllAgents(int numberSteps, smlRunStepSize stepSize,
  ...
  // to execute a run command, so we provide one (which one should make no
  // difference).
  ...
  char const* pResult =
      ExecuteCommandLine(cmd.c_str(), pFirstAgent->GetAgentName());
  return pResult;
}
...
char const* Kernel::RunAllTilOutput(smlRunStepSize interleaveStepSize) {
  ...
  // Run all agents until each has generated output.  Each agent will stop at
  // the point it has generated output, so they may run for different numbers of
  // decisions. For now, maxDecisions is being ignored.  We should make this a
  // separate call to set this parameter.
 ...
 * The usual way to do this is to register for an event (e.g.
 *AFTER_DECISION_CYCLE) and in that event handler decide if the user wishes to
 *stop soar. If so, call to this method inside that handler.
 *
...
char const* Kernel::StopAllAgents() {
  ...
  // The command line currently requires an agent in order
  // to execute a stop-soar command, so we provide one (which one should make no
  // difference).
  if (GetNumberAgents() == 0) {
...
bool Kernel::IsSoarRunning() {
  ...
  bool ok = (GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_IsSoarRunning));
...
bool Kernel::FireStartSystemEvent() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_FireEvent, NULL, sml_Names::kParamEventID,
      m_pEventMap->ConvertToString(smlEVENT_SYSTEM_START));
  return ok;
}
...
bool Kernel::FireStopSystemEvent() {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_FireEvent, NULL, sml_Names::kParamEventID,
      m_pEventMap->ConvertToString(smlEVENT_SYSTEM_STOP));
  return ok;
}
...
bool Kernel::SuppressSystemStop(bool state) {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_SuppressEvent, NULL,
      sml_Names::kParamEventID,
      m_pEventMap->ConvertToString(smlEVENT_SYSTEM_STOP),
      sml_Names::kParamValue, state ? sml_Names::kTrue : sml_Names::kFalse);
  return ok;
}
...
bool Kernel::CheckForIncomingCommands() {
  AnalyzeXML response;
  if (GetConnection()->SendAgentCommand(
          &response, sml_Names::kCommand_CheckForIncomingCommands)) {
    return response.GetResultBool(false);
  }
 ...
 *        have shut down the event thread and are taking over event handling
 *directly. (A common reason is to ensure the callbacks from events are on the
 *main execution thread of the caller).
 *
...
void Kernel::RegisterForEventWithKernel(int id, char const* pAgentName) {
  ...
  GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_RegisterForEvent, pAgentName,
...
void Kernel::UnregisterForEventWithKernel(int id, char const* pAgentName) {
  ...
  GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_UnregisterForEvent, pAgentName,
      sml_Names::kParamEventID, pEvent);
}
 ...
 * @param pUserData      Arbitrary data that will be passed back to the handler
 *function when the event happens.
 * @param addToBack      If true add this handler is called after existing
 *handlers.  If false, called before existing handlers.
 *
 * Current set is:
...
int Kernel::RegisterForSystemEvent(smlSystemEventId id,
  ...
  // Record the handler
  // We use a struct rather than a pointer to a struct, so there's no need to
  // new/delete everything as the objects are added and deleted.
  m_CallbackIDCounter++;

  SystemEventHandlerPlusData handlerPlus(id, handler, pUserData,
                                         m_CallbackIDCounter);
 ...
 *handlers.  If false, called before existing handlers.
 *
 * This event is registered with the kernel because they relate to events we
 *think may be useful to use to trigger updates in synchronous environments.
 *
...
int Kernel::RegisterForUpdateEvent(smlUpdateEventId id,
  ...
  UpdateEventHandlerPlusData handlerPlus(id, handler, pUserData,
                                         m_CallbackIDCounter);
...
int Kernel::RegisterForStringEvent(smlStringEventId id,
  ...
  StringEventHandlerPlusData handlerPlus(id, handler, pUserData,
                                         m_CallbackIDCounter);
...
int Kernel::RegisterForAgentEvent(smlAgentEventId id, AgentEventHandler handler,
  ...
  // Record the handler
  m_CallbackIDCounter++;
  AgentEventHandlerPlusData handlerPlus(id, handler, pUserData,
                                        m_CallbackIDCounter);
...
int Kernel::InternalAddRhsFunction(smlRhsEventId id,
  ...
  // Start by checking if this event type and function namecombination has
  ...
  if (found && optionalFoundHandler.m_Handler != 0) {
    std::cerr
        << "WARNING: Attempt to register a duplicate RHS function handler for "
        << pRhsFunctionName << std::endl;
  ...
  if (m_RhsEventMap.getListSize(pRhsFunctionName) == 0) {
    ...
    GetConnection()->SendAgentCommand(&response,
                                      sml_Names::kCommand_RegisterForEvent,
                                      NULL, sml_Names::kParamEventID, pEvent,
                                      sml_Names::kParamName, pRhsFunctionName);
  }

  // Record the handler
  m_CallbackIDCounter++;
  RhsEventHandlerPlusData handlerPlus(id, pRhsFunctionName, handler,
                                      m_CallbackIDCounter);
...
bool Kernel::InternalRemoveRhsFunction(smlRhsEventId id, int callbackID) {
  ...
  // Find the function for this callbackID (for RHS functions the key is a
  // function name not an event id)
  ...
  // If we just removed the last handler, then unregister from the kernel for
  // this event
  if (m_RhsEventMap.getListSize(functionName) == 0) {
    ...
    // Send the unregister command
    GetConnection()->SendAgentCommand(
        &response, sml_Names::kCommand_UnregisterForEvent, NULL,
        sml_Names::kParamEventID, pEvent, sml_Names::kParamName,
        functionName.c_str());
  }
...
// RHS function and client message handlers can be added via 2 separate API's;
// this helper converts the function pointer-style handlers into
// std::function-style handlers, because the std::function-style is used
// internally.
RhsEventHandlerCpp funPointer2StdFunction(RhsEventHandler handler,
                                          void* pUserData) {
  return [handler, pUserData](smlRhsEventId id, Agent* pAgent,
                              char const* pFunctionName,
 ...
 *the client.
 *
 *        The methods should only operate on the incoming argument list and
 *return a result without access to other external information to remain with
 *the theory of a Soar agent.
 *
 *        Multiple handlers can be registered for the same function but only one
 *will ever be called. This will be the first handler registered in the local
 *process (where the Kernel is executing). If no handler is available there then
 *the first handler registered in an external process will be called. (The
 *latter case could obviously be quite slow).
 *
 *        The function is implemented by providing a handler (a
 *RhsEventHandler).  This will be passed a single string and returns a string.
 *The incoming argument string can contain arguments that the client should
 *parse (e.g. passing a coordinate as "12 56").  The format of the string is up
 *to the implementor of the specific RHS function.
 *
 * @param pRhsFunctionName   The name of the method we are implementing
 *(case-sensitive)
 * @param handler            A function that will be called when the event
 *happens
 * @param pUserData          Arbitrary data that will be passed back to the
 *handler function when the event happens.
 * @param addToBack          If true add this handler is called after existing
 *handlers.  If false, called before existing handlers.
 *
 * @returns Unique ID for this callback.  Required when unregistering this
 ...
 *
 * @see sml::Kernel::AddRhsFunction(char const* pClientName, RhsEventHandler
 * handler, void* pUserData, bool addToBack)
 */
int Kernel::AddRhsFunction(char const* pRhsFunctionName,
                           RhsEventHandlerCpp handler, bool addToBack) {
 ...
 *to any other client is sometimes useful.
 *
 *        When the original client sends a message, the RHS function handler is
 *called to process and (optionally) return a message to the caller.
 *
 *        A RHS (right hand side) function handler is used just to reduce the
 *number of types in the system and because it is sufficient for this purpose.
 *
 *        The function is implemented by providing a handler (a
 *RhsEventHandler).  This will be passed a single string and returns a string.
 *The incoming argument string can contain arguments that the client should
 ...
 *to the implementor of the specific RHS function.
 *
 * @param pClientName        The name of the client that this message relates to
 *(e.g. "TankSoar" for messages sent to/from tank soar).  The clients
 *sending/receiving just need to agree on this string. Due to the way this is
 *implemented, the type here must not be the same as the name of a RHS function
 *registered with Soar.
 * @param pUserData          Arbitrary data that will be passed back to the
 *handler function when the event happens.
 * @param addToBack          If true add this handler is called after existing
 *handlers.  If false, called before existing handlers.
 ...
 * ergonomic C++.
 *
 * @see sml::Kernel::RegisterForClientMessageEvent(char const* pClientName,
 * ClientMessageHandler handler, void* pUserData, bool addToBack)
 */
int Kernel::RegisterForClientMessageEvent(char const* pClientName,
                                          ClientMessageHandlerCpp handler,
                                          bool addToBack) {
  smlRhsEventId id = smlEVENT_CLIENT_MESSAGE;
  // We actually use the RHS function code internally to process this message
  // (since it's almost exactly like calling a RHS function that's processed on
  // a client).
 ...
 *
 *        This mechanism allows one client to send an arbitrary string to
 *another client and get a response (as a string).  The contents of the string
 *are up to the clients to agree upon. Passing a string is sufficient to support
 *either simple functionality (passing just a single value) up to complex
 *functionality (passing a complete XML message).
 *
 *        If multiple other clients register for this message, their responses
 *are concatenated together in the result. (This is not expected to be a common
 *usage).
 *
 * @param pAgent             The originating agent (can be NULL), if this
 *message is specific to an agent.
 * @param pClientName        The name of the client that this message relates to
 *(e.g. "TankSoar" for messages sent to/from tank soar).  The clients
 *sending/receiving just need to agree on this string. Due to the way this is
 *implemented, the type here must not be the same as the name of a RHS function
...
std::string Kernel::SendClientMessage(Agent* pAgent, char const* pClientName,
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_SendClientMessage,
      pAgent ? pAgent->GetAgentName() : NULL, sml_Names::kParamName,
      pClientName, sml_Names::kParamMessage, pMessage);
...
bool Kernel::UnregisterForSystemEvent(int callbackID) {
  ...
  smlSystemEventId id =
      m_SystemEventMap.findFirstKeyByTest(&test, smlSYSTEM_EVENT_BAD);
...
bool Kernel::UnregisterForStringEvent(int callbackID) {
  ...
  // Find the event ID for this callbackID
  smlStringEventId id =
      m_StringEventMap.findFirstKeyByTest(&test, smlSTRING_EVENT_BAD);
...
bool Kernel::UnregisterForUpdateEvent(int callbackID) {
  ...
  // Find the event ID for this callbackID
  smlUpdateEventId id =
      m_UpdateEventMap.findFirstKeyByTest(&test, smlUPDATE_EVENT_BAD);
...
bool Kernel::UnregisterForAgentEvent(int callbackID) {
  ...
  // Find the event ID for this callbackID
  smlAgentEventId id =
      m_AgentEventMap.findFirstKeyByTest(&test, smlAGENT_EVENT_BAD);
...
bool Kernel::SetInterruptCheckRate(int newRate) {
  ...
  bool ok = GetConnection()->SendAgentCommand(
      &response, sml_Names::kCommand_SetInterruptCheckRate, NULL,
      sml_Names::kParamValue, ostr.str().c_str());
...
// -- numeric code (bad because codes will be different for each library, or
// library can fail in ways not covered by pre-defined codes)
...
std::string Kernel::LoadExternalLibrary(const char* pLibraryCommand) {
  ...
  if (!hLibrary) {
    ...
    InitLibraryFunction pInitLibraryFunction =
        Dangerous_Pointer_Cast<InitLibraryFunction>::from(
            GetProcAddress(hLibrary, "sml_InitLibrary"));
    ...
    char** charArgv =
        new char*[vectorArgv.size() + 1];  // plus 1 for null-termination
    for (std::vector<std::string>::size_type index = 0;
         index < vectorArgv.size(); ++index) {
      // copy
      charArgv[index] = new char[vectorArgv[index].size() + 1];
      strncpy(charArgv[index], vectorArgv[index].data(),
              vectorArgv[index].size());
...
void Kernel::SendSVSInput(const char* agentName, const std::string& txt) {
  AnalyzeXML response;
  GetConnection()->SendAgentCommand(&response, sml_Names::kCommand_SVSInput,
                                    agentName, sml_Names::kParamLine,
                                    txt.c_str());
}
...
std::string Kernel::GetSVSOutput(const char* agentName) {
  AnalyzeXML response;
  if (GetConnection()->SendAgentCommand(
          &response, sml_Names::kCommand_SVSOutput, agentName,
          sml_Names::kParamLine, NULL)) {
    return response.GetResultString();
  } else {
    return "";
  }
...
std::string Kernel::SVSQuery(const char* agentName, const std::string& q) {
  AnalyzeXML response;
  if (GetConnection()->SendAgentCommand(&response, sml_Names::kCommand_SVSQuery,
                                        agentName, sml_Names::kParamLine,
                                        q.c_str())) {
    return response.GetResultString();
  } else {
