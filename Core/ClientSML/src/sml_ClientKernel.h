...
class SystemEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  SystemEventHandlerPlusData(int eventID, SystemEventHandler handler,
                             void* userData, int callbackID)
      : EventHandlerPlusData(eventID, userData, callbackID) {
    m_Handler = handler;
  }
};
...
class UpdateEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  UpdateEventHandlerPlusData(int eventID, UpdateEventHandler handler,
                             void* userData, int callbackID)
      : EventHandlerPlusData(eventID, userData, callbackID) {
    m_Handler = handler;
  }
...
class StringEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  StringEventHandlerPlusData(int eventID, StringEventHandler handler,
                             void* userData, int callbackID)
      : EventHandlerPlusData(eventID, userData, callbackID) {
    m_Handler = handler;
  }
...
class AgentEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  AgentEventHandlerPlusData(int eventID, AgentEventHandler handler,
                            void* userData, int callbackID)
      : EventHandlerPlusData(eventID, userData, callbackID) {
    m_Handler = handler;
  }
...
class RhsEventHandlerPlusData : public EventHandlerPlusData {
 public:
  ...
  RhsEventHandlerPlusData(int eventID, char const* pFunctionName,
                          RhsEventHandlerCpp handler, int callbackID)
      : EventHandlerPlusData(eventID, nullptr, callbackID) {
    m_Handler = handler;
...
class EXPORT Kernel : public ClientErrors {
 ...
 protected:
  long long
      m_TimeTagCounter;  // Used to generate time tags (we do them in the kernel
                         // not the agent, so ids are unique for all agents)
  long long m_IdCounter;    // Used to generate unique id names
  int m_CallbackIDCounter;  // Used to generate unique callback IDs

  // The mapping from event number to a list of handlers to call when that event
  // fires
  typedef sml::ListMap<smlSystemEventId, SystemEventHandlerPlusData>
      SystemEventMap;
  typedef sml::ListMap<smlAgentEventId, AgentEventHandlerPlusData>
      AgentEventMap;
  typedef sml::ListMap<std::string, RhsEventHandlerPlusData> RhsEventMap;
  typedef sml::ListMap<smlUpdateEventId, UpdateEventHandlerPlusData>
      UpdateEventMap;
  typedef sml::ListMap<smlStringEventId, StringEventHandlerPlusData>
      StringEventMap;
  ...
  // When true, commands are sent to external filters (if they are registered)
  // for filtering before execution.
  ...
  // (that is send them over to kernelSML immediately, rather than collecting
  // them up for a single commit)
  bool m_bAutoCommit;

  // This thread is used to check for incoming events when the client goes to
  // sleep It ensures the client stays "alive" and is optional (there are other
  // ways for clients to keep themselves responsive).
  EventThread* m_pEventThread;

  // To create a kernel object, use one of the static methods, e.g.
  // Kernel::CreateEmbeddedConnection().
  ...
  */
  int InternalAddRhsFunction(smlRhsEventId id, char const* pRhsFunctionName,
   ...
   * @param pResponse  The reply (no real need to fill anything in here
   *currently)
   */
  void ReceivedEvent(AnalyzeXML* pIncoming, soarxml::ElementXML* pResponse);
  void ReceivedSystemEvent(smlSystemEventId id, AnalyzeXML* pIncoming,
                           soarxml::ElementXML* pResponse);
  void ReceivedAgentEvent(smlAgentEventId id, AnalyzeXML* pIncoming,
                          soarxml::ElementXML* pResponse);
  void ReceivedRhsEvent(smlRhsEventId id, AnalyzeXML* pIncoming,
                        soarxml::ElementXML* pResponse);
  void ReceivedUpdateEvent(smlUpdateEventId id, AnalyzeXML* pIncoming,
                           soarxml::ElementXML* pResponse);
 ...
 public:
   ...
   *
   *        If you're not sure which method to use, you generally want
   *"InNewThread". That extra thread usually makes your life easier.
   *
   *        Creating in "current thread" will produce maximum performance but
   *requires a little more work for the developer (you must call
   *CheckForIncomingCommands() periodically and you should not register for
   *events and then go to sleep).
   *
   *        Creating in "new thread" is simpler for the developer but will be
   *slower (around a factor 2). (It's simpler because there's no need to call
   *CheckForIncomingCommands() periodically as this happens in a separate thread
   *running inside the kernel and incoming events are handled by another thread
   *in the client).
   *
   * @param Optimized      If this is a current thread connection, we can
   ...
   *goes through the standard paths).  Not available if running in a new thread.
   *                       Also if you're looking for maximum performance be
   *sure to read about the "auto commit" options below.
   * @param port           The port number the kernel should use to receive
   *remote connections.  The default port for SML is 12121 (kDefaultSMLPort)
   *                       (picked at random). Passing 0 (kSuppressListener)
   *means no listening port will be created (so it will be impossible to make
   *                       remote connections to the kernel). Passing -1
   *(kUseAnyPort) means bind to any availble port (retrieve after success using
   *                       GetListenerPort()), and, for local connections,
   *create a named pipe using the PID. To connect to this high-performance local
   *connection, simply pass the PID as the port in CreateRemoteConnection().
   *
   ...
   */
  static Kernel* CreateKernelInCurrentThread(
      bool optimized = false, int portToListenOn = kDefaultSMLPort);
   ...
   *machine.
   *
   * @param sharedFileSystem   If true the local and remote machines can access
   *the same set of files. For example, this means when loading a file of
   *productions, sending the filename is sufficient, without actually sending
   *the contents of the file. (NOTE: It may be a while before we really support
   *passing in 'false' here)
   * @param pIPaddress The IP address of the remote machine (e.g.
   *"202.55.12.54"). Pass "127.0.0.1" or NULL to create a connection between two
   *processes on the same machine.
   * @param port       The port number to connect to.  The default port for SML
   *is 12121 (picked at random). If connecting to a local machine with a kernel
   *created using kUseAnyPort, you can pass the PID here to connect to a
   *high-performance local connection.
   * @param ignoreOutput Setting this to true means output link changes won't be
   *sent to this client (improving performance if you aren't interested in
   *output)
   *
   * @returns A new kernel object which is used to communicate with the kernel
   *          If an error occurs a Kernel object is still returned.  Call
   *"HadError()" and "GetLastErrorDescription()" on it.
   */
  static Kernel* CreateRemoteConnection(bool sharedFileSystem = true,
                                        char const* pIPaddress = 0,
   ...
   * @param noFilter     If true this command line by-passes any external
   *filters that have been registered (this is not common) and is executed
   *immediately.
   ...
   * @param stepSize       The size of step we're running (e.g. phases,
   *decisions, outputs)
   * @param interleaveStepSize  How much to run each agent before running the
   *next agent.
   *
   ...
   */
  char const* RunAllAgents(int numberSteps,
                           smlRunStepSize stepSize = sml_DECIDE,
   ...
   *          after each agent has generated output (or has reached the decision
   *limit).
   *
   ...
   * before then that agent will stop running.  (This value can be changed with
   *the max-nil-output-cycles command).
   */
   ...
   *
   *        Multiple handlers can be registered for the same function but only
   *one will ever be called. This will be the first handler registered in the
   *local process (where the Kernel is executing). If no handler is available
   *there then the first handler registered in an external process will be
   *called. (The latter case could obviously be quite slow).
   *
   ...
   *parse (e.g. passing a coordinate as "12 56").  The format of the string is
   *up to the implementor of the specific RHS function.
   *
   ...
   *
   *        When the original client sends a message, the RHS function handler
   *is called to process and (optionally) return a message to the caller.
   *
   *        Multiple handlers can be registered for a given message type and the
   *results will be concatenated together and returned to the original caller.
   *(This is expected to be an usual situation).
   *
   ...
   *up to the implementor of the specific RHS function.
   *
   * @param pClientName        The name of the client that this message relates
   *to (e.g. "TankSoar" for messages sent to/from tank soar).  The clients
   *sending/receiving just need to agree on this string. Due to the way this is
   *implemented, the type here must not be the same as the name of a RHS
   *function registered with Soar.
   * @param handler            A function that will be called when the event
   *happens
   * @param pUserData          Arbitrary data that will be passed back to the
   *handler function when the event happens.
   ...
   *are up to the clients to agree upon. Passing a string is sufficient to
   *support either simple functionality (passing just a single value) up to
   *        complex functionality (passing a complete XML message).
   *
   *        If multiple other clients register for this message, their responses
   *are concatenated together in the result. (This is not expected to be a
   *common usage).
   *
   ...
   *handlers.  If false, called before existing handlers.
   *
   * This event is registered with the kernel because they relate to the
   *creation and destruction of agents (amongst other things). If we associated
   *it with the agent there'd be no way to register for "agent creation" events.
   *
   * Current set is:
   * // Agent manager
 ...
 protected:
   ...
   */
  static soarxml::ElementXML* ReceivedCall(Connection* pConnection,
   ...
   */
  static Kernel* CreateEmbeddedConnection(bool clientThread, bool optimized,
                                          int portToListenOn);
