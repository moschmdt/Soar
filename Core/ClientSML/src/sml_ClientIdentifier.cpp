...
void IdentifierSymbol::DeleteAllChildren() {
  ...
  for (Identifier::ChildrenIter iter = m_Children.begin();
...
std::list<WMElement *>::iterator
IdentifierSymbol::FindChildByTimeTag(long long timeTag) {
  for (std::list<WMElement *>::iterator i = m_Children.begin();
       i != m_Children.end(); ++i) {
...
void IdentifierSymbol::AddChild(WMElement *pWME) {
  ...
  if (iter == m_Children.end()) {
    // std::cout << "AddChild: " << pWME->GetIdentifierName() << ", " <<
    // pWME->GetAttribute() << ", " << pWME->GetValueAsString() << " (" <<
    // pWME->GetTimeTag() << ")" << " (" << pWME << ")" << std::endl;
    m_Children.push_back(pWME);
  } else {
    // std::cout << "Did not AddChild: " << pWME->GetIdentifierName() << ", " <<
    // pWME->GetAttribute() << ", " << pWME->GetValueAsString() << " (" <<
    // pWME->GetTimeTag() << ")" << " (" << pWME << ")" << std::endl;
  }
}
...
void IdentifierSymbol::RemoveChild(WMElement *pWME) {
  ...
  Identifier::ChildrenIter iter = FindChildByTimeTag(pWME->GetTimeTag());
  if (iter != m_Children.end()) {
    // std::cout << "RemoveChild: " << pWME->GetIdentifierName() << ", " <<
    // pWME->GetAttribute() << ", " << pWME->GetValueAsString() << " (" <<
    // pWME->GetTimeTag() << ")" << " (" << pWME << ")" << std::endl;
    m_Children.erase(iter);
  } else {
    // std::cout << "Did not RemoveChild: " << pWME->GetIdentifierName() << ", "
    // << pWME->GetAttribute() << ", " << pWME->GetValueAsString() << " (" <<
    // pWME->GetTimeTag() << ")" << " (" << pWME << ")" << std::endl;
  }
}
...
void IdentifierSymbol::DebugString(std::string &result) {
  ...
  if (!m_UsedBy.empty()) {
    ss << " used by:";
    for (std::list<Identifier *>::iterator iter = m_UsedBy.begin();
         iter != m_UsedBy.end(); ++iter) {
      ss << " " << (*iter)->GetTimeTag();
    }
  }
  ss << "]";
...
Identifier::Identifier(Agent *pAgent, char const *pAttributeName,
                       char const *pIdentifier, long long timeTag)
  ...
  RecordSymbolInMap();
  //}
  // std::cout << "created (" << this->GetIdentifierName() << " ^" <<
  // this->GetAttribute()
  //  << " " << this->GetValueAsString() << ": " << this->GetTimeTag() << ")" <<
  //  std::endl;
}

// The normal case (where there is a parent id)
Identifier::Identifier(Agent *pAgent, Identifier *pParent, char const *pID,
                       char const *pAttributeName, char const *pIdentifier,
                       long long timeTag)
...
Identifier::Identifier(Agent *pAgent, IdentifierSymbol *pParentSymbol,
                       char const *pID, char const *pAttributeName,
                       char const *pIdentifier, long long timeTag)
...
Identifier::Identifier(Agent *pAgent, Identifier *pParent, char const *pID,
                       char const *pAttributeName,
                       Identifier *pLinkedIdentifier, long long timeTag)
    : WMElement(pAgent, pParent->GetSymbol(), pID, pAttributeName, timeTag) {
  m_pSymbol = pLinkedIdentifier->m_pSymbol;
  m_pSymbol->UsedBy(this);
  // RecordSymbolInMap();  // redundant, if it is on another symbol, it is
  // already in the map std::cout << "created (" << this->GetIdentifierName() <<
  // " ^" << this->GetAttribute()
  //   << " " << this->GetValueAsString() << ": " << this->GetTimeTag() << ")"
  //   << std::endl;
}
...
Identifier::Identifier(Agent *pAgent, IdentifierSymbol *pParentSymbol,
                       char const *pID, char const *pAttributeName,
                       IdentifierSymbol *pLinkedIdentifierSymbol,
                       long long timeTag)
    : WMElement(pAgent, pParentSymbol, pID, pAttributeName, timeTag) {
...
void Identifier::ReleaseSymbol() {
  ...
  // Decide if we need to delete the identifier symbol (or is someone else still
  // using it)
...
WMElement *Identifier::FindByAttribute(char const *pAttribute,
  ...
  std::string err_msg = this->m_AttributeName + " has no child attribute \"" +
                        std::string(pAttribute) + "\"";

  spdlog::error(err_msg);
  // Would love to throw an error here, but this lets the legacy tests fail.
...
WMElement *Identifier::FindFromTimeTag(long long timeTag) const {
  ...
  // from the output link, but if clients start to use this (or the output link
  // is really large for an application) the saving could be significant.

  if (this->GetTimeTag() == timeTag) {
    return const_cast<WMElement *>(static_cast<const WMElement *>(this));
  }

  // Go through each child in turn and if it's an identifier search its children
  // for a matching id.
  for (ChildrenConstIter iter = m_pSymbol->m_Children.begin();
       iter != m_pSymbol->m_Children.end(); iter++) {
    WMElement *pWME = *iter;
...
char const *Identifier::GetValueType() const { return sml_Names::kTypeID; }

// Send over to the kernel again
void Identifier::Refresh() {
  // Add this identifier into the tree (except for input link which is auto
  // created)
...
#ifdef SML_DIRECT
void Identifier::DirectAdd(Direct_AgentSML_Handle pAgentSML,
                           long long timeTag) {
  EmbeddedConnection *pConnection =
      static_cast<EmbeddedConnection *>(GetAgent()->GetConnection());
  pConnection->DirectAddID(pAgentSML, m_ID->GetIdentifierSymbol(),
                           GetAttribute(), GetValueAsString(), timeTag);
}
#endif

StringElement *Identifier::CreateStringWME(char const *pAttribute,
                                           char const *pValue) {
...
Identifier *Identifier::CreateSharedIdWME(char const *pAttribute,
                                          Identifier *pSharedValue) {
  return this->m_Agent->GetWM()->CreateSharedIdWME(this, pAttribute,
                                                   pSharedValue);
}
