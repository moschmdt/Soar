...
WMElement::WMElement(Agent* pAgent, IdentifierSymbol* pParentSymbol,
  ...
  // parent and attribute name can both be NULL if this is at the top of the
  // tree.
  if (pAttributeName) {
...
void WMElement::Refresh() {
  ...
  if (GetAgent()->GetConnection()->IsDirectConnection()) {
    ...
    Direct_AgentSML_Handle agentSMLHandle =
        pConnection->DirectGetAgentSMLHandle(GetAgent()->GetAgentName());
...
void WMElement::DebugString(std::string& result) {
  std::stringstream ss;
  ss << "(" << GetTimeTag() << ": " << GetIdentifierName() << " ^"
     << GetAttribute() << " " << GetValueAsString() << ")";
