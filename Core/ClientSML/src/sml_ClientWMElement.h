...
class EXPORT WMElement {
  // Making most methods protected, so users don't use them directly by
  // accident. But allow working memory to work with them directly.
 ...
 protected:
  ...
  // We used negative values so it's clear that this time tag is a client side
  // tag.
  long long m_TimeTag;

  // The identifier symbol as a string.  This can be necessary when connecting
  // up disconnected segments of a graph.
  std::string m_IDName;
  ...
  // This is true if the wme was just added.  The client chooses when to clear
  // these flags.
  bool m_JustAdded;

 public:
  // This is true if the wme was just added.  The client chooses when to clear
  // these flags. This is only maintained for output wmes (the client controls
  // input wmes).
 ...
 protected:
  ...
  WMElement(Agent* pAgent, IdentifierSymbol* pParentSymbol, char const* pID,
            char const* pAttributeName, long long timeTag);
...
#ifdef SML_DIRECT
  virtual void DirectAdd(Direct_AgentSML_Handle pAgentSML,
                         long long timeTag) = 0;
#endif
