...
QString SoarAgent::executeCommand(const QString &command)
{
    ...
    try {
        const char* result = m_smlAgent->ExecuteCommandLine(command.toUtf8().constData());
...
bool SoarAgent::initSoar()
{
    ...
    try {
        ...
        return result == nullptr || strlen(result) == 0; // No error message means success
    } catch (const std::exception& e) {
...
QString SoarAgent::getSlotValue(const QString &object, const QString &attribute)
{
    if (!m_smlAgent) {
        return QString();
    }
...
void SoarAgent::removeCallbacks()
{
    ...
    try {
        ...
        qWarning() << "Exception removing callbacks:" << e.what();
    }
}
                            ...
                            sml::Agent* agent, const char* message)
{
    ...
    SoarAgent* soarAgent = static_cast<SoarAgent*>(userData);
    if (soarAgent && message) {
        emit soarAgent->outputReceived(QString::fromUtf8(message));
    }
}
