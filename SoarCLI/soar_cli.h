...
void PrintCallbackHandler(sml::smlPrintEventId, void *, sml::Agent *,
                          char const *);
void XMLCallbackHandler(sml::smlXMLEventId, void *, sml::Agent *,
                        sml::ClientXML *);
...
public:
  SoarCLI()
      : m_kernel(0), m_currentAgent(0), m_port(sml::Kernel::kUseAnyPort),
        m_listen(false),
...
#endif
        m_quit(false), m_seen_newline(true), m_isMultiAgent(false),
  ...
  inline const char *getcol(const char *pColor) {
    if (m_color)
      return pColor;
    else
      return "";
