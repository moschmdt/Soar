...
#include "SoarUnitTests/AgentTest.hpp"
#include "SoarUnitTests/AliasTest.hpp"
#include "SoarUnitTests/BasicTests.hpp"
#include "SoarUnitTests/BuiltinRHSTests.hpp"
#include "SoarUnitTests/ChunkingDemoTests.hpp"
#include "SoarUnitTests/ChunkingTests.hpp"
#include "SoarUnitTests/ElementXMLTest.hpp"
#include "SoarUnitTests/EpMemFunctionalTests.hpp"
#include "SoarUnitTests/ExternalLibraryTest.hpp"
#include "SoarUnitTests/FullTests.hpp"
#include "SoarUnitTests/FullTestsClientThread.hpp"
#include "SoarUnitTests/FullTestsClientThreadFullyOptimized.hpp"
#include "SoarUnitTests/FullTestsRemote.hpp"
#include "SoarUnitTests/FunctionalTests.hpp"
#include "SoarUnitTests/IOTests.hpp"
#include "SoarUnitTests/MiscTests.hpp"
#include "SoarUnitTests/MultiAgentTest.hpp"
#include "SoarUnitTests/SMemFunctionalTests.hpp"
#include "SoarUnitTests/SimpleListener.hpp"
#include "SoarUnitTests/SvsTests.hpp"
#include "SoarUnitTests/TokenizerTest.hpp"
#include "SoarUnitTests/WmaFunctionalTests.hpp"
...
void usage(std::string arg0) {
  ...
  std::cout << "\t"
            << "-c --category"
            << "\t\t\t\t"
            << "Run only these categories." << std::endl;
  std::cout << "\t"
            << "-t --test"
            << "\t\t\t\t"
            << "Run only these tests." << std::endl;
  std::cout << "\t"
            << "-E --exclude-category"
            << "\t\t\t"
            << "Exclude this category." << std::endl;
  std::cout << "\t"
            << "-e --exclude-test"
            << "\t\t\t"
            << "Exclude this test." << std::endl;
  std::cout << "\t"
            << "-F --expected-failure-category"
            << "\t\t"
            << "Ignore failures in this category." << std::endl;
  std::cout << "\t"
            << "-f --expected-failure-test"
            << "\t\t"
            << "Ignore this test failing." << std::endl;
  std::cout << "\t"
            << "-n --no-refcount-leak-check"
            << "\t\t"
            << "Do not init-soar and check for refcount leaks." << std::endl;
  std::cout << "\t"
            << "-a --after-action-reports"
            << "\t\t"
            << "Generate reports for each agent trial." << std::endl;
  std::cout << "\t"
            << "-l --logs"
            << "\t\t\t\t"
            << "Record logs of agent trials." << std::endl;
  std::cout << "\t"
            << "-x --no-explainer"
            << "\t\t\t"
            << "Run learning agents with explainer on." << std::endl;
  std::cout << "\t"
            << "-r --run-debug-mode"
            << "\t\t\t"
            << "Don't force strict unit test settings." << std::endl;
  std::cout << "\t"
            << "-h --help"
            << "\t\t\t\t"
  ...
  std::cout << "\t"
            << "-s --silent"
            << "\t\t\t\t"
            << "Always return 0.  Read Test.xml for results." << std::endl;
  std::cout << std::endl;
}
...
int main(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (index + 1 < argc) parameter = argv[index + 1];
    ...
    if (argument == "--listener") {
    ...
    } else if ((argument == "--category" || argument == "-c") &&
               parameter.length() > 0) {
      ...
      ++index;
    } else if ((argument == "--exclude-category" || argument == "-E") &&
               parameter.length() > 0) {
      excludeCategories.push_back(parameter);
      ++index;
    } else if ((argument == "--exclude-test" || argument == "-e") &&
               parameter.length() > 0) {
      excludeTests.push_back(parameter);
      ++index;
    } else if ((argument == "--expected-failure-category" ||
                argument == "-F") &&
               parameter.length() > 0) {
      expectedFailureCategories.push_back(parameter);
      ++index;
    } else if ((argument == "--expected-failure-test" || argument == "-f") &&
               parameter.length() > 0) {
  ...
  for (TestCategory* category : tests) {
    if (runCategories.size() > 0 &&
        std::find(runCategories.begin(), runCategories.end(),
                  category->getCategoryName()) == runCategories.end()) {
      continue;
    }

    if (excludeCategories.size() > 0 &&
        std::find(excludeCategories.begin(), excludeCategories.end(),
                  category->getCategoryName()) != excludeCategories.end()) {
    ...
    for (TestCategory::TestCategory_test test : category->getTests()) {
      if (runTests.size() > 0 &&
          std::find(runTests.begin(), runTests.end(), std::get<2>(test)) ==
              runTests.end()) {
        continue;
      }

      if (excludeTests.size() > 0 &&
          std::find(excludeTests.begin(), excludeTests.end(),
                    std::get<2>(test)) != excludeTests.end()) {
        ++skipCount;
        continue;
      }
      ...
      xml << "\t<testcase classname=\"" << category->getCategoryName() << " - "
          << OS << "\" name=\"" << std::get<2>(test) << "\"";
      ...
      variable.notify_one();
      while (!runner->done &&
             variable.wait_for(lock, std::chrono::seconds(1)) ==
      ...
      if (timeElapsed > timeout) {
        ...
        failedTests.push_back(
            {category->getCategoryName() + "::" + std::get<2>(test),
             "Test timed out."});

        xml << " >" << std::endl
            << "\t\t"
            << "<failure type=\"Timeout\">" << runner->output.str()
            << "</failure>" << std::endl
            << "\t</testcase>" << std::endl;
        ...
        xml << " />" << std::endl;
                 ...
                 (std::find(expectedFailureCategories.begin(),
                            expectedFailureCategories.end(),
                            category->getCategoryName()) !=
                      expectedFailureCategories.end() ||
                  std::find(expectedFailureTests.begin(),
                            expectedFailureTests.end(),
                            std::get<2>(test)) != expectedFailureTests.end())) {
        ...
        // runner->failureMessage << "</failure>" << std::endl
        // << "\t</testcase>" << std::endl;
      } else if (runner->failed) {
        ...
        failedTests.push_back(
            {category->getCategoryName() + "::" + std::get<2>(test),
             runner->failureMessage + "\n\n" + runner->output.str()});
        unexpectedFailure = true;

        xml << " >" << std::endl
            << "\t\t"
            << "<failure type=\"Test Failure\">" << runner->failureMessage
            << "</failure>" << std::endl
            << "\t</testcase>" << std::endl;
      }
  ...
  std::cout << "================================================" << std::endl
            << std::endl;
  std::cout << "Completed " << successCount << "/" << testCount
            << " successfully. "
            << testCount - successCount - expectedFailureCount
            << " failed unexpectedly. " << expectedFailureCount
            << " failed as expected. " << skipCount << " tests skipped."
            << std::endl;
  std::cout.flush();
