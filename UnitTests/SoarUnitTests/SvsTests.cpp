...
void SvsTests::testEnableFromTopState() {
  ...
  assertTrue_msg("Failed to enable SVS.", agent->GetLastCommandLineResult());
...
void SvsTests::testCannotDisableInSubstate() {
  ...
  assertFalse_msg(
      "Expected not to be able to disable SVS from a substate; return was: " +
          result + " (" +
          (agent->GetLastCommandLineResult() ? "true" : "false") + ")",
      agent->GetLastCommandLineResult());

  result = agent->ExecuteCommandLine("svs --disable-in-substates");
  assertFalse_msg(
      "Expected not to be able to disable SVS in substates from a substate; "
      "return was: " +
          result,
      agent->GetLastCommandLineResult());
}
...
void SvsTests::testEnableFromSubstate() {
  ...
  assertTrue_msg("Failed to disable SVS.", agent->GetLastCommandLineResult());
...
void SvsTests::testEnableInSubstatesFromSubstate() {
  // disable-in-substates, step and then enable, check that ^svs exists only on
  // top state
