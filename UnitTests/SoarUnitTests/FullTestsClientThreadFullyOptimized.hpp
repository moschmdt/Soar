#ifndef FullTests_USE_CLIENT_THREAD_FULLY_OPTIMIZED
#define FullTests_USE_CLIENT_THREAD_FULLY_OPTIMIZED

#include "FullTests.hpp"

class FullTestsClientThreadFullyOptimized : public FullTests_Parent,
                                            public TestCategory {
 public:
  TEST_CATEGORY(FullTestsClientThreadFullyOptimized);
  ...
  void testClientMessageHandler() {
    this->FullTests_Parent::testClientMessageHandler();
  }
  ...
  void testSimpleReteNetLoader() {
    this->FullTests_Parent::testSimpleReteNetLoader();
  }
  ...
  void testRunningAgentCreation() {
    this->FullTests_Parent::testRunningAgentCreation();
  }
  ...
  void testStatusCompleteDuplication() {
    this->FullTests_Parent::testStatusCompleteDuplication();
  }

  TEST(testStopSoarVsInterrupt, -1);
  void testStopSoarVsInterrupt() {
    this->FullTests_Parent::testStopSoarVsInterrupt();
  }

  TEST(testSharedWmeSetViolation, -1);
  void testSharedWmeSetViolation() {
    this->FullTests_Parent::testSharedWmeSetViolation();
  }
  ...
  void testTemplateVariableNameBug() {
    this->FullTests_Parent::testTemplateVariableNameBug();
  }

  TEST(testNegatedConjunctiveChunkLoopBug510, -1);
  void testNegatedConjunctiveChunkLoopBug510() {
    this->FullTests_Parent::testNegatedConjunctiveChunkLoopBug510();
  }
  ...
  void testConvertIdentifier() {
    this->FullTests_Parent::testConvertIdentifier();
  }

  TEST(testOutputLinkRemovalOrdering, -1);
  void testOutputLinkRemovalOrdering() {
    this->FullTests_Parent::testOutputLinkRemovalOrdering();
  }
