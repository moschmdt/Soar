...
class ChunkingTests : public FunctionalTestHarness {
 public:
  ...
  TEST(BUNCPS_1, -1);  // (most came from problems found testing on Kirk's game
                       // learning agents)
  TEST(BUNCPS_2, -1);
  TEST(BUNCPS_3, -1);
  TEST(BUNCPS_4, -1);
 ...
 private:
  void check_chunk(const char* pTestName, int64_t decisions,
                   int64_t expected_chunks, bool directSourceChunks = false);
  void verify_chunk(const char* pTestName, int64_t expected_chunks,
                    bool directSourceChunks = false);
