...
void Handlers::MyBoolShutdownHandler(sml::smlSystemEventId, void* pUserData,
                                     sml::Kernel*) {
...
void Handlers::MyEventShutdownHandler(sml::smlSystemEventId, void* pUserData,
                                      sml::Kernel*) {
  no_agent_assertTrue(pUserData);
...
void Handlers::MyShutdownTestShutdownHandler(sml::smlSystemEventId,
                                             void* pUserData,
                                             sml::Kernel* pKernel) {
  no_agent_assertTrue(pUserData);
...
void Handlers::MyDeletionHandler(sml::smlAgentEventId, void* pUserData,
                                 sml::Agent*) {
  no_agent_assertTrue(pUserData);
  bool* pHandlerReceived = static_cast<bool*>(pUserData);
  *pHandlerReceived = true;
...
void Handlers::MySystemEventHandler(sml::smlSystemEventId, void*,
                                    sml::Kernel*) {
  // BUGBUG see comments above registration line
}

void Handlers::MyCreationHandler(sml::smlAgentEventId, void* pUserData,
                                 sml::Agent*) {
  no_agent_assertTrue(pUserData);
  bool* pHandlerReceived = static_cast<bool*>(pUserData);
  *pHandlerReceived = true;
...
void Handlers::MyProductionHandler(sml::smlProductionEventId id,
                                   void* pUserData, sml::Agent*, char const*,
                                   char const*) {
  no_agent_assertTrue(pUserData);
...
std::string Handlers::MyClientMessageHandler(sml::smlRhsEventId,
                                             void* pUserData, sml::Agent*,
                                             char const*,
                                             char const* pMessage) {
...
const sml::ClientMessageHandlerCpp Handlers::GetClientMessageHandlerCpp(
    bool* receivedFlag) {
  return
      [receivedFlag](sml::smlRhsEventId, sml::Agent*, char const* pMessageType,
...
std::string Handlers::MyFilterHandler(sml::smlRhsEventId, void* pUserData,
                                      sml::Agent*, char const*,
                                      char const* pCommandLine) {
  soarxml::ElementXML* pXML =
      soarxml::ElementXML::ParseXMLFromString(pCommandLine);
  no_agent_assertTrue(pXML);
  no_agent_assertTrue(pXML->GetAttribute(sml::sml_Names::kFilterCommand));

  std::stringstream commandLine;
  commandLine << pXML->GetAttribute(sml::sml_Names::kFilterCommand)
              << " --depth 2";

  // Replace the command attribute in the XML
  no_agent_assertTrue(pXML->AddAttribute(sml::sml_Names::kFilterCommand,
                                         commandLine.str().c_str()));

  // Convert the XML back to a string and put it into a std::string ready to
  // return
...
void Handlers::MyUpdateEventHandler(sml::smlUpdateEventId, void* pUserData,
                                    sml::Kernel*, sml::smlRunFlags) {
  no_agent_assertTrue(pUserData);
  int* pInt = static_cast<int*>(pUserData);
...
void Handlers::MyRunSelfRemovingHandler(sml::smlRunEventId, void* pUserData,
                                        sml::Agent* pAgent, sml::smlPhase) {
  no_agent_assertTrue(pUserData);
...
std::string Handlers::MyStringEventHandler(sml::smlStringEventId id,
                                           void* pUserData, sml::Kernel*,
                                           char const*) {
  no_agent_assertTrue(pUserData);
  bool* pHandlerReceived = static_cast<bool*>(pUserData);
  *pHandlerReceived = true;
...
void Handlers::MyDuplicateRunEventHandler(sml::smlRunEventId, void* pUserData,
                                          sml::Agent*, sml::smlPhase) {
  no_agent_assertTrue(pUserData);
  int* pInt = static_cast<int*>(pUserData);
...
void Handlers::DebugPrintEventHandler(sml::smlPrintEventId, void* pUserData,
                                      sml::Agent*, char const* pMessage) {
  std::cout << pMessage;
  std::cout.flush();
}

void Handlers::MyPrintEventHandler(sml::smlPrintEventId, void* pUserData,
                                   sml::Agent*, char const* pMessage) {
...
void Handlers::MyXMLEventHandler(sml::smlXMLEventId, void* pUserData,
                                 sml::Agent*, sml::ClientXML* pXML) {
  // pXML should be some structured trace output.
  // Let's examine it a bit.
  // We'll start by turning it back into XML so we can look at it in the
  // debugger.
  ...
  // We're not allowed to keep pXML that we're passed, but we can copy it and
  // keep the copy. (The copy is very efficient, the underlying object is
  // ref-counted).
...
void Handlers::MyInterruptHandler(sml::smlRunEventId, void* pUserData,
                                  sml::Agent* pAgent, sml::smlPhase) {
...
std::string Handlers::MyRhsFunctionHandler(sml::smlRhsEventId, void* pUserData,
                                           sml::Agent*, char const*,
                                           char const* pArgument) {
  // This is optional because the callback needs to be around for the program to
  // function properly we only test for this specifically in one part of
  // clientsmltest
  if (pUserData) {
    bool* pHandlerReceived = static_cast<bool*>(pUserData);
    *pHandlerReceived = true;
...
const sml::RhsEventHandlerCpp Handlers::GetRhsFunctionHandlerCpp(
    bool* receivedFlag) {
  return
      [receivedFlag](sml::smlRhsEventId, sml::Agent*, char const* pFunctionName,
...
void Handlers::MyMemoryLeakUpdateHandlerDestroyChildren(
    sml::smlUpdateEventId id, void* pUserData, sml::Kernel* pKernel,
    sml::smlRunFlags runFlags) {
  MyMemoryLeakUpdateHandlerInternal(true, id, pUserData, pKernel, runFlags);
}

void Handlers::MyMemoryLeakUpdateHandler(sml::smlUpdateEventId id,
                                         void* pUserData, sml::Kernel* pKernel,
                                         sml::smlRunFlags runFlags) {
  MyMemoryLeakUpdateHandlerInternal(false, id, pUserData, pKernel, runFlags);
}

void Handlers::MyMemoryLeakUpdateHandlerInternal(bool destroyChildren,
                                                 sml::smlUpdateEventId id,
                                                 void* pUserData,
                                                 sml::Kernel* pKernel,
                                                 sml::smlRunFlags runFlags) {
  static int step(0);
  ...
  switch (step % 3) {
    ...
    case 1:
      if (destroyChildren) {
        // Destroying the children should be unnecessary but should not be
        // illegal
        ...
        // These three child leaks are not detected by looking at
        // GetIWMObjMapSize
...
void Handlers::MyCallStopOnUpdateEventHandler(sml::smlUpdateEventId, void*,
                                              sml::Kernel* pKernel,
                                              sml::smlRunFlags) {
  pKernel->StopAllAgents();
}

void Handlers::MyAgentCreationUpdateEventHandler(sml::smlUpdateEventId,
                                                 void* pUserData,
                                                 sml::Kernel* pKernel,
                                                 sml::smlRunFlags) {
  no_agent_assertTrue(pUserData);
...
void Handlers::MyOrderingPrintHandler(sml::smlPrintEventId /*id*/,
...
void Handlers::MyOrderingRunHandler(sml::smlRunEventId /*id*/, void* pUserData,
                                    sml::Agent* /*pAgent*/,
                                    sml::smlPhase /*phase*/) {
  no_agent_assertTrue(pUserData);
  int* pInt = static_cast<int*>(pUserData);
...
std::string Handlers::MyRhsFunctionFailureHandler(sml::smlRhsEventId, void*,
                                                  sml::Agent*, char const*,
                                                  char const* pArgument) {
...
std::string Handlers::MySuccessHandler(sml::smlRhsEventId id, void* pUserData,
                                       sml::Agent* pAgent,
