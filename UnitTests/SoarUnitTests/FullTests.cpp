...
void FullTests_Parent::tearDown(bool caught) { destroySoar(); }
...
void FullTests_Parent::createSoar() {
  ...
  if (m_Options.remote) {
    ...
    if (m_Options.useClientThread) {
      ...
      m_pKernel = sml::Kernel::CreateKernelInCurrentThread(
          optimized, sml::Kernel::kUseAnyPort);
    } else {
      m_pKernel =
  ...
  no_agent_assertTrue(m_pKernel != NULL);
  no_agent_assertTrue_msg(m_pKernel->GetLastErrorDescription(),
                          !m_pKernel->HadError());

  if (m_Options.verbose) {
    std::cout << "Soar kernel version " << m_pKernel->GetSoarKernelVersion()
              << std::endl;
  }
  if (m_Options.verbose) {
    std::cout << "SML version " << sml::sml_Names::kSMLVersionValue
              << std::endl;
  }

  no_agent_assertTrue(std::string(m_pKernel->GetSoarKernelVersion()) ==
                      std::string(sml::sml_Names::kSoarVersionValue));

  bool creationHandlerReceived(false);
  int agentCreationCallback = m_pKernel->RegisterForAgentEvent(
      sml::smlEVENT_AFTER_AGENT_CREATED, Handlers::MyCreationHandler,
      &creationHandlerReceived);

  // Report the number of agents (always 0 unless this is a remote connection to
  // a CLI or some such)
  ...
  if (SoarHelper::run_as_unit_test) {
    agent_struct* lAgent = Soar_Instance::Get_Soar_Instance()
                               .Get_Agent_Info(kAgentName.c_str())
                               ->GetSoarAgent();
    configure_agent_for_unit_tests(lAgent);
  }

  no_agent_assertTrue(
      m_pKernel->UnregisterForAgentEvent(agentCreationCallback));

  // a number of tests below depend on running full decision cycles.
  agent->ExecuteCommandLine("soar stop-phase input");
  no_agent_assertTrue_msg("soar stop-phase input",
...
void FullTests_Parent::destroySoar() {
  ...
  // The Before_Agent_Destroyed callback is a tricky one so we'll register for
  // it to test it. We need to get this callback just before the agentSML data
  // is deleted (otherwise there'll be no way to send/receive the callback) and
  // then continue on to delete the agent after we've responded to the callback.
  // Interestingly, we don't explicitly unregister this callback because the
  // agent has already been destroyed so that's another test, that this callback
  // is cleaned up correctly (and automatically).
  bool deletionHandlerReceived(false);
  m_pKernel->RegisterForAgentEvent(sml::smlEVENT_BEFORE_AGENT_DESTROYED,
                                   Handlers::MyDeletionHandler,
                                   &deletionHandlerReceived);
  ...
  if (m_Options.remote) {
    ...
    m_pKernel->RegisterForSystemEvent(sml::smlEVENT_BEFORE_SHUTDOWN,
                                      Handlers::MyEventShutdownHandler,
                                      &shutdownEvent);
    ...
    std::string shutdownResponse =
        m_pKernel->SendClientMessage(0, "test-listener", "shutdown");
    no_agent_assertTrue(shutdownResponse == "ok");

    no_agent_assertTrue_msg(
        "Listener side kernel shutdown failed to fire smlEVENT_BEFORE_SHUTDOWN",
        shutdownEvent.WaitForEvent(5, 0));

    // Note, in the remote case, this does not fire smlEVENT_BEFORE_SHUTDOWN
    // the listener side shutdown does trigger the event when it is deleted, see
    // simplelistener.cpp
    m_pKernel->Shutdown();
    ...
    m_pKernel->RegisterForSystemEvent(sml::smlEVENT_BEFORE_SHUTDOWN,
                                      Handlers::MyBoolShutdownHandler,
                                      &shutdownHandlerReceived);
    m_pKernel->Shutdown();
...
int FullTests_Parent::spawnListener() {
  ...
  int error =
      posix_spawn(&targetPid, executable.c_str(), NULL, NULL, argv, NULL);
...
void FullTests_Parent::cleanUpListener() {
  ...
  wait(&status);
  if (WIFEXITED(status)) {
    no_agent_assertTrue_msg("listener terminated with nonzero status",
...
void FullTests_Parent::testProductions() {
  ...
  int excisedCount(0);
  int prodCall = agent->RegisterForProductionEvent(
      sml::smlEVENT_BEFORE_PRODUCTION_REMOVED, Handlers::MyProductionHandler,
      &excisedCount);
...
void FullTests_Parent::testUngroundedLHS() {
  ...
  std::string result = agent->ExecuteCommandLine(
      "sp { ok*standard (state <s> ^superstate nil) -->}");
  ...
  result = agent->ExecuteCommandLine(
      "sp { ok*no*explicit*id*test (state ^superstate nil) -->}");
  no_agent_assertTrue(agent->GetLastCommandLineResult());

  // Wildcard matches are also fine
  result =
      agent->ExecuteCommandLine("sp { ok*wildcard (state ^<any1> <any2>) -->}");
  no_agent_assertTrue(agent->GetLastCommandLineResult());

  // Matches on just <s> will add the attr-val tests automatically
  result = agent->ExecuteCommandLine(
      "sp { ok*lone*s (state <s> ^superstate nil) (<s>) -->}");
  no_agent_assertTrue(agent->GetLastCommandLineResult());

  // TODO: this warns but should also fail, as there's no state test
  result = agent->ExecuteCommandLine(
      "sp {warns*no*state*test (<s> ^results <any>)-->}");
  no_agent_assertTrue(agent->GetLastCommandLineResult());
  const char* expected_message =
      "Warning: On the LHS of production warns*no*state*test, identifier <s> "
      "is not connected to any goal or impasse.";
  no_agent_assertTrue_msg(
      "Expected1 warning message not found in '" + result + "'",
      result.find(expected_message) != std::string::npos);

  // at least one attr/val test is required with state test
  result =
      agent->ExecuteCommandLine("sp { fails*no*attr*val*test (state <s>) -->}");
  no_agent_assertFalse(agent->GetLastCommandLineResult());
  expected_message =
      "Error: Expected attribute-value test after state/impasse test. Did you "
      "forget to add \"^type state\" or \"^superstate nil\"?";
  no_agent_assertTrue_msg(
      "Expected2 error message not found in '" + result + "'",
      result.find(expected_message) != std::string::npos);

  // We require the attr/val test to be in the same condition as the state test
  result = agent->ExecuteCommandLine(
      "sp { fails*missing*attr*val*test (state <s>) (<s> ^superstate nil) "
      "-->}");
  no_agent_assertFalse(agent->GetLastCommandLineResult());
  expected_message =
      "Error: Expected attribute-value test after state/impasse test. Did you "
      "forget to add \"^type state\" or \"^superstate nil\"?";
  no_agent_assertTrue_msg(
      "Expected3 error message not found in '" + result + "'",
      result.find(expected_message) != std::string::npos);

  // negative conditions do not serve to ground the state
  result = agent->ExecuteCommandLine(
      "sp { fails*negative*doesnt*ground (state -^result <any>) -->}");
  no_agent_assertFalse(agent->GetLastCommandLineResult());
  expected_message =
      "Error: production fails*negative*doesnt*ground has no positive "
      "conditions that reference a goal state.\nDid you forget to add \"^type "
      "state\" or \"^superstate nil\"?";
  no_agent_assertTrue_msg(
      "Expected4 error message not found in '" + result + "'",
      result.find(expected_message) != std::string::npos);
}
...
void FullTests_Parent::testRHSHandler() {
  ...
  int callback_rhs1 = m_pKernel->AddRhsFunction(
      "test-rhs", Handlers::MyRhsFunctionHandler, &rhsFunctionHandlerReceived);
  int callback_rhs_dup = m_pKernel->AddRhsFunction(
      "test-rhs", Handlers::MyRhsFunctionHandler, &rhsFunctionHandlerReceived);
  // agent->RegisterForPrintEvent( sml::smlEVENT_PRINT,
  // Handlers::DebugPrintEventHandler, 0) ;
  no_agent_assertTrue_msg(
      "Duplicate RHS function registration should be detected and be ignored",
      callback_rhs_dup == callback_rhs1);

  bool cppRhsHandlerReceived(false);
  int callback_rhs_cpp = m_pKernel->AddRhsFunction(
      "test-rhs-cpp",
      Handlers::GetRhsFunctionHandlerCpp(&cppRhsHandlerReceived));
  ...
  no_agent_assertTrue(
      m_pKernel->AddRhsFunction("test-rhs", Handlers::MyRhsFunctionHandler, 0));
...
void FullTests_Parent::testClientMessageHandler() {
  ...
  int clientCallback = m_pKernel->RegisterForClientMessageEvent(
      "test-client", Handlers::MyClientMessageHandler, &clientHandlerReceived);

  bool clientHandlerReceivedCpp(false);
  int clientCallbackCpp = m_pKernel->RegisterForClientMessageEvent(
      "test-client-cpp",
      Handlers::GetClientMessageHandlerCpp(&clientHandlerReceivedCpp));

  // This is a bit dopey--but we'll send a message to ourselves for this test
  std::string message("foo-bar-baz-qux");
  std::string response =
      m_pKernel->SendClientMessage(agent, "test-client", message.c_str());
  ...
  response =
      m_pKernel->SendClientMessage(agent, "test-client-cpp", message.c_str());
  no_agent_assertTrue(clientHandlerReceivedCpp);
  expected = "handler-message-cpp" + message;
  no_agent_assertTrue(response == expected);

  no_agent_assertTrue(
      m_pKernel->UnregisterForClientMessageEvent(clientCallback));
  no_agent_assertTrue(
      m_pKernel->UnregisterForClientMessageEvent(clientCallbackCpp));
}

void FullTests_Parent::testFilterHandler() {
  // Record a filter
  bool filterHandlerReceived(false);
  int clientFilter = m_pKernel->RegisterForClientMessageEvent(
      sml::sml_Names::kFilterName, Handlers::MyFilterHandler,
      &filterHandlerReceived);
...
void FullTests_Parent::testWMEs() {
  ...
  // This is to test a bug where an identifier isn't fully removed from working
  // memory (you can still print it) after it is destroyed.
  ...
  no_agent_assertTrue_msg("print i2 --depth 3",
                          agent->GetLastCommandLineResult());

  no_agent_assertTrue(agent->ExecuteCommandLine(
      "print F1"));  // BUGBUG: This wme remains in memory even after we add the
                     // "RunSelf" at which point it should be gone.
  ...
  agent->Update(
      pWME3, 50.5);  // Should not change the wme, so timetag should be the same

  int64_t timeTag2 = pWME3->GetTimeTag();
  agent->SetBlinkIfNoChange(true);  // Back to the default
  agent->Update(pWME3,
                50.5);  // Should change the wme, so timetag should be different

  int64_t timeTag3 = pWME3->GetTimeTag();

  no_agent_assertTrue_msg("Error in handling of SetBlinkIfNoChange flag",
                          timeTag1 == timeTag2);
  no_agent_assertTrue_msg("Error in handling of SetBlinkIfNoChange flag",
                          timeTag2 != timeTag3);
  ...
  // BUGBUG: This is triggering an no_agent_assert and memory leak now after the
  // changes to InputWME not calling Update() immediately.  For now I've removed
  // the test until we have time to figure out what's going wrong.
  ...
  /*
     ...
     ; std::cout << printInput1 << std::endl ; std::cout << std::endl << "Now
     work with the input link" << std::endl ;
   */
  ...
  no_agent_assertTrue_msg("print -i (s1 ^* *)",
                          agent->GetLastCommandLineResult());
...
void FullTests_Parent::testXML() {
  ...
  no_agent_assertTrue(m_pKernel->ExecuteCommandLineXML(
      "print -i --depth 3 s1", agent->GetAgentName(), &xml2));
...
void FullTests_Parent::testAgent() {
  ...
  // We'll pass in an "int" and use it to count decisions (just as an example of
  // passing user data around)
  int count(0);
  int callback1 = agent->RegisterForRunEvent(
      sml::smlEVENT_AFTER_DECISION_CYCLE, Handlers::MyRunEventHandler, &count);
  int callback_dup = agent->RegisterForRunEvent(
      sml::smlEVENT_AFTER_DECISION_CYCLE, Handlers::MyRunEventHandler, &count);

  no_agent_assertTrue_msg(
      "Duplicate handler registration should be detected and be ignored",
      callback1 == callback_dup);

  // This callback unregisters itself in the callback -- as a test to see if we
  // can do that safely.
  int selfRemovingCallback(-1);
  selfRemovingCallback = agent->RegisterForRunEvent(
      sml::smlEVENT_AFTER_DECISION_CYCLE, Handlers::MyRunSelfRemovingHandler,
      &selfRemovingCallback);

  // Register for a String event
  //	bool stringEventHandlerReceived(false);
  //	int stringCall =
  // m_pKernel->RegisterForStringEvent(sml::smlEVENT_LOAD_LIBRARY,
  // Handlers::MyStringEventHandler, &stringEventHandlerReceived) ;
  //	no_agent_assertTrue(m_pKernel->ExecuteCommandLine("load-library
  // TestExternalLibraryLib", NULL)); 	no_agent_assertTrue_msg("echo hello
  // world", agent->GetLastCommandLineResult());
  ...
  // Register this one ahead of the previous handler (so it will fire before
  // MyRunEventHandler)
  bool addToBack = true;
  int testData(25);
  int callback2 = agent->RegisterForRunEvent(
      sml::smlEVENT_AFTER_DECISION_CYCLE, Handlers::MyDuplicateRunEventHandler,
      &testData, !addToBack);

  // Run returns the result (succeeded, failed etc.)
  // To catch the trace output we have to register a print event listener
  std::stringstream
      trace;  // We'll pass this into the handler and build up the output in it
  std::string structured;  // Structured trace goes here
  int callbackp = agent->RegisterForPrintEvent(
      sml::smlEVENT_PRINT, Handlers::MyPrintEventHandler, &trace);

  sml::ClientXML* clientXMLStorage = 0;
  int callbackx = agent->RegisterForXMLEvent(sml::smlEVENT_XML_TRACE_OUTPUT,
                                             Handlers::MyXMLEventHandler,
                                             &clientXMLStorage);

  int beforeCount(0);
  int afterCount(0);
  int callback_before =
      agent->RegisterForRunEvent(sml::smlEVENT_BEFORE_RUN_STARTS,
                                 Handlers::MyRunEventHandler, &beforeCount);
  int callback_after = agent->RegisterForRunEvent(
      sml::smlEVENT_AFTER_RUN_ENDS, Handlers::MyRunEventHandler, &afterCount);

  // Some temp code to generate more complex watch traces.  Not usually part of
  // the test
  /*
  ...
  int outputPhases(0);
  int callback_u = m_pKernel->RegisterForUpdateEvent(
      sml::smlEVENT_AFTER_ALL_OUTPUT_PHASES, Handlers::MyUpdateEventHandler,
      &outputPhases);

  int phaseCount(0);
  int callbackPhase =
      agent->RegisterForRunEvent(sml::smlEVENT_BEFORE_PHASE_EXECUTED,
                                 Handlers::MyRunEventHandler, &phaseCount);
  ...
  /* Not true now we support stopping before/after phases when running by
     decision. if (phaseCount != 20)
       {
  ...
  // By this point the static variable ClientXMLStorage should have been filled
  // in and it should be valid, even though the event handler for
  // MyXMLEventHandler has completed.
  no_agent_assertTrue_msg("Error receiving XML trace events",
  ...
  int myCount(0);
  int callback_run_count =
      agent->RegisterForRunEvent(sml::smlEVENT_AFTER_DECISION_CYCLE,
                                 Handlers::MyRunEventHandler, &myCount);

  int outputsGenerated(0);
  int callback_g = m_pKernel->RegisterForUpdateEvent(
      sml::smlEVENT_AFTER_ALL_GENERATED_OUTPUT, Handlers::MyUpdateEventHandler,
      &outputsGenerated);

  int outputNotifications(0);
  int callback_notify = agent->RegisterForOutputNotification(
      Handlers::MyOutputNotificationHandler, &outputNotifications);

  // Can't test this at the same time as testing the getCommand() methods as
  // registering for this clears the output link information
  // int outputHandler = agent->AddOutputHandler("move", MyOutputEventHandler,
  // NULL) ;
  ...
  int callbackp1 = agent->RegisterForPrintEvent(
      sml::smlEVENT_PRINT, Handlers::MyPrintEventHandler, &trace);

  // Now we should match (if we really loaded the tictactoe example rules) and
  // so generate some real output We'll use RunAll just to test it out.  Could
  // use RunSelf and get same result (presumably)
  m_pKernel->RunAllTilOutput();  // Should just cause Soar to run a decision or
                                 // two (this is a test that run til output
                                 // works stops at output)
  no_agent_assertTrue_msg("RunAllTilOutput", agent->GetLastCommandLineResult());

  // We should stop quickly (after a decision or two)
  no_agent_assertTrue_msg(
      "Error in RunTilOutput -- it didn't stop on the output", myCount <= 10);
  no_agent_assertTrue_msg(
      "Error in callback handler for MyRunEventHandler -- failed to update "
      "count",
      myCount > 0);

  if (m_Options.verbose) {
    std::cout << "Agent ran for " << myCount
              << " decisions before we got output" << std::endl;
  }
  if (m_Options.verbose) {
    std::cout << trace.str() << std::endl;
  }
  ...
  no_agent_assertTrue_msg("Error in AFTER_ALL_GENERATED event.",
                          outputsGenerated == 1);

  no_agent_assertTrue_msg("Error in OUTPUT_NOTIFICATION event.",
  ...
  no_agent_assertTrue_msg("RunSelfTilOutput",
                          agent->GetLastCommandLineResult());
  ...
  sml::Identifier* pMove = static_cast<sml::Identifier*>(
      agent->GetOutputLink()->FindByAttribute("move", 0));
  no_agent_assertTrue(pMove);

  // Try to find an attribute that's missing to make sure we get null back
  sml::Identifier* pMissing = static_cast<sml::Identifier*>(
      agent->GetOutputLink()->FindByAttribute("not-there", 0));
  no_agent_assertTrue(!pMissing);

  sml::Identifier* pMissingInput = static_cast<sml::Identifier*>(
      agent->GetInputLink()->FindByAttribute("not-there", 0));
  no_agent_assertTrue(!pMissingInput);

  // We add an "alternative" to check that we handle shared WMEs correctly.
  // Look it up here.
  sml::Identifier* pAlt = static_cast<sml::Identifier*>(
      agent->GetOutputLink()->FindByAttribute("alternative", 0));
  no_agent_assertTrue(pAlt);

  // Should also be able to get the command through the "GetCommands" route
  // which tests whether we've flagged the right wmes as "just added" or not.
  ...
  no_agent_assertTrue(
      std::string(pCommand1->GetCommandName()) == "alternative" ||
  ...
  int callback3 = agent->RegisterForRunEvent(sml::smlEVENT_AFTER_DECISION_CYCLE,
                                             Handlers::MyInterruptHandler,
                                             &interruptHandlerReceived);
  ...
  /*
       ...
       {
       std::cout << "*** ERROR: Failed to interrupt Soar during a run." <<
  ...
  /* These comments haven't kept up with the test -- does a lot more now
       std::cout << std::endl << "If this test worked should see something like
     this (above here):" << std::endl ; std::cout << "Top Identifier I3" <<
     std::endl << "(I3 ^move M1)" << std::endl << "(M1 ^row 1)" << std::endl ;
       std::cout << "(M1 ^col 1)" << std::endl << "(I3 ^alternative M1)" <<
     std::endl ; std::cout << "And then after the command is marked as completed
     (during the test):" << std::endl ; std::cout << "Top Identifier I3" <<
     std::endl ; std::cout << "Together with about 6 received events" <<
     std::endl ;
   */
...
void FullTests_Parent::testSimpleCopy() {
  ...
  // std::cout << agent->ExecuteCommandLine("print --depth 5 s1 --tree") <<
  // std::endl;
  ...
  while (iter != pOutputLink->GetChildrenEnd()) {
    ...
    no_agent_assertTrue(std::string(pTextOutputWME->GetAttribute()) ==
                        "text-output");
    ++iter;
  }
  ...
  sml::Identifier::ChildrenIter textOutputIter =
      pTextOutput->GetChildrenBegin();
  int count = 0;
  ...
  //  std::cout << pOutputWme->GetIdentifier()->GetIdentifierSymbol() << " ^ "
  //  << pOutputWme->GetAttribute() << " " << pOutputWme->GetValueAsString() <<
  //  std::endl ;
  //}

  // We had a bug where some of these wmes would get dropped (the orphaned wme
  // scheme didn't handle multiple levels) so check now that we got the correct
  // number of changes.
  std::stringstream changesString;
  // changesString << "Number of changes: " << changes << ", this failure is
  // currently expected but needs to be addressed, see wiki gSKI removal page";
...
void FullTests_Parent::testSimpleReteNetLoader() {
  std::string result = agent->ExecuteCommandLine(
      ("rete-net -l \"" + SoarHelper::GetResource("test.soarx") + "\"")
          .c_str());
  no_agent_assertTrue(agent->GetLastCommandLineResult());
...
void FullTests_Parent::test64BitReteNet() {
  std::string result = agent->ExecuteCommandLine(
      ("rete-net -l \"" + SoarHelper::GetResource("test64.soarx") + "\"")
          .c_str());
  no_agent_assertTrue(agent->GetLastCommandLineResult());
...
void FullTests_Parent::testOSupportCopyDestroy() {
  ...
  m_pKernel->AddRhsFunction("bad-copy-exists", Handlers::MyRhsFunctionHandler,
                            &badCopyExists);
...
void FullTests_Parent::testRunningAgentCreation() {
  ...
  m_pKernel->RegisterForUpdateEvent(sml::smlEVENT_AFTER_ALL_OUTPUT_PHASES,
                                    Handlers::MyAgentCreationUpdateEventHandler,
                                    &data);
  m_pKernel->RunAllAgents(10);

  // std::cout << std::endl;
  // std::cout << "count: " << data.count;
  //  FIXME: in a perfect world, this is 10 not 12 but since the run isn't
  //  forever, the newly created agent runs 10.
  ...
  // std::cout << "original: " <<
  // response1.GetArgInt(sml::sml_Names::kParamStatsCycleCountDecision, -1);
  // std::cout << " " <<
  // response1.GetArgInt(sml::sml_Names::kParamStatsCycleCountElaboration, -1);
  // std::cout << " " <<
  // response1.GetArgInt(sml::sml_Names::kParamStatsCycleCountInnerElaboration,
  // -1); std::cout << " onthefly: " <<
  // response2.GetArgInt(sml::sml_Names::kParamStatsCycleCountDecision, -1);
  // std::cout << " " <<
  // response2.GetArgInt(sml::sml_Names::kParamStatsCycleCountElaboration, -1);
  // std::cout << " " <<
  // response2.GetArgInt(sml::sml_Names::kParamStatsCycleCountInnerElaboration,
  // -1) << std::endl;
  no_agent_assertTrue(
      response1.GetArgInt(sml::sml_Names::kParamStatsCycleCountDecision, -1) ==
      10);
...
void FullTests_Parent::testEventOrdering() {
  ...
  agent->RegisterForPrintEvent(sml::smlEVENT_PRINT,
                               Handlers::MyOrderingPrintHandler, &count);
  agent->RegisterForRunEvent(sml::smlEVENT_AFTER_OUTPUT_PHASE,
                             Handlers::MyOrderingRunHandler, &count);
...
void FullTests_Parent::testStopSoarVsInterrupt() {
  ...
  // expected: agent stops after 15 decision cycles
  {
    ...
    no_agent_assertTrue(
        response.GetArgInt(sml::sml_Names::kParamStatsCycleCountDecision, -1) ==
        15);
  }
  ...
  // expected: agent stops after 1 elaboration
  {
    ...
    no_agent_assertTrue(
        ...
        0);
    no_agent_assertTrue(
        response.GetArgInt(sml::sml_Names::kParamStatsCycleCountElaboration,
                           -1) == 1);
  }
...
void FullTests_Parent::testSharedWmeSetViolation() {
  ...
  // TODO: This is illegal, but is probably too expensive to test for in
  // release. See bug 1060
  sml::Identifier* pFoo2 =
      agent->GetInputLink()->CreateSharedIdWME("foo", pFoo1);
  no_agent_assertTrue_msg("CreateSharedIdWME was able to create duplicate wme",
                          pFoo2 == 0);
...
void FullTests_Parent::testFindAttrPipes() {
  sml::ClientAnalyzedXML response;
  no_agent_assertTrue(
      agent->ExecuteCommandLineXML("add-wme I2 ^A a", &response));
  no_agent_assertTrue(agent->SynchronizeInputLink());
...
void FullTests_Parent::testNegatedConjunctiveChunkLoopBug510() {
  loadProductions(
      SoarHelper::GetResource("testNegatedConjunctiveChunkLoopBug510.soar"));
  agent->ExecuteCommandLine("run");
  sml::ClientAnalyzedXML response;
  agent->ExecuteCommandLineXML("stats", &response);
  no_agent_assertTrue(
...
void FullTests_Parent::testNegatedConjunctiveTestReorder() {
  agent->ExecuteCommandLine(
      "sp {test (state <s> ^a <val> -^a {<val> < 1}) --> }");
  std::string production(agent->ExecuteCommandLine("print test"));
  no_agent_assertTrue(production ==
...
void FullTests_Parent::testNegatedConjunctiveTestUnbound() {
  ...
  agent->ExecuteCommandLine(
      "sp {test (state <s> ^superstate <d>) -{(<s> ^bar <b>) (<s> -^bar { <> "
      "<d>})} --> }");
  // <d> is bound outside of ncc but in scope
  no_agent_assertTrue(agent->GetLastCommandLineResult());

  agent->ExecuteCommandLine(
      "sp {test (state <s> ^superstate nil) -{(<s> ^bar <d>) (<s> -^bar { <> "
      "<d>})} --> }");
...
// Maintainer note: this test fails if the WORKING_DIRECTORY env var is not set,
// and the current working directory is not the same as the one that the
// UnitTests exe is in. If you noticed it failing, try `cd out` and run
// UnitTests again.
void FullTests_Parent::testCommandToFile() {
  ...
  if (workingDirectory) resourceDirectory = workingDirectory;

  agent->ExecuteCommandLine(
      ("command-to-file \"" + resourceDirectory +
       "testCommandToFile-output.soar\" print --rl --full")
          .c_str());
  no_agent_assertTrue(agent->GetLastCommandLineResult());
  const char* result =
      agent->ExecuteCommandLine(("source \"" + resourceDirectory + "/" +
  ...
  remove(("\"" + resourceDirectory + "/" + "testCommandToFile-output.soar\"")
             .c_str());
  SoarHelper::init_check_to_find_refcount_leaks(agent);
}
...
void FullTests_Parent::testConvertIdentifier() {
  agent->ExecuteCommandLine(
      "sp {test (state <s> ^io.output-link <out>) --> (<out> ^a b)}");
  agent->ExecuteCommandLine(
      "sp {fnord (state <s> ^io.output-link <out>) (<out> ^foo <f>) --> (<out> "
      "^bar <f>)}");
...
void FullTests_Parent::testOutputLinkRemovalOrdering() {
  ...
  // Creates an output link command O3 ^one.two 2 and then a shared wme with the
  // same child O3 ^three.two 2
  agent->ExecuteCommandLine(
      "sp {one (state <s> ^superstate nil -^phase) --> (<s> ^operator.name "
      "one) }");
  agent->ExecuteCommandLine(
      "sp {one-apply (state <s> ^operator.name one ^io.output-link <ol>) --> "
      "(<ol> ^one.two 2) (<s> ^phase 1) }");
  agent->ExecuteCommandLine(
      "sp {two (state <s> ^superstate nil ^phase 1) --> (<s> ^operator.name "
      "two) }");
  agent->ExecuteCommandLine(
      "sp {two-apply (state <s> ^operator.name two ^io.output-link <ol>) (<ol> "
      "^one <one>) --> (<ol> ^three <one>) (<s> ^phase 1 - 2) }");

  // Removes O3 ^one <one> leaving a parent with a higher timetag than it's
  // children, and then removes the other, then halts
  agent->ExecuteCommandLine(
      "sp {three (state <s> ^superstate nil ^phase 2) --> (<s> ^operator.name "
      "three) }");
  agent->ExecuteCommandLine(
      "sp {three-apply (state <s> ^operator.name three ^io.output-link <ol>) "
      "(<ol> ^one <one>) --> (<ol> ^one <one> -) (<s> ^phase 2 - 3) }");
  agent->ExecuteCommandLine(
      "sp {four (state <s> ^superstate nil ^phase 3) --> (<s> ^operator.name "
      "four) }");
  agent->ExecuteCommandLine(
      "sp {four-apply (state <s> ^operator.name four ^io.output-link <ol>) "
      "(<ol> ^<asdf> <one>) --> (<ol> ^<asdf> <one> -) (<s> ^phase 3 - 4) }");
  agent->ExecuteCommandLine(
      "sp {five (state <s> ^superstate nil ^phase 4) --> (<s> ^operator.name "
      "five) }");
  agent->ExecuteCommandLine(
      "sp {five-apply (state <s> ^operator.name five) --> (halt) }");
