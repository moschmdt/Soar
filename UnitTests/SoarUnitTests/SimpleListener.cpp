...
std::string listenerRhsFunctionHandler(sml::smlRhsEventId id, void* pUserData,
                                       sml::Agent* pAgent,
                                       char const* pFunctionName,
                                       char const* pArgument) {
  // Return the argument we are passed plus some of our text
  // cout << "Received rhs function call with argument: " << pArgument <<
  // std::endl ;
...
std::string SimpleListener::MyClientMessageHandler(sml::smlRhsEventId,
...
int SimpleListener::run() {
  ...
  sml::Kernel* pKernel =
      useCurrentThread
          ? sml::Kernel::CreateKernelInCurrentThread(false,
  ...
  pKernel->RegisterForClientMessageEvent(
      "test-listener", SimpleListener::MyClientMessageHandler, 0);
