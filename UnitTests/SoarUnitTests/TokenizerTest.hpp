...
struct CallData {
  CallData(const char* input) : input(input) {}

  CallData(const char* input, int expected, ...) : input(input) {
    va_list args;
...
class TokenizerTest : public TestCategory, public soar::tokenizer_callback {
 public:
  ...
  void tearDown(
      bool caught);  // Called after each function outlined by CPPUNIT_TEST
