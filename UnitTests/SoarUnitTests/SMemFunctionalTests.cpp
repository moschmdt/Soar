...
void SMemFunctionalTests::setUp() { FunctionalTestHarness::setUp(); }
...
void SMemFunctionalTests::testSimpleFloat() { runTest("testSimpleFloat", 5); }
...
void SMemFunctionalTests::testNegQuery() { runTest("testNegQuery", 248); }
...
void SMemFunctionalTests::testCueSelection() {
  ...
  assertTrue_msg("Incorrect cue selected",
                 bwc && (std::string(bwc->cue->attr->to_string()) == "name") &&
...
void SMemFunctionalTests::testSimpleNonCueBasedRetrieval_ActivationRecency() {
  ...
  assertTrue_msg(
      "testSimpleNonCueBasedRetrieval_ActivationRecency functional test did "
      "not halt",
      halted);

  result = agent->ExecuteCommandLine("print @1 -d 1");
  expected = "(@1 ^location @2 ^name foo [+1.000])\n";
  assertTrue_msg(std::string("Activation value ") + expected +
                     std::string(" != " + result),
                 result == expected);
...
void SMemFunctionalTests::
    testSimpleNonCueBasedRetrieval_ActivationRecency_WithoutActivateOnQuery() {
  runTestSetup(
      "testSimpleNonCueBasedRetrieval_ActivationRecency_"
      "WithoutActivateOnQuery");
...
void SMemFunctionalTests::testSimpleNonCueBasedRetrieval_ActivationFrequency() {
  ...
  assertTrue_msg(
      "testSimpleNonCueBasedRetrieval_ActivationFrequency functional test did "
      "not halt",
      halted);
...
void SMemFunctionalTests::
    testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Stable() {
  ...
  assertTrue_msg(
      "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Stable functional "
      "test did not halt",
      halted);
...
void SMemFunctionalTests::
    testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Naive() {
  runTestSetup("testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Naive");

  agent->RunSelf(6);

  assertTrue_msg(
      "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Naive functional "
      "test did not halt",
      halted);
...
void SMemFunctionalTests::
    testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Incremental() {
  runTestSetup(
      "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Incremental");
...
void SMemFunctionalTests::testSpreadingActivation_AlphabetAgentAllOn() {
  ...
  std::string dc_count(std::to_string(
      stats.GetArgInt(sml::sml_Names::kParamStatsCycleCountDecision, -1)));
  std::string msg;
  SoarHelper::close_log(agent);
  assertTrue_msg(msg.append("testSpreadingActivation_AlphabetAgentAllOn halted "
                            "too early. Letters were likely skipped. DC = ")
                     .append(dc_count)
                     .c_str(),
                 stats.GetArgInt(sml::sml_Names::kParamStatsCycleCountDecision,
                                 -1) == 1649);
  assertTrue_msg(msg.append("testSpreadingActivation_AlphabetAgentAllOn "
                            "functional test did not halt. DC = ")
                     .append(dc_count)
                     .c_str(),
                 halted);
}
...
void SMemFunctionalTests::testDbBackupAndLoadTests() {
  ...
  assertTrue_msg("smem --clear didn't empty long-term memory!",
                 result.length() == 0);
  ...
  assertTrue_msg(
      std::string("After loading backup db and running, agent did not halt."),
      halted);

  result = agent->ExecuteCommandLine("p @197 -d 1");
  expected = "(@197 ^complete true ^factor @48 @198 ^number 100 [+573.000])\n";
  assertTrue_msg("testFactorization: Test did not get the correct result!",
                 result == result);
...
void SMemFunctionalTests::testReadCSoarDB() {
  ...
  agent->ExecuteCommandLine(
      std::string("smem --set path \"" + db + "\"").c_str());
  agent->ExecuteCommandLine("smem --set database file");
  ...
  std::string expectedResult =
      "(@1 ^complete true ^factor @2 ^number 2 [+0.000])\n(@2 ^multiplicity 1 "
      "^value 2 [+0.000])\n(@3 ^complete true ^factor @4 ^number 3 "
      "[+0.000])\n(@4 ^multiplicity 1 ^value 3 [+0.000])\n(@5 ^complete true "
      "^factor @6 ^number 4 [+0.000])\n(@6 ^multiplicity 2 ^value 2 "
      "[+0.000])\n";
...
void SMemFunctionalTests::testISupport() { runTest("smem-i-support", 6); }
