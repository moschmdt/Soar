...
// #define NEVER_LEARN                     // Overrides learning settings in
// many unit tests #define ALWAYS_LEARN
...
class SoarHelper {
 public:
  ...
  enum class StopPhase { INPUT, PROPOSE, DECIDE, APPLY, OUTPUT };

  // StopPhase, before=true/false
  static std::tuple<StopPhase, bool> getStopPhase(sml::Agent* agent);
  static void setStopPhase(sml::Agent* agent, StopPhase phase,
                           bool before = true);
  ...
  static bool source(sml::Agent* agent, const std::string& pCategoryName,
