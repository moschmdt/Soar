...
public class Pane
{
    ...
    /** This window becomes the parent of the modules beneath it */
    ...
    /**
     * Some windows can be vertically or horizontally oriented. Others can just
     ...
     * @param tabAtTop
     *            If true, put the tab at the top (if not a single window)
     ********************************************************************************************/
    public Pane(Composite parent, boolean singleView, boolean tabAtTop)
    {
        ...
        if (singleView)
        {
            // We provide a border around the pane so we can find the edges to
            // drag
    ...
    // I'm including a String "noParent" parameter here to make sure folks don't
    ...
    public void setHorizontalOrientation(boolean state)
    {
        m_HorizontalOrientation = state;
    }
    ...
    public void setSingleView(boolean state)
    {
        m_SingleView = state;
    }
    ...
    public void setTabAtTop(boolean state)
    {
        m_TabAtTop = state;
    }
        ...
        else
        {
            if (m_Views.size() != 0)
                throw new IllegalStateException(
                        "Adding multiple views to a pane that was created to only hold one");
        }
    ...
    /**
     * Update the names on the tabs (if there are any) to reflect the current
     * view names
     */
    ...
    public JavaElementXML convertToXML(String tagName, boolean storeContent)
    {
        ...
        element
                .addAttribute("horiz", Boolean
                        .toString(m_HorizontalOrientation));
        element.addAttribute(kAttributeSingleView, Boolean
                .toString(m_SingleView));
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            JavaElementXML child = view.convertToXML(AbstractView.kTagView,
                    storeContent);
    ...
    /**
     * Look up the node in the XML tree that matches this pane. Need to have
     * called "convertToXML" for everything before calling this
     */
    ...
    /**
     * This load code creates the pane, because we need to know what type of
     * pane to create -- single or multiple
     **/
    public static Pane loadFromXML(MainFrame frame, Document doc,
            Composite parent, JavaElementXML element) throws Exception
    {
        boolean singleView = element
                .getAttributeBooleanThrows(kAttributeSingleView);
        boolean tabAtTop = element
                .getAttributeBooleanThrows(kAttributeTabAtTop);
    ...
    private void loadFromXMLInternal(MainFrame frame, Document doc,
            Composite parent, JavaElementXML element) throws Exception
    {
        m_Views.clear();

        // FIXUP: We used to use the term orientation (during development).
        // Changed to horiz which is more informative. Should remove this extra
        // test for the old value
        // once we're into release.
        String orientation = element.getAttribute("orientation");

        if (orientation == null)
        {
            m_HorizontalOrientation = element
                    .getAttributeBooleanThrows("horiz");
        }
        else
        {
        ...
        for (int i = 0; i < element.getNumberChildren(); i++)
        {
            ...
            AbstractView view = (AbstractView) child
                    .CreateObjectFromXMLDefaultConstructor();
