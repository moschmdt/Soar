...
import edu.umich.soar.debugger.modules.KeepCommandView;
...
public class MainWindow
{
    ...
    // around between frameworks and decouple the code from the particular
    // window system
    // we're using.
    ...
    // We'll store information inside the SWT windows to tell us how they fit
    // together
    // in our window hierarchy. This is mostly to support debugging.
    public final static String kWindowType = "WindowType";

    public final static String kTypeModule = "TypeModule"; // The top of a
                                                           // module itself

    public final static String kTypePane = "TypePane"; // A window that contains
                                                       // modules

    public final static String kTypePair = "TypePair"; // Contains one fixed
                                                       // size window and a
                                                       // variable size one
                                                       // (creating a single
                                                       // unit)

    public final static String kTypeSash = "TypeSash"; // Contains two variable
                                                       // sized windows joined
                                                       // with a sash control
    ...
    /**
     * Returns the first view that reports it can be a "prime" view (a trace
     * window usually)
     **/
    ...
    /**
     * Attaches the first and second pane together such that the second pane
     * doesn't resize -- e.g. for button bars
     */
    protected void attachTogether(Composite first, Composite second,
            String attachType)
    {
        if (first.getParent() != second.getParent())
            throw new IllegalStateException(
                    "To attach these two together they need to have the same parent");

        // Set the layout to be form layout (top's parent == bottom's parent so
        // could use either)
        Composite parent = first.getParent();
        parent.setLayout(new FormLayout());

        // Record how we are attaching these together (later we may offer other
        // methods)
        ...
        if (attachType.equals(kAttachBottomValue)
                || attachType.equals(kAttachTopValue))
        {
            ...
            if (attachType.equals(kAttachBottomValue))
            {
                ...
                // If we bind the button's top to the window it makes the top
                // window very small and the buttons very large
    ...
    /**
     * Create the windows for the default layout. We're keeping this as a
     * fallback, but now load the default layout from a file usually.
     */
    protected void createInternalDefaultLayout()
    {
        if (m_Window.getChildren().length != 0)
            throw new IllegalStateException(
                    "Need to start with a blank base window");

        // Horiz sash has 3 windows has vertSashLeft and vertSashRight as its
        // children
        ...
        // The button bar is a fixed size window so it is linked to the window
        // above
        // and moves as one with it. To make this happen we'll create this pair.
        // These panes contain a SWT Window and a module/view that provides
        // specific debugging content
        ...
        attachTogether(top.getWindow(), buttonPane.getWindow(),
                kAttachLeftValue);
        buttonPane.setHorizontalOrientation(false);
        attachTogether(rightBottom.getWindow(), buttonsRightBottom.getWindow(),
                kAttachTopValue);

        // Have to set the weights after we have added the panes, so that the
        ...
        buttons.addDefaultCommands(); // We have a default set of commands to
                                      // show on a button bar
    ...
    public void useDefaultLayout(String filename)
    {
        ...
        boolean ok = m_Frame.loadLayoutFile(
                defaultLayoutFile.getAbsolutePath(), false);
        ...
        if (!ok)
        {
            ...
            // Note: Must update the parent because we've changed its children
            // here.
            // Without this the new windows don't appear on screen at all.
            m_Window.getParent().layout(true);

            // We reset the agent focus (if it existed before).
            // This allows the new windows to all register for events with this
            // agent.
    ...
    public void loadFromXML(JavaElementXML root) throws Exception
    {
        ...
        String version = root
                .getAttributeThrows(JavaElementXML.kVersionAttribute);

        if (!version.equals("1.0"))
            throw new Exception("Layout file is from an unsupported version "
                    + version);
        ...
        // (The font is considered a user's preference currently not part of the
        // layout)
    ...
    {
        // Convert to XML but without storing all of the current text in the
        // trace etc.
    ...
    public JavaElementXML buildXMLForSashForm(int sashOrientation, int[] weights)
    {
        ...
        // Record the class and style of this composite so we can rebuild it
        // later
        element.addAttribute(JavaElementXML.kClassAttribute, SashForm.class
                .toString());

        // The sash form stores its orientation (the style we really need)
        // separately
    ...
    protected JavaElementXML convertSashFormToXML(SashForm sash,
    ...
    protected void addChildrenToXML(JavaElementXML element,
            Composite composite, boolean storeContent)
    {
    ...
    public JavaElementXML buildXMLforComposite(String attachType)
    {
        ...
        // Record the class and style of this composite so we can rebuild it
        // later
        element.addAttribute(JavaElementXML.kClassAttribute, Composite.class
                .toString());

        // int style = composite.getStyle() ;
        int style = 0; // Override this for now until I figure out why I get
                       // back such a different value
        element.addAttribute("style", Integer.toString(style));

        // All composites beneath the top currently have to have this key
        // so we'll know how they were attached together and can rebuild it
        // later
        if (attachType == null)
            throw new IllegalStateException(
                    "Composite window missing its required attach key.  When the layout was built did we connect up the windows without calling one of our attachX methods?");
    ...
    protected JavaElementXML convertCompositeToXML(Composite composite,
            boolean storeContent)
    {
    ...
    // Convert the layout to XML, optionally recording content (e.g. text in a
    // trace window).
    // If in doubt, you usually do NOT want the content--it could add enormously
    ...
    public JavaElementXML convertToXML(boolean storeContent)
    {
        ...
        // Add a version to the layout file so we can have later debuggers
        // handle earlier layout files
        // in a special manner if we wish.
        root.addAttribute(JavaElementXML.kVersionAttribute, "1.0");

        // The display consists of a list of panes (the modules that provide
        // functionality) together
        // with a collection of SashForms and Composite windows that are used to
        // lay them out.
        // We navigate down through the windows until we reach the panes,
        // recording information as we go.
        // Once we reach the pane we switch to logic that's module specific
        // about how to save that pane.
        addChildrenToXML(root, m_Window, storeContent);

        return root;
    }

    protected void loadChildrenFromXML(MainFrame frame, Document doc,
            Composite parent, JavaElementXML element) throws Exception
    {
        for (int i = 0; i < element.getNumberChildren(); i++)
        {
            ...
            if (tagName.equalsIgnoreCase("sash")
                    || tagName.equalsIgnoreCase("composite"))
                ...
                // Throw during development, but really we should just ignore
                // this
                // when reading XML (in case we add tags later and load this
                // into an earlier version)
                throw new Exception("Unknown tag " + tagName);
        }
    }

    protected void loadPaneFromXML(MainFrame frame, Document doc,
            Composite parent, JavaElementXML element) throws Exception
    {
    ...
    protected void loadSwtFromXML(MainFrame frame, Document doc,
            Composite parent, JavaElementXML element) throws Exception
    {
        String tag = element.getTagName();
        // String className =
        // element.getAttributeThrows(JavaElementXML.kClassAttribute) ;
        int style = element.getAttributeIntThrows("style");

        // We could rebuild a generic SWT object from the class using
        // reflection, but it's actually simpler (so far) to just
        ...
        if (tag.equalsIgnoreCase("sash"))
        {
            ...
            // Have to create all of the children *before* we can set the
            // weights
        ...
        else if (tag.equalsIgnoreCase("composite"))
        {
            ...
            if (children.length != 2)
                throw new Exception(
                        "Trying to attach two windows together but found "
                                + children.length + " instead");

            // The children must be composites -- either they are panes or other
            // composites forming a tree of these pairs
